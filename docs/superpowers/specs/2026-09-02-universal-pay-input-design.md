...
- `reset()` with `initialRequest` (payee) or `initialSession` (payer) calls `onExit()` instead of re-entering a phase this mount can no longer reach; without either prop, unchanged. It is what every Cancel button calls. The `failed` screen's Retry button is different: it must try again, not leave, so it calls a separate `retry()` that clears the flow state and then re-enters — `startRequest(initialRequest.sats)` for a payee with a pre-set request, `adoptSession(initialSession)` for a payer with a pre-scanned session, and the old `receive_amount` / `entry` phase when neither prop is set. The phase machine itself is untouched; `receive_amount` and the payer `entry` view stay for the no-prop path.

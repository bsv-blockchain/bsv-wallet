...
| Android TIGER 13 → iOS 15 Pro (payer Android, peripheral) — reversed role, first attempt, 2026-09-03 22:22 | 517 both sides | 45 ms | payer `central subscribed … at 1946 ms`; payee `subscribed ms=20575` | 2047 bytes, 4 chunks at mtu 517 | never confirmed | n/a | — | **fail**: `timed out waiting for peer` at 30 s. `payer: HELLO_B verified … indicating frame` logged, then nothing — no `frame indicated` success or failure line ever appears on Android, and iOS never sees a FRAME indication (`hello verified` is the last payee(scan) line before the 30 s idle-reaper rescans). Root cause below. |
| Android TIGER 13 → iOS 15 Pro (payer Android, peripheral) — reversed role, run first, **fix retest**, 2026-09-03 22:56 | 517 both sides | 45 ms | payer `central subscribed … at 2211 ms`; payee `subscribed ms=25038` | 1989 bytes, 4 chunks at mtu 517 (payee: `frame accepted bytes=1989`) | `frame indicated in 299 ms` | `ack written ok=1`; `ack sent ok=1 bytes=44 via scan link` | `ack verified; total 2746 ms` | **pass** — first successful reversed-role hardware run, after the chunk-size fix below. |
...
| iOS 15 Pro → Android TIGER 13 (payer iOS, payee Android) — standard-direction regression, 2026-09-03 22:57 | 517 both sides | 69 ms | Android `subscribed to ACK` (no relative ms logged; ~1.4 s after connect from wall-clock) | 2453 bytes (Android: `frame accepted (2453 bytes, mtu 517) … at 7772 ms`); iOS wrote 2458 bytes | iOS `frame written bytes=2458 ms=60` | iOS `ack verified bytes=11 ms=2508`; Android `ack ok=true delivered=false … in 234 ms` | 2508 ms (iOS side) | **pass** — standard (non-reversed) direction still works, run immediately after the reversed-role fix retest as a regression check. Note: Android logged `delivered=false` on its own ack-indication confirmation (central had already disconnected by the time that line was logged) even though `ok=true` — money-safety unaffected (JS's `ok=true` already reflects the payee's own durable write) and this path predates the reversed-role work entirely, but worth a closer look separately if it recurs. |
...
- MTU negotiation against iOS: confirmed 517 on both the reversed-role and standard-direction 2026-09-03 22:56/22:57 runs.
- Anything that deviated from spec §3 "Expected performance" (0.5–2.5 s connect, 1–3 s end to end for 3–8 KB): the reversed-role run landed at 2746 ms total (`ack verified`) for a 1989-byte frame — inside the expected window. The standard-direction regression run landed at 2508 ms for a 2453-byte frame — also inside it.
- **Reversed-role FRAME indication silently failed on the first hardware attempt (2026-09-03 22:22), fixed and confirmed on retest (22:56).** Root cause: Android's native Bluetooth stack hard-rejects `notifyCharacteristicChanged`/indication payloads over 512 bytes regardless of the negotiated ATT MTU (`bt_stack: [ERROR:bta_gatts_api.cc(247)] BTA_GATTS_HandleValueIndication — data to indicate is too long`, logged at the instant Android tried to indicate the first of 4 chunks at `mtu-3=514` bytes with MTU=517). The app-level `sendIndication` callback never fired — no success or failure — so the payer just ran out its 30 s `wholeTimer`. This limit was never hit before the reversed role: in the standard direction the peripheral only ever indicates small payloads (HELLO_B's proof, the ACK json); the reversed role is the first time a peripheral has to indicate the multi-chunk FRAME itself. Fix: `BleGattProfile.chunkSize`/`chunks` on both platforms now clamp every chunk (not just indications, for one uniform defensive ceiling — see the amended 09-02 spec) at `min(ATT_MTU − 3, 512)` instead of the raw `ATT_MTU − 3`. Confirmed fixed by an immediate retest on the same hardware, same session.

## Status: reversed role confirmed working on hardware, 2026-09-03

Five rows above now carry real 2026-09-03 measurements: the two `Android TIGER 13 → iOS 15 Pro` direct-central attempts (link-layer desync, spec 2026-09-03, which motivated the reversed role), the reversed-role first attempt (FRAME-indication chunk-size failure) and its fix retest (pass), and the standard-direction regression check (pass). The `Android A → Android B` and screen-locked rows, and the negative-case table, are still `pending — requires devices`. The Kotlin backend compiles (`:react-native-localpay-transport:compileDebugKotlin`) and the fourteen `BleGattProfileTest` known-answer vectors pass on the plain JVM, which pins the wire format against `ios/BleGattProfile.swift`.

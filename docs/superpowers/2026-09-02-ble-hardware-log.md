...
| 6 | iOS↔iOS, both Bluetooth on, Wi-Fi on | Payee mints, payer scans | Presence shows the **wifi** glyph on both (AWDL outranks BLE, spec §5); payment completes over AWDL. | Payee (Console): `advertising started …` from the BLE object AND the AWDL listener's own start; after the frame lands over AWDL, `listener stopped by JS` (raceReceivers aborted the BLE loser) and NO `frame accepted` / `ack sent` from the BLE object. | pending — requires devices; not run in the implementation session |

...
namespace margelo::nitro::localpaytransport {
  ...
  /**
   * An abstract base class for `LocalPayBleTransport`
   * Inherit this class to create instances of `HybridLocalPayBleTransportSpec` in C++.
   * You must explicitly call `HybridObject`'s constructor yourself, because it is virtual.
   * @example
   * ```cpp
   * class HybridLocalPayBleTransport: public HybridLocalPayBleTransportSpec {
   * public:
   *   HybridLocalPayBleTransport(...): HybridObject(TAG) { ... }
   *   // ...
   * };
   * ```
   */
  class HybridLocalPayBleTransportSpec: public virtual HybridObject {
    public:
      // Constructor
      explicit HybridLocalPayBleTransportSpec(): HybridObject(TAG) { }

      // Destructor
      ~HybridLocalPayBleTransportSpec() override = default;
    ...
    public:
      ...
      virtual std::string bluetoothState() = 0;
      virtual bool nfcAvailable() = 0;
      virtual std::shared_ptr<Promise<std::string>> prepare(double timeoutMs) = 0;
    ...
    protected:
      // Tag for logging
      static constexpr auto TAG = "LocalPayBleTransport";
  };

import React, { createContext, useContext, useState, ReactNode } from 'react'
import i18n from 'i18next'
import { initReactI18next } from 'react-i18next'

// Detect language with multiple fallback methods
let detectedLanguage = 'en'

try {
  // Try expo-localization first (most reliable for Expo apps)
  const Localization = require('expo-localization')
  const deviceLanguage = Localization.getLocales()?.[0]?.languageCode
  if (deviceLanguage) {
    detectedLanguage = deviceLanguage
    console.log('🌍 Device language detected via expo-localization:', deviceLanguage)
    console.log('🔤 Detected language code:', detectedLanguage)
    console.log('📱 Full locale info:', Localization.getLocales()?.[0])
  } else {
    throw new Error('expo-localization returned no language')
  }
} catch (localeError: unknown) {
  const localeErrorMsg = localeError instanceof Error ? localeError.message : String(localeError)
  console.warn('⚠️ expo-localization not available, trying react-native-localize:', localeErrorMsg)

  try {
    // Fallback to react-native-localize
    const { getLocales } = require('react-native-localize')
    const deviceLocales = getLocales()
    detectedLanguage = deviceLocales[0]?.languageCode || 'en'
    console.log('🌍 Device locales detected via react-native-localize:', deviceLocales)
    console.log('🔤 Detected language code:', detectedLanguage)
    console.log('📱 Full locale info:', deviceLocales[0])
  } catch (rnLocalizeError: unknown) {
    const rnLocalizeErrorMsg = rnLocalizeError instanceof Error ? rnLocalizeError.message : String(rnLocalizeError)
    console.warn('⚠️ react-native-localize also not available:', rnLocalizeErrorMsg)

    try {
      // Enhanced fallback to platform-specific detection
      const { Platform } = require('react-native')

      if (Platform.OS === 'ios') {
        console.log('🍎 iOS detected, trying enhanced locale detection...')
        const { NativeModules } = require('react-native')

        // Try multiple iOS methods
        let iosLocale = null

        // Method 1: SettingsManager AppleLocale
        if (NativeModules.SettingsManager?.settings?.AppleLocale) {
          iosLocale = NativeModules.SettingsManager.settings.AppleLocale
          console.log('🍎 iOS AppleLocale found:', iosLocale)
        }

        // Method 2: SettingsManager AppleLanguages array
        if (!iosLocale && NativeModules.SettingsManager?.settings?.AppleLanguages) {
          const languages = NativeModules.SettingsManager.settings.AppleLanguages
          iosLocale = languages[0]
          console.log('🍎 iOS AppleLanguages found:', languages, '-> using:', iosLocale)
        }

        // Method 3: I18nManager
        if (!iosLocale) {
          const { I18nManager } = require('react-native')
          if (I18nManager.localeIdentifier) {
            iosLocale = I18nManager.localeIdentifier
            console.log('🍎 iOS I18nManager localeIdentifier found:', iosLocale)
          }
        }

        if (iosLocale) {
          // Extract language code (handle both "es_ES" and "es-ES" formats)
          detectedLanguage = String(iosLocale).split(/[-_]/)[0]
          console.log('🔤 iOS extracted language code:', detectedLanguage)
        } else {
          console.log('🍎 No iOS locale found, using default: en')
        }
      } else if (Platform.OS === 'android') {
        console.log('🤖 Android detected, trying locale detection...')
        const { I18nManager } = require('react-native')
        if (I18nManager.localeIdentifier) {
          detectedLanguage = I18nManager.localeIdentifier.split(/[-_]/)[0]
          console.log('🤖 Android locale detected:', I18nManager.localeIdentifier, '-> extracted:', detectedLanguage)
        }
      } else {
        console.log('🌐 Web/other platform detected...')
        // Web fallback
        if (typeof navigator !== 'undefined' && navigator.language) {
          detectedLanguage = navigator.language.split(/[-_]/)[0]
          console.log('🌐 Web locale detected:', navigator.language, '-> extracted:', detectedLanguage)
        }
      }
    } catch (platformError: unknown) {
      const platformErrorMsg = platformError instanceof Error ? platformError.message : String(platformError)
      console.warn('⚠️ Platform-specific locale detection failed:', platformErrorMsg)
      detectedLanguage = 'en'
      console.log('🔧 Using default language: en')
    }
  }
}

const resources = {
  en: {
    translation: {
      token_balances_header: 'Balances',
      wallet_coin_switcher: 'Choose which coin to show',
      wallet_activity_empty_asset: 'No {{ticker}} activity yet',
      pay_direction_pay_asset: 'Pay {{ticker}}',
      token_fee_footer: "Sending needs a little BSV for the network fee. Receiving doesn't.",
      token_new_tap: 'New — tap to see who issues it',
      token_unresolved: 'Unrecognised token',
      token_unresolved_sub: "This wallet couldn't look up who issues it. Try again when you're online.",
      token_paused_short: 'Transfers paused',
      token_frozen_short: '{{amount}} frozen',
      token_sheet_issuer: 'Issued by',
      token_sheet_issuer_unknown: 'Not identified',
      token_sheet_asset_id: 'Token ID',
      token_sheet_powers_header: 'What {{issuer}} can do',
      token_power_pause: 'Pause all transfers of {{ticker}}',
      token_power_freeze: 'Freeze specific coins, including yours',
      token_power_admit: 'Decide who may hold {{ticker}}',
      token_power_replace: 'Replace a frozen coin with a new one',
      token_powers_limit:
        "They can't move {{ticker}} out of your wallet without you, and a frozen coin simply can't be spent.",
      token_frozen_sub: '{{issuer}} has frozen {{amount}} {{ticker}} of your balance. Contact them to dispute.',
      pay_asset_label: 'Paying with',
      pay_asset_label_get: 'Getting paid in',
      pay_asset_cta: 'Send {{amount}} {{ticker}}',
      pay_asset_over_balance: 'More than your {{ticker}} balance.',
      pay_asset_needs_bsv: "Sending {{ticker}} needs a little BSV for the network fee. Receiving doesn't.",
      pay_asset_get_bsv: 'Get BSV',
      pay_asset_needs_bsv_short: 'Needs BSV for the fee',
      pay_asset_frozen_note: "{{amount}} {{ticker}} of your balance is frozen and can't be sent.",
      pay_asset_paused: "{{issuer}} has paused {{ticker}} transfers, so this can't be sent right now.",
      pay_asset_no_address: '{{ticker}} can only be sent to a person or a nearby device — not to a Bitcoin address.',
      pay_asset_address_status: 'Not available for {{ticker}}',
      pay_asset_self_unregistered: "{{issuer}} hasn't registered you to send {{ticker}}. You can still be paid in it.",
      pay_asset_unidentified: "This wallet couldn't identify this token, so it can't send it yet.",
      pay_asset_recipient_unregistered:
        "{{issuer}} hasn't registered this person for {{ticker}} yet, so they can't be paid in it.",
      pay_asset_recipient_blocked: '{{issuer}} does not allow payments to this person.',
      token_err_refused: '{{issuer}} refused this transfer. Nothing was sent and your {{ticker}} is unchanged.',
      token_err_refused_paused: '{{issuer}} has paused {{ticker}}. Nothing was sent and your balance is unchanged.',
      token_err_refused_recipient:
        '{{issuer}} does not allow payments to this person. Nothing was sent and your balance is unchanged.',
      token_err_refused_frozen:
        'Some of the {{ticker}} you were sending has just been frozen. Nothing was sent; your balance has been updated.',
      token_err_send_failed: "Couldn't send this payment: {{reason}}. Nothing left your wallet.",
      token_err_reason_unknown: 'an unknown problem',
      token_err_unreachable:
        "We couldn't reach {{issuer}} to confirm this transfer. Check your {{ticker}} balance before trying again.",
      token_err_check_again: 'Check again',
      token_err_balance_changed:
        'Your {{ticker}} balance changed while this was being prepared. Check the amount and try again.',
      token_err_busy: 'A {{ticker}} payment is already going out.',
      pay_sent_not_broadcast: "Sent. It hasn't reached the network yet — we'll keep trying.",
      pay_sent_not_notified: "Sent. We couldn't tell them yet — they'll see it when their wallet next checks.",
      pay_sent_handed_to_wallet: "Sent. They didn't take it in person, so we've sent it to their wallet instead.",
      token_first_hold: '{{ticker}} is issued by {{issuer}}, who can pause transfers and freeze coins.',
      pay_asset_request_paused: '{{issuer}} has paused {{ticker}}, so a payment may not arrive yet.',
      token_admit_title: "You can't be paid in {{ticker}} yet",
      token_admit_body:
        "{{issuer}} only sends {{ticker}} to people it has registered. Until then, payments to you are refused before they leave the sender's wallet.",
      token_admit_bsv_instead: 'Get paid in BSV instead',
      token_row_sent: 'Sent {{ticker}}',
      token_row_received: 'Received {{ticker}}',
      token_row_amount_pending: 'Amount unavailable',
      token_evicted_title: 'Some {{ticker}} was withdrawn',
      token_evicted_body: '{{issuer}} withdrew {{amount}} {{ticker}} from your wallet.',
      local_pay_token_not_cleared: 'Received offline · not yet confirmed by {{issuer}}',
      pay_asset_nearby_needs_network:
        'Paying in {{ticker}} in person needs a connection. You can pay in BSV without one.',
      pay_asset_nearby_wrong_overlay: "This request is for a token this wallet isn't set up for.",
      pay_asset_link_not_held: "This request is for a token this wallet doesn't hold. You can still pay them in BSV.",
      token_messagebox_mismatch: 'Your wallet is set to a different message box, so token payments may not reach you.',
      token_issuer_fallback: 'the issuer',
      token_row_hint: 'Opens who issues it and what they can do',
      token_attention_one: 'Waiting on {{issuer}} to confirm — tap for details',
      token_attention_many: '{{count}} token payments are waiting to settle — tap for details',
      token_recv_reversed: 'This payment could not be confirmed by {{issuer}} and has been reversed.',
      token_settle_paused_retry: "{{issuer}} has paused {{ticker}} right now — we'll keep trying.",
      token_settle_unreachable_retry: "We couldn't reach {{issuer}} to confirm this transfer. We'll keep trying.",
      token_settle_evicted:
        "That payment didn't reach the network in time and has been reversed — your {{ticker}} is back in your balance.",
      token_frame_unreadable: "We couldn't read the payment you received — ask the sender to send it again.",
      token_chain_reversed:
        'A payment earlier in this chain was refused by {{issuer}}, so this one has been reversed too.',
      token_settle_stuck_bound:
        'This payment has been waiting to settle for over 3 days. You can keep waiting or contact {{issuer}}.',
      token_cover_failed: "This payment doesn't check out with {{issuer}}'s records and was not accepted.",
      token_sent_settling: "Sent. Settling with {{issuer}} — you'll see it confirm once you or {{payee}} reconnect.",
      token_sent_settling_unnamed: "Sent. Settling with {{issuer}} — you'll see it confirm once one of you reconnects.",
      token_sent_settled: 'Sent · settled with {{issuer}}',
      token_status_settling: 'Settling',
      token_status_settled: 'Settled',
      token_status_refused: 'Refused',
      token_status_reversed: 'Reversed',
      token_status_stuck: 'Waiting to settle',
      // Browser UI
      new_tab: 'New Tab',
      untitled: 'Untitled',
      share: 'Share',
      back_to_homepage: 'Back to Homepage',
      bookmarks: 'Bookmarks',
      add_bookmark: 'Add Bookmark',
      bookmark: 'Bookmark',
      unbookmark: 'Unbookmark',
      find_in_page: 'Find in Page',
      find_in_page_placeholder: 'Find in page...',
      find_in_page_matches: '{{current}} of {{total}}',
      find_in_page_no_matches: 'No matches',
      history: 'History',
      clear_all: 'Clear All',
      clear_all_tabs: 'Clear All Tabs',
      search_or_enter_website: 'Search or enter website',
      browser: 'Browser',
      bug_report: 'Bug Report',
      enable_web3: 'Enable Web3',
      tabs: 'Tabs',
      unlock_web3_features: 'Unlock Web3 features',

      // General actions
      cancel: 'Cancel',
      done: 'Done',
      discard: 'Discard',
      deny: 'Deny',
      allow: 'Allow',
      save: 'Save',
      remove: 'Remove',
      delete: 'Delete',
      search: 'Search',
      loading: 'Loading...',
      continue: 'Continue',
      go_back: 'Go Back',
      continue_action: 'Continue',
      camera_denied_description: 'Camera access was denied. You can enable it in Settings to use this feature.',
      open_settings: 'Open Settings',
      copied: 'Copied',
      copy: 'Copy',
      copy_to_clipboard: 'Copy to Clipboard',
      details: 'Details',
      hide_details: 'Hide details',

      // Navigation
      identity: 'Identity',
      settings: 'Settings',
      wallet: 'Wallet',
      activity: 'Activity',
      configuration: 'Configuration',
      data_and_security: 'Data & Security',
      storage_row: 'Storage',
      storage_title: 'Wallet storage',
      storage_db_size: 'Database: {{size}}',
      storage_free_space: 'Free space: {{size}}',
      storage_reclaimable: 'Reclaimable now: {{size}} across {{count}} transactions',
      storage_held_back: '{{count}} transactions held back as still in use',
      storage_reclaim_explain:
        'Reclaiming clears validation data the wallet no longer needs. Your transactions, balances and history are untouched. This frees space inside the database file; the file itself does not shrink until the system compacts it.',
      storage_reclaim_confirm: 'Reclaim',
      storage_reclaimed: 'Reclaimed {{count}} transactions',
      storage_failed: 'Could not read storage use.',
      storage_unavailable: 'Storage is not ready yet.',
      backup_push_section: 'Private backup',
      backup_push_toggle: 'Back up to server',
      backup_erase_row: 'Delete backup from server',
      backup_erase_title: 'Delete your backup?',
      backup_erase_message:
        "Everything this wallet has sent to the backup server is permanently deleted, across every device. Backing up is switched off at the same time. Your coins, keys and recovery phrase are untouched — but a new device will no longer be able to restore this wallet's history, and some of your existing coins will not be spendable there.",
      backup_erase_confirm: 'Delete',
      backup_erase_done: 'Backup deleted — {{count}} items removed',
      backup_erase_failed: 'Could not delete the backup. Nothing was removed — please try again.',
      backup_erase_no_key: 'No wallet key available',
      backup_push_disclosure:
        'An encrypted copy of your wallet database is sent to a BSV Association server so you can recover it on a new device. It is encrypted with a key only your recovery phrase can derive — the operator cannot read it. You still need your recovery phrase or printed shares: without them the backup cannot be unlocked, by us or by anyone else.',
      backup_push_off_title: 'Turn off private backup?',
      backup_push_off_message:
        'Your recovery phrase alone cannot rebuild this wallet. Information associated with securing your coins exists only in the database, so with backups off, a new device can restore your phrase but not your funds. Nothing already on the server is deleted.',
      backup_push_off_confirm: 'Turn off',
      backup_push_on_toast: 'Backups on — catching up now',
      backup_push_off_toast: 'Backups off — nothing more will be sent',
      permissions: 'Permissions',
      trust_network: 'Trust Network',

      // ── Vault (YubiKey-gated high-value basket) ──
      vault_title: 'Vault',
      vault_row_title: 'Vault',
      vault_hero_title: 'Secure Holdings',
      vault_hero_body: 'Cool storage for the long term, locked to your own YubiKeys. Needs two or more YubiKey 5 NFC.',
      vault_unsupported_title: 'Needs a YubiKey',
      vault_unsupported_body: 'Two or more YubiKey 5 NFC, plus a phone that can read them.',
      vault_restore_from_chain: 'Restore vault from the blockchain',
      vault_restore_scanning: 'Scanning the blockchain…',
      vault_restore_found: 'Found {{count}} vault deposit(s) on the blockchain.',
      vault_restore_pending: '{{count}} deposit(s) found, awaiting their first confirmation — try again shortly.',
      vault_restore_none_found: 'No vault deposits were found on the blockchain for this wallet.',
      vault_balance_label: 'Vault holds',
      vault_key_section: 'Security keys ({{count}} of 5)',
      vault_manage_section: 'Manage',
      vault_deposit_cta: 'Deposit',
      vault_withdraw_cta: 'Withdraw',
      vault_deposit_title: 'Deposit to vault',
      vault_withdraw_title: 'Withdraw from vault',
      vault_deposit_sub: 'From your everyday balance. No YubiKey needed.',
      vault_withdraw_sub: 'Back to your everyday balance. Tap any of your vault keys.',
      vault_deposit_done: 'Moved to vault',
      vault_reclaim_done: 'Recovered funds from an interrupted vault deposit',
      vault_withdraw_done: 'Withdrawn from vault',
      vault_withdraw_partial: '{{count}} more deposits remain — withdraw again to move them.',
      vault_withdraw_reason: 'Hold your YubiKey here to sign — withdraw {{amount}} sats',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: 'Not available yet — vault deposits are switched off in this release.',
      vault_not_on_mainnet_body: 'Vault is available on mainnet only. Switch the wallet back to mainnet to use it.',
      vault_intro_title: 'Set up your vault',
      vault_intro_what:
        'The vault locks money to your own YubiKeys. Nothing about the vault key ever exists on this phone; any one of your keys opens it.',
      vault_intro_two_keys: 'You need at least two YubiKeys.',
      vault_intro_apart: 'Keep them in different places — two keys stored together are one key.',
      vault_intro_backup: "Keep the wallet's encrypted backup on — it preserves the authenticated recovery record.",
      vault_intro_ack:
        'I understand: only my YubiKeys open this vault. My recovery phrase does not. If I lose all of them, the money is gone.',
      vault_intro_piv_ack:
        "I confirm this YubiKey's entire PIV application is factory-reset or dedicated to Vault. Setup changes its global PIV PIN, PUK, and management key, may affect every PIV slot, and I will securely record the new PUK.",
      vault_intro_begin: 'Begin',
      vault_key_step_title: 'Key {{k}} of up to 5',
      vault_key_step_replace:
        'Use a factory-reset YubiKey dedicated to Vault. If its Vault slot contains an old key, setup will warn before replacing it.',
      vault_pin_choose_title: 'Choose a PIN',
      vault_pin_choose_sub: 'You will enter this every time you use this key. 6 to 8 digits.',
      vault_pin_confirm_label: 'Enter it again',
      vault_pin_mismatch: 'Those two PINs do not match.',
      vault_pin_not_default: 'Pick something other than 123456.',
      vault_puk_title: 'Your recovery code',
      vault_puk_body:
        'Write this down and keep it somewhere safe. It is the only way to unlock this key if the PIN is forgotten, and it will not be shown again.',
      vault_puk_ack: 'I have written this down',
      vault_reset_offer: 'Reset this key',
      vault_reset_title: 'Reset this key?',
      vault_reset_body:
        'This key has been used before. Resetting erases every certificate and key in its PIV application, not only the vault slot. Anything that relies on this key stops working.',
      vault_reset_ack: 'I understand this erases everything on this key',
      vault_reset_unknown_ack:
        "This device cannot confirm whether this key still secures a different profile's vault. I understand erasing it may destroy that key for good.",
      vault_reset_own_draft_ack:
        'This key holds the unfinished key from your own earlier attempt, and this device still has its record. I understand resetting erases it and that attempt cannot be resumed.',
      vault_reset_nothing_erased:
        'Nothing was erased. This key’s vault slot is not empty, so there is one more thing to confirm.',
      vault_reset_confirm: 'Erase and reset',
      vault_resetting: 'Resetting the key…',
      vault_reset_done: 'Key reset. Tap it again to continue.',
      vault_reset_uncertain:
        'The reset did not finish. This key may already have been erased — check it before relying on it.',
      vault_err_reset_enrolled:
        'That key is already part of your vault — resetting it would destroy one of your vault keys.',
      vault_err_reset_unreadable:
        'Your vault records on this device could not be read, so there is no way to prove this key is not one of your vault keys. Nothing was erased and the reset is refused.',
      vault_enrollment_resume_pin_hint:
        'Resuming a saved key needs the PIN you chose for it earlier, not a new one. Three wrong tries lock the key.',
      vault_setup_step: 'Step {{n}} of {{total}}',
      vault_enrollment_resume: 'Resume saved key',
      vault_enrollment_reset_required:
        'This YubiKey is no longer in its factory state, so it cannot be enrolled as it is. Its PIV application has to be erased back to factory before it can hold a vault key.',
      vault_enrollment_state_uncertain:
        'A setup attempt was interrupted, so there is no way to be sure what state it left the YubiKey in. That key will not be used again until its PIV application has been erased back to factory.',
      vault_nfc_enroll_message: 'Hold your YubiKey here to set it up',
      vault_nfc_adopt_message: 'Hold this recovered YubiKey here to verify it',
      vault_nfc_sign_batch: 'Hold your YubiKey here to sign — batch {{b}} of {{n}}',
      vault_key_use_different: 'Use a different YubiKey',
      vault_key_setup_again: 'Set it up again',
      vault_name_default: 'Key {{k}}',
      vault_more_title: 'Add another key?',
      vault_more_body: 'Up to 5 keys. Any one of them opens the whole vault on its own.',
      vault_more_add: 'Add another key',
      vault_more_finish: 'Finish',
      vault_more_need_two: 'Add a second key before finishing — one key means no recovery.',
      vault_leave_setup: 'Leave set-up',
      vault_leave_title: 'Leave set-up?',
      vault_leave_body:
        "The {{count}} YubiKey(s) you set up won't be saved yet. They keep their keys, so you can add them again in a minute.",
      vault_leave_confirm: 'Leave',
      vault_leave_stay: 'Stay',
      vault_done_body: '{{count}} keys can open this vault. Only these keys open it — your recovery phrase does not.',
      vault_done_cta: 'Done',
      vault_add_key_row: 'Add key',
      vault_add_key_done:
        '{{nickname}} can open deposits made from now on. Re-lock the vault so it can open everything.',
      vault_key_added_toast: 'Key added',
      vault_recovery_verified: 'Recovered key verified on this device',
      vault_relock_row: 'Re-lock vault',
      vault_relock_now: 'Re-lock now',
      vault_relock_choose: 'Which key will you tap to re-lock?',
      vault_relock_reason: 'Tap one of your existing keys ({{names}}) — not the one you just added.',
      vault_relock_reason_generic: 'Hold your YubiKey here to sign — re-lock the vault to your current keys',
      vault_relock_reason_remaining: 'Re-lock to only your remaining keys: {{names}}.',
      vault_key_pending_removal: 'Removed — re-lock to finish',
      vault_key_removal_awaiting_network: 'Removing — waiting for the network to accept the re-lock',
      vault_removal_awaiting_network: 'Re-lock sent. Deposits and withdrawals open again once the network accepts it.',
      vault_err_too_many_active_keys: 'The vault locks to at most five keys. Remove one before depositing, re-locking or withdrawing part of the balance.',
      vault_key_section_over_limit: 'Security keys ({{count}}, remove one)',
      vault_add_key_done_over_limit: '{{nickname}} is added. The vault locks to at most five keys, so remove the key this one replaces, then re-lock once.',
      vault_relock_done: 'Vault re-locked to your current keys',
      vault_relock_capped: '{{count}} more deposits remain — re-lock again to move them.',
      vault_relock_unreachable:
        '{{count}} deposits can only be opened by {{names}}. Re-lock again with one of those keys.',
      vault_badge_missing: '{{count}} deposits not yet open to {{nickname}}',
      vault_badge_removed: '{{count}} deposits still open to a removed key',
      vault_key_action_rename: 'Rename',
      vault_key_action_remove: 'Remove',
      vault_rename_title: 'Rename {{nickname}}',
      vault_rename_save: 'Save',
      vault_remove_title: 'Remove {{nickname}}?',
      vault_remove_body:
        'This stops the wallet using {{nickname}}. Money already in the vault stays openable by it until you re-lock (≈ {{fee}} sats).',
      vault_remove_only: 'Remove',
      vault_key_removed_toast: 'Key removed',
      vault_export_explainer:
        'Every vault deposit hides its salt until you spend it — nothing about it is visible in the locking script. Keep encrypted backup on so a new device can restore and authenticate the key records needed to use your YubiKeys.',
      vault_footnote: 'Only these keys open the vault. Your recovery phrase does not.',
      vault_floor_line:
        'Minimum deposit {{floorDisplay}} ({{floorSats}} sats). Creating a vault deposit costs about {{feeDisplay}}.',
      vault_first_deposit_title: 'First vault deposit',
      vault_first_deposit_body:
        "First vault deposit — {{amount}} will be openable only with {{count}} YubiKeys ({{names}}). Your recovery phrase won't help. Each deposit hides its salt until spend, but recovery also needs authenticated key records and wallet history.",
      vault_backup_off_title: 'Turn private backup on',
      vault_backup_off_body:
        'Private backup must be on before the wallet creates another Vault output. It stores the authenticated YubiKey records and wallet history needed for recovery. You can still withdraw everything without creating Vault change.',
      vault_backup_off_cta: 'Open backup settings',
      vault_seed_not_preserved_title: 'Save your recovery phrase first',
      vault_seed_not_preserved_body:
        'Your Vault can only be recovered with your recovery phrase plus one of your YubiKeys. YubiKeys hold no copy of it — if it is lost before you write it down, these funds cannot be recovered. Save it now before depositing.',
      vault_seed_not_preserved_cta: 'Save recovery phrase',
      vault_choose_key: 'Which key will you tap?',
      vault_remainder_title: 'Withdraw everything?',
      vault_remainder_body:
        'Withdrawing {{amount}} leaves {{remainder}}, which is below the 100,000-sat vault minimum. The whole vault will move to your everyday balance.',
      vault_remainder_all: 'Withdraw everything',
      vault_remainder_change: 'Change amount',
      vault_unreachable_title: 'Part of the vault needs another key',
      vault_unreachable_body:
        'Part of the vault needs another key — moved {{moved}}. {{count}} deposits holding {{amount}} can only be opened by {{names}}. Withdraw again with one of those keys.',
      vault_sign_progress: 'Signed {{signed}} of {{total}}',
      vault_err_pin_locked_enroll:
        "This YubiKey's PIN is blocked. Unblock it with its PUK in Yubico Authenticator, or set up with a different YubiKey.",
      vault_err_serial_mismatch_chosen:
        "That's {{tappedName}}. You chose {{chosenName}} — tap it, or go back and choose {{tappedName}}.",
      vault_err_key_already_enrolled: "You've already added this YubiKey ({{nickname}}). Tap a different one.",
      vault_err_not_released: 'Vault deposits are switched off in this release.',
      vault_err_not_on_mainnet: 'Vault is available on mainnet only. Switch the wallet to mainnet and try again.',
      vault_err_backup_off:
        'Turn encrypted private backup on before creating another Vault output. You can still withdraw everything.',
      vault_err_not_enough_keys: 'This vault has fewer than two keys. Set it up again.',
      vault_err_too_many_keys: 'A vault holds at most five keys.',
      vault_err_last_keys: 'A vault needs at least two keys. Add another key before removing this one.',
      vault_err_relock_required: 'Re-lock the vault first so your other keys can open every deposit.',
      vault_err_action_pending: 'A vault transaction has not finished yet. Try again once it settles.',
      vault_resolve_held_deposit_action: 'Finish the interrupted deposit',
      vault_resolve_held_deposit_done: 'Deposit resolved',
      vault_resolve_held_deposit_failed: 'Could not resolve the deposit. Try again.',
      vault_resolve_held_deposit_none:
        'Nothing to resolve here — check the Activity tab for a pending transaction to retry or cancel.',
      vault_err_key_not_committed:
        "{{nickname}} can't open any of the deposits in this vault. Use another of your vault keys.",
      vault_err_key_cannot_cover:
        '{{nickname}} can open {{reachable}} of the {{total}} in the vault. Withdraw up to {{reachable}}, or use {{otherNames}} instead.',
      vault_err_too_small_to_relock:
        'This vault holds less than 100,000 sats, which is too small to re-lock. Withdraw it instead and deposit again.',
      vault_err_bad_version: 'The transaction was built in the wrong format. Nothing was signed — try again.',
      vault_err_chain_scan_failed:
        'Could not finish scanning the blockchain for vault deposits. Check your connection and try again.',
      // enrollment
      vault_shares_unavailable: 'No key material available to print.',
      vault_enroll_begin: 'Set up vault',
      vault_enroll_phase_connecting: 'Reading your key…',
      vault_enroll_phase_pin_check: 'Checking your key…',
      vault_enroll_phase_checking_slot: 'Checking the vault slot…',
      vault_enroll_phase_personalizing: 'Setting your PIN and recovery code…',
      vault_enroll_phase_generating: 'Creating the vault key — touch the key when it blinks',
      vault_enroll_phase_challenging: 'Verifying the new vault key — touch the key when it blinks',
      vault_enroll_phase_done: 'Done',
      vault_touch_when_blinks: 'Touch the gold contact when the key blinks.',
      vault_nfc_activation_hint:
        'First time with this YubiKey? Plug it into a USB-C port for a few seconds first — new keys ship with NFC switched off until then.',
      vault_continue: 'Continue',
      vault_back: 'Back',
      vault_enrolled_toast: 'Vault ready',
      // ceremony
      vault_insert_key: 'Insert your YubiKey',
      vault_hold_key_nfc: 'Hold your YubiKey to the top of your phone',
      vault_keep_holding_nfc: 'Keep holding — unlocking…',
      // Busy-phase copy. Says the work is real and asks the user to stay put:
      // K1 signing is fast software, but assembling the transaction and
      // waiting on the broadcast still take a few seconds, and a user who
      // force-quits here can leave the vault output reserved.
      vault_unlocking_funds: 'Unlocking your funds',
      vault_unlocking_sub: 'This takes a few seconds. Please keep the app open.',
      vault_sending_to_network: 'Sending to the network',
      vault_sending_sub: "If it doesn't go through right away, we'll keep trying.",
      vault_reading_key: 'Reading key…',
      vault_enter_pin: 'Enter YubiKey PIN',
      vault_pin_sub_nfc: 'Then hold your key to the top of your phone to finish.',
      vault_pin_sub_usb: 'Then touch the gold contact to finish.',
      vault_touch_contact: 'Touch the gold contact',
      vault_unlock_cta: 'Unlock',
      vault_pin_retries: '{{count}} attempts left',
      vault_locked: 'Vault locked',
      vault_retry: 'Try again',
      vault_cancel: 'Cancel',
      vault_dismiss: 'Dismiss',
      // errors
      vault_err_wrong_key: "This YubiKey isn't one of this vault's keys.",
      vault_err_serial_mismatch: "This YubiKey isn't one of this vault's keys ({{names}}).",
      vault_err_touch_timeout: 'No touch detected. Try again and touch the gold contact.',
      vault_err_pin_locked:
        "This YubiKey's PIN is blocked. Use another of your vault keys, or unblock this one with its PUK in Yubico Authenticator.",
      vault_err_pin_invalid: 'Wrong PIN.',
      vault_err_puk_invalid: 'Wrong PUK.',
      vault_err_puk_locked: "This YubiKey's PUK is blocked. Use a fresh YubiKey.",
      vault_err_key_removed_mid_op: 'The key was removed. Insert it and try again.',
      vault_err_ceremony_active:
        'Another Vault operation is still using the YubiKey. Wait for it to finish and try again.',
      vault_err_scope_changed:
        'The wallet or network changed during this Vault operation. Start again in the current wallet.',
      vault_err_no_key: 'No YubiKey detected.',
      vault_err_driver_unavailable: 'YubiKey support is unavailable on this device.',
      vault_err_mgmt_key_custom: 'This key has a custom management key and cannot be used automatically.',
      vault_err_attestation_invalid:
        "This YubiKey's manufacturer attestation could not be verified. Use a genuine, factory-attested YubiKey with an untouched PIV attestation slot.",
      vault_err_generic: 'Something went wrong. Please try again.',
      vault_err_template_invalid: 'Something went wrong while attempting to access the Vault. Please try again.',
      vault_err_not_enrolled: 'No vault is set up.',
      vault_err_pin_required: 'Enter your PIN.',
      vault_err_unsupported_platform: 'YubiKey support is unavailable on this device.',
      vault_err_slot_occupied: 'The key slot is already in use.',
      vault_replace_key_warning:
        'Replacing it permanently destroys the existing P-256 private key. Any vault funds that require that key could become unspendable.',
      vault_replace_key_confirm: 'Replace existing key',
      vault_resume_draft_confirm_title: 'Use this prepared key?',
      vault_resume_draft_confirm_body:
        'A previously prepared key — {{nickname}} · …{{tail}} — is ready to add, but this device did not just verify it on a card. Only continue if you recognize it.',
      vault_resume_draft_confirm_use: 'Use this key',
      vault_resume_draft_confirm_discard: 'Discard it',
      vault_err_enrollment_partial:
        'YubiKey setup stopped after changing the key. Follow the retry instructions; this key was not added to the Vault.',
      vault_err_key_not_adopted:
        'Verify possession of this recovered YubiKey on this device before using it to withdraw.',
      vault_err_too_many_inputs:
        'Too many vault deposits to move at once. Withdraw a smaller amount first — each withdrawal also consolidates the vault, so the next one can move more.',
      vault_err_requires_online:
        'Vault transfers need a connection. Reconnect and try again — vault funds are never queued for later.',
      vault_err_user_cancelled: 'Cancelled.',
      vault_err_vault_empty: 'The vault has no funds to withdraw yet.',
      vault_err_amount_exceeds_balance: 'That is more than the vault holds.',
      vault_err_below_dust: 'That amount is too small to send.',
      vault_err_no_transaction: 'The transfer could not be completed. Please try again.',
      vault_err_nfc_lost: 'The connection dropped. Hold the key steady against your phone.',
      vault_pin_invalid_retry: 'Wrong PIN — {{count}} attempts left. Tap Withdraw again to retry.',
      vault_pin_invalid_retry_generic: 'Wrong PIN. Tap Withdraw again to retry.',
      // disable
      vault_disable_row: 'Disable vault',
      vault_disable_title: 'Disable vault?',
      vault_disable_message: "This forgets the vault's key list on this phone. The keys stay on your YubiKeys.",
      vault_disable_confirm: 'Disable',
      vault_disabled_toast: 'Vault disabled',
      vault_disable_blocked_title: 'Withdraw funds first',
      vault_disable_blocked_message:
        'The vault still holds funds. Withdraw everything to your everyday balance before disabling — otherwise those funds would be locked with no way to sign them.',
      vault_ok: 'OK',
      vault_mock_toggle: 'Use mock YubiKey (dev)',
      vault_on: 'On',
      vault_off: 'Off',

      vault_shares_word_count: 'Recovery shares support twelve-word phrases only. Write your phrase down instead.',

      // Legacy backup shares
      scan_shares_legacy_title: 'Older backup format',
      scan_shares_legacy_message:
        'These shares are an older format that restores your everyday balance but cannot open a vault. If you still have your twelve-word recovery phrase, restore from that instead — it recovers everything — and print fresh shares afterwards. If the phrase is gone, send your funds to a new wallet and start again.',
      scan_shares_legacy_ack: 'I understand',

      // Account
      delete_wallet: 'Delete Wallet',
      delete_wallet_warning_title: 'Delete Wallet?',
      delete_wallet_warning_body:
        'This will permanently delete your wallet keys from this device. If you have not backed up your recovery phrase or printed recovery shares, you will lose access to your wallet forever.',
      delete_wallet_confirm: 'Delete Wallet',
      delete_wallet_failed_title: 'Delete Wallet Failed',
      delete_wallet_failed_body: 'Your wallet could not be fully erased from this device. Some key material may still be present. Please try again.',
      bsv_network: 'BSV Network',
      arc_endpoint: 'ARC Endpoint',
      arc_api_token: 'API Token',
      arc_custom_url: 'Custom URL',
      arc_apply: 'Apply',
      arc_reset_default: 'Reset to Default',
      arc_requires_token: 'requires token',
      arc_url_https_required: 'Must be https (or http://localhost for local testing)',
      recovery_phrase: 'Recovery Key',
      you_have: 'You have',
      switching: 'Switching...',
      mainnet: 'Mainnet',
      testnet: 'Testnet',
      teratest: 'Teratest',
      identity_key: 'Identifier',
      transactions: 'Transactions',

      // Transactions screen
      no_transactions: 'No transactions yet',
      tx_status_confirmed: 'Confirmed',
      tx_status_received: 'Received',
      tx_status_sent: 'Sent',
      tx_status_spent: 'Spent',
      tx_status_transferred: 'Transferred',
      tx_status_accepted: 'Accepted',
      tx_status_broadcasting: 'Processing',
      tx_status_not_sent: 'Not Sent',
      tx_status_unsigned: 'Draft',
      tx_status_nonfinal: 'Pending',
      tx_status_pending: 'Pending',
      tx_status_failed: 'Failed',
      tx_status_offline_queued: 'Offline · queued',
      tx_status_offline_sending: 'Offline · sending',
      tx_status_offline_rejected: 'Offline · rejected',
      tx_copied: 'Transaction copied',
      tx_not_available: 'Transaction not available',
      tx_copy_failed: 'Failed to copy transaction',
      tx_abort_success: 'Transaction aborted',
      tx_abort_failed: 'Failed to abort transaction',
      tx_abort_maybe_delivered: "Can't cancel — this payment may already be with the recipient",
      tx_proof_refreshed: 'Transaction confirmed',
      tx_proof_refresh_failed: 'Could not fetch proof — transaction may not be confirmed yet',
      tx_marked_failed: 'Not processed — marked failed and funds released',
      tx_still_pending: 'Still pending — not confirmed yet',
      tx_action_explorer: 'View on block explorer',
      explorer_open_failed: 'Could not open block explorer',
      tx_action_refresh: 'Refresh status',
      // Chip label — sits next to BEEF / WoC / TXID, so it has to be one word.
      tx_action_refresh_short: 'Status',
      tx_action_abort: 'Cancel transaction',
      tx_export_success: 'Exported {{count}} transactions',
      tx_export_failed: 'Failed to export transactions',
      tx_export_csv: 'Export CSV',
      tx_export: 'Export',
      activity_filter: 'Filter activity',
      activity_filter_all: 'All',
      activity_search_placeholder: 'Search notes, names, @handles',
      activity_search_clear: 'Clear search',
      activity_filter_no_match: 'No matching activity',
      // The full-screen wallet: balance, three destinations, inline activity.
      // ('wallet' itself is already defined above.)
      wallet_balance_label: 'Balance',
      // Deliberately not "Total balance": this figure is the spendable wallet
      // balance only and excludes the vault basket.
      wallet_balance_you_have: 'You have',
      wallet_balance_refresh: 'Refresh balance',
      wallet_group_today: 'Today',
      wallet_group_yesterday: 'Yesterday',
      back: 'Back',
      wallet_payments: 'Payments',
      wallet_vault: 'Vault',
      wallet_settings: 'Settings',
      wallet_activity: 'Activity',

      // Identity / Payments screen
      message_box_server: 'Message Box Server',
      message_box_required: 'Required to send and receive identity-based payments.',
      send_payment: 'Send Payment',
      recipient: 'Recipient',
      amount: 'Amount',
      enter_valid_amount: 'Enter an amount greater than zero',
      display_currency: 'Display Currency',
      no_pending_payments: 'No pending payments',
      outgoing_payments: 'Outgoing',
      payment_not_delivered: 'Not delivered',
      payment_delivered: 'Delivered',
      retry: 'Retry',
      retry_failed: 'Retry failed',
      dismiss: 'Dismiss',
      valid_identity_key: 'Valid identifier entered',
      valid_bsv_address: 'Valid BSV address entered',
      searching: 'Searching...',
      unknown: 'Unknown',
      from_prefix: 'From:',
      message_box_saved: 'Message Box server saved',
      message_box_removed: 'Message Box server removed',
      message_box_off: 'Off',
      message_box_off_hint: 'Message box server is off. Turn it on in Settings › Advanced.',
      enter_valid_url: 'Please enter a valid URL',
      checking_connection: 'Checking connection...',
      connection_failed: 'Connection failed',
      recipient_placeholder: 'Name, Identifier, or BSV Address...',

      // Trust screen
      trust_updated: 'Trust network updated',
      failed_to_save: 'Failed to save',
      confirm_delete: 'Confirm Delete',
      confirm_delete_body: 'Are you sure you want to delete this certifier?',
      order_certifiers_hint: 'Order certifiers by priority. Apps will display higher-ranked certifiers first.',
      certifiers: 'Certifiers',
      no_certifiers: 'No certifiers yet.',
      add_provider: 'Add Provider',
      unsaved_changes: 'You have unsaved changes',
      duplicate_key_error: 'An entity with this public key is already in the list!',
      get_provider_details: 'Get Provider Details',
      validate_details: 'Validate Details',
      show_advanced: 'Show Advanced',
      hide_advanced: 'Hide Advanced',
      description_length_error: 'Description must be between 5 and 50 characters',
      enter_domain_hint: 'Enter provider domain',
      enter_details_hint: 'Enter provider details',

      // Legacy payments screen
      receive: 'Receive',
      send: 'Send',
      generating_address: 'Generating BSV address...',
      imported: 'Imported',
      unable_to_generate_address: 'Unable to generate BSV address',
      send_bsv: 'Send BSV',
      recipient_address: 'Recipient BSV Address',
      enter_bsv_address: 'Enter BSV address',

      // Recovery key / wallet setup screen
      wallet_data: 'Wallet Data',
      create_new_wallet: 'Create New Wallet',
      generate_recovery_phrase_caption: 'Generate a new recovery key',
      import_existing_wallet: 'Import Existing Wallet',
      paste_recovery_phrase: 'Paste your recovery key',
      save_recovery_phrase_heading: 'Save Your Recovery Key',
      save_recovery_phrase_btn: 'Save Recovery Key',
      recovery_phrase_write_down: 'Write down these 12 words in order and store them in a safe place.',
      acknowledgment_text:
        'I have saved my recovery key and understand that losing it will result in total and permanent loss of all associated funds, tokens, and certificates.',
      enter_recovery_words: 'Enter 12 word mnemonic',
      restore_wallet_description: 'Enter your recovery key to restore your wallet',
      backup_reminder_title: 'Your Wallet is Not Backed Up',
      backup_reminder_body: 'Export your keys to prevent loss of funds.',
      biometric_advisory_title: 'Secure Your Wallet',
      wallet_balance_show_in: 'Show in {{unit}}',
      tx_detail_block: 'Block',
      tx_detail_block_pending: 'Waiting for a block',
      biometric_advisory_body: 'Protect your assets with Face ID or your fingerprint',
      biometric_advisory_body_degraded:
        'This device has no Face ID or fingerprint set up, so your wallet key is protected by your device passcode only.',
      import_prompt_title: 'Already have a wallet?',
      import_prompt_body: 'Restore it from your recovery key or shares.',
      import_prompt_action: 'Import from backup',
      nearby_advisory_title: 'Connecting Nearby',
      nearby_advisory_body:
        "You'll be asked to allow Wi-Fi, Bluetooth, or NFC access. This is only used to connect you directly to a nearby device for payment — never sent to our servers, and never used to build a profile of you.",
      backup_reminder_now: 'Back up keys now',
      backup_reminder_import: 'Import from backup',
      restore_backup_checking: 'Looking for your encrypted backup…',
      restore_backup_progress: 'Restoring your wallet history — {{chunks}} of {{total}}…',
      restore_backup_failed_title: 'Could not restore your history',
      restore_backup_failed_message:
        'Your recovery phrase is fine, but the encrypted backup could not be replayed. Trying again is usually enough. Continuing without it gives you a working wallet with no past transactions, and some of your existing coins will not be spendable.',
      restore_backup_retry: 'Try again',
      restore_backup_skip: 'Abandon',
      restore_backup_unverified_title: 'Backup may be incomplete',
      restore_backup_unverified_body:
        'Your wallet restored, but the backup on the server could not be confirmed as complete. If some history is missing, restore again later once the other device has finished uploading.',
      import_wallet: 'Import Wallet',

      // Permission sheet
      reject: 'Reject',
      authorize: 'Authorize',
      renewal: 'Renewal',
      spending_section: 'Spending',
      protocols_section: 'Protocols',
      baskets_section: 'Baskets',
      certificates_section: 'Certificates',
      requested_fields: 'Requested fields',

      // Amount input
      send_max: 'Send Max',
      message_box_unreachable: 'Could not reach the payment server. Check the message box URL in your settings.',
      available: 'available',
      note: 'Note',
      note_placeholder: "What's it for? (optional)",
      entire_wallet_balance: 'Entire wallet balance',
      entire_vault_balance: 'Entire vault balance',
      export_wallet_data: 'Export Wallet Data',
      export_unencrypted_title: 'This File Is Not Encrypted',
      export_unencrypted_message:
        'The exported file is a plain, unencrypted copy of your wallet data — including identity and certificate information, transaction history, and contacts. Anyone who gets this file can read it. Only share it somewhere you trust.',
      import_wallet_data: 'Import Wallet Data',
      import_conflict_title: 'Database Already Exists',
      import_conflict_message:
        'A wallet database for this identity already exists with a more recent timestamp. The backup with the highest number in its filename is always used.\n\nTo use this older backup instead: export your current wallet data, delete and reinstall the app, then import only the desired backup.',
      import_confirm_title: 'Import Database',
      import_confirm_message: 'This database will become your active wallet. Continue?',
      import_success: 'Database imported successfully. Restarting wallet...',
      import_invalid_file: 'Invalid Backup File',
      import_invalid_file_detail:
        'This file is not a valid wallet database backup. Expected a filename like wallet-XXXXXXXX-mainnet-TIMESTAMP.db.',
      import_oversized_file: 'Backup File Too Large',
      import_oversized_file_detail:
        'This file is larger than a wallet backup should be, so it was not read. Choose a genuine wallet database backup file.',
      import_untrusted_file: 'Cannot Import This Backup',
      import_untrusted_file_detail:
        'This backup could not be verified as belonging to this wallet, or its contents could not be trusted. It was not imported.',
      import_anyway: 'Import Anyway',
      import_invalid_input_title: 'Invalid Input',
      import_invalid_input_message:
        'Please enter a valid recovery phrase (12–24 words) or a 64-character hex private key.',
      import_setup_failed: 'Failed to set up wallet: {{error}}',
      create_wallet_refused: 'Unable to create wallet. Please try again.',
      create_wallet_failed: 'Failed to generate mnemonic. Please try again.',

      // Backup shares
      or: 'or',
      print_recovery_shares: 'Print Recovery Shares',
      scan_backup_shares: 'Scan Backup Shares',
      scan_backup_shares_caption: 'Recover wallet from printed shares',
      scan_shares_invalid_format: 'Invalid share format. Please scan a valid backup share QR code.',
      scan_shares_camera_needed: 'Camera Access Needed',
      scan_shares_camera_description: 'Camera access is required to scan your printed backup share QR codes.',
      scan_shares_grant_camera: 'Grant Camera Access',
      scan_shares_recovering: 'Recovering wallet...',
      scan_shares_scan_first: 'Scan your first backup share',
      scan_shares_progress: '{{scanned}} of {{needed}} shares scanned',
      scan_shares_remaining: '{{count}} more share needed',
      scan_shares_complete: 'All shares collected!',
      scan_shares_recovery_failed: 'Recovery failed. Please try scanning again.',
      scan_shares_threshold_mismatch: 'Threshold does not match previous shares',
      scan_shares_integrity_mismatch: 'Integrity hash does not match — shares are from different keys',
      scan_shares_duplicate: 'This share has already been scanned',
      // Shown when the biometric prompt guarding the keychain is refused, so
      // the recovered secret could not be stored. The buttons reuse the generic
      // `cancel` / `retry` keys, which are already translated everywhere.
      scan_shares_biometric_title: 'Biometric Access Required',
      scan_shares_biometric_message: 'Biometric access is needed to protect your wallet keys. Please try again.',
      recovery_replace_wallet_title: 'Replace the wallet on this phone?',
      recovery_replace_wallet_body:
        'This phone already holds a wallet. Continuing replaces it and everything it can spend. If its recovery phrase is not saved somewhere, that money is gone for good.',
      recovery_replace_wallet_confirm: 'Replace wallet',
      invalid_bsv_address: 'Invalid BSV address',
      scan_recipient_hint: 'Point the camera at a payment code',
      scan_qr_default_hint: 'Point the camera at a QR code',
      scan_wallet_qr_hint: 'Point the camera at a wallet pairing QR code',

      // Biometric prompts
      biometric_store_wallet: 'Securely Store Your Wallet',
      biometric_load_wallet: 'Securely Load Your Wallet',
      biometric_unlock_wallet: 'Unlock your wallet',
      wallet_lost_title: 'Wallet key destroyed',
      wallet_lost_body:
        'This device changed its biometric settings, so the system destroyed the key protecting this wallet. Your coins are safe on the blockchain — restore this wallet with your recovery phrase.',
      wallet_lost_restore: 'Restore',
      wallet_lost_later: 'Not now',
      wallet_locked_title: 'Wallet locked',
      wallet_locked_body: 'Unlock to load your wallet keys.',
      wallet_unlock_retry: 'Unlock wallet',
      wallet_lockout_body: 'Too many failed attempts. Unlock your phone with its passcode, then try again.',
      wallet_no_biometrics_body:
        'This device has no biometrics set up. Your wallet keys are still encrypted by the device keystore, but they are not protected by a biometric check.',

      // Connections screen
      connections: 'Connections',
      advanced: 'Advanced',
      copy_secret_words: 'Copy Secret Words',
      print_recovery_keys: 'Print Recovery Keys',
      backup_wallet_keys: 'Backup Wallet Keys',
      log_out: 'Log Out',
      connect_to_app: 'Connect to App',
      no_connections_yet: 'No connections yet',
      no_connections_subtitle: 'Scan a QR code from a desktop app to connect your wallet',
      connections_active: 'Active',
      connections_disconnected: 'Disconnected',
      disconnect: 'Disconnect',
      reconnect: 'Reconnect',
      reconnect_failed: 'Reconnect Failed',
      reconnect_confirm_title: 'Reconnect to this site?',
      reconnect_confirm_message: '{{origin}} will be able to access your wallet again.',
      reconnect_needs_reapproval: 'This connection could not be verified. Scan a new QR code to reconnect.',
      scan_qr_code: 'Scan QR Code',
      paste_uri: 'Paste URI',
      invalid_qr_code: 'Invalid QR Code',
      wallet_not_ready: 'Wallet not ready',
      please_log_in_first: 'Please log in first',
      unknown_error: 'Unknown error',

      // Pay screen
      payments: 'Payments',
      pay: 'Pay',
      pay_direction_pay: 'Pay',
      pay_direction_receive: 'Get paid',
      pay_cell_nearby_pay: 'Someone nearby',
      pay_cell_nearby_pay_sub: 'Scan their code',
      pay_cell_handle_pay: 'Someone remote',
      pay_cell_handle_pay_sub: 'Pick a name — they need this app',
      pay_cell_address_pay: 'To a BSV address',
      pay_cell_address_pay_sub: 'Paste or scan a BSV address',
      pay_cell_nearby_get: 'Someone nearby',
      pay_cell_nearby_get_sub: 'Show your payment code',
      pay_cell_handle_get: 'Someone remote',
      pay_cell_handle_get_sub: 'Share your name',
      pay_cell_address_get: 'To a BSV address',
      pay_cell_address_get_sub: 'Show a BSV address',
      pay_method: 'Method',
      pay_method_nearby: 'Nearby',
      pay_method_remote_link: 'Share remote link',
      pay_method_address: 'To a BSV address',
      pay_pre_nearby: 'You are both here.',
      pay_pre_handle: 'They use this app.',
      pay_pre_address: 'You have their BSV address.',
      pay_conseq_nearby: 'Settles in seconds.',
      pay_conseq_handle: 'Lands when their wallet next checks.',
      pay_conseq_address: 'Sent — they are not notified.',
      paid: 'Paid',
      pay_inbox_attention: 'Couldn’t be added',
      pay_inbox_attention_hint:
        'These arrived but your wallet could not add them. Try again, or dismiss to give up on them for good.',
      pay_dismiss_confirm: 'Tap again to give up',
      pay_dismissed: 'Payment given up',
      payment_arrived: 'Payment received',
      payment_arrived_damaged: 'This payment arrived damaged',
      discard_payment_title: 'Discard this payment?',
      discard_payment_body:
        'The sender will be asked to send the details again. The money stays on the chain until they do.',
      discard_void_title: "This payment didn't go through",
      discard_void_body: "It was cancelled or spent elsewhere. It's safe to dismiss.",
      waiting_network_confirm: 'Waiting for the network to confirm this payment',
      finish_or_cancel_outgoing: 'Finish or cancel the payment below before sending a new one',
      identity_search_unavailable: 'Search is unavailable right now. Try again in a moment.',
      abandon_payment: 'Abandon payment',
      finish_payment: 'Finish payment',
      cancel_this_payment: 'Cancel this payment?',
      cancel_payment: 'Cancel payment',
      resend: 'Resend',
      resending: 'Resending…',
      send_payment_details_again: 'Send details again',
      tx_action_resend_short: 'Resend',
      tx_action_request_confirmation: 'Request confirmation from recipient',
      resend_sent: 'Payment details sent again',
      resend_failed_no_record: "Couldn't find this payment's details on this device, so it can't be sent again.",
      resend_failed_no_recipient: "This payment doesn't say who it was for, so it can't be sent again.",
      resend_failed_no_transaction: "Couldn't get this payment's data. Try again when you're online.",
      resend_requested: 'They asked you to resend a payment',
      request_again: 'Request again',
      send_again: 'Send again',
      copy_details: 'Copy details',
      payment_bounced_resend: 'Asked them to send this payment again',
      dismiss_rejected_payment: 'Dismissed this rejected payment',
      activity_load_failed: "Couldn't load activity",
      activity_load_retry: 'Tap to retry',
      activity_view_contact: 'View contact',
      home_payments_need_attention: '{{count}} payments need attention',
      home_payments_unsent: '{{count}} payments not delivered',
      pay_copy: 'Copy',
      pay_share_link: 'Share link',
      pay_address_watching: 'Money sent here is added to your wallet automatically.',
      pay_address_earlier_day: 'Reach an earlier day',
      pay_address_sweep_now: 'Check this BSV address now',
      pay_address_swept: 'Received {{amount}}',

      check_wallet: 'Troubleshooting',
      settings_version: 'v{{version}}',
      error_review_actions: 'This payment is stuck. Check Wallet can try to free it.',
      wallet_check_ok: 'Everything looks good',
      wallet_check_attention: 'Some checks need your attention',
      wallet_check_couldnt: "Couldn't check",
      wallet_check_couldnt_body: "A repair step didn't finish. Nothing was assumed about your coins.",
      wallet_check_summary: 'Freed {{freed}} stuck coins · Recovered {{recovered}} payments',
      wallet_check_step_coins: 'Your coins are valid',
      wallet_check_skip: 'Skip this check',
      wallet_check_skipped: 'Skipped',
      wallet_check_step_proofs: 'Transaction proofs',
      wallet_check_step_records: 'Local database records',
      wallet_check_step_tokens: 'Stablecoin transfers verified',
      wallet_check_step_online: 'Online',
      wallet_check_step_missed: 'Missed payments',
      wallet_check_step_backup: 'Private backup uploaded',
      wallet_check_step_phrase: 'Recovery phrase saved',
      wallet_check_progress: '{{current}} of {{total}}',
      wallet_check_stuck_title: "Couldn't send this payment",
      wallet_check_stuck_message: 'Some coins look stuck. Check Wallet can free them.',
      wallet_check_failed: "Couldn't finish checking. Try again.",

      // Local payments
      local_pay_request: 'Request Payment',
      local_pay_send: 'Send Payment',
      local_pay_amount: 'Amount',
      local_pay_show_qr: 'Show this code to the payer',
      local_pay_scan_qr: 'Scan the payee’s code',
      local_pay_scan_payer_qr: 'Scan the payer’s code',
      local_pay_scan_progress: '{{have}} of {{total}} pieces',
      local_pay_show_payment_qr: 'Show this code to the payee',
      local_pay_animated_hint:
        'Ask them to scan the animated code below — it keeps changing until their phone has all of it.',
      local_pay_show_qr_instead: 'Show a QR code instead',
      local_pay_preparing: 'Preparing the request…',
      local_pay_delivering: 'Delivering the payment…',
      local_pay_saving: 'Saving the payment…',
      local_pay_sent: 'Payment sent',
      local_pay_received: 'Payment received',
      local_pay_added: 'Added to your wallet.',
      local_pay_added_multiple: '{{count}} payments added to your wallet.',
      local_pay_received_confirming: 'Confirming with your wallet…',
      local_pay_received_not_credited: 'Not added to your wallet yet. Check Activity before handing anything over.',
      local_pay_decline_broadcast_title: 'A declined payment went through',
      local_pay_decline_broadcast_body:
        'A payment that was declined has since appeared on the network. Do not send it again — check Activity.',
      local_pay_pending_abort_dropped_title: 'Ignored an unverified release request',
      local_pay_pending_abort_dropped_body:
        'A saved payment-release record could not be verified, so nothing was released. If a payment still looks stuck, cancel it from Activity.',
      local_pay_cancel_unverifiable_title: 'Cannot check this payment while offline',
      local_pay_cancel_unverifiable_body:
        'If the other person already scanned it, cancelling lets these coins be spent twice. Only cancel if you are sure it was never scanned.',
      local_pay_cancel_unverifiable_confirm: 'Cancel anyway',
      local_pay_queued: 'Saved — it will be added to your wallet automatically.',
      local_pay_already_paid: 'This request has already been paid.',
      local_pay_failed: 'Payment failed',
      local_pay_network_denied: 'Local Network access is off. Enable it in Settings to pay nearby devices.',
      local_pay_radio_fallback: 'Wireless link unavailable — show this code to the other device instead.',
      local_pay_floor_peer_bt_off: 'Their Bluetooth is off, so this payment will go by code.',
      local_pay_floor_local_bt_off: 'Your Bluetooth is off. Turn it on for a faster nearby link.',
      local_pay_floor_local_ble_denied:
        'Bluetooth access is off for BSV Wallet. Allow it in Settings for a faster nearby link.',
      local_pay_floor_cross_os: 'The other device cannot use Bluetooth right now, so this payment will go by code.',
      local_pay_floor_peer_no_radio:
        'The other device is not listening for a wireless link, so this payment will go by code.',
      local_pay_ble_unavailable: 'Bluetooth is unavailable. The code above still works.',
      local_pay_too_large: 'This payment’s data is too large to hand over with QR codes.',
      local_pay_nearby_unavailable: 'Nearby Wi-Fi is unavailable. The code above still works.',
      local_pay_retry_save: 'Try saving again',
      local_pay_wrong_session:
        'That payment doesn’t match this request and was not accepted. Ask the payer to scan the code again.',
      local_pay_declined_mismatch:
        'The recipient didn’t accept this payment — it doesn’t match their request. Scan their code again.',
      local_pay_declined_already_paid: 'That request has already been paid. Nothing was sent.',
      local_pay_declined_save: 'The recipient couldn’t save the payment. Nothing was sent — try again.',
      local_pay_declined_decode: 'The recipient couldn’t read the payment. Nothing was sent — try again.',
      local_pay_declined_not_covered:
        'The recipient couldn’t verify this payment against the issuer’s records. Nothing was sent.',
      local_pay_broadcast_pending: 'Sent. This device couldn’t get it processed yet — the recipient will.',
      local_pay_any_amount: 'Any amount',
      local_pay_choose_amount: 'Choose an amount',
      local_pay_enter_amount_send: 'How much would you like to send?',
      local_pay_qr_unavailable: 'This code could not be created. Start the request again.',
      local_pay_presence_qr: 'By code — no live connection',
      local_pay_presence_ready: 'Encrypted nearby link ready',
      local_pay_presence_waiting_payee: 'Waiting for a nearby device',
      local_pay_presence_waiting_payer: 'Finding the recipient’s device…',
      local_pay_presence_linked: 'Encrypted link open',
      local_pay_presence_paid: 'Payment confirmed',
      pay_offline_title: 'You’re offline',
      pay_offline_body: 'Only nearby payments work right now. They will be processed once you reconnect.',
      pay_offline_queued: '{{count}} payment(s) waiting to be processed when you reconnect.',
      pay_offline_pending_title: 'Waiting to be processed',
      pay_offline_pending_body:
        '{{count}} payment(s) have not been processed yet. Nothing is settled until they do — this keeps retrying.',
      pay_offline_needs_internet: 'Needs internet',
      pay_offline_rejected_title: 'A payment was rejected',
      pay_offline_rejected_body: 'The network refused a payment handed over by {{sender}} via {{via}} on {{when}}.',
      pay_offline_unknown_sender: 'an unknown sender',
      pay_offline_unknown_via: 'an unknown method',
      pay_received_not_broadcast: 'Received offline · not yet processed',
      pay_offline_sent_rejected_title: 'A payment could not be delivered',
      pay_offline_sent_rejected_body: 'The network refused a payment you sent on {{when}}.',
      pay_offline_send_now: 'Send now',
      pay_offline_stalled_body: 'Some queued payments can’t be sent automatically: {{detail}}',
      offline_stall_no_request: 'A queued payment is missing its send record.',
      offline_stall_bad_beef: "A queued payment's transaction data couldn't be read.",
      offline_stall_foreign_ancestor:
        'A queued payment is waiting on a parent transaction another wallet never processed.',
      pay_offline_kv_pending: '{{count}} nearby payment(s) waiting to be added to this wallet.',
      pay_offline_kv_stuck: '{{count}} nearby payment(s) could not be added. Ask the sender to send again.',
      pay_offline_kv_corrupt: 'Damaged payment data was found on this device.',
      pay_offline_kv_corrupt_body:
        'The affected payments were set aside on this device, not deleted. Some of them may already have been added to your balance, so only ask a sender to send again if their payment is still missing.',
      pay_offline_show_code: 'Show code again',
      pay_parked_cancel: 'Cancel payment',
      pay_parked_already_sent: 'This payment was already sent',
      token_cancel_already_sent: "This payment already went through and can't be cancelled.",
      tx_status_parked: 'Not handed over',
      tx_status_import_hold: 'Held (imported)',
      resend_sending: 'Sending via message box',
      cancelling_payment: 'Cancelling the payment',

      // Contacts
      contacts: 'Contacts',
      contacts_search_placeholder: 'Search by name or handle',
      contacts_empty_title: 'No contacts yet',
      contacts_empty_body:
        "People you pay show up here once you save them. Scan someone's profile code with the button top right.",
      contacts_recent: 'Recent',
      contact_edit_name: 'Edit name',
      contact_save_name: 'Save name',
      contact_activity: 'Activity',
      contact_activity_empty: 'No activity with this contact yet',
      contact_delete: 'Delete contact',
      contact_delete_body: 'This removes {{name}} from your contacts. It does not affect any past payments.',
      contact_handle_caption: 'Their registered handle · only they can change it',
      contact_new_title: 'New Contact',
      contact_save: 'Save contact',
      contact_identifier: 'Identifier',
      contact_saved: 'Contact saved',
      contact_scan_hint: 'Scan a profile code to add someone',
      contact_identifier_placeholder: 'Paste their Identifier',
      contact_identifier_invalid: "That doesn't look like a valid Identifier",
      contact_add_from_payment: 'Add to contacts',

      // Profile
      profile: 'Profile',
      profile_display_name: 'Display name',
      profile_display_name_hint: 'Shown publicly next to your handle. Anyone who looks you up can see it.',
      profile_handle: 'Handle',
      profile_handle_placeholder: 'yourname',
      profile_handle_checking: 'Checking availability…',
      profile_handle_available: '{{handle}} is available',
      profile_handle_taken: '{{handle}} is already taken',
      profile_handle_invalid:
        '3-32 characters: a-z, 0-9, dot, underscore or hyphen, starting and ending with a letter or digit',
      profile_handle_failed: "Couldn't check availability. Try again.",
      profile_handle_unavailable: 'Handle registration is not available on this build yet.',
      profile_handle_registered: 'Handle registered',
      profile_handle_register_action: 'Register',
      profile_handle_too_similar: '{{handle}} is too close to one already in use',
      profile_handle_reserved: '{{handle}} is reserved',
      profile_handle_cooldown: '{{handle}} was released recently and is not free yet',
      profile_handle_pending: 'Finishing registration…',
      profile_handle_rolled_back: '{{handle}} was taken. You kept {{previous}}.',
      profile_handle_changed: 'Handle changed',
      profile_handle_rejected: 'The registry refused that handle.',
      profile_handle_clock_ahead: 'Your device clock is too far ahead. Try again in a few minutes.',
      profile_handle_wrong_registry: 'That handle is not on this registry. Open Profile again to refresh it.',
      profile_display_name_publish_failed: 'Your name was saved here, but the registry would not publish it.',
      profile_qr_hint: 'Others can scan this to add you as a contact',

      // Pay: 3-step (who / amount / review)
      pay_step_contacts: 'Contacts',
      pay_step_continue: 'Continue',
      pay_review_to: 'To',
      pay_review_amount: 'Amount',
      pay_review_note: 'Note',
      pay_review_note_edit: 'Add a note',
      pay_review_delivery_host: 'Delivering to',
      pay_trust_contact: 'Your contact',
      pay_trust_handle_attested: 'Registered',
      pay_trust_handle_domain_attested: 'Verified by {{domain}}',
      pay_trust_unverified: 'Not verified. Double-check this Identifier.',
      pay_address_network_mismatch: 'This address belongs to a different network from the one you are using.',
      contacts_your_contacts: 'Your contacts',
      contacts_other_people: 'Other people',
      contacts_not_saved: 'Not saved',
      contacts_network_footer: 'Found on the network. Not saved by you.',
      contact_no_handle: 'No handle yet',
      contact_no_handle_registered: 'No handle registered',
      contact_name_section: 'Your name for them',
      contact_name_hint: "Only you see this. It doesn't change what they call themselves.",
      contact_new_name_hint: 'Only you see this. Saved on this device.',
      contact_copy_identifier: 'Copy Identifier',
      profile_handle_registered_hint: 'Registered to your Identifier. People can find you by it.',
      profile_handle_change: 'Change',
      profile_handle_claim: 'Claim {{handle}}',
      profile_handle_replace_warning:
        'Claiming replaces {{handle}}. People who saved you keep their contact; the old handle stops resolving to you.',
      profile_identifier_section: 'Your Identifier',
      profile_show_qr: 'Show as QR',
      profile_show_qr_hint: 'Let someone scan you to get your profile',
      profile_qr_callout:
        "Scanning this opens BSV Wallet and adds you as a contact, with your name, handle and photo. It doesn't ask for a payment — that code is under Get paid.",
      pay_review_title: 'Review',
      pay_send_amount: 'Send {{amount}}',
      pay_sent_to: 'to {{name}}',
      pay_save_as_contact: 'Save as a contact'
    }
  },
  zh: {
    translation: {
      token_balances_header: '余额',
      wallet_coin_switcher: '选择要显示的币种',
      wallet_activity_empty_asset: '暂无 {{ticker}} 活动',
      pay_direction_pay_asset: '支付 {{ticker}}',
      token_fee_footer: '发送需要少量 BSV 支付网络手续费，接收则不需要。',
      token_new_tap: '新增 — 点按查看发行方',
      token_unresolved: '无法识别的代币',
      token_unresolved_sub: '此钱包无法查到发行方。请在联网后重试。',
      token_paused_short: '转账已暂停',
      token_frozen_short: '{{amount}} 已冻结',
      token_sheet_issuer: '发行方',
      token_sheet_issuer_unknown: '未识别',
      token_sheet_asset_id: '代币 ID',
      token_sheet_powers_header: '{{issuer}} 可以做什么',
      token_power_pause: '暂停所有 {{ticker}} 转账',
      token_power_freeze: '冻结特定币，包括你的',
      token_power_admit: '决定谁可以持有 {{ticker}}',
      token_power_replace: '用新币替换被冻结的币',
      token_powers_limit: '没有你，他们无法把 {{ticker}} 转出你的钱包；被冻结的币只是不能花费。',
      token_frozen_sub: '{{issuer}} 已冻结你余额中的 {{amount}} {{ticker}}。如有异议请联系对方。',
      pay_asset_label: '支付方式',
      pay_asset_label_get: '收款币种',
      pay_asset_cta: '发送 {{amount}} {{ticker}}',
      pay_asset_over_balance: '超过你的 {{ticker}} 余额。',
      pay_asset_needs_bsv: '发送 {{ticker}} 需要少量 BSV 支付网络手续费，接收则不需要。',
      pay_asset_get_bsv: '获取 BSV',
      pay_asset_needs_bsv_short: '需要 BSV 支付手续费',
      pay_asset_frozen_note: '你余额中的 {{amount}} {{ticker}} 已冻结，无法发送。',
      pay_asset_paused: '{{issuer}} 已暂停 {{ticker}} 转账，现在无法发送。',
      pay_asset_no_address: '{{ticker}} 只能发送给某个人或附近的设备，不能发送到比特币地址。',
      pay_asset_address_status: '{{ticker}} 不支持',
      pay_asset_self_unregistered: '{{issuer}} 尚未登记你发送 {{ticker}}。你仍然可以收款。',
      pay_asset_unidentified: '此钱包无法识别该代币，因此暂时无法发送。',
      pay_asset_recipient_unregistered: '{{issuer}} 尚未为此人登记 {{ticker}}，因此无法向其支付该代币。',
      pay_asset_recipient_blocked: '{{issuer}} 不允许向此人付款。',
      token_err_refused: '{{issuer}} 拒绝了此次转账。没有发送任何款项，你的 {{ticker}} 未变。',
      token_err_refused_paused: '{{issuer}} 已暂停 {{ticker}}。没有发送任何款项，你的余额未变。',
      token_err_refused_recipient: '{{issuer}} 不允许向此人付款。没有发送任何款项，你的余额未变。',
      token_err_refused_frozen: '你要发送的部分 {{ticker}} 刚刚被冻结。没有发送任何款项，余额已更新。',
      token_err_send_failed: '无法发送此付款：{{reason}}。没有任何款项离开你的钱包。',
      token_err_reason_unknown: '未知问题',
      token_err_unreachable: '我们无法联系 {{issuer}} 确认此次转账。再次尝试前请查看你的 {{ticker}} 余额。',
      token_err_check_again: '重新检查',
      token_err_balance_changed: '准备过程中你的 {{ticker}} 余额发生了变化。请检查金额后重试。',
      token_err_busy: '已有一笔 {{ticker}} 付款正在发送。',
      pay_sent_not_broadcast: '已发送。尚未到达网络，我们会继续尝试。',
      pay_sent_not_notified: '已发送。我们还无法通知对方 — 对方的钱包下次检查时会看到。',
      pay_sent_handed_to_wallet: '已发送。对方没有当面接收，因此我们改为发送到其钱包。',
      token_first_hold: '{{ticker}} 由 {{issuer}} 发行，对方可以暂停转账并冻结币。',
      pay_asset_request_paused: '{{issuer}} 已暂停 {{ticker}}，付款可能暂时无法到达。',
      token_admit_title: '你暂时还不能收取 {{ticker}}',
      token_admit_body: '{{issuer}} 只向已登记的人发送 {{ticker}}。在此之前，发给你的付款会在离开付款方钱包前被拒绝。',
      token_admit_bsv_instead: '改为以 BSV 收款',
      token_row_sent: '已发送 {{ticker}}',
      token_row_received: '已收到 {{ticker}}',
      token_row_amount_pending: '金额不可用',
      token_evicted_title: '部分 {{ticker}} 已被收回',
      token_evicted_body: '{{issuer}} 从你的钱包中收回了 {{amount}} {{ticker}}。',
      local_pay_token_not_cleared: '离线收到 · {{issuer}} 尚未确认',
      pay_asset_nearby_needs_network: '当面支付 {{ticker}} 需要网络连接。无网络时可以用 BSV 支付。',
      pay_asset_link_not_held: '此请求使用的代币不在本钱包中。你仍可以用 BSV 向对方付款。',
      pay_asset_nearby_wrong_overlay: '此请求使用的代币未在本钱包中配置。',
      token_messagebox_mismatch: '你的钱包设置了不同的消息箱，代币付款可能无法送达。',
      token_issuer_fallback: '发行方',
      token_row_hint: '打开发行方及其权限说明',
      token_attention_one: '正在等待 {{issuer}} 确认 — 点按查看详情',
      token_attention_many: '{{count}} 笔代币付款正在等待结算 — 点按查看详情',
      token_recv_reversed: '此付款未能获得 {{issuer}} 确认，已被撤销。',
      token_settle_paused_retry: '{{issuer}} 目前已暂停 {{ticker}} — 我们会继续尝试。',
      token_settle_unreachable_retry: '我们无法联系 {{issuer}} 确认此次转账。我们会继续尝试。',
      token_settle_evicted: '该付款未能及时到达网络，已被撤销 — 你的 {{ticker}} 已回到余额中。',
      token_frame_unreadable: '我们无法读取你收到的这笔付款 — 请让付款方重新发送。',
      token_chain_reversed: '此链条中较早的一笔付款被 {{issuer}} 拒绝，因此这笔也已被撤销。',
      token_settle_stuck_bound: '这笔付款已等待结算超过 3 天。你可以继续等待，或联系 {{issuer}}。',
      token_cover_failed: '此付款与 {{issuer}} 的记录不符，未被接受。',
      token_sent_settling: '已发送。正在与 {{issuer}} 结算 — 你或 {{payee}} 重新联网后即可看到确认。',
      token_sent_settling_unnamed: '已发送。正在与 {{issuer}} 结算 — 你们中任一方重新联网后即可看到确认。',
      token_sent_settled: '已发送 · 已与 {{issuer}} 结算',
      token_status_settling: '结算中',
      token_status_settled: '已结算',
      token_status_refused: '已拒绝',
      token_status_reversed: '已撤销',
      token_status_stuck: '等待结算',
      new_tab: '新标签',
      untitled: '无标题',
      share: '分享',
      back_to_homepage: '返回主页',
      bookmarks: '书签',
      add_bookmark: '添加书签',
      bookmark: '书签',
      unbookmark: '取消书签',
      find_in_page: '在页面中查找',
      find_in_page_placeholder: '在页面中查找...',
      find_in_page_matches: '{{current}} / {{total}}',
      find_in_page_no_matches: '无匹配项',
      history: '历史记录',
      clear_all: '全部清除',
      clear_all_tabs: '清除所有标签',
      search_or_enter_website: '搜索或输入网站',
      browser: '浏览器',
      bug_report: '错误报告',
      enable_web3: '启用 Web3',
      tabs: '标签',
      unlock_web3_features: '解锁 Web3 功能',

      cancel: '取消',
      done: '完成',
      discard: '放弃',
      deny: '拒绝',
      allow: '允许',
      save: '保存',
      remove: '移除',
      delete: '删除',
      search: '搜索',
      loading: '加载中...',
      continue: '继续',
      go_back: '返回',
      continue_action: '继续',
      camera_denied_description: '相机权限已被拒绝。您可以在设置中启用以使用此功能。',
      open_settings: '打开设置',
      copied: '已复制',
      explorer_open_failed: '无法打开区块浏览器',
      tx_action_refresh: '刷新状态',
      tx_action_refresh_short: '状态',
      tx_action_abort: '取消交易',
      copy: '复制',
      copy_to_clipboard: '复制到剪贴板',
      details: '详情',
      hide_details: '隐藏详情',

      identity: '身份',
      settings: '设置',
      wallet: '钱包',
      activity: '活动',
      configuration: '配置',
      data_and_security: '数据与安全',
      storage_row: '存储',
      storage_title: '钱包存储',
      storage_db_size: '数据库：{{size}}',
      storage_free_space: '可用空间：{{size}}',
      storage_reclaimable: '当前可回收：{{size}}，涉及 {{count}} 笔交易',
      storage_held_back: '{{count}} 笔交易因仍在使用而保留',
      storage_reclaim_explain:
        '回收会清除钱包不再需要的验证数据。您的交易、余额和历史不受影响。这会释放数据库文件内部的空间；文件本身在系统压缩前不会变小。',
      storage_reclaim_confirm: '回收',
      storage_reclaimed: '已回收 {{count}} 笔交易',
      storage_failed: '无法读取存储使用情况。',
      storage_unavailable: '存储尚未就绪。',
      backup_push_section: '私密备份',
      backup_push_toggle: '备份到服务器',
      backup_erase_row: '从服务器删除备份',
      backup_erase_title: '删除您的备份？',
      backup_erase_message:
        '此钱包发送到备份服务器的所有内容将被永久删除，涵盖所有设备。同时会关闭备份功能。您的币、密钥和助记词不受影响 — 但新设备将无法恢复此钱包的历史，且您已有的部分币在新设备上无法花费。',
      backup_erase_confirm: '删除',
      backup_erase_done: '备份已删除 — 移除 {{count}} 项',
      backup_erase_failed: '无法删除备份。未移除任何内容 — 请重试。',
      backup_erase_no_key: '无可用的钱包密钥',
      backup_push_disclosure:
        '钱包数据库的加密副本会发送到 BSV 协会的服务器，以便您在新设备上恢复。它使用只有您的助记词才能派生的密钥加密 — 运营方无法读取。您仍然需要助记词或打印的份额：没有它们，任何人（包括我们）都无法解锁备份。',
      backup_push_off_title: '关闭私密备份？',
      backup_push_off_message:
        '仅凭助记词无法重建此钱包。与保护您的币相关的信息仅存在于数据库中，因此关闭备份后，新设备可以恢复助记词但无法恢复资金。已在服务器上的数据不会被删除。',
      backup_push_off_confirm: '关闭',
      backup_push_on_toast: '备份已开启 — 正在同步',
      backup_push_off_toast: '备份已关闭 — 不再发送任何内容',
      permissions: '权限',
      trust_network: '信任网络',
      // vault (machine-translated, needs native review)
      vault_title: '保险库',
      vault_row_title: '保险库',
      vault_hero_title: '安全持仓',
      vault_hero_body: '适合长期持有的存储，锁定到您自己的 YubiKey。需要两把或更多 YubiKey 5 NFC。',
      vault_unsupported_title: '需要 YubiKey',
      vault_unsupported_body: '需要两把或更多 YubiKey 5 NFC，以及能读取它们的手机。',
      vault_restore_from_chain: '从区块链恢复保险库',
      vault_restore_scanning: '正在扫描区块链…',
      vault_restore_found: '在区块链上找到 {{count}} 笔保险库存款。',
      vault_restore_pending: '找到 {{count}} 笔存款，正在等待首次确认——请稍后重试。',
      vault_restore_none_found: '未在区块链上找到此钱包的保险库存款。',
      vault_balance_label: '保险库存有',
      vault_key_section: '安全密钥（{{count}} / 5）',
      vault_manage_section: '管理',
      vault_deposit_cta: '存入',
      vault_withdraw_cta: '提取',
      vault_deposit_title: '存入保险库',
      vault_withdraw_title: '从保险库提取',
      vault_deposit_sub: '来自您的日常余额。无需 YubiKey。',
      vault_withdraw_sub: '回到您的日常余额。轻触您的任意一把保险库密钥。',
      vault_deposit_done: '已转入保险库',
      vault_reclaim_done: '已从中断的保险库存入中恢复资金',
      vault_withdraw_done: '已从保险库提取',
      vault_withdraw_partial: '还剩 {{count}} 笔存入 — 再次取出以移动它们。',
      vault_withdraw_reason: '将 YubiKey 靠在这里以签名 — 从保险库提取 {{amount}} sats',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: '暂不可用 — 此版本已关闭保险库存入。',
      vault_not_on_mainnet_body: '保险库仅在主网上可用。请将钱包切换回主网后使用。',
      vault_intro_title: '设置您的保险库',
      vault_intro_what:
        '保险库将资金锁定到您自己的 YubiKey。保险库密钥的任何部分都不会存在于这部手机上；您的任何一把密钥都能打开它。',
      vault_intro_two_keys: '您至少需要两把 YubiKey。',
      vault_intro_apart: '请把它们放在不同的地方 — 放在一起的两把密钥等于一把。',
      vault_intro_backup: '请保持钱包的加密备份开启 — 它会保存经过验证的恢复记录。',
      vault_intro_ack:
        '我明白：只有我的 YubiKey 能打开这个保险库，我的恢复助记词不能。如果我丢失了全部密钥，这些钱就没了。',
      vault_intro_piv_ack:
        '我确认这把 YubiKey 的整个 PIV 应用已恢复出厂设置或专用于保险库。设置会更改全局 PIV PIN、PUK 和管理密钥，可能影响所有 PIV 槽位；我会安全记录新的 PUK。',
      vault_intro_begin: '开始',
      vault_key_step_title: '第 {{k}} 把密钥（最多 5 把）',
      vault_key_step_replace:
        '请使用专供保险库且已恢复出厂设置的 YubiKey。若其保险库槽位中已有旧密钥，设置会在替换前提示您。',
      vault_pin_choose_title: '设置 PIN 码',
      vault_pin_choose_sub: '每次使用此密钥时都需要输入。6 至 8 位数字。',
      vault_pin_confirm_label: '再次输入',
      vault_pin_mismatch: '两次输入的 PIN 码不一致。',
      vault_pin_not_default: '请选择 123456 以外的数字。',
      vault_puk_title: '您的恢复码',
      vault_puk_body: '请抄下并妥善保管。忘记 PIN 码时，这是解锁此密钥的唯一方式，且不会再次显示。',
      vault_puk_ack: '我已抄下',
      vault_reset_offer: '重置此密钥',
      vault_reset_title: '要重置此密钥吗？',
      vault_reset_body:
        '此密钥曾被使用过。重置会抹除其 PIV 应用中的所有证书和密钥，而不仅仅是保险库槽位。任何依赖此密钥的功能都将失效。',
      vault_reset_ack: '我明白这会抹除此密钥上的所有内容',
      vault_reset_unknown_ack:
        '本设备无法确认这把密钥是否仍在为另一个身份的保险库提供保护。我明白抹除它可能会永久销毁那把密钥。',
      vault_reset_own_draft_ack:
        '此密钥中存放的是您先前未完成的那次尝试留下的密钥，本设备仍保有它的记录。我明白重置会将其抹除，那次尝试将无法继续。',
      vault_reset_nothing_erased: '尚未抹除任何内容。此密钥的保险库槽位并非空的，因此还需再确认一项。',
      vault_reset_confirm: '抹除并重置',
      vault_resetting: '正在重置密钥…',
      vault_reset_done: '密钥已重置。请再次轻触以继续。',
      vault_reset_uncertain: '重置未完成。此密钥可能已被抹除 — 在依赖它之前请先检查。',
      vault_err_reset_enrolled: '该密钥已是您保险库的一部分 — 重置它会销毁您的一把保险库密钥。',
      vault_err_reset_unreadable:
        '无法读取本设备上的保险库记录，因此无法证明此密钥不是您的保险库密钥之一。未抹除任何内容，重置已被拒绝。',
      vault_enrollment_resume_pin_hint:
        '恢复已保存的密钥需要您先前为它设定的 PIN，而不是新的 PIN。连续输错三次会锁定该密钥。',
      vault_setup_step: '第 {{n}} 步，共 {{total}} 步',
      vault_enrollment_resume: '继续已保存的密钥',
      vault_enrollment_reset_required:
        '这把 YubiKey 已不再处于出厂状态，因此无法按现状注册。必须先将其 PIV 应用擦除并恢复出厂状态，它才能保存保险库密钥。',
      vault_enrollment_state_uncertain:
        '一次设置尝试被中断，因此无法确定这把 YubiKey 被留在了什么状态。在其 PIV 应用被擦除并恢复出厂状态之前，设置不会再使用这把密钥。',
      vault_nfc_enroll_message: '将 YubiKey 靠在这里以完成设置',
      vault_nfc_adopt_message: '将这把恢复的 YubiKey 靠在这里以验证',
      vault_nfc_sign_batch: '将 YubiKey 靠在这里以签名 — 第 {{b}} 批，共 {{n}} 批',
      vault_key_use_different: '使用另一把 YubiKey',
      vault_key_setup_again: '重新设置',
      vault_name_default: '密钥 {{k}}',
      vault_more_title: '再添加一把密钥？',
      vault_more_body: '最多 5 把密钥。其中任何一把都能独立打开整个保险库。',
      vault_more_add: '再添加一把密钥',
      vault_more_finish: '完成',
      vault_more_need_two: '完成前请添加第二把密钥 — 只有一把密钥意味着无法恢复。',
      vault_leave_setup: '离开设置',
      vault_leave_title: '离开设置？',
      vault_leave_body: '您已设置的 {{count}} 把 YubiKey 尚未保存。它们仍保留各自的密钥，稍后可以再次添加。',
      vault_leave_confirm: '离开',
      vault_leave_stay: '留下',
      vault_done_body: '{{count}} 把密钥可以打开这个保险库。只有这些密钥能打开它 — 您的恢复助记词不能。',
      vault_done_cta: '完成',
      vault_add_key_row: '添加密钥',
      vault_add_key_done: '{{nickname}} 可以打开从现在起的存入。重新锁定保险库后，它就能打开全部资金。',
      vault_key_added_toast: '密钥已添加',
      vault_recovery_verified: '已在此设备上验证恢复的密钥',
      vault_relock_row: '重新锁定保险库',
      vault_relock_now: '立即重新锁定',
      vault_relock_choose: '您将用哪把密钥重新锁定？',
      vault_relock_reason: '请轻触您现有的密钥之一（{{names}}）— 不是刚添加的那把。',
      vault_relock_reason_generic: '将 YubiKey 靠在这里以签名 — 将保险库重新锁定到您当前的密钥',
      vault_relock_reason_remaining: '仅重新锁定到您剩余的密钥：{{names}}。',
      vault_key_pending_removal: '已移除 — 重新锁定以完成',
      vault_key_removal_awaiting_network: '正在移除 — 等待网络接受重新锁定',
      vault_removal_awaiting_network: '重新锁定已发送。网络接受后即可再次存入和提取。',
      vault_err_too_many_active_keys: '保险库最多锁定到五把密钥。请先移除一把，再存入、重新锁定或提取部分余额。',
      vault_key_section_over_limit: '安全密钥（{{count}}，请移除一把）',
      vault_add_key_done_over_limit: '已添加 {{nickname}}。保险库最多锁定到五把密钥，请移除它所替换的那把，然后重新锁定一次。',
      vault_relock_done: '保险库已重新锁定到当前密钥',
      vault_relock_capped: '还剩 {{count}} 笔存入 — 再次重新锁定以移动它们。',
      vault_relock_unreachable: '{{count}} 笔存入只能由 {{names}} 打开。请用其中一把密钥再次重新锁定。',
      vault_badge_missing: '{{count}} 笔存入尚未对 {{nickname}} 开放',
      vault_badge_removed: '{{count}} 笔存入仍可由已移除的密钥打开',
      vault_key_action_rename: '重命名',
      vault_key_action_remove: '移除',
      vault_rename_title: '重命名 {{nickname}}',
      vault_rename_save: '保存',
      vault_remove_title: '移除 {{nickname}}？',
      vault_remove_body:
        '钱包将不再使用 {{nickname}}。保险库中已有的资金在您重新锁定之前仍可由它打开（约 {{fee}} sats）。',
      vault_remove_only: '仅移除',
      vault_key_removed_toast: '密钥已移除',
      vault_export_explainer:
        '每笔保险库存入都会隐藏其盐值，直到花费时才会用到——锁定脚本中看不到任何相关信息。请保持加密备份开启，以便新设备恢复并验证使用您的 YubiKey 所需的密钥记录。',
      vault_footnote: '只有这些密钥能打开保险库。您的恢复助记词不能。',
      vault_floor_line: '最低存入 {{floorDisplay}}（{{floorSats}} sats）。创建一笔保险库存入约需 {{feeDisplay}}。',
      vault_first_deposit_title: '首次保险库存入',
      vault_first_deposit_body:
        '首次保险库存入 — {{amount}} 将只能由 {{count}} 把 YubiKey（{{names}}）打开。您的恢复助记词无济于事。',
      vault_backup_off_title: '请先开启私密备份',
      vault_backup_off_body:
        '钱包创建新的保险库输出前，必须开启私密备份。备份会保存恢复所需的经过验证的 YubiKey 记录和钱包历史。您仍可全部取出，而不创建保险库找零。',
      vault_backup_off_cta: '打开备份设置',
      vault_seed_not_preserved_title: '请先保存您的恢复短语',
      vault_seed_not_preserved_body:
        '您的保险库只能使用恢复短语加上一枚 YubiKey 来恢复。YubiKey 上不保存该短语的副本——如果在您记下它之前将其遗失，这些资金将无法恢复。请在存款前立即保存。',
      vault_seed_not_preserved_cta: '保存恢复短语',
      vault_choose_key: '您将轻触哪把密钥？',
      vault_remainder_title: '全部取出？',
      vault_remainder_body:
        '取出 {{amount}} 后将剩余 {{remainder}}，低于 100,000 sat 的保险库最低额。整个保险库将转入您的日常余额。',
      vault_remainder_all: '全部取出',
      vault_remainder_change: '修改金额',
      vault_unreachable_title: '保险库的一部分需要另一把密钥',
      vault_unreachable_body:
        '保险库的一部分需要另一把密钥 — 已移动 {{moved}}。{{count}} 笔存入（共 {{amount}}）只能由 {{names}} 打开。请用其中一把密钥再次取出。',
      vault_sign_progress: '已签名 {{signed}} / {{total}}',
      vault_err_pin_locked_enroll:
        '这把 YubiKey 的 PIN 已被锁定。请在 Yubico Authenticator 中用其 PUK 解锁，或改用另一把 YubiKey 设置。',
      vault_err_serial_mismatch_chosen:
        '这是 {{tappedName}}。您选择的是 {{chosenName}} — 请轻触它，或返回并选择 {{tappedName}}。',
      vault_err_key_already_enrolled: '您已经添加过这把 YubiKey（{{nickname}}）。请轻触另一把。',
      vault_err_not_released: '此版本已关闭保险库存入。',
      vault_err_not_on_mainnet: '保险库仅在主网上可用。请将钱包切换到主网后重试。',
      vault_err_backup_off: '创建新的保险库输出前，请先开启加密私密备份。您仍可全部取出。',
      vault_err_not_enough_keys: '此保险库的密钥少于两把。请重新设置。',
      vault_err_too_many_keys: '一个保险库最多可有五把密钥。',
      vault_err_last_keys: '保险库至少需要两把密钥。请先添加另一把再移除这把。',
      vault_err_relock_required: '请先重新锁定保险库，让您的其他密钥能打开每笔存入。',
      vault_err_action_pending: '保险库交易尚未完成。待其结束后再试。',
      vault_resolve_held_deposit_action: '完成中断的存款',
      vault_resolve_held_deposit_done: '存款问题已解决',
      vault_resolve_held_deposit_failed: '无法解决存款问题，请重试。',
      vault_resolve_held_deposit_none: '这里没有可解决的问题——请在"动态"标签页查看待处理的交易以重试或取消。',
      vault_err_key_not_committed: '{{nickname}} 无法打开此保险库中的任何存入。请使用您的另一把保险库密钥。',
      vault_err_key_cannot_cover:
        '{{nickname}} 可以打开保险库中 {{total}} 里的 {{reachable}}。请最多取出 {{reachable}}，或改用 {{otherNames}}。',
      vault_err_too_small_to_relock: '此保险库的余额不足 100,000 sats，太少而无法重新锁定。请改为取出后再存入。',
      vault_err_bad_version: '交易以错误的格式构建。尚未签名 — 请重试。',
      vault_err_chain_scan_failed: '无法完成保险库存款的区块链扫描。请检查网络连接后重试。',
      vault_shares_unavailable: '没有可打印的密钥材料。',
      vault_enroll_begin: '设置保险库',
      vault_enroll_phase_connecting: '正在读取您的密钥…',
      vault_enroll_phase_pin_check: '正在检查您的密钥…',
      vault_enroll_phase_checking_slot: '正在检查保险库槽位…',
      vault_enroll_phase_personalizing: '正在设置您的 PIN 码和恢复码…',
      vault_enroll_phase_generating: '正在创建保险库密钥 — 密钥闪烁时请轻触',
      vault_enroll_phase_challenging: '正在验证新的保险库密钥 — 密钥闪烁时请轻触',
      vault_enroll_phase_done: '完成',
      vault_touch_when_blinks: '密钥闪烁时请轻触金色触点。',
      vault_nfc_activation_hint:
        '首次使用这个 YubiKey？请先将它插入 USB-C 接口几秒钟——新密钥出厂时 NFC 处于关闭状态，插入后才会开启。',
      vault_continue: '继续',
      vault_back: '返回',
      vault_enrolled_toast: '保险库已就绪',
      vault_insert_key: '插入您的 YubiKey',
      vault_hold_key_nfc: '请将 YubiKey 贴近手机顶部',
      vault_keep_holding_nfc: '请继续贴着 — 正在解锁…',
      vault_unlocking_funds: '正在解锁您的资金',
      vault_unlocking_sub: '需要几秒钟。请保持应用打开。',
      vault_sending_to_network: '正在发送到网络',
      vault_sending_sub: '如果没有立即成功，我们会继续重试。',
      vault_reading_key: '正在读取密钥…',
      vault_enter_pin: '输入 YubiKey PIN',
      vault_pin_sub_nfc: '然后将密钥贴近手机顶部以完成操作。',
      vault_pin_sub_usb: '然后触摸金色触点以完成操作。',
      vault_touch_contact: '轻触金色触点',
      vault_unlock_cta: '解锁',
      vault_pin_retries: '剩余 {{count}} 次尝试',
      vault_locked: '保险库已锁定',
      vault_retry: '重试',
      vault_cancel: '取消',
      vault_dismiss: '关闭',
      vault_err_wrong_key: '这把 YubiKey 不是此保险库的密钥之一。',
      vault_err_serial_mismatch: '这把 YubiKey 不是此保险库的密钥之一（{{names}}）。',
      vault_err_touch_timeout: '未检测到轻触。请重试并轻触金色触点。',
      vault_err_pin_locked:
        '这把 YubiKey 的 PIN 已被锁定。请使用您的另一把保险库密钥，或在 Yubico Authenticator 中用其 PUK 解锁。',
      vault_err_pin_invalid: 'PIN 码错误。',
      vault_err_puk_invalid: 'PUK 码错误。',
      vault_err_puk_locked: '这把 YubiKey 的 PUK 已被锁定。请使用一把新的 YubiKey。',
      vault_err_ceremony_active: '另一个保险库操作仍在使用 YubiKey。请等待完成后重试。',
      vault_err_scope_changed: '此保险库操作期间钱包或网络已更改。请在当前钱包中重新开始。',
      vault_err_key_removed_mid_op: '密钥已被移除。请插入后重试。',
      vault_err_no_key: '未检测到 YubiKey。',
      vault_err_driver_unavailable: '此设备不支持 YubiKey。',
      vault_err_mgmt_key_custom: '此密钥设置了自定义管理密钥，无法自动使用。',
      vault_err_attestation_invalid: '无法验证此 YubiKey 的制造商证明。请使用 PIV 证明槽未被修改的原厂正品 YubiKey。',
      vault_err_generic: '出了点问题。请重试。',
      vault_err_template_invalid: '尝试访问保险库时出错。请重试。',
      vault_err_not_enrolled: '尚未设置保险库。',
      vault_err_pin_required: '请输入您的 PIN 码。',
      vault_err_unsupported_platform: '此设备不支持 YubiKey。',
      vault_err_slot_occupied: '密钥槽已被占用。',
      vault_replace_key_warning: '替换它将永久销毁现有的 P-256 私钥。任何需要该密钥的保险库资金可能变得无法花费。',
      vault_replace_key_confirm: '替换现有密钥',
      vault_resume_draft_confirm_title: '使用这个已准备好的密钥？',
      vault_resume_draft_confirm_body:
        '有一个先前准备好的密钥——{{nickname}} · …{{tail}}——已就绪待添加，但本设备刚才并未在卡片上验证它。只有在你确认这是你自己的密钥时才继续。',
      vault_resume_draft_confirm_use: '使用此密钥',
      vault_resume_draft_confirm_discard: '丢弃它',
      vault_err_enrollment_partial: '更改密钥后 YubiKey 设置中断。请按照重试说明操作；此密钥尚未添加到保险库。',
      vault_err_key_not_adopted: '在使用这把恢复的 YubiKey 提款前，请先在此设备上验证您持有它。',
      vault_err_too_many_inputs:
        '一次要移动的保险库存入过多。请先提取较小金额 — 每次提取也会整合保险库，下次即可移动更多。',
      vault_err_requires_online: '保险库转账需要网络连接。请重新连接后再试 — 保险库资金不会排队稍后发送。',
      vault_err_user_cancelled: '已取消。',
      vault_err_vault_empty: '保险库中还没有可提取的资金。',
      vault_err_amount_exceeds_balance: '金额超过了保险库中的余额。',
      vault_err_below_dust: '该金额太小，无法发送。',
      vault_err_no_transaction: '转账未能完成。请重试。',
      vault_err_nfc_lost: '连接已断开。请将密钥稳定贴在手机上。',
      vault_pin_invalid_retry: 'PIN 码错误 — 还剩 {{count}} 次机会。再次点击“提取”重试。',
      vault_pin_invalid_retry_generic: 'PIN 码错误。再次点击“提取”重试。',
      vault_disable_row: '停用保险库',
      vault_disable_title: '停用保险库？',
      vault_disable_message: '这将忘记这部手机上的保险库密钥列表。密钥仍保留在您的 YubiKey 上。',
      vault_disable_confirm: '停用',
      vault_disabled_toast: '保险库已停用',
      vault_disable_blocked_title: '请先提取资金',
      vault_disable_blocked_message:
        '保险库中仍有资金。请在停用前将全部资金提取到日常余额 — 否则这些资金将被锁定，无法再签名动用。',
      vault_ok: '确定',
      vault_mock_toggle: '使用模拟 YubiKey（开发）',
      vault_on: '开',
      vault_off: '关',
      vault_shares_word_count: '恢复份额仅支持十二词助记词。请改为手写抄录您的助记词。',
      scan_shares_legacy_title: '旧版备份格式',
      scan_shares_legacy_message:
        '这些份额采用旧格式，可以恢复日常余额，但无法打开保险库。如果您还保留着十二词恢复助记词，请改用它来恢复 — 它能恢复全部资产 — 之后再打印新的份额。如果助记词已遗失，请将资金转入新钱包并重新开始。',
      scan_shares_legacy_ack: '我知道了',

      delete_wallet: '删除钱包',
      delete_wallet_warning_title: '删除钱包？',
      delete_wallet_warning_body:
        '这将永久从此设备删除您的钱包密钥。如果您没有备份助记词或打印恢复份额，您将永远无法访问您的钱包。',
      delete_wallet_confirm: '删除钱包',
      delete_wallet_failed_title: '删除钱包失败',
      delete_wallet_failed_body: '未能从此设备完全删除您的钱包。部分密钥数据可能仍然存在。请重试。',
      bsv_network: 'BSV 网络',
      arc_endpoint: 'ARC 节点',
      arc_api_token: 'API 密钥',
      arc_custom_url: '自定义 URL',
      arc_apply: '应用',
      arc_reset_default: '重置为默认',
      arc_requires_token: '需要密钥',
      arc_url_https_required: '必须使用 https（本地测试可用 http://localhost）',
      recovery_phrase: '恢复密钥',
      you_have: '您有',
      switching: '切换中...',
      mainnet: '主网',
      testnet: '测试网',
      teratest: 'Teratest',
      identity_key: '标识符',
      transactions: '交易',

      no_transactions: '暂无交易',
      tx_status_confirmed: '已确认',
      tx_status_received: '已收到',
      tx_status_sent: '已发送',
      tx_status_spent: '已支出',
      tx_status_transferred: '已转移',
      tx_status_accepted: '已接受',
      tx_status_broadcasting: '处理中',
      tx_status_not_sent: '未发送',
      tx_status_unsigned: '草稿',
      tx_status_nonfinal: '待处理',
      tx_status_pending: '待处理',
      tx_status_failed: '失败',
      tx_status_offline_queued: '离线 · 已排队',
      tx_status_offline_sending: '离线 · 发送中',
      tx_status_offline_rejected: '离线 · 已拒绝',
      tx_copied: '交易已复制',
      tx_not_available: '交易不可用',
      tx_copy_failed: '复制交易失败',
      tx_abort_success: '交易已中止',
      tx_abort_failed: '中止交易失败',
      tx_abort_maybe_delivered: '无法取消 — 该付款可能已送达收款人',
      tx_proof_refreshed: '交易已确认',
      tx_proof_refresh_failed: '无法获取证明 — 交易可能尚未确认',
      tx_marked_failed: '未处理 — 已标记为失败并释放资金',
      tx_still_pending: '仍在等待 — 尚未确认',
      tx_action_explorer: '在区块浏览器中查看',
      tx_export_success: '已导出 {{count}} 笔交易',
      tx_export_failed: '导出交易失败',
      tx_export_csv: '导出 CSV',
      tx_export: '导出',
      activity_filter: '筛选活动',
      activity_filter_all: '全部',
      activity_search_placeholder: '搜索备注、姓名、@用户名',
      activity_search_clear: '清除搜索',
      activity_filter_no_match: '没有匹配的活动',
      wallet_balance_label: '余额',
      wallet_balance_you_have: '您有',
      wallet_balance_refresh: '刷新余额',
      wallet_group_today: '今天',
      wallet_group_yesterday: '昨天',
      back: '返回',
      wallet_payments: '支付',
      wallet_vault: '保险库',
      wallet_settings: '设置',
      wallet_activity: '活动',

      message_box_server: '消息箱服务器',
      message_box_required: '发送和接收基于身份的付款所必需的。',
      send_payment: '发送付款',
      recipient: '收款人',
      amount: '金额',
      enter_valid_amount: '请输入大于零的金额',
      display_currency: '显示货币',
      no_pending_payments: '没有待处理的付款',
      outgoing_payments: '已发出',
      payment_not_delivered: '未送达',
      payment_delivered: '已送达',
      retry: '重试',
      retry_failed: '重试失败',
      dismiss: '关闭',
      valid_identity_key: '已输入有效的标识符',
      valid_bsv_address: '已输入有效的 BSV 地址',
      searching: '搜索中...',
      unknown: '未知',
      from_prefix: '来自：',
      message_box_saved: '消息箱服务器已保存',
      message_box_removed: '消息箱服务器已删除',
      message_box_off: '关闭',
      message_box_off_hint: '消息箱服务器已关闭。请在“设置 › 高级”中开启。',
      enter_valid_url: '请输入有效的网址',
      checking_connection: '正在检查连接...',
      connection_failed: '连接失败',
      recipient_placeholder: '名称、标识符或 BSV 地址...',

      trust_updated: '信任网络已更新',
      failed_to_save: '保存失败',
      confirm_delete: '确认删除',
      confirm_delete_body: '您确定要删除这个认证机构吗？',
      order_certifiers_hint: '按优先级排列认证机构。应用程序将优先显示排名较高的认证机构。',
      certifiers: '认证机构',
      no_certifiers: '暂无认证机构。',
      add_provider: '添加提供者',
      unsaved_changes: '您有未保存的更改',
      duplicate_key_error: '具有此公钥的实体已在列表中！',
      get_provider_details: '获取提供者详情',
      validate_details: '验证详情',
      show_advanced: '显示高级设置',
      hide_advanced: '隐藏高级设置',
      description_length_error: '描述必须在5到50个字符之间',
      enter_domain_hint: '输入提供者域名',
      enter_details_hint: '输入提供者详情',

      receive: '接收',
      send: '发送',
      generating_address: '正在生成 BSV 地址...',
      imported: '已导入',
      unable_to_generate_address: '无法生成 BSV 地址',
      send_bsv: '发送 BSV',
      recipient_address: '收款人 BSV 地址',
      enter_bsv_address: '输入 BSV 地址',

      wallet_data: '钱包数据',
      create_new_wallet: '创建新钱包',
      generate_recovery_phrase_caption: '生成新的恢复密钥',
      import_existing_wallet: '导入现有钱包',
      paste_recovery_phrase: '粘贴您的恢复密钥',
      save_recovery_phrase_heading: '保存您的恢复密钥',
      save_recovery_phrase_btn: '保存恢复密钥',
      recovery_phrase_write_down: '按顺序写下这12个单词并将其存放在安全的地方。',
      acknowledgment_text: '我已保存我的恢复密钥，并了解丢失它将导致所有相关资金、代币和证书的永久损失。',
      enter_recovery_words: '输入 12 个助记词',
      restore_wallet_description: '输入您的恢复密钥以恢复钱包',
      backup_reminder_title: '您的钱包尚未备份',
      backup_reminder_body: '请导出密钥，以免资金丢失。',
      biometric_advisory_title: '保护您的钱包',
      wallet_balance_show_in: '以 {{unit}} 显示',
      tx_detail_block: '区块',
      tx_detail_block_pending: '等待打包进区块',
      biometric_advisory_body: '使用面容 ID 或指纹保护您的资产',
      biometric_advisory_body_degraded: '此设备未设置面容 ID 或指纹，因此您的钱包密钥仅由设备密码保护。',
      import_prompt_title: '已经有钱包了？',
      import_prompt_body: '使用恢复密钥或恢复份额还原钱包。',
      import_prompt_action: '从备份导入',
      nearby_advisory_title: '连接附近设备',
      nearby_advisory_body:
        '系统会请求您允许使用 Wi-Fi、蓝牙或 NFC。这仅用于直接连接附近的设备完成支付 — 绝不会发送到我们的服务器，也绝不会用来建立您的画像。',
      backup_reminder_now: '立即备份密钥',
      backup_reminder_import: '从备份导入',
      restore_backup_checking: '正在查找您的加密备份…',
      restore_backup_progress: '正在恢复钱包历史 — {{chunks}}/{{total}}…',
      restore_backup_failed_title: '无法恢复历史记录',
      restore_backup_failed_message:
        '您的助记词没有问题，但加密备份无法重放。通常重试一次即可。若继续，钱包可用但没有历史交易，且您已有的部分币将无法花费。',
      restore_backup_retry: '重试',
      restore_backup_skip: '放弃',
      restore_backup_unverified_title: '备份可能不完整',
      restore_backup_unverified_body:
        '您的钱包已恢复，但服务器上的备份无法确认是否完整。如果缺少部分历史记录，请在另一台设备完成上传后再次恢复。',
      import_wallet: '导入钱包',

      reject: '拒绝',
      authorize: '授权',
      renewal: '续订',
      spending_section: '消费',
      protocols_section: '协议',
      baskets_section: '篮子',
      certificates_section: '证书',
      requested_fields: '请求的字段',

      send_max: '发送最大值',
      message_box_unreachable: '无法连接支付服务器。请在设置中检查消息盒 URL。',
      available: '可用',
      note: '备注',
      note_placeholder: '用途？（可选）',
      entire_wallet_balance: '全部钱包余额',
      entire_vault_balance: '保险库全部余额',
      export_wallet_data: '导出钱包数据',
      export_unencrypted_title: '此文件未加密',
      export_unencrypted_message:
        '导出的文件是钱包数据的明文未加密副本，包含身份和证书信息、交易记录及联系人。任何拿到此文件的人都能读取其内容。请只将其分享到您信任的地方。',
      import_wallet_data: '导入钱包数据',
      import_conflict_title: '数据库已存在',
      import_conflict_message:
        '此身份已存在更近时间戳的钱包数据库。文件名中数字最大的备份将始终被使用。\n\n要使用此旧备份：先导出当前钱包数据，删除并重新安装应用，然后仅导入所需的备份。',
      import_confirm_title: '导入数据库',
      import_confirm_message: '此数据库将成为您的活动钱包。继续？',
      import_success: '数据库导入成功。正在重启钱包...',
      import_invalid_file: '无效的备份文件',
      import_invalid_file_detail:
        '此文件不是有效的钱包数据库备份。预期文件名格式如 wallet-XXXXXXXX-mainnet-TIMESTAMP.db。',
      import_oversized_file: '备份文件过大',
      import_oversized_file_detail: '此文件大小超出钱包备份应有的范围，因此未被读取。请选择真实的钱包数据库备份文件。',
      import_untrusted_file: '无法导入此备份',
      import_untrusted_file_detail: '此备份无法验证为属于此钱包,或其内容不可信。未导入。',
      import_anyway: '仍然导入',
      import_invalid_input_title: '无效输入',
      import_invalid_input_message: '请输入有效的恢复短语（12-24 个单词）或 64 位十六进制私钥。',
      import_setup_failed: '设置钱包失败：{{error}}',
      create_wallet_refused: '无法创建钱包，请重试。',
      create_wallet_failed: '生成助记词失败，请重试。',
      or: '或',
      print_recovery_shares: '打印恢复份额',
      scan_backup_shares: '扫描备份份额',
      scan_backup_shares_caption: '从打印的份额恢复钱包',
      scan_shares_invalid_format: '无效的份额格式。请扫描有效的备份份额二维码。',
      scan_shares_camera_needed: '需要相机权限',
      scan_shares_camera_description: '需要相机权限来扫描您打印的备份份额二维码。',
      scan_shares_grant_camera: '授予相机权限',
      scan_shares_recovering: '正在恢复钱包...',
      scan_shares_scan_first: '扫描您的第一个备份份额',
      scan_shares_progress: '已扫描 {{scanned}}/{{needed}} 个份额',
      scan_shares_remaining: '还需要 {{count}} 个份额',
      scan_shares_complete: '所有份额已收集！',
      scan_shares_recovery_failed: '恢复失败。请重试扫描。',
      scan_shares_threshold_mismatch: '阈值与之前的份额不匹配',
      scan_shares_integrity_mismatch: '完整性哈希值不匹配 — 份额来自不同的密钥',
      scan_shares_duplicate: '此份额已经扫描过',
      scan_shares_biometric_title: '需要生物识别验证',
      scan_shares_biometric_message: '需要生物识别验证以保护您的钱包密钥。请重试。',
      recovery_replace_wallet_title: '要替换此手机上的钱包吗？',
      recovery_replace_wallet_body:
        '此手机已保存一个钱包。继续操作将替换它以及它能花费的所有资金。如果它的恢复助记词没有保存在别处，那笔钱就会永久丢失。',
      recovery_replace_wallet_confirm: '替换钱包',
      invalid_bsv_address: '无效的 BSV 地址',
      scan_recipient_hint: '将相机对准付款码',
      scan_qr_default_hint: '将相机对准二维码',
      scan_wallet_qr_hint: '将相机对准钱包配对二维码',

      biometric_store_wallet: '安全存储您的钱包',
      biometric_load_wallet: '安全加载您的钱包',
      biometric_unlock_wallet: '解锁您的钱包',
      wallet_lost_title: '钱包密钥已被销毁',
      wallet_lost_body:
        '此设备的生物识别设置已更改，系统因此销毁了保护此钱包的密钥。您的币在区块链上是安全的——请使用助记词恢复此钱包。',
      wallet_lost_restore: '恢复',
      wallet_lost_later: '暂不',
      wallet_locked_title: '钱包已锁定',
      wallet_locked_body: '解锁以加载您的钱包密钥。',
      wallet_unlock_retry: '解锁钱包',
      wallet_lockout_body: '尝试次数过多。请先用密码解锁手机，然后重试。',
      wallet_no_biometrics_body: '此设备未设置生物识别。您的钱包密钥仍由设备密钥库加密，但没有生物识别保护。',

      // Connections screen
      connections: '连接',
      advanced: '高级设置',
      copy_secret_words: '复制助记词',
      print_recovery_keys: '打印恢复密钥',
      backup_wallet_keys: '备份钱包密钥',
      log_out: '退出登录',
      connect_to_app: '连接应用',
      no_connections_yet: '暂无连接',
      no_connections_subtitle: '扫描桌面应用的二维码以连接您的钱包',
      connections_active: '活跃',
      connections_disconnected: '已断开',
      disconnect: '断开连接',
      reconnect: '重新连接',
      reconnect_failed: '重新连接失败',
      reconnect_confirm_title: '重新连接到此网站？',
      reconnect_confirm_message: '{{origin}} 将再次能够访问您的钱包。',
      reconnect_needs_reapproval: '无法验证此连接。请扫描新的二维码重新连接。',
      scan_qr_code: '扫描二维码',
      paste_uri: '粘贴 URI',
      invalid_qr_code: '无效的二维码',
      wallet_not_ready: '钱包未就绪',
      please_log_in_first: '请先登录',
      unknown_error: '未知错误',

      // Pay screen
      payments: '支付',
      pay: '支付',
      pay_direction_pay: '付款',
      pay_direction_receive: '收款',
      pay_cell_nearby_pay: '附近的人',
      pay_cell_nearby_pay_sub: '扫描对方的二维码',
      pay_cell_handle_pay: '远程的某人',
      pay_cell_handle_pay_sub: '选择一个名称 — 对方需要安装此应用',
      pay_cell_address_pay: '到 BSV 地址',
      pay_cell_address_pay_sub: '粘贴或扫描 BSV 地址',
      pay_cell_nearby_get: '附近的人',
      pay_cell_nearby_get_sub: '出示您的收款二维码',
      pay_cell_handle_get: '远程的某人',
      pay_cell_handle_get_sub: '分享您的名称',
      pay_cell_address_get: '到 BSV 地址',
      pay_cell_address_get_sub: '出示 BSV 地址',
      pay_method: '方式',
      pay_method_nearby: '附近',
      pay_method_remote_link: '分享远程链接',
      pay_method_address: '到 BSV 地址',
      pay_pre_nearby: '双方都在现场。',
      pay_pre_handle: '对方使用此应用。',
      pay_pre_address: '您有对方的 BSV 地址。',
      pay_conseq_nearby: '几秒内完成。',
      pay_conseq_handle: '对方的钱包下次检查时到账。',
      pay_conseq_address: '款项直接发出 — 对方不会收到通知。',
      paid: '已支付',
      pay_inbox_attention: '无法加入钱包',
      pay_inbox_attention_hint: '这些款项已到达，但钱包无法将其加入。请重试，或忽略以彻底放弃。',
      pay_dismiss_confirm: '再次点按即放弃',
      pay_dismissed: '已放弃该款项',
      payment_arrived: '已收到付款',
      payment_arrived_damaged: '此付款到达时已损坏',
      discard_payment_title: '放弃这笔付款？',
      discard_payment_body: '系统会请付款方重新发送付款详情。在此之前，这笔钱仍留在链上。',
      discard_void_title: '这笔付款未能完成',
      discard_void_body: '它已被取消，或者资金已在别处花掉。可以安全忽略。',
      waiting_network_confirm: '等待网络确认这笔付款',
      finish_or_cancel_outgoing: '请先完成或取消下方的付款，再发起新的付款',
      identity_search_unavailable: '搜索暂时不可用。请稍后再试。',
      abandon_payment: '放弃付款',
      finish_payment: '完成付款',
      cancel_this_payment: '取消这笔付款？',
      cancel_payment: '取消付款',
      resend: '重新发送',
      resending: '正在重新发送…',
      send_payment_details_again: '重新发送详情',
      tx_action_resend_short: '重新发送',
      tx_action_request_confirmation: '请求收款人确认',
      resend_sent: '已重新发送付款详情',
      resend_failed_no_record: '本机未找到该付款的详细信息，无法重新发送。',
      resend_failed_no_recipient: '该付款没有记录收款方，无法重新发送。',
      resend_failed_no_transaction: '无法获取该付款的数据。请在联网后重试。',
      resend_requested: '对方请您重新发送一笔付款',
      request_again: '重新请求',
      send_again: '重新发送',
      copy_details: '复制详情',
      payment_bounced_resend: '已请对方重新发送这笔付款',
      dismiss_rejected_payment: '已忽略这笔被拒付款',
      activity_load_failed: '无法加载活动记录',
      activity_load_retry: '点击重试',
      activity_view_contact: '查看联系人',
      home_payments_need_attention: '{{count}} 笔付款需要处理',
      home_payments_unsent: '{{count}} 笔付款未送达',
      pay_copy: '复制',
      pay_share_link: '分享链接',
      pay_address_watching: '发送到此地址的款项会自动添加到您的钱包。',
      pay_address_earlier_day: '回到更早的日期',
      pay_address_sweep_now: '立即检查此 BSV 地址',
      pay_address_swept: '已收到 {{amount}}',

      check_wallet: '疑难解答',
      settings_version: 'v{{version}}',
      error_review_actions: '这笔付款卡住了。“疑难解答”可以尝试解除。',
      wallet_check_ok: '一切正常',
      wallet_check_attention: '有些检查需要你处理',
      wallet_check_couldnt: '无法检查',
      wallet_check_couldnt_body: '某个修复步骤未完成。我们没有对您的币做任何假设。',
      wallet_check_summary: '释放了 {{freed}} 个卡住的币 · 恢复了 {{recovered}} 笔付款',
      wallet_check_step_coins: '你的币有效',
      wallet_check_skip: '跳过此项检查',
      wallet_check_skipped: '已跳过',
      wallet_check_step_proofs: '交易证明',
      wallet_check_step_records: '本地数据库记录',
      wallet_check_step_tokens: '稳定币转账已核验',
      wallet_check_step_online: '网络连接',
      wallet_check_step_missed: '遗漏的付款',
      wallet_check_step_backup: '私密备份已上传',
      wallet_check_step_phrase: '恢复短语已保存',
      wallet_check_progress: '{{current}} / {{total}}',
      wallet_check_stuck_title: '无法发送这笔付款',
      wallet_check_stuck_message: '有些币似乎卡住了。“疑难解答”可以释放它们。',
      wallet_check_failed: '检查未能完成。请重试。',

      // Local payments
      local_pay_request: '请求付款',
      local_pay_send: '发送付款',
      local_pay_amount: '金额',
      local_pay_show_qr: '请向付款方出示此二维码',
      local_pay_scan_qr: '扫描收款方的二维码',
      local_pay_scan_payer_qr: '扫描付款方的二维码',
      local_pay_scan_progress: '已接收 {{have}}/{{total}} 片段',
      local_pay_show_payment_qr: '请向收款方出示此二维码',
      local_pay_animated_hint: '请对方扫描下方的动态二维码——它会持续变化，直到对方手机接收完整。',
      local_pay_show_qr_instead: '改用二维码',
      local_pay_preparing: '正在准备请求…',
      local_pay_delivering: '正在发送付款…',
      local_pay_saving: '正在保存付款…',
      local_pay_sent: '付款已发送',
      local_pay_received: '已收到付款',
      local_pay_added: '已添加到您的钱包。',
      local_pay_added_multiple: '已将 {{count}} 笔付款添加到您的钱包。',
      local_pay_received_confirming: '正在与您的钱包确认…',
      local_pay_received_not_credited: '尚未添加到您的钱包。交出任何东西之前，请先查看"动态"。',
      local_pay_decline_broadcast_title: '一笔被拒绝的付款已经上链',
      local_pay_decline_broadcast_body: '一笔已被拒绝的付款后来出现在网络上。请勿再次发送——请查看"动态"。',
      local_pay_pending_abort_dropped_title: '已忽略一条未经验证的释放请求',
      local_pay_pending_abort_dropped_body: '一条已保存的付款释放记录无法验证，因此未释放任何内容。如果某笔付款仍显示卡住，请在"动态"中取消它。',
      local_pay_cancel_unverifiable_title: '离线状态下无法核实此付款',
      local_pay_cancel_unverifiable_body:
        '如果对方已经扫描过此付款，取消会让这些币被双重花费。只有在确定它从未被扫描过时才能取消。',
      local_pay_cancel_unverifiable_confirm: '仍要取消',
      local_pay_queued: '已保存 — 稍后会自动添加到您的钱包。',
      local_pay_already_paid: '此请求已经支付过了。',
      local_pay_failed: '付款失败',
      local_pay_network_denied: '本地网络访问已关闭。请在“设置”中开启，才能向附近的设备付款。',
      local_pay_radio_fallback: '无线连接不可用——请改为向对方出示此码。',
      local_pay_floor_peer_bt_off: '对方的蓝牙已关闭，此付款将通过二维码完成。',
      local_pay_floor_local_bt_off: '你的蓝牙已关闭。开启后可获得更快的近距离连接。',
      local_pay_floor_local_ble_denied: 'BSV Wallet 的蓝牙访问权限已关闭。请在“设置”中允许，以获得更快的近距离连接。',
      local_pay_floor_cross_os: '对方设备目前无法使用蓝牙，此付款将通过二维码完成。',
      local_pay_floor_peer_no_radio: '对方设备未在监听无线连接，此付款将通过二维码完成。',
      local_pay_ble_unavailable: '蓝牙不可用。上方的二维码仍然有效。',
      local_pay_too_large: '此付款数据过大，无法通过二维码传输。',
      local_pay_nearby_unavailable: '附近 Wi-Fi 不可用。上方的二维码仍然有效。',
      local_pay_retry_save: '重新尝试保存',
      local_pay_wrong_session: '该付款与此请求不匹配，未被接受。请让付款方重新扫描上方的二维码。',
      local_pay_declined_mismatch: '收款方未接受这笔付款 — 它与对方的请求不匹配。请重新扫描对方的二维码。',
      local_pay_declined_already_paid: '该请求已经支付过了，本次未发送任何款项。',
      local_pay_declined_save: '收款方无法保存这笔付款。本次未发送任何款项 — 请重试。',
      local_pay_declined_decode: '收款方无法读取这笔付款。本次未发送任何款项 — 请重试。',
      local_pay_declined_not_covered: '收款方无法根据发行方的记录核验这笔付款。本次未发送任何款项。',
      local_pay_broadcast_pending: '已发送。本设备暂时无法完成处理 — 收款方会完成。',
      local_pay_any_amount: '任意金额',
      local_pay_choose_amount: '选择金额',
      local_pay_enter_amount_send: '您想支付多少金额？',
      local_pay_qr_unavailable: '无法生成此二维码。请重新发起请求。',
      local_pay_presence_qr: '通过二维码交付 — 没有实时连接',
      local_pay_presence_ready: '附近的加密连接已就绪',
      local_pay_presence_waiting_payee: '正在等待附近的设备',
      local_pay_presence_waiting_payer: '正在查找收款人的设备…',
      local_pay_presence_linked: '加密连接已建立',
      local_pay_presence_paid: '付款已确认',
      pay_offline_title: '已离线',
      pay_offline_body: '目前仅附近支付可用。重新联网后会自动处理。',
      pay_offline_queued: '{{count}} 笔付款正等待您重新联网后处理。',
      pay_offline_pending_title: '等待处理',
      pay_offline_pending_body: '{{count}} 笔付款尚未处理。在处理完成前一切都未完成，系统会持续重试。',
      pay_offline_needs_internet: '需要联网',
      pay_offline_rejected_title: '一笔付款被拒绝',
      pay_offline_rejected_body: '网络拒绝了由 {{sender}} 于 {{when}} 通过 {{via}} 转来的付款。',
      pay_offline_unknown_sender: '未知发送者',
      pay_offline_unknown_via: '未知方式',
      pay_received_not_broadcast: '离线收到 · 尚未处理',
      pay_offline_sent_rejected_title: '一笔付款未能送达',
      pay_offline_sent_rejected_body: '网络拒绝了您于 {{when}} 发送的付款。',
      pay_offline_send_now: '立即发送',
      pay_offline_stalled_body: '部分排队付款无法自动发送：{{detail}}',
      offline_stall_no_request: '队列中的一笔付款缺少发送记录。',
      offline_stall_bad_beef: '无法读取队列中某笔付款的交易数据。',
      offline_stall_foreign_ancestor: '队列中的一笔付款正在等待另一个钱包始终未处理的父交易。',
      pay_offline_kv_pending: '有 {{count}} 笔附近付款等待加入此钱包。',
      pay_offline_kv_stuck: '有 {{count}} 笔附近付款无法添加。请让付款方重新发送。',
      pay_offline_kv_corrupt: '在此设备上发现了损坏的付款数据。',
      pay_offline_kv_corrupt_body:
        '受影响的付款已被移到此设备上的一旁保存，并未删除。其中一些可能已经计入你的余额，因此只有在某笔付款仍然缺失时，才请付款方重新发送。',
      pay_offline_show_code: '再次显示付款码',
      pay_parked_cancel: '取消付款',
      pay_parked_already_sent: '此付款已发送',
      token_cancel_already_sent: '此付款已经完成，无法取消。',
      tx_status_parked: '尚未交付',
      tx_status_import_hold: '已导入并暂扣',
      resend_sending: '正在通过消息盒发送',
      cancelling_payment: '正在取消付款',

      contacts: '联系人',
      contacts_search_placeholder: '按姓名或用户名搜索',
      contacts_empty_title: '暂无联系人',
      contacts_empty_body: '您付款的对象保存后会显示在这里。使用右上角的按钮扫描对方的个人资料码。',
      contacts_recent: '最近',
      contact_edit_name: '编辑姓名',
      contact_save_name: '保存姓名',
      contact_activity: '活动记录',
      contact_activity_empty: '与该联系人暂无活动记录',
      contact_delete: '删除联系人',
      contact_delete_body: '这将从您的联系人中移除 {{name}},不会影响任何过往付款。',
      contact_handle_caption: '对方注册的用户名 · 仅对方可以更改',
      contact_new_title: '新建联系人',
      contact_save: '保存',
      contact_identifier: '标识符',
      contact_saved: '联系人已保存',
      contact_scan_hint: '扫描个人资料码以添加联系人',
      contact_identifier_placeholder: '粘贴对方的标识符',
      contact_identifier_invalid: '这不是有效的标识符',
      contact_add_from_payment: '添加到联系人',

      profile: '个人资料',
      profile_display_name: '显示名称',
      profile_display_name_hint: '会公开显示在你的用户名旁边。任何查找你的人都能看到。',
      profile_handle: '用户名',
      profile_handle_placeholder: '你的用户名',
      profile_handle_checking: '正在检查可用性…',
      profile_handle_available: '{{handle}} 可以使用',
      profile_handle_taken: '{{handle}} 已被占用',
      profile_handle_invalid: '3-32 个字符：a-z、0-9、点、下划线或连字符，且以字母或数字开头和结尾',
      profile_handle_failed: '无法检查可用性,请重试。',
      profile_handle_unavailable: '此版本尚不支持注册用户名。',
      profile_handle_registered: '用户名已注册',
      profile_handle_register_action: '注册',
      profile_handle_too_similar: '{{handle}} 与已有的名称过于相似',
      profile_handle_reserved: '{{handle}} 是保留名称',
      profile_handle_cooldown: '{{handle}} 最近被释放，暂时还不能使用',
      profile_handle_pending: '正在完成注册…',
      profile_handle_rolled_back: '{{handle}} 已被占用，已为你保留 {{previous}}。',
      profile_handle_changed: '用户名已更改',
      profile_handle_rejected: '注册服务拒绝了该用户名。',
      profile_handle_clock_ahead: '你的设备时间太超前了。请过几分钟再试。',
      profile_handle_wrong_registry: '该用户名不在此注册服务上。请重新打开“个人资料”以刷新。',
      profile_display_name_publish_failed: '你的名称已保存在本机，但注册服务未能公开发布。',
      profile_qr_hint: '他人可扫描此码将您添加为联系人',

      pay_step_contacts: '联系人',
      pay_step_continue: '继续',
      pay_review_to: '收款人',
      pay_review_amount: '金额',
      pay_review_note: '备注',
      pay_review_note_edit: '添加备注',
      pay_review_delivery_host: '发送至',
      pay_trust_contact: '您的联系人',
      pay_trust_handle_attested: '已注册',
      pay_trust_handle_domain_attested: '已由 {{domain}} 验证',
      pay_trust_unverified: '未验证。请仔细核对此标识符。',
      pay_address_network_mismatch: '此地址属于与您当前使用的网络不同的网络。',
      contacts_your_contacts: '你的联系人',
      contacts_other_people: '其他人',
      contacts_not_saved: '未保存',
      contacts_network_footer: '在网络上找到。你尚未保存。',
      contact_no_handle: '尚无用户名',
      contact_no_handle_registered: '未注册用户名',
      contact_name_section: '你对他们的称呼',
      contact_name_hint: '只有你能看到。这不会改变他们对自己的称呼。',
      contact_new_name_hint: '只有你能看到。保存在此设备上。',
      contact_copy_identifier: '复制标识符',
      profile_handle_registered_hint: '已注册到你的标识符。他人可以通过它找到你。',
      profile_handle_change: '更改',
      profile_handle_claim: '认领 {{handle}}',
      profile_handle_replace_warning: '认领将替换 {{handle}}。已保存你的人仍保留联系人；旧用户名将不再指向你。',
      profile_identifier_section: '你的标识符',
      profile_show_qr: '显示为二维码',
      profile_show_qr_hint: '让他人扫描以获取你的资料',
      profile_qr_callout:
        '扫描后会打开 BSV Wallet 并将你添加为联系人，包含你的名字、用户名和头像。它不会请求付款——付款码在“收款”中。',
      pay_review_title: '确认',
      pay_send_amount: '发送 {{amount}}',
      pay_sent_to: '给 {{name}}',
      pay_save_as_contact: '保存为联系人'
    }
  },
  hi: {
    translation: {
      token_balances_header: 'शेष राशि',
      wallet_coin_switcher: 'दिखाने के लिए सिक्का चुनें',
      wallet_activity_empty_asset: 'अभी तक कोई {{ticker}} गतिविधि नहीं',
      pay_direction_pay_asset: '{{ticker}} भुगतान करें',
      token_fee_footer: 'भेजने के लिए नेटवर्क शुल्क हेतु थोड़ा BSV चाहिए। प्राप्त करने के लिए नहीं।',
      token_new_tap: 'नया — जारीकर्ता देखने के लिए टैप करें',
      token_unresolved: 'अपरिचित टोकन',
      token_unresolved_sub: 'यह वॉलेट जारीकर्ता का पता नहीं लगा सका। ऑनलाइन होने पर फिर से कोशिश करें।',
      token_paused_short: 'स्थानांतरण रोके गए',
      token_frozen_short: '{{amount}} फ़्रीज़',
      token_sheet_issuer: 'जारीकर्ता',
      token_sheet_issuer_unknown: 'पहचाना नहीं गया',
      token_sheet_asset_id: 'टोकन आईडी',
      token_sheet_powers_header: '{{issuer}} क्या कर सकता है',
      token_power_pause: '{{ticker}} के सभी स्थानांतरण रोकना',
      token_power_freeze: 'विशिष्ट सिक्के फ़्रीज़ करना, आपके भी',
      token_power_admit: 'तय करना कि {{ticker}} कौन रख सकता है',
      token_power_replace: 'फ़्रीज़ किए गए सिक्के को नए से बदलना',
      token_powers_limit:
        'वे आपके बिना {{ticker}} आपके वॉलेट से बाहर नहीं भेज सकते, और फ़्रीज़ सिक्का बस खर्च नहीं हो सकता।',
      token_frozen_sub:
        '{{issuer}} ने आपके शेष में से {{amount}} {{ticker}} फ़्रीज़ कर दिए हैं। विवाद के लिए उनसे संपर्क करें।',
      pay_asset_label: 'किससे भुगतान',
      pay_asset_label_get: 'किसमें भुगतान लेना',
      pay_asset_cta: '{{amount}} {{ticker}} भेजें',
      pay_asset_over_balance: 'आपके {{ticker}} शेष से अधिक।',
      pay_asset_needs_bsv: '{{ticker}} भेजने के लिए नेटवर्क शुल्क हेतु थोड़ा BSV चाहिए। प्राप्त करने के लिए नहीं।',
      pay_asset_get_bsv: 'BSV लें',
      pay_asset_needs_bsv_short: 'शुल्क के लिए BSV चाहिए',
      pay_asset_frozen_note: 'आपके शेष में से {{amount}} {{ticker}} फ़्रीज़ है और भेजा नहीं जा सकता।',
      pay_asset_paused: '{{issuer}} ने {{ticker}} स्थानांतरण रोक दिए हैं, इसलिए यह अभी नहीं भेजा जा सकता।',
      pay_asset_no_address: '{{ticker}} केवल किसी व्यक्ति या पास के डिवाइस को भेजा जा सकता है — बिटकॉइन पते पर नहीं।',
      pay_asset_address_status: '{{ticker}} के लिए उपलब्ध नहीं',
      pay_asset_self_unregistered:
        '{{issuer}} ने आपको {{ticker}} भेजने के लिए पंजीकृत नहीं किया है। आप फिर भी इसमें भुगतान ले सकते हैं।',
      pay_asset_unidentified: 'यह वॉलेट इस टोकन को पहचान नहीं सका, इसलिए अभी इसे भेज नहीं सकता।',
      pay_asset_recipient_unregistered:
        '{{issuer}} ने इस व्यक्ति को {{ticker}} के लिए अभी पंजीकृत नहीं किया है, इसलिए उन्हें इसमें भुगतान नहीं हो सकता।',
      pay_asset_recipient_blocked: '{{issuer}} इस व्यक्ति को भुगतान की अनुमति नहीं देता।',
      token_err_refused:
        '{{issuer}} ने यह स्थानांतरण अस्वीकार कर दिया। कुछ भी नहीं भेजा गया और आपका {{ticker}} अपरिवर्तित है।',
      token_err_refused_paused: '{{issuer}} ने {{ticker}} रोक दिया है। कुछ भी नहीं भेजा गया और आपका शेष अपरिवर्तित है।',
      token_err_refused_recipient:
        '{{issuer}} इस व्यक्ति को भुगतान की अनुमति नहीं देता। कुछ भी नहीं भेजा गया और आपका शेष अपरिवर्तित है।',
      token_err_refused_frozen:
        'आप जो {{ticker}} भेज रहे थे उसका कुछ हिस्सा अभी फ़्रीज़ हो गया। कुछ भी नहीं भेजा गया; आपका शेष अपडेट कर दिया गया है।',
      token_err_send_failed: 'यह भुगतान नहीं भेजा जा सका: {{reason}}। आपके वॉलेट से कुछ भी नहीं गया।',
      token_err_reason_unknown: 'एक अज्ञात समस्या',
      token_err_unreachable:
        'हम इस स्थानांतरण की पुष्टि के लिए {{issuer}} तक नहीं पहुँच सके। दोबारा कोशिश करने से पहले अपना {{ticker}} शेष जाँचें।',
      token_err_check_again: 'फिर से जाँचें',
      token_err_balance_changed: 'यह तैयार होते समय आपका {{ticker}} शेष बदल गया। राशि जाँचें और फिर कोशिश करें।',
      token_err_busy: 'एक {{ticker}} भुगतान पहले ही भेजा जा रहा है।',
      pay_sent_not_broadcast: 'भेज दिया गया। यह अभी नेटवर्क तक नहीं पहुँचा — हम कोशिश करते रहेंगे।',
      pay_sent_not_notified: 'भेज दिया गया। हम उन्हें अभी नहीं बता सके — उनका वॉलेट अगली बार जाँचने पर दिख जाएगा।',
      pay_sent_handed_to_wallet: 'भेज दिया गया। उन्होंने सामने से नहीं लिया, इसलिए हमने इसे उनके वॉलेट में भेज दिया।',
      token_first_hold:
        '{{ticker}} {{issuer}} द्वारा जारी किया जाता है, जो स्थानांतरण रोक सकता है और सिक्के फ़्रीज़ कर सकता है।',
      pay_asset_request_paused: '{{issuer}} ने {{ticker}} रोक दिया है, इसलिए भुगतान अभी नहीं पहुँच सकता।',
      token_admit_title: 'आपको अभी {{ticker}} में भुगतान नहीं मिल सकता',
      token_admit_body:
        '{{issuer}} केवल पंजीकृत लोगों को ही {{ticker}} भेजता है। तब तक आपके लिए भुगतान भेजने वाले के वॉलेट से निकलने से पहले ही अस्वीकार हो जाते हैं।',
      token_admit_bsv_instead: 'इसके बजाय BSV में भुगतान लें',
      token_row_sent: '{{ticker}} भेजा',
      token_row_received: '{{ticker}} प्राप्त हुआ',
      token_row_amount_pending: 'राशि उपलब्ध नहीं',
      token_evicted_title: 'कुछ {{ticker}} वापस ले लिया गया',
      token_evicted_body: '{{issuer}} ने आपके वॉलेट से {{amount}} {{ticker}} वापस ले लिया।',
      local_pay_token_not_cleared: 'ऑफ़लाइन प्राप्त · {{issuer}} ने अभी पुष्टि नहीं की',
      pay_asset_nearby_needs_network:
        'आमने-सामने {{ticker}} में भुगतान के लिए कनेक्शन चाहिए। बिना कनेक्शन के आप BSV में भुगतान कर सकते हैं।',
      pay_asset_link_not_held:
        'यह अनुरोध ऐसे टोकन के लिए है जो इस वॉलेट में नहीं है। आप फिर भी उन्हें BSV में भुगतान कर सकते हैं।',
      pay_asset_nearby_wrong_overlay: 'यह अनुरोध ऐसे टोकन के लिए है जिसके लिए यह वॉलेट सेट नहीं है।',
      token_messagebox_mismatch:
        'आपका वॉलेट किसी दूसरे मैसेज बॉक्स पर सेट है, इसलिए टोकन भुगतान आप तक नहीं पहुँच सकते।',
      token_issuer_fallback: 'जारीकर्ता',
      token_row_hint: 'जारीकर्ता और उनके अधिकार खोलता है',
      token_attention_one: '{{issuer}} की पुष्टि का इंतज़ार — विवरण के लिए टैप करें',
      token_attention_many: '{{count}} टोकन भुगतान निपटान की प्रतीक्षा में — विवरण के लिए टैप करें',
      token_recv_reversed: 'इस भुगतान की {{issuer}} द्वारा पुष्टि नहीं हो सकी और इसे वापस कर दिया गया है।',
      token_settle_paused_retry: '{{issuer}} ने अभी {{ticker}} रोक रखा है — हम कोशिश करते रहेंगे।',
      token_settle_unreachable_retry:
        'हम इस स्थानांतरण की पुष्टि के लिए {{issuer}} तक नहीं पहुँच सके। हम कोशिश करते रहेंगे।',
      token_settle_evicted:
        'वह भुगतान समय पर नेटवर्क तक नहीं पहुँचा और वापस कर दिया गया — आपका {{ticker}} शेष में लौट आया है।',
      token_frame_unreadable: 'आपको मिला भुगतान हम पढ़ नहीं सके — भेजने वाले से दोबारा भेजने को कहें।',
      token_chain_reversed:
        'इस शृंखला में पहले का एक भुगतान {{issuer}} ने अस्वीकार कर दिया, इसलिए यह भी वापस कर दिया गया है।',
      token_settle_stuck_bound:
        'यह भुगतान 3 दिनों से अधिक समय से निपटान की प्रतीक्षा में है। आप इंतज़ार कर सकते हैं या {{issuer}} से संपर्क करें।',
      token_cover_failed: 'यह भुगतान {{issuer}} के रिकॉर्ड से मेल नहीं खाता और स्वीकार नहीं किया गया।',
      token_sent_settling:
        'भेज दिया गया। {{issuer}} के साथ निपटान जारी — आप या {{payee}} दोबारा ऑनलाइन होने पर पुष्टि दिखेगी।',
      token_sent_settling_unnamed:
        'भेज दिया गया। {{issuer}} के साथ निपटान जारी — आप में से कोई भी दोबारा ऑनलाइन होने पर पुष्टि दिखेगी।',
      token_sent_settled: 'भेजा गया · {{issuer}} के साथ निपटा',
      token_status_settling: 'निपटान जारी',
      token_status_settled: 'निपट गया',
      token_status_refused: 'अस्वीकृत',
      token_status_reversed: 'वापस किया गया',
      token_status_stuck: 'निपटान की प्रतीक्षा',
      new_tab: 'नया टैब',
      untitled: 'अनामांकित',
      share: 'साझा करें',
      back_to_homepage: 'मुख्य पृष्ठ पर वापस जाएं',
      bookmarks: 'बुकमार्क',
      add_bookmark: 'बुकमार्क जोड़ें',
      bookmark: 'बुकमार्क',
      unbookmark: 'बुकमार्क हटाएं',
      find_in_page: 'पृष्ठ में खोजें',
      find_in_page_placeholder: 'पृष्ठ में खोजें...',
      find_in_page_matches: '{{current}} / {{total}}',
      find_in_page_no_matches: 'कोई मिलान नहीं',
      history: 'इतिहास',
      clear_all: 'सभी साफ करें',
      clear_all_tabs: 'सभी टैब साफ करें',
      search_or_enter_website: 'खोजें या वेबसाइट दर्ज करें',
      browser: 'ब्राउज़र',
      bug_report: 'बग रिपोर्ट',
      enable_web3: 'Web3 सक्षम करें',
      tabs: 'टैब',
      unlock_web3_features: 'Web3 सुविधाएं अनलॉक करें',

      cancel: 'रद्द करें',
      done: 'हो गया',
      discard: 'छोड़ें',
      deny: 'अस्वीकार करें',
      allow: 'अनुमति दें',
      save: 'सहेजें',
      remove: 'हटाएं',
      delete: 'हटाएं',
      search: 'खोजें',
      loading: 'लोड हो रहा है...',
      continue: 'जारी रखें',
      go_back: 'वापस जाएं',
      continue_action: 'जारी रखें',
      camera_denied_description:
        'कैमरा एक्सेस अस्वीकार कर दी गई। इस सुविधा का उपयोग करने के लिए सेटिंग्स में इसे सक्षम करें।',
      open_settings: 'सेटिंग्स खोलें',
      copied: 'कॉपी किया गया',
      explorer_open_failed: 'ब्लॉक एक्सप्लोरर नहीं खोला जा सका',
      tx_action_refresh: 'स्थिति ताज़ा करें',
      tx_action_refresh_short: 'स्थिति',
      tx_action_abort: 'लेनदेन रद्द करें',
      copy: 'कॉपी',
      copy_to_clipboard: 'क्लिपबोर्ड में कॉपी करें',
      details: 'विवरण',
      hide_details: 'विवरण छुपाएं',

      identity: 'पहचान',
      settings: 'सेटिंग्स',
      wallet: 'वॉलेट',
      activity: 'गतिविधि',
      configuration: 'कॉन्फ़िगरेशन',
      data_and_security: 'डेटा और सुरक्षा',
      storage_row: 'संग्रहण',
      storage_title: 'वॉलेट संग्रहण',
      storage_db_size: 'डेटाबेस: {{size}}',
      storage_free_space: 'खाली जगह: {{size}}',
      storage_reclaimable: 'अब पुनःप्राप्य: {{size}}, {{count}} लेनदेन में',
      storage_held_back: '{{count}} लेनदेन अभी उपयोग में होने के कारण रोके गए',
      storage_reclaim_explain:
        'पुनःप्राप्ति वह सत्यापन डेटा हटाती है जिसकी वॉलेट को अब आवश्यकता नहीं है। आपके लेनदेन, शेष और इतिहास अछूते रहते हैं। यह डेटाबेस फ़ाइल के भीतर जगह मुक्त करता है; फ़ाइल स्वयं सिस्टम द्वारा संपीड़न तक छोटी नहीं होती।',
      storage_reclaim_confirm: 'पुनःप्राप्त करें',
      storage_reclaimed: '{{count}} लेनदेन पुनःप्राप्त किए गए',
      storage_failed: 'संग्रहण उपयोग नहीं पढ़ा जा सका।',
      storage_unavailable: 'संग्रहण अभी तैयार नहीं है।',
      backup_push_section: 'निजी बैकअप',
      backup_push_toggle: 'सर्वर पर बैकअप',
      backup_erase_row: 'सर्वर से बैकअप हटाएँ',
      backup_erase_title: 'अपना बैकअप हटाएँ?',
      backup_erase_message:
        'इस वॉलेट ने बैकअप सर्वर पर जो कुछ भेजा है, वह सभी उपकरणों सहित स्थायी रूप से हटा दिया जाएगा। साथ ही बैकअप बंद कर दिया जाएगा। आपके सिक्के, कुंजियाँ और रिकवरी फ़्रेज़ अछूते रहते हैं — लेकिन नया उपकरण इस वॉलेट का इतिहास बहाल नहीं कर पाएगा, और आपके कुछ मौजूदा सिक्के वहाँ खर्च नहीं होंगे।',
      backup_erase_confirm: 'हटाएँ',
      backup_erase_done: 'बैकअप हटाया गया — {{count}} आइटम हटाए गए',
      backup_erase_failed: 'बैकअप नहीं हटाया जा सका। कुछ भी नहीं हटा — कृपया फिर प्रयास करें।',
      backup_erase_no_key: 'कोई वॉलेट कुंजी उपलब्ध नहीं',
      backup_push_disclosure:
        'आपके वॉलेट डेटाबेस की एन्क्रिप्टेड प्रति BSV एसोसिएशन के सर्वर पर भेजी जाती है ताकि आप नए उपकरण पर पुनर्प्राप्त कर सकें। इसे उस कुंजी से एन्क्रिप्ट किया जाता है जो केवल आपका रिकवरी फ़्रेज़ बना सकता है — संचालक इसे पढ़ नहीं सकता। आपको फिर भी रिकवरी फ़्रेज़ या मुद्रित शेयर चाहिए: उनके बिना बैकअप कोई नहीं खोल सकता, हम भी नहीं।',
      backup_push_off_title: 'निजी बैकअप बंद करें?',
      backup_push_off_message:
        'केवल रिकवरी फ़्रेज़ से यह वॉलेट पुनर्निर्मित नहीं हो सकता। आपके सिक्कों की सुरक्षा से जुड़ी जानकारी केवल डेटाबेस में मौजूद है, इसलिए बैकअप बंद होने पर नया उपकरण आपका फ़्रेज़ बहाल कर सकेगा पर धन नहीं। सर्वर पर पहले से मौजूद कुछ भी नहीं मिटाया जाता।',
      backup_push_off_confirm: 'बंद करें',
      backup_push_on_toast: 'बैकअप चालू — अभी सिंक हो रहा है',
      backup_push_off_toast: 'बैकअप बंद — अब कुछ नहीं भेजा जाएगा',
      permissions: 'अनुमतियां',
      trust_network: 'विश्वास नेटवर्क',
      // vault (machine-translated, needs native review)
      vault_title: 'वॉल्ट',
      vault_row_title: 'वॉल्ट',
      vault_hero_title: 'सुरक्षित होल्डिंग्स',
      vault_hero_body: 'लंबी अवधि के लिए कूल स्टोरेज, आपकी अपनी YubiKeys से लॉक। दो या अधिक YubiKey 5 NFC चाहिए।',
      vault_unsupported_title: 'YubiKey चाहिए',
      vault_unsupported_body: 'दो या अधिक YubiKey 5 NFC, और एक फ़ोन जो उन्हें पढ़ सके।',
      vault_restore_from_chain: 'ब्लॉकचेन से वॉल्ट पुनर्स्थापित करें',
      vault_restore_scanning: 'ब्लॉकचेन स्कैन हो रहा है…',
      vault_restore_found: 'ब्लॉकचेन पर {{count}} वॉल्ट जमा मिले।',
      vault_restore_pending: '{{count}} जमा मिले, पहली पुष्टि की प्रतीक्षा में — कृपया थोड़ी देर बाद पुनः प्रयास करें।',
      vault_restore_none_found: 'इस वॉलेट के लिए ब्लॉकचेन पर कोई वॉल्ट जमा नहीं मिला।',
      vault_balance_label: 'वॉल्ट में है',
      vault_key_section: 'सुरक्षा कुंजियाँ ({{count}}/5)',
      vault_manage_section: 'प्रबंधित करें',
      vault_deposit_cta: 'जमा करें',
      vault_withdraw_cta: 'निकालें',
      vault_deposit_title: 'वॉल्ट में जमा करें',
      vault_withdraw_title: 'वॉल्ट से निकालें',
      vault_deposit_sub: 'आपके रोज़मर्रा के बैलेंस से। YubiKey की ज़रूरत नहीं।',
      vault_withdraw_sub: 'वापस आपके रोज़मर्रा के बैलेंस में। अपनी किसी भी वॉल्ट कुंजी को टैप करें।',
      vault_deposit_done: 'वॉल्ट में ले जाया गया',
      vault_reclaim_done: 'बाधित वॉल्ट जमा से धनराशि पुनर्प्राप्त की गई',
      vault_withdraw_done: 'वॉल्ट से निकाला गया',
      vault_withdraw_partial: '{{count}} और जमा बाकी हैं — उन्हें ले जाने के लिए फिर निकालें।',
      vault_withdraw_reason: 'हस्ताक्षर के लिए अपनी YubiKey यहाँ रखें — वॉल्ट से {{amount}} sats निकालें',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: 'अभी उपलब्ध नहीं — इस रिलीज़ में वॉल्ट जमा बंद हैं।',
      vault_not_on_mainnet_body:
        'वॉल्ट केवल मेननेट पर उपलब्ध है। इसका उपयोग करने के लिए वॉलेट को मेननेट पर वापस ले जाएँ।',
      vault_intro_title: 'अपना वॉल्ट सेट करें',
      vault_intro_what:
        'वॉल्ट पैसे को आपकी अपनी YubiKeys से लॉक करता है। वॉल्ट कुंजी का कोई भी हिस्सा इस फ़ोन पर कभी मौजूद नहीं होता; आपकी कोई भी एक कुंजी इसे खोलती है।',
      vault_intro_two_keys: 'आपको कम से कम दो YubiKeys चाहिए।',
      vault_intro_apart: 'उन्हें अलग-अलग जगहों पर रखें — एक साथ रखी दो कुंजियाँ एक कुंजी के बराबर हैं।',
      vault_intro_backup: 'वॉलेट का एन्क्रिप्टेड बैकअप चालू रखें — यह प्रमाणित रिकवरी रिकॉर्ड सुरक्षित रखता है।',
      vault_intro_ack:
        'मैं समझता/समझती हूँ: केवल मेरी YubiKeys यह वॉल्ट खोलती हैं। मेरा रिकवरी फ़्रेज़ नहीं। यदि मैं सभी खो दूँ, तो पैसा चला जाएगा।',
      vault_intro_piv_ack:
        'मैं पुष्टि करता/करती हूँ कि इस YubiKey का पूरा PIV ऐप फ़ैक्टरी-रीसेट है या वॉल्ट के लिए समर्पित है। सेटअप वैश्विक PIV PIN, PUK और प्रबंधन कुंजी बदलता है, सभी PIV स्लॉट को प्रभावित कर सकता है, और मैं नया PUK सुरक्षित रूप से दर्ज करूँगा/करूँगी।',
      vault_intro_begin: 'शुरू करें',
      vault_key_step_title: 'कुंजी {{k}}, अधिकतम 5 में से',
      vault_key_step_replace:
        'वॉल्ट के लिए समर्पित, फ़ैक्टरी-रीसेट YubiKey का उपयोग करें। यदि उसके वॉल्ट स्लॉट में पुरानी कुंजी है, तो बदलने से पहले सेटअप चेतावनी देगा।',
      vault_pin_choose_title: 'PIN चुनें',
      vault_pin_choose_sub: 'इस कुंजी का उपयोग करते समय हर बार यही दर्ज करना होगा। 6 से 8 अंक।',
      vault_pin_confirm_label: 'इसे दोबारा दर्ज करें',
      vault_pin_mismatch: 'दोनों PIN मेल नहीं खाते।',
      vault_pin_not_default: '123456 के अलावा कुछ चुनें।',
      vault_puk_title: 'आपका रिकवरी कोड',
      vault_puk_body:
        'इसे लिखकर सुरक्षित जगह रखें। PIN भूल जाने पर इस कुंजी को खोलने का यही एकमात्र तरीका है, और यह दोबारा नहीं दिखाया जाएगा।',
      vault_puk_ack: 'मैंने इसे लिख लिया है',
      vault_reset_offer: 'इस कुंजी को रीसेट करें',
      vault_reset_title: 'इस कुंजी को रीसेट करें?',
      vault_reset_body:
        'यह कुंजी पहले इस्तेमाल हो चुकी है। रीसेट करने पर इसके PIV ऐप्लिकेशन का हर प्रमाणपत्र और कुंजी मिट जाएगी, सिर्फ़ वॉल्ट स्लॉट नहीं। इस कुंजी पर निर्भर सब कुछ काम करना बंद कर देगा।',
      vault_reset_ack: 'मैं समझता हूँ कि इससे इस कुंजी का सब कुछ मिट जाएगा',
      vault_reset_unknown_ack:
        'यह डिवाइस पुष्टि नहीं कर सकता कि यह कुंजी किसी दूसरे प्रोफ़ाइल के वॉल्ट को अभी भी सुरक्षित कर रही है या नहीं। मैं समझता हूँ कि इसे मिटाने से वह कुंजी हमेशा के लिए नष्ट हो सकती है।',
      vault_reset_own_draft_ack:
        'इस कुंजी में आपके पिछले अधूरे प्रयास की कुंजी है, और इस डिवाइस के पास अब भी उसका रिकॉर्ड है। मैं समझता हूँ कि रीसेट करने पर वह मिट जाएगी और उस प्रयास को जारी नहीं रखा जा सकेगा।',
      vault_reset_nothing_erased:
        'कुछ भी नहीं मिटाया गया। इस कुंजी का वॉल्ट स्लॉट खाली नहीं है, इसलिए एक बात और पुष्टि करनी होगी।',
      vault_reset_confirm: 'मिटाएँ और रीसेट करें',
      vault_resetting: 'कुंजी रीसेट की जा रही है…',
      vault_reset_done: 'कुंजी रीसेट हो गई। जारी रखने के लिए इसे फिर से टैप करें।',
      vault_reset_uncertain:
        'रीसेट पूरा नहीं हुआ। यह कुंजी शायद पहले ही मिट चुकी हो — इस पर भरोसा करने से पहले इसे जाँच लें।',
      vault_err_reset_enrolled:
        'वह कुंजी पहले से आपके वॉल्ट का हिस्सा है — उसे रीसेट करने से आपकी एक वॉल्ट कुंजी नष्ट हो जाएगी।',
      vault_err_reset_unreadable:
        'इस डिवाइस पर आपके वॉल्ट रिकॉर्ड पढ़े नहीं जा सके, इसलिए यह साबित नहीं किया जा सकता कि यह कुंजी आपकी वॉल्ट कुंजियों में से एक नहीं है। कुछ भी नहीं मिटाया गया और रीसेट अस्वीकार कर दिया गया।',
      vault_enrollment_resume_pin_hint:
        'सहेजी गई कुंजी फिर से शुरू करने के लिए वही PIN चाहिए जो आपने पहले उसके लिए चुना था, नया नहीं। तीन बार गलत होने पर कुंजी लॉक हो जाएगी।',
      vault_setup_step: 'चरण {{n}} / {{total}}',
      vault_enrollment_resume: 'सहेजी गई कुंजी जारी रखें',
      vault_enrollment_reset_required:
        'यह YubiKey अब फ़ैक्टरी स्थिति में नहीं है, इसलिए इसे मौजूदा हालत में नामांकित नहीं किया जा सकता। वॉल्ट कुंजी रखने से पहले इसके PIV ऐप्लिकेशन को मिटाकर फ़ैक्टरी स्थिति में लाना होगा।',
      vault_enrollment_state_uncertain:
        'एक सेटअप प्रयास बीच में रुक गया, इसलिए यह पक्का नहीं कहा जा सकता कि YubiKey किस स्थिति में छूटी। जब तक उसका PIV ऐप्लिकेशन मिटाकर फ़ैक्टरी स्थिति में नहीं लाया जाता, उस कुंजी का दोबारा उपयोग नहीं होगा।',
      vault_nfc_enroll_message: 'सेट करने के लिए अपनी YubiKey यहाँ रखें',
      vault_nfc_adopt_message: 'सत्यापित करने के लिए इस पुनर्प्राप्त YubiKey को यहाँ रखें',
      vault_nfc_sign_batch: 'हस्ताक्षर के लिए अपनी YubiKey यहाँ रखें — बैच {{b}}/{{n}}',
      vault_key_use_different: 'दूसरी YubiKey उपयोग करें',
      vault_key_setup_again: 'फिर से सेट करें',
      vault_name_default: 'कुंजी {{k}}',
      vault_more_title: 'एक और कुंजी जोड़ें?',
      vault_more_body: 'अधिकतम 5 कुंजियाँ। इनमें से कोई भी एक अकेले पूरा वॉल्ट खोल सकती है।',
      vault_more_add: 'एक और कुंजी जोड़ें',
      vault_more_finish: 'समाप्त',
      vault_more_need_two: 'समाप्त करने से पहले दूसरी कुंजी जोड़ें — एक कुंजी का मतलब कोई रिकवरी नहीं।',
      vault_leave_setup: 'सेटअप छोड़ें',
      vault_leave_title: 'सेटअप छोड़ें?',
      vault_leave_body:
        'आपने जो {{count}} YubiKey सेट की हैं, वे अभी सहेजी नहीं जाएँगी। उनकी कुंजियाँ बनी रहती हैं, इसलिए आप उन्हें कुछ ही मिनट में फिर जोड़ सकते हैं।',
      vault_leave_confirm: 'छोड़ें',
      vault_leave_stay: 'रहें',
      vault_done_body:
        '{{count}} कुंजियाँ यह वॉल्ट खोल सकती हैं। केवल ये कुंजियाँ इसे खोलती हैं — आपका रिकवरी फ़्रेज़ नहीं।',
      vault_done_cta: 'हो गया',
      vault_add_key_row: 'कुंजी जोड़ें',
      vault_add_key_done: '{{nickname}} अब से की गई जमाओं को खोल सकती है। वॉल्ट को री-लॉक करें ताकि यह सब कुछ खोल सके।',
      vault_key_added_toast: 'कुंजी जोड़ी गई',
      vault_recovery_verified: 'पुनर्प्राप्त कुंजी इस डिवाइस पर सत्यापित हुई',
      vault_relock_row: 'वॉल्ट री-लॉक करें',
      vault_relock_now: 'अभी री-लॉक करें',
      vault_relock_choose: 'री-लॉक करने के लिए आप कौन-सी कुंजी टैप करेंगे?',
      vault_relock_reason: 'अपनी मौजूदा कुंजियों में से एक टैप करें ({{names}}) — वह नहीं जो आपने अभी जोड़ी।',
      vault_relock_reason_generic:
        'हस्ताक्षर के लिए अपनी YubiKey यहाँ रखें — वॉल्ट को अपनी वर्तमान कुंजियों पर री-लॉक करें',
      vault_relock_reason_remaining: 'केवल आपकी शेष कुंजियों पर री-लॉक करें: {{names}}।',
      vault_key_pending_removal: 'हटाया गया — पूरा करने के लिए री-लॉक करें',
      vault_key_removal_awaiting_network: 'हटाया जा रहा है — नेटवर्क द्वारा री-लॉक स्वीकारे जाने की प्रतीक्षा',
      vault_removal_awaiting_network: 'री-लॉक भेज दिया गया। नेटवर्क के स्वीकार करते ही जमा और निकासी फिर से खुल जाएँगी।',
      vault_err_too_many_active_keys: 'वॉल्ट अधिकतम पाँच कुंजियों पर लॉक होता है। जमा करने, री-लॉक करने या शेष राशि का कुछ हिस्सा निकालने से पहले एक कुंजी हटाएँ।',
      vault_key_section_over_limit: 'सुरक्षा कुंजियाँ ({{count}}, एक हटाएँ)',
      vault_add_key_done_over_limit: '{{nickname}} जोड़ दी गई। वॉल्ट अधिकतम पाँच कुंजियों पर लॉक होता है, इसलिए जिस कुंजी की जगह यह आई है उसे हटाएँ, फिर एक बार री-लॉक करें।',
      vault_relock_done: 'वॉल्ट आपकी वर्तमान कुंजियों पर री-लॉक हो गया',
      vault_relock_capped: '{{count}} और जमा बाकी हैं — उन्हें ले जाने के लिए फिर री-लॉक करें।',
      vault_relock_unreachable:
        '{{count}} जमा केवल {{names}} से खुल सकती हैं। उनमें से किसी एक कुंजी से फिर री-लॉक करें।',
      vault_badge_missing: '{{count}} जमा अभी {{nickname}} के लिए खुली नहीं हैं',
      vault_badge_removed: '{{count}} जमा अभी भी हटाई गई कुंजी से खुल सकती हैं',
      vault_key_action_rename: 'नाम बदलें',
      vault_key_action_remove: 'हटाएँ',
      vault_rename_title: '{{nickname}} का नाम बदलें',
      vault_rename_save: 'सहेजें',
      vault_remove_title: '{{nickname}} हटाएँ?',
      vault_remove_body:
        'इससे वॉलेट {{nickname}} का उपयोग बंद कर देगा। वॉल्ट में पहले से मौजूद पैसा री-लॉक करने तक इससे खुलता रहेगा (≈ {{fee}} sats)।',
      vault_remove_only: 'केवल हटाएँ',
      vault_key_removed_toast: 'कुंजी हटाई गई',
      vault_export_explainer:
        'हर वॉल्ट जमा तब तक अपना सॉल्ट छुपाए रखता है जब तक आप उसे खर्च नहीं करते — लॉकिंग स्क्रिप्ट में इसके बारे में कुछ भी दिखाई नहीं देता। एन्क्रिप्टेड बैकअप चालू रखें, ताकि नया डिवाइस आपकी YubiKeys के उपयोग के लिए आवश्यक कुंजी रिकॉर्ड पुनर्स्थापित और प्रमाणित कर सके।',
      vault_footnote: 'केवल ये कुंजियाँ वॉल्ट खोलती हैं। आपका रिकवरी फ़्रेज़ नहीं।',
      vault_floor_line:
        'न्यूनतम जमा {{floorDisplay}} ({{floorSats}} sats)। वॉल्ट जमा बनाने में लगभग {{feeDisplay}} लगता है।',
      vault_first_deposit_title: 'पहली वॉल्ट जमा',
      vault_first_deposit_body:
        'पहली वॉल्ट जमा — {{amount}} केवल {{count}} YubiKeys ({{names}}) से खुल पाएगा। आपका रिकवरी फ़्रेज़ मदद नहीं करेगा।',
      vault_backup_off_title: 'पहले निजी बैकअप चालू करें',
      vault_backup_off_body:
        'वॉलेट के नया वॉल्ट आउटपुट बनाने से पहले निजी बैकअप चालू होना चाहिए। इसमें रिकवरी के लिए आवश्यक प्रमाणित YubiKey रिकॉर्ड और वॉलेट इतिहास सुरक्षित रहते हैं। आप वॉल्ट चेंज बनाए बिना अब भी सब कुछ निकाल सकते हैं।',
      vault_backup_off_cta: 'बैकअप सेटिंग्स खोलें',
      vault_seed_not_preserved_title: 'पहले अपना रिकवरी वाक्यांश सहेजें',
      vault_seed_not_preserved_body:
        'आपका वॉल्ट केवल आपके रिकवरी वाक्यांश और आपकी किसी एक YubiKey से ही पुनर्प्राप्त किया जा सकता है। YubiKeys में इसकी कोई प्रति नहीं रहती — यदि आप इसे लिखने से पहले खो देते हैं, तो यह धनराशि पुनर्प्राप्त नहीं की जा सकेगी। जमा करने से पहले अभी इसे सहेजें।',
      vault_seed_not_preserved_cta: 'रिकवरी वाक्यांश सहेजें',
      vault_choose_key: 'आप कौन-सी कुंजी टैप करेंगे?',
      vault_remainder_title: 'सब कुछ निकालें?',
      vault_remainder_body:
        '{{amount}} निकालने पर {{remainder}} बचेगा, जो 100,000-sat वॉल्ट न्यूनतम से कम है। पूरा वॉल्ट आपके रोज़मर्रा के बैलेंस में चला जाएगा।',
      vault_remainder_all: 'सब कुछ निकालें',
      vault_remainder_change: 'राशि बदलें',
      vault_unreachable_title: 'वॉल्ट के एक हिस्से को दूसरी कुंजी चाहिए',
      vault_unreachable_body:
        'वॉल्ट के एक हिस्से को दूसरी कुंजी चाहिए — {{moved}} ले जाया गया। {{amount}} वाली {{count}} जमा केवल {{names}} से खुल सकती हैं। उनमें से किसी एक कुंजी से फिर निकालें।',
      vault_sign_progress: '{{total}} में से {{signed}} हस्ताक्षरित',
      vault_err_pin_locked_enroll:
        'इस YubiKey का PIN ब्लॉक है। Yubico Authenticator में इसके PUK से अनब्लॉक करें, या दूसरी YubiKey से सेट करें।',
      vault_err_serial_mismatch_chosen:
        'यह {{tappedName}} है। आपने {{chosenName}} चुनी थी — उसे टैप करें, या वापस जाकर {{tappedName}} चुनें।',
      vault_err_key_already_enrolled: 'आप यह YubiKey ({{nickname}}) पहले ही जोड़ चुके हैं। दूसरी टैप करें।',
      vault_err_not_released: 'इस रिलीज़ में वॉल्ट जमा बंद हैं।',
      vault_err_not_on_mainnet: 'वॉल्ट केवल मेननेट पर उपलब्ध है। वॉलेट को मेननेट पर ले जाकर फिर कोशिश करें।',
      vault_err_backup_off:
        'नया वॉल्ट आउटपुट बनाने से पहले एन्क्रिप्टेड निजी बैकअप चालू करें। आप अब भी सब कुछ निकाल सकते हैं।',
      vault_err_not_enough_keys: 'इस वॉल्ट में दो से कम कुंजियाँ हैं। इसे फिर से सेट करें।',
      vault_err_too_many_keys: 'एक वॉल्ट में अधिकतम पाँच कुंजियाँ हो सकती हैं।',
      vault_err_last_keys: 'वॉल्ट को कम से कम दो कुंजियाँ चाहिए। इसे हटाने से पहले दूसरी कुंजी जोड़ें।',
      vault_err_relock_required: 'पहले वॉल्ट री-लॉक करें ताकि आपकी अन्य कुंजियाँ हर जमा खोल सकें।',
      vault_err_action_pending: 'वॉल्ट का एक लेन-देन अभी पूरा नहीं हुआ है। पूरा होने पर फिर कोशिश करें।',
      vault_resolve_held_deposit_action: 'बाधित जमा पूरा करें',
      vault_resolve_held_deposit_done: 'जमा की समस्या हल हो गई',
      vault_resolve_held_deposit_failed: 'जमा की समस्या हल नहीं हो सकी। फिर कोशिश करें।',
      vault_resolve_held_deposit_none:
        'यहाँ हल करने के लिए कुछ नहीं है — लंबित लेनदेन को फिर से आज़माने या रद्द करने के लिए गतिविधि टैब देखें।',
      vault_err_key_not_committed:
        '{{nickname}} इस वॉल्ट की कोई भी जमा नहीं खोल सकती। अपनी दूसरी वॉल्ट कुंजी उपयोग करें।',
      vault_err_key_cannot_cover:
        '{{nickname}} वॉल्ट के {{total}} में से {{reachable}} खोल सकती है। अधिकतम {{reachable}} निकालें, या {{otherNames}} उपयोग करें।',
      vault_err_too_small_to_relock:
        'इस वॉल्ट में 100,000 sats से कम है, जो री-लॉक के लिए बहुत कम है। इसे निकालें और फिर जमा करें।',
      vault_err_bad_version: 'लेन-देन गलत फ़ॉर्मैट में बना। कुछ भी हस्ताक्षरित नहीं हुआ — फिर कोशिश करें।',
      vault_err_chain_scan_failed:
        'वॉल्ट जमा के लिए ब्लॉकचेन स्कैन पूरा नहीं हो सका। अपना कनेक्शन जाँचें और फिर कोशिश करें।',
      vault_shares_unavailable: 'प्रिंट करने के लिए कोई कुंजी सामग्री उपलब्ध नहीं है।',
      vault_enroll_begin: 'वॉल्ट सेट करें',
      vault_enroll_phase_connecting: 'आपकी कुंजी पढ़ी जा रही है…',
      vault_enroll_phase_pin_check: 'आपकी कुंजी जाँची जा रही है…',
      vault_enroll_phase_checking_slot: 'वॉल्ट स्लॉट जाँचा जा रहा है…',
      vault_enroll_phase_personalizing: 'आपका PIN और रिकवरी कोड सेट किया जा रहा है…',
      vault_enroll_phase_generating: 'वॉल्ट कुंजी बनाई जा रही है — कुंजी के झपकने पर उसे स्पर्श करें',
      vault_enroll_phase_challenging: 'नई वॉल्ट कुंजी सत्यापित हो रही है — झपकने पर कुंजी को स्पर्श करें',
      vault_enroll_phase_done: 'हो गया',
      vault_touch_when_blinks: 'कुंजी के झपकने पर सुनहरे संपर्क को स्पर्श करें।',
      vault_nfc_activation_hint:
        'इस YubiKey का पहली बार उपयोग कर रहे हैं? पहले इसे कुछ सेकंड के लिए USB-C पोर्ट में लगाएँ — नई कुंजियाँ NFC बंद करके भेजी जाती हैं, तब तक यह चालू नहीं होता।',
      vault_continue: 'जारी रखें',
      vault_back: 'वापस',
      vault_enrolled_toast: 'वॉल्ट तैयार',
      vault_insert_key: 'अपनी YubiKey डालें',
      vault_hold_key_nfc: 'अपनी YubiKey को फ़ोन के ऊपरी हिस्से से सटाकर रखें',
      vault_keep_holding_nfc: 'सटाए रखें — अनलॉक हो रहा है…',
      vault_unlocking_funds: 'आपकी धनराशि अनलॉक हो रही है',
      vault_unlocking_sub: 'इसमें कुछ सेकंड लगते हैं। कृपया ऐप खुला रखें।',
      vault_sending_to_network: 'नेटवर्क पर भेजा जा रहा है',
      vault_sending_sub: 'अगर तुरंत नहीं जाता, तो हम कोशिश करते रहेंगे।',
      vault_reading_key: 'कुंजी पढ़ी जा रही है…',
      vault_enter_pin: 'YubiKey PIN दर्ज करें',
      vault_pin_sub_nfc: 'फिर पूरा करने के लिए अपनी कुंजी फ़ोन के ऊपरी हिस्से से सटाएँ।',
      vault_pin_sub_usb: 'फिर पूरा करने के लिए सुनहरे संपर्क को छुएँ।',
      vault_touch_contact: 'सुनहरे संपर्क को स्पर्श करें',
      vault_unlock_cta: 'अनलॉक करें',
      vault_pin_retries: '{{count}} प्रयास शेष',
      vault_locked: 'वॉल्ट लॉक है',
      vault_retry: 'पुनः प्रयास करें',
      vault_cancel: 'रद्द करें',
      vault_dismiss: 'खारिज करें',
      vault_err_wrong_key: 'यह YubiKey इस वॉल्ट की कुंजियों में से नहीं है।',
      vault_err_serial_mismatch: 'यह YubiKey इस वॉल्ट की कुंजियों में से नहीं है ({{names}})।',
      vault_err_touch_timeout: 'कोई स्पर्श नहीं मिला। पुनः प्रयास करें और सुनहरे संपर्क को स्पर्श करें।',
      vault_err_pin_locked:
        'इस YubiKey का PIN ब्लॉक है। अपनी दूसरी वॉल्ट कुंजी उपयोग करें, या Yubico Authenticator में इसके PUK से इसे अनब्लॉक करें।',
      vault_err_pin_invalid: 'गलत PIN।',
      vault_err_puk_invalid: 'गलत PUK।',
      vault_err_puk_locked: 'इस YubiKey का PUK ब्लॉक है। नई YubiKey उपयोग करें।',
      vault_err_ceremony_active:
        'एक अन्य वॉल्ट कार्य अभी YubiKey का उपयोग कर रहा है। उसके पूरा होने की प्रतीक्षा करें और फिर प्रयास करें।',
      vault_err_scope_changed: 'इस वॉल्ट कार्य के दौरान वॉलेट या नेटवर्क बदल गया। मौजूदा वॉलेट में फिर शुरू करें।',
      vault_err_key_removed_mid_op: 'कुंजी हटा दी गई। इसे डालें और पुनः प्रयास करें।',
      vault_err_no_key: 'कोई YubiKey नहीं मिली।',
      vault_err_driver_unavailable: 'इस उपकरण पर YubiKey समर्थन उपलब्ध नहीं है।',
      vault_err_mgmt_key_custom: 'इस कुंजी में कस्टम प्रबंधन कुंजी है और इसे स्वचालित रूप से उपयोग नहीं किया जा सकता।',
      vault_err_attestation_invalid:
        'इस YubiKey के निर्माता प्रमाण की पुष्टि नहीं हो सकी। ऐसे असली, फ़ैक्टरी-प्रमाणित YubiKey का उपयोग करें जिसका PIV प्रमाण स्लॉट बदला न गया हो।',
      vault_err_generic: 'कुछ गलत हो गया। कृपया पुनः प्रयास करें।',
      vault_err_template_invalid: 'वॉल्ट खोलने की कोशिश में कुछ गड़बड़ हुई। कृपया पुनः प्रयास करें।',
      vault_err_not_enrolled: 'कोई वॉल्ट सेट नहीं है।',
      vault_err_pin_required: 'अपना PIN दर्ज करें।',
      vault_err_unsupported_platform: 'इस उपकरण पर YubiKey समर्थन उपलब्ध नहीं है।',
      vault_err_slot_occupied: 'कुंजी स्लॉट पहले से उपयोग में है।',
      vault_replace_key_warning:
        'इसे बदलने पर मौजूदा P-256 निजी कुंजी स्थायी रूप से नष्ट हो जाएगी। उस कुंजी पर निर्भर वॉल्ट राशि अख़र्च योग्य हो सकती है।',
      vault_replace_key_confirm: 'मौजूदा कुंजी बदलें',
      vault_resume_draft_confirm_title: 'क्या इस तैयार कुंजी का उपयोग करें?',
      vault_resume_draft_confirm_body:
        'पहले से तैयार एक कुंजी — {{nickname}} · …{{tail}} — जोड़ने के लिए तैयार है, लेकिन इस डिवाइस ने अभी कार्ड पर इसे सत्यापित नहीं किया। केवल तभी आगे बढ़ें जब आप इसे पहचानते हों।',
      vault_resume_draft_confirm_use: 'इस कुंजी का उपयोग करें',
      vault_resume_draft_confirm_discard: 'इसे अस्वीकार करें',
      vault_err_enrollment_partial:
        'कुंजी बदलने के बाद YubiKey सेटअप रुक गया। पुनः प्रयास के निर्देश मानें; यह कुंजी वॉल्ट में नहीं जोड़ी गई।',
      vault_err_key_not_adopted:
        'इस पुनर्प्राप्त YubiKey से निकासी करने से पहले इस डिवाइस पर इसका कब्ज़ा सत्यापित करें।',
      vault_err_too_many_inputs:
        'एक बार में ले जाने के लिए बहुत अधिक वॉल्ट जमा हैं। पहले छोटी राशि निकालें — प्रत्येक निकासी वॉल्ट को समेकित भी करती है, इसलिए अगली बार अधिक राशि जा सकेगी।',
      vault_err_requires_online:
        'वॉल्ट लेनदेन के लिए कनेक्शन चाहिए। दोबारा जुड़ें और फिर प्रयास करें — वॉल्ट राशि बाद के लिए कतार में नहीं रखी जाती।',
      vault_err_user_cancelled: 'रद्द किया गया।',
      vault_err_vault_empty: 'वॉल्ट में अभी निकालने के लिए कोई राशि नहीं है।',
      vault_err_amount_exceeds_balance: 'यह वॉल्ट में मौजूद राशि से अधिक है।',
      vault_err_below_dust: 'यह राशि भेजने के लिए बहुत छोटी है।',
      vault_err_no_transaction: 'स्थानांतरण पूरा नहीं हो सका। कृपया पुनः प्रयास करें।',
      vault_err_nfc_lost: 'कनेक्शन टूट गया। कुंजी को फ़ोन पर स्थिर रखें।',
      vault_pin_invalid_retry: 'गलत PIN — {{count}} प्रयास शेष। पुनः प्रयास के लिए फिर से निकालें दबाएँ।',
      vault_pin_invalid_retry_generic: 'गलत PIN। पुनः प्रयास के लिए फिर से निकालें दबाएँ।',
      vault_disable_row: 'वॉल्ट अक्षम करें',
      vault_disable_title: 'वॉल्ट अक्षम करें?',
      vault_disable_message:
        'इससे इस फ़ोन पर वॉल्ट की कुंजी सूची भुला दी जाएगी। कुंजियाँ आपकी YubiKeys पर बनी रहती हैं।',
      vault_disable_confirm: 'अक्षम करें',
      vault_disabled_toast: 'वॉल्ट अक्षम',
      vault_disable_blocked_title: 'पहले राशि निकालें',
      vault_disable_blocked_message:
        'वॉल्ट में अब भी राशि है। बंद करने से पहले सब कुछ अपने रोज़मर्रा के बैलेंस में निकाल लें — वरना वह राशि लॉक हो जाएगी और उस पर हस्ताक्षर करने का कोई तरीका नहीं बचेगा।',
      vault_ok: 'ठीक है',
      vault_mock_toggle: 'मॉक YubiKey उपयोग करें (dev)',
      vault_on: 'चालू',
      vault_off: 'बंद',
      vault_shares_word_count:
        'रिकवरी शेयर केवल बारह-शब्द वाक्यांशों का समर्थन करते हैं। इसके बजाय अपना वाक्यांश लिख लें।',
      scan_shares_legacy_title: 'पुराना बैकअप प्रारूप',
      scan_shares_legacy_message:
        'ये शेयर पुराने प्रारूप के हैं जो आपका रोज़मर्रा का बैलेंस तो लौटा देते हैं पर वॉल्ट नहीं खोल सकते। अगर आपके पास अब भी बारह-शब्द रिकवरी वाक्यांश है, तो उसी से पुनर्स्थापित करें — वह सब कुछ लौटा देता है — और उसके बाद नए शेयर प्रिंट करें। अगर वाक्यांश खो गया है, तो अपनी राशि नए वॉलेट में भेजें और दोबारा शुरू करें।',
      scan_shares_legacy_ack: 'मैं समझ गया',

      delete_wallet: 'वॉलेट हटाएं',
      delete_wallet_warning_title: 'वॉलेट हटाएं?',
      delete_wallet_warning_body:
        'यह इस डिवाइस से आपकी वॉलेट कुंजियाँ स्थायी रूप से हटा देगा। यदि आपने अपना रिकवरी फ्रेज़ बैकअप नहीं किया है या रिकवरी शेयर प्रिंट नहीं किए हैं, तो आप अपने वॉलेट तक हमेशा के लिए पहुँच खो देंगे।',
      delete_wallet_confirm: 'वॉलेट हटाएं',
      delete_wallet_failed_title: 'वॉलेट हटाना विफल रहा',
      delete_wallet_failed_body: 'आपका वॉलेट इस डिवाइस से पूरी तरह हटाया नहीं जा सका। कुछ कुंजी सामग्री अभी भी मौजूद हो सकती है। कृपया पुनः प्रयास करें।',
      bsv_network: 'BSV नेटवर्क',
      arc_endpoint: 'ARC एंडपॉइंट',
      arc_api_token: 'API टोकन',
      arc_custom_url: 'कस्टम URL',
      arc_apply: 'लागू करें',
      arc_reset_default: 'डिफ़ॉल्ट पर रीसेट',
      arc_requires_token: 'टोकन आवश्यक',
      arc_url_https_required: 'https आवश्यक है (स्थानीय परीक्षण के लिए http://localhost मान्य)',
      recovery_phrase: 'रिकवरी कुंजी',
      you_have: 'आपके पास है',
      switching: 'बदल रहा है...',
      mainnet: 'मेननेट',
      testnet: 'टेस्टनेट',
      teratest: 'Teratest',
      identity_key: 'पहचानकर्ता',
      transactions: 'लेनदेन',

      no_transactions: 'अभी तक कोई लेनदेन नहीं',
      tx_status_confirmed: 'पुष्ट',
      tx_status_received: 'प्राप्त हुआ',
      tx_status_sent: 'भेजा गया',
      tx_status_spent: 'खर्च किया गया',
      tx_status_transferred: 'स्थानांतरित',
      tx_status_accepted: 'स्वीकृत',
      tx_status_broadcasting: 'प्रोसेस हो रहा है',
      tx_status_not_sent: 'नहीं भेजा गया',
      tx_status_unsigned: 'ड्राफ़्ट',
      tx_status_nonfinal: 'लंबित',
      tx_status_pending: 'लंबित',
      tx_status_failed: 'विफल',
      tx_status_offline_queued: 'ऑफ़लाइन · कतार में',
      tx_status_offline_sending: 'ऑफ़लाइन · भेजा जा रहा है',
      tx_status_offline_rejected: 'ऑफ़लाइन · अस्वीकृत',
      tx_copied: 'लेनदेन कॉपी किया गया',
      tx_not_available: 'लेनदेन उपलब्ध नहीं है',
      tx_copy_failed: 'लेनदेन कॉपी करने में विफल',
      tx_abort_success: 'लेनदेन रद्द किया गया',
      tx_abort_failed: 'लेनदेन रद्द करने में विफल',
      tx_abort_maybe_delivered: 'रद्द नहीं कर सकते — यह भुगतान प्राप्तकर्ता तक पहले से पहुंच चुका हो सकता है',
      tx_proof_refreshed: 'लेनदेन की पुष्टि हुई',
      tx_proof_refresh_failed: 'प्रमाण नहीं मिल सका — हो सकता है लेनदेन की अभी पुष्टि न हुई हो',
      tx_marked_failed: 'प्रोसेस नहीं हुआ — विफल चिह्नित किया और राशि मुक्त की',
      tx_still_pending: 'अब भी लंबित — अभी पुष्टि नहीं हुई',
      tx_action_explorer: 'ब्लॉक एक्सप्लोरर में देखें',
      tx_export_success: '{{count}} लेनदेन निर्यात किए गए',
      tx_export_failed: 'लेनदेन निर्यात करने में विफल',
      tx_export_csv: 'CSV निर्यात करें',
      tx_export: 'निर्यात करें',
      activity_filter: 'गतिविधि फ़िल्टर करें',
      activity_filter_all: 'सभी',
      activity_search_placeholder: 'नोट, नाम, @हैंडल खोजें',
      activity_search_clear: 'खोज साफ़ करें',
      activity_filter_no_match: 'कोई मेल खाती गतिविधि नहीं',
      wallet_balance_label: 'बैलेंस',
      wallet_balance_you_have: 'आपके पास है',
      wallet_balance_refresh: 'बैलेंस ताज़ा करें',
      wallet_group_today: 'आज',
      wallet_group_yesterday: 'कल',
      back: 'वापस',
      wallet_payments: 'भुगतान',
      wallet_vault: 'वॉल्ट',
      wallet_settings: 'सेटिंग्स',
      wallet_activity: 'गतिविधि',

      message_box_server: 'मैसेज बॉक्स सर्वर',
      message_box_required: 'पहचान-आधारित भुगतान भेजने और प्राप्त करने के लिए आवश्यक है।',
      send_payment: 'भुगतान भेजें',
      recipient: 'प्राप्तकर्ता',
      amount: 'राशि',
      enter_valid_amount: 'शून्य से बड़ी राशि दर्ज करें',
      display_currency: 'प्रदर्शन मुद्रा',
      no_pending_payments: 'कोई लंबित भुगतान नहीं',
      outgoing_payments: 'भेजे गए',
      payment_not_delivered: 'डिलीवर नहीं हुआ',
      payment_delivered: 'डिलीवर हो गया',
      retry: 'पुनः प्रयास',
      retry_failed: 'पुनः प्रयास विफल',
      dismiss: 'खारिज करें',
      valid_identity_key: 'वैध पहचानकर्ता दर्ज किया गया',
      valid_bsv_address: 'वैध BSV पता दर्ज किया गया',
      searching: 'खोज रहा है...',
      unknown: 'अज्ञात',
      from_prefix: 'से:',
      message_box_saved: 'मैसेज बॉक्स सर्वर सहेजा गया',
      message_box_removed: 'मैसेज बॉक्स सर्वर हटाया गया',
      message_box_off: 'बंद',
      message_box_off_hint: 'मैसेज बॉक्स सर्वर बंद है। इसे सेटिंग्स › एडवांस्ड में चालू करें।',
      enter_valid_url: 'कृपया एक वैध URL दर्ज करें',
      checking_connection: 'कनेक्शन जाँच रहे हैं...',
      connection_failed: 'कनेक्शन विफल',
      recipient_placeholder: 'नाम, पहचानकर्ता या BSV पता...',

      trust_updated: 'विश्वास नेटवर्क अपडेट किया गया',
      failed_to_save: 'सहेजने में विफल',
      confirm_delete: 'हटाने की पुष्टि करें',
      confirm_delete_body: 'क्या आप वाकई इस सर्टिफायर को हटाना चाहते हैं?',
      order_certifiers_hint:
        'सर्टिफायरों को प्राथमिकता के अनुसार क्रमबद्ध करें। ऐप्स उच्च-रैंक वाले सर्टिफायरों को पहले दिखाएंगे।',
      certifiers: 'सर्टिफायर',
      no_certifiers: 'अभी तक कोई सर्टिफायर नहीं।',
      add_provider: 'प्रदाता जोड़ें',
      unsaved_changes: 'आपके पास असहेजे परिवर्तन हैं',
      duplicate_key_error: 'इस सार्वजनिक कुंजी वाली संस्था पहले से सूची में है!',
      get_provider_details: 'प्रदाता विवरण प्राप्त करें',
      validate_details: 'विवरण सत्यापित करें',
      show_advanced: 'उन्नत दिखाएं',
      hide_advanced: 'उन्नत छुपाएं',
      description_length_error: 'विवरण 5 से 50 अक्षरों के बीच होना चाहिए',
      enter_domain_hint: 'प्रदाता डोमेन दर्ज करें',
      enter_details_hint: 'प्रदाता विवरण दर्ज करें',

      receive: 'प्राप्त करें',
      send: 'भेजें',
      generating_address: 'BSV पता उत्पन्न हो रहा है...',
      imported: 'आयातित',
      unable_to_generate_address: 'BSV पता उत्पन्न करने में असमर्थ',
      send_bsv: 'BSV भेजें',
      recipient_address: 'प्राप्तकर्ता का BSV पता',
      enter_bsv_address: 'BSV पता दर्ज करें',

      wallet_data: 'वॉलेट डेटा',
      create_new_wallet: 'नया वॉलेट बनाएं',
      generate_recovery_phrase_caption: 'एक नई रिकवरी कुंजी उत्पन्न करें',
      import_existing_wallet: 'मौजूदा वॉलेट आयात करें',
      paste_recovery_phrase: 'अपनी रिकवरी कुंजी पेस्ट करें',
      save_recovery_phrase_heading: 'अपनी रिकवरी कुंजी सहेजें',
      save_recovery_phrase_btn: 'रिकवरी कुंजी सहेजें',
      recovery_phrase_write_down: 'इन 12 शब्दों को क्रम में लिखें और उन्हें सुरक्षित स्थान पर रखें।',
      acknowledgment_text:
        'मैंने अपनी रिकवरी कुंजी सहेज ली है और समझता/समझती हूं कि इसे खोने से सभी संबंधित धनराशि, टोकन और प्रमाणपत्रों का स्थायी नुकसान होगा।',
      enter_recovery_words: '12 शब्दों का नेमोनिक दर्ज करें',
      restore_wallet_description: 'अपना वॉलेट पुनर्स्थापित करने के लिए रिकवरी कुंजी दर्ज करें',
      backup_reminder_title: 'आपके वॉलेट का बैकअप नहीं है',
      backup_reminder_body: 'धनराशि खोने से बचने के लिए अपनी कुंजियाँ निर्यात करें।',
      biometric_advisory_title: 'अपना वॉलेट सुरक्षित करें',
      wallet_balance_show_in: '{{unit}} में दिखाएँ',
      tx_detail_block: 'ब्लॉक',
      tx_detail_block_pending: 'ब्लॉक की प्रतीक्षा',
      biometric_advisory_body: 'अपनी संपत्ति को Face ID या अपनी उंगली की छाप से सुरक्षित करें',
      biometric_advisory_body_degraded:
        'इस डिवाइस पर Face ID या फ़िंगरप्रिंट सेट नहीं है, इसलिए आपकी वॉलेट कुंजी केवल डिवाइस पासकोड से सुरक्षित है।',
      import_prompt_title: 'पहले से वॉलेट है?',
      import_prompt_body: 'इसे अपनी रिकवरी कुंजी या शेयर से पुनर्स्थापित करें।',
      import_prompt_action: 'बैकअप से आयात करें',
      nearby_advisory_title: 'आस-पास कनेक्ट हो रहे हैं',
      nearby_advisory_body:
        'आपसे Wi-Fi, ब्लूटूथ या NFC की अनुमति माँगी जाएगी। इसका उपयोग केवल भुगतान के लिए आपको पास के डिवाइस से सीधे जोड़ने में होता है — यह कभी हमारे सर्वर पर नहीं भेजा जाता और न ही आपकी प्रोफ़ाइल बनाने में उपयोग होता है।',
      backup_reminder_now: 'अभी कुंजियों का बैकअप लें',
      backup_reminder_import: 'बैकअप से आयात करें',
      restore_backup_checking: 'आपका एन्क्रिप्टेड बैकअप खोजा जा रहा है…',
      restore_backup_progress: 'वॉलेट इतिहास पुनर्स्थापित हो रहा है — {{total}} में से {{chunks}}…',
      restore_backup_failed_title: 'इतिहास पुनर्स्थापित नहीं हो सका',
      restore_backup_failed_message:
        'आपका रिकवरी फ़्रेज़ ठीक है, लेकिन एन्क्रिप्टेड बैकअप दोहराया नहीं जा सका। आमतौर पर दोबारा कोशिश करना पर्याप्त होता है। इसके बिना जारी रखने पर वॉलेट काम करेगा लेकिन कोई पुराना लेनदेन नहीं होगा, और आपके कुछ मौजूदा सिक्के खर्च नहीं हो पाएँगे।',
      restore_backup_retry: 'पुनः प्रयास करें',
      restore_backup_skip: 'छोड़ें',
      restore_backup_unverified_title: 'बैकअप अधूरा हो सकता है',
      restore_backup_unverified_body:
        'आपका वॉलेट पुनर्स्थापित हो गया है, लेकिन सर्वर पर मौजूद बैकअप के पूर्ण होने की पुष्टि नहीं हो सकी। यदि कुछ इतिहास गायब है, तो दूसरे डिवाइस का अपलोड पूरा होने के बाद दोबारा पुनर्स्थापित करें।',
      import_wallet: 'वॉलेट आयात करें',

      reject: 'अस्वीकार करें',
      authorize: 'अधिकृत करें',
      renewal: 'नवीनीकरण',
      spending_section: 'खर्च',
      protocols_section: 'प्रोटोकॉल',
      baskets_section: 'बास्केट',
      certificates_section: 'प्रमाणपत्र',
      requested_fields: 'अनुरोधित फ़ील्ड',

      send_max: 'अधिकतम भेजें',
      message_box_unreachable: 'भुगतान सर्वर से संपर्क नहीं हो सका। सेटिंग्स में message box URL जांचें।',
      available: 'उपलब्ध',
      note: 'नोट',
      note_placeholder: 'किसलिए? (वैकल्पिक)',
      entire_wallet_balance: 'पूरी वॉलेट शेष राशि',
      entire_vault_balance: 'पूरा वॉल्ट बैलेंस',
      export_wallet_data: 'वॉलेट डेटा निर्यात करें',
      export_unencrypted_title: 'यह फ़ाइल एन्क्रिप्टेड नहीं है',
      export_unencrypted_message:
        'निर्यात की गई फ़ाइल आपके वॉलेट डेटा की एक सादी, अनएन्क्रिप्टेड प्रति है — जिसमें पहचान और प्रमाणपत्र जानकारी, लेन-देन इतिहास और संपर्क शामिल हैं। यह फ़ाइल जिसके पास भी हो, वह इसे पढ़ सकता है। इसे केवल किसी विश्वसनीय जगह ही साझा करें।',
      import_wallet_data: 'वॉलेट डेटा आयात करें',
      import_conflict_title: 'डेटाबेस पहले से मौजूद है',
      import_conflict_message:
        'इस पहचान के लिए एक अधिक हाल के टाइमस्टैम्प वाला वॉलेट डेटाबेस पहले से मौजूद है। फ़ाइल नाम में सबसे बड़ी संख्या वाला बैकअप हमेशा उपयोग किया जाता है।\n\nइस पुराने बैकअप का उपयोग करने के लिए: पहले अपना वर्तमान वॉलेट डेटा निर्यात करें, ऐप हटाएं और पुनः इंस्टॉल करें, फिर केवल वांछित बैकअप आयात करें।',
      import_confirm_title: 'डेटाबेस आयात करें',
      import_confirm_message: 'यह डेटाबेस आपका सक्रिय वॉलेट बन जाएगा। जारी रखें?',
      import_success: 'डेटाबेस सफलतापूर्वक आयात किया गया। वॉलेट पुनः आरंभ हो रहा है...',
      import_invalid_file: 'अमान्य बैकअप फ़ाइल',
      import_invalid_file_detail:
        'यह फ़ाइल एक मान्य वॉलेट डेटाबेस बैकअप नहीं है। अपेक्षित फ़ाइल नाम जैसे wallet-XXXXXXXX-mainnet-TIMESTAMP.db।',
      import_oversized_file: 'बैकअप फ़ाइल बहुत बड़ी है',
      import_oversized_file_detail:
        'यह फ़ाइल वॉलेट बैकअप के लिए अपेक्षित आकार से बड़ी है, इसलिए इसे नहीं पढ़ा गया। कृपया एक वास्तविक वॉलेट डेटाबेस बैकअप फ़ाइल चुनें।',
      import_untrusted_file: 'यह बैकअप आयात नहीं किया जा सका',
      import_untrusted_file_detail:
        'यह बैकअप इस वॉलेट से संबंधित सत्यापित नहीं किया जा सका, या इसकी सामग्री पर भरोसा नहीं किया जा सका। इसे आयात नहीं किया गया।',
      import_anyway: 'फिर भी आयात करें',
      import_invalid_input_title: 'अमान्य इनपुट',
      import_invalid_input_message:
        'कृपया एक मान्य रिकवरी फ़्रेज़ (12–24 शब्द) या 64-अक्षर की हेक्स प्राइवेट की दर्ज करें।',
      import_setup_failed: 'वॉलेट सेट अप करने में विफल: {{error}}',
      create_wallet_refused: 'वॉलेट नहीं बनाया जा सका। कृपया पुनः प्रयास करें।',
      create_wallet_failed: 'निमोनिक जनरेट करने में विफल। कृपया पुनः प्रयास करें।',
      or: 'या',
      print_recovery_shares: 'रिकवरी शेयर प्रिंट करें',
      scan_backup_shares: 'बैकअप शेयर स्कैन करें',
      scan_backup_shares_caption: 'प्रिंटेड शेयर से वॉलेट पुनर्प्राप्त करें',
      scan_shares_invalid_format: 'अमान्य शेयर प्रारूप। कृपया एक वैध बैकअप शेयर QR कोड स्कैन करें।',
      scan_shares_camera_needed: 'कैमरा एक्सेस आवश्यक',
      scan_shares_camera_description: 'आपके प्रिंटेड बैकअप शेयर QR कोड स्कैन करने के लिए कैमरा एक्सेस आवश्यक है।',
      scan_shares_grant_camera: 'कैमरा एक्सेस दें',
      scan_shares_recovering: 'वॉलेट पुनर्प्राप्त हो रहा है...',
      scan_shares_scan_first: 'अपना पहला बैकअप शेयर स्कैन करें',
      scan_shares_progress: '{{scanned}}/{{needed}} शेयर स्कैन किए गए',
      scan_shares_remaining: '{{count}} और शेयर आवश्यक',
      scan_shares_complete: 'सभी शेयर एकत्रित!',
      scan_shares_recovery_failed: 'पुनर्प्राप्ति विफल। कृपया पुनः स्कैन करें।',
      scan_shares_threshold_mismatch: 'थ्रेशोल्ड पिछले शेयरों से मेल नहीं खाता',
      scan_shares_integrity_mismatch: 'अखंडता हैश मेल नहीं खाता — शेयर विभिन्न कुंजियों से हैं',
      scan_shares_duplicate: 'यह शेयर पहले से ही स्कैन किया जा चुका है',
      scan_shares_biometric_title: 'बायोमेट्रिक एक्सेस आवश्यक',
      scan_shares_biometric_message:
        'आपके वॉलेट की कुंजियों की सुरक्षा के लिए बायोमेट्रिक एक्सेस आवश्यक है। कृपया पुनः प्रयास करें।',
      recovery_replace_wallet_title: 'इस फ़ोन पर मौजूद वॉलेट बदलें?',
      recovery_replace_wallet_body:
        'इस फ़ोन में पहले से एक वॉलेट मौजूद है। जारी रखने पर यह उसे और उससे खर्च होने वाली हर चीज़ को बदल देगा। यदि उसका रिकवरी फ़्रेज़ कहीं सहेजा नहीं गया है, तो वह पैसा हमेशा के लिए खो जाएगा।',
      recovery_replace_wallet_confirm: 'वॉलेट बदलें',
      invalid_bsv_address: 'अमान्य BSV पता',
      scan_recipient_hint: 'कैमरे को भुगतान कोड पर लगाएं',
      scan_qr_default_hint: 'कैमरे को QR कोड पर लगाएं',
      scan_wallet_qr_hint: 'कैमरे को वॉलेट पेयरिंग QR कोड पर लगाएं',

      biometric_store_wallet: 'अपने वॉलेट को सुरक्षित रूप से संग्रहित करें',
      biometric_load_wallet: 'अपने वॉलेट को सुरक्षित रूप से लोड करें',
      biometric_unlock_wallet: 'अपना वॉलेट अनलॉक करें',
      wallet_lost_title: 'वॉलेट कुंजी नष्ट हो गई',
      wallet_lost_body:
        'इस डिवाइस की बायोमेट्रिक सेटिंग्स बदल गईं, इसलिए सिस्टम ने इस वॉलेट की सुरक्षा करने वाली कुंजी नष्ट कर दी। आपके सिक्के ब्लॉकचेन पर सुरक्षित हैं — रिकवरी वाक्यांश से इस वॉलेट को पुनर्स्थापित करें।',
      wallet_lost_restore: 'बहाल करें',
      wallet_lost_later: 'अभी नहीं',
      wallet_locked_title: 'वॉलेट लॉक है',
      wallet_locked_body: 'अपनी वॉलेट कुंजियाँ लोड करने के लिए अनलॉक करें।',
      wallet_unlock_retry: 'वॉलेट अनलॉक करें',
      wallet_lockout_body: 'बहुत अधिक असफल प्रयास। पहले अपने फ़ोन को पासकोड से अनलॉक करें, फिर पुनः प्रयास करें।',
      wallet_no_biometrics_body:
        'इस डिवाइस पर बायोमेट्रिक्स सेट नहीं है। आपकी वॉलेट कुंजियाँ अभी भी डिवाइस कीस्टोर द्वारा एन्क्रिप्टेड हैं, लेकिन बायोमेट्रिक जाँच से सुरक्षित नहीं हैं।',

      // Connections screen
      connections: 'कनेक्शन',
      advanced: 'उन्नत',
      copy_secret_words: 'गुप्त शब्द कॉपी करें',
      print_recovery_keys: 'रिकवरी कुंजियाँ प्रिंट करें',
      backup_wallet_keys: 'वॉलेट की कुंजियों का बैकअप लें',
      log_out: 'लॉग आउट',
      connect_to_app: 'ऐप से कनेक्ट करें',
      no_connections_yet: 'अभी तक कोई कनेक्शन नहीं',
      no_connections_subtitle: 'अपने वॉलेट को कनेक्ट करने के लिए डेस्कटॉप ऐप से QR कोड स्कैन करें',
      connections_active: 'सक्रिय',
      connections_disconnected: 'डिस्कनेक्टेड',
      disconnect: 'डिस्कनेक्ट',
      reconnect: 'पुनः कनेक्ट',
      reconnect_failed: 'पुनः कनेक्ट विफल',
      reconnect_confirm_title: 'इस साइट से पुनः कनेक्ट करें?',
      reconnect_confirm_message: '{{origin}} फिर से आपके वॉलेट तक पहुंच पाएगी।',
      reconnect_needs_reapproval: 'इस कनेक्शन को सत्यापित नहीं किया जा सका। पुनः कनेक्ट करने के लिए नया क्यूआर कोड स्कैन करें।',
      scan_qr_code: 'QR कोड स्कैन करें',
      paste_uri: 'URI पेस्ट करें',
      invalid_qr_code: 'अमान्य QR कोड',
      wallet_not_ready: 'वॉलेट तैयार नहीं',
      please_log_in_first: 'पहले लॉग इन करें',
      unknown_error: 'अज्ञात त्रुटि',

      // Pay screen
      payments: 'भुगतान',
      pay: 'भुगतान',
      pay_direction_pay: 'भुगतान करें',
      pay_direction_receive: 'भुगतान पाएं',
      pay_cell_nearby_pay: 'नज़दीक मौजूद कोई व्यक्ति',
      pay_cell_nearby_pay_sub: 'उनका कोड स्कैन करें',
      pay_cell_handle_pay: 'कोई दूर का व्यक्ति',
      pay_cell_handle_pay_sub: 'कोई नाम चुनें — उन्हें यह ऐप चाहिए',
      pay_cell_address_pay: 'किसी BSV पते पर',
      pay_cell_address_pay_sub: 'BSV पता चिपकाएं या स्कैन करें',
      pay_cell_nearby_get: 'नज़दीक मौजूद कोई व्यक्ति',
      pay_cell_nearby_get_sub: 'अपना भुगतान कोड दिखाएं',
      pay_cell_handle_get: 'कोई दूर का व्यक्ति',
      pay_cell_handle_get_sub: 'अपना नाम साझा करें',
      pay_cell_address_get: 'किसी BSV पते पर',
      pay_cell_address_get_sub: 'एक BSV पता दिखाएं',
      pay_method: 'तरीका',
      pay_method_nearby: 'नज़दीक',
      pay_method_remote_link: 'रिमोट लिंक साझा करें',
      pay_method_address: 'किसी BSV पते पर',
      pay_pre_nearby: 'आप दोनों यहीं मौजूद हैं।',
      pay_pre_handle: 'वे इस ऐप का उपयोग करते हैं।',
      pay_pre_address: 'आपके पास उनका BSV पता है।',
      pay_conseq_nearby: 'कुछ ही सेकंड में पूरा हो जाता है।',
      pay_conseq_handle: 'उनका वॉलेट अगली बार जांचने पर पहुंच जाएगा।',
      pay_conseq_address: 'पैसा सीधे भेजा जाता है — उन्हें कोई सूचना नहीं मिलती।',
      paid: 'भुगतान हो गया',
      pay_inbox_attention: 'वॉलेट में नहीं जोड़ा जा सका',
      pay_inbox_attention_hint:
        'ये आ गए हैं लेकिन आपका वॉलेट इन्हें जोड़ नहीं सका। फिर कोशिश करें, या हमेशा के लिए छोड़ने हेतु खारिज करें।',
      pay_dismiss_confirm: 'छोड़ने के लिए दोबारा टैप करें',
      pay_dismissed: 'भुगतान छोड़ दिया गया',
      payment_arrived: 'भुगतान प्राप्त हुआ',
      payment_arrived_damaged: 'यह भुगतान क्षतिग्रस्त अवस्था में आया',
      discard_payment_title: 'यह भुगतान छोड़ें?',
      discard_payment_body: 'भेजने वाले से विवरण दोबारा भेजने को कहा जाएगा। तब तक पैसा चेन पर ही रहता है।',
      discard_void_title: 'यह भुगतान पूरा नहीं हुआ',
      discard_void_body: 'यह रद्द हो गया या कहीं और खर्च हो गया। इसे हटाना सुरक्षित है।',
      waiting_network_confirm: 'इस भुगतान की पुष्टि के लिए नेटवर्क का इंतज़ार',
      finish_or_cancel_outgoing: 'नया भुगतान भेजने से पहले नीचे का भुगतान पूरा करें या रद्द करें',
      identity_search_unavailable: 'खोज अभी उपलब्ध नहीं है। थोड़ी देर बाद पुनः प्रयास करें।',
      abandon_payment: 'भुगतान छोड़ें',
      finish_payment: 'भुगतान पूरा करें',
      cancel_this_payment: 'यह भुगतान रद्द करें?',
      cancel_payment: 'भुगतान रद्द करें',
      resend: 'दोबारा भेजें',
      resending: 'दोबारा भेजा जा रहा है…',
      send_payment_details_again: 'विवरण दोबारा भेजें',
      tx_action_resend_short: 'फिर भेजें',
      tx_action_request_confirmation: 'प्राप्तकर्ता से पुष्टि का अनुरोध करें',
      resend_sent: 'भुगतान विवरण फिर भेजा गया',
      resend_failed_no_record: 'इस डिवाइस पर इस भुगतान का विवरण नहीं मिला, इसलिए इसे दोबारा नहीं भेजा जा सकता।',
      resend_failed_no_recipient:
        'इस भुगतान में यह दर्ज नहीं है कि यह किसके लिए था, इसलिए इसे दोबारा नहीं भेजा जा सकता।',
      resend_failed_no_transaction: 'इस भुगतान का डेटा नहीं मिल सका। ऑनलाइन होने पर फिर कोशिश करें।',
      resend_requested: 'उन्होंने आपसे भुगतान दोबारा भेजने को कहा',
      request_again: 'दोबारा अनुरोध करें',
      send_again: 'दोबारा भेजें',
      copy_details: 'विवरण कॉपी करें',
      payment_bounced_resend: 'उनसे यह भुगतान दोबारा भेजने को कहा गया',
      dismiss_rejected_payment: 'अस्वीकृत भुगतान हटा दिया',
      activity_load_failed: 'गतिविधि लोड नहीं हो सकी',
      activity_load_retry: 'पुनः प्रयास के लिए टैप करें',
      activity_view_contact: 'संपर्क देखें',
      home_payments_need_attention: '{{count}} भुगतानों पर ध्यान देना है',
      home_payments_unsent: '{{count}} भुगतान डिलीवर नहीं हुए',
      pay_copy: 'कॉपी',
      pay_share_link: 'लिंक साझा करें',
      pay_address_watching: 'यहां भेजा गया पैसा अपने आप आपके वॉलेट में जुड़ जाता है।',
      pay_address_earlier_day: 'किसी पिछले दिन तक जाएं',
      pay_address_sweep_now: 'यह BSV पता अभी जांचें',
      pay_address_swept: '{{amount}} प्राप्त हुआ',

      check_wallet: 'समस्या निवारण',
      settings_version: 'v{{version}}',
      error_review_actions: 'यह भुगतान अटका है। समस्या निवारण इसे छुड़ाने की कोशिश कर सकता है।',
      wallet_check_ok: 'सब कुछ ठीक लग रहा है',
      wallet_check_attention: 'कुछ जाँचों पर ध्यान दें',
      wallet_check_couldnt: 'जाँच नहीं हो सकी',
      wallet_check_couldnt_body: 'एक मरम्मत चरण पूरा नहीं हुआ। आपके सिक्कों के बारे में कोई अनुमान नहीं लगाया गया।',
      wallet_check_summary: '{{freed}} अटके सिक्के छुड़ाए · {{recovered}} भुगतान पुनर्प्राप्त किए',
      wallet_check_step_coins: 'आपके सिक्के मान्य हैं',
      wallet_check_skip: 'यह जाँच छोड़ें',
      wallet_check_skipped: 'छोड़ा गया',
      wallet_check_step_proofs: 'ट्रांज़ैक्शन प्रमाण',
      wallet_check_step_records: 'लोकल डेटाबेस रिकॉर्ड',
      wallet_check_step_tokens: 'स्टेबलकॉइन ट्रांसफ़र सत्यापित',
      wallet_check_step_online: 'ऑनलाइन',
      wallet_check_step_missed: 'छूटे हुए भुगतान',
      wallet_check_step_backup: 'निजी बैकअप अपलोड',
      wallet_check_step_phrase: 'रिकवरी फ़्रेज़ सेव',
      wallet_check_progress: '{{total}} में से {{current}}',
      wallet_check_stuck_title: 'यह भुगतान भेजा नहीं जा सका',
      wallet_check_stuck_message: 'कुछ सिक्के अटके लगते हैं। समस्या निवारण उन्हें छुड़ा सकता है।',
      wallet_check_failed: 'जाँच पूरी नहीं हो सकी। पुनः प्रयास करें।',

      // Local payments
      local_pay_request: 'भुगतान का अनुरोध करें',
      local_pay_send: 'भुगतान भेजें',
      local_pay_amount: 'राशि',
      local_pay_show_qr: 'यह कोड भुगतानकर्ता को दिखाएं',
      local_pay_scan_qr: 'प्राप्तकर्ता का कोड स्कैन करें',
      local_pay_scan_payer_qr: 'भुगतानकर्ता का कोड स्कैन करें',
      local_pay_scan_progress: '{{total}} में से {{have}} हिस्से',
      local_pay_show_payment_qr: 'यह कोड प्राप्तकर्ता को दिखाएं',
      local_pay_animated_hint:
        'उनसे कहें कि वे नीचे दिया गया चलता हुआ कोड स्कैन करें — यह तब तक बदलता रहेगा जब तक उनके फ़ोन को पूरा डेटा न मिल जाए।',
      local_pay_show_qr_instead: 'इसके बजाय QR कोड दिखाएं',
      local_pay_preparing: 'अनुरोध तैयार किया जा रहा है…',
      local_pay_delivering: 'भुगतान भेजा जा रहा है…',
      local_pay_saving: 'भुगतान सहेजा जा रहा है…',
      local_pay_sent: 'भुगतान भेज दिया गया',
      local_pay_received: 'भुगतान प्राप्त हुआ',
      local_pay_added: 'आपके वॉलेट में जोड़ दिया गया।',
      local_pay_added_multiple: '{{count}} भुगतान आपके वॉलेट में जोड़ दिए गए।',
      local_pay_received_confirming: 'आपके वॉलेट से पुष्टि हो रही है…',
      local_pay_received_not_credited: 'अभी आपके वॉलेट में नहीं जोड़ा गया है। कुछ भी सौंपने से पहले गतिविधि जांच लें।',
      local_pay_decline_broadcast_title: 'अस्वीकृत भुगतान नेटवर्क पर चला गया',
      local_pay_decline_broadcast_body:
        'जो भुगतान अस्वीकृत किया गया था वह बाद में नेटवर्क पर दिखाई दिया है। इसे फिर से न भेजें — गतिविधि जांचें।',
      local_pay_pending_abort_dropped_title: 'एक असत्यापित रिलीज़ अनुरोध को अनदेखा किया गया',
      local_pay_pending_abort_dropped_body:
        'एक सहेजा गया भुगतान-रिलीज़ रिकॉर्ड सत्यापित नहीं किया जा सका, इसलिए कुछ भी रिलीज़ नहीं किया गया। यदि कोई भुगतान अभी भी अटका हुआ दिखे, तो उसे गतिविधि से रद्द करें।',
      local_pay_cancel_unverifiable_title: 'ऑफ़लाइन रहते हुए यह भुगतान जांचा नहीं जा सकता',
      local_pay_cancel_unverifiable_body:
        'अगर दूसरे व्यक्ति ने इसे पहले ही स्कैन कर लिया है, तो रद्द करने से ये सिक्के दोबारा खर्च हो सकते हैं। केवल तभी रद्द करें जब आप सुनिश्चित हों कि इसे कभी स्कैन नहीं किया गया।',
      local_pay_cancel_unverifiable_confirm: 'फिर भी रद्द करें',
      local_pay_queued: 'सहेज लिया गया — यह अपने आप आपके वॉलेट में जुड़ जाएगा।',
      local_pay_already_paid: 'इस अनुरोध का भुगतान पहले ही हो चुका है।',
      local_pay_failed: 'भुगतान विफल',
      local_pay_network_denied:
        'लोकल नेटवर्क एक्सेस बंद है। नज़दीकी डिवाइस को भुगतान करने के लिए इसे सेटिंग्स में चालू करें।',
      local_pay_radio_fallback: 'वायरलेस लिंक उपलब्ध नहीं — इसके बजाय यह कोड दूसरे डिवाइस को दिखाएँ।',
      local_pay_floor_peer_bt_off: 'उनका ब्लूटूथ बंद है, इसलिए यह भुगतान कोड से होगा।',
      local_pay_floor_local_bt_off: 'आपका ब्लूटूथ बंद है। तेज़ नज़दीकी लिंक के लिए इसे चालू करें।',
      local_pay_floor_local_ble_denied:
        'BSV Wallet के लिए ब्लूटूथ एक्सेस बंद है। तेज़ नज़दीकी लिंक के लिए इसे सेटिंग्स में अनुमति दें।',
      local_pay_floor_cross_os: 'दूसरा डिवाइस अभी ब्लूटूथ का उपयोग नहीं कर सकता, इसलिए यह भुगतान कोड से होगा।',
      local_pay_floor_peer_no_radio: 'दूसरा डिवाइस वायरलेस लिंक के लिए सुन नहीं रहा है, इसलिए यह भुगतान कोड से होगा।',
      local_pay_ble_unavailable: 'ब्लूटूथ उपलब्ध नहीं है। ऊपर दिया गया कोड अब भी काम करता है।',
      local_pay_too_large: 'इस भुगतान का डेटा QR कोड से देने के लिए बहुत बड़ा है।',
      local_pay_nearby_unavailable: 'नज़दीकी Wi-Fi उपलब्ध नहीं है। ऊपर दिया गया कोड अब भी काम करता है।',
      local_pay_retry_save: 'फिर से सहेजने की कोशिश करें',
      local_pay_wrong_session:
        'वह भुगतान इस अनुरोध से मेल नहीं खाता, इसलिए स्वीकार नहीं किया गया। भुगतानकर्ता से कोड दोबारा स्कैन करने को कहें।',
      local_pay_declined_mismatch:
        'प्राप्तकर्ता ने यह भुगतान स्वीकार नहीं किया — यह उनके अनुरोध से मेल नहीं खाता। उनका कोड दोबारा स्कैन करें।',
      local_pay_declined_already_paid: 'उस अनुरोध का भुगतान पहले ही हो चुका है। कुछ भी नहीं भेजा गया।',
      local_pay_declined_save: 'प्राप्तकर्ता भुगतान सहेज नहीं सका। कुछ भी नहीं भेजा गया — फिर से कोशिश करें।',
      local_pay_declined_decode: 'प्राप्तकर्ता भुगतान पढ़ नहीं सका। कुछ भी नहीं भेजा गया — फिर से कोशिश करें।',
      local_pay_declined_not_covered:
        'प्राप्तकर्ता इस भुगतान को जारीकर्ता के रिकॉर्ड से सत्यापित नहीं कर सका। कुछ भी नहीं भेजा गया।',
      local_pay_broadcast_pending: 'भेज दिया गया। यह डिवाइस इसे अभी प्रोसेस नहीं करा सका — प्राप्तकर्ता करा देगा।',
      local_pay_any_amount: 'कोई भी राशि',
      local_pay_choose_amount: 'राशि चुनें',
      local_pay_enter_amount_send: 'आप कितनी राशि भेजना चाहते हैं?',
      local_pay_qr_unavailable: 'यह कोड नहीं बनाया जा सका। अनुरोध दोबारा शुरू करें।',
      local_pay_presence_qr: 'कोड के ज़रिए — कोई लाइव कनेक्शन नहीं',
      local_pay_presence_ready: 'पास का एन्क्रिप्टेड कनेक्शन तैयार है',
      local_pay_presence_waiting_payee: 'पास के डिवाइस की प्रतीक्षा हो रही है',
      local_pay_presence_waiting_payer: 'प्राप्तकर्ता का डिवाइस खोजा जा रहा है…',
      local_pay_presence_linked: 'एन्क्रिप्टेड कनेक्शन चालू है',
      local_pay_presence_paid: 'भुगतान की पुष्टि हो गई',
      pay_offline_title: 'आप ऑफ़लाइन हैं',
      pay_offline_body: 'अभी केवल नज़दीकी भुगतान काम करते हैं। दोबारा नेटवर्क मिलते ही ये प्रोसेस हो जाएँगे।',
      pay_offline_queued: '{{count}} भुगतान नेटवर्क मिलने पर प्रोसेस होने की प्रतीक्षा में हैं।',
      pay_offline_pending_title: 'प्रोसेस होने की प्रतीक्षा में',
      pay_offline_pending_body:
        '{{count}} भुगतान अभी प्रोसेस नहीं हुए हैं। जब तक नहीं होते, कुछ भी पूरा नहीं हुआ — कोशिश जारी रहेगी।',
      pay_offline_needs_internet: 'इंटरनेट ज़रूरी है',
      pay_offline_rejected_title: 'एक भुगतान अस्वीकार कर दिया गया',
      pay_offline_rejected_body:
        'नेटवर्क ने {{sender}} द्वारा {{via}} के ज़रिए {{when}} को भेजा गया भुगतान अस्वीकार कर दिया।',
      pay_offline_unknown_sender: 'एक अज्ञात भेजने वाला',
      pay_offline_unknown_via: 'एक अज्ञात माध्यम',
      pay_received_not_broadcast: 'ऑफ़लाइन प्राप्त हुआ · अभी प्रोसेस नहीं हुआ',
      pay_offline_sent_rejected_title: 'एक भुगतान पहुंचाया नहीं जा सका',
      pay_offline_sent_rejected_body: 'नेटवर्क ने {{when}} को आपके द्वारा भेजा गया भुगतान अस्वीकार कर दिया।',
      pay_offline_send_now: 'अभी भेजें',
      pay_offline_stalled_body: 'कुछ कतारबद्ध भुगतान स्वतः नहीं भेजे जा सकते: {{detail}}',
      offline_stall_no_request: 'कतार में मौजूद एक भुगतान का भेजने का रिकॉर्ड नहीं मिल रहा।',
      offline_stall_bad_beef: 'कतार में मौजूद एक भुगतान का लेनदेन डेटा पढ़ा नहीं जा सका।',
      offline_stall_foreign_ancestor:
        'कतार में मौजूद एक भुगतान ऐसे मूल लेनदेन का इंतज़ार कर रहा है जिसे किसी दूसरे वॉलेट ने कभी प्रोसेस नहीं किया।',
      pay_offline_kv_pending: '{{count}} आस-पास के भुगतान इस वॉलेट में जुड़ने के इंतज़ार में हैं।',
      pay_offline_kv_stuck: '{{count}} नज़दीकी भुगतान जोड़े नहीं जा सके। भेजने वाले से दोबारा भेजने को कहें।',
      pay_offline_kv_corrupt: 'इस डिवाइस पर क्षतिग्रस्त भुगतान डेटा मिला।',
      pay_offline_kv_corrupt_body:
        'प्रभावित भुगतान इस डिवाइस पर अलग रख दिए गए हैं, मिटाए नहीं गए। इनमें से कुछ पहले ही आपके बैलेंस में जुड़ चुके हो सकते हैं, इसलिए भेजने वाले से दोबारा भेजने को तभी कहें जब उनका भुगतान अब भी नहीं मिला हो।',
      pay_offline_show_code: 'कोड फिर दिखाएँ',
      pay_parked_cancel: 'भुगतान रद्द करें',
      pay_parked_already_sent: 'यह भुगतान पहले ही भेज दिया गया है',
      token_cancel_already_sent: 'यह भुगतान पहले ही पूरा हो चुका है और इसे रद्द नहीं किया जा सकता।',
      tx_status_parked: 'सौंपा नहीं गया',
      tx_status_import_hold: 'आयात के बाद रोका गया',
      resend_sending: 'मैसेज बॉक्स से भेजा जा रहा है',
      cancelling_payment: 'भुगतान रद्द किया जा रहा है',

      contacts: 'संपर्क',
      contacts_search_placeholder: 'नाम या हैंडल से खोजें',
      contacts_empty_title: 'अभी तक कोई संपर्क नहीं',
      contacts_empty_body:
        'जिन लोगों को आप भुगतान करते हैं, वे सहेजे जाने के बाद यहाँ दिखाई देंगे। ऊपर दाईं ओर के बटन से किसी की प्रोफ़ाइल कोड स्कैन करें।',
      contacts_recent: 'हाल के',
      contact_edit_name: 'नाम संपादित करें',
      contact_save_name: 'नाम सहेजें',
      contact_activity: 'गतिविधि',
      contact_activity_empty: 'इस संपर्क के साथ अभी तक कोई गतिविधि नहीं',
      contact_delete: 'संपर्क हटाएं',
      contact_delete_body: 'इससे {{name}} आपके संपर्कों से हट जाएगा। इससे पिछले भुगतानों पर कोई असर नहीं पड़ेगा।',
      contact_handle_caption: 'उनका पंजीकृत हैंडल · केवल वे ही इसे बदल सकते हैं',
      contact_new_title: 'नया संपर्क',
      contact_save: 'सहेजें',
      contact_identifier: 'पहचानकर्ता',
      contact_saved: 'संपर्क सहेजा गया',
      contact_scan_hint: 'किसी को जोड़ने के लिए प्रोफ़ाइल कोड स्कैन करें',
      contact_identifier_placeholder: 'उनका पहचानकर्ता पेस्ट करें',
      contact_identifier_invalid: 'यह एक मान्य पहचानकर्ता नहीं लगता',
      contact_add_from_payment: 'संपर्कों में जोड़ें',

      profile: 'प्रोफ़ाइल',
      profile_display_name: 'प्रदर्शन नाम',
      profile_display_name_hint:
        'आपके हैंडल के बगल में सार्वजनिक रूप से दिखता है। आपको खोजने वाला कोई भी इसे देख सकता है।',
      profile_handle: 'हैंडल',
      profile_handle_placeholder: 'आपका हैंडल',
      profile_handle_checking: 'उपलब्धता जांची जा रही है…',
      profile_handle_available: '{{handle}} उपलब्ध है',
      profile_handle_taken: '{{handle}} पहले से लिया जा चुका है',
      profile_handle_invalid: '3-32 अक्षर: a-z, 0-9, बिंदु, अंडरस्कोर या हाइफ़न; शुरू और अंत अक्षर या अंक से',
      profile_handle_failed: 'उपलब्धता जांच नहीं सकी। फिर से प्रयास करें।',
      profile_handle_unavailable: 'इस बिल्ड में हैंडल पंजीकरण अभी उपलब्ध नहीं है।',
      profile_handle_registered: 'हैंडल पंजीकृत हो गया',
      profile_handle_register_action: 'पंजीकृत करें',
      profile_handle_too_similar: '{{handle}} पहले से मौजूद किसी नाम से बहुत मिलता-जुलता है',
      profile_handle_reserved: '{{handle}} आरक्षित है',
      profile_handle_cooldown: '{{handle}} हाल ही में छोड़ा गया था और अभी उपलब्ध नहीं है',
      profile_handle_pending: 'पंजीकरण पूरा किया जा रहा है…',
      profile_handle_rolled_back: '{{handle}} ले लिया गया था। आपके पास {{previous}} बना रहा।',
      profile_handle_changed: 'हैंडल बदल गया',
      profile_handle_rejected: 'रजिस्ट्री ने उस हैंडल को अस्वीकार कर दिया।',
      profile_handle_clock_ahead: 'आपके डिवाइस की घड़ी बहुत आगे है। कुछ मिनट बाद फिर कोशिश करें।',
      profile_handle_wrong_registry: 'वह हैंडल इस रजिस्ट्री पर नहीं है। इसे ताज़ा करने के लिए प्रोफ़ाइल दोबारा खोलें।',
      profile_display_name_publish_failed: 'आपका नाम यहाँ सहेज लिया गया, लेकिन रजिस्ट्री ने इसे प्रकाशित नहीं किया।',
      profile_qr_hint: 'दूसरे लोग इसे स्कैन करके आपको संपर्क के रूप में जोड़ सकते हैं',

      pay_step_contacts: 'संपर्क',
      pay_step_continue: 'जारी रखें',
      pay_review_to: 'प्रति',
      pay_review_amount: 'राशि',
      pay_review_note: 'नोट',
      pay_review_note_edit: 'नोट जोड़ें',
      pay_review_delivery_host: 'इस पते पर भेजा जा रहा है',
      pay_trust_contact: 'आपका संपर्क',
      pay_trust_handle_attested: 'पंजीकृत',
      pay_trust_handle_domain_attested: '{{domain}} द्वारा सत्यापित',
      pay_trust_unverified: 'सत्यापित नहीं। कृपया इस पहचानकर्ता की दोबारा जांच करें।',
      pay_address_network_mismatch: 'यह पता आपके द्वारा उपयोग किए जा रहे नेटवर्क से अलग नेटवर्क का है।',
      contacts_your_contacts: 'आपके संपर्क',
      contacts_other_people: 'अन्य लोग',
      contacts_not_saved: 'सहेजा नहीं',
      contacts_network_footer: 'नेटवर्क पर मिला। आपने सहेजा नहीं है।',
      contact_no_handle: 'अभी कोई हैंडल नहीं',
      contact_no_handle_registered: 'कोई हैंडल पंजीकृत नहीं',
      contact_name_section: 'उनके लिए आपका नाम',
      contact_name_hint: 'यह केवल आप देखते हैं। इससे वे खुद को क्या कहते हैं, नहीं बदलता।',
      contact_new_name_hint: 'यह केवल आप देखते हैं। इस डिवाइस पर सहेजा गया।',
      contact_copy_identifier: 'पहचानकर्ता कॉपी करें',
      profile_handle_registered_hint: 'आपके पहचानकर्ता पर पंजीकृत। लोग इससे आपको खोज सकते हैं।',
      profile_handle_change: 'बदलें',
      profile_handle_claim: '{{handle}} लें',
      profile_handle_replace_warning:
        'लेने से {{handle}} बदल जाएगा। जिन्होंने आपको सहेजा है उनका संपर्क बना रहेगा; पुराना हैंडल अब आप तक नहीं पहुँचेगा।',
      profile_identifier_section: 'आपका पहचानकर्ता',
      profile_show_qr: 'QR के रूप में दिखाएँ',
      profile_show_qr_hint: 'किसी को स्कैन करने दें ताकि उन्हें आपकी प्रोफ़ाइल मिले',
      profile_qr_callout:
        'इसे स्कैन करने से BSV Wallet खुलता है और आप संपर्क के रूप में जुड़ जाते हैं—नाम, हैंडल और फ़ोटो के साथ। यह भुगतान नहीं माँगता—वह कोड “भुगतान पाएँ” में है।',
      pay_review_title: 'समीक्षा',
      pay_send_amount: '{{amount}} भेजें',
      pay_sent_to: '{{name}} को',
      pay_save_as_contact: 'संपर्क के रूप में सहेजें'
    }
  },
  es: {
    translation: {
      token_balances_header: 'Saldos',
      wallet_coin_switcher: 'Elige qué moneda mostrar',
      wallet_activity_empty_asset: 'Aún no hay actividad en {{ticker}}',
      pay_direction_pay_asset: 'Pagar {{ticker}}',
      token_fee_footer: 'Enviar necesita un poco de BSV para la comisión de red. Recibir no.',
      token_new_tap: 'Nuevo: toca para ver quién lo emite',
      token_unresolved: 'Token no reconocido',
      token_unresolved_sub: 'Esta billetera no pudo averiguar quién lo emite. Inténtalo de nuevo con conexión.',
      token_paused_short: 'Transferencias en pausa',
      token_frozen_short: '{{amount}} congelado',
      token_sheet_issuer: 'Emitido por',
      token_sheet_issuer_unknown: 'Sin identificar',
      token_sheet_asset_id: 'ID del token',
      token_sheet_powers_header: 'Lo que {{issuer}} puede hacer',
      token_power_pause: 'Pausar todas las transferencias de {{ticker}}',
      token_power_freeze: 'Congelar monedas concretas, incluidas las tuyas',
      token_power_admit: 'Decidir quién puede tener {{ticker}}',
      token_power_replace: 'Sustituir una moneda congelada por una nueva',
      token_powers_limit:
        'No pueden sacar {{ticker}} de tu billetera sin ti, y una moneda congelada simplemente no se puede gastar.',
      token_frozen_sub: '{{issuer}} ha congelado {{amount}} {{ticker}} de tu saldo. Contáctalos para reclamar.',
      pay_asset_label: 'Pagar con',
      pay_asset_label_get: 'Cobrar en',
      pay_asset_cta: 'Enviar {{amount}} {{ticker}}',
      pay_asset_over_balance: 'Supera tu saldo de {{ticker}}.',
      pay_asset_needs_bsv: 'Enviar {{ticker}} necesita un poco de BSV para la comisión de red. Recibir no.',
      pay_asset_get_bsv: 'Obtener BSV',
      pay_asset_needs_bsv_short: 'Necesita BSV para la comisión',
      pay_asset_frozen_note: '{{amount}} {{ticker}} de tu saldo está congelado y no se puede enviar.',
      pay_asset_paused:
        '{{issuer}} ha pausado las transferencias de {{ticker}}, así que esto no se puede enviar ahora.',
      pay_asset_no_address:
        '{{ticker}} solo se puede enviar a una persona o a un dispositivo cercano, no a una dirección de Bitcoin.',
      pay_asset_address_status: 'No disponible para {{ticker}}',
      pay_asset_self_unregistered:
        '{{issuer}} no te ha registrado para enviar {{ticker}}. Aun así puedes cobrar en este token.',
      pay_asset_unidentified: 'Esta billetera no pudo identificar este token, así que aún no puede enviarlo.',
      pay_asset_recipient_unregistered:
        '{{issuer}} aún no ha registrado a esta persona para {{ticker}}, así que no se le puede pagar con él.',
      pay_asset_recipient_blocked: '{{issuer}} no permite pagos a esta persona.',
      token_err_refused: '{{issuer}} rechazó esta transferencia. No se envió nada y tu {{ticker}} no cambió.',
      token_err_refused_paused: '{{issuer}} ha pausado {{ticker}}. No se envió nada y tu saldo no cambió.',
      token_err_refused_recipient: '{{issuer}} no permite pagos a esta persona. No se envió nada y tu saldo no cambió.',
      token_err_refused_frozen:
        'Parte del {{ticker}} que ibas a enviar acaba de ser congelado. No se envió nada; tu saldo se ha actualizado.',
      token_err_send_failed: 'No se pudo enviar este pago: {{reason}}. Nada salió de tu billetera.',
      token_err_reason_unknown: 'un problema desconocido',
      token_err_unreachable:
        'No pudimos contactar con {{issuer}} para confirmar esta transferencia. Revisa tu saldo de {{ticker}} antes de reintentar.',
      token_err_check_again: 'Comprobar de nuevo',
      token_err_balance_changed:
        'Tu saldo de {{ticker}} cambió mientras se preparaba esto. Revisa el importe e inténtalo de nuevo.',
      token_err_busy: 'Ya hay un pago de {{ticker}} en curso.',
      pay_sent_not_broadcast: 'Enviado. Todavía no ha llegado a la red; seguiremos intentándolo.',
      pay_sent_not_notified: 'Enviado. Aún no pudimos avisarle; lo verá cuando su billetera vuelva a consultar.',
      pay_sent_handed_to_wallet: 'Enviado. No lo recibió en persona, así que lo enviamos a su billetera.',
      token_first_hold: '{{ticker}} lo emite {{issuer}}, que puede pausar transferencias y congelar monedas.',
      pay_asset_request_paused: '{{issuer}} ha pausado {{ticker}}, así que puede que aún no llegue un pago.',
      token_admit_title: 'Todavía no puedes cobrar en {{ticker}}',
      token_admit_body:
        '{{issuer}} solo envía {{ticker}} a personas que ha registrado. Hasta entonces, los pagos hacia ti se rechazan antes de salir de la billetera del remitente.',
      token_admit_bsv_instead: 'Cobrar en BSV en su lugar',
      token_row_sent: '{{ticker}} enviado',
      token_row_received: '{{ticker}} recibido',
      token_row_amount_pending: 'Importe no disponible',
      token_evicted_title: 'Se retiró parte de tu {{ticker}}',
      token_evicted_body: '{{issuer}} retiró {{amount}} {{ticker}} de tu billetera.',
      local_pay_token_not_cleared: 'Recibido sin conexión · {{issuer}} aún no lo ha confirmado',
      pay_asset_nearby_needs_network: 'Pagar en {{ticker}} en persona necesita conexión. Sin ella puedes pagar en BSV.',
      pay_asset_link_not_held: 'Esta solicitud es de un token que esta billetera no tiene. Aún puedes pagarles en BSV.',
      pay_asset_nearby_wrong_overlay: 'Esta solicitud es de un token para el que esta billetera no está configurada.',
      token_messagebox_mismatch:
        'Tu billetera apunta a otro buzón de mensajes, así que los pagos con tokens podrían no llegarte.',
      token_issuer_fallback: 'el emisor',
      token_row_hint: 'Abre quién lo emite y qué puede hacer',
      token_attention_one: 'Esperando la confirmación de {{issuer}}: toca para ver detalles',
      token_attention_many: '{{count}} pagos con tokens esperan liquidarse: toca para ver detalles',
      token_recv_reversed: '{{issuer}} no pudo confirmar este pago y se ha revertido.',
      token_settle_paused_retry: '{{issuer}} tiene pausado {{ticker}} ahora mismo; seguiremos intentándolo.',
      token_settle_unreachable_retry:
        'No pudimos contactar con {{issuer}} para confirmar esta transferencia. Seguiremos intentándolo.',
      token_settle_evicted:
        'Ese pago no llegó a la red a tiempo y se ha revertido: tu {{ticker}} vuelve a estar en tu saldo.',
      token_frame_unreadable: 'No pudimos leer el pago que recibiste: pide al remitente que lo envíe otra vez.',
      token_chain_reversed: '{{issuer}} rechazó un pago anterior de esta cadena, así que este también se ha revertido.',
      token_settle_stuck_bound:
        'Este pago lleva más de 3 días esperando liquidarse. Puedes seguir esperando o contactar con {{issuer}}.',
      token_cover_failed: 'Este pago no cuadra con los registros de {{issuer}} y no se aceptó.',
      token_sent_settling:
        'Enviado. Liquidando con {{issuer}}: verás la confirmación cuando tú o {{payee}} volváis a conectaros.',
      token_sent_settling_unnamed:
        'Enviado. Liquidando con {{issuer}}: verás la confirmación cuando alguno de los dos se reconecte.',
      token_sent_settled: 'Enviado · liquidado con {{issuer}}',
      token_status_settling: 'Liquidando',
      token_status_settled: 'Liquidado',
      token_status_refused: 'Rechazado',
      token_status_reversed: 'Revertido',
      token_status_stuck: 'Esperando liquidación',
      new_tab: 'Nueva pestaña',
      untitled: 'Sin título',
      share: 'Compartir',
      back_to_homepage: 'Volver a la página de inicio',
      bookmarks: 'Marcadores',
      add_bookmark: 'Agregar marcador',
      bookmark: 'Marcador',
      unbookmark: 'Quitar marcador',
      find_in_page: 'Buscar en la página',
      find_in_page_placeholder: 'Buscar en la página...',
      find_in_page_matches: '{{current}} de {{total}}',
      find_in_page_no_matches: 'Sin resultados',
      history: 'Historial',
      clear_all: 'Borrar todo',
      clear_all_tabs: 'Cerrar todas las pestañas',
      search_or_enter_website: 'Buscar o ingresar sitio web',
      browser: 'Navegador',
      bug_report: 'Reporte de error',
      enable_web3: 'Habilitar Web3',
      tabs: 'Pestañas',
      unlock_web3_features: 'Desbloquear funciones de Web3',

      cancel: 'Cancelar',
      done: 'Listo',
      discard: 'Descartar',
      deny: 'Denegar',
      allow: 'Permitir',
      save: 'Guardar',
      remove: 'Eliminar',
      delete: 'Eliminar',
      search: 'Buscar',
      loading: 'Cargando...',
      continue: 'Continuar',
      go_back: 'Volver',
      continue_action: 'Continuar',
      camera_denied_description:
        'Se denegó el acceso a la cámara. Puedes habilitarlo en Configuración para usar esta función.',
      open_settings: 'Abrir Configuración',
      copied: 'Copiado',
      explorer_open_failed: 'No se pudo abrir el explorador de bloques',
      tx_action_refresh: 'Actualizar estado',
      tx_action_refresh_short: 'Estado',
      tx_action_abort: 'Cancelar transacción',
      copy: 'Copiar',
      copy_to_clipboard: 'Copiar al portapapeles',
      details: 'Detalles',
      hide_details: 'Ocultar detalles',

      identity: 'Identidad',
      settings: 'Configuración',
      wallet: 'Billetera',
      activity: 'Actividad',
      configuration: 'Configuración',
      data_and_security: 'Datos y seguridad',
      storage_row: 'Almacenamiento',
      storage_title: 'Almacenamiento de la billetera',
      storage_db_size: 'Base de datos: {{size}}',
      storage_free_space: 'Espacio libre: {{size}}',
      storage_reclaimable: 'Recuperable ahora: {{size}} en {{count}} transacciones',
      storage_held_back: '{{count}} transacciones retenidas por estar aún en uso',
      storage_reclaim_explain:
        'La recuperación borra datos de validación que la billetera ya no necesita. Tus transacciones, saldos e historial no se tocan. Libera espacio dentro del archivo de base de datos; el archivo no se reduce hasta que el sistema lo compacta.',
      storage_reclaim_confirm: 'Recuperar',
      storage_reclaimed: '{{count}} transacciones recuperadas',
      storage_failed: 'No se pudo leer el uso de almacenamiento.',
      storage_unavailable: 'El almacenamiento aún no está listo.',
      backup_push_section: 'Copia privada',
      backup_push_toggle: 'Respaldar en el servidor',
      backup_erase_row: 'Eliminar la copia del servidor',
      backup_erase_title: '¿Eliminar tu copia de seguridad?',
      backup_erase_message:
        'Todo lo que esta billetera ha enviado al servidor se elimina de forma permanente, en todos los dispositivos. Además se desactivan las copias. Tus monedas, claves y frase de recuperación no se tocan, pero un dispositivo nuevo ya no podrá restaurar el historial de esta billetera y algunas de tus monedas actuales no serán gastables allí.',
      backup_erase_confirm: 'Eliminar',
      backup_erase_done: 'Copia eliminada: {{count}} elementos',
      backup_erase_failed: 'No se pudo eliminar la copia. No se eliminó nada; inténtalo de nuevo.',
      backup_erase_no_key: 'No hay clave de billetera disponible',
      backup_push_disclosure:
        'Se envía una copia cifrada de la base de datos de tu billetera a un servidor de la BSV Association para que puedas recuperarla en un dispositivo nuevo. Está cifrada con una clave que solo tu frase de recuperación puede derivar: el operador no puede leerla. Aun así necesitas tu frase o tus partes impresas: sin ellas nadie puede abrir la copia, ni nosotros.',
      backup_push_off_title: '¿Desactivar la copia privada?',
      backup_push_off_message:
        'Tu frase de recuperación por sí sola no puede reconstruir esta billetera. La información asociada a la protección de tus monedas solo existe en la base de datos, así que sin copias un dispositivo nuevo restaurará tu frase pero no tus fondos. No se elimina nada de lo ya enviado.',
      backup_push_off_confirm: 'Desactivar',
      backup_push_on_toast: 'Copias activadas: sincronizando',
      backup_push_off_toast: 'Copias desactivadas: no se enviará nada más',
      permissions: 'Permisos',
      trust_network: 'Red de Confianza',
      // vault (machine-translated, needs native review)
      vault_title: 'Caja fuerte',
      vault_row_title: 'Caja fuerte',
      vault_hero_title: 'Fondos protegidos',
      vault_hero_body:
        'Almacenamiento en frío a largo plazo, bloqueado con tus propias YubiKeys. Requiere dos o más YubiKey 5 NFC.',
      vault_unsupported_title: 'Necesita una YubiKey',
      vault_unsupported_body: 'Dos o más YubiKey 5 NFC, más un teléfono que pueda leerlas.',
      vault_restore_from_chain: 'Restaurar la caja fuerte desde la blockchain',
      vault_restore_scanning: 'Escaneando la blockchain…',
      vault_restore_found: 'Se encontraron {{count}} depósito(s) de la caja fuerte en la blockchain.',
      vault_restore_pending:
        'Se encontraron {{count}} depósito(s), a la espera de su primera confirmación — inténtalo de nuevo en breve.',
      vault_restore_none_found: 'No se encontraron depósitos de la caja fuerte en la blockchain para esta billetera.',
      vault_balance_label: 'la caja fuerte tiene',
      vault_key_section: 'Llaves de seguridad ({{count}} de 5)',
      vault_manage_section: 'Gestionar',
      vault_deposit_cta: 'Depositar',
      vault_withdraw_cta: 'Retirar',
      vault_deposit_title: 'Depositar en la caja fuerte',
      vault_withdraw_title: 'Retirar de la caja fuerte',
      vault_deposit_sub: 'Desde tu saldo diario. No hace falta YubiKey.',
      vault_withdraw_sub: 'De vuelta a tu saldo diario. Toca cualquiera de tus llaves de la caja fuerte.',
      vault_deposit_done: 'Movido a la caja fuerte',
      vault_reclaim_done: 'Fondos recuperados de un depósito interrumpido a la caja fuerte',
      vault_withdraw_done: 'Retirado de la caja fuerte',
      vault_withdraw_partial: 'Quedan {{count}} depósitos más: retira de nuevo para moverlos.',
      vault_withdraw_reason: 'Acerca tu YubiKey aquí para firmar: retirar {{amount}} sats de la caja fuerte',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: 'Aún no disponible: los depósitos a la caja fuerte están desactivados en esta versión.',
      vault_not_on_mainnet_body:
        'La caja fuerte solo está disponible en la red principal. Vuelve a cambiar la cartera a la red principal para usarla.',
      vault_intro_title: 'Configura tu caja fuerte',
      vault_intro_what:
        'La caja fuerte bloquea el dinero con tus propias YubiKeys. Nada de la clave de la caja fuerte existe jamás en este teléfono; cualquiera de tus llaves la abre.',
      vault_intro_two_keys: 'Necesitas al menos dos YubiKeys.',
      vault_intro_apart: 'Guárdalas en lugares distintos: dos llaves guardadas juntas son una sola llave.',
      vault_intro_backup:
        'Mantén activada la copia de seguridad cifrada de la billetera: conserva el registro de recuperación autenticado.',
      vault_intro_ack:
        'Entiendo: solo mis YubiKeys abren esta caja fuerte. Mi frase de recuperación no. Si las pierdo todas, el dinero se pierde.',
      vault_intro_piv_ack:
        'Confirmo que toda la aplicación PIV de esta YubiKey está restablecida de fábrica o dedicada a la caja fuerte. La configuración cambia el PIN, el PUK y la clave de gestión PIV globales, puede afectar a todas las ranuras PIV, y guardaré el nuevo PUK de forma segura.',
      vault_intro_begin: 'Empezar',
      vault_key_step_title: 'Llave {{k}} de hasta 5',
      vault_key_step_replace:
        'Usa una YubiKey restablecida de fábrica y dedicada a la caja fuerte. Si su ranura contiene una clave antigua, la configuración avisará antes de sustituirla.',
      vault_pin_choose_title: 'Elige un PIN',
      vault_pin_choose_sub: 'Lo introducirás cada vez que uses esta llave. De 6 a 8 dígitos.',
      vault_pin_confirm_label: 'Introdúcelo otra vez',
      vault_pin_mismatch: 'Esos dos PIN no coinciden.',
      vault_pin_not_default: 'Elige algo distinto de 123456.',
      vault_puk_title: 'Tu código de recuperación',
      vault_puk_body:
        'Anótalo y guárdalo en un lugar seguro. Es la única forma de desbloquear esta llave si olvidas el PIN, y no volverá a mostrarse.',
      vault_puk_ack: 'Lo he anotado',
      vault_reset_offer: 'Restablecer esta llave',
      vault_reset_title: '¿Restablecer esta llave?',
      vault_reset_body:
        'Esta llave ya se ha usado antes. Restablecerla borra todos los certificados y claves de su aplicación PIV, no solo la ranura de la caja fuerte. Todo lo que dependa de ella dejará de funcionar.',
      vault_reset_ack: 'Entiendo que esto borra todo lo que hay en esta llave',
      vault_reset_unknown_ack:
        'Este dispositivo no puede confirmar si esta llave todavía protege la caja fuerte de otro perfil. Entiendo que borrarla puede destruir esa clave para siempre.',
      vault_reset_own_draft_ack:
        'Esta llave contiene la clave sin terminar de tu intento anterior, y este dispositivo aún guarda su registro. Entiendo que restablecerla la borra y que ese intento no podrá retomarse.',
      vault_reset_nothing_erased:
        'No se borró nada. La ranura de caja fuerte de esta llave no está vacía, así que queda una cosa más por confirmar.',
      vault_reset_confirm: 'Borrar y restablecer',
      vault_resetting: 'Restableciendo la llave…',
      vault_reset_done: 'Llave restablecida. Vuelve a acercarla para continuar.',
      vault_reset_uncertain:
        'El restablecimiento no terminó. Puede que esta llave ya se haya borrado: compruébala antes de confiar en ella.',
      vault_err_reset_enrolled:
        'Esa llave ya forma parte de tu caja fuerte: restablecerla destruiría una de tus claves.',
      vault_err_reset_unreadable:
        'No se pudieron leer los registros de tu caja fuerte en este dispositivo, así que no hay forma de probar que esta llave no es una de tus claves. No se borró nada y el restablecimiento queda rechazado.',
      vault_enrollment_resume_pin_hint:
        'Retomar una llave guardada requiere el PIN que elegiste antes para ella, no uno nuevo. Tres intentos fallidos la bloquean.',
      vault_setup_step: 'Paso {{n}} de {{total}}',
      vault_enrollment_resume: 'Reanudar clave guardada',
      vault_enrollment_reset_required:
        'Esta YubiKey ya no está en su estado de fábrica, así que no puede inscribirse tal cual. Hay que borrar su aplicación PIV y devolverla al estado de fábrica antes de que pueda guardar una clave de la caja fuerte.',
      vault_enrollment_state_uncertain:
        'Un intento de configuración se interrumpió, así que no hay forma de saber en qué estado quedó la YubiKey. Esa llave no se volverá a usar hasta que su aplicación PIV se borre y vuelva al estado de fábrica.',
      vault_nfc_enroll_message: 'Acerca tu YubiKey aquí para configurarla',
      vault_nfc_adopt_message: 'Acerca esta YubiKey recuperada para verificarla',
      vault_nfc_sign_batch: 'Acerca tu YubiKey aquí para firmar: lote {{b}} de {{n}}',
      vault_key_use_different: 'Usar otra YubiKey',
      vault_key_setup_again: 'Configurarla de nuevo',
      vault_name_default: 'Llave {{k}}',
      vault_more_title: '¿Añadir otra llave?',
      vault_more_body: 'Hasta 5 llaves. Cualquiera de ellas abre toda la caja fuerte por sí sola.',
      vault_more_add: 'Añadir otra llave',
      vault_more_finish: 'Terminar',
      vault_more_need_two:
        'Añade una segunda llave antes de terminar: una sola llave significa que no hay recuperación.',
      vault_leave_setup: 'Salir de la configuración',
      vault_leave_title: '¿Salir de la configuración?',
      vault_leave_body:
        'Las {{count}} YubiKey(s) que configuraste todavía no se guardarán. Conservan sus claves, así que podrás añadirlas de nuevo en un minuto.',
      vault_leave_confirm: 'Salir',
      vault_leave_stay: 'Quedarme',
      vault_done_body:
        '{{count}} llaves pueden abrir esta caja fuerte. Solo estas llaves la abren; tu frase de recuperación no.',
      vault_done_cta: 'Listo',
      vault_add_key_row: 'Añadir llave',
      vault_add_key_done:
        '{{nickname}} puede abrir los depósitos hechos a partir de ahora. Vuelve a bloquear la caja fuerte para que pueda abrirlo todo.',
      vault_key_added_toast: 'Llave añadida',
      vault_recovery_verified: 'Llave recuperada verificada en este dispositivo',
      vault_relock_row: 'Volver a bloquear la caja fuerte',
      vault_relock_now: 'Volver a bloquear ahora',
      vault_relock_choose: '¿Qué llave tocarás para volver a bloquear?',
      vault_relock_reason: 'Toca una de tus llaves existentes ({{names}}), no la que acabas de añadir.',
      vault_relock_reason_generic:
        'Acerca tu YubiKey aquí para firmar: volver a bloquear la caja fuerte con tus llaves actuales',
      vault_relock_reason_remaining: 'Vuelve a bloquear solo con tus claves restantes: {{names}}.',
      vault_key_pending_removal: 'Eliminada: vuelve a bloquear para terminar',
      vault_key_removal_awaiting_network: 'Eliminándose: esperando a que la red acepte el nuevo bloqueo',
      vault_removal_awaiting_network: 'Nuevo bloqueo enviado. Los depósitos y retiros vuelven a estar disponibles cuando la red lo acepte.',
      vault_err_too_many_active_keys: 'La caja fuerte se bloquea con cinco llaves como máximo. Quita una antes de depositar, volver a bloquear o retirar parte del saldo.',
      vault_key_section_over_limit: 'Llaves de seguridad ({{count}}, quita una)',
      vault_add_key_done_over_limit: '{{nickname}} se ha añadido. La caja fuerte se bloquea con cinco llaves como máximo: quita la llave a la que sustituye y vuelve a bloquear una vez.',
      vault_relock_done: 'Caja fuerte bloqueada de nuevo con tus llaves actuales',
      vault_relock_capped: 'Quedan {{count}} depósitos más: vuelve a bloquear otra vez para moverlos.',
      vault_relock_unreachable:
        '{{count}} depósitos solo pueden abrirse con {{names}}. Vuelve a bloquear con una de esas llaves.',
      vault_badge_missing: '{{count}} depósitos aún no abiertos para {{nickname}}',
      vault_badge_removed: '{{count}} depósitos aún abiertos para una llave eliminada',
      vault_key_action_rename: 'Renombrar',
      vault_key_action_remove: 'Eliminar',
      vault_rename_title: 'Renombrar {{nickname}}',
      vault_rename_save: 'Guardar',
      vault_remove_title: '¿Eliminar {{nickname}}?',
      vault_remove_body:
        'La billetera dejará de usar {{nickname}}. El dinero que ya está en la caja fuerte seguirá pudiendo abrirse con ella hasta que vuelvas a bloquear (≈ {{fee}} sats).',
      vault_remove_only: 'Solo eliminar',
      vault_key_removed_toast: 'Llave eliminada',
      vault_export_explainer:
        'Cada depósito en la caja fuerte oculta su salt hasta que lo gastas — nada de eso es visible en el script de bloqueo. Mantén activada la copia de seguridad cifrada para que un dispositivo nuevo pueda restaurar y autenticar los registros de claves necesarios para usar tus YubiKeys.',
      vault_footnote: 'Solo estas llaves abren la caja fuerte. Tu frase de recuperación no.',
      vault_floor_line:
        'Depósito mínimo {{floorDisplay}} ({{floorSats}} sats). Crear un depósito en la caja fuerte cuesta unos {{feeDisplay}}.',
      vault_first_deposit_title: 'Primer depósito en la caja fuerte',
      vault_first_deposit_body:
        'Primer depósito en la caja fuerte: {{amount}} solo podrá abrirse con {{count}} YubiKeys ({{names}}). Tu frase de recuperación no servirá.',
      vault_backup_off_title: 'Activa la copia privada',
      vault_backup_off_body:
        'La copia privada debe estar activada antes de que la billetera cree otra salida de la caja fuerte. Conserva los registros autenticados de las YubiKey y el historial de la billetera necesarios para la recuperación. Aún puedes retirar todo sin crear cambio en la caja fuerte.',
      vault_backup_off_cta: 'Abrir ajustes de copia',
      vault_seed_not_preserved_title: 'Guarda primero tu frase de recuperación',
      vault_seed_not_preserved_body:
        'Tu Bóveda solo puede recuperarse con tu frase de recuperación más una de tus YubiKeys. Las YubiKeys no guardan ninguna copia de ella; si se pierde antes de que la anotes, estos fondos no podrán recuperarse. Guárdala ahora antes de depositar.',
      vault_seed_not_preserved_cta: 'Guardar frase de recuperación',
      vault_choose_key: '¿Qué llave tocarás?',
      vault_remainder_title: '¿Retirar todo?',
      vault_remainder_body:
        'Retirar {{amount}} deja {{remainder}}, por debajo del mínimo de 100,000 sats de la caja fuerte. Toda la caja fuerte pasará a tu saldo diario.',
      vault_remainder_all: 'Retirar todo',
      vault_remainder_change: 'Cambiar importe',
      vault_unreachable_title: 'Parte de la caja fuerte necesita otra llave',
      vault_unreachable_body:
        'Parte de la caja fuerte necesita otra llave: se movieron {{moved}}. {{count}} depósitos con {{amount}} solo pueden abrirse con {{names}}. Retira de nuevo con una de esas llaves.',
      vault_sign_progress: 'Firmados {{signed}} de {{total}}',
      vault_err_pin_locked_enroll:
        'El PIN de esta YubiKey está bloqueado. Desbloquéalo con su PUK en Yubico Authenticator o configura con otra YubiKey.',
      vault_err_serial_mismatch_chosen:
        'Esa es {{tappedName}}. Elegiste {{chosenName}}: tócala, o vuelve atrás y elige {{tappedName}}.',
      vault_err_key_already_enrolled: 'Ya añadiste esta YubiKey ({{nickname}}). Toca otra distinta.',
      vault_err_not_released: 'Los depósitos a la caja fuerte están desactivados en esta versión.',
      vault_err_not_on_mainnet:
        'La caja fuerte solo está disponible en la red principal. Cambia la cartera a la red principal e inténtalo de nuevo.',
      vault_err_backup_off:
        'Activa la copia privada cifrada antes de crear otra salida de la caja fuerte. Aún puedes retirar todo.',
      vault_err_not_enough_keys: 'Esta caja fuerte tiene menos de dos llaves. Configúrala de nuevo.',
      vault_err_too_many_keys: 'Una caja fuerte admite como máximo cinco llaves.',
      vault_err_last_keys: 'Una caja fuerte necesita al menos dos llaves. Añade otra antes de eliminar esta.',
      vault_err_relock_required:
        'Vuelve a bloquear la caja fuerte primero, para que tus otras llaves puedan abrir cada depósito.',
      vault_err_action_pending:
        'Una transacción de la bóveda aún no ha terminado. Inténtalo de nuevo cuando se complete.',
      vault_resolve_held_deposit_action: 'Terminar el depósito interrumpido',
      vault_resolve_held_deposit_done: 'Depósito resuelto',
      vault_resolve_held_deposit_failed: 'No se pudo resolver el depósito. Inténtalo de nuevo.',
      vault_resolve_held_deposit_none:
        'No hay nada que resolver aquí: revisa la pestaña Actividad para reintentar o cancelar una transacción pendiente.',
      vault_err_key_not_committed:
        '{{nickname}} no puede abrir ninguno de los depósitos de esta caja fuerte. Usa otra de tus llaves.',
      vault_err_key_cannot_cover:
        '{{nickname}} puede abrir {{reachable}} de los {{total}} de la caja fuerte. Retira hasta {{reachable}} o usa {{otherNames}}.',
      vault_err_too_small_to_relock:
        'Esta caja fuerte tiene menos de 100,000 sats, demasiado poco para volver a bloquear. Retíralo y deposita de nuevo.',
      vault_err_bad_version:
        'La transacción se construyó en un formato incorrecto. No se firmó nada: inténtalo de nuevo.',
      vault_err_chain_scan_failed:
        'No se pudo terminar de escanear la cadena de bloques en busca de depósitos de la bóveda. Comprueba tu conexión e inténtalo de nuevo.',
      vault_shares_unavailable: 'No hay material de clave disponible para imprimir.',
      vault_enroll_begin: 'Configurar caja fuerte',
      vault_enroll_phase_connecting: 'Leyendo tu llave…',
      vault_enroll_phase_pin_check: 'Comprobando tu llave…',
      vault_enroll_phase_checking_slot: 'Comprobando la ranura de la caja fuerte…',
      vault_enroll_phase_personalizing: 'Configurando tu PIN y código de recuperación…',
      vault_enroll_phase_generating: 'Creando la clave de la caja fuerte: toca la llave cuando parpadee',
      vault_enroll_phase_challenging: 'Verificando la nueva clave de la caja fuerte: tócala cuando parpadee',
      vault_enroll_phase_done: 'Hecho',
      vault_touch_when_blinks: 'Toca el contacto dorado cuando la llave parpadee.',
      vault_nfc_activation_hint:
        '¿Es la primera vez que usas esta YubiKey? Conéctala primero a un puerto USB-C durante unos segundos — las llaves nuevas vienen con el NFC desactivado hasta entonces.',
      vault_continue: 'Continuar',
      vault_back: 'Atrás',
      vault_enrolled_toast: 'Caja fuerte lista',
      vault_insert_key: 'Inserta tu YubiKey',
      vault_hold_key_nfc: 'Acerca tu YubiKey a la parte superior del teléfono',
      vault_keep_holding_nfc: 'Mantenla ahí — desbloqueando…',
      vault_unlocking_funds: 'Desbloqueando tus fondos',
      vault_unlocking_sub: 'Esto tarda unos segundos. Mantén la aplicación abierta.',
      vault_sending_to_network: 'Enviando a la red',
      vault_sending_sub: 'Si no se procesa de inmediato, seguiremos intentándolo.',
      vault_reading_key: 'Leyendo la llave…',
      vault_enter_pin: 'Introduce el PIN de la YubiKey',
      vault_pin_sub_nfc: 'Luego acerca tu llave a la parte superior del teléfono para terminar.',
      vault_pin_sub_usb: 'Luego toca el contacto dorado para terminar.',
      vault_touch_contact: 'Toca el contacto dorado',
      vault_unlock_cta: 'Desbloquear',
      vault_pin_retries: '{{count}} intentos restantes',
      vault_locked: 'Caja fuerte bloqueada',
      vault_retry: 'Reintentar',
      vault_cancel: 'Cancelar',
      vault_dismiss: 'Descartar',
      vault_err_wrong_key: 'Esta YubiKey no es una de las llaves de esta caja fuerte.',
      vault_err_serial_mismatch: 'Esta YubiKey no es una de las llaves de esta caja fuerte ({{names}}).',
      vault_err_touch_timeout: 'No se detectó ningún toque. Inténtalo de nuevo y toca el contacto dorado.',
      vault_err_pin_locked:
        'El PIN de esta YubiKey está bloqueado. Usa otra de tus llaves de la caja fuerte, o desbloquéala con su PUK en Yubico Authenticator.',
      vault_err_pin_invalid: 'PIN incorrecto.',
      vault_err_puk_invalid: 'PUK incorrecto.',
      vault_err_puk_locked: 'El PUK de esta YubiKey está bloqueado. Usa una YubiKey nueva.',
      vault_err_ceremony_active:
        'Otra operación de la caja fuerte sigue usando la YubiKey. Espera a que termine e inténtalo de nuevo.',
      vault_err_scope_changed:
        'La cartera o la red cambió durante esta operación. Empieza de nuevo en la cartera actual.',
      vault_err_key_removed_mid_op: 'Se retiró la llave. Insértala e inténtalo de nuevo.',
      vault_err_no_key: 'No se detectó ninguna YubiKey.',
      vault_err_driver_unavailable: 'La compatibilidad con YubiKey no está disponible en este dispositivo.',
      vault_err_mgmt_key_custom:
        'Esta llave tiene una clave de gestión personalizada y no se puede usar automáticamente.',
      vault_err_attestation_invalid:
        'No se pudo verificar la certificación del fabricante de esta YubiKey. Usa una YubiKey auténtica, certificada de fábrica y con la ranura de certificación PIV intacta.',
      vault_err_generic: 'Algo salió mal. Inténtalo de nuevo.',
      vault_err_template_invalid: 'Algo salió mal al intentar acceder a la caja fuerte. Inténtalo de nuevo.',
      vault_err_not_enrolled: 'No hay ninguna caja fuerte configurada.',
      vault_err_pin_required: 'Introduce tu PIN.',
      vault_err_unsupported_platform: 'La compatibilidad con YubiKey no está disponible en este dispositivo.',
      vault_err_slot_occupied: 'La ranura de la llave ya está en uso.',
      vault_replace_key_warning:
        'Sustituirla destruye de forma permanente la clave privada P-256 existente. Los fondos de la caja fuerte que necesiten esa clave podrían volverse inutilizables.',
      vault_replace_key_confirm: 'Sustituir la clave existente',
      vault_resume_draft_confirm_title: '¿Usar esta clave preparada?',
      vault_resume_draft_confirm_body:
        'Hay una clave preparada previamente — {{nickname}} · …{{tail}} — lista para añadirse, pero este dispositivo no acaba de verificarla en una tarjeta. Continúa solo si la reconoces.',
      vault_resume_draft_confirm_use: 'Usar esta clave',
      vault_resume_draft_confirm_discard: 'Descartarla',
      vault_err_enrollment_partial:
        'La configuración de la YubiKey se detuvo después de modificarla. Sigue las instrucciones para reintentar; la llave no se añadió a la caja fuerte.',
      vault_err_key_not_adopted:
        'Verifica que tienes esta YubiKey recuperada en este dispositivo antes de usarla para retirar.',
      vault_err_too_many_inputs:
        'Demasiados depósitos de la caja fuerte para mover a la vez. Retira primero una cantidad menor: cada retiro también consolida la caja, así que el siguiente podrá mover más.',
      vault_err_requires_online:
        'Las transferencias de la caja fuerte necesitan conexión. Reconecta e inténtalo de nuevo: los fondos de la caja nunca se ponen en cola.',
      vault_err_user_cancelled: 'Cancelado.',
      vault_err_vault_empty: 'La caja fuerte todavía no tiene fondos para retirar.',
      vault_err_amount_exceeds_balance: 'Eso es más de lo que hay en la caja fuerte.',
      vault_err_below_dust: 'Ese importe es demasiado pequeño para enviarlo.',
      vault_err_no_transaction: 'No se pudo completar la transferencia. Inténtalo de nuevo.',
      vault_err_nfc_lost: 'Se perdió la conexión. Mantén la llave firme contra el teléfono.',
      vault_pin_invalid_retry: 'PIN incorrecto — quedan {{count}} intentos. Toca Retirar de nuevo para reintentar.',
      vault_pin_invalid_retry_generic: 'PIN incorrecto. Toca Retirar de nuevo para reintentar.',
      vault_disable_row: 'Desactivar caja fuerte',
      vault_disable_title: '¿Desactivar caja fuerte?',
      vault_disable_message:
        'Esto olvida la lista de llaves de la caja fuerte en este teléfono. Las claves permanecen en tus YubiKeys.',
      vault_disable_confirm: 'Desactivar',
      vault_disabled_toast: 'Caja fuerte desactivada',
      vault_disable_blocked_title: 'Retira los fondos primero',
      vault_disable_blocked_message:
        'La caja fuerte todavía tiene fondos. Retíralos todos a tu saldo diario antes de desactivarla — de lo contrario, esos fondos quedarían bloqueados sin forma de firmarlos.',
      vault_ok: 'Aceptar',
      vault_mock_toggle: 'Usar YubiKey simulada (dev)',
      vault_on: 'Activada',
      vault_off: 'Desactivada',
      vault_shares_word_count:
        'Las partes de recuperación solo admiten frases de doce palabras. Anota tu frase en su lugar.',
      scan_shares_legacy_title: 'Formato de respaldo antiguo',
      scan_shares_legacy_message:
        'Estas partes tienen un formato antiguo que restaura tu saldo diario pero no puede abrir una caja fuerte. Si aún conservas tu frase de recuperación de doce palabras, restaura desde ella —recupera todo— e imprime partes nuevas después. Si perdiste la frase, envía tus fondos a una billetera nueva y empieza de cero.',
      scan_shares_legacy_ack: 'Entendido',

      delete_wallet: 'Eliminar billetera',
      delete_wallet_warning_title: '¿Eliminar billetera?',
      delete_wallet_warning_body:
        'Esto eliminará permanentemente las claves de tu billetera de este dispositivo. Si no has guardado tu frase de recuperación ni impreso las partes de recuperación, perderás el acceso a tu billetera para siempre.',
      delete_wallet_confirm: 'Eliminar',
      delete_wallet_failed_title: 'Error al eliminar la billetera',
      delete_wallet_failed_body: 'No se pudo eliminar completamente tu billetera de este dispositivo. Es posible que parte del material de la clave aún esté presente. Vuelve a intentarlo.',
      bsv_network: 'Red BSV',
      arc_endpoint: 'Endpoint ARC',
      arc_api_token: 'Token API',
      arc_custom_url: 'URL personalizada',
      arc_apply: 'Aplicar',
      arc_reset_default: 'Restablecer predeterminado',
      arc_requires_token: 'requiere token',
      arc_url_https_required: 'Debe ser https (o http://localhost para pruebas locales)',
      recovery_phrase: 'Clave de recuperación',
      you_have: 'Tienes',
      switching: 'Cambiando...',
      mainnet: 'Red principal',
      testnet: 'Red de prueba',
      teratest: 'Teratest',
      identity_key: 'Identificador',
      transactions: 'Transacciones',

      no_transactions: 'No hay transacciones aún',
      tx_status_confirmed: 'Confirmado',
      tx_status_received: 'Recibido',
      tx_status_sent: 'Enviado',
      tx_status_spent: 'Gastado',
      tx_status_transferred: 'Transferido',
      tx_status_accepted: 'Aceptada',
      tx_status_broadcasting: 'Procesando',
      tx_status_not_sent: 'No enviado',
      tx_status_unsigned: 'Borrador',
      tx_status_nonfinal: 'Pendiente',
      tx_status_pending: 'Pendiente',
      tx_status_failed: 'Fallido',
      tx_status_offline_queued: 'Sin conexión · en cola',
      tx_status_offline_sending: 'Sin conexión · enviando',
      tx_status_offline_rejected: 'Sin conexión · rechazado',
      tx_copied: 'Transacción copiada',
      tx_not_available: 'Transacción no disponible',
      tx_copy_failed: 'Error al copiar la transacción',
      tx_abort_success: 'Transacción abortada',
      tx_abort_failed: 'Error al abortar la transacción',
      tx_abort_maybe_delivered: 'No se puede cancelar: es posible que el destinatario ya tenga este pago',
      tx_proof_refreshed: 'Transacción confirmada',
      tx_proof_refresh_failed: 'No se pudo obtener la prueba — puede que la transacción aún no esté confirmada',
      tx_marked_failed: 'No procesada — marcada como fallida y fondos liberados',
      tx_still_pending: 'Sigue pendiente — aún sin confirmar',
      tx_action_explorer: 'Ver en el explorador de bloques',
      tx_export_success: 'Se exportaron {{count}} transacciones',
      tx_export_failed: 'No se pudieron exportar las transacciones',
      tx_export_csv: 'Exportar CSV',
      tx_export: 'Exportar',
      activity_filter: 'Filtrar actividad',
      activity_filter_all: 'Todo',
      activity_search_placeholder: 'Buscar notas, nombres, @alias',
      activity_search_clear: 'Borrar búsqueda',
      activity_filter_no_match: 'No hay actividad que coincida',
      wallet_balance_label: 'Saldo',
      wallet_balance_you_have: 'Tienes',
      wallet_balance_refresh: 'Actualizar saldo',
      wallet_group_today: 'Hoy',
      wallet_group_yesterday: 'Ayer',
      back: 'Atrás',
      wallet_payments: 'Pagos',
      wallet_vault: 'Caja fuerte',
      wallet_settings: 'Configuración',
      wallet_activity: 'Actividad',

      message_box_server: 'Servidor de Buzón de Mensajes',
      message_box_required: 'Necesario para enviar y recibir pagos basados en identidad.',
      send_payment: 'Enviar pago',
      recipient: 'Destinatario',
      amount: 'Monto',
      enter_valid_amount: 'Introduce un importe mayor que cero',
      display_currency: 'Moneda de visualización',
      no_pending_payments: 'No hay pagos pendientes',
      outgoing_payments: 'Salientes',
      payment_not_delivered: 'No entregado',
      payment_delivered: 'Entregado',
      retry: 'Reintentar',
      retry_failed: 'Reintento fallido',
      dismiss: 'Descartar',
      valid_identity_key: 'Identificador válido ingresado',
      valid_bsv_address: 'Dirección BSV válida ingresada',
      searching: 'Buscando...',
      unknown: 'Desconocido',
      from_prefix: 'De:',
      message_box_saved: 'Servidor de buzón guardado',
      message_box_removed: 'Servidor de buzón eliminado',
      message_box_off: 'Desactivado',
      message_box_off_hint: 'El servidor de buzón está desactivado. Actívalo en Ajustes › Avanzado.',
      enter_valid_url: 'Por favor ingresa una URL válida',
      checking_connection: 'Verificando conexión...',
      connection_failed: 'Conexión fallida',
      recipient_placeholder: 'Nombre, identificador o dirección BSV...',

      trust_updated: 'Red de confianza actualizada',
      failed_to_save: 'Error al guardar',
      confirm_delete: 'Confirmar eliminación',
      confirm_delete_body: '¿Estás seguro de que deseas eliminar este certificador?',
      order_certifiers_hint:
        'Ordena los certificadores por prioridad. Las aplicaciones mostrarán primero los certificadores con mayor rango.',
      certifiers: 'Certificadores',
      no_certifiers: 'Aún no hay certificadores.',
      add_provider: 'Agregar proveedor',
      unsaved_changes: 'Tienes cambios sin guardar',
      duplicate_key_error: '¡Una entidad con esta clave pública ya está en la lista!',
      get_provider_details: 'Obtener detalles del proveedor',
      validate_details: 'Validar detalles',
      show_advanced: 'Mostrar avanzado',
      hide_advanced: 'Ocultar avanzado',
      description_length_error: 'La descripción debe tener entre 5 y 50 caracteres',
      enter_domain_hint: 'Ingresa el dominio del proveedor',
      enter_details_hint: 'Ingresa los detalles del proveedor',

      receive: 'Recibir',
      send: 'Enviar',
      generating_address: 'Generando dirección BSV...',
      imported: 'Importado',
      unable_to_generate_address: 'No se puede generar la dirección BSV',
      send_bsv: 'Enviar BSV',
      recipient_address: 'Dirección BSV del destinatario',
      enter_bsv_address: 'Ingresa dirección BSV',

      wallet_data: 'Datos de billetera',
      create_new_wallet: 'Crear nueva billetera',
      generate_recovery_phrase_caption: 'Generar una nueva clave de recuperación',
      import_existing_wallet: 'Importar billetera existente',
      paste_recovery_phrase: 'Pega tu clave de recuperación',
      save_recovery_phrase_heading: 'Guarda tu clave de recuperación',
      save_recovery_phrase_btn: 'Guardar clave de recuperación',
      recovery_phrase_write_down: 'Escribe estas 12 palabras en orden y guárdalas en un lugar seguro.',
      acknowledgment_text:
        'He guardado mi clave de recuperación y entiendo que perderla resultará en la pérdida total y permanente de todos los fondos, tokens y certificados asociados.',
      enter_recovery_words: 'Introduce la frase de 12 palabras',
      restore_wallet_description: 'Ingresa tu clave de recuperación para restaurar tu billetera',
      backup_reminder_title: 'Tu billetera no está respaldada',
      backup_reminder_body: 'Exporta tus claves para evitar perder los fondos.',
      biometric_advisory_title: 'Protege tu billetera',
      wallet_balance_show_in: 'Mostrar en {{unit}}',
      tx_detail_block: 'Bloque',
      tx_detail_block_pending: 'Esperando un bloque',
      biometric_advisory_body: 'Protege tus activos con Face ID o tu huella digital',
      biometric_advisory_body_degraded:
        'Este dispositivo no tiene Face ID ni huella digital configurados, por lo que la clave de tu billetera solo está protegida por el código de acceso del dispositivo.',
      import_prompt_title: '¿Ya tienes una billetera?',
      import_prompt_body: 'Restáurala desde tu clave de recuperación o tus partes.',
      import_prompt_action: 'Importar desde un respaldo',
      nearby_advisory_title: 'Conexión cercana',
      nearby_advisory_body:
        'Se te pedirá permiso para usar Wi-Fi, Bluetooth o NFC. Solo se usa para conectarte directamente con un dispositivo cercano para el pago — nunca se envía a nuestros servidores ni se usa para crear un perfil tuyo.',
      backup_reminder_now: 'Respaldar claves ahora',
      backup_reminder_import: 'Importar desde un respaldo',
      restore_backup_checking: 'Buscando tu copia de seguridad cifrada…',
      restore_backup_progress: 'Restaurando el historial — {{chunks}} de {{total}}…',
      restore_backup_failed_title: 'No se pudo restaurar el historial',
      restore_backup_failed_message:
        'Tu frase de recuperación está bien, pero la copia cifrada no se pudo reproducir. Normalmente basta con reintentar. Si continúas sin ella tendrás una billetera funcional sin transacciones anteriores, y algunas de tus monedas actuales no serán gastables.',
      restore_backup_retry: 'Reintentar',
      restore_backup_skip: 'Abandonar',
      restore_backup_unverified_title: 'La copia de seguridad podría estar incompleta',
      restore_backup_unverified_body:
        'Tu billetera se restauró, pero no se pudo confirmar que la copia de seguridad en el servidor esté completa. Si falta algo de historial, vuelve a restaurar más tarde una vez que el otro dispositivo termine de subirla.',
      import_wallet: 'Importar billetera',

      reject: 'Rechazar',
      authorize: 'Autorizar',
      renewal: 'Renovación',
      spending_section: 'Gasto',
      protocols_section: 'Protocolos',
      baskets_section: 'Cestas',
      certificates_section: 'Certificados',
      requested_fields: 'Campos solicitados',

      send_max: 'Enviar máximo',
      message_box_unreachable:
        'No se pudo conectar con el servidor de pagos. Revisa la URL del message box en los ajustes.',
      available: 'disponible',
      note: 'Nota',
      note_placeholder: '¿Para qué es? (opcional)',
      entire_wallet_balance: 'Todo el saldo de la billetera',
      entire_vault_balance: 'Todo el saldo de la caja fuerte',
      export_wallet_data: 'Exportar datos de cartera',
      export_unencrypted_title: 'Este archivo no está cifrado',
      export_unencrypted_message:
        'El archivo exportado es una copia sin cifrar de los datos de tu cartera, incluida información de identidad y certificados, historial de transacciones y contactos. Cualquiera que obtenga este archivo puede leerlo. Compártelo solo en un lugar de confianza.',
      import_wallet_data: 'Importar datos de cartera',
      import_conflict_title: 'La base de datos ya existe',
      import_conflict_message:
        'Ya existe una base de datos de cartera para esta identidad con una marca de tiempo más reciente. La copia con el número más alto en su nombre de archivo siempre se utiliza.\n\nPara usar esta copia antigua: exporte sus datos actuales, elimine y reinstale la app, luego importe solo la copia deseada.',
      import_confirm_title: 'Importar base de datos',
      import_confirm_message: 'Esta base de datos se convertirá en su cartera activa. ¿Continuar?',
      import_success: 'Base de datos importada con éxito. Reiniciando cartera...',
      import_invalid_file: 'Archivo de respaldo inválido',
      import_invalid_file_detail:
        'Este archivo no es un respaldo válido de base de datos. Se esperaba un nombre como wallet-XXXXXXXX-mainnet-TIMESTAMP.db.',
      import_oversized_file: 'Archivo de respaldo demasiado grande',
      import_oversized_file_detail:
        'Este archivo es más grande de lo que debería ser un respaldo de cartera, por lo que no se leyó. Elige un archivo de respaldo de base de datos de cartera genuino.',
      import_untrusted_file: 'No se puede importar este respaldo',
      import_untrusted_file_detail:
        'No se pudo verificar que este respaldo pertenezca a esta cartera, o no se pudo confiar en su contenido. No se importó.',
      import_anyway: 'Importar igual',
      import_invalid_input_title: 'Entrada no válida',
      import_invalid_input_message:
        'Introduce una frase de recuperación válida (12-24 palabras) o una clave privada hexadecimal de 64 caracteres.',
      import_setup_failed: 'Error al configurar la billetera: {{error}}',
      create_wallet_refused: 'No se pudo crear la billetera. Inténtalo de nuevo.',
      create_wallet_failed: 'No se pudo generar la frase mnemotécnica. Inténtalo de nuevo.',
      or: 'o',
      print_recovery_shares: 'Imprimir Partes de Recuperación',
      scan_backup_shares: 'Escanear Partes de Respaldo',
      scan_backup_shares_caption: 'Recuperar billetera desde partes impresas',
      scan_shares_invalid_format: 'Formato de parte inválido. Escanee un código QR de parte de respaldo válido.',
      scan_shares_camera_needed: 'Se Necesita Acceso a la Cámara',
      scan_shares_camera_description:
        'Se requiere acceso a la cámara para escanear los códigos QR de sus partes de respaldo impresas.',
      scan_shares_grant_camera: 'Conceder Acceso a la Cámara',
      scan_shares_recovering: 'Recuperando billetera...',
      scan_shares_scan_first: 'Escanee su primera parte de respaldo',
      scan_shares_progress: '{{scanned}} de {{needed}} partes escaneadas',
      scan_shares_remaining: '{{count}} parte más necesaria',
      scan_shares_complete: '¡Todas las partes recopiladas!',
      scan_shares_recovery_failed: 'La recuperación falló. Intente escanear de nuevo.',
      scan_shares_threshold_mismatch: 'El umbral no coincide con las partes anteriores',
      scan_shares_integrity_mismatch: 'El hash de integridad no coincide: las partes son de claves diferentes',
      scan_shares_duplicate: 'Esta parte ya ha sido escaneada',
      scan_shares_biometric_title: 'Se requiere acceso biométrico',
      scan_shares_biometric_message:
        'Se necesita acceso biométrico para proteger las claves de tu billetera. Inténtalo de nuevo.',
      recovery_replace_wallet_title: '¿Reemplazar la billetera de este teléfono?',
      recovery_replace_wallet_body:
        'Este teléfono ya tiene una billetera. Si continúas, la reemplazarás junto con todo lo que puede gastar. Si su frase de recuperación no está guardada en otro lugar, ese dinero se perderá para siempre.',
      recovery_replace_wallet_confirm: 'Reemplazar billetera',
      invalid_bsv_address: 'Dirección BSV inválida',
      scan_recipient_hint: 'Apunte la cámara hacia un código de pago',
      scan_qr_default_hint: 'Apunte la cámara hacia un código QR',
      scan_wallet_qr_hint: 'Apunte la cámara hacia un código QR de emparejamiento',

      biometric_store_wallet: 'Almacene su billetera de forma segura',
      biometric_load_wallet: 'Cargue su billetera de forma segura',
      biometric_unlock_wallet: 'Desbloquee su billetera',
      wallet_lost_title: 'Clave de la billetera destruida',
      wallet_lost_body:
        'Este dispositivo cambió su configuración biométrica, por lo que el sistema destruyó la clave que protegía esta billetera. Sus monedas están seguras en la blockchain: restaure esta billetera con su frase de recuperación.',
      wallet_lost_restore: 'Restaurar',
      wallet_lost_later: 'Ahora no',
      wallet_locked_title: 'Billetera bloqueada',
      wallet_locked_body: 'Desbloquee para cargar las claves de su billetera.',
      wallet_unlock_retry: 'Desbloquear billetera',
      wallet_lockout_body: 'Demasiados intentos fallidos. Desbloquee su teléfono con su código y vuelva a intentarlo.',
      wallet_no_biometrics_body:
        'Este dispositivo no tiene biometría configurada. Las claves de su billetera siguen cifradas por el almacén de claves del dispositivo, pero no están protegidas por una comprobación biométrica.',

      // Connections screen
      connections: 'Conexiones',
      advanced: 'Avanzado',
      copy_secret_words: 'Copiar palabras secretas',
      print_recovery_keys: 'Imprimir claves de recuperación',
      backup_wallet_keys: 'Respaldar claves de la cartera',
      log_out: 'Cerrar sesión',
      connect_to_app: 'Conectar a una app',
      no_connections_yet: 'Aún no hay conexiones',
      no_connections_subtitle: 'Escanea un código QR desde una app de escritorio para conectar tu billetera',
      connections_active: 'Activo',
      connections_disconnected: 'Desconectado',
      disconnect: 'Desconectar',
      reconnect: 'Reconectar',
      reconnect_failed: 'Reconexión fallida',
      reconnect_confirm_title: '¿Reconectar a este sitio?',
      reconnect_confirm_message: '{{origin}} podrá acceder a tu billetera de nuevo.',
      reconnect_needs_reapproval: 'No se pudo verificar esta conexión. Escanea un nuevo código QR para reconectar.',
      scan_qr_code: 'Escanear código QR',
      paste_uri: 'Pegar URI',
      invalid_qr_code: 'Código QR inválido',
      wallet_not_ready: 'Billetera no lista',
      please_log_in_first: 'Por favor, inicia sesión primero',
      unknown_error: 'Error desconocido',

      // Pay screen
      payments: 'Pagos',
      pay: 'Pagar',
      pay_direction_pay: 'Pagar',
      pay_direction_receive: 'Cobrar',
      pay_cell_nearby_pay: 'Alguien cerca',
      pay_cell_nearby_pay_sub: 'Escanea su código',
      pay_cell_handle_pay: 'Alguien remoto',
      pay_cell_handle_pay_sub: 'Elige un nombre — necesitan esta aplicación',
      pay_cell_address_pay: 'A una dirección BSV',
      pay_cell_address_pay_sub: 'Pega o escanea una dirección BSV',
      pay_cell_nearby_get: 'Alguien cerca',
      pay_cell_nearby_get_sub: 'Muestra tu código de cobro',
      pay_cell_handle_get: 'Alguien remoto',
      pay_cell_handle_get_sub: 'Comparte tu nombre',
      pay_cell_address_get: 'A una dirección BSV',
      pay_cell_address_get_sub: 'Muestra una dirección BSV',
      pay_method: 'Método',
      pay_method_nearby: 'Cerca',
      pay_method_remote_link: 'Compartir enlace remoto',
      pay_method_address: 'A una dirección BSV',
      pay_pre_nearby: 'Están los dos aquí.',
      pay_pre_handle: 'Usa esta aplicación.',
      pay_pre_address: 'Tienes su dirección BSV.',
      pay_conseq_nearby: 'Se liquida en segundos.',
      pay_conseq_handle: 'Llega cuando su billetera vuelva a consultar.',
      pay_conseq_address: 'El dinero se envía: no se le notifica.',
      paid: 'Pagado',
      pay_inbox_attention: 'No se pudieron añadir',
      pay_inbox_attention_hint:
        'Llegaron, pero tu billetera no pudo añadirlos. Inténtalo de nuevo o descártalos para renunciar a ellos definitivamente.',
      pay_dismiss_confirm: 'Toca otra vez para renunciar',
      pay_dismissed: 'Pago descartado',
      payment_arrived: 'Pago recibido',
      payment_arrived_damaged: 'Este pago llegó dañado',
      discard_payment_title: '¿Descartar este pago?',
      discard_payment_body:
        'Se le pedirá al remitente que envíe los detalles de nuevo. El dinero permanece en la cadena hasta que lo haga.',
      discard_void_title: 'Este pago no se completó',
      discard_void_body: 'Fue cancelado o gastado en otro lugar. Puedes descartarlo sin riesgo.',
      waiting_network_confirm: 'Esperando a que la red confirme este pago',
      finish_or_cancel_outgoing: 'Termina o cancela el pago de abajo antes de enviar uno nuevo',
      identity_search_unavailable: 'La búsqueda no está disponible ahora mismo. Inténtalo en un momento.',
      abandon_payment: 'Abandonar el pago',
      finish_payment: 'Terminar el pago',
      cancel_this_payment: '¿Cancelar este pago?',
      cancel_payment: 'Cancelar el pago',
      resend: 'Reenviar',
      resending: 'Reenviando…',
      send_payment_details_again: 'Enviar los detalles de nuevo',
      tx_action_resend_short: 'Reenviar',
      tx_action_request_confirmation: 'Pedir confirmación al destinatario',
      resend_sent: 'Detalles del pago reenviados',
      resend_failed_no_record:
        'No se encontraron los detalles de este pago en este dispositivo, así que no se puede reenviar.',
      resend_failed_no_recipient: 'Este pago no indica para quién era, así que no se puede reenviar.',
      resend_failed_no_transaction: 'No se pudieron obtener los datos de este pago. Inténtalo cuando tengas conexión.',
      resend_requested: 'Te pidieron reenviar un pago',
      request_again: 'Solicitar de nuevo',
      send_again: 'Enviar de nuevo',
      copy_details: 'Copiar detalles',
      payment_bounced_resend: 'Se les pidió enviar este pago de nuevo',
      dismiss_rejected_payment: 'Se descartó este pago rechazado',
      activity_load_failed: 'No se pudo cargar la actividad',
      activity_load_retry: 'Toca para reintentar',
      activity_view_contact: 'Ver contacto',
      home_payments_need_attention: '{{count}} pagos necesitan atención',
      home_payments_unsent: '{{count}} pagos sin entregar',
      pay_copy: 'Copiar',
      pay_share_link: 'Compartir enlace',
      pay_address_watching: 'El dinero enviado aquí se añade a tu billetera automáticamente.',
      pay_address_earlier_day: 'Ir a un día anterior',
      pay_address_sweep_now: 'Comprobar esta dirección BSV ahora',
      pay_address_swept: 'Recibido {{amount}}',

      check_wallet: 'Solución de problemas',
      settings_version: 'v{{version}}',
      error_review_actions: 'Este pago está atascado. Solución de problemas puede intentar liberarlo.',
      wallet_check_ok: 'Todo se ve bien',
      wallet_check_attention: 'Algunas comprobaciones necesitan tu atención',
      wallet_check_couldnt: 'No se pudo revisar',
      wallet_check_couldnt_body: 'Un paso de reparación no terminó. No se dio nada por supuesto sobre tus monedas.',
      wallet_check_summary: '{{freed}} monedas atascadas liberadas · {{recovered}} pagos recuperados',
      wallet_check_step_coins: 'Tus monedas son válidas',
      wallet_check_skip: 'Omitir esta comprobación',
      wallet_check_skipped: 'Omitida',
      wallet_check_step_proofs: 'Pruebas de transacción',
      wallet_check_step_records: 'Registros de la base de datos local',
      wallet_check_step_tokens: 'Transferencias de stablecoin verificadas',
      wallet_check_step_online: 'Conexión',
      wallet_check_step_missed: 'Pagos no recibidos',
      wallet_check_step_backup: 'Copia privada subida',
      wallet_check_step_phrase: 'Frase de recuperación guardada',
      wallet_check_progress: '{{current}} de {{total}}',
      wallet_check_stuck_title: 'No se pudo enviar este pago',
      wallet_check_stuck_message: 'Algunas monedas parecen atascadas. Solución de problemas puede liberarlas.',
      wallet_check_failed: 'No se pudo terminar la revisión. Inténtalo de nuevo.',

      // Local payments
      local_pay_request: 'Solicitar pago',
      local_pay_send: 'Enviar pago',
      local_pay_amount: 'Monto',
      local_pay_show_qr: 'Muestra este código a quien paga',
      local_pay_scan_qr: 'Escanea el código de quien cobra',
      local_pay_scan_payer_qr: 'Escanea el código de quien paga',
      local_pay_scan_progress: '{{have}} de {{total}} fragmentos',
      local_pay_show_payment_qr: 'Muestra este código a quien cobra',
      local_pay_animated_hint:
        'Pídele que escanee el código animado de abajo: seguirá cambiando hasta que su teléfono lo reciba entero.',
      local_pay_show_qr_instead: 'Mostrar un código QR en su lugar',
      local_pay_preparing: 'Preparando la solicitud…',
      local_pay_delivering: 'Entregando el pago…',
      local_pay_saving: 'Guardando el pago…',
      local_pay_sent: 'Pago enviado',
      local_pay_received: 'Pago recibido',
      local_pay_added: 'Añadido a tu billetera.',
      local_pay_added_multiple: '{{count}} pagos añadidos a tu billetera.',
      local_pay_received_confirming: 'Confirmando con tu billetera…',
      local_pay_received_not_credited: 'Aún no se ha añadido a tu billetera. Revisa Actividad antes de entregar nada.',
      local_pay_decline_broadcast_title: 'Un pago rechazado se transmitió',
      local_pay_decline_broadcast_body:
        'Un pago que fue rechazado ha aparecido en la red. No lo envíes de nuevo — revisa Actividad.',
      local_pay_pending_abort_dropped_title: 'Se ignoró una solicitud de liberación no verificada',
      local_pay_pending_abort_dropped_body:
        'No se pudo verificar un registro guardado de liberación de pago, así que no se liberó nada. Si un pago sigue pareciendo bloqueado, cancélalo desde Actividad.',
      local_pay_cancel_unverifiable_title: 'No se puede verificar este pago sin conexión',
      local_pay_cancel_unverifiable_body:
        'Si la otra persona ya lo escaneó, cancelar permite que estas monedas se gasten dos veces. Cancela solo si estás seguro de que nunca se escaneó.',
      local_pay_cancel_unverifiable_confirm: 'Cancelar de todos modos',
      local_pay_queued: 'Guardado: se añadirá a tu billetera automáticamente.',
      local_pay_already_paid: 'Esta solicitud ya fue pagada.',
      local_pay_failed: 'El pago falló',
      local_pay_network_denied:
        'El acceso a la red local está desactivado. Actívalo en Configuración para pagar a dispositivos cercanos.',
      local_pay_radio_fallback: 'Enlace inalámbrico no disponible: muestra este código al otro dispositivo.',
      local_pay_floor_peer_bt_off: 'Su Bluetooth está apagado, así que este pago se hará por código.',
      local_pay_floor_local_bt_off: 'Tu Bluetooth está apagado. Actívalo para un enlace cercano más rápido.',
      local_pay_floor_local_ble_denied:
        'El acceso a Bluetooth está desactivado para BSV Wallet. Permítelo en Configuración para un enlace cercano más rápido.',
      local_pay_floor_cross_os:
        'El otro dispositivo no puede usar Bluetooth ahora mismo, así que este pago se hará por código.',
      local_pay_floor_peer_no_radio:
        'El otro dispositivo no está a la escucha de un enlace inalámbrico, así que este pago se hará por código.',
      local_pay_ble_unavailable: 'Bluetooth no está disponible. El código de arriba sigue funcionando.',
      local_pay_too_large: 'Los datos de este pago son demasiado grandes para entregarlos con códigos QR.',
      local_pay_nearby_unavailable: 'El Wi-Fi cercano no está disponible. El código de arriba sigue funcionando.',
      local_pay_retry_save: 'Intentar guardar de nuevo',
      local_pay_wrong_session:
        'Ese pago no coincide con esta solicitud y no se aceptó. Pide a quien paga que vuelva a escanear el código.',
      local_pay_declined_mismatch:
        'Quien recibe no aceptó este pago: no coincide con su solicitud. Vuelve a escanear su código.',
      local_pay_declined_already_paid: 'Esa solicitud ya se pagó. No se envió nada.',
      local_pay_declined_save: 'Quien recibe no pudo guardar el pago. No se envió nada; inténtalo de nuevo.',
      local_pay_declined_decode: 'Quien recibe no pudo leer el pago. No se envió nada; inténtalo de nuevo.',
      local_pay_declined_not_covered:
        'Quien recibe no pudo verificar este pago con los registros del emisor. No se envió nada.',
      local_pay_broadcast_pending: 'Enviado. Este dispositivo aún no pudo procesarlo; quien recibe lo hará.',
      local_pay_any_amount: 'Cualquier importe',
      local_pay_choose_amount: 'Elige un importe',
      local_pay_enter_amount_send: '¿Cuánto quieres enviar?',
      local_pay_qr_unavailable: 'No se pudo crear este código. Vuelve a iniciar la solicitud.',
      local_pay_presence_qr: 'Por código: sin conexión en directo',
      local_pay_presence_ready: 'Enlace cifrado cercano listo',
      local_pay_presence_waiting_payee: 'Esperando un dispositivo cercano',
      local_pay_presence_waiting_payer: 'Buscando el dispositivo del destinatario…',
      local_pay_presence_linked: 'Enlace cifrado abierto',
      local_pay_presence_paid: 'Pago confirmado',
      pay_offline_title: 'Estás sin conexión',
      pay_offline_body:
        'Solo los pagos cercanos funcionan ahora mismo. Se procesarán en cuanto vuelvas a tener conexión.',
      pay_offline_queued: '{{count}} pago(s) esperando para procesarse en cuanto vuelvas a tener conexión.',
      pay_offline_pending_title: 'Esperando procesamiento',
      pay_offline_pending_body:
        '{{count}} pago(s) aún no se han procesado. Nada queda liquidado hasta que ocurra; se seguirá intentando.',
      pay_offline_needs_internet: 'Necesita conexión',
      pay_offline_rejected_title: 'Se rechazó un pago',
      pay_offline_rejected_body: 'La red rechazó un pago entregado por {{sender}} a través de {{via}} el {{when}}.',
      pay_offline_unknown_sender: 'un remitente desconocido',
      pay_offline_unknown_via: 'un método desconocido',
      pay_received_not_broadcast: 'Recibido sin conexión · aún no procesado',
      pay_offline_sent_rejected_title: 'No se pudo entregar un pago',
      pay_offline_sent_rejected_body: 'La red rechazó un pago que enviaste el {{when}}.',
      pay_offline_send_now: 'Enviar ahora',
      pay_offline_stalled_body: 'Algunos pagos en cola no se pueden enviar automáticamente: {{detail}}',
      offline_stall_no_request: 'A un pago en cola le falta su registro de envío.',
      offline_stall_bad_beef: 'No se pudieron leer los datos de la transacción de un pago en cola.',
      offline_stall_foreign_ancestor:
        'Un pago en cola está esperando una transacción previa que otra billetera nunca procesó.',
      pay_offline_kv_pending: '{{count}} pago(s) cercano(s) esperando a añadirse a esta billetera.',
      pay_offline_kv_stuck:
        'No se pudieron añadir {{count}} pago(s) cercano(s). Pide a quien pagó que lo envíe otra vez.',
      pay_offline_kv_corrupt: 'Se encontraron datos de pago dañados en este dispositivo.',
      pay_offline_kv_corrupt_body:
        'Los pagos afectados se apartaron en este dispositivo, no se borraron. Puede que algunos ya se hayan sumado a tu saldo, así que pide a alguien que vuelva a enviar solo si su pago sigue sin aparecer.',
      pay_offline_show_code: 'Mostrar el código otra vez',
      pay_parked_cancel: 'Cancelar el pago',
      pay_parked_already_sent: 'Este pago ya se envió',
      token_cancel_already_sent: 'Este pago ya se completó y no se puede cancelar.',
      tx_status_parked: 'No entregado',
      tx_status_import_hold: 'Retenido (importado)',
      resend_sending: 'Enviando por el buzón de mensajes',
      cancelling_payment: 'Cancelando el pago',

      contacts: 'Contactos',
      contacts_search_placeholder: 'Buscar por nombre o alias',
      contacts_empty_title: 'Aún no hay contactos',
      contacts_empty_body:
        'Las personas a las que pagas aparecerán aquí una vez que las guardes. Escanea el código de perfil de alguien con el botón de arriba a la derecha.',
      contacts_recent: 'Recientes',
      contact_edit_name: 'Editar nombre',
      contact_save_name: 'Guardar nombre',
      contact_activity: 'Actividad',
      contact_activity_empty: 'Aún no hay actividad con este contacto',
      contact_delete: 'Eliminar contacto',
      contact_delete_body: 'Esto elimina a {{name}} de tus contactos. No afecta a ningún pago anterior.',
      contact_handle_caption: 'Su alias registrado · solo ellos pueden cambiarlo',
      contact_new_title: 'Nuevo contacto',
      contact_save: 'Guardar',
      contact_identifier: 'Identificador',
      contact_saved: 'Contacto guardado',
      contact_scan_hint: 'Escanea un código de perfil para añadir a alguien',
      contact_identifier_placeholder: 'Pega su identificador',
      contact_identifier_invalid: 'Eso no parece un identificador válido',
      contact_add_from_payment: 'Añadir a contactos',

      profile: 'Perfil',
      profile_display_name: 'Nombre visible',
      profile_display_name_hint: 'Se muestra públicamente junto a tu alias. Cualquiera que te busque puede verlo.',
      profile_handle: 'Alias',
      profile_handle_placeholder: 'tunombre',
      profile_handle_checking: 'Comprobando disponibilidad…',
      profile_handle_available: '{{handle}} está disponible',
      profile_handle_taken: '{{handle}} ya está en uso',
      profile_handle_invalid:
        '3-32 caracteres: a-z, 0-9, punto, guion bajo o guion; empieza y acaba con letra o dígito',
      profile_handle_failed: 'No se pudo comprobar la disponibilidad. Inténtalo de nuevo.',
      profile_handle_unavailable: 'El registro de alias aún no está disponible en esta versión.',
      profile_handle_registered: 'Alias registrado',
      profile_handle_register_action: 'Registrar',
      profile_handle_too_similar: '{{handle}} se parece demasiado a uno ya en uso',
      profile_handle_reserved: '{{handle}} está reservado',
      profile_handle_cooldown: '{{handle}} se liberó hace poco y aún no está libre',
      profile_handle_pending: 'Terminando el registro…',
      profile_handle_rolled_back: '{{handle}} ya estaba ocupado. Conservas {{previous}}.',
      profile_handle_changed: 'Alias cambiado',
      profile_handle_rejected: 'El registro rechazó ese alias.',
      profile_handle_clock_ahead:
        'El reloj de tu dispositivo va demasiado adelantado. Inténtalo de nuevo en unos minutos.',
      profile_handle_wrong_registry: 'Ese alias no está en este registro. Vuelve a abrir Perfil para actualizarlo.',
      profile_display_name_publish_failed: 'Tu nombre se guardó aquí, pero el registro no lo publicó.',
      profile_qr_hint: 'Otras personas pueden escanear esto para añadirte como contacto',

      pay_step_contacts: 'Contactos',
      pay_step_continue: 'Continuar',
      pay_review_to: 'Para',
      pay_review_amount: 'Monto',
      pay_review_note: 'Nota',
      pay_review_note_edit: 'Añadir una nota',
      pay_review_delivery_host: 'Entregando a',
      pay_trust_contact: 'Tu contacto',
      pay_trust_handle_attested: 'Registrado',
      pay_trust_handle_domain_attested: 'Verificado por {{domain}}',
      pay_trust_unverified: 'No verificado. Verifica bien este identificador.',
      pay_address_network_mismatch: 'Esta dirección pertenece a una red distinta de la que estás usando.',
      contacts_your_contacts: 'Tus contactos',
      contacts_other_people: 'Otras personas',
      contacts_not_saved: 'Sin guardar',
      contacts_network_footer: 'Encontrado en la red. No lo has guardado.',
      contact_no_handle: 'Aún sin alias',
      contact_no_handle_registered: 'Sin alias registrado',
      contact_name_section: 'Tu nombre para esta persona',
      contact_name_hint: 'Solo tú lo ves. No cambia cómo se llama a sí misma.',
      contact_new_name_hint: 'Solo tú lo ves. Guardado en este dispositivo.',
      contact_copy_identifier: 'Copiar identificador',
      profile_handle_registered_hint: 'Registrado a tu identificador. La gente puede encontrarte con él.',
      profile_handle_change: 'Cambiar',
      profile_handle_claim: 'Reclamar {{handle}}',
      profile_handle_replace_warning:
        'Reclamar sustituye a {{handle}}. Quienes te guardaron conservan el contacto; el alias antiguo deja de apuntar a ti.',
      profile_identifier_section: 'Tu identificador',
      profile_show_qr: 'Mostrar como QR',
      profile_show_qr_hint: 'Deja que alguien te escanee para obtener tu perfil',
      profile_qr_callout:
        'Al escanearlo se abre BSV Wallet y te añade como contacto, con tu nombre, alias y foto. No pide ningún pago: ese código está en Cobrar.',
      pay_review_title: 'Revisar',
      pay_send_amount: 'Enviar {{amount}}',
      pay_sent_to: 'a {{name}}',
      pay_save_as_contact: 'Guardar como contacto'
    }
  },
  fr: {
    translation: {
      token_balances_header: 'Soldes',
      wallet_coin_switcher: 'Choisir la monnaie à afficher',
      wallet_activity_empty_asset: 'Aucune activité {{ticker}} pour l’instant',
      pay_direction_pay_asset: 'Payer en {{ticker}}',
      token_fee_footer: 'Envoyer nécessite un peu de BSV pour les frais de réseau. Recevoir, non.',
      token_new_tap: "Nouveau — touchez pour voir qui l'émet",
      token_unresolved: 'Jeton non reconnu',
      token_unresolved_sub: "Ce portefeuille n'a pas pu trouver l'émetteur. Réessayez une fois en ligne.",
      token_paused_short: 'Transferts suspendus',
      token_frozen_short: '{{amount}} gelé',
      token_sheet_issuer: 'Émis par',
      token_sheet_issuer_unknown: 'Non identifié',
      token_sheet_asset_id: 'ID du jeton',
      token_sheet_powers_header: 'Ce que {{issuer}} peut faire',
      token_power_pause: 'Suspendre tous les transferts de {{ticker}}',
      token_power_freeze: 'Geler des pièces précises, y compris les vôtres',
      token_power_admit: 'Décider qui peut détenir des {{ticker}}',
      token_power_replace: 'Remplacer une pièce gelée par une nouvelle',
      token_powers_limit:
        'Ils ne peuvent pas sortir des {{ticker}} de votre portefeuille sans vous, et une pièce gelée ne peut tout simplement pas être dépensée.',
      token_frozen_sub: '{{issuer}} a gelé {{amount}} {{ticker}} de votre solde. Contactez-les pour contester.',
      pay_asset_label: 'Payer avec',
      pay_asset_label_get: 'Être payé en',
      pay_asset_cta: 'Envoyer {{amount}} {{ticker}}',
      pay_asset_over_balance: 'Supérieur à votre solde de {{ticker}}.',
      pay_asset_needs_bsv: 'Envoyer des {{ticker}} nécessite un peu de BSV pour les frais de réseau. Recevoir, non.',
      pay_asset_get_bsv: 'Obtenir des BSV',
      pay_asset_needs_bsv_short: 'Nécessite des BSV pour les frais',
      pay_asset_frozen_note: '{{amount}} {{ticker}} de votre solde est gelé et ne peut pas être envoyé.',
      pay_asset_paused:
        '{{issuer}} a suspendu les transferts de {{ticker}} ; cet envoi est donc impossible pour le moment.',
      pay_asset_no_address:
        "Les {{ticker}} ne peuvent être envoyés qu'à une personne ou à un appareil à proximité, pas à une adresse Bitcoin.",
      pay_asset_address_status: 'Indisponible pour les {{ticker}}',
      pay_asset_self_unregistered:
        '{{issuer}} ne vous a pas enregistré pour envoyer des {{ticker}}. Vous pouvez toujours en recevoir.',
      pay_asset_unidentified: "Ce portefeuille n'a pas pu identifier ce jeton ; il ne peut donc pas encore l'envoyer.",
      pay_asset_recipient_unregistered:
        "{{issuer}} n'a pas encore enregistré cette personne pour les {{ticker}} ; elle ne peut donc pas en recevoir.",
      pay_asset_recipient_blocked: "{{issuer}} n'autorise pas les paiements à cette personne.",
      token_err_refused: "{{issuer}} a refusé ce transfert. Rien n'a été envoyé et vos {{ticker}} sont inchangés.",
      token_err_refused_paused:
        "{{issuer}} a suspendu les {{ticker}}. Rien n'a été envoyé et votre solde est inchangé.",
      token_err_refused_recipient:
        "{{issuer}} n'autorise pas les paiements à cette personne. Rien n'a été envoyé et votre solde est inchangé.",
      token_err_refused_frozen:
        "Une partie des {{ticker}} que vous envoyiez vient d'être gelée. Rien n'a été envoyé ; votre solde a été mis à jour.",
      token_err_send_failed: "Impossible d'envoyer ce paiement : {{reason}}. Rien n'a quitté votre portefeuille.",
      token_err_reason_unknown: 'un problème inconnu',
      token_err_unreachable:
        'Impossible de joindre {{issuer}} pour confirmer ce transfert. Vérifiez votre solde de {{ticker}} avant de réessayer.',
      token_err_check_again: 'Vérifier à nouveau',
      token_err_balance_changed:
        'Votre solde de {{ticker}} a changé pendant la préparation. Vérifiez le montant et réessayez.',
      token_err_busy: "Un paiement en {{ticker}} est déjà en cours d'envoi.",
      pay_sent_not_broadcast: "Envoyé. Ce n'est pas encore arrivé sur le réseau — nous continuons d'essayer.",
      pay_sent_not_notified:
        "Envoyé. Nous n'avons pas encore pu le lui signaler — il le verra au prochain contrôle de son portefeuille.",
      pay_sent_handed_to_wallet:
        "Envoyé. La personne ne l'a pas pris en main propre, nous l'avons donc envoyé à son portefeuille.",
      token_first_hold:
        'Les {{ticker}} sont émis par {{issuer}}, qui peut suspendre les transferts et geler des pièces.',
      pay_asset_request_paused: '{{issuer}} a suspendu les {{ticker}} ; un paiement pourrait ne pas encore arriver.',
      token_admit_title: 'Vous ne pouvez pas encore être payé en {{ticker}}',
      token_admit_body:
        "{{issuer}} n'envoie des {{ticker}} qu'aux personnes qu'il a enregistrées. D'ici là, les paiements vers vous sont refusés avant même de quitter le portefeuille de l'expéditeur.",
      token_admit_bsv_instead: 'Être payé en BSV à la place',
      token_row_sent: '{{ticker}} envoyé',
      token_row_received: '{{ticker}} reçu',
      token_row_amount_pending: 'Montant indisponible',
      token_evicted_title: 'Une partie de vos {{ticker}} a été retirée',
      token_evicted_body: '{{issuer}} a retiré {{amount}} {{ticker}} de votre portefeuille.',
      local_pay_token_not_cleared: 'Reçu hors ligne · pas encore confirmé par {{issuer}}',
      pay_asset_nearby_needs_network:
        'Payer en {{ticker}} en personne nécessite une connexion. Sans connexion, vous pouvez payer en BSV.',
      pay_asset_link_not_held:
        'Cette demande concerne un jeton que ce portefeuille ne détient pas. Vous pouvez toujours payer en BSV.',
      pay_asset_nearby_wrong_overlay:
        "Cette demande concerne un jeton pour lequel ce portefeuille n'est pas configuré.",
      token_messagebox_mismatch:
        'Votre portefeuille utilise une autre boîte de messages ; les paiements en jetons pourraient ne pas vous parvenir.',
      token_issuer_fallback: "l'émetteur",
      token_row_hint: "Ouvre l'émetteur et ses pouvoirs",
      token_attention_one: 'En attente de confirmation de {{issuer}} — touchez pour les détails',
      token_attention_many: '{{count}} paiements en jetons attendent leur règlement — touchez pour les détails',
      token_recv_reversed: "Ce paiement n'a pas pu être confirmé par {{issuer}} et a été annulé.",
      token_settle_paused_retry: "{{issuer}} a suspendu les {{ticker}} pour l'instant — nous continuons d'essayer.",
      token_settle_unreachable_retry:
        "Impossible de joindre {{issuer}} pour confirmer ce transfert. Nous continuons d'essayer.",
      token_settle_evicted:
        "Ce paiement n'a pas atteint le réseau à temps et a été annulé — vos {{ticker}} sont de retour dans votre solde.",
      token_frame_unreadable: "Nous n'avons pas pu lire le paiement reçu — demandez à l'expéditeur de le renvoyer.",
      token_chain_reversed:
        'Un paiement antérieur de cette chaîne a été refusé par {{issuer}} ; celui-ci a donc aussi été annulé.',
      token_settle_stuck_bound:
        'Ce paiement attend son règlement depuis plus de 3 jours. Vous pouvez patienter ou contacter {{issuer}}.',
      token_cover_failed: "Ce paiement ne correspond pas aux registres de {{issuer}} et n'a pas été accepté.",
      token_sent_settling:
        'Envoyé. Règlement avec {{issuer}} en cours — la confirmation apparaîtra dès que vous ou {{payee}} serez reconnecté.',
      token_sent_settling_unnamed:
        "Envoyé. Règlement avec {{issuer}} en cours — la confirmation apparaîtra dès que l'un de vous sera reconnecté.",
      token_sent_settled: 'Envoyé · réglé avec {{issuer}}',
      token_status_settling: 'Règlement en cours',
      token_status_settled: 'Réglé',
      token_status_refused: 'Refusé',
      token_status_reversed: 'Annulé',
      token_status_stuck: 'En attente de règlement',
      new_tab: 'Nouvel onglet',
      untitled: 'Sans titre',
      share: 'Partager',
      back_to_homepage: "Retour à l'accueil",
      bookmarks: 'Favoris',
      add_bookmark: 'Ajouter aux favoris',
      bookmark: 'Favori',
      unbookmark: 'Retirer le favori',
      find_in_page: 'Rechercher dans la page',
      find_in_page_placeholder: 'Rechercher dans la page...',
      find_in_page_matches: '{{current}} sur {{total}}',
      find_in_page_no_matches: 'Aucun résultat',
      history: 'Historique',
      clear_all: 'Tout effacer',
      clear_all_tabs: 'Fermer tous les onglets',
      search_or_enter_website: 'Rechercher ou entrer un site web',
      browser: 'Navigateur',
      bug_report: 'Rapport de bogue',
      enable_web3: 'Activer Web3',
      tabs: 'Onglets',
      unlock_web3_features: 'Débloquer les fonctionnalités Web3',

      cancel: 'Annuler',
      done: 'Terminé',
      discard: 'Abandonner',
      deny: 'Refuser',
      allow: 'Autoriser',
      save: 'Enregistrer',
      remove: 'Supprimer',
      delete: 'Supprimer',
      search: 'Rechercher',
      loading: 'Chargement...',
      continue: 'Continuer',
      go_back: 'Retour',
      continue_action: 'Continuer',
      camera_denied_description:
        "L'accès à la caméra a été refusé. Vous pouvez l'activer dans les Réglages pour utiliser cette fonctionnalité.",
      open_settings: 'Ouvrir les Réglages',
      copied: 'Copié',
      explorer_open_failed: "Impossible d'ouvrir l'explorateur de blocs",
      tx_action_refresh: 'Actualiser le statut',
      tx_action_refresh_short: 'Statut',
      tx_action_abort: 'Annuler la transaction',
      copy: 'Copier',
      copy_to_clipboard: 'Copier dans le presse-papiers',
      details: 'Détails',
      hide_details: 'Masquer les détails',

      identity: 'Identité',
      settings: 'Paramètres',
      wallet: 'Portefeuille',
      activity: 'Activité',
      configuration: 'Configuration',
      data_and_security: 'Données et sécurité',
      storage_row: 'Stockage',
      storage_title: 'Stockage du portefeuille',
      storage_db_size: 'Base de données : {{size}}',
      storage_free_space: 'Espace libre : {{size}}',
      storage_reclaimable: 'Récupérable maintenant : {{size}} sur {{count}} transactions',
      storage_held_back: '{{count}} transactions conservées car encore utilisées',
      storage_reclaim_explain:
        "La récupération efface des données de validation dont le portefeuille n'a plus besoin. Vos transactions, soldes et historique restent intacts. Cela libère de l'espace dans le fichier de base de données ; le fichier lui-même ne rétrécit qu'après compactage par le système.",
      storage_reclaim_confirm: 'Récupérer',
      storage_reclaimed: '{{count}} transactions récupérées',
      storage_failed: "Impossible de lire l'utilisation du stockage.",
      storage_unavailable: "Le stockage n'est pas encore prêt.",
      backup_push_section: 'Sauvegarde privée',
      backup_push_toggle: 'Sauvegarder sur le serveur',
      backup_erase_row: 'Supprimer la sauvegarde du serveur',
      backup_erase_title: 'Supprimer votre sauvegarde ?',
      backup_erase_message:
        "Tout ce que ce portefeuille a envoyé au serveur est définitivement supprimé, pour tous les appareils. La sauvegarde est également désactivée. Vos pièces, clés et phrase de récupération sont intactes, mais un nouvel appareil ne pourra plus restaurer l'historique de ce portefeuille et certaines de vos pièces actuelles n'y seront pas dépensables.",
      backup_erase_confirm: 'Supprimer',
      backup_erase_done: 'Sauvegarde supprimée — {{count}} éléments',
      backup_erase_failed: "Impossible de supprimer la sauvegarde. Rien n'a été supprimé, veuillez réessayer.",
      backup_erase_no_key: 'Aucune clé de portefeuille disponible',
      backup_push_disclosure:
        "Une copie chiffrée de la base de données de votre portefeuille est envoyée à un serveur de la BSV Association afin que vous puissiez la restaurer sur un nouvel appareil. Elle est chiffrée avec une clé que seule votre phrase de récupération peut dériver : l'opérateur ne peut pas la lire. Votre phrase ou vos parts imprimées restent indispensables : sans elles, personne ne peut ouvrir la sauvegarde, nous compris.",
      backup_push_off_title: 'Désactiver la sauvegarde privée ?',
      backup_push_off_message:
        "Votre phrase de récupération seule ne peut pas reconstruire ce portefeuille. Les informations liées à la protection de vos pièces n'existent que dans la base de données : sans sauvegarde, un nouvel appareil restaurera votre phrase mais pas vos fonds. Rien de déjà envoyé n'est supprimé.",
      backup_push_off_confirm: 'Désactiver',
      backup_push_on_toast: 'Sauvegardes activées — synchronisation en cours',
      backup_push_off_toast: 'Sauvegardes désactivées — plus rien ne sera envoyé',
      permissions: 'Autorisations',
      trust_network: 'Réseau de confiance',
      // vault (machine-translated, needs native review)
      vault_title: 'Coffre',
      vault_row_title: 'Coffre',
      vault_hero_title: 'Avoirs sécurisés',
      vault_hero_body:
        'Stockage à froid pour le long terme, verrouillé avec vos propres YubiKeys. Nécessite deux YubiKey 5 NFC ou plus.',
      vault_unsupported_title: 'Nécessite une YubiKey',
      vault_unsupported_body: 'Deux YubiKey 5 NFC ou plus, et un téléphone capable de les lire.',
      vault_restore_from_chain: 'Restaurer le coffre depuis la blockchain',
      vault_restore_scanning: 'Analyse de la blockchain…',
      vault_restore_found: '{{count}} dépôt(s) du coffre trouvé(s) sur la blockchain.',
      vault_restore_pending:
        '{{count}} dépôt(s) trouvé(s), en attente de leur première confirmation — réessayez bientôt.',
      vault_restore_none_found: "Aucun dépôt du coffre n'a été trouvé sur la blockchain pour ce portefeuille.",
      vault_balance_label: 'le coffre contient',
      vault_key_section: 'Clés de sécurité ({{count}} sur 5)',
      vault_manage_section: 'Gérer',
      vault_deposit_cta: 'Déposer',
      vault_withdraw_cta: 'Retirer',
      vault_deposit_title: 'Déposer dans le coffre',
      vault_withdraw_title: 'Retirer du coffre',
      vault_deposit_sub: 'Depuis votre solde courant. Aucune YubiKey nécessaire.',
      vault_withdraw_sub: 'Retour vers votre solde courant. Approchez n’importe laquelle de vos clés du coffre.',
      vault_deposit_done: 'Transféré vers le coffre',
      vault_reclaim_done: 'Fonds récupérés d’un dépôt interrompu vers le coffre',
      vault_withdraw_done: 'Retiré du coffre',
      vault_withdraw_partial: 'Il reste {{count}} dépôts — retirez à nouveau pour les déplacer.',
      vault_withdraw_reason: 'Tenez votre YubiKey ici pour signer — retirer {{amount}} sats du coffre',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: 'Pas encore disponible — les dépôts dans le coffre sont désactivés dans cette version.',
      vault_not_on_mainnet_body:
        'Le coffre n’est disponible que sur le réseau principal. Repassez le portefeuille sur le réseau principal pour l’utiliser.',
      vault_intro_title: 'Configurer votre coffre',
      vault_intro_what:
        'Le coffre verrouille l’argent avec vos propres YubiKeys. Rien de la clé du coffre n’existe jamais sur ce téléphone ; n’importe laquelle de vos clés l’ouvre.',
      vault_intro_two_keys: 'Il vous faut au moins deux YubiKeys.',
      vault_intro_apart: 'Gardez-les à des endroits différents — deux clés rangées ensemble n’en font qu’une.',
      vault_intro_backup:
        'Laissez la sauvegarde chiffrée du portefeuille activée — elle conserve le dossier de récupération authentifié.',
      vault_intro_ack:
        'Je comprends : seules mes YubiKeys ouvrent ce coffre. Ma phrase de récupération, non. Si je les perds toutes, l’argent est perdu.',
      vault_intro_piv_ack:
        'Je confirme que toute l’application PIV de cette YubiKey est réinitialisée ou dédiée au coffre. La configuration modifie le PIN, le PUK et la clé de gestion PIV globaux, peut affecter tous les emplacements PIV, et je conserverai le nouveau PUK en lieu sûr.',
      vault_intro_begin: 'Commencer',
      vault_key_step_title: 'Clé {{k}} sur 5 maximum',
      vault_key_step_replace:
        'Utilisez une YubiKey réinitialisée en usine et dédiée au coffre. Si son emplacement contient une ancienne clé, la configuration vous avertira avant de la remplacer.',
      vault_pin_choose_title: 'Choisissez un code PIN',
      vault_pin_choose_sub: 'Vous le saisirez à chaque utilisation de cette clé. 6 à 8 chiffres.',
      vault_pin_confirm_label: 'Saisissez-le à nouveau',
      vault_pin_mismatch: 'Ces deux codes PIN ne correspondent pas.',
      vault_pin_not_default: 'Choisissez autre chose que 123456.',
      vault_puk_title: 'Votre code de récupération',
      vault_puk_body:
        'Notez-le et conservez-le en lieu sûr. C’est le seul moyen de débloquer cette clé en cas d’oubli du code PIN, et il ne sera plus affiché.',
      vault_puk_ack: 'Je l’ai noté',
      vault_reset_offer: 'Réinitialiser cette clé',
      vault_reset_title: 'Réinitialiser cette clé ?',
      vault_reset_body:
        'Cette clé a déjà servi. La réinitialiser efface tous les certificats et toutes les clés de son application PIV, pas seulement l’emplacement du coffre. Tout ce qui en dépend cessera de fonctionner.',
      vault_reset_ack: 'Je comprends que cela efface tout sur cette clé',
      vault_reset_unknown_ack:
        "Cet appareil ne peut pas confirmer si cette clé protège encore le coffre d'un autre profil. Je comprends que l'effacer peut détruire cette clé définitivement.",
      vault_reset_own_draft_ack:
        'Cette clé contient la clé inachevée de votre tentative précédente, dont cet appareil conserve encore la trace. Je comprends que la réinitialiser l’efface et que cette tentative ne pourra plus être reprise.',
      vault_reset_nothing_erased:
        'Rien n’a été effacé. L’emplacement de coffre de cette clé n’est pas vide : il reste une chose à confirmer.',
      vault_reset_confirm: 'Effacer et réinitialiser',
      vault_resetting: 'Réinitialisation de la clé…',
      vault_reset_done: 'Clé réinitialisée. Présentez-la à nouveau pour continuer.',
      vault_reset_uncertain:
        'La réinitialisation ne s’est pas terminée. Cette clé a peut-être déjà été effacée — vérifiez-la avant de vous y fier.',
      vault_err_reset_enrolled:
        'Cette clé fait déjà partie de votre coffre — la réinitialiser détruirait une de vos clés.',
      vault_err_reset_unreadable:
        'Les enregistrements de votre coffre sur cet appareil n’ont pas pu être lus : impossible de prouver que cette clé n’est pas l’une des vôtres. Rien n’a été effacé et la réinitialisation est refusée.',
      vault_enrollment_resume_pin_hint:
        'Reprendre une clé enregistrée demande le code PIN que vous aviez choisi pour elle, pas un nouveau. Trois essais erronés la bloquent.',
      vault_setup_step: 'Étape {{n}} sur {{total}}',
      vault_enrollment_resume: 'Reprendre la clé enregistrée',
      vault_enrollment_reset_required:
        'Cette YubiKey n’est plus dans son état d’usine ; elle ne peut donc pas être enrôlée telle quelle. Son application PIV doit être effacée et ramenée à l’état d’usine avant de pouvoir contenir une clé du coffre.',
      vault_enrollment_state_uncertain:
        'Une tentative de configuration a été interrompue ; impossible de savoir dans quel état la YubiKey a été laissée. Cette clé ne sera plus utilisée tant que son application PIV n’aura pas été effacée et ramenée à l’état d’usine.',
      vault_nfc_enroll_message: 'Tenez votre YubiKey ici pour la configurer',
      vault_nfc_adopt_message: 'Tenez cette YubiKey récupérée ici pour la vérifier',
      vault_nfc_sign_batch: 'Tenez votre YubiKey ici pour signer — lot {{b}} sur {{n}}',
      vault_key_use_different: 'Utiliser une autre YubiKey',
      vault_key_setup_again: 'La reconfigurer',
      vault_name_default: 'Clé {{k}}',
      vault_more_title: 'Ajouter une autre clé ?',
      vault_more_body: 'Jusqu’à 5 clés. N’importe laquelle ouvre tout le coffre à elle seule.',
      vault_more_add: 'Ajouter une autre clé',
      vault_more_finish: 'Terminer',
      vault_more_need_two: 'Ajoutez une deuxième clé avant de terminer — une seule clé, c’est aucune récupération.',
      vault_leave_setup: 'Quitter la configuration',
      vault_leave_title: 'Quitter la configuration ?',
      vault_leave_body:
        'Les {{count}} YubiKey(s) que vous avez configurées ne seront pas encore enregistrées. Elles conservent leurs clés, vous pourrez donc les rajouter dans une minute.',
      vault_leave_confirm: 'Quitter',
      vault_leave_stay: 'Rester',
      vault_done_body:
        '{{count}} clés peuvent ouvrir ce coffre. Seules ces clés l’ouvrent — pas votre phrase de récupération.',
      vault_done_cta: 'Terminé',
      vault_add_key_row: 'Ajouter une clé',
      vault_add_key_done:
        '{{nickname}} peut ouvrir les dépôts effectués à partir de maintenant. Reverrouillez le coffre pour qu’elle puisse tout ouvrir.',
      vault_key_added_toast: 'Clé ajoutée',
      vault_recovery_verified: 'Clé récupérée vérifiée sur cet appareil',
      vault_relock_row: 'Reverrouiller le coffre',
      vault_relock_now: 'Reverrouiller maintenant',
      vault_relock_choose: 'Quelle clé allez-vous approcher pour reverrouiller ?',
      vault_relock_reason: 'Approchez l’une de vos clés existantes ({{names}}) — pas celle que vous venez d’ajouter.',
      vault_relock_reason_generic:
        'Tenez votre YubiKey ici pour signer — reverrouiller le coffre avec vos clés actuelles',
      vault_relock_reason_remaining: 'Re-verrouillez uniquement avec vos clés restantes : {{names}}.',
      vault_key_pending_removal: 'Retirée — re-verrouillez pour terminer',
      vault_key_removal_awaiting_network: 'Retrait en cours — en attente de l’acceptation du reverrouillage par le réseau',
      vault_removal_awaiting_network: 'Reverrouillage envoyé. Les dépôts et retraits reprennent dès que le réseau l’accepte.',
      vault_err_too_many_active_keys: 'Le coffre se verrouille avec cinq clés au plus. Retirez-en une avant de déposer, de reverrouiller ou de retirer une partie du solde.',
      vault_key_section_over_limit: 'Clés de sécurité ({{count}}, retirez-en une)',
      vault_add_key_done_over_limit: '{{nickname}} a été ajoutée. Le coffre se verrouille avec cinq clés au plus : retirez la clé qu’elle remplace, puis reverrouillez une fois.',
      vault_relock_done: 'Coffre reverrouillé avec vos clés actuelles',
      vault_relock_capped: 'Il reste {{count}} dépôts — reverrouillez à nouveau pour les déplacer.',
      vault_relock_unreachable:
        '{{count}} dépôts ne peuvent être ouverts que par {{names}}. Reverrouillez à nouveau avec l’une de ces clés.',
      vault_badge_missing: '{{count}} dépôts pas encore ouverts à {{nickname}}',
      vault_badge_removed: '{{count}} dépôts encore ouverts à une clé retirée',
      vault_key_action_rename: 'Renommer',
      vault_key_action_remove: 'Retirer',
      vault_rename_title: 'Renommer {{nickname}}',
      vault_rename_save: 'Enregistrer',
      vault_remove_title: 'Retirer {{nickname}} ?',
      vault_remove_body:
        'Le portefeuille cessera d’utiliser {{nickname}}. L’argent déjà dans le coffre reste ouvrable par cette clé jusqu’au reverrouillage (≈ {{fee}} sats).',
      vault_remove_only: 'Retirer seulement',
      vault_key_removed_toast: 'Clé retirée',
      vault_export_explainer:
        'Chaque dépôt dans le coffre cache son sel jusqu’à ce que vous le dépensiez — rien à ce sujet n’est visible dans le script de verrouillage. Laissez la sauvegarde chiffrée activée afin qu’un nouvel appareil puisse restaurer et authentifier les fiches de clés nécessaires pour utiliser vos YubiKeys.',
      vault_footnote: 'Seules ces clés ouvrent le coffre. Votre phrase de récupération, non.',
      vault_floor_line:
        'Dépôt minimum {{floorDisplay}} ({{floorSats}} sats). Créer un dépôt dans le coffre coûte environ {{feeDisplay}}.',
      vault_first_deposit_title: 'Premier dépôt dans le coffre',
      vault_first_deposit_body:
        'Premier dépôt dans le coffre — {{amount}} ne pourra être ouvert qu’avec {{count}} YubiKeys ({{names}}). Votre phrase de récupération ne servira à rien.',
      vault_backup_off_title: 'Activez la sauvegarde privée',
      vault_backup_off_body:
        'La sauvegarde privée doit être activée avant que le portefeuille crée une nouvelle sortie du coffre. Elle conserve les fiches YubiKey authentifiées et l’historique du portefeuille nécessaires à la récupération. Vous pouvez toujours tout retirer sans créer de monnaie dans le coffre.',
      vault_backup_off_cta: 'Ouvrir les réglages de sauvegarde',
      vault_seed_not_preserved_title: "Enregistrez d'abord votre phrase de récupération",
      vault_seed_not_preserved_body:
        "Votre coffre ne peut être récupéré qu'avec votre phrase de récupération et l'une de vos YubiKeys. Les YubiKeys n'en conservent aucune copie : si elle est perdue avant que vous ne la notiez, ces fonds ne pourront pas être récupérés. Enregistrez-la maintenant avant de déposer.",
      vault_seed_not_preserved_cta: 'Enregistrer la phrase de récupération',
      vault_choose_key: 'Quelle clé allez-vous approcher ?',
      vault_remainder_title: 'Tout retirer ?',
      vault_remainder_body:
        'Retirer {{amount}} laisse {{remainder}}, en dessous du minimum de 100 000 sats du coffre. Tout le coffre passera dans votre solde courant.',
      vault_remainder_all: 'Tout retirer',
      vault_remainder_change: 'Changer le montant',
      vault_unreachable_title: 'Une partie du coffre nécessite une autre clé',
      vault_unreachable_body:
        'Une partie du coffre nécessite une autre clé — {{moved}} déplacés. {{count}} dépôts totalisant {{amount}} ne peuvent être ouverts que par {{names}}. Retirez à nouveau avec l’une de ces clés.',
      vault_sign_progress: '{{signed}} sur {{total}} signés',
      vault_err_pin_locked_enroll:
        'Le PIN de cette YubiKey est bloqué. Débloquez-le avec son PUK dans Yubico Authenticator, ou configurez avec une autre YubiKey.',
      vault_err_serial_mismatch_chosen:
        'C’est {{tappedName}}. Vous avez choisi {{chosenName}} — approchez-la, ou revenez en arrière et choisissez {{tappedName}}.',
      vault_err_key_already_enrolled: 'Vous avez déjà ajouté cette YubiKey ({{nickname}}). Approchez-en une autre.',
      vault_err_not_released: 'Les dépôts dans le coffre sont désactivés dans cette version.',
      vault_err_not_on_mainnet:
        'Le coffre n’est disponible que sur le réseau principal. Basculez le portefeuille sur le réseau principal, puis réessayez.',
      vault_err_backup_off:
        'Activez la sauvegarde privée chiffrée avant de créer une nouvelle sortie du coffre. Vous pouvez toujours tout retirer.',
      vault_err_not_enough_keys: 'Ce coffre a moins de deux clés. Reconfigurez-le.',
      vault_err_too_many_keys: 'Un coffre accepte au plus cinq clés.',
      vault_err_last_keys: 'Un coffre a besoin d’au moins deux clés. Ajoutez-en une autre avant de retirer celle-ci.',
      vault_err_relock_required:
        'Reverrouillez d’abord le coffre pour que vos autres clés puissent ouvrir chaque dépôt.',
      vault_err_action_pending:
        'Une transaction du coffre n’est pas encore terminée. Réessayez une fois qu’elle est réglée.',
      vault_resolve_held_deposit_action: 'Terminer le dépôt interrompu',
      vault_resolve_held_deposit_done: 'Dépôt résolu',
      vault_resolve_held_deposit_failed: 'Impossible de résoudre le dépôt. Réessayez.',
      vault_resolve_held_deposit_none:
        "Il n'y a rien à résoudre ici : consultez l'onglet Activité pour relancer ou annuler une transaction en attente.",
      vault_err_key_not_committed:
        '{{nickname}} ne peut ouvrir aucun des dépôts de ce coffre. Utilisez une autre de vos clés.',
      vault_err_key_cannot_cover:
        '{{nickname}} peut ouvrir {{reachable}} sur les {{total}} du coffre. Retirez jusqu’à {{reachable}}, ou utilisez {{otherNames}}.',
      vault_err_too_small_to_relock:
        'Ce coffre contient moins de 100 000 sats, trop peu pour reverrouiller. Retirez-le plutôt et déposez à nouveau.',
      vault_err_bad_version: 'La transaction a été construite dans le mauvais format. Rien n’a été signé — réessayez.',
      vault_err_chain_scan_failed:
        "Impossible de terminer l'analyse de la blockchain à la recherche de dépôts du coffre. Vérifiez votre connexion et réessayez.",
      vault_shares_unavailable: 'Aucun matériel de clé disponible à imprimer.',
      vault_enroll_begin: 'Configurer le coffre',
      vault_enroll_phase_connecting: 'Lecture de votre clé…',
      vault_enroll_phase_pin_check: 'Vérification de votre clé…',
      vault_enroll_phase_checking_slot: 'Vérification de l’emplacement du coffre…',
      vault_enroll_phase_personalizing: 'Configuration de votre code PIN et de votre code de récupération…',
      vault_enroll_phase_generating: "Création de la clé du coffre — touchez la clé lorsqu'elle clignote",
      vault_enroll_phase_challenging: "Vérification de la nouvelle clé du coffre — touchez-la lorsqu'elle clignote",
      vault_enroll_phase_done: 'Terminé',
      vault_touch_when_blinks: 'Touchez le contact doré lorsque la clé clignote.',
      vault_nfc_activation_hint:
        "C'est la première fois avec cette YubiKey ? Branchez-la d'abord sur un port USB-C pendant quelques secondes — les nouvelles clés sont livrées avec le NFC désactivé jusque-là.",
      vault_continue: 'Continuer',
      vault_back: 'Retour',
      vault_enrolled_toast: 'Coffre prêt',
      vault_insert_key: 'Insérez votre YubiKey',
      vault_hold_key_nfc: 'Approchez votre YubiKey du haut de votre téléphone',
      vault_keep_holding_nfc: 'Maintenez la position — déverrouillage…',
      vault_unlocking_funds: 'Déverrouillage de vos fonds',
      vault_unlocking_sub: "Cela prend quelques secondes. Gardez l'application ouverte.",
      vault_sending_to_network: 'Envoi au réseau',
      vault_sending_sub: "Si cela ne passe pas tout de suite, nous continuerons d'essayer.",
      vault_reading_key: 'Lecture de la clé…',
      vault_enter_pin: 'Saisissez le PIN YubiKey',
      vault_pin_sub_nfc: 'Approchez ensuite votre clé du haut de votre téléphone pour terminer.',
      vault_pin_sub_usb: 'Touchez ensuite le contact doré pour terminer.',
      vault_touch_contact: 'Touchez le contact doré',
      vault_unlock_cta: 'Déverrouiller',
      vault_pin_retries: '{{count}} tentatives restantes',
      vault_locked: 'Coffre verrouillé',
      vault_retry: 'Réessayer',
      vault_cancel: 'Annuler',
      vault_dismiss: 'Ignorer',
      vault_err_wrong_key: 'Cette YubiKey n’est pas l’une des clés de ce coffre.',
      vault_err_serial_mismatch: 'Cette YubiKey n’est pas l’une des clés de ce coffre ({{names}}).',
      vault_err_touch_timeout: 'Aucun contact détecté. Réessayez et touchez le contact doré.',
      vault_err_pin_locked:
        'Le PIN de cette YubiKey est bloqué. Utilisez une autre de vos clés du coffre, ou débloquez celle-ci avec son PUK dans Yubico Authenticator.',
      vault_err_pin_invalid: 'PIN incorrect.',
      vault_err_puk_invalid: 'PUK incorrect.',
      vault_err_puk_locked: 'Le PUK de cette YubiKey est bloqué. Utilisez une YubiKey neuve.',
      vault_err_ceremony_active:
        "Une autre opération du coffre utilise encore la YubiKey. Attendez qu'elle se termine et réessayez.",
      vault_err_scope_changed:
        'Le portefeuille ou le réseau a changé pendant cette opération. Recommencez dans le portefeuille actuel.',
      vault_err_key_removed_mid_op: 'La clé a été retirée. Insérez-la et réessayez.',
      vault_err_no_key: 'Aucune YubiKey détectée.',
      vault_err_driver_unavailable: "La prise en charge de YubiKey n'est pas disponible sur cet appareil.",
      vault_err_mgmt_key_custom:
        'Cette clé possède une clé de gestion personnalisée et ne peut pas être utilisée automatiquement.',
      vault_err_attestation_invalid:
        'L’attestation du fabricant de cette YubiKey n’a pas pu être vérifiée. Utilisez une YubiKey authentique, attestée en usine, dont l’emplacement d’attestation PIV est intact.',
      vault_err_generic: "Une erreur s'est produite. Veuillez réessayer.",
      vault_err_template_invalid: "Un problème est survenu lors de la tentative d'accès au coffre. Veuillez réessayer.",
      vault_err_not_enrolled: "Aucun coffre n'est configuré.",
      vault_err_pin_required: 'Saisissez votre PIN.',
      vault_err_unsupported_platform: "La prise en charge de YubiKey n'est pas disponible sur cet appareil.",
      vault_err_slot_occupied: "L'emplacement de la clé est déjà utilisé.",
      vault_replace_key_warning:
        'La remplacer détruit définitivement la clé privée P-256 existante. Les fonds du coffre qui dépendent de cette clé pourraient devenir indépensables.',
      vault_replace_key_confirm: 'Remplacer la clé existante',
      vault_resume_draft_confirm_title: 'Utiliser cette clé préparée ?',
      vault_resume_draft_confirm_body:
        'Une clé préparée précédemment — {{nickname}} · …{{tail}} — est prête à être ajoutée, mais cet appareil ne vient pas de la vérifier sur une carte. Continuez uniquement si vous la reconnaissez.',
      vault_resume_draft_confirm_use: 'Utiliser cette clé',
      vault_resume_draft_confirm_discard: 'La rejeter',
      vault_err_enrollment_partial:
        "La configuration de la YubiKey s'est arrêtée après sa modification. Suivez les instructions de nouvel essai ; cette clé n'a pas été ajoutée au coffre.",
      vault_err_key_not_adopted:
        'Vérifiez que vous possédez cette YubiKey récupérée sur cet appareil avant de l’utiliser pour retirer.',
      vault_err_too_many_inputs:
        "Trop de dépôts du coffre à déplacer d'un coup. Retirez d'abord un montant plus petit : chaque retrait consolide aussi le coffre, donc le suivant pourra en déplacer davantage.",
      vault_err_requires_online:
        "Les transferts du coffre nécessitent une connexion. Reconnectez-vous et réessayez : les fonds du coffre ne sont jamais mis en file d'attente.",
      vault_err_user_cancelled: 'Annulé.',
      vault_err_vault_empty: "Le coffre n'a pas encore de fonds à retirer.",
      vault_err_amount_exceeds_balance: "C'est plus que ce que contient le coffre.",
      vault_err_below_dust: 'Ce montant est trop faible pour être envoyé.',
      vault_err_no_transaction: "Le transfert n'a pas pu être effectué. Veuillez réessayer.",
      vault_err_nfc_lost: 'La connexion a été perdue. Maintenez la clé bien à plat contre votre téléphone.',
      vault_pin_invalid_retry:
        'Code PIN incorrect — {{count}} tentatives restantes. Touchez à nouveau Retirer pour réessayer.',
      vault_pin_invalid_retry_generic: 'Code PIN incorrect. Touchez à nouveau Retirer pour réessayer.',
      vault_disable_row: 'Désactiver le coffre',
      vault_disable_title: 'Désactiver le coffre ?',
      vault_disable_message:
        'Cela oublie la liste des clés du coffre sur ce téléphone. Les clés restent sur vos YubiKeys.',
      vault_disable_confirm: 'Désactiver',
      vault_disabled_toast: 'Coffre désactivé',
      vault_disable_blocked_title: "Retirez d'abord les fonds",
      vault_disable_blocked_message:
        'Le coffre contient encore des fonds. Retirez tout vers votre solde courant avant de le désactiver — sinon ces fonds seraient bloqués, sans aucun moyen de les signer.',
      vault_ok: 'OK',
      vault_mock_toggle: 'Utiliser une YubiKey simulée (dev)',
      vault_on: 'Activé',
      vault_off: 'Désactivé',
      vault_shares_word_count:
        'Les parts de récupération ne prennent en charge que les phrases de douze mots. Notez plutôt votre phrase.',
      scan_shares_legacy_title: 'Ancien format de sauvegarde',
      scan_shares_legacy_message:
        'Ces parts utilisent un ancien format qui restaure votre solde courant mais ne peut pas ouvrir de coffre. Si vous avez encore votre phrase de récupération de douze mots, restaurez plutôt à partir de celle-ci — elle récupère tout — puis imprimez de nouvelles parts. Si la phrase est perdue, envoyez vos fonds vers un nouveau portefeuille et recommencez.',
      scan_shares_legacy_ack: "J'ai compris",

      delete_wallet: 'Supprimer le portefeuille',
      delete_wallet_warning_title: 'Supprimer le portefeuille ?',
      delete_wallet_warning_body:
        "Cela supprimera définitivement les clés de votre portefeuille de cet appareil. Si vous n'avez pas sauvegardé votre phrase de récupération ou imprimé les parts de récupération, vous perdrez l'accès à votre portefeuille pour toujours.",
      delete_wallet_confirm: 'Supprimer',
      delete_wallet_failed_title: 'Échec de la suppression du portefeuille',
      delete_wallet_failed_body: 'Votre portefeuille n\'a pas pu être entièrement supprimé de cet appareil. Certaines données de clé peuvent encore être présentes. Veuillez réessayer.',
      bsv_network: 'Réseau BSV',
      arc_endpoint: 'Point de terminaison ARC',
      arc_api_token: "Jeton d'API",
      arc_custom_url: 'URL personnalisée',
      arc_apply: 'Appliquer',
      arc_reset_default: 'Rétablir la valeur par défaut',
      arc_requires_token: 'jeton requis',
      arc_url_https_required: 'Doit être en https (ou http://localhost pour les tests locaux)',
      recovery_phrase: 'Clé de récupération',
      you_have: 'Vous avez',
      switching: 'Changement...',
      mainnet: 'Réseau principal',
      testnet: 'Réseau de test',
      teratest: 'Teratest',
      identity_key: 'Identifiant',
      transactions: 'Transactions',

      no_transactions: 'Aucune transaction pour le moment',
      tx_status_confirmed: 'Confirmé',
      tx_status_received: 'Reçu',
      tx_status_sent: 'Envoyé',
      tx_status_spent: 'Dépensé',
      tx_status_transferred: 'Transféré',
      tx_status_accepted: 'Acceptée',
      tx_status_broadcasting: 'Traitement',
      tx_status_not_sent: 'Non envoyé',
      tx_status_unsigned: 'Brouillon',
      tx_status_nonfinal: 'En attente',
      tx_status_pending: 'En attente',
      tx_status_failed: 'Échoué',
      tx_status_offline_queued: 'Hors ligne · en file',
      tx_status_offline_sending: 'Hors ligne · envoi',
      tx_status_offline_rejected: 'Hors ligne · rejeté',
      tx_copied: 'Transaction copiée',
      tx_not_available: 'Transaction non disponible',
      tx_copy_failed: 'Échec de la copie de la transaction',
      tx_abort_success: 'Transaction annulée',
      tx_abort_failed: "Échec de l'annulation de la transaction",
      tx_abort_maybe_delivered: "Impossible d'annuler : ce paiement est peut-être déjà chez le destinataire",
      tx_proof_refreshed: 'Transaction confirmée',
      tx_proof_refresh_failed: "Preuve introuvable — la transaction n'est peut-être pas encore confirmée",
      tx_marked_failed: 'Non traitée — marquée comme échouée et fonds libérés',
      tx_still_pending: 'Toujours en attente — pas encore confirmée',
      tx_action_explorer: "Voir dans l'explorateur de blocs",
      tx_export_success: '{{count}} transactions exportées',
      tx_export_failed: "Échec de l'exportation des transactions",
      tx_export_csv: 'Exporter en CSV',
      tx_export: 'Exporter',
      activity_filter: 'Filtrer l’activité',
      activity_filter_all: 'Tout',
      activity_search_placeholder: 'Rechercher notes, noms, @pseudos',
      activity_search_clear: 'Effacer la recherche',
      activity_filter_no_match: 'Aucune activité correspondante',
      wallet_balance_label: 'Solde',
      wallet_balance_you_have: 'Vous avez',
      wallet_balance_refresh: 'Actualiser le solde',
      wallet_group_today: "Aujourd'hui",
      wallet_group_yesterday: 'Hier',
      back: 'Retour',
      wallet_payments: 'Paiements',
      wallet_vault: 'Coffre',
      wallet_settings: 'Paramètres',
      wallet_activity: 'Activité',

      message_box_server: 'Serveur de boîte aux lettres',
      message_box_required: "Requis pour envoyer et recevoir des paiements basés sur l'identité.",
      send_payment: 'Envoyer un paiement',
      recipient: 'Destinataire',
      amount: 'Montant',
      enter_valid_amount: 'Saisissez un montant supérieur à zéro',
      display_currency: "Devise d'affichage",
      no_pending_payments: 'Aucun paiement en attente',
      outgoing_payments: 'Sortants',
      payment_not_delivered: 'Non livré',
      payment_delivered: 'Livré',
      retry: 'Réessayer',
      retry_failed: 'Nouvelle tentative échouée',
      dismiss: 'Ignorer',
      valid_identity_key: 'Identifiant valide saisi',
      valid_bsv_address: 'Adresse BSV valide saisie',
      searching: 'Recherche en cours...',
      unknown: 'Inconnu',
      from_prefix: 'De :',
      message_box_saved: 'Serveur de boîte aux lettres enregistré',
      message_box_removed: 'Serveur de boîte aux lettres supprimé',
      message_box_off: 'Désactivé',
      message_box_off_hint: 'Le serveur de boîte aux lettres est désactivé. Activez-le dans Réglages › Avancé.',
      enter_valid_url: 'Veuillez saisir une URL valide',
      checking_connection: 'Vérification de la connexion...',
      connection_failed: 'Échec de la connexion',
      recipient_placeholder: 'Nom, identifiant ou adresse BSV...',

      trust_updated: 'Réseau de confiance mis à jour',
      failed_to_save: "Échec de l'enregistrement",
      confirm_delete: 'Confirmer la suppression',
      confirm_delete_body: 'Êtes-vous sûr de vouloir supprimer ce certificateur ?',
      order_certifiers_hint:
        'Classez les certificateurs par priorité. Les applications afficheront en premier les certificateurs les mieux classés.',
      certifiers: 'Certificateurs',
      no_certifiers: 'Aucun certificateur pour le moment.',
      add_provider: 'Ajouter un fournisseur',
      unsaved_changes: 'Vous avez des modifications non enregistrées',
      duplicate_key_error: 'Une entité avec cette clé publique est déjà dans la liste !',
      get_provider_details: 'Obtenir les détails du fournisseur',
      validate_details: 'Valider les détails',
      show_advanced: 'Afficher avancé',
      hide_advanced: 'Masquer avancé',
      description_length_error: 'La description doit comporter entre 5 et 50 caractères',
      enter_domain_hint: 'Saisissez le domaine du fournisseur',
      enter_details_hint: 'Saisissez les détails du fournisseur',

      receive: 'Recevoir',
      send: 'Envoyer',
      generating_address: "Génération de l'adresse BSV...",
      imported: 'Importé',
      unable_to_generate_address: "Impossible de générer l'adresse BSV",
      send_bsv: 'Envoyer BSV',
      recipient_address: 'Adresse BSV du destinataire',
      enter_bsv_address: "Saisir l'adresse BSV",

      wallet_data: 'Données du portefeuille',
      create_new_wallet: 'Créer un nouveau portefeuille',
      generate_recovery_phrase_caption: 'Générer une nouvelle clé de récupération',
      import_existing_wallet: 'Importer un portefeuille existant',
      paste_recovery_phrase: 'Collez votre clé de récupération',
      save_recovery_phrase_heading: 'Sauvegardez votre clé de récupération',
      save_recovery_phrase_btn: 'Enregistrer la clé de récupération',
      recovery_phrase_write_down: "Notez ces 12 mots dans l'ordre et conservez-les en lieu sûr.",
      acknowledgment_text:
        "J'ai sauvegardé ma clé de récupération et je comprends que sa perte entraînera la perte totale et permanente de tous les fonds, jetons et certificats associés.",
      enter_recovery_words: 'Saisissez la phrase de 12 mots',
      restore_wallet_description: 'Entrez votre clé de récupération pour restaurer votre portefeuille',
      backup_reminder_title: "Votre portefeuille n'est pas sauvegardé",
      backup_reminder_body: 'Exportez vos clés pour éviter de perdre vos fonds.',
      biometric_advisory_title: 'Sécurisez votre portefeuille',
      wallet_balance_show_in: 'Afficher en {{unit}}',
      tx_detail_block: 'Bloc',
      tx_detail_block_pending: "En attente d'un bloc",
      biometric_advisory_body: 'Protégez vos actifs avec Face ID ou votre empreinte digitale',
      biometric_advisory_body_degraded:
        "Cet appareil n'a pas de Face ID ni d'empreinte digitale configurés, votre clé de portefeuille n'est donc protégée que par le code d'accès de l'appareil.",
      import_prompt_title: 'Vous avez déjà un portefeuille ?',
      import_prompt_body: 'Restaurez-le à partir de votre clé de récupération ou de vos parts.',
      import_prompt_action: 'Importer depuis une sauvegarde',
      nearby_advisory_title: 'Connexion à proximité',
      nearby_advisory_body:
        "Il vous sera demandé d'autoriser l'accès au Wi-Fi, au Bluetooth ou au NFC. Cela sert uniquement à vous connecter directement à un appareil proche pour le paiement — jamais envoyé à nos serveurs, jamais utilisé pour établir un profil de vous.",
      backup_reminder_now: 'Sauvegarder les clés maintenant',
      backup_reminder_import: 'Importer depuis une sauvegarde',
      restore_backup_checking: 'Recherche de votre sauvegarde chiffrée…',
      restore_backup_progress: "Restauration de l'historique — {{chunks}} sur {{total}}…",
      restore_backup_failed_title: "Impossible de restaurer l'historique",
      restore_backup_failed_message:
        "Votre phrase de récupération est valide, mais la sauvegarde chiffrée n'a pas pu être rejouée. Un nouvel essai suffit généralement. Sans elle, le portefeuille fonctionne mais sans transactions passées, et certaines de vos pièces actuelles ne seront pas dépensables.",
      restore_backup_retry: 'Réessayer',
      restore_backup_skip: 'Abandonner',
      restore_backup_unverified_title: 'La sauvegarde est peut-être incomplète',
      restore_backup_unverified_body:
        "Votre portefeuille a été restauré, mais il n'a pas été possible de confirmer que la sauvegarde sur le serveur était complète. S'il manque de l'historique, restaurez à nouveau plus tard une fois que l'autre appareil aura terminé son envoi.",
      import_wallet: 'Importer le portefeuille',

      reject: 'Rejeter',
      authorize: 'Autoriser',
      renewal: 'Renouvellement',
      spending_section: 'Dépenses',
      protocols_section: 'Protocoles',
      baskets_section: 'Paniers',
      certificates_section: 'Certificats',
      requested_fields: 'Champs demandés',

      send_max: 'Envoyer le maximum',
      message_box_unreachable:
        "Impossible de joindre le serveur de paiement. Vérifiez l'URL de la message box dans les réglages.",
      available: 'disponible',
      note: 'Note',
      note_placeholder: "C'est pour quoi ? (facultatif)",
      entire_wallet_balance: 'Solde total du portefeuille',
      entire_vault_balance: 'Tout le solde du coffre',
      export_wallet_data: 'Exporter les données du portefeuille',
      export_unencrypted_title: "Ce fichier n'est pas chiffré",
      export_unencrypted_message:
        "Le fichier exporté est une copie non chiffrée de vos données de portefeuille — informations d'identité et de certificat, historique des transactions et contacts inclus. Quiconque obtient ce fichier peut le lire. Ne le partagez qu'avec une personne ou un endroit de confiance.",
      import_wallet_data: 'Importer les données du portefeuille',
      import_conflict_title: 'Base de données existante',
      import_conflict_message:
        "Une base de données de portefeuille pour cette identité existe déjà avec un horodatage plus récent. La sauvegarde avec le numéro le plus élevé dans son nom de fichier est toujours utilisée.\n\nPour utiliser cette ancienne sauvegarde : exportez vos données actuelles, supprimez et réinstallez l'application, puis importez uniquement la sauvegarde souhaitée.",
      import_confirm_title: 'Importer la base de données',
      import_confirm_message: 'Cette base de données deviendra votre portefeuille actif. Continuer ?',
      import_success: 'Base de données importée avec succès. Redémarrage du portefeuille...',
      import_invalid_file: 'Fichier de sauvegarde invalide',
      import_invalid_file_detail:
        "Ce fichier n'est pas une sauvegarde de base de données valide. Nom de fichier attendu : wallet-XXXXXXXX-mainnet-TIMESTAMP.db.",
      import_oversized_file: 'Fichier de sauvegarde trop volumineux',
      import_oversized_file_detail:
        "Ce fichier est plus volumineux qu'une sauvegarde de portefeuille ne devrait l'être, il n'a donc pas été lu. Choisissez un fichier de sauvegarde de base de données de portefeuille authentique.",
      import_untrusted_file: "Impossible d'importer cette sauvegarde",
      import_untrusted_file_detail:
        "Cette sauvegarde n'a pas pu être vérifiée comme appartenant à ce portefeuille, ou son contenu n'a pas pu être approuvé. Elle n'a pas été importée.",
      import_anyway: 'Importer',
      import_invalid_input_title: 'Saisie invalide',
      import_invalid_input_message:
        'Veuillez saisir une phrase de récupération valide (12 à 24 mots) ou une clé privée hexadécimale de 64 caractères.',
      import_setup_failed: 'Échec de la configuration du portefeuille : {{error}}',
      create_wallet_refused: 'Impossible de créer le portefeuille. Veuillez réessayer.',
      create_wallet_failed: 'Échec de la génération de la phrase mnémotechnique. Veuillez réessayer.',
      or: 'ou',
      print_recovery_shares: 'Imprimer les Parts de Récupération',
      scan_backup_shares: 'Scanner les Parts de Sauvegarde',
      scan_backup_shares_caption: 'Récupérer le portefeuille à partir des parts imprimées',
      scan_shares_invalid_format: 'Format de part invalide. Veuillez scanner un code QR de part de sauvegarde valide.',
      scan_shares_camera_needed: 'Accès à la Caméra Requis',
      scan_shares_camera_description:
        "L'accès à la caméra est nécessaire pour scanner vos codes QR de parts de sauvegarde imprimées.",
      scan_shares_grant_camera: "Accorder l'Accès à la Caméra",
      scan_shares_recovering: 'Récupération du portefeuille...',
      scan_shares_scan_first: 'Scannez votre première part de sauvegarde',
      scan_shares_progress: '{{scanned}} sur {{needed}} parts scannées',
      scan_shares_remaining: '{{count}} part supplémentaire nécessaire',
      scan_shares_complete: 'Toutes les parts collectées !',
      scan_shares_recovery_failed: 'La récupération a échoué. Veuillez réessayer.',
      scan_shares_threshold_mismatch: 'Le seuil ne correspond pas aux parts précédentes',
      scan_shares_integrity_mismatch:
        "Le hash d'intégrité ne correspond pas — les parts proviennent de clés différentes",
      scan_shares_duplicate: 'Cette part a déjà été scannée',
      scan_shares_biometric_title: 'Accès biométrique requis',
      scan_shares_biometric_message:
        "L'accès biométrique est nécessaire pour protéger les clés de votre portefeuille. Veuillez réessayer.",
      recovery_replace_wallet_title: 'Remplacer le portefeuille de ce téléphone ?',
      recovery_replace_wallet_body:
        'Ce téléphone contient déjà un portefeuille. Continuer le remplacera, ainsi que tout ce qu’il peut dépenser. Si sa phrase de récupération n’est pas sauvegardée ailleurs, cet argent est perdu pour toujours.',
      recovery_replace_wallet_confirm: 'Remplacer le portefeuille',
      invalid_bsv_address: 'Adresse BSV invalide',
      scan_recipient_hint: 'Pointez la caméra vers un code de paiement',
      scan_qr_default_hint: 'Pointez la caméra vers un code QR',
      scan_wallet_qr_hint: "Pointez la caméra vers un code QR d'appariement",

      biometric_store_wallet: 'Stocker votre portefeuille en toute sécurité',
      biometric_load_wallet: 'Charger votre portefeuille en toute sécurité',
      biometric_unlock_wallet: 'Déverrouillez votre portefeuille',
      wallet_lost_title: 'Clé du portefeuille détruite',
      wallet_lost_body:
        'Cet appareil a modifié ses réglages biométriques ; le système a donc détruit la clé qui protégeait ce portefeuille. Vos pièces sont en sécurité sur la blockchain — restaurez ce portefeuille avec votre phrase de récupération.',
      wallet_lost_restore: 'Restaurer',
      wallet_lost_later: 'Plus tard',
      wallet_locked_title: 'Portefeuille verrouillé',
      wallet_locked_body: 'Déverrouillez pour charger les clés de votre portefeuille.',
      wallet_unlock_retry: 'Déverrouiller le portefeuille',
      wallet_lockout_body: 'Trop de tentatives échouées. Déverrouillez votre téléphone avec son code, puis réessayez.',
      wallet_no_biometrics_body:
        'Aucune biométrie n’est configurée sur cet appareil. Vos clés restent chiffrées par le magasin de clés de l’appareil, mais elles ne sont pas protégées par une vérification biométrique.',

      // Connections screen
      connections: 'Connexions',
      advanced: 'Avancé',
      copy_secret_words: 'Copier les mots secrets',
      print_recovery_keys: 'Imprimer les clés de récupération',
      backup_wallet_keys: 'Sauvegarder les clés du portefeuille',
      log_out: 'Se déconnecter',
      connect_to_app: 'Connecter une app',
      no_connections_yet: "Aucune connexion pour l'instant",
      no_connections_subtitle: 'Scannez un code QR depuis une app bureau pour connecter votre portefeuille',
      connections_active: 'Actif',
      connections_disconnected: 'Déconnecté',
      disconnect: 'Déconnecter',
      reconnect: 'Reconnecter',
      reconnect_failed: 'Reconnexion échouée',
      reconnect_confirm_title: 'Se reconnecter à ce site ?',
      reconnect_confirm_message: '{{origin}} pourra à nouveau accéder à votre portefeuille.',
      reconnect_needs_reapproval: 'Cette connexion n\'a pas pu être vérifiée. Scannez un nouveau code QR pour vous reconnecter.',
      scan_qr_code: 'Scanner le code QR',
      paste_uri: "Coller l'URI",
      invalid_qr_code: 'Code QR invalide',
      wallet_not_ready: 'Portefeuille non prêt',
      please_log_in_first: "Veuillez d'abord vous connecter",
      unknown_error: 'Erreur inconnue',

      // Pay screen
      payments: 'Paiements',
      pay: 'Payer',
      pay_direction_pay: 'Payer',
      pay_direction_receive: 'Être payé',
      pay_cell_nearby_pay: 'Quelqu’un à proximité',
      pay_cell_nearby_pay_sub: 'Scannez son code',
      pay_cell_handle_pay: 'Quelqu’un à distance',
      pay_cell_handle_pay_sub: 'Choisissez un nom — cette application leur est nécessaire',
      pay_cell_address_pay: 'Vers une adresse BSV',
      pay_cell_address_pay_sub: 'Collez ou scannez une adresse BSV',
      pay_cell_nearby_get: 'Quelqu’un à proximité',
      pay_cell_nearby_get_sub: 'Montrez votre code de paiement',
      pay_cell_handle_get: 'Quelqu’un à distance',
      pay_cell_handle_get_sub: 'Partagez votre nom',
      pay_cell_address_get: 'Vers une adresse BSV',
      pay_cell_address_get_sub: 'Affichez une adresse BSV',
      pay_method: 'Méthode',
      pay_method_nearby: 'À proximité',
      pay_method_remote_link: 'Partager un lien à distance',
      pay_method_address: 'Vers une adresse BSV',
      pay_pre_nearby: 'Vous êtes tous les deux là.',
      pay_pre_handle: 'Cette personne utilise cette application.',
      pay_pre_address: 'Vous avez son adresse BSV.',
      pay_conseq_nearby: 'Réglé en quelques secondes.',
      pay_conseq_handle: 'Arrive dès que son portefeuille vérifie à nouveau.',
      pay_conseq_address: 'L’argent part — cette personne n’est pas prévenue.',
      paid: 'Payé',
      pay_inbox_attention: 'Impossible à ajouter',
      pay_inbox_attention_hint:
        'Ils sont arrivés mais votre portefeuille n’a pas pu les ajouter. Réessayez, ou ignorez-les pour y renoncer définitivement.',
      pay_dismiss_confirm: 'Touchez à nouveau pour renoncer',
      pay_dismissed: 'Paiement abandonné',
      payment_arrived: 'Paiement reçu',
      payment_arrived_damaged: 'Ce paiement est arrivé endommagé',
      discard_payment_title: 'Abandonner ce paiement ?',
      discard_payment_body:
        "Il sera demandé à l'expéditeur de renvoyer les détails. L'argent reste sur la chaîne jusque-là.",
      discard_void_title: "Ce paiement n'a pas abouti",
      discard_void_body: "Il a été annulé ou dépensé ailleurs. Vous pouvez l'ignorer sans risque.",
      waiting_network_confirm: 'En attente de la confirmation de ce paiement par le réseau',
      finish_or_cancel_outgoing: "Terminez ou annulez le paiement ci-dessous avant d'en envoyer un nouveau",
      identity_search_unavailable: 'La recherche est indisponible pour le moment. Réessayez dans un instant.',
      abandon_payment: 'Abandonner le paiement',
      finish_payment: 'Terminer le paiement',
      cancel_this_payment: 'Annuler ce paiement ?',
      cancel_payment: 'Annuler le paiement',
      resend: 'Renvoyer',
      resending: 'Renvoi…',
      send_payment_details_again: 'Renvoyer les détails',
      tx_action_resend_short: 'Renvoyer',
      tx_action_request_confirmation: 'Demander une confirmation au destinataire',
      resend_sent: 'Détails du paiement renvoyés',
      resend_failed_no_record:
        'Les détails de ce paiement sont introuvables sur cet appareil : impossible de le renvoyer.',
      resend_failed_no_recipient: 'Ce paiement n’indique pas à qui il était destiné : impossible de le renvoyer.',
      resend_failed_no_transaction: 'Impossible de récupérer les données de ce paiement. Réessayez une fois en ligne.',
      resend_requested: 'On vous a demandé de renvoyer un paiement',
      request_again: 'Redemander',
      send_again: 'Renvoyer',
      copy_details: 'Copier les détails',
      payment_bounced_resend: 'Demande envoyée de renvoyer ce paiement',
      dismiss_rejected_payment: 'Paiement refusé ignoré',
      activity_load_failed: "Impossible de charger l'activité",
      activity_load_retry: 'Touchez pour réessayer',
      activity_view_contact: 'Voir le contact',
      home_payments_need_attention: '{{count}} paiements demandent votre attention',
      home_payments_unsent: '{{count}} paiements non remis',
      pay_copy: 'Copier',
      pay_share_link: 'Partager le lien',
      pay_address_watching: 'L’argent envoyé ici est ajouté automatiquement à votre portefeuille.',
      pay_address_earlier_day: 'Revenir à un jour précédent',
      pay_address_sweep_now: 'Vérifier cette adresse BSV maintenant',
      pay_address_swept: '{{amount}} reçu',

      check_wallet: 'Dépannage',
      settings_version: 'v{{version}}',
      error_review_actions: 'Ce paiement est bloqué. Dépannage peut essayer de le débloquer.',
      wallet_check_ok: 'Tout semble en ordre',
      wallet_check_attention: 'Certaines vérifications demandent votre attention',
      wallet_check_couldnt: 'Vérification impossible',
      wallet_check_couldnt_body: "Une étape de réparation n'a pas abouti. Rien n'a été supposé au sujet de vos pièces.",
      wallet_check_summary: '{{freed}} pièces bloquées libérées · {{recovered}} paiements récupérés',
      wallet_check_step_coins: 'Vos pièces sont valides',
      wallet_check_skip: 'Ignorer cette vérification',
      wallet_check_skipped: 'Ignorée',
      wallet_check_step_proofs: 'Preuves de transaction',
      wallet_check_step_records: 'Registres de la base locale',
      wallet_check_step_tokens: 'Transferts de stablecoin vérifiés',
      wallet_check_step_online: 'Connexion',
      wallet_check_step_missed: 'Paiements manqués',
      wallet_check_step_backup: 'Sauvegarde privée envoyée',
      wallet_check_step_phrase: 'Phrase de récupération enregistrée',
      wallet_check_progress: '{{current}} sur {{total}}',
      wallet_check_stuck_title: "Impossible d'envoyer ce paiement",
      wallet_check_stuck_message: 'Certaines pièces semblent bloquées. Dépannage peut les libérer.',
      wallet_check_failed: 'Impossible de terminer la vérification. Réessayez.',

      // Local payments
      local_pay_request: 'Demander un paiement',
      local_pay_send: 'Envoyer un paiement',
      local_pay_amount: 'Montant',
      local_pay_show_qr: 'Montrez ce code au payeur',
      local_pay_scan_qr: 'Scannez le code du bénéficiaire',
      local_pay_scan_payer_qr: 'Scannez le code du payeur',
      local_pay_scan_progress: '{{have}} sur {{total}} fragments',
      local_pay_show_payment_qr: 'Montrez ce code au bénéficiaire',
      local_pay_animated_hint:
        'Demandez-lui de scanner le code animé ci-dessous : il continue de changer jusqu’à ce que son téléphone l’ait reçu en entier.',
      local_pay_show_qr_instead: 'Afficher un code QR à la place',
      local_pay_preparing: 'Préparation de la demande…',
      local_pay_delivering: 'Envoi du paiement…',
      local_pay_saving: 'Enregistrement du paiement…',
      local_pay_sent: 'Paiement envoyé',
      local_pay_received: 'Paiement reçu',
      local_pay_added: 'Ajouté à votre portefeuille.',
      local_pay_added_multiple: '{{count}} paiements ajoutés à votre portefeuille.',
      local_pay_received_confirming: 'Confirmation avec votre portefeuille…',
      local_pay_received_not_credited:
        'Pas encore ajouté à votre portefeuille. Vérifiez l’Activité avant de remettre quoi que ce soit.',
      local_pay_decline_broadcast_title: 'Un paiement refusé est passé sur le réseau',
      local_pay_decline_broadcast_body:
        'Un paiement qui avait été refusé est depuis apparu sur le réseau. Ne l’envoyez pas à nouveau — vérifiez l’Activité.',
      local_pay_pending_abort_dropped_title: 'Demande de libération non vérifiée ignorée',
      local_pay_pending_abort_dropped_body:
        'Un enregistrement de libération de paiement n’a pas pu être vérifié, donc rien n’a été libéré. Si un paiement semble toujours bloqué, annulez-le depuis l’Activité.',
      local_pay_cancel_unverifiable_title: 'Impossible de vérifier ce paiement hors connexion',
      local_pay_cancel_unverifiable_body:
        'Si l’autre personne l’a déjà scanné, annuler permettrait de dépenser ces pièces deux fois. N’annulez que si vous êtes sûr qu’il n’a jamais été scanné.',
      local_pay_cancel_unverifiable_confirm: 'Annuler quand même',
      local_pay_queued: 'Enregistré — il sera ajouté automatiquement à votre portefeuille.',
      local_pay_already_paid: 'Cette demande a déjà été payée.',
      local_pay_failed: 'Échec du paiement',
      local_pay_network_denied:
        'L’accès au réseau local est désactivé. Activez-le dans Réglages pour payer les appareils à proximité.',
      local_pay_radio_fallback: 'Liaison sans fil indisponible — présentez plutôt ce code à l’autre appareil.',
      local_pay_floor_peer_bt_off: 'Leur Bluetooth est désactivé, ce paiement passera donc par le code.',
      local_pay_floor_local_bt_off:
        'Votre Bluetooth est désactivé. Activez-le pour une liaison de proximité plus rapide.',
      local_pay_floor_local_ble_denied:
        'L’accès au Bluetooth est désactivé pour BSV Wallet. Autorisez-le dans Réglages pour une liaison de proximité plus rapide.',
      local_pay_floor_cross_os:
        'L’autre appareil ne peut pas utiliser le Bluetooth pour le moment, ce paiement passera donc par le code.',
      local_pay_floor_peer_no_radio:
        'L’autre appareil n’attend pas de liaison sans fil, ce paiement passera donc par le code.',
      local_pay_ble_unavailable: 'Le Bluetooth est indisponible. Le code ci-dessus fonctionne toujours.',
      local_pay_too_large: 'Les données de ce paiement sont trop volumineuses pour être transmises par code QR.',
      local_pay_nearby_unavailable: 'Le Wi-Fi à proximité est indisponible. Le code ci-dessus fonctionne toujours.',
      local_pay_retry_save: 'Réessayer d’enregistrer',
      local_pay_wrong_session:
        'Ce paiement ne correspond pas à cette demande et n’a pas été accepté. Demandez au payeur de scanner le code à nouveau.',
      local_pay_declined_mismatch:
        'Le bénéficiaire n’a pas accepté ce paiement : il ne correspond pas à sa demande. Scannez à nouveau son code.',
      local_pay_declined_already_paid: 'Cette demande a déjà été payée. Rien n’a été envoyé.',
      local_pay_declined_save: 'Le bénéficiaire n’a pas pu enregistrer le paiement. Rien n’a été envoyé — réessayez.',
      local_pay_declined_decode: 'Le bénéficiaire n’a pas pu lire le paiement. Rien n’a été envoyé — réessayez.',
      local_pay_declined_not_covered:
        'Le bénéficiaire n’a pas pu vérifier ce paiement auprès des registres de l’émetteur. Rien n’a été envoyé.',
      local_pay_broadcast_pending:
        'Envoyé. Cet appareil n’a pas encore pu le faire traiter — le bénéficiaire s’en chargera.',
      local_pay_any_amount: 'Montant libre',
      local_pay_choose_amount: 'Choisissez un montant',
      local_pay_enter_amount_send: 'Quel montant souhaitez-vous envoyer ?',
      local_pay_qr_unavailable: 'Ce code n’a pas pu être créé. Relancez la demande.',
      local_pay_presence_qr: 'Par code — aucune connexion en direct',
      local_pay_presence_ready: 'Liaison chiffrée à proximité prête',
      local_pay_presence_waiting_payee: 'En attente d’un appareil à proximité',
      local_pay_presence_waiting_payer: 'Recherche de l’appareil du destinataire…',
      local_pay_presence_linked: 'Liaison chiffrée ouverte',
      local_pay_presence_paid: 'Paiement confirmé',
      pay_offline_title: 'Vous êtes hors ligne',
      pay_offline_body:
        'Seuls les paiements à proximité fonctionnent pour l’instant. Ils seront traités dès que vous serez reconnecté.',
      pay_offline_queued: '{{count}} paiement(s) en attente de traitement dès que vous serez reconnecté.',
      pay_offline_pending_title: 'En attente de traitement',
      pay_offline_pending_body:
        '{{count}} paiement(s) n’ont pas encore été traités. Rien n’est réglé avant cela ; les tentatives continuent.',
      pay_offline_needs_internet: 'Connexion requise',
      pay_offline_rejected_title: 'Un paiement a été refusé',
      pay_offline_rejected_body: 'Le réseau a refusé un paiement remis par {{sender}} via {{via}} le {{when}}.',
      pay_offline_unknown_sender: 'un expéditeur inconnu',
      pay_offline_unknown_via: 'une méthode inconnue',
      pay_received_not_broadcast: 'Reçu hors ligne · pas encore traité',
      pay_offline_sent_rejected_title: 'Un paiement n’a pas pu être livré',
      pay_offline_sent_rejected_body: 'Le réseau a refusé un paiement que vous avez envoyé le {{when}}.',
      pay_offline_send_now: 'Envoyer maintenant',
      pay_offline_stalled_body:
        'Certains paiements en attente ne peuvent pas être envoyés automatiquement : {{detail}}',
      offline_stall_no_request: "Il manque son enregistrement d'envoi à un paiement en file d'attente.",
      offline_stall_bad_beef: "Les données de transaction d'un paiement en file d'attente n'ont pas pu être lues.",
      offline_stall_foreign_ancestor:
        "Un paiement en file d'attente attend une transaction parente qu'un autre portefeuille n'a jamais traitée.",
      pay_offline_kv_pending: "{{count}} paiement(s) à proximité en attente d'ajout à ce portefeuille.",
      pay_offline_kv_stuck:
        'Impossible d’ajouter {{count}} paiement(s) à proximité. Demandez à l’expéditeur de renvoyer.',
      pay_offline_kv_corrupt: 'Des données de paiement endommagées ont été trouvées sur cet appareil.',
      pay_offline_kv_corrupt_body:
        'Les paiements concernés ont été mis de côté sur cet appareil, pas supprimés. Certains ont peut-être déjà été ajoutés à votre solde : ne demandez un nouvel envoi que si le paiement manque toujours.',
      pay_offline_show_code: 'Réafficher le code',
      pay_parked_cancel: 'Annuler le paiement',
      pay_parked_already_sent: 'Ce paiement a déjà été envoyé',
      token_cancel_already_sent: 'Ce paiement est déjà passé et ne peut pas être annulé.',
      tx_status_parked: 'Non remis',
      tx_status_import_hold: 'Suspendu (importé)',
      resend_sending: 'Envoi via la boîte à messages',
      cancelling_payment: 'Annulation du paiement',

      contacts: 'Contacts',
      contacts_search_placeholder: 'Rechercher par nom ou pseudo',
      contacts_empty_title: 'Aucun contact pour le moment',
      contacts_empty_body:
        "Les personnes que vous payez apparaissent ici une fois enregistrées. Scannez le code de profil de quelqu'un avec le bouton en haut à droite.",
      contacts_recent: 'Récents',
      contact_edit_name: 'Modifier le nom',
      contact_save_name: 'Enregistrer le nom',
      contact_activity: 'Activité',
      contact_activity_empty: 'Aucune activité avec ce contact pour le moment',
      contact_delete: 'Supprimer le contact',
      contact_delete_body: "Cela retire {{name}} de vos contacts. Cela n'affecte aucun paiement passé.",
      contact_handle_caption: 'Son pseudo enregistré · lui seul peut le modifier',
      contact_new_title: 'Nouveau contact',
      contact_save: 'Enregistrer',
      contact_identifier: 'Identifiant',
      contact_saved: 'Contact enregistré',
      contact_scan_hint: "Scannez un code de profil pour ajouter quelqu'un",
      contact_identifier_placeholder: 'Collez son identifiant',
      contact_identifier_invalid: 'Cela ne ressemble pas à un identifiant valide',
      contact_add_from_payment: 'Ajouter aux contacts',

      profile: 'Profil',
      profile_display_name: 'Nom affiché',
      profile_display_name_hint:
        'Affiché publiquement à côté de votre pseudo. Toute personne qui vous recherche peut le voir.',
      profile_handle: 'Pseudo',
      profile_handle_placeholder: 'votrenom',
      profile_handle_checking: 'Vérification de la disponibilité…',
      profile_handle_available: '{{handle}} est disponible',
      profile_handle_taken: '{{handle}} est déjà pris',
      profile_handle_invalid:
        '3-32 caractères : a-z, 0-9, point, tiret bas ou tiret ; commence et finit par une lettre ou un chiffre',
      profile_handle_failed: 'Impossible de vérifier la disponibilité. Réessayez.',
      profile_handle_unavailable: "L'enregistrement de pseudo n'est pas encore disponible sur cette version.",
      profile_handle_registered: 'Pseudo enregistré',
      profile_handle_register_action: 'Enregistrer',
      profile_handle_too_similar: '{{handle}} ressemble trop à un pseudo déjà utilisé',
      profile_handle_reserved: '{{handle}} est réservé',
      profile_handle_cooldown: '{{handle}} a été libéré récemment et n’est pas encore disponible',
      profile_handle_pending: 'Finalisation de l’enregistrement…',
      profile_handle_rolled_back: '{{handle}} était déjà pris. Vous gardez {{previous}}.',
      profile_handle_changed: 'Pseudo modifié',
      profile_handle_rejected: 'Le registre a refusé ce pseudo.',
      profile_handle_clock_ahead: "L'horloge de votre appareil est trop en avance. Réessayez dans quelques minutes.",
      profile_handle_wrong_registry: "Ce pseudo n'est pas sur ce registre. Rouvrez Profil pour l'actualiser.",
      profile_display_name_publish_failed: "Votre nom est enregistré ici, mais le registre ne l'a pas publié.",
      profile_qr_hint: "D'autres personnes peuvent scanner ceci pour vous ajouter comme contact",

      pay_step_contacts: 'Contacts',
      pay_step_continue: 'Continuer',
      pay_review_to: 'À',
      pay_review_amount: 'Montant',
      pay_review_note: 'Note',
      pay_review_note_edit: 'Ajouter une note',
      pay_review_delivery_host: 'Livraison à',
      pay_trust_contact: 'Votre contact',
      pay_trust_handle_attested: 'Enregistré',
      pay_trust_handle_domain_attested: 'Vérifié par {{domain}}',
      pay_trust_unverified: 'Non vérifié. Vérifiez bien cet identifiant.',
      pay_address_network_mismatch: 'Cette adresse appartient à un réseau différent de celui que vous utilisez.',
      contacts_your_contacts: 'Vos contacts',
      contacts_other_people: 'Autres personnes',
      contacts_not_saved: 'Non enregistré',
      contacts_network_footer: 'Trouvé sur le réseau. Vous ne l’avez pas enregistré.',
      contact_no_handle: 'Pas encore de pseudo',
      contact_no_handle_registered: 'Aucun pseudo enregistré',
      contact_name_section: 'Votre nom pour cette personne',
      contact_name_hint: 'Vous seul le voyez. Cela ne change pas comment elle se nomme.',
      contact_new_name_hint: 'Vous seul le voyez. Enregistré sur cet appareil.',
      contact_copy_identifier: 'Copier l’identifiant',
      profile_handle_registered_hint: 'Enregistré sur votre identifiant. On peut vous trouver grâce à lui.',
      profile_handle_change: 'Modifier',
      profile_handle_claim: 'Prendre {{handle}}',
      profile_handle_replace_warning:
        'Prendre ce pseudo remplace {{handle}}. Ceux qui vous ont enregistré gardent leur contact ; l’ancien pseudo ne mène plus à vous.',
      profile_identifier_section: 'Votre identifiant',
      profile_show_qr: 'Afficher en QR',
      profile_show_qr_hint: 'Laissez quelqu’un vous scanner pour obtenir votre profil',
      profile_qr_callout:
        'Scanner ce code ouvre BSV Wallet et vous ajoute comme contact, avec votre nom, votre pseudo et votre photo. Il ne demande aucun paiement : ce code-là se trouve dans Recevoir.',
      pay_review_title: 'Vérification',
      pay_send_amount: 'Envoyer {{amount}}',
      pay_sent_to: 'à {{name}}',
      pay_save_as_contact: 'Enregistrer comme contact'
    }
  },
  ar: {
    translation: {
      token_balances_header: 'الأرصدة',
      wallet_coin_switcher: 'اختر العملة المعروضة',
      wallet_activity_empty_asset: 'لا يوجد نشاط {{ticker}} بعد',
      pay_direction_pay_asset: 'ادفع {{ticker}}',
      token_fee_footer: 'يتطلب الإرسال قليلاً من BSV لرسوم الشبكة. أما الاستلام فلا.',
      token_new_tap: 'جديد — اضغط لمعرفة جهة الإصدار',
      token_unresolved: 'رمز غير معروف',
      token_unresolved_sub: 'لم تتمكن المحفظة من معرفة جهة الإصدار. حاول مرة أخرى عند الاتصال بالإنترنت.',
      token_paused_short: 'التحويلات موقوفة',
      token_frozen_short: '{{amount}} مجمّد',
      token_sheet_issuer: 'جهة الإصدار',
      token_sheet_issuer_unknown: 'غير محدد',
      token_sheet_asset_id: 'معرّف الرمز',
      token_sheet_powers_header: 'ما الذي يمكن أن تفعله {{issuer}}',
      token_power_pause: 'إيقاف جميع تحويلات {{ticker}}',
      token_power_freeze: 'تجميد عملات محددة، بما فيها عملاتك',
      token_power_admit: 'تحديد من يجوز له حيازة {{ticker}}',
      token_power_replace: 'استبدال عملة مجمّدة بأخرى جديدة',
      token_powers_limit: 'لا يمكنهم إخراج {{ticker}} من محفظتك بدونك، والعملة المجمّدة ببساطة لا يمكن إنفاقها.',
      token_frozen_sub: 'جمّدت {{issuer}} مبلغ {{amount}} {{ticker}} من رصيدك. تواصل معهم للاعتراض.',
      pay_asset_label: 'الدفع بـ',
      pay_asset_label_get: 'الاستلام بـ',
      pay_asset_cta: 'إرسال {{amount}} {{ticker}}',
      pay_asset_over_balance: 'أكبر من رصيدك من {{ticker}}.',
      pay_asset_needs_bsv: 'يتطلب إرسال {{ticker}} قليلاً من BSV لرسوم الشبكة. أما الاستلام فلا.',
      pay_asset_get_bsv: 'احصل على BSV',
      pay_asset_needs_bsv_short: 'يلزم BSV لدفع الرسوم',
      pay_asset_frozen_note: '{{amount}} {{ticker}} من رصيدك مجمّد ولا يمكن إرساله.',
      pay_asset_paused: 'أوقفت {{issuer}} تحويلات {{ticker}}، لذا لا يمكن الإرسال الآن.',
      pay_asset_no_address: 'لا يمكن إرسال {{ticker}} إلا إلى شخص أو جهاز قريب — وليس إلى عنوان بيتكوين.',
      pay_asset_address_status: 'غير متاح لـ {{ticker}}',
      pay_asset_self_unregistered: 'لم تسجّلك {{issuer}} لإرسال {{ticker}}. لا يزال بإمكانك استلامه.',
      pay_asset_unidentified: 'لم تتمكن المحفظة من التعرف على هذا الرمز، لذا لا يمكنها إرساله بعد.',
      pay_asset_recipient_unregistered: 'لم تسجّل {{issuer}} هذا الشخص لـ {{ticker}} بعد، لذا لا يمكن الدفع له به.',
      pay_asset_recipient_blocked: 'لا تسمح {{issuer}} بالدفع لهذا الشخص.',
      token_err_refused: 'رفضت {{issuer}} هذا التحويل. لم يُرسل شيء ورصيدك من {{ticker}} لم يتغير.',
      token_err_refused_paused: 'أوقفت {{issuer}} رمز {{ticker}}. لم يُرسل شيء ورصيدك لم يتغير.',
      token_err_refused_recipient: 'لا تسمح {{issuer}} بالدفع لهذا الشخص. لم يُرسل شيء ورصيدك لم يتغير.',
      token_err_refused_frozen: 'جرى للتو تجميد جزء من {{ticker}} الذي كنت ترسله. لم يُرسل شيء؛ وتم تحديث رصيدك.',
      token_err_send_failed: 'تعذّر إرسال هذه الدفعة: {{reason}}. لم يغادر محفظتك أي شيء.',
      token_err_reason_unknown: 'مشكلة غير معروفة',
      token_err_unreachable:
        'تعذّر الوصول إلى {{issuer}} لتأكيد هذا التحويل. تحقق من رصيدك من {{ticker}} قبل المحاولة مجددًا.',
      token_err_check_again: 'تحقق مرة أخرى',
      token_err_balance_changed: 'تغيّر رصيدك من {{ticker}} أثناء التحضير. تحقق من المبلغ وحاول مجددًا.',
      token_err_busy: 'هناك دفعة {{ticker}} قيد الإرسال بالفعل.',
      pay_sent_not_broadcast: 'تم الإرسال. لم يصل إلى الشبكة بعد — سنواصل المحاولة.',
      pay_sent_not_notified: 'تم الإرسال. لم نتمكن من إبلاغهم بعد — سيرونه عند فحص محفظتهم في المرة القادمة.',
      pay_sent_handed_to_wallet: 'تم الإرسال. لم يستلموه شخصيًا، لذا أرسلناه إلى محفظتهم بدلًا من ذلك.',
      token_first_hold: 'يصدر {{ticker}} عن {{issuer}}، ويمكنها إيقاف التحويلات وتجميد العملات.',
      pay_asset_request_paused: 'أوقفت {{issuer}} رمز {{ticker}}، لذا قد لا تصل أي دفعة الآن.',
      token_admit_title: 'لا يمكنك استلام {{ticker}} بعد',
      token_admit_body:
        'لا ترسل {{issuer}} رمز {{ticker}} إلا إلى الأشخاص المسجلين لديها. وحتى ذلك الحين، تُرفض المدفوعات إليك قبل أن تغادر محفظة المرسل.',
      token_admit_bsv_instead: 'استلم بـ BSV بدلاً من ذلك',
      token_row_sent: 'تم إرسال {{ticker}}',
      token_row_received: 'تم استلام {{ticker}}',
      token_row_amount_pending: 'المبلغ غير متاح',
      token_evicted_title: 'تم سحب جزء من {{ticker}}',
      token_evicted_body: 'سحبت {{issuer}} مبلغ {{amount}} {{ticker}} من محفظتك.',
      local_pay_token_not_cleared: 'تم الاستلام دون اتصال · لم تؤكده {{issuer}} بعد',
      pay_asset_nearby_needs_network: 'يتطلب الدفع بـ {{ticker}} شخصيًا وجود اتصال. يمكنك الدفع بـ BSV بدونه.',
      pay_asset_link_not_held: 'هذا الطلب يخص رمزًا لا تحتفظ به هذه المحفظة. لا يزال بإمكانك الدفع لهم بـ BSV.',
      pay_asset_nearby_wrong_overlay: 'هذا الطلب يخص رمزًا غير مهيأة له هذه المحفظة.',
      token_messagebox_mismatch: 'محفظتك مضبوطة على صندوق رسائل مختلف، لذا قد لا تصلك مدفوعات الرموز.',
      token_issuer_fallback: 'جهة الإصدار',
      token_row_hint: 'يفتح جهة الإصدار وما يمكنها فعله',
      token_attention_one: 'بانتظار تأكيد {{issuer}} — اضغط للتفاصيل',
      token_attention_many: '{{count}} من مدفوعات الرموز بانتظار التسوية — اضغط للتفاصيل',
      token_recv_reversed: 'تعذّر تأكيد هذه الدفعة من قبل {{issuer}} وتم عكسها.',
      token_settle_paused_retry: 'أوقفت {{issuer}} رمز {{ticker}} حاليًا — سنواصل المحاولة.',
      token_settle_unreachable_retry: 'تعذّر الوصول إلى {{issuer}} لتأكيد هذا التحويل. سنواصل المحاولة.',
      token_settle_evicted: 'لم تصل تلك الدفعة إلى الشبكة في الوقت المناسب وتم عكسها — عاد رصيدك من {{ticker}}.',
      token_frame_unreadable: 'تعذّر علينا قراءة الدفعة التي استلمتها — اطلب من المرسل إعادة إرسالها.',
      token_chain_reversed: 'رفضت {{issuer}} دفعة سابقة في هذه السلسلة، لذا تم عكس هذه الدفعة أيضًا.',
      token_settle_stuck_bound: 'تنتظر هذه الدفعة التسوية منذ أكثر من 3 أيام. يمكنك الانتظار أو التواصل مع {{issuer}}.',
      token_cover_failed: 'لا تتطابق هذه الدفعة مع سجلات {{issuer}} ولم يتم قبولها.',
      token_sent_settling: 'تم الإرسال. جارٍ التسوية مع {{issuer}} — سترى التأكيد عند إعادة الاتصال أنت أو {{payee}}.',
      token_sent_settling_unnamed: 'تم الإرسال. جارٍ التسوية مع {{issuer}} — سترى التأكيد عند إعادة اتصال أحدكما.',
      token_sent_settled: 'تم الإرسال · وتمت التسوية مع {{issuer}}',
      token_status_settling: 'قيد التسوية',
      token_status_settled: 'تمت التسوية',
      token_status_refused: 'مرفوض',
      token_status_reversed: 'تم العكس',
      token_status_stuck: 'بانتظار التسوية',
      new_tab: 'علامة تبويب جديدة',
      untitled: 'بدون عنوان',
      share: 'مشاركة',
      back_to_homepage: 'العودة إلى الصفحة الرئيسية',
      bookmarks: 'الإشارات المرجعية',
      add_bookmark: 'إضافة إشارة مرجعية',
      bookmark: 'إشارة مرجعية',
      unbookmark: 'إزالة الإشارة المرجعية',
      find_in_page: 'البحث في الصفحة',
      find_in_page_placeholder: 'البحث في الصفحة...',
      find_in_page_matches: '{{current}} من {{total}}',
      find_in_page_no_matches: 'لا توجد نتائج',
      history: 'السجل',
      clear_all: 'مسح الكل',
      clear_all_tabs: 'مسح كل علامات التبويب',
      search_or_enter_website: 'البحث أو إدخال موقع الويب',
      browser: 'المتصفح',
      bug_report: 'تقرير خطأ',
      enable_web3: 'تفعيل Web3',
      tabs: 'علامات التبويب',
      unlock_web3_features: 'فتح ميزات Web3',

      cancel: 'إلغاء',
      done: 'تم',
      discard: 'تجاهل',
      deny: 'رفض',
      allow: 'السماح',
      save: 'حفظ',
      remove: 'إزالة',
      delete: 'حذف',
      search: 'بحث',
      loading: '...جار التحميل',
      continue: 'استمرار',
      go_back: 'العودة',
      continue_action: 'متابعة',
      camera_denied_description: 'تم رفض الوصول إلى الكاميرا. يمكنك تمكينه في الإعدادات لاستخدام هذه الميزة.',
      open_settings: 'فتح الإعدادات',
      copied: 'تم النسخ',
      explorer_open_failed: 'تعذر فتح مستكشف الكتل',
      tx_action_refresh: 'تحديث الحالة',
      tx_action_refresh_short: 'الحالة',
      tx_action_abort: 'إلغاء المعاملة',
      copy: 'نسخ',
      copy_to_clipboard: 'نسخ إلى الحافظة',
      details: 'التفاصيل',
      hide_details: 'إخفاء التفاصيل',

      identity: 'الهوية',
      settings: 'الإعدادات',
      wallet: 'المحفظة',
      activity: 'النشاط',
      configuration: 'التكوين',
      data_and_security: 'البيانات والأمان',
      storage_row: 'التخزين',
      storage_title: 'تخزين المحفظة',
      storage_db_size: 'قاعدة البيانات: {{size}}',
      storage_free_space: 'المساحة الحرة: {{size}}',
      storage_reclaimable: 'القابل للاستعادة الآن: {{size}} في {{count}} معاملة',
      storage_held_back: 'تم الإبقاء على {{count}} معاملة لأنها ما زالت مستخدمة',
      storage_reclaim_explain:
        'الاستعادة تحذف بيانات تحقق لم تعد المحفظة بحاجة إليها. معاملاتك وأرصدتك وسجلك لا تتأثر. هذا يفرّغ مساحة داخل ملف قاعدة البيانات؛ الملف نفسه لا يصغر حتى يضغطه النظام.',
      storage_reclaim_confirm: 'استعادة',
      storage_reclaimed: 'تمت استعادة {{count}} معاملة',
      storage_failed: 'تعذّر قراءة استخدام التخزين.',
      storage_unavailable: 'التخزين غير جاهز بعد.',
      backup_push_section: 'نسخة احتياطية خاصة',
      backup_push_toggle: 'النسخ الاحتياطي إلى الخادم',
      backup_erase_row: 'حذف النسخة من الخادم',
      backup_erase_title: 'حذف نسختك الاحتياطية؟',
      backup_erase_message:
        'كل ما أرسلته هذه المحفظة إلى خادم النسخ الاحتياطي يُحذف نهائياً، ولجميع الأجهزة. كما يتم إيقاف النسخ الاحتياطي. عملاتك ومفاتيحك وعبارة الاسترداد لا تتأثر — لكن الجهاز الجديد لن يستطيع استعادة سجل هذه المحفظة، ولن تكون بعض عملاتك الحالية قابلة للإنفاق هناك.',
      backup_erase_confirm: 'حذف',
      backup_erase_done: 'تم حذف النسخة — أُزيل {{count}} عنصراً',
      backup_erase_failed: 'تعذّر حذف النسخة. لم يُحذف شيء — يُرجى المحاولة مرة أخرى.',
      backup_erase_no_key: 'لا يوجد مفتاح محفظة متاح',
      backup_push_disclosure:
        'تُرسل نسخة مشفّرة من قاعدة بيانات محفظتك إلى خادم تابع لجمعية BSV لتتمكن من استعادتها على جهاز جديد. وهي مشفّرة بمفتاح لا تشتقّه إلا عبارة الاسترداد الخاصة بك — لا يستطيع المشغّل قراءتها. ما زلت بحاجة إلى عبارة الاسترداد أو الأجزاء المطبوعة: بدونها لا يمكن لأحد فتح النسخة، ولا نحن.',
      backup_push_off_title: 'إيقاف النسخة الاحتياطية الخاصة؟',
      backup_push_off_message:
        'عبارة الاسترداد وحدها لا تستطيع إعادة بناء هذه المحفظة. المعلومات المرتبطة بتأمين عملاتك موجودة في قاعدة البيانات فقط، لذا مع إيقاف النسخ يمكن لجهاز جديد استعادة عبارتك لكن ليس أموالك. لا يُحذف شيء موجود بالفعل على الخادم.',
      backup_push_off_confirm: 'إيقاف',
      backup_push_on_toast: 'النسخ الاحتياطي مفعّل — جارٍ اللحاق',
      backup_push_off_toast: 'النسخ الاحتياطي متوقف — لن يُرسل شيء بعد الآن',
      permissions: 'الأذونات',
      trust_network: 'شبكة الثقة',
      // vault (machine-translated, needs native review)
      vault_title: 'الخزنة',
      vault_row_title: 'الخزنة',
      vault_hero_title: 'أصول محمية',
      vault_hero_body: 'تخزين طويل الأمد مقفل بمفاتيح YubiKey الخاصة بك. يحتاج إلى مفتاحَي YubiKey 5 NFC أو أكثر.',
      vault_unsupported_title: 'يتطلب YubiKey',
      vault_unsupported_body: 'مفتاحا YubiKey 5 NFC أو أكثر، وهاتف قادر على قراءتها.',
      vault_restore_from_chain: 'استعادة الخزنة من البلوكتشين',
      vault_restore_scanning: 'جارٍ فحص البلوكتشين…',
      vault_restore_found: 'تم العثور على {{count}} إيداع (إيداعات) للخزنة على البلوكتشين.',
      vault_restore_pending: 'تم العثور على {{count}} إيداع (إيداعات)، بانتظار أول تأكيد لها — أعد المحاولة قريبًا.',
      vault_restore_none_found: 'لم يتم العثور على أي إيداعات للخزنة على البلوكتشين لهذه المحفظة.',
      vault_balance_label: 'الخزنة تحتوي على',
      vault_key_section: 'مفاتيح الأمان ({{count}} من 5)',
      vault_manage_section: 'إدارة',
      vault_deposit_cta: 'إيداع',
      vault_withdraw_cta: 'سحب',
      vault_deposit_title: 'إيداع في الخزنة',
      vault_withdraw_title: 'سحب من الخزنة',
      vault_deposit_sub: 'من رصيدك اليومي. لا حاجة إلى YubiKey.',
      vault_withdraw_sub: 'عودة إلى رصيدك اليومي. المس أي مفتاح من مفاتيح خزنتك.',
      vault_deposit_done: 'تم النقل إلى الخزنة',
      vault_reclaim_done: 'تم استرداد الأموال من إيداع خزنة متقطع',
      vault_withdraw_done: 'تم السحب من الخزنة',
      vault_withdraw_partial: 'بقي {{count}} إيداعًا — اسحب مجددًا لنقلها.',
      vault_withdraw_reason: 'ضع مفتاح YubiKey هنا للتوقيع — سحب {{amount}} sats من الخزنة',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: 'غير متاح بعد — إيداعات الخزنة معطّلة في هذا الإصدار.',
      vault_not_on_mainnet_body: 'الخزنة متاحة على الشبكة الرئيسية فقط. أعِد المحفظة إلى الشبكة الرئيسية لاستخدامها.',
      vault_intro_title: 'إعداد خزنتك',
      vault_intro_what:
        'تقفل الخزنة الأموال بمفاتيح YubiKey الخاصة بك. لا يوجد أي جزء من مفتاح الخزنة على هذا الهاتف أبدًا؛ وأي مفتاح من مفاتيحك يفتحها.',
      vault_intro_two_keys: 'تحتاج إلى مفتاحَي YubiKey على الأقل.',
      vault_intro_apart: 'احتفظ بها في أماكن مختلفة — مفتاحان محفوظان معًا هما مفتاح واحد.',
      vault_intro_backup: 'أبقِ النسخة الاحتياطية المشفّرة للمحفظة مفعّلة — فهي تحفظ سجل الاسترداد الموثّق.',
      vault_intro_ack:
        'أفهم: مفاتيح YubiKey الخاصة بي فقط تفتح هذه الخزنة. عبارة الاسترداد لا تفتحها. إذا فقدتها كلها، تضيع الأموال.',
      vault_intro_piv_ack:
        'أؤكد أن تطبيق PIV بالكامل في مفتاح YubiKey هذا أُعيد إلى إعدادات المصنع أو مخصص للخزنة. يغيّر الإعداد رمز PIN ورمز PUK ومفتاح الإدارة العام لتطبيق PIV، وقد يؤثر في جميع خانات PIV، وسأحفظ رمز PUK الجديد بأمان.',
      vault_intro_begin: 'ابدأ',
      vault_key_step_title: 'المفتاح {{k}} من 5 كحدّ أقصى',
      vault_key_step_replace:
        'استخدم YubiKey مُعاد ضبطه للمصنع ومخصصًا للخزنة. إذا كانت خانة الخزنة تحتوي على مفتاح قديم، فسينبّهك الإعداد قبل استبداله.',
      vault_pin_choose_title: 'اختر رمز PIN',
      vault_pin_choose_sub: 'ستُدخله في كل مرة تستخدم فيها هذا المفتاح. من 6 إلى 8 أرقام.',
      vault_pin_confirm_label: 'أدخله مرة أخرى',
      vault_pin_mismatch: 'الرمزان غير متطابقين.',
      vault_pin_not_default: 'اختر رمزًا غير 123456.',
      vault_puk_title: 'رمز الاسترداد الخاص بك',
      vault_puk_body:
        'دوّنه واحتفظ به في مكان آمن. إنه الطريقة الوحيدة لفتح هذا المفتاح إذا نسيت رمز PIN، ولن يُعرض مرة أخرى.',
      vault_puk_ack: 'لقد دوّنته',
      vault_reset_offer: 'إعادة ضبط هذا المفتاح',
      vault_reset_title: 'إعادة ضبط هذا المفتاح؟',
      vault_reset_body:
        'استُخدم هذا المفتاح من قبل. إعادة الضبط تمحو كل شهادة ومفتاح في تطبيق PIV الخاص به، وليس خانة الخزنة وحدها. كل ما يعتمد على هذا المفتاح سيتوقف عن العمل.',
      vault_reset_ack: 'أفهم أن هذا يمحو كل شيء على هذا المفتاح',
      vault_reset_unknown_ack:
        'لا يمكن لهذا الجهاز التأكد مما إذا كان هذا المفتاح لا يزال يؤمّن خزنة ملف شخصي آخر. أفهم أن محوه قد يدمّر ذلك المفتاح نهائيًا.',
      vault_reset_own_draft_ack:
        'يحمل هذا المفتاح المفتاح غير المكتمل من محاولتك السابقة، ولا يزال هذا الجهاز يحتفظ بسجله. أفهم أن إعادة الضبط تمحوه وأن تلك المحاولة لن يمكن استئنافها.',
      vault_reset_nothing_erased: 'لم يُمحَ شيء. خانة الخزنة في هذا المفتاح ليست فارغة، لذا بقي أمر آخر لتأكيده.',
      vault_reset_confirm: 'محو وإعادة ضبط',
      vault_resetting: 'جارٍ إعادة ضبط المفتاح…',
      vault_reset_done: 'أُعيد ضبط المفتاح. قرّبه مرة أخرى للمتابعة.',
      vault_reset_uncertain: 'لم تكتمل إعادة الضبط. قد يكون هذا المفتاح قد مُحي بالفعل — تحقّق منه قبل الاعتماد عليه.',
      vault_err_reset_enrolled: 'هذا المفتاح جزء من خزنتك بالفعل — إعادة ضبطه ستدمّر أحد مفاتيح خزنتك.',
      vault_err_reset_unreadable:
        'تعذّرت قراءة سجلات خزنتك على هذا الجهاز، لذا لا سبيل لإثبات أن هذا المفتاح ليس أحد مفاتيح خزنتك. لم يُمحَ شيء، وقد رُفضت إعادة الضبط.',
      vault_enrollment_resume_pin_hint:
        'استئناف مفتاح محفوظ يتطلّب رمز PIN الذي اخترته له سابقًا، لا رمزًا جديدًا. ثلاث محاولات خاطئة تقفل المفتاح.',
      vault_setup_step: 'الخطوة {{n}} من {{total}}',
      vault_enrollment_resume: 'متابعة المفتاح المحفوظ',
      vault_enrollment_reset_required:
        'لم يعد مفتاح YubiKey هذا في حالة المصنع، لذا لا يمكن تسجيله كما هو. يجب مسح تطبيق PIV الخاص به وإعادته إلى حالة المصنع قبل أن يتمكن من حفظ مفتاح الخزنة.',
      vault_enrollment_state_uncertain:
        'توقفت محاولة إعداد في منتصفها، لذا لا سبيل للتأكد من الحالة التي تُرك عليها مفتاح YubiKey. لن يُستخدم هذا المفتاح مرة أخرى حتى يُمسح تطبيق PIV الخاص به ويعود إلى حالة المصنع.',
      vault_nfc_enroll_message: 'ضع مفتاح YubiKey هنا لإعداده',
      vault_nfc_adopt_message: 'ضع مفتاح YubiKey المستعاد هنا للتحقق منه',
      vault_nfc_sign_batch: 'ضع مفتاح YubiKey هنا للتوقيع — الدفعة {{b}} من {{n}}',
      vault_key_use_different: 'استخدام مفتاح YubiKey آخر',
      vault_key_setup_again: 'إعداده من جديد',
      vault_name_default: 'المفتاح {{k}}',
      vault_more_title: 'إضافة مفتاح آخر؟',
      vault_more_body: 'حتى 5 مفاتيح. أي واحد منها يفتح الخزنة كاملة بمفرده.',
      vault_more_add: 'إضافة مفتاح آخر',
      vault_more_finish: 'إنهاء',
      vault_more_need_two: 'أضف مفتاحًا ثانيًا قبل الإنهاء — مفتاح واحد يعني عدم وجود استرداد.',
      vault_leave_setup: 'مغادرة الإعداد',
      vault_leave_title: 'مغادرة الإعداد؟',
      vault_leave_body:
        'لن تُحفظ مفاتيح YubiKey ({{count}}) التي أعددتها بعد. تحتفظ بمفاتيحها، فيمكنك إضافتها مجددًا خلال دقيقة.',
      vault_leave_confirm: 'مغادرة',
      vault_leave_stay: 'البقاء',
      vault_done_body: '{{count}} مفاتيح يمكنها فتح هذه الخزنة. هذه المفاتيح فقط تفتحها — وليس عبارة الاسترداد.',
      vault_done_cta: 'تم',
      vault_add_key_row: 'إضافة مفتاح',
      vault_add_key_done: 'يمكن لـ {{nickname}} فتح الإيداعات التي تُجرى من الآن. أعد قفل الخزنة ليتمكن من فتح كل شيء.',
      vault_key_added_toast: 'تمت إضافة المفتاح',
      vault_recovery_verified: 'تم التحقق من المفتاح المستعاد على هذا الجهاز',
      vault_relock_row: 'إعادة قفل الخزنة',
      vault_relock_now: 'إعادة القفل الآن',
      vault_relock_choose: 'أي مفتاح ستلمس لإعادة القفل؟',
      vault_relock_reason: 'المس أحد مفاتيحك الحالية ({{names}}) — وليس الذي أضفته للتو.',
      vault_relock_reason_generic: 'ضع مفتاح YubiKey هنا للتوقيع — إعادة قفل الخزنة على مفاتيحك الحالية',
      vault_relock_reason_remaining: 'أعد القفل على مفاتيحك المتبقية فقط: {{names}}.',
      vault_key_pending_removal: 'تمت الإزالة — أعد القفل لإتمامها',
      vault_key_removal_awaiting_network: 'جارٍ الإزالة — بانتظار قبول الشبكة لإعادة القفل',
      vault_removal_awaiting_network: 'أُرسلت إعادة القفل. يُتاح الإيداع والسحب مجددًا بعد أن تقبلها الشبكة.',
      vault_err_too_many_active_keys: 'تُقفل الخزنة بخمسة مفاتيح كحدّ أقصى. أزل مفتاحًا قبل الإيداع أو إعادة القفل أو سحب جزء من الرصيد.',
      vault_key_section_over_limit: 'مفاتيح الأمان ({{count}}، أزل واحدًا)',
      vault_add_key_done_over_limit: 'أُضيف {{nickname}}. تُقفل الخزنة بخمسة مفاتيح كحدّ أقصى، لذا أزل المفتاح الذي يحلّ هذا محلّه، ثم أعد القفل مرة واحدة.',
      vault_relock_done: 'أُعيد قفل الخزنة على مفاتيحك الحالية',
      vault_relock_capped: 'بقي {{count}} إيداعًا — أعد القفل مجددًا لنقلها.',
      vault_relock_unreachable: '{{count}} إيداعًا لا يفتحها سوى {{names}}. أعد القفل مجددًا بأحد تلك المفاتيح.',
      vault_badge_missing: '{{count}} إيداعًا غير مفتوح بعد لـ {{nickname}}',
      vault_badge_removed: '{{count}} إيداعًا ما زال مفتوحًا لمفتاح تمت إزالته',
      vault_key_action_rename: 'إعادة التسمية',
      vault_key_action_remove: 'إزالة',
      vault_rename_title: 'إعادة تسمية {{nickname}}',
      vault_rename_save: 'حفظ',
      vault_remove_title: 'إزالة {{nickname}}؟',
      vault_remove_body:
        'ستتوقف المحفظة عن استخدام {{nickname}}. تبقى الأموال الموجودة في الخزنة قابلة للفتح به حتى تعيد القفل (≈ {{fee}} ساتوشي).',
      vault_remove_only: 'إزالة فقط',
      vault_key_removed_toast: 'تمت إزالة المفتاح',
      vault_export_explainer:
        'كل إيداع في الخزنة يُخفي قيمة ملحه إلى أن يُصرف — لا يظهر أي شيء عنه في نص القفل. أبقِ النسخة الاحتياطية المشفّرة مفعّلة كي يتمكن جهاز جديد من استعادة سجلات المفاتيح وتوثيقها لاستخدام مفاتيح YubiKey.',
      vault_footnote: 'هذه المفاتيح فقط تفتح الخزنة. عبارة الاسترداد لا تفتحها.',
      vault_floor_line:
        'الحدّ الأدنى للإيداع {{floorDisplay}} ({{floorSats}} ساتوشي). يكلّف إنشاء إيداع في الخزنة نحو {{feeDisplay}}.',
      vault_first_deposit_title: 'أول إيداع في الخزنة',
      vault_first_deposit_body:
        'أول إيداع في الخزنة — لن يمكن فتح {{amount}} إلا بـ {{count}} مفاتيح YubiKey ({{names}}). عبارة الاسترداد لن تفيد.',
      vault_backup_off_title: 'فعّل النسخة الاحتياطية الخاصة',
      vault_backup_off_body:
        'يجب تفعيل النسخة الاحتياطية الخاصة قبل أن تنشئ المحفظة مخرج خزنة جديدًا. فهي تحفظ سجلات YubiKey الموثّقة وسجل المحفظة اللازمين للاسترداد. ولا يزال بإمكانك سحب كل شيء من دون إنشاء باقي في الخزنة.',
      vault_backup_off_cta: 'فتح إعدادات النسخ الاحتياطي',
      vault_seed_not_preserved_title: 'احفظ عبارة الاسترداد أولاً',
      vault_seed_not_preserved_body:
        'لا يمكن استرداد خزنتك إلا باستخدام عبارة الاسترداد الخاصة بك مع إحدى مفاتيح YubiKey. لا تحتفظ مفاتيح YubiKey بأي نسخة منها — إذا فُقدت قبل أن تدوّنها، فلن يمكن استرداد هذه الأموال. احفظها الآن قبل الإيداع.',
      vault_seed_not_preserved_cta: 'حفظ عبارة الاسترداد',
      vault_choose_key: 'أي مفتاح ستلمس؟',
      vault_remainder_title: 'سحب كل شيء؟',
      vault_remainder_body:
        'سحب {{amount}} يترك {{remainder}}، وهو أقل من الحدّ الأدنى للخزنة 100,000 ساتوشي. ستنتقل الخزنة كاملة إلى رصيدك اليومي.',
      vault_remainder_all: 'سحب كل شيء',
      vault_remainder_change: 'تغيير المبلغ',
      vault_unreachable_title: 'جزء من الخزنة يحتاج إلى مفتاح آخر',
      vault_unreachable_body:
        'جزء من الخزنة يحتاج إلى مفتاح آخر — نُقل {{moved}}. {{count}} إيداعًا بقيمة {{amount}} لا يفتحها سوى {{names}}. اسحب مجددًا بأحد تلك المفاتيح.',
      vault_sign_progress: 'تم توقيع {{signed}} من {{total}}',
      vault_err_pin_locked_enroll:
        'رمز PIN لهذا المفتاح محظور. أزل الحظر بـ PUK في Yubico Authenticator، أو أعدّ بمفتاح YubiKey آخر.',
      vault_err_serial_mismatch_chosen:
        'هذا هو {{tappedName}}. اخترت {{chosenName}} — المسه، أو ارجع واختر {{tappedName}}.',
      vault_err_key_already_enrolled: 'لقد أضفت هذا المفتاح من قبل ({{nickname}}). المس مفتاحًا آخر.',
      vault_err_not_released: 'إيداعات الخزنة معطّلة في هذا الإصدار.',
      vault_err_not_on_mainnet:
        'الخزنة متاحة على الشبكة الرئيسية فقط. حوِّل المحفظة إلى الشبكة الرئيسية ثم أعِد المحاولة.',
      vault_err_backup_off:
        'فعّل النسخة الاحتياطية الخاصة المشفّرة قبل إنشاء مخرج خزنة جديد. ولا يزال بإمكانك سحب كل شيء.',
      vault_err_not_enough_keys: 'هذه الخزنة لديها أقل من مفتاحين. أعد إعدادها.',
      vault_err_too_many_keys: 'تتّسع الخزنة لخمسة مفاتيح كحدّ أقصى.',
      vault_err_last_keys: 'تحتاج الخزنة إلى مفتاحين على الأقل. أضف مفتاحًا آخر قبل إزالة هذا.',
      vault_err_relock_required: 'أعد قفل الخزنة أولًا حتى تتمكن مفاتيحك الأخرى من فتح كل إيداع.',
      vault_err_action_pending: 'لم تنتهِ إحدى معاملات الخزنة بعد. أعد المحاولة بعد اكتمالها.',
      vault_resolve_held_deposit_action: 'إتمام الإيداع المتوقف',
      vault_resolve_held_deposit_done: 'تم حل مشكلة الإيداع',
      vault_resolve_held_deposit_failed: 'تعذّر حل مشكلة الإيداع. حاول مرة أخرى.',
      vault_resolve_held_deposit_none:
        'لا يوجد هنا ما يمكن حله - تحقق من علامة التبويب النشاط بحثًا عن معاملة معلقة لإعادة المحاولة أو الإلغاء.',
      vault_err_key_not_committed:
        'لا يمكن لـ {{nickname}} فتح أي من إيداعات هذه الخزنة. استخدم مفتاحًا آخر من مفاتيح خزنتك.',
      vault_err_key_cannot_cover:
        'يمكن لـ {{nickname}} فتح {{reachable}} من أصل {{total}} في الخزنة. اسحب حتى {{reachable}}، أو استخدم {{otherNames}} بدلًا منه.',
      vault_err_too_small_to_relock:
        'تحوي هذه الخزنة أقل من 100,000 ساتوشي، وهو أقل من أن يُعاد قفله. اسحبها بدلًا من ذلك ثم أودع مجددًا.',
      vault_err_bad_version: 'بُنيت المعاملة بتنسيق خاطئ. لم يُوقَّع شيء — حاول مجددًا.',
      vault_err_chain_scan_failed: 'تعذّر إكمال فحص سلسلة الكتل بحثًا عن إيداعات الخزنة. تحقق من اتصالك وحاول مجددًا.',
      vault_shares_unavailable: 'لا توجد مادة مفتاح متاحة للطباعة.',
      vault_enroll_begin: 'إعداد الخزنة',
      vault_enroll_phase_connecting: 'جارٍ قراءة مفتاحك…',
      vault_enroll_phase_pin_check: 'جارٍ التحقق من مفتاحك…',
      vault_enroll_phase_checking_slot: 'جارٍ فحص خانة الخزنة…',
      vault_enroll_phase_personalizing: 'جارٍ ضبط رمز PIN ورمز الاسترداد…',
      vault_enroll_phase_generating: 'جارٍ إنشاء مفتاح الخزنة — المس المفتاح عندما يومض',
      vault_enroll_phase_challenging: 'جارٍ التحقق من مفتاح الخزنة الجديد — المس المفتاح عندما يومض',
      vault_enroll_phase_done: 'تم',
      vault_touch_when_blinks: 'المس نقطة التلامس الذهبية عندما يومض المفتاح.',
      vault_nfc_activation_hint:
        'هل هذه أول مرة تستخدم فيها مفتاح YubiKey هذا؟ وصّله أولاً بمنفذ USB-C لبضع ثوانٍ — المفاتيح الجديدة تُشحن وتقنية NFC معطّلة فيها حتى ذلك الحين.',
      vault_continue: 'متابعة',
      vault_back: 'رجوع',
      vault_enrolled_toast: 'الخزنة جاهزة',
      vault_insert_key: 'أدخل مفتاح YubiKey',
      vault_hold_key_nfc: 'قرّب YubiKey من أعلى هاتفك',
      vault_keep_holding_nfc: 'استمر في التقريب — جارٍ الفتح…',
      vault_unlocking_funds: 'جارٍ فتح أموالك',
      vault_unlocking_sub: 'يستغرق ذلك بضع ثوانٍ. أبقِ التطبيق مفتوحًا من فضلك.',
      vault_sending_to_network: 'جارٍ الإرسال إلى الشبكة',
      vault_sending_sub: 'إذا لم تمر العملية فورًا، فسنواصل المحاولة.',
      vault_reading_key: 'جارٍ قراءة المفتاح…',
      vault_enter_pin: 'أدخل رمز PIN لمفتاح YubiKey',
      vault_pin_sub_nfc: 'ثم قرّب مفتاحك من أعلى هاتفك لإتمام العملية.',
      vault_pin_sub_usb: 'ثم المس نقطة التلامس الذهبية لإتمام العملية.',
      vault_touch_contact: 'المس نقطة التلامس الذهبية',
      vault_unlock_cta: 'فتح',
      vault_pin_retries: 'المحاولات المتبقية: {{count}}',
      vault_locked: 'الخزنة مقفلة',
      vault_retry: 'إعادة المحاولة',
      vault_cancel: 'إلغاء',
      vault_dismiss: 'تجاهل',
      vault_err_wrong_key: 'هذا المفتاح ليس من مفاتيح هذه الخزنة.',
      vault_err_serial_mismatch: 'هذا المفتاح ليس من مفاتيح هذه الخزنة ({{names}}).',
      vault_err_touch_timeout: 'لم يتم اكتشاف لمسة. أعد المحاولة والمس نقطة التلامس الذهبية.',
      vault_err_pin_locked:
        'رمز PIN لهذا المفتاح محظور. استخدم مفتاحًا آخر من مفاتيح خزنتك، أو أزل حظره بـ PUK في Yubico Authenticator.',
      vault_err_pin_invalid: 'رمز PIN غير صحيح.',
      vault_err_puk_invalid: 'رمز PUK غير صحيح.',
      vault_err_puk_locked: 'رمز PUK لهذا المفتاح محظور. استخدم مفتاح YubiKey جديدًا.',
      vault_err_ceremony_active: 'لا تزال عملية خزنة أخرى تستخدم مفتاح YubiKey. انتظر حتى تنتهي ثم حاول مجددًا.',
      vault_err_scope_changed: 'تغيّرت المحفظة أو الشبكة أثناء هذه العملية. ابدأ مجددًا في المحفظة الحالية.',
      vault_err_key_removed_mid_op: 'تمت إزالة المفتاح. أدخله وأعد المحاولة.',
      vault_err_no_key: 'لم يتم اكتشاف أي YubiKey.',
      vault_err_driver_unavailable: 'دعم YubiKey غير متوفر على هذا الجهاز.',
      vault_err_mgmt_key_custom: 'يحتوي هذا المفتاح على مفتاح إدارة مخصص ولا يمكن استخدامه تلقائيًا.',
      vault_err_attestation_invalid:
        'تعذّر التحقق من شهادة الشركة المصنّعة لمفتاح YubiKey هذا. استخدم مفتاح YubiKey أصليًا موثّقًا من المصنع مع خانة توثيق PIV غير معدّلة.',
      vault_err_generic: 'حدث خطأ ما. يرجى إعادة المحاولة.',
      vault_err_template_invalid: 'حدث خطأ أثناء محاولة الوصول إلى الخزنة. يرجى المحاولة مرة أخرى.',
      vault_err_not_enrolled: 'لا توجد خزنة مُعدّة.',
      vault_err_pin_required: 'أدخل رمز PIN.',
      vault_err_unsupported_platform: 'دعم YubiKey غير متوفر على هذا الجهاز.',
      vault_err_slot_occupied: 'خانة المفتاح مستخدمة بالفعل.',
      vault_replace_key_warning:
        'استبدالها يدمّر نهائيًا مفتاح P-256 الخاص الحالي. قد تصبح أموال الخزنة التي تعتمد على ذلك المفتاح غير قابلة للإنفاق.',
      vault_replace_key_confirm: 'استبدال المفتاح الحالي',
      vault_resume_draft_confirm_title: 'هل تريد استخدام هذا المفتاح المُجهَّز؟',
      vault_resume_draft_confirm_body:
        'هناك مفتاح تم تجهيزه مسبقًا — {{nickname}} · …{{tail}} — جاهز للإضافة، لكن هذا الجهاز لم يتحقق للتو منه على بطاقة. تابع فقط إذا كنت تتعرف عليه.',
      vault_resume_draft_confirm_use: 'استخدام هذا المفتاح',
      vault_resume_draft_confirm_discard: 'تجاهله',
      vault_err_enrollment_partial:
        'توقف إعداد YubiKey بعد تغيير المفتاح. اتبع تعليمات إعادة المحاولة؛ لم تتم إضافة هذا المفتاح إلى الخزنة.',
      vault_err_key_not_adopted: 'تحقق من حيازتك لمفتاح YubiKey المستعاد على هذا الجهاز قبل استخدامه للسحب.',
      vault_err_too_many_inputs:
        'عدد إيداعات الخزنة كبير جداً للنقل مرة واحدة. اسحب مبلغاً أصغر أولاً — كل عملية سحب تدمج الخزنة أيضاً، فيصبح بالإمكان نقل مبلغ أكبر لاحقاً.',
      vault_err_requires_online:
        'تحويلات الخزنة تحتاج اتصالاً. أعد الاتصال وحاول مرة أخرى — أموال الخزنة لا تُدرج في قائمة انتظار للإرسال لاحقاً.',
      vault_err_user_cancelled: 'تم الإلغاء.',
      vault_err_vault_empty: 'لا توجد في الخزنة أموال للسحب بعد.',
      vault_err_amount_exceeds_balance: 'هذا أكثر مما تحتويه الخزنة.',
      vault_err_below_dust: 'هذا المبلغ أصغر من أن يُرسل.',
      vault_err_no_transaction: 'تعذّر إتمام التحويل. يرجى المحاولة مرة أخرى.',
      vault_err_nfc_lost: 'انقطع الاتصال. أمسك المفتاح ثابتًا على الهاتف.',
      vault_pin_invalid_retry: 'رمز PIN خاطئ — تبقّت {{count}} محاولات. اضغط سحب مرة أخرى للمحاولة من جديد.',
      vault_pin_invalid_retry_generic: 'رمز PIN خاطئ. اضغط سحب مرة أخرى للمحاولة من جديد.',
      vault_disable_row: 'تعطيل الخزنة',
      vault_disable_title: 'تعطيل الخزنة؟',
      vault_disable_message: 'هذا يمحو قائمة مفاتيح الخزنة من هذا الهاتف. تبقى المفاتيح على مفاتيح YubiKey الخاصة بك.',
      vault_disable_confirm: 'تعطيل',
      vault_disabled_toast: 'تم تعطيل الخزنة',
      vault_disable_blocked_title: 'اسحب الأموال أولًا',
      vault_disable_blocked_message:
        'لا تزال الخزنة تحتوي على أموال. اسحب كل شيء إلى رصيدك اليومي قبل التعطيل — وإلا ستبقى تلك الأموال مقفلة دون أي وسيلة لتوقيعها.',
      vault_ok: 'حسنًا',
      vault_mock_toggle: 'استخدام YubiKey وهمي (dev)',
      vault_on: 'تشغيل',
      vault_off: 'إيقاف',
      vault_shares_word_count: 'حصص الاسترداد تدعم عبارات من اثنتي عشرة كلمة فقط. اكتب عبارتك بدلًا من ذلك.',
      scan_shares_legacy_title: 'تنسيق نسخ احتياطي قديم',
      scan_shares_legacy_message:
        'هذه الحصص بتنسيق قديم يستعيد رصيدك اليومي لكنه لا يفتح خزنة. إن كانت عبارة الاسترداد المكوّنة من اثنتي عشرة كلمة ما زالت لديك، فاستعد منها بدلًا من ذلك — فهي تسترد كل شيء — ثم اطبع حصصًا جديدة بعدها. وإن ضاعت العبارة، فأرسل أموالك إلى محفظة جديدة وابدأ من جديد.',
      scan_shares_legacy_ack: 'فهمت',

      delete_wallet: 'حذف المحفظة',
      delete_wallet_warning_title: 'حذف المحفظة؟',
      delete_wallet_warning_body:
        'سيؤدي هذا إلى حذف مفاتيح محفظتك بشكل دائم من هذا الجهاز. إذا لم تقم بعمل نسخة احتياطية لعبارة الاسترداد أو طباعة حصص الاسترداد، فستفقد الوصول إلى محفظتك إلى الأبد.',
      delete_wallet_confirm: 'حذف المحفظة',
      delete_wallet_failed_title: 'فشل حذف المحفظة',
      delete_wallet_failed_body: 'تعذّر حذف محفظتك بالكامل من هذا الجهاز. قد تظل بعض بيانات المفتاح موجودة. يرجى المحاولة مرة أخرى.',
      bsv_network: 'شبكة BSV',
      arc_endpoint: 'نقطة نهاية ARC',
      arc_api_token: 'رمز API',
      arc_custom_url: 'رابط مخصص',
      arc_apply: 'تطبيق',
      arc_reset_default: 'إعادة التعيين إلى الافتراضي',
      arc_requires_token: 'يتطلب رمزًا',
      arc_url_https_required: 'يجب أن يكون https (أو http://localhost للاختبار المحلي)',
      recovery_phrase: 'مفتاح الاسترداد',
      you_have: 'لديك',
      switching: '...جار التحويل',
      mainnet: 'الشبكة الرئيسية',
      testnet: 'شبكة الاختبار',
      teratest: 'Teratest',
      identity_key: 'المعرّف',
      transactions: 'المعاملات',

      no_transactions: 'لا توجد معاملات بعد',
      tx_status_confirmed: 'مؤكد',
      tx_status_received: 'تم الاستلام',
      tx_status_sent: 'تم الإرسال',
      tx_status_spent: 'تم الإنفاق',
      tx_status_transferred: 'تم التحويل',
      tx_status_accepted: 'مقبولة',
      tx_status_broadcasting: 'جارٍ المعالجة',
      tx_status_not_sent: 'لم يتم الإرسال',
      tx_status_unsigned: 'مسودة',
      tx_status_nonfinal: 'قيد الانتظار',
      tx_status_pending: 'قيد الانتظار',
      tx_status_failed: 'فشل',
      tx_status_offline_queued: 'دون اتصال · في قائمة الانتظار',
      tx_status_offline_sending: 'دون اتصال · جارٍ الإرسال',
      tx_status_offline_rejected: 'دون اتصال · مرفوض',
      tx_copied: 'تم نسخ المعاملة',
      tx_not_available: 'المعاملة غير متوفرة',
      tx_copy_failed: 'فشل نسخ المعاملة',
      tx_abort_success: 'تم إلغاء المعاملة',
      tx_abort_failed: 'فشل إلغاء المعاملة',
      tx_abort_maybe_delivered: 'لا يمكن الإلغاء — قد تكون هذه الدفعة قد وصلت بالفعل إلى المستلم',
      tx_proof_refreshed: 'تم تأكيد المعاملة',
      tx_proof_refresh_failed: 'تعذّر جلب الإثبات — قد لا تكون المعاملة قد تأكدت بعد',
      tx_marked_failed: 'لم تُعالج — وُسمت كفاشلة وأُفرج عن الأموال',
      tx_still_pending: 'لا تزال معلّقة — لم تُؤكَّد بعد',
      tx_action_explorer: 'عرض في مستكشف الكتل',
      tx_export_success: 'تم تصدير {{count}} معاملة',
      tx_export_failed: 'فشل تصدير المعاملات',
      tx_export_csv: 'تصدير CSV',
      tx_export: 'تصدير',
      activity_filter: 'تصفية النشاط',
      activity_filter_all: 'الكل',
      activity_search_placeholder: 'ابحث في الملاحظات والأسماء و@أسماء المستخدمين',
      activity_search_clear: 'مسح البحث',
      activity_filter_no_match: 'لا يوجد نشاط مطابق',
      wallet_balance_label: 'الرصيد',
      wallet_balance_you_have: 'لديك',
      wallet_balance_refresh: 'تحديث الرصيد',
      wallet_group_today: 'اليوم',
      wallet_group_yesterday: 'أمس',
      back: 'رجوع',
      wallet_payments: 'المدفوعات',
      wallet_vault: 'الخزنة',
      wallet_settings: 'الإعدادات',
      wallet_activity: 'النشاط',

      message_box_server: 'خادم صندوق الرسائل',
      message_box_required: 'مطلوب لإرسال واستقبال المدفوعات القائمة على الهوية.',
      send_payment: 'إرسال دفعة',
      recipient: 'المستلم',
      amount: 'المبلغ',
      enter_valid_amount: 'أدخل مبلغًا أكبر من صفر',
      display_currency: 'عملة العرض',
      no_pending_payments: 'لا توجد مدفوعات معلقة',
      outgoing_payments: 'صادرة',
      payment_not_delivered: 'لم يتم التسليم',
      payment_delivered: 'تم التسليم',
      retry: 'إعادة المحاولة',
      retry_failed: 'فشلت إعادة المحاولة',
      dismiss: 'رفض',
      valid_identity_key: 'تم إدخال معرّف صالح',
      valid_bsv_address: 'تم إدخال عنوان BSV صالح',
      searching: '...جار البحث',
      unknown: 'غير معروف',
      from_prefix: ':من',
      message_box_saved: 'تم حفظ خادم صندوق الرسائل',
      message_box_removed: 'تم إزالة خادم صندوق الرسائل',
      message_box_off: 'متوقف',
      message_box_off_hint: 'خادم صندوق الرسائل متوقف. فعّله من الإعدادات › متقدم.',
      enter_valid_url: 'يرجى إدخال عنوان URL صالح',
      checking_connection: 'جارٍ التحقق من الاتصال...',
      connection_failed: 'فشل الاتصال',
      recipient_placeholder: 'الاسم أو المعرّف أو عنوان BSV...',

      trust_updated: 'تم تحديث شبكة الثقة',
      failed_to_save: 'فشل الحفظ',
      confirm_delete: 'تأكيد الحذف',
      confirm_delete_body: 'هل أنت متأكد أنك تريد حذف هذا المُصادق؟',
      order_certifiers_hint: 'رتب المُصادقين حسب الأولوية. ستعرض التطبيقات المُصادقين ذوي الترتيب الأعلى أولاً.',
      certifiers: 'المُصادقون',
      no_certifiers: 'لا توجد مُصادقون بعد.',
      add_provider: 'إضافة مزود',
      unsaved_changes: 'لديك تغييرات غير محفوظة',
      duplicate_key_error: 'الكيان الذي يحمل هذا المفتاح العام موجود بالفعل في القائمة!',
      get_provider_details: 'الحصول على تفاصيل المزود',
      validate_details: 'التحقق من التفاصيل',
      show_advanced: 'إظهار الخيارات المتقدمة',
      hide_advanced: 'إخفاء الخيارات المتقدمة',
      description_length_error: 'يجب أن يكون الوصف بين 5 و50 حرفًا',
      enter_domain_hint: 'أدخل نطاق المزود',
      enter_details_hint: 'أدخل تفاصيل المزود',

      receive: 'استقبال',
      send: 'إرسال',
      generating_address: '...جارٍ إنشاء عنوان BSV',
      imported: 'مستورد',
      unable_to_generate_address: 'تعذر إنشاء عنوان BSV',
      send_bsv: 'إرسال BSV',
      recipient_address: 'عنوان BSV للمستلم',
      enter_bsv_address: 'أدخل عنوان BSV',

      wallet_data: 'بيانات المحفظة',
      create_new_wallet: 'إنشاء محفظة جديدة',
      generate_recovery_phrase_caption: 'إنشاء مفتاح استرداد جديد',
      import_existing_wallet: 'استيراد محفظة موجودة',
      paste_recovery_phrase: 'الصق مفتاح الاسترداد الخاص بك',
      save_recovery_phrase_heading: 'احفظ مفتاح الاسترداد الخاص بك',
      save_recovery_phrase_btn: 'حفظ مفتاح الاسترداد',
      recovery_phrase_write_down: 'اكتب هذه الكلمات الـ12 بالترتيب واحتفظ بها في مكان آمن.',
      acknowledgment_text:
        'لقد حفظت مفتاح الاسترداد الخاص بي وأفهم أن فقدانه سيؤدي إلى خسارة دائمة وكاملة لجميع الأموال والرموز والشهادات المرتبطة.',
      enter_recovery_words: 'أدخل عبارة الاسترداد المكوّنة من 12 كلمة',
      restore_wallet_description: 'أدخل مفتاح الاسترداد لاستعادة محفظتك',
      backup_reminder_title: 'محفظتك بلا نسخة احتياطية',
      backup_reminder_body: 'صدّر مفاتيحك لتفادي فقدان الأموال.',
      biometric_advisory_title: 'أمّن محفظتك',
      wallet_balance_show_in: 'العرض بـ {{unit}}',
      tx_detail_block: 'الكتلة',
      tx_detail_block_pending: 'بانتظار كتلة',
      biometric_advisory_body: 'احمِ أصولك باستخدام Face ID أو بصمة إصبعك',
      biometric_advisory_body_degraded:
        'لا يحتوي هذا الجهاز على Face ID أو بصمة إصبع مُفعّلة، لذا فإن مفتاح محفظتك محمي فقط برمز مرور الجهاز.',
      import_prompt_title: 'لديك محفظة بالفعل؟',
      import_prompt_body: 'استعدها من مفتاح الاسترداد أو من الحصص.',
      import_prompt_action: 'استيراد من نسخة احتياطية',
      nearby_advisory_title: 'الاتصال بالأجهزة القريبة',
      nearby_advisory_body:
        'سيُطلب منك السماح بالوصول إلى Wi-Fi أو البلوتوث أو NFC. يُستخدم ذلك فقط لربطك مباشرة بجهاز قريب من أجل الدفع — ولا يُرسل أبدًا إلى خوادمنا، ولا يُستخدم لبناء ملف تعريفي عنك.',
      backup_reminder_now: 'انسخ المفاتيح احتياطيًا الآن',
      backup_reminder_import: 'استيراد من نسخة احتياطية',
      restore_backup_checking: 'جارٍ البحث عن نسختك الاحتياطية المشفّرة…',
      restore_backup_progress: 'جارٍ استعادة سجل المحفظة — {{chunks}} من {{total}}…',
      restore_backup_failed_title: 'تعذّرت استعادة السجل',
      restore_backup_failed_message:
        'عبارة الاسترداد سليمة، لكن تعذّر إعادة تشغيل النسخة الاحتياطية المشفّرة. عادةً تكفي المحاولة مرة أخرى. المتابعة بدونها تمنحك محفظة عاملة بلا معاملات سابقة، ولن تكون بعض عملاتك الحالية قابلة للإنفاق.',
      restore_backup_retry: 'إعادة المحاولة',
      restore_backup_skip: 'تخلّي',
      restore_backup_unverified_title: 'قد تكون النسخة الاحتياطية غير مكتملة',
      restore_backup_unverified_body:
        'تمت استعادة محفظتك، لكن تعذّر تأكيد اكتمال النسخة الاحتياطية على الخادم. إذا كان بعض السجل مفقودًا، فاستعد مرة أخرى لاحقًا بعد أن ينتهي الجهاز الآخر من الرفع.',
      import_wallet: 'استيراد المحفظة',

      reject: 'رفض',
      authorize: 'تفويض',
      renewal: 'تجديد',
      spending_section: 'الإنفاق',
      protocols_section: 'البروتوكولات',
      baskets_section: 'السلال',
      certificates_section: 'الشهادات',
      requested_fields: 'الحقول المطلوبة',

      send_max: 'إرسال الحد الأقصى',
      message_box_unreachable: 'تعذر الوصول إلى خادم الدفع. تحقق من عنوان URL لصندوق الرسائل في الإعدادات.',
      available: 'متاح',
      note: 'ملاحظة',
      note_placeholder: 'ما الغرض؟ (اختياري)',
      entire_wallet_balance: 'رصيد المحفظة بالكامل',
      entire_vault_balance: 'كامل رصيد الخزنة',
      export_wallet_data: 'تصدير بيانات المحفظة',
      export_unencrypted_title: 'هذا الملف غير مشفّر',
      export_unencrypted_message:
        'الملف المُصدَّر هو نسخة غير مشفّرة من بيانات محفظتك — بما في ذلك معلومات الهوية والشهادات وسجل المعاملات وجهات الاتصال. يمكن لأي شخص يحصل على هذا الملف قراءته. شاركه فقط في مكان تثق به.',
      import_wallet_data: 'استيراد بيانات المحفظة',
      import_conflict_title: 'قاعدة البيانات موجودة بالفعل',
      import_conflict_message:
        'توجد بالفعل قاعدة بيانات محفظة لهذه الهوية بطابع زمني أحدث. يتم دائمًا استخدام النسخة الاحتياطية ذات الرقم الأعلى في اسم الملف.\n\nلاستخدام هذه النسخة الاحتياطية القديمة: قم بتصدير بياناتك الحالية، احذف التطبيق وأعد تثبيته، ثم استورد النسخة الاحتياطية المطلوبة فقط.',
      import_confirm_title: 'استيراد قاعدة البيانات',
      import_confirm_message: 'ستصبح قاعدة البيانات هذه محفظتك النشطة. متابعة؟',
      import_success: 'تم استيراد قاعدة البيانات بنجاح. جارٍ إعادة تشغيل المحفظة...',
      import_invalid_file: 'ملف نسخ احتياطي غير صالح',
      import_invalid_file_detail:
        'هذا الملف ليس نسخة احتياطية صالحة لقاعدة بيانات المحفظة. اسم الملف المتوقع مثل wallet-XXXXXXXX-mainnet-TIMESTAMP.db.',
      import_oversized_file: 'ملف النسخة الاحتياطية كبير جدًا',
      import_oversized_file_detail:
        'هذا الملف أكبر مما ينبغي أن تكون عليه النسخة الاحتياطية للمحفظة، لذا لم تتم قراءته. اختر ملف نسخة احتياطية حقيقي لقاعدة بيانات المحفظة.',
      import_untrusted_file: 'تعذر استيراد هذه النسخة الاحتياطية',
      import_untrusted_file_detail:
        'تعذر التحقق من أن هذه النسخة الاحتياطية تخص هذه المحفظة، أو تعذر الوثوق بمحتواها. لم يتم استيرادها.',
      import_anyway: 'استيراد على أي حال',
      import_invalid_input_title: 'إدخال غير صالح',
      import_invalid_input_message:
        'الرجاء إدخال عبارة استرداد صالحة (12-24 كلمة) أو مفتاح خاص سداسي عشري مكون من 64 حرفًا.',
      import_setup_failed: 'فشل إعداد المحفظة: {{error}}',
      create_wallet_refused: 'تعذر إنشاء المحفظة. يرجى المحاولة مرة أخرى.',
      create_wallet_failed: 'فشل إنشاء العبارة التذكيرية. يرجى المحاولة مرة أخرى.',
      or: 'أو',
      print_recovery_shares: 'طباعة حصص الاسترداد',
      scan_backup_shares: 'مسح حصص النسخ الاحتياطي',
      scan_backup_shares_caption: 'استرداد المحفظة من الحصص المطبوعة',
      scan_shares_invalid_format: 'تنسيق حصة غير صالح. يرجى مسح رمز QR صالح لحصة النسخ الاحتياطي.',
      scan_shares_camera_needed: 'يلزم الوصول إلى الكاميرا',
      scan_shares_camera_description: 'يلزم الوصول إلى الكاميرا لمسح رموز QR لحصص النسخ الاحتياطي المطبوعة.',
      scan_shares_grant_camera: 'منح الوصول إلى الكاميرا',
      scan_shares_recovering: 'جار استرداد المحفظة...',
      scan_shares_scan_first: 'امسح حصة النسخ الاحتياطي الأولى',
      scan_shares_progress: '{{scanned}} من {{needed}} حصص تم مسحها',
      scan_shares_remaining: 'يلزم {{count}} حصة إضافية',
      scan_shares_complete: 'تم جمع جميع الحصص!',
      scan_shares_recovery_failed: 'فشل الاسترداد. يرجى المحاولة مرة أخرى.',
      scan_shares_threshold_mismatch: 'لا تتطابق العتبة مع الحصص السابقة',
      scan_shares_integrity_mismatch: 'لا تتطابق تجزئة السلامة — الحصص من مفاتيح مختلفة',
      scan_shares_duplicate: 'تم مسح هذه الحصة بالفعل',
      scan_shares_biometric_title: 'يلزم الوصول البيومتري',
      scan_shares_biometric_message: 'يلزم الوصول البيومتري لحماية مفاتيح محفظتك. يرجى المحاولة مرة أخرى.',
      recovery_replace_wallet_title: 'استبدال المحفظة الموجودة على هذا الهاتف؟',
      recovery_replace_wallet_body:
        'يحتوي هذا الهاتف بالفعل على محفظة. الاستمرار يستبدلها مع كل ما يمكنها إنفاقه. إذا لم تكن عبارة الاسترداد الخاصة بها محفوظة في مكان آخر، فسيضيع ذلك المال إلى الأبد.',
      recovery_replace_wallet_confirm: 'استبدال المحفظة',
      invalid_bsv_address: 'عنوان BSV غير صالح',
      scan_recipient_hint: 'وجّه الكاميرا نحو رمز الدفع',
      scan_qr_default_hint: 'وجّه الكاميرا نحو رمز QR',
      scan_wallet_qr_hint: 'وجّه الكاميرا نحو رمز QR لإقران المحفظة',

      biometric_store_wallet: 'تخزين محفظتك بأمان',
      biometric_load_wallet: 'تحميل محفظتك بأمان',
      biometric_unlock_wallet: 'افتح قفل محفظتك',
      wallet_lost_title: 'تم إتلاف مفتاح المحفظة',
      wallet_lost_body:
        'غيّر هذا الجهاز إعدادات القياسات الحيوية، لذلك أتلف النظام المفتاح الذي يحمي هذه المحفظة. عملاتك آمنة على البلوكتشين — استعد هذه المحفظة باستخدام عبارة الاسترداد.',
      wallet_lost_restore: 'استعادة',
      wallet_lost_later: 'ليس الآن',
      wallet_locked_title: 'المحفظة مقفلة',
      wallet_locked_body: 'افتح القفل لتحميل مفاتيح محفظتك.',
      wallet_unlock_retry: 'فتح قفل المحفظة',
      wallet_lockout_body: 'محاولات فاشلة كثيرة. افتح قفل هاتفك برمز المرور ثم حاول مرة أخرى.',
      wallet_no_biometrics_body:
        'لا توجد قياسات حيوية مُعدّة على هذا الجهاز. لا تزال مفاتيح محفظتك مشفّرة بمخزن مفاتيح الجهاز، لكنها غير محمية بفحص حيوي.',

      // Connections screen
      connections: 'الاتصالات',
      advanced: 'خيارات متقدمة',
      copy_secret_words: 'نسخ الكلمات السرية',
      print_recovery_keys: 'طباعة مفاتيح الاسترداد',
      backup_wallet_keys: 'نسخ مفاتيح المحفظة احتياطيًا',
      log_out: 'تسجيل الخروج',
      connect_to_app: 'الاتصال بتطبيق',
      no_connections_yet: 'لا اتصالات حتى الآن',
      no_connections_subtitle: 'امسح رمز QR من تطبيق سطح المكتب لتوصيل محفظتك',
      connections_active: 'نشط',
      connections_disconnected: 'منقطع',
      disconnect: 'قطع الاتصال',
      reconnect: 'إعادة الاتصال',
      reconnect_failed: 'فشل إعادة الاتصال',
      reconnect_confirm_title: 'إعادة الاتصال بهذا الموقع؟',
      reconnect_confirm_message: 'سيتمكن {{origin}} من الوصول إلى محفظتك مرة أخرى.',
      reconnect_needs_reapproval: 'تعذر التحقق من هذا الاتصال. امسح رمز QR جديدًا لإعادة الاتصال.',
      scan_qr_code: 'مسح رمز QR',
      paste_uri: 'لصق URI',
      invalid_qr_code: 'رمز QR غير صالح',
      wallet_not_ready: 'المحفظة غير جاهزة',
      please_log_in_first: 'الرجاء تسجيل الدخول أولاً',
      unknown_error: 'خطأ غير معروف',

      // Pay screen
      payments: 'المدفوعات',
      pay: 'الدفع',
      pay_direction_pay: 'ادفع',
      pay_direction_receive: 'استلم دفعة',
      pay_cell_nearby_pay: 'شخص قريب',
      pay_cell_nearby_pay_sub: 'امسح رمزه',
      pay_cell_handle_pay: 'شخص عن بُعد',
      pay_cell_handle_pay_sub: 'اختر اسمًا — يحتاجون هذا التطبيق',
      pay_cell_address_pay: 'إلى عنوان BSV',
      pay_cell_address_pay_sub: 'الصق عنوان BSV أو امسحه',
      pay_cell_nearby_get: 'شخص قريب',
      pay_cell_nearby_get_sub: 'اعرض رمز الدفع الخاص بك',
      pay_cell_handle_get: 'شخص عن بُعد',
      pay_cell_handle_get_sub: 'شارك اسمك',
      pay_cell_address_get: 'إلى عنوان BSV',
      pay_cell_address_get_sub: 'اعرض عنوان BSV',
      pay_method: 'الطريقة',
      pay_method_nearby: 'قريب',
      pay_method_remote_link: 'مشاركة رابط عن بُعد',
      pay_method_address: 'إلى عنوان BSV',
      pay_pre_nearby: 'كلاكما هنا.',
      pay_pre_handle: 'هذا الشخص يستخدم هذا التطبيق.',
      pay_pre_address: 'لديك عنوان BSV الخاص به.',
      pay_conseq_nearby: 'تتم التسوية في ثوانٍ.',
      pay_conseq_handle: 'تصل عند التحقق التالي من محفظته.',
      pay_conseq_address: 'يُرسل المال — ولا يتلقى المستلم أي إشعار.',
      paid: 'تم الدفع',
      pay_inbox_attention: 'لم تتم الإضافة',
      pay_inbox_attention_hint: 'وصلت لكن محفظتك لم تتمكن من إضافتها. أعد المحاولة، أو تجاهلها للتخلي عنها نهائيًا.',
      pay_dismiss_confirm: 'اضغط مرة أخرى للتخلي',
      pay_dismissed: 'تم التخلي عن الدفعة',
      payment_arrived: 'تم استلام الدفعة',
      payment_arrived_damaged: 'وصل هذا الدفع تالفًا',
      discard_payment_title: 'تجاهل هذه الدفعة؟',
      discard_payment_body: 'سيُطلب من المرسل إرسال التفاصيل مرة أخرى. تبقى الأموال على السلسلة حتى يفعل ذلك.',
      discard_void_title: 'لم تتم هذه الدفعة',
      discard_void_body: 'أُلغيت أو أُنفقت في مكان آخر. يمكن تجاهلها بأمان.',
      waiting_network_confirm: 'في انتظار تأكيد الشبكة لهذه الدفعة',
      finish_or_cancel_outgoing: 'أنهِ الدفعة أدناه أو ألغِها قبل إرسال دفعة جديدة',
      identity_search_unavailable: 'البحث غير متاح حاليًا. حاول بعد قليل.',
      abandon_payment: 'التخلي عن الدفعة',
      finish_payment: 'إنهاء الدفعة',
      cancel_this_payment: 'إلغاء هذه الدفعة؟',
      cancel_payment: 'إلغاء الدفعة',
      resend: 'إعادة الإرسال',
      resending: 'جارٍ إعادة الإرسال…',
      send_payment_details_again: 'إرسال التفاصيل مرة أخرى',
      tx_action_resend_short: 'إعادة الإرسال',
      tx_action_request_confirmation: 'طلب تأكيد من المستلم',
      resend_sent: 'تم إرسال تفاصيل الدفعة مرة أخرى',
      resend_failed_no_record: 'لا توجد تفاصيل هذه الدفعة على هذا الجهاز، لذا لا يمكن إرسالها مرة أخرى.',
      resend_failed_no_recipient: 'لا تحتوي هذه الدفعة على المستلم، لذا لا يمكن إرسالها مرة أخرى.',
      resend_failed_no_transaction: 'تعذّر الحصول على بيانات هذه الدفعة. حاول مرة أخرى عند الاتصال بالإنترنت.',
      resend_requested: 'طُلب منك إعادة إرسال دفعة',
      request_again: 'الطلب مرة أخرى',
      send_again: 'الإرسال مرة أخرى',
      copy_details: 'نسخ التفاصيل',
      payment_bounced_resend: 'طُلب منهم إرسال هذه الدفعة مرة أخرى',
      dismiss_rejected_payment: 'تم تجاهل هذه الدفعة المرفوضة',
      activity_load_failed: 'تعذّر تحميل النشاط',
      activity_load_retry: 'اضغط لإعادة المحاولة',
      activity_view_contact: 'عرض جهة الاتصال',
      home_payments_need_attention: '{{count}} دفعات تحتاج إلى انتباهك',
      home_payments_unsent: '{{count}} دفعات لم تُسلَّم',
      pay_copy: 'نسخ',
      pay_share_link: 'مشاركة الرابط',
      pay_address_watching: 'الأموال المرسلة إلى هذا العنوان تُضاف إلى محفظتك تلقائيًا.',
      pay_address_earlier_day: 'الانتقال إلى يوم أسبق',
      pay_address_sweep_now: 'تحقق من عنوان BSV هذا الآن',
      pay_address_swept: 'تم استلام {{amount}}',

      check_wallet: 'استكشاف الأخطاء',
      settings_version: 'v{{version}}',
      error_review_actions: 'هذه الدفعة متعثّرة. يمكن لاستكشاف الأخطاء محاولة تحريرها.',
      wallet_check_ok: 'كل شيء يبدو على ما يرام',
      wallet_check_attention: 'بعض الفحوصات تحتاج انتباهك',
      wallet_check_couldnt: 'تعذّر الفحص',
      wallet_check_couldnt_body: 'لم تكتمل إحدى خطوات الإصلاح. ولم يُفترض أي شيء بشأن عملاتك.',
      wallet_check_summary: 'تحرير {{freed}} عملة متعثّرة · استرداد {{recovered}} دفعة',
      wallet_check_step_coins: 'عملاتك صالحة',
      wallet_check_skip: 'تخطَّ هذا الفحص',
      wallet_check_skipped: 'تم التخطي',
      wallet_check_step_proofs: 'إثباتات المعاملات',
      wallet_check_step_records: 'سجلات قاعدة البيانات المحلية',
      wallet_check_step_tokens: 'تم التحقق من تحويلات العملة المستقرة',
      wallet_check_step_online: 'الاتصال بالإنترنت',
      wallet_check_step_missed: 'مدفوعات لم تصل',
      wallet_check_step_backup: 'رفع النسخة الاحتياطية الخاصة',
      wallet_check_step_phrase: 'حفظ عبارة الاسترداد',
      wallet_check_progress: '{{current}} من {{total}}',
      wallet_check_stuck_title: 'تعذّر إرسال هذه الدفعة',
      wallet_check_stuck_message: 'يبدو أن بعض العملات متعثّرة. يمكن لاستكشاف الأخطاء تحريرها.',
      wallet_check_failed: 'تعذّر إكمال الفحص. حاول مرة أخرى.',

      // Local payments
      local_pay_request: 'طلب دفعة',
      local_pay_send: 'إرسال دفعة',
      local_pay_amount: 'المبلغ',
      local_pay_show_qr: 'اعرض هذا الرمز على الدافع',
      local_pay_scan_qr: 'امسح رمز المستلم',
      local_pay_scan_payer_qr: 'امسح رمز الدافع',
      local_pay_scan_progress: '{{have}} من {{total}} أجزاء',
      local_pay_show_payment_qr: 'اعرض هذا الرمز على المستلم',
      local_pay_animated_hint: 'اطلب منه مسح الرمز المتحرك أدناه — سيستمر في التغيّر حتى يستلم هاتفه البيانات كاملة.',
      local_pay_show_qr_instead: 'اعرض رمز QR بدلاً من ذلك',
      local_pay_preparing: 'جارٍ تجهيز الطلب…',
      local_pay_delivering: 'جارٍ إرسال الدفعة…',
      local_pay_saving: 'جارٍ حفظ الدفعة…',
      local_pay_sent: 'تم إرسال الدفعة',
      local_pay_received: 'تم استلام الدفعة',
      local_pay_added: 'تمت الإضافة إلى محفظتك.',
      local_pay_added_multiple: 'تمت إضافة {{count}} مدفوعات إلى محفظتك.',
      local_pay_received_confirming: 'جارٍ التأكيد مع محفظتك…',
      local_pay_received_not_credited: 'لم تُضَف إلى محفظتك بعد. تحقق من النشاط قبل تسليم أي شيء.',
      local_pay_decline_broadcast_title: 'دفعة مرفوضة تم بثّها على الشبكة',
      local_pay_decline_broadcast_body: 'ظهرت على الشبكة دفعة كانت قد رُفضت. لا ترسلها مرة أخرى — تحقق من النشاط.',
      local_pay_pending_abort_dropped_title: 'تم تجاهل طلب تحرير غير موثّق',
      local_pay_pending_abort_dropped_body:
        'تعذّر التحقق من سجل محفوظ لتحرير دفعة، لذا لم يتم تحرير أي شيء. إذا بدت إحدى الدفعات عالقة، فألغِها من النشاط.',
      local_pay_cancel_unverifiable_title: 'لا يمكن التحقق من هذه الدفعة أثناء عدم الاتصال',
      local_pay_cancel_unverifiable_body:
        'إذا كان الطرف الآخر قد مسحها بالفعل، فإن الإلغاء يسمح بإنفاق هذه العملات مرتين. لا تُلغِ إلا إذا كنت متأكدًا من أنها لم تُمسح مطلقًا.',
      local_pay_cancel_unverifiable_confirm: 'إلغاء مع ذلك',
      local_pay_queued: 'تم الحفظ — ستُضاف إلى محفظتك تلقائيًا.',
      local_pay_already_paid: 'تم دفع هذا الطلب بالفعل.',
      local_pay_failed: 'فشلت عملية الدفع',
      local_pay_network_denied: 'الوصول إلى الشبكة المحلية مُعطَّل. فعّله من الإعدادات للدفع إلى الأجهزة القريبة.',
      local_pay_radio_fallback: 'الاتصال اللاسلكي غير متاح — اعرض هذا الرمز على الجهاز الآخر بدلاً من ذلك.',
      local_pay_floor_peer_bt_off: 'بلوتوث الجهاز الآخر مُغلق، لذا ستتم هذه الدفعة عبر الرمز.',
      local_pay_floor_local_bt_off: 'بلوتوث جهازك مُغلق. شغّله للحصول على اتصال قريب أسرع.',
      local_pay_floor_local_ble_denied:
        'الوصول إلى البلوتوث مُعطَّل لتطبيق BSV Wallet. اسمح به من الإعدادات للحصول على اتصال قريب أسرع.',
      local_pay_floor_cross_os: 'لا يمكن للجهاز الآخر استخدام البلوتوث الآن، لذا ستتم هذه الدفعة عبر الرمز.',
      local_pay_floor_peer_no_radio: 'الجهاز الآخر لا يستمع لاتصال لاسلكي، لذا ستتم هذه الدفعة عبر الرمز.',
      local_pay_ble_unavailable: 'البلوتوث غير متاح. الرمز أعلاه ما زال يعمل.',
      local_pay_too_large: 'بيانات هذه الدفعة أكبر من أن تُنقل عبر رموز QR.',
      local_pay_nearby_unavailable: 'شبكة Wi-Fi القريبة غير متاحة. الرمز أعلاه ما زال يعمل.',
      local_pay_retry_save: 'حاول الحفظ مرة أخرى',
      local_pay_wrong_session: 'هذه الدفعة لا تطابق هذا الطلب ولم يتم قبولها. اطلب من الدافع مسح الرمز مرة أخرى.',
      local_pay_declined_mismatch: 'لم يقبل المستلم هذه الدفعة لأنها لا تطابق طلبه. امسح رمزه مرة أخرى.',
      local_pay_declined_already_paid: 'تم دفع هذا الطلب من قبل. لم يُرسل أي شيء.',
      local_pay_declined_save: 'تعذّر على المستلم حفظ الدفعة. لم يُرسل أي شيء — حاول مرة أخرى.',
      local_pay_declined_decode: 'تعذّر على المستلم قراءة الدفعة. لم يُرسل أي شيء — حاول مرة أخرى.',
      local_pay_declined_not_covered:
        'تعذّر على المستلم التحقق من هذه الدفعة مقابل سجلات جهة الإصدار. لم يُرسل أي شيء.',
      local_pay_broadcast_pending: 'تم الإرسال. لم يتمكن هذا الجهاز من معالجتها بعد — سيقوم المستلم بذلك.',
      local_pay_any_amount: 'أي مبلغ',
      local_pay_choose_amount: 'اختر المبلغ',
      local_pay_enter_amount_send: 'ما المبلغ الذي تريد إرساله؟',
      local_pay_qr_unavailable: 'تعذّر إنشاء هذا الرمز. ابدأ الطلب من جديد.',
      local_pay_presence_qr: 'عبر الرمز — لا يوجد اتصال مباشر',
      local_pay_presence_ready: 'الاتصال القريب المشفّر جاهز',
      local_pay_presence_waiting_payee: 'في انتظار جهاز قريب',
      local_pay_presence_waiting_payer: 'جارٍ البحث عن جهاز المستلم…',
      local_pay_presence_linked: 'الاتصال المشفّر مفتوح',
      local_pay_presence_paid: 'تم تأكيد الدفع',
      pay_offline_title: 'أنت غير متصل بالإنترنت',
      pay_offline_body: 'تعمل الآن المدفوعات القريبة فقط. ستتم معالجتها بمجرد اتصالك مجددًا.',
      pay_offline_queued: 'هناك {{count}} من المدفوعات في انتظار المعالجة بمجرد اتصالك مجددًا.',
      pay_offline_pending_title: 'في انتظار المعالجة',
      pay_offline_pending_body: 'لم تُعالج {{count}} من المدفوعات بعد. لا شيء مكتمل حتى تُعالج، وستتم إعادة المحاولة.',
      pay_offline_needs_internet: 'يتطلب اتصالًا بالإنترنت',
      pay_offline_rejected_title: 'تم رفض دفعة',
      pay_offline_rejected_body: 'رفضت الشبكة دفعة سلّمها {{sender}} عبر {{via}} في {{when}}.',
      pay_offline_unknown_sender: 'مرسل غير معروف',
      pay_offline_unknown_via: 'وسيلة غير معروفة',
      pay_received_not_broadcast: 'تم الاستلام دون اتصال · لم تُعالج بعد',
      pay_offline_sent_rejected_title: 'تعذّر تسليم دفعة',
      pay_offline_sent_rejected_body: 'رفضت الشبكة دفعة أرسلتها في {{when}}.',
      pay_offline_send_now: 'أرسل الآن',
      pay_offline_stalled_body: 'تعذّر إرسال بعض المدفوعات في قائمة الانتظار تلقائيًا: {{detail}}',
      offline_stall_no_request: 'دفعة في قائمة الانتظار ينقصها سجل الإرسال.',
      offline_stall_bad_beef: 'تعذّرت قراءة بيانات معاملة لدفعة في قائمة الانتظار.',
      offline_stall_foreign_ancestor: 'دفعة في قائمة الانتظار تنتظر معاملة أصلية لم تعالجها محفظة أخرى قط.',
      pay_offline_kv_pending: '{{count}} دفعة قريبة في انتظار الإضافة إلى هذه المحفظة.',
      pay_offline_kv_stuck: 'تعذّرت إضافة {{count}} دفعة قريبة. اطلب من المُرسِل الإرسال مرة أخرى.',
      pay_offline_kv_corrupt: 'تم العثور على بيانات دفع تالفة على هذا الجهاز.',
      pay_offline_kv_corrupt_body:
        'تم وضع المدفوعات المتأثرة جانبًا على هذا الجهاز ولم تُحذف. ربما أُضيف بعضها إلى رصيدك بالفعل، لذا اطلب من المُرسِل الإرسال مرة أخرى فقط إذا كانت دفعته ما زالت مفقودة.',
      pay_offline_show_code: 'إظهار الرمز مرة أخرى',
      pay_parked_cancel: 'إلغاء الدفعة',
      pay_parked_already_sent: 'تم إرسال هذه الدفعة بالفعل',
      token_cancel_already_sent: 'تم تنفيذ هذه الدفعة بالفعل ولا يمكن إلغاؤها.',
      tx_status_parked: 'لم يُسلَّم',
      tx_status_import_hold: 'موقوف (مستورد)',
      resend_sending: 'جارٍ الإرسال عبر صندوق الرسائل',
      cancelling_payment: 'جارٍ إلغاء الدفعة',

      contacts: 'جهات الاتصال',
      contacts_search_placeholder: 'ابحث بالاسم أو اسم المستخدم',
      contacts_empty_title: 'لا توجد جهات اتصال بعد',
      contacts_empty_body:
        'يظهر هنا الأشخاص الذين تدفع لهم بعد حفظهم. امسح رمز الملف الشخصي لشخص ما باستخدام الزر أعلى اليمين.',
      contacts_recent: 'الأخيرة',
      contact_edit_name: 'تعديل الاسم',
      contact_save_name: 'حفظ الاسم',
      contact_activity: 'النشاط',
      contact_activity_empty: 'لا يوجد نشاط مع جهة الاتصال هذه بعد',
      contact_delete: 'حذف جهة الاتصال',
      contact_delete_body: 'سيؤدي هذا إلى إزالة {{name}} من جهات اتصالك. لا يؤثر ذلك على أي مدفوعات سابقة.',
      contact_handle_caption: 'اسم المستخدم المسجَّل الخاص به · هو وحده من يمكنه تغييره',
      contact_new_title: 'جهة اتصال جديدة',
      contact_save: 'حفظ',
      contact_identifier: 'المعرّف',
      contact_saved: 'تم حفظ جهة الاتصال',
      contact_scan_hint: 'امسح رمز الملف الشخصي لإضافة شخص',
      contact_identifier_placeholder: 'الصق معرّفه',
      contact_identifier_invalid: 'هذا لا يبدو معرّفًا صالحًا',
      contact_add_from_payment: 'إضافة إلى جهات الاتصال',

      profile: 'الملف الشخصي',
      profile_display_name: 'الاسم المعروض',
      profile_display_name_hint: 'يظهر علنًا بجوار اسم المستخدم الخاص بك. يمكن لأي شخص يبحث عنك رؤيته.',
      profile_handle: 'اسم المستخدم',
      profile_handle_placeholder: 'معرّفك',
      profile_handle_checking: 'جارٍ التحقق من التوفر…',
      profile_handle_available: '{{handle}} متاح',
      profile_handle_taken: '{{handle}} مُستخدم بالفعل',
      profile_handle_invalid: '3-32 حرفًا: a-z و0-9 والنقطة والشرطة السفلية والشرطة، ويبدأ وينتهي بحرف أو رقم',
      profile_handle_failed: 'تعذّر التحقق من التوفر. حاول مرة أخرى.',
      profile_handle_unavailable: 'تسجيل اسم المستخدم غير متاح بعد في هذا الإصدار.',
      profile_handle_registered: 'تم تسجيل اسم المستخدم',
      profile_handle_register_action: 'تسجيل',
      profile_handle_too_similar: '{{handle}} يشبه كثيرًا اسمًا مستخدمًا بالفعل',
      profile_handle_reserved: '{{handle}} محجوز',
      profile_handle_cooldown: '{{handle}} تم تحريره مؤخرًا وغير متاح بعد',
      profile_handle_pending: 'جارٍ إنهاء التسجيل…',
      profile_handle_rolled_back: '{{handle}} أصبح محجوزًا. احتفظت بـ {{previous}}.',
      profile_handle_changed: 'تم تغيير اسم المستخدم',
      profile_handle_rejected: 'رفضت خدمة التسجيل اسم المستخدم هذا.',
      profile_handle_clock_ahead: 'ساعة جهازك متقدّمة كثيرًا. حاول مرة أخرى بعد بضع دقائق.',
      profile_handle_wrong_registry: 'اسم المستخدم هذا ليس على خدمة التسجيل هذه. افتح الملف الشخصي مجددًا لتحديثه.',
      profile_display_name_publish_failed: 'تم حفظ اسمك هنا، لكن خدمة التسجيل لم تنشره.',
      profile_qr_hint: 'يمكن للآخرين مسح هذا لإضافتك كجهة اتصال',

      pay_step_contacts: 'جهات الاتصال',
      pay_step_continue: 'متابعة',
      pay_review_to: 'إلى',
      pay_review_amount: 'المبلغ',
      pay_review_note: 'ملاحظة',
      pay_review_note_edit: 'إضافة ملاحظة',
      pay_review_delivery_host: 'التسليم إلى',
      pay_trust_contact: 'جهة اتصالك',
      pay_trust_handle_attested: 'مسجَّل',
      pay_trust_handle_domain_attested: 'موثّق بواسطة {{domain}}',
      pay_trust_unverified: 'غير موثّق. تحقق جيدًا من هذا المعرّف.',
      pay_address_network_mismatch: 'هذا العنوان ينتمي إلى شبكة مختلفة عن الشبكة التي تستخدمها.',
      contacts_your_contacts: 'جهات اتصالك',
      contacts_other_people: 'أشخاص آخرون',
      contacts_not_saved: 'غير محفوظ',
      contacts_network_footer: 'موجود على الشبكة. لم تحفظه بعد.',
      contact_no_handle: 'لا يوجد اسم مستخدم بعد',
      contact_no_handle_registered: 'لا يوجد اسم مستخدم مسجَّل',
      contact_name_section: 'اسمك له',
      contact_name_hint: 'أنت وحدك تراه. لا يغيّر ما يسمّي به نفسه.',
      contact_new_name_hint: 'أنت وحدك تراه. محفوظ على هذا الجهاز.',
      contact_copy_identifier: 'نسخ المعرّف',
      profile_handle_registered_hint: 'مسجَّل على معرّفك. يمكن للناس إيجادك من خلاله.',
      profile_handle_change: 'تغيير',
      profile_handle_claim: 'حجز {{handle}}',
      profile_handle_replace_warning:
        'الحجز يستبدل {{handle}}. من حفظوك يحتفظون بجهة الاتصال؛ ولن يقود اسم المستخدم القديم إليك بعد الآن.',
      profile_identifier_section: 'معرّفك',
      profile_show_qr: 'عرض كرمز QR',
      profile_show_qr_hint: 'دع أحدهم يمسحك ليحصل على ملفك الشخصي',
      profile_qr_callout:
        'مسح هذا الرمز يفتح BSV Wallet ويضيفك كجهة اتصال، مع اسمك واسم المستخدم وصورتك. لا يطلب أي دفعة — ذلك الرمز موجود في «استلام».',
      pay_review_title: 'مراجعة',
      pay_send_amount: 'إرسال {{amount}}',
      pay_sent_to: 'إلى {{name}}',
      pay_save_as_contact: 'حفظ كجهة اتصال'
    }
  },
  pt: {
    translation: {
      token_balances_header: 'Saldos',
      wallet_coin_switcher: 'Escolha qual moeda mostrar',
      wallet_activity_empty_asset: 'Ainda sem atividade em {{ticker}}',
      pay_direction_pay_asset: 'Pagar {{ticker}}',
      token_fee_footer: 'Enviar precisa de um pouco de BSV para a taxa de rede. Receber não.',
      token_new_tap: 'Novo — toque para ver quem o emite',
      token_unresolved: 'Token não reconhecido',
      token_unresolved_sub: 'Esta carteira não conseguiu descobrir quem o emite. Tente de novo quando estiver online.',
      token_paused_short: 'Transferências pausadas',
      token_frozen_short: '{{amount}} congelado',
      token_sheet_issuer: 'Emitido por',
      token_sheet_issuer_unknown: 'Não identificado',
      token_sheet_asset_id: 'ID do token',
      token_sheet_powers_header: 'O que {{issuer}} pode fazer',
      token_power_pause: 'Pausar todas as transferências de {{ticker}}',
      token_power_freeze: 'Congelar moedas específicas, incluindo as suas',
      token_power_admit: 'Decidir quem pode ter {{ticker}}',
      token_power_replace: 'Substituir uma moeda congelada por uma nova',
      token_powers_limit:
        'Eles não podem tirar {{ticker}} da sua carteira sem você, e uma moeda congelada simplesmente não pode ser gasta.',
      token_frozen_sub: '{{issuer}} congelou {{amount}} {{ticker}} do seu saldo. Fale com eles para contestar.',
      pay_asset_label: 'Pagar com',
      pay_asset_label_get: 'Receber em',
      pay_asset_cta: 'Enviar {{amount}} {{ticker}}',
      pay_asset_over_balance: 'Maior que o seu saldo de {{ticker}}.',
      pay_asset_needs_bsv: 'Enviar {{ticker}} precisa de um pouco de BSV para a taxa de rede. Receber não.',
      pay_asset_get_bsv: 'Obter BSV',
      pay_asset_needs_bsv_short: 'Precisa de BSV para a taxa',
      pay_asset_frozen_note: '{{amount}} {{ticker}} do seu saldo está congelado e não pode ser enviado.',
      pay_asset_paused: '{{issuer}} pausou as transferências de {{ticker}}, então isto não pode ser enviado agora.',
      pay_asset_no_address:
        '{{ticker}} só pode ser enviado a uma pessoa ou a um dispositivo próximo — não a um endereço Bitcoin.',
      pay_asset_address_status: 'Indisponível para {{ticker}}',
      pay_asset_self_unregistered:
        '{{issuer}} não registou você para enviar {{ticker}}. Ainda pode receber nesse token.',
      pay_asset_unidentified: 'Esta carteira não conseguiu identificar este token, por isso ainda não pode enviá-lo.',
      pay_asset_recipient_unregistered:
        '{{issuer}} ainda não registou esta pessoa para {{ticker}}, por isso não é possível pagar-lhe nesse token.',
      pay_asset_recipient_blocked: '{{issuer}} não permite pagamentos a esta pessoa.',
      token_err_refused: '{{issuer}} recusou esta transferência. Nada foi enviado e o seu {{ticker}} não mudou.',
      token_err_refused_paused: '{{issuer}} pausou {{ticker}}. Nada foi enviado e o seu saldo não mudou.',
      token_err_refused_recipient:
        '{{issuer}} não permite pagamentos a esta pessoa. Nada foi enviado e o seu saldo não mudou.',
      token_err_refused_frozen:
        'Parte do {{ticker}} que estava a enviar acabou de ser congelada. Nada foi enviado; o seu saldo foi atualizado.',
      token_err_send_failed: 'Não foi possível enviar este pagamento: {{reason}}. Nada saiu da sua carteira.',
      token_err_reason_unknown: 'um problema desconhecido',
      token_err_unreachable:
        'Não conseguimos contactar {{issuer}} para confirmar esta transferência. Verifique o seu saldo de {{ticker}} antes de tentar de novo.',
      token_err_check_again: 'Verificar de novo',
      token_err_balance_changed:
        'O seu saldo de {{ticker}} mudou enquanto isto era preparado. Verifique o valor e tente de novo.',
      token_err_busy: 'Já existe um pagamento de {{ticker}} a sair.',
      pay_sent_not_broadcast: 'Enviado. Ainda não chegou à rede — vamos continuar a tentar.',
      pay_sent_not_notified: 'Enviado. Ainda não conseguimos avisar — verão quando a carteira deles verificar de novo.',
      pay_sent_handed_to_wallet: 'Enviado. Não receberam pessoalmente, por isso enviámos para a carteira deles.',
      token_first_hold: '{{ticker}} é emitido por {{issuer}}, que pode pausar transferências e congelar moedas.',
      pay_asset_request_paused: '{{issuer}} pausou {{ticker}}, por isso um pagamento pode ainda não chegar.',
      token_admit_title: 'Ainda não pode receber em {{ticker}}',
      token_admit_body:
        '{{issuer}} só envia {{ticker}} a pessoas que registou. Até lá, os pagamentos para si são recusados antes de saírem da carteira do remetente.',
      token_admit_bsv_instead: 'Receber em BSV em vez disso',
      token_row_sent: '{{ticker}} enviado',
      token_row_received: '{{ticker}} recebido',
      token_row_amount_pending: 'Valor indisponível',
      token_evicted_title: 'Parte do {{ticker}} foi retirada',
      token_evicted_body: '{{issuer}} retirou {{amount}} {{ticker}} da sua carteira.',
      local_pay_token_not_cleared: 'Recebido offline · ainda não confirmado por {{issuer}}',
      pay_asset_nearby_needs_network:
        'Pagar em {{ticker}} presencialmente precisa de ligação. Sem ela, pode pagar em BSV.',
      pay_asset_link_not_held: 'Este pedido é de um token que esta carteira não possui. Você ainda pode pagar em BSV.',
      pay_asset_nearby_wrong_overlay: 'Este pedido é de um token para o qual esta carteira não está configurada.',
      token_messagebox_mismatch:
        'A sua carteira está definida para outra caixa de mensagens, por isso pagamentos em tokens podem não chegar.',
      token_issuer_fallback: 'o emissor',
      token_row_hint: 'Abre quem o emite e o que pode fazer',
      token_attention_one: 'À espera da confirmação de {{issuer}} — toque para detalhes',
      token_attention_many: '{{count}} pagamentos em tokens aguardam liquidação — toque para detalhes',
      token_recv_reversed: 'Este pagamento não pôde ser confirmado por {{issuer}} e foi revertido.',
      token_settle_paused_retry: '{{issuer}} pausou {{ticker}} neste momento — vamos continuar a tentar.',
      token_settle_unreachable_retry:
        'Não conseguimos contactar {{issuer}} para confirmar esta transferência. Vamos continuar a tentar.',
      token_settle_evicted:
        'Esse pagamento não chegou à rede a tempo e foi revertido — o seu {{ticker}} voltou ao saldo.',
      token_frame_unreadable: 'Não conseguimos ler o pagamento que recebeu — peça ao remetente para enviar de novo.',
      token_chain_reversed:
        '{{issuer}} recusou um pagamento anterior desta cadeia, por isso este também foi revertido.',
      token_settle_stuck_bound:
        'Este pagamento está à espera de liquidação há mais de 3 dias. Pode continuar a aguardar ou contactar {{issuer}}.',
      token_cover_failed: 'Este pagamento não bate com os registos de {{issuer}} e não foi aceite.',
      token_sent_settling:
        'Enviado. A liquidar com {{issuer}} — verá a confirmação quando você ou {{payee}} voltarem a ligar-se.',
      token_sent_settling_unnamed:
        'Enviado. A liquidar com {{issuer}} — verá a confirmação quando um de vocês voltar a ligar-se.',
      token_sent_settled: 'Enviado · liquidado com {{issuer}}',
      token_status_settling: 'A liquidar',
      token_status_settled: 'Liquidado',
      token_status_refused: 'Recusado',
      token_status_reversed: 'Revertido',
      token_status_stuck: 'À espera de liquidação',
      new_tab: 'Nova aba',
      untitled: 'Sem título',
      share: 'Compartilhar',
      back_to_homepage: 'Voltar à página inicial',
      bookmarks: 'Favoritos',
      add_bookmark: 'Adicionar favorito',
      bookmark: 'Favorito',
      unbookmark: 'Remover favorito',
      find_in_page: 'Buscar na página',
      find_in_page_placeholder: 'Buscar na página...',
      find_in_page_matches: '{{current}} de {{total}}',
      find_in_page_no_matches: 'Sem resultados',
      history: 'Histórico',
      clear_all: 'Limpar tudo',
      clear_all_tabs: 'Fechar todas as abas',
      search_or_enter_website: 'Pesquisar ou digitar site',
      browser: 'Navegador',
      bug_report: 'Relatório de bug',
      enable_web3: 'Ativar Web3',
      tabs: 'Abas',
      unlock_web3_features: 'Desbloquear recursos Web3',

      cancel: 'Cancelar',
      done: 'Concluído',
      discard: 'Descartar',
      deny: 'Negar',
      allow: 'Permitir',
      save: 'Salvar',
      remove: 'Remover',
      delete: 'Excluir',
      search: 'Pesquisar',
      loading: 'Carregando...',
      continue: 'Continuar',
      go_back: 'Voltar',
      continue_action: 'Continuar',
      camera_denied_description:
        'O acesso à câmera foi negado. Você pode habilitá-lo em Ajustes para usar este recurso.',
      open_settings: 'Abrir Ajustes',
      copied: 'Copiado',
      explorer_open_failed: 'Não foi possível abrir o explorador de blocos',
      tx_action_refresh: 'Atualizar estado',
      tx_action_refresh_short: 'Estado',
      tx_action_abort: 'Cancelar transação',
      copy: 'Copiar',
      copy_to_clipboard: 'Copiar para área de transferência',
      details: 'Detalhes',
      hide_details: 'Ocultar detalhes',

      identity: 'Identidade',
      settings: 'Configurações',
      wallet: 'Carteira',
      activity: 'Atividade',
      configuration: 'Configuração',
      data_and_security: 'Dados e segurança',
      storage_row: 'Armazenamento',
      storage_title: 'Armazenamento da carteira',
      storage_db_size: 'Banco de dados: {{size}}',
      storage_free_space: 'Espaço livre: {{size}}',
      storage_reclaimable: 'Recuperável agora: {{size}} em {{count}} transações',
      storage_held_back: '{{count}} transações mantidas por ainda estarem em uso',
      storage_reclaim_explain:
        'A recuperação apaga dados de validação que a carteira já não precisa. Suas transações, saldos e histórico ficam intactos. Isso libera espaço dentro do arquivo do banco de dados; o arquivo só encolhe quando o sistema o compacta.',
      storage_reclaim_confirm: 'Recuperar',
      storage_reclaimed: '{{count}} transações recuperadas',
      storage_failed: 'Não foi possível ler o uso de armazenamento.',
      storage_unavailable: 'O armazenamento ainda não está pronto.',
      backup_push_section: 'Backup privado',
      backup_push_toggle: 'Fazer backup no servidor',
      backup_erase_row: 'Excluir backup do servidor',
      backup_erase_title: 'Excluir seu backup?',
      backup_erase_message:
        'Tudo o que esta carteira enviou ao servidor de backup é excluído permanentemente, em todos os dispositivos. O backup também é desligado. Suas moedas, chaves e frase de recuperação ficam intactas — mas um novo dispositivo não poderá mais restaurar o histórico desta carteira, e algumas das suas moedas atuais não serão gastáveis lá.',
      backup_erase_confirm: 'Excluir',
      backup_erase_done: 'Backup excluído — {{count}} itens removidos',
      backup_erase_failed: 'Não foi possível excluir o backup. Nada foi removido — tente novamente.',
      backup_erase_no_key: 'Nenhuma chave de carteira disponível',
      backup_push_disclosure:
        'Uma cópia criptografada do banco de dados da sua carteira é enviada a um servidor da BSV Association para que você possa recuperá-la em um novo dispositivo. Ela é criptografada com uma chave que só a sua frase de recuperação deriva — o operador não pode lê-la. Você ainda precisa da frase ou das partes impressas: sem elas ninguém abre o backup, nem nós.',
      backup_push_off_title: 'Desativar o backup privado?',
      backup_push_off_message:
        'Sua frase de recuperação sozinha não reconstrói esta carteira. As informações associadas à proteção das suas moedas existem apenas no banco de dados, então com o backup desligado um novo dispositivo restaura a frase, mas não os fundos. Nada que já está no servidor é apagado.',
      backup_push_off_confirm: 'Desativar',
      backup_push_on_toast: 'Backup ativado — sincronizando',
      backup_push_off_toast: 'Backup desativado — nada mais será enviado',
      permissions: 'Permissões',
      trust_network: 'Rede de Confiança',
      // vault (machine-translated, needs native review)
      vault_title: 'Cofre',
      vault_row_title: 'Cofre',
      vault_hero_title: 'Fundos protegidos',
      vault_hero_body:
        'Armazenamento a frio de longo prazo, trancado com as suas próprias YubiKeys. Requer duas ou mais YubiKey 5 NFC.',
      vault_unsupported_title: 'Requer uma YubiKey',
      vault_unsupported_body: 'Duas ou mais YubiKey 5 NFC, mais um telefone que consiga lê-las.',
      vault_restore_from_chain: 'Restaurar o cofre a partir da blockchain',
      vault_restore_scanning: 'Analisando a blockchain…',
      vault_restore_found: 'Encontrado(s) {{count}} depósito(s) do cofre na blockchain.',
      vault_restore_pending:
        '{{count}} depósito(s) encontrado(s), aguardando a primeira confirmação — tente novamente em breve.',
      vault_restore_none_found: 'Nenhum depósito do cofre foi encontrado na blockchain para esta carteira.',
      vault_balance_label: 'o cofre tem',
      vault_key_section: 'Chaves de segurança ({{count}} de 5)',
      vault_manage_section: 'Gerenciar',
      vault_deposit_cta: 'Depositar',
      vault_withdraw_cta: 'Retirar',
      vault_deposit_title: 'Depositar no cofre',
      vault_withdraw_title: 'Retirar do cofre',
      vault_deposit_sub: 'Do seu saldo do dia a dia. Não precisa de YubiKey.',
      vault_withdraw_sub: 'De volta ao seu saldo do dia a dia. Encoste qualquer uma das suas chaves do cofre.',
      vault_deposit_done: 'Movido para o cofre',
      vault_reclaim_done: 'Fundos recuperados de um depósito interrompido no cofre',
      vault_withdraw_done: 'Retirado do cofre',
      vault_withdraw_partial: 'Restam {{count}} depósitos — saque de novo para movê-los.',
      vault_withdraw_reason: 'Encoste a sua YubiKey aqui para assinar — retirar {{amount}} sats do cofre',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: 'Ainda não disponível — os depósitos no cofre estão desativados nesta versão.',
      vault_not_on_mainnet_body:
        'O cofre só está disponível na rede principal. Volte a colocar a carteira na rede principal para o utilizar.',
      vault_intro_title: 'Configure o seu cofre',
      vault_intro_what:
        'O cofre tranca o dinheiro com as suas próprias YubiKeys. Nada da chave do cofre existe neste telefone; qualquer uma das suas chaves o abre.',
      vault_intro_two_keys: 'Você precisa de pelo menos duas YubiKeys.',
      vault_intro_apart: 'Guarde-as em lugares diferentes — duas chaves guardadas juntas são uma só.',
      vault_intro_backup:
        'Mantenha o backup criptografado da carteira ativado — ele preserva o registro de recuperação autenticado.',
      vault_intro_ack:
        'Eu entendo: só as minhas YubiKeys abrem este cofre. A minha frase de recuperação não. Se eu perder todas, o dinheiro se perde.',
      vault_intro_piv_ack:
        'Confirmo que toda a aplicação PIV desta YubiKey foi reposta de fábrica ou é dedicada ao cofre. A configuração altera o PIN, o PUK e a chave de gestão PIV globais, pode afetar todos os slots PIV, e guardarei o novo PUK em segurança.',
      vault_intro_begin: 'Começar',
      vault_key_step_title: 'Chave {{k}} de até 5',
      vault_key_step_replace:
        'Use uma YubiKey reposta de fábrica e dedicada ao cofre. Se a ranhura do cofre contiver uma chave antiga, a configuração avisará antes de a substituir.',
      vault_pin_choose_title: 'Escolha um PIN',
      vault_pin_choose_sub: 'Vai introduzi-lo sempre que usar esta chave. 6 a 8 dígitos.',
      vault_pin_confirm_label: 'Introduza-o novamente',
      vault_pin_mismatch: 'Os dois PIN não coincidem.',
      vault_pin_not_default: 'Escolha algo diferente de 123456.',
      vault_puk_title: 'O seu código de recuperação',
      vault_puk_body:
        'Anote-o e guarde-o num local seguro. É a única forma de desbloquear esta chave se esquecer o PIN, e não voltará a ser mostrado.',
      vault_puk_ack: 'Já o anotei',
      vault_reset_offer: 'Repor esta chave',
      vault_reset_title: 'Repor esta chave?',
      vault_reset_body:
        'Esta chave já foi usada. Repô-la apaga todos os certificados e chaves da sua aplicação PIV, não apenas a ranhura do cofre. Tudo o que depende desta chave deixa de funcionar.',
      vault_reset_ack: 'Compreendo que isto apaga tudo nesta chave',
      vault_reset_unknown_ack:
        'Este dispositivo não pode confirmar se esta chave ainda protege o cofre de outro perfil. Compreendo que apagá-la pode destruir essa chave para sempre.',
      vault_reset_own_draft_ack:
        'Esta chave contém a chave inacabada da sua tentativa anterior, e este dispositivo ainda guarda o respetivo registo. Compreendo que repô-la apaga-a e que essa tentativa não poderá ser retomada.',
      vault_reset_nothing_erased:
        'Nada foi apagado. A ranhura do cofre desta chave não está vazia, por isso falta confirmar mais uma coisa.',
      vault_reset_confirm: 'Apagar e repor',
      vault_resetting: 'A repor a chave…',
      vault_reset_done: 'Chave reposta. Aproxime-a novamente para continuar.',
      vault_reset_uncertain:
        'A reposição não terminou. Esta chave pode já ter sido apagada — verifique-a antes de confiar nela.',
      vault_err_reset_enrolled:
        'Essa chave já faz parte do seu cofre — repô-la destruiria uma das suas chaves do cofre.',
      vault_err_reset_unreadable:
        'Não foi possível ler os registos do seu cofre neste dispositivo, por isso não há como provar que esta chave não é uma das suas chaves do cofre. Nada foi apagado e a reposição é recusada.',
      vault_enrollment_resume_pin_hint:
        'Retomar uma chave guardada exige o PIN que escolheu para ela antes, não um novo. Três tentativas erradas bloqueiam a chave.',
      vault_setup_step: 'Passo {{n}} de {{total}}',
      vault_enrollment_resume: 'Retomar chave guardada',
      vault_enrollment_reset_required:
        'Esta YubiKey já não está no estado de fábrica, por isso não pode ser inscrita tal como está. A sua aplicação PIV tem de ser apagada e reposta de fábrica antes de poder guardar uma chave do cofre.',
      vault_enrollment_state_uncertain:
        'Uma tentativa de configuração foi interrompida, por isso não há como saber em que estado a YubiKey ficou. Essa chave não voltará a ser usada até que a sua aplicação PIV seja apagada e reposta de fábrica.',
      vault_nfc_enroll_message: 'Encoste a sua YubiKey aqui para configurá-la',
      vault_nfc_adopt_message: 'Encoste esta YubiKey recuperada aqui para verificá-la',
      vault_nfc_sign_batch: 'Encoste a sua YubiKey aqui para assinar — lote {{b}} de {{n}}',
      vault_key_use_different: 'Usar outra YubiKey',
      vault_key_setup_again: 'Configurar de novo',
      vault_name_default: 'Chave {{k}}',
      vault_more_title: 'Adicionar outra chave?',
      vault_more_body: 'Até 5 chaves. Qualquer uma delas abre o cofre inteiro sozinha.',
      vault_more_add: 'Adicionar outra chave',
      vault_more_finish: 'Concluir',
      vault_more_need_two: 'Adicione uma segunda chave antes de concluir — uma só chave significa nenhuma recuperação.',
      vault_leave_setup: 'Sair da configuração',
      vault_leave_title: 'Sair da configuração?',
      vault_leave_body:
        'As {{count}} YubiKey(s) que você configurou ainda não serão salvas. Elas mantêm as suas chaves, então você pode adicioná-las de novo em um minuto.',
      vault_leave_confirm: 'Sair',
      vault_leave_stay: 'Ficar',
      vault_done_body:
        '{{count}} chaves podem abrir este cofre. Só estas chaves o abrem — a sua frase de recuperação não.',
      vault_done_cta: 'Concluído',
      vault_add_key_row: 'Adicionar chave',
      vault_add_key_done:
        '{{nickname}} pode abrir os depósitos feitos a partir de agora. Retranque o cofre para que ela possa abrir tudo.',
      vault_key_added_toast: 'Chave adicionada',
      vault_recovery_verified: 'Chave recuperada verificada neste dispositivo',
      vault_relock_row: 'Retrancar o cofre',
      vault_relock_now: 'Retrancar agora',
      vault_relock_choose: 'Qual chave você vai encostar para retrancar?',
      vault_relock_reason: 'Encoste uma das suas chaves existentes ({{names}}) — não a que você acabou de adicionar.',
      vault_relock_reason_generic:
        'Encoste a sua YubiKey aqui para assinar — retrancar o cofre com as suas chaves atuais',
      vault_relock_reason_remaining: 'Bloqueie novamente apenas com as suas chaves restantes: {{names}}.',
      vault_key_pending_removal: 'Removida — bloqueie novamente para concluir',
      vault_key_removal_awaiting_network: 'Removendo — aguardando a rede aceitar o novo bloqueio',
      vault_removal_awaiting_network: 'Novo bloqueio enviado. Depósitos e retiradas voltam a funcionar assim que a rede o aceitar.',
      vault_err_too_many_active_keys: 'O cofre é bloqueado com no máximo cinco chaves. Remova uma antes de depositar, bloquear novamente ou retirar parte do saldo.',
      vault_key_section_over_limit: 'Chaves de segurança ({{count}}, remova uma)',
      vault_add_key_done_over_limit: '{{nickname}} foi adicionada. O cofre é bloqueado com no máximo cinco chaves: remova a chave que ela substitui e bloqueie novamente uma vez.',
      vault_relock_done: 'Cofre retrancado com as suas chaves atuais',
      vault_relock_capped: 'Restam {{count}} depósitos — retranque de novo para movê-los.',
      vault_relock_unreachable:
        '{{count}} depósitos só podem ser abertos por {{names}}. Retranque de novo com uma dessas chaves.',
      vault_badge_missing: '{{count}} depósitos ainda não abertos para {{nickname}}',
      vault_badge_removed: '{{count}} depósitos ainda abertos para uma chave removida',
      vault_key_action_rename: 'Renomear',
      vault_key_action_remove: 'Remover',
      vault_rename_title: 'Renomear {{nickname}}',
      vault_rename_save: 'Salvar',
      vault_remove_title: 'Remover {{nickname}}?',
      vault_remove_body:
        'A carteira deixará de usar {{nickname}}. O dinheiro que já está no cofre continua podendo ser aberto por ela até você retrancar (≈ {{fee}} sats).',
      vault_remove_only: 'Só remover',
      vault_key_removed_toast: 'Chave removida',
      vault_export_explainer:
        'Cada depósito no cofre esconde o seu salt até que você o gaste — nada disso é visível no script de bloqueio. Mantenha o backup criptografado ativado para que um novo dispositivo possa restaurar e autenticar os registros de chaves necessários para usar suas YubiKeys.',
      vault_footnote: 'Só estas chaves abrem o cofre. A sua frase de recuperação não.',
      vault_floor_line:
        'Depósito mínimo {{floorDisplay}} ({{floorSats}} sats). Criar um depósito no cofre custa cerca de {{feeDisplay}}.',
      vault_first_deposit_title: 'Primeiro depósito no cofre',
      vault_first_deposit_body:
        'Primeiro depósito no cofre — {{amount}} só poderá ser aberto com {{count}} YubiKeys ({{names}}). A sua frase de recuperação não vai ajudar.',
      vault_backup_off_title: 'Ative o backup privado',
      vault_backup_off_body:
        'O backup privado deve estar ativo antes que a carteira crie outra saída do cofre. Ele guarda os registros autenticados das YubiKeys e o histórico da carteira necessários para a recuperação. Você ainda pode sacar tudo sem criar troco no cofre.',
      vault_backup_off_cta: 'Abrir configurações de backup',
      vault_seed_not_preserved_title: 'Salve primeiro sua frase de recuperação',
      vault_seed_not_preserved_body:
        'Seu Cofre só pode ser recuperado com sua frase de recuperação mais uma de suas YubiKeys. As YubiKeys não guardam nenhuma cópia dela — se ela for perdida antes de você anotá-la, esses fundos não poderão ser recuperados. Salve-a agora antes de depositar.',
      vault_seed_not_preserved_cta: 'Salvar frase de recuperação',
      vault_choose_key: 'Qual chave você vai encostar?',
      vault_remainder_title: 'Sacar tudo?',
      vault_remainder_body:
        'Sacar {{amount}} deixa {{remainder}}, abaixo do mínimo de 100.000 sats do cofre. O cofre inteiro vai para o seu saldo do dia a dia.',
      vault_remainder_all: 'Sacar tudo',
      vault_remainder_change: 'Alterar valor',
      vault_unreachable_title: 'Parte do cofre precisa de outra chave',
      vault_unreachable_body:
        'Parte do cofre precisa de outra chave — {{moved}} movidos. {{count}} depósitos com {{amount}} só podem ser abertos por {{names}}. Saque de novo com uma dessas chaves.',
      vault_sign_progress: '{{signed}} de {{total}} assinados',
      vault_err_pin_locked_enroll:
        'O PIN desta YubiKey está bloqueado. Desbloqueie-o com o PUK no Yubico Authenticator, ou configure com outra YubiKey.',
      vault_err_serial_mismatch_chosen:
        'Essa é {{tappedName}}. Você escolheu {{chosenName}} — encoste-a, ou volte e escolha {{tappedName}}.',
      vault_err_key_already_enrolled: 'Você já adicionou esta YubiKey ({{nickname}}). Encoste outra.',
      vault_err_not_released: 'Os depósitos no cofre estão desativados nesta versão.',
      vault_err_not_on_mainnet:
        'O cofre só está disponível na rede principal. Mude a carteira para a rede principal e tente novamente.',
      vault_err_backup_off:
        'Ative o backup privado criptografado antes de criar outra saída do cofre. Você ainda pode sacar tudo.',
      vault_err_not_enough_keys: 'Este cofre tem menos de duas chaves. Configure-o de novo.',
      vault_err_too_many_keys: 'Um cofre aceita no máximo cinco chaves.',
      vault_err_last_keys: 'Um cofre precisa de pelo menos duas chaves. Adicione outra antes de remover esta.',
      vault_err_relock_required:
        'Retranque o cofre primeiro para que as suas outras chaves possam abrir cada depósito.',
      vault_err_action_pending: 'Uma transação do cofre ainda não terminou. Tente novamente quando ela concluir.',
      vault_resolve_held_deposit_action: 'Concluir o depósito interrompido',
      vault_resolve_held_deposit_done: 'Depósito resolvido',
      vault_resolve_held_deposit_failed: 'Não foi possível resolver o depósito. Tente novamente.',
      vault_resolve_held_deposit_none:
        'Não há nada a resolver aqui — verifique a aba Atividade para repetir ou cancelar uma transação pendente.',
      vault_err_key_not_committed:
        '{{nickname}} não consegue abrir nenhum dos depósitos deste cofre. Use outra das suas chaves.',
      vault_err_key_cannot_cover:
        '{{nickname}} consegue abrir {{reachable}} dos {{total}} do cofre. Saque até {{reachable}}, ou use {{otherNames}}.',
      vault_err_too_small_to_relock:
        'Este cofre tem menos de 100.000 sats, pouco demais para retrancar. Saque-o e deposite de novo.',
      vault_err_bad_version: 'A transação foi montada no formato errado. Nada foi assinado — tente de novo.',
      vault_err_chain_scan_failed:
        'Não foi possível concluir a varredura da blockchain em busca de depósitos do cofre. Verifique sua conexão e tente novamente.',
      vault_shares_unavailable: 'Não há material de chave disponível para imprimir.',
      vault_enroll_begin: 'Configurar cofre',
      vault_enroll_phase_connecting: 'Lendo a sua chave…',
      vault_enroll_phase_pin_check: 'Verificando a sua chave…',
      vault_enroll_phase_checking_slot: 'A verificar a ranhura do cofre…',
      vault_enroll_phase_personalizing: 'A definir o seu PIN e código de recuperação…',
      vault_enroll_phase_generating: 'Criando a chave do cofre — toque na chave quando ela piscar',
      vault_enroll_phase_challenging: 'Verificando a nova chave do cofre — toque nela quando piscar',
      vault_enroll_phase_done: 'Concluído',
      vault_touch_when_blinks: 'Toque no contato dourado quando a chave piscar.',
      vault_nfc_activation_hint:
        'Primeira vez com esta YubiKey? Ligue-a primeiro a uma porta USB-C por alguns segundos — as chaves novas vêm com o NFC desativado até então.',
      vault_continue: 'Continuar',
      vault_back: 'Voltar',
      vault_enrolled_toast: 'Cofre pronto',
      vault_insert_key: 'Insira a sua YubiKey',
      vault_hold_key_nfc: 'Encoste a sua YubiKey ao topo do telemóvel',
      vault_keep_holding_nfc: 'Mantenha encostada — a desbloquear…',
      vault_unlocking_funds: 'A desbloquear os seus fundos',
      vault_unlocking_sub: 'Isto demora alguns segundos. Mantenha a aplicação aberta.',
      vault_sending_to_network: 'A enviar para a rede',
      vault_sending_sub: 'Se não passar de imediato, continuaremos a tentar.',
      vault_reading_key: 'Lendo a chave…',
      vault_enter_pin: 'Introduza o PIN da YubiKey',
      vault_pin_sub_nfc: 'Depois encoste a sua chave ao topo do telemóvel para terminar.',
      vault_pin_sub_usb: 'Depois toque no contacto dourado para terminar.',
      vault_touch_contact: 'Toque no contato dourado',
      vault_unlock_cta: 'Desbloquear',
      vault_pin_retries: '{{count}} tentativas restantes',
      vault_locked: 'Cofre bloqueado',
      vault_retry: 'Tentar novamente',
      vault_cancel: 'Cancelar',
      vault_dismiss: 'Dispensar',
      vault_err_wrong_key: 'Esta YubiKey não é uma das chaves deste cofre.',
      vault_err_serial_mismatch: 'Esta YubiKey não é uma das chaves deste cofre ({{names}}).',
      vault_err_touch_timeout: 'Nenhum toque detectado. Tente novamente e toque no contato dourado.',
      vault_err_pin_locked:
        'O PIN desta YubiKey está bloqueado. Use outra das suas chaves do cofre, ou desbloqueie esta com o PUK no Yubico Authenticator.',
      vault_err_pin_invalid: 'PIN incorreto.',
      vault_err_puk_invalid: 'PUK incorreto.',
      vault_err_puk_locked: 'O PUK desta YubiKey está bloqueado. Use uma YubiKey nova.',
      vault_err_ceremony_active:
        'Outra operação do cofre ainda está usando a YubiKey. Aguarde a conclusão e tente novamente.',
      vault_err_scope_changed: 'A carteira ou a rede mudou durante esta operação. Recomece na carteira atual.',
      vault_err_key_removed_mid_op: 'A chave foi removida. Insira-a e tente novamente.',
      vault_err_no_key: 'Nenhuma YubiKey detectada.',
      vault_err_driver_unavailable: 'O suporte a YubiKey não está disponível neste dispositivo.',
      vault_err_mgmt_key_custom:
        'Esta chave tem uma chave de gerenciamento personalizada e não pode ser usada automaticamente.',
      vault_err_attestation_invalid:
        'Não foi possível verificar o atestado do fabricante desta YubiKey. Use uma YubiKey autêntica, atestada de fábrica e com o slot de atestado PIV intacto.',
      vault_err_generic: 'Algo deu errado. Tente novamente.',
      vault_err_template_invalid: 'Algo correu mal ao tentar acessar o cofre. Tente novamente.',
      vault_err_not_enrolled: 'Nenhum cofre está configurado.',
      vault_err_pin_required: 'Digite o seu PIN.',
      vault_err_unsupported_platform: 'O suporte a YubiKey não está disponível neste dispositivo.',
      vault_err_slot_occupied: 'O slot da chave já está em uso.',
      vault_replace_key_warning:
        'Substituí-la destrói permanentemente a chave privada P-256 existente. Os fundos do cofre que dependem dessa chave podem ficar impossíveis de gastar.',
      vault_replace_key_confirm: 'Substituir a chave existente',
      vault_resume_draft_confirm_title: 'Usar esta chave preparada?',
      vault_resume_draft_confirm_body:
        'Uma chave preparada anteriormente — {{nickname}} · …{{tail}} — está pronta para ser adicionada, mas este dispositivo não acabou de verificá-la em um cartão. Continue apenas se você a reconhecer.',
      vault_resume_draft_confirm_use: 'Usar esta chave',
      vault_resume_draft_confirm_discard: 'Descartá-la',
      vault_err_enrollment_partial:
        'A configuração da YubiKey parou depois de alterá-la. Siga as instruções para tentar novamente; esta chave não foi adicionada ao cofre.',
      vault_err_key_not_adopted:
        'Verifique que possui esta YubiKey recuperada neste dispositivo antes de usá-la para retirar.',
      vault_err_too_many_inputs:
        'Depósitos demais no cofre para mover de uma vez. Retire um valor menor primeiro — cada retirada também consolida o cofre, então a próxima poderá mover mais.',
      vault_err_requires_online:
        'Transferências do cofre precisam de conexão. Reconecte e tente de novo — os fundos do cofre nunca ficam na fila.',
      vault_err_user_cancelled: 'Cancelado.',
      vault_err_vault_empty: 'O cofre ainda não tem fundos para retirar.',
      vault_err_amount_exceeds_balance: 'Isso é mais do que o cofre contém.',
      vault_err_below_dust: 'Esse montante é demasiado pequeno para enviar.',
      vault_err_no_transaction: 'Não foi possível concluir a transferência. Tente novamente.',
      vault_err_nfc_lost: 'A ligação caiu. Mantenha a chave firme contra o telemóvel.',
      vault_pin_invalid_retry:
        'PIN errado — restam {{count}} tentativas. Toque em Retirar novamente para tentar outra vez.',
      vault_pin_invalid_retry_generic: 'PIN errado. Toque em Retirar novamente para tentar outra vez.',
      vault_disable_row: 'Desativar cofre',
      vault_disable_title: 'Desativar cofre?',
      vault_disable_message:
        'Isto esquece a lista de chaves do cofre neste telefone. As chaves permanecem nas suas YubiKeys.',
      vault_disable_confirm: 'Desativar',
      vault_disabled_toast: 'Cofre desativado',
      vault_disable_blocked_title: 'Retire os fundos primeiro',
      vault_disable_blocked_message:
        'O cofre ainda tem fundos. Retire tudo para o seu saldo do dia a dia antes de o desativar — caso contrário, esses fundos ficariam bloqueados sem forma de os assinar.',
      vault_ok: 'OK',
      vault_mock_toggle: 'Usar YubiKey simulada (dev)',
      vault_on: 'Ativado',
      vault_off: 'Desativado',
      vault_shares_word_count:
        'As partes de recuperação só suportam frases de doze palavras. Escreva antes a sua frase.',
      scan_shares_legacy_title: 'Formato de backup antigo',
      scan_shares_legacy_message:
        'Estas partes têm um formato antigo que restaura o seu saldo do dia a dia mas não consegue abrir um cofre. Se ainda tiver a sua frase de recuperação de doze palavras, restaure a partir dela — recupera tudo — e imprima partes novas depois. Se a frase se perdeu, envie os seus fundos para uma carteira nova e comece de novo.',
      scan_shares_legacy_ack: 'Compreendo',

      delete_wallet: 'Excluir carteira',
      delete_wallet_warning_title: 'Excluir carteira?',
      delete_wallet_warning_body:
        'Isso excluirá permanentemente as chaves da sua carteira deste dispositivo. Se você não fez backup da sua frase de recuperação ou imprimiu as partes de recuperação, você perderá o acesso à sua carteira para sempre.',
      delete_wallet_confirm: 'Excluir carteira',
      delete_wallet_failed_title: 'Falha ao excluir carteira',
      delete_wallet_failed_body: 'Não foi possível excluir totalmente sua carteira deste dispositivo. Parte do material da chave pode ainda estar presente. Tente novamente.',
      bsv_network: 'Rede BSV',
      arc_endpoint: 'Endpoint ARC',
      arc_api_token: 'Token de API',
      arc_custom_url: 'URL personalizado',
      arc_apply: 'Aplicar',
      arc_reset_default: 'Repor predefinição',
      arc_requires_token: 'requer token',
      arc_url_https_required: 'Deve ser https (ou http://localhost para testes locais)',
      recovery_phrase: 'Chave de recuperação',
      you_have: 'Você tem',
      switching: 'Alterando...',
      mainnet: 'Rede principal',
      testnet: 'Rede de teste',
      teratest: 'Teratest',
      identity_key: 'Identificador',
      transactions: 'Transações',

      no_transactions: 'Nenhuma transação ainda',
      tx_status_confirmed: 'Confirmado',
      tx_status_received: 'Recebido',
      tx_status_sent: 'Enviado',
      tx_status_spent: 'Gasto',
      tx_status_transferred: 'Transferido',
      tx_status_accepted: 'Aceita',
      tx_status_broadcasting: 'Processando',
      tx_status_not_sent: 'Não enviado',
      tx_status_unsigned: 'Rascunho',
      tx_status_nonfinal: 'Pendente',
      tx_status_pending: 'Pendente',
      tx_status_failed: 'Falhou',
      tx_status_offline_queued: 'Offline · em fila',
      tx_status_offline_sending: 'Offline · enviando',
      tx_status_offline_rejected: 'Offline · rejeitado',
      tx_copied: 'Transação copiada',
      tx_not_available: 'Transação não disponível',
      tx_copy_failed: 'Falha ao copiar transação',
      tx_abort_success: 'Transação abortada',
      tx_abort_failed: 'Falha ao abortar transação',
      tx_abort_maybe_delivered: 'Não é possível cancelar — este pagamento pode já estar com o destinatário',
      tx_proof_refreshed: 'Transação confirmada',
      tx_proof_refresh_failed: 'Não foi possível obter a prova — a transação pode ainda não estar confirmada',
      tx_marked_failed: 'Não processada — marcada como falhada e fundos libertados',
      tx_still_pending: 'Ainda pendente — ainda não confirmada',
      tx_action_explorer: 'Ver no explorador de blocos',
      tx_export_success: '{{count}} transações exportadas',
      tx_export_failed: 'Falha ao exportar as transações',
      tx_export_csv: 'Exportar CSV',
      tx_export: 'Exportar',
      activity_filter: 'Filtrar atividade',
      activity_filter_all: 'Tudo',
      activity_search_placeholder: 'Pesquisar notas, nomes, @apelidos',
      activity_search_clear: 'Limpar pesquisa',
      activity_filter_no_match: 'Nenhuma atividade correspondente',
      wallet_balance_label: 'Saldo',
      wallet_balance_you_have: 'Você tem',
      wallet_balance_refresh: 'Atualizar saldo',
      wallet_group_today: 'Hoje',
      wallet_group_yesterday: 'Ontem',
      back: 'Voltar',
      wallet_payments: 'Pagamentos',
      wallet_vault: 'Cofre',
      wallet_settings: 'Configurações',
      wallet_activity: 'Atividade',

      message_box_server: 'Servidor de Caixa de Mensagens',
      message_box_required: 'Necessário para enviar e receber pagamentos baseados em identidade.',
      send_payment: 'Enviar pagamento',
      recipient: 'Destinatário',
      amount: 'Valor',
      enter_valid_amount: 'Insira um valor maior que zero',
      display_currency: 'Moeda de exibição',
      no_pending_payments: 'Nenhum pagamento pendente',
      outgoing_payments: 'Saídas',
      payment_not_delivered: 'Não entregue',
      payment_delivered: 'Entregue',
      retry: 'Tentar novamente',
      retry_failed: 'Erro ao tentar novamente',
      dismiss: 'Descartar',
      valid_identity_key: 'Identificador válido inserido',
      valid_bsv_address: 'Endereço BSV válido inserido',
      searching: 'Pesquisando...',
      unknown: 'Desconhecido',
      from_prefix: 'De:',
      message_box_saved: 'Servidor de caixa de mensagens salvo',
      message_box_removed: 'Servidor de caixa de mensagens removido',
      message_box_off: 'Desligado',
      message_box_off_hint: 'O servidor de caixa de mensagens está desligado. Ative-o em Configurações › Avançado.',
      enter_valid_url: 'Por favor, insira uma URL válida',
      checking_connection: 'Verificando conexão...',
      connection_failed: 'Falha na conexão',
      recipient_placeholder: 'Nome, identificador ou endereço BSV...',

      trust_updated: 'Rede de confiança atualizada',
      failed_to_save: 'Falha ao salvar',
      confirm_delete: 'Confirmar exclusão',
      confirm_delete_body: 'Tem certeza de que deseja excluir este certificador?',
      order_certifiers_hint:
        'Ordene os certificadores por prioridade. Os aplicativos exibirão os certificadores de maior ranking primeiro.',
      certifiers: 'Certificadores',
      no_certifiers: 'Nenhum certificador ainda.',
      add_provider: 'Adicionar provedor',
      unsaved_changes: 'Você tem alterações não salvas',
      duplicate_key_error: 'Uma entidade com esta chave pública já está na lista!',
      get_provider_details: 'Obter detalhes do provedor',
      validate_details: 'Validar detalhes',
      show_advanced: 'Mostrar avançado',
      hide_advanced: 'Ocultar avançado',
      description_length_error: 'A descrição deve ter entre 5 e 50 caracteres',
      enter_domain_hint: 'Digite o domínio do provedor',
      enter_details_hint: 'Digite os detalhes do provedor',

      receive: 'Receber',
      send: 'Enviar',
      generating_address: 'Gerando endereço BSV...',
      imported: 'Importado',
      unable_to_generate_address: 'Não é possível gerar o endereço BSV',
      send_bsv: 'Enviar BSV',
      recipient_address: 'Endereço BSV do destinatário',
      enter_bsv_address: 'Digite o endereço BSV',

      wallet_data: 'Dados da carteira',
      create_new_wallet: 'Criar nova carteira',
      generate_recovery_phrase_caption: 'Gerar uma nova chave de recuperação',
      import_existing_wallet: 'Importar carteira existente',
      paste_recovery_phrase: 'Cole sua chave de recuperação',
      save_recovery_phrase_heading: 'Salve sua chave de recuperação',
      save_recovery_phrase_btn: 'Salvar chave de recuperação',
      recovery_phrase_write_down: 'Anote essas 12 palavras em ordem e guarde-as em um lugar seguro.',
      acknowledgment_text:
        'Salvei minha chave de recuperação e entendo que perdê-la resultará na perda total e permanente de todos os fundos, tokens e certificados associados.',
      enter_recovery_words: 'Introduza a frase de 12 palavras',
      restore_wallet_description: 'Digite sua chave de recuperação para restaurar sua carteira',
      backup_reminder_title: 'A sua carteira não tem backup',
      backup_reminder_body: 'Exporte as suas chaves para evitar a perda de fundos.',
      biometric_advisory_title: 'Proteja a sua carteira',
      wallet_balance_show_in: 'Mostrar em {{unit}}',
      tx_detail_block: 'Bloco',
      tx_detail_block_pending: 'À espera de um bloco',
      biometric_advisory_body: 'Proteja os seus ativos com o Face ID ou a sua impressão digital',
      biometric_advisory_body_degraded:
        'Este dispositivo não tem Face ID ou impressão digital configurados, por isso a chave da sua carteira está protegida apenas pelo código do dispositivo.',
      import_prompt_title: 'Já tem uma carteira?',
      import_prompt_body: 'Restaure-a a partir da sua chave de recuperação ou das partes.',
      import_prompt_action: 'Importar de um backup',
      nearby_advisory_title: 'A ligar por proximidade',
      nearby_advisory_body:
        'Ser-lhe-á pedido que autorize o acesso a Wi-Fi, Bluetooth ou NFC. Isto serve apenas para o ligar diretamente a um dispositivo próximo para o pagamento — nunca é enviado para os nossos servidores nem usado para criar um perfil seu.',
      backup_reminder_now: 'Fazer backup das chaves agora',
      backup_reminder_import: 'Importar de um backup',
      restore_backup_checking: 'Procurando seu backup criptografado…',
      restore_backup_progress: 'Restaurando o histórico — {{chunks}} de {{total}}…',
      restore_backup_failed_title: 'Não foi possível restaurar o histórico',
      restore_backup_failed_message:
        'Sua frase de recuperação está correta, mas o backup criptografado não pôde ser reproduzido. Normalmente basta tentar de novo. Continuar sem ele dá uma carteira funcional sem transações anteriores, e algumas das suas moedas atuais não serão gastáveis.',
      restore_backup_retry: 'Tentar novamente',
      restore_backup_skip: 'Abandonar',
      restore_backup_unverified_title: 'O backup pode estar incompleto',
      restore_backup_unverified_body:
        'Sua carteira foi restaurada, mas não foi possível confirmar que o backup no servidor está completo. Se faltar algum histórico, restaure novamente mais tarde, depois que o outro dispositivo terminar o envio.',
      import_wallet: 'Importar carteira',

      reject: 'Rejeitar',
      authorize: 'Autorizar',
      renewal: 'Renovação',
      spending_section: 'Gastos',
      protocols_section: 'Protocolos',
      baskets_section: 'Cestas',
      certificates_section: 'Certificados',
      requested_fields: 'Campos solicitados',

      send_max: 'Enviar máximo',
      message_box_unreachable:
        'Não foi possível conectar ao servidor de pagamentos. Verifique o URL da message box nas configurações.',
      available: 'disponível',
      note: 'Nota',
      note_placeholder: 'Para que é? (opcional)',
      entire_wallet_balance: 'Saldo total da carteira',
      entire_vault_balance: 'Todo o saldo do cofre',
      export_wallet_data: 'Exportar dados da carteira',
      export_unencrypted_title: 'Este arquivo não está criptografado',
      export_unencrypted_message:
        'O arquivo exportado é uma cópia simples e não criptografada dos dados da sua carteira — incluindo informações de identidade e certificados, histórico de transações e contatos. Qualquer pessoa que obtiver este arquivo poderá lê-lo. Compartilhe-o apenas em um lugar de confiança.',
      import_wallet_data: 'Importar dados da carteira',
      import_conflict_title: 'Banco de dados já existe',
      import_conflict_message:
        'Já existe um banco de dados de carteira para esta identidade com um timestamp mais recente. O backup com o número mais alto no nome do arquivo é sempre utilizado.\n\nPara usar este backup antigo: exporte seus dados atuais, exclua e reinstale o app, depois importe apenas o backup desejado.',
      import_confirm_title: 'Importar banco de dados',
      import_confirm_message: 'Este banco de dados se tornará sua carteira ativa. Continuar?',
      import_success: 'Banco de dados importado com sucesso. Reiniciando carteira...',
      import_invalid_file: 'Arquivo de backup inválido',
      import_invalid_file_detail:
        'Este arquivo não é um backup válido de banco de dados. Nome de arquivo esperado como wallet-XXXXXXXX-mainnet-TIMESTAMP.db.',
      import_oversized_file: 'Arquivo de backup muito grande',
      import_oversized_file_detail:
        'Este arquivo é maior do que um backup de carteira deveria ser, por isso não foi lido. Escolha um arquivo de backup de banco de dados de carteira genuíno.',
      import_untrusted_file: 'Não é possível importar este backup',
      import_untrusted_file_detail:
        'Não foi possível verificar se este backup pertence a esta carteira, ou seu conteúdo não pôde ser confiável. Não foi importado.',
      import_anyway: 'Importar',
      import_invalid_input_title: 'Entrada inválida',
      import_invalid_input_message:
        'Insira uma frase de recuperação válida (12–24 palavras) ou uma chave privada hexadecimal de 64 caracteres.',
      import_setup_failed: 'Falha ao configurar a carteira: {{error}}',
      create_wallet_refused: 'Não foi possível criar a carteira. Tente novamente.',
      create_wallet_failed: 'Falha ao gerar a frase mnemônica. Tente novamente.',
      or: 'ou',
      print_recovery_shares: 'Imprimir Partes de Recuperação',
      scan_backup_shares: 'Escanear Partes de Backup',
      scan_backup_shares_caption: 'Recuperar carteira a partir das partes impressas',
      scan_shares_invalid_format: 'Formato de parte inválido. Escaneie um código QR de parte de backup válido.',
      scan_shares_camera_needed: 'Acesso à Câmera Necessário',
      scan_shares_camera_description:
        'O acesso à câmera é necessário para escanear os códigos QR das suas partes de backup impressas.',
      scan_shares_grant_camera: 'Conceder Acesso à Câmera',
      scan_shares_recovering: 'Recuperando carteira...',
      scan_shares_scan_first: 'Escaneie sua primeira parte de backup',
      scan_shares_progress: '{{scanned}} de {{needed}} partes escaneadas',
      scan_shares_remaining: '{{count}} parte a mais necessária',
      scan_shares_complete: 'Todas as partes coletadas!',
      scan_shares_recovery_failed: 'A recuperação falhou. Tente escanear novamente.',
      scan_shares_threshold_mismatch: 'O limite não corresponde às partes anteriores',
      scan_shares_integrity_mismatch: 'O hash de integridade não corresponde — as partes são de chaves diferentes',
      scan_shares_duplicate: 'Esta parte já foi escaneada',
      scan_shares_biometric_title: 'Acesso biométrico necessário',
      scan_shares_biometric_message:
        'É necessário acesso biométrico para proteger as chaves da sua carteira. Tente novamente.',
      recovery_replace_wallet_title: 'Substituir a carteira deste telefone?',
      recovery_replace_wallet_body:
        'Este telefone já tem uma carteira. Continuar vai substituí-la e tudo o que ela pode gastar. Se a frase de recuperação dela não estiver guardada em outro lugar, esse dinheiro estará perdido para sempre.',
      recovery_replace_wallet_confirm: 'Substituir carteira',
      invalid_bsv_address: 'Endereço BSV inválido',
      scan_recipient_hint: 'Aponte a câmera para um código de pagamento',
      scan_qr_default_hint: 'Aponte a câmera para um código QR',
      scan_wallet_qr_hint: 'Aponte a câmera para um código QR de emparelhamento',

      biometric_store_wallet: 'Armazene sua carteira com segurança',
      biometric_load_wallet: 'Carregue sua carteira com segurança',
      biometric_unlock_wallet: 'Desbloqueie sua carteira',
      wallet_lost_title: 'Chave da carteira destruída',
      wallet_lost_body:
        'Este dispositivo alterou suas configurações biométricas, então o sistema destruiu a chave que protegia esta carteira. Suas moedas estão seguras na blockchain — restaure esta carteira com sua frase de recuperação.',
      wallet_lost_restore: 'Restaurar',
      wallet_lost_later: 'Agora não',
      wallet_locked_title: 'Carteira bloqueada',
      wallet_locked_body: 'Desbloqueie para carregar as chaves da sua carteira.',
      wallet_unlock_retry: 'Desbloquear carteira',
      wallet_lockout_body: 'Muitas tentativas malsucedidas. Desbloqueie seu telefone com a senha e tente novamente.',
      wallet_no_biometrics_body:
        'Este dispositivo não tem biometria configurada. Suas chaves continuam criptografadas pelo keystore do dispositivo, mas não são protegidas por uma verificação biométrica.',

      // Connections screen
      connections: 'Conexões',
      advanced: 'Avançado',
      copy_secret_words: 'Copiar palavras secretas',
      print_recovery_keys: 'Imprimir chaves de recuperação',
      backup_wallet_keys: 'Fazer backup das chaves da carteira',
      log_out: 'Sair',
      connect_to_app: 'Conectar ao app',
      no_connections_yet: 'Nenhuma conexão ainda',
      no_connections_subtitle: 'Digitalize um código QR de um app de desktop para conectar sua carteira',
      connections_active: 'Ativo',
      connections_disconnected: 'Desconectado',
      disconnect: 'Desconectar',
      reconnect: 'Reconectar',
      reconnect_failed: 'Falha na reconexão',
      reconnect_confirm_title: 'Reconectar a este site?',
      reconnect_confirm_message: '{{origin}} poderá acessar sua carteira novamente.',
      reconnect_needs_reapproval: 'Não foi possível verificar esta conexão. Leia um novo código QR para reconectar.',
      scan_qr_code: 'Escanear código QR',
      paste_uri: 'Colar URI',
      invalid_qr_code: 'Código QR inválido',
      wallet_not_ready: 'Carteira não pronta',
      please_log_in_first: 'Por favor, faça login primeiro',
      unknown_error: 'Erro desconhecido',

      // Pay screen
      payments: 'Pagamentos',
      pay: 'Pagar',
      pay_direction_pay: 'Pagar',
      pay_direction_receive: 'Receber',
      pay_cell_nearby_pay: 'Alguém por perto',
      pay_cell_nearby_pay_sub: 'Escaneie o código da pessoa',
      pay_cell_handle_pay: 'Alguém remoto',
      pay_cell_handle_pay_sub: 'Escolha um nome — eles precisam deste aplicativo',
      pay_cell_address_pay: 'Para um endereço BSV',
      pay_cell_address_pay_sub: 'Cole ou escaneie um endereço BSV',
      pay_cell_nearby_get: 'Alguém por perto',
      pay_cell_nearby_get_sub: 'Mostre seu código de pagamento',
      pay_cell_handle_get: 'Alguém remoto',
      pay_cell_handle_get_sub: 'Compartilhe seu nome',
      pay_cell_address_get: 'Para um endereço BSV',
      pay_cell_address_get_sub: 'Mostre um endereço BSV',
      pay_method: 'Método',
      pay_method_nearby: 'Por perto',
      pay_method_remote_link: 'Compartilhar link remoto',
      pay_method_address: 'Para um endereço BSV',
      pay_pre_nearby: 'Vocês dois estão aqui.',
      pay_pre_handle: 'A pessoa usa este aplicativo.',
      pay_pre_address: 'Você tem o endereço BSV dela.',
      pay_conseq_nearby: 'Liquida em segundos.',
      pay_conseq_handle: 'Chega quando a carteira dela verificar de novo.',
      pay_conseq_address: 'O dinheiro é enviado — a pessoa não é avisada.',
      paid: 'Pago',
      pay_inbox_attention: 'Não foi possível adicionar',
      pay_inbox_attention_hint:
        'Chegaram, mas sua carteira não conseguiu adicioná-los. Tente novamente ou descarte para desistir deles de vez.',
      pay_dismiss_confirm: 'Toque novamente para desistir',
      pay_dismissed: 'Pagamento descartado',
      payment_arrived: 'Pagamento recebido',
      payment_arrived_damaged: 'Este pagamento chegou danificado',
      discard_payment_title: 'Descartar este pagamento?',
      discard_payment_body:
        'Será pedido ao remetente que envie os detalhes novamente. O dinheiro permanece na cadeia até isso acontecer.',
      discard_void_title: 'Este pagamento não se concretizou',
      discard_void_body: 'Foi cancelado ou gasto noutro lugar. Pode dispensá-lo em segurança.',
      waiting_network_confirm: 'A aguardar que a rede confirme este pagamento',
      finish_or_cancel_outgoing: 'Termine ou cancele o pagamento abaixo antes de enviar um novo',
      identity_search_unavailable: 'A pesquisa está indisponível neste momento. Tente novamente daqui a pouco.',
      abandon_payment: 'Abandonar o pagamento',
      finish_payment: 'Terminar o pagamento',
      cancel_this_payment: 'Cancelar este pagamento?',
      cancel_payment: 'Cancelar o pagamento',
      resend: 'Reenviar',
      resending: 'A reenviar…',
      send_payment_details_again: 'Enviar os detalhes novamente',
      tx_action_resend_short: 'Reenviar',
      tx_action_request_confirmation: 'Pedir confirmação ao destinatário',
      resend_sent: 'Detalhes do pagamento reenviados',
      resend_failed_no_record: 'Os detalhes deste pagamento não estão neste aparelho, então não dá para reenviar.',
      resend_failed_no_recipient: 'Este pagamento não registra para quem foi, então não dá para reenviar.',
      resend_failed_no_transaction: 'Não foi possível obter os dados deste pagamento. Tente de novo com conexão.',
      resend_requested: 'Pediram-lhe para reenviar um pagamento',
      request_again: 'Pedir novamente',
      send_again: 'Enviar novamente',
      copy_details: 'Copiar detalhes',
      payment_bounced_resend: 'Foi-lhes pedido que enviassem este pagamento novamente',
      dismiss_rejected_payment: 'Pagamento recusado dispensado',
      activity_load_failed: 'Não foi possível carregar a atividade',
      activity_load_retry: 'Toque para tentar novamente',
      activity_view_contact: 'Ver contato',
      home_payments_need_attention: '{{count}} pagamentos precisam de atenção',
      home_payments_unsent: '{{count}} pagamentos não entregues',
      pay_copy: 'Copiar',
      pay_share_link: 'Compartilhar link',
      pay_address_watching: 'O dinheiro enviado para cá é adicionado à sua carteira automaticamente.',
      pay_address_earlier_day: 'Voltar para um dia anterior',
      pay_address_sweep_now: 'Verificar este endereço BSV agora',
      pay_address_swept: '{{amount}} recebido',

      check_wallet: 'Solução de problemas',
      settings_version: 'v{{version}}',
      error_review_actions: 'Este pagamento está preso. A Solução de problemas pode tentar libertá-lo.',
      wallet_check_ok: 'Está tudo bem',
      wallet_check_attention: 'Algumas verificações precisam da sua atenção',
      wallet_check_couldnt: 'Não foi possível verificar',
      wallet_check_couldnt_body: 'Um passo de reparação não terminou. Nada foi presumido sobre as suas moedas.',
      wallet_check_summary: '{{freed}} moedas presas libertadas · {{recovered}} pagamentos recuperados',
      wallet_check_step_coins: 'Suas moedas são válidas',
      wallet_check_skip: 'Pular esta verificação',
      wallet_check_skipped: 'Ignorada',
      wallet_check_step_proofs: 'Provas de transação',
      wallet_check_step_records: 'Registros do banco de dados local',
      wallet_check_step_tokens: 'Transferências de stablecoin verificadas',
      wallet_check_step_online: 'Conexão',
      wallet_check_step_missed: 'Pagamentos não recebidos',
      wallet_check_step_backup: 'Backup privado enviado',
      wallet_check_step_phrase: 'Frase de recuperação salva',
      wallet_check_progress: '{{current}} de {{total}}',
      wallet_check_stuck_title: 'Não foi possível enviar este pagamento',
      wallet_check_stuck_message: 'Algumas moedas parecem presas. A Solução de problemas pode libertá-las.',
      wallet_check_failed: 'Não foi possível concluir a verificação. Tente novamente.',

      // Local payments
      local_pay_request: 'Solicitar pagamento',
      local_pay_send: 'Enviar pagamento',
      local_pay_amount: 'Valor',
      local_pay_show_qr: 'Mostre este código a quem vai pagar',
      local_pay_scan_qr: 'Escaneie o código de quem vai receber',
      local_pay_scan_payer_qr: 'Escaneie o código de quem vai pagar',
      local_pay_scan_progress: '{{have}} de {{total}} fragmentos',
      local_pay_show_payment_qr: 'Mostre este código a quem vai receber',
      local_pay_animated_hint:
        'Peça para escanear o código animado abaixo — ele continua mudando até o celular receber tudo.',
      local_pay_show_qr_instead: 'Mostrar um código QR',
      local_pay_preparing: 'Preparando a solicitação…',
      local_pay_delivering: 'Entregando o pagamento…',
      local_pay_saving: 'Salvando o pagamento…',
      local_pay_sent: 'Pagamento enviado',
      local_pay_received: 'Pagamento recebido',
      local_pay_added: 'Adicionado à sua carteira.',
      local_pay_added_multiple: '{{count}} pagamentos adicionados à sua carteira.',
      local_pay_received_confirming: 'Confirmando com a sua carteira…',
      local_pay_received_not_credited:
        'Ainda não foi adicionado à sua carteira. Verifique a Atividade antes de entregar qualquer coisa.',
      local_pay_decline_broadcast_title: 'Um pagamento recusado foi transmitido',
      local_pay_decline_broadcast_body:
        'Um pagamento que havia sido recusado apareceu na rede. Não o envie de novo — verifique a Atividade.',
      local_pay_pending_abort_dropped_title: 'Um pedido de liberação não verificado foi ignorado',
      local_pay_pending_abort_dropped_body:
        'Não foi possível verificar um registro salvo de liberação de pagamento, então nada foi liberado. Se um pagamento ainda parecer travado, cancele-o em Atividade.',
      local_pay_cancel_unverifiable_title: 'Não é possível verificar este pagamento offline',
      local_pay_cancel_unverifiable_body:
        'Se a outra pessoa já escaneou, cancelar permite que essas moedas sejam gastas duas vezes. Só cancele se tiver certeza de que nunca foi escaneado.',
      local_pay_cancel_unverifiable_confirm: 'Cancelar mesmo assim',
      local_pay_queued: 'Salvo — será adicionado à sua carteira automaticamente.',
      local_pay_already_paid: 'Esta solicitação já foi paga.',
      local_pay_failed: 'Falha no pagamento',
      local_pay_network_denied:
        'O acesso à rede local está desativado. Ative-o em Ajustes para pagar dispositivos por perto.',
      local_pay_radio_fallback: 'Link sem fio indisponível — mostre este código ao outro aparelho.',
      local_pay_floor_peer_bt_off: 'O Bluetooth deles está desligado, então este pagamento será feito por código.',
      local_pay_floor_local_bt_off: 'Seu Bluetooth está desligado. Ligue-o para um link por perto mais rápido.',
      local_pay_floor_local_ble_denied:
        'O acesso ao Bluetooth está desativado para o BSV Wallet. Permita-o em Ajustes para um link por perto mais rápido.',
      local_pay_floor_cross_os:
        'O outro aparelho não pode usar Bluetooth agora, então este pagamento será feito por código.',
      local_pay_floor_peer_no_radio:
        'O outro aparelho não está aguardando um link sem fio, então este pagamento será feito por código.',
      local_pay_ble_unavailable: 'O Bluetooth está indisponível. O código acima continua funcionando.',
      local_pay_too_large: 'Os dados deste pagamento são grandes demais para serem transmitidos por códigos QR.',
      local_pay_nearby_unavailable: 'O Wi-Fi por perto está indisponível. O código acima continua funcionando.',
      local_pay_retry_save: 'Tentar salvar de novo',
      local_pay_wrong_session:
        'Esse pagamento não corresponde a esta solicitação e não foi aceito. Peça a quem vai pagar para escanear o código de novo.',
      local_pay_declined_mismatch:
        'Quem recebe não aceitou este pagamento: ele não corresponde à solicitação. Escaneie o código novamente.',
      local_pay_declined_already_paid: 'Essa solicitação já foi paga. Nada foi enviado.',
      local_pay_declined_save: 'Quem recebe não conseguiu salvar o pagamento. Nada foi enviado — tente de novo.',
      local_pay_declined_decode: 'Quem recebe não conseguiu ler o pagamento. Nada foi enviado — tente de novo.',
      local_pay_declined_not_covered:
        'Quem recebe não conseguiu verificar este pagamento nos registros do emissor. Nada foi enviado.',
      local_pay_broadcast_pending:
        'Enviado. Este aparelho ainda não conseguiu processá-lo — quem recebe vai fazer isso.',
      local_pay_any_amount: 'Qualquer valor',
      local_pay_choose_amount: 'Escolha um valor',
      local_pay_enter_amount_send: 'Quanto você quer enviar?',
      local_pay_qr_unavailable: 'Não foi possível criar este código. Comece a solicitação de novo.',
      local_pay_presence_qr: 'Por código — sem conexão ao vivo',
      local_pay_presence_ready: 'Conexão próxima criptografada pronta',
      local_pay_presence_waiting_payee: 'Aguardando um aparelho por perto',
      local_pay_presence_waiting_payer: 'Procurando o aparelho do destinatário…',
      local_pay_presence_linked: 'Conexão criptografada aberta',
      local_pay_presence_paid: 'Pagamento confirmado',
      pay_offline_title: 'Você está offline',
      pay_offline_body: 'Só os pagamentos por perto funcionam agora. Eles serão processados assim que você reconectar.',
      pay_offline_queued: '{{count}} pagamento(s) aguardando processamento assim que você reconectar.',
      pay_offline_pending_title: 'Aguardando processamento',
      pay_offline_pending_body:
        '{{count}} pagamento(s) ainda não foram processados. Nada está concluído até que sejam; as tentativas continuam.',
      pay_offline_needs_internet: 'Precisa de internet',
      pay_offline_rejected_title: 'Um pagamento foi rejeitado',
      pay_offline_rejected_body: 'A rede rejeitou um pagamento entregue por {{sender}} via {{via}} em {{when}}.',
      pay_offline_unknown_sender: 'um remetente desconhecido',
      pay_offline_unknown_via: 'um método desconhecido',
      pay_received_not_broadcast: 'Recebido offline · ainda não processado',
      pay_offline_sent_rejected_title: 'Um pagamento não pôde ser entregue',
      pay_offline_sent_rejected_body: 'A rede rejeitou um pagamento que você enviou em {{when}}.',
      pay_offline_send_now: 'Enviar agora',
      pay_offline_stalled_body: 'Alguns pagamentos na fila não podem ser enviados automaticamente: {{detail}}',
      offline_stall_no_request: 'Falta o registo de envio a um pagamento em fila.',
      offline_stall_bad_beef: 'Não foi possível ler os dados da transação de um pagamento em fila.',
      offline_stall_foreign_ancestor:
        'Um pagamento em fila aguarda uma transação anterior que outra carteira nunca processou.',
      pay_offline_kv_pending: '{{count}} pagamento(s) por proximidade à espera de serem adicionados a esta carteira.',
      pay_offline_kv_stuck: 'Não foi possível adicionar {{count}} pagamento(s) próximo(s). Peça para enviarem de novo.',
      pay_offline_kv_corrupt: 'Foram encontrados dados de pagamento danificados neste dispositivo.',
      pay_offline_kv_corrupt_body:
        'Os pagamentos afetados foram guardados à parte neste dispositivo, não foram apagados. Alguns podem já ter entrado no seu saldo, por isso peça para enviarem de novo apenas se o pagamento continuar a faltar.',
      pay_offline_show_code: 'Mostrar o código novamente',
      pay_parked_cancel: 'Cancelar pagamento',
      pay_parked_already_sent: 'Este pagamento já foi enviado',
      token_cancel_already_sent: 'Este pagamento já foi concluído e não pode ser cancelado.',
      tx_status_parked: 'Não entregue',
      tx_status_import_hold: 'Retido (importado)',
      resend_sending: 'Enviando pela caixa de mensagens',
      cancelling_payment: 'Cancelando o pagamento',

      contacts: 'Contatos',
      contacts_search_placeholder: 'Pesquisar por nome ou apelido',
      contacts_empty_title: 'Ainda não há contatos',
      contacts_empty_body:
        'As pessoas para quem você paga aparecem aqui depois de salvas. Escaneie o código de perfil de alguém com o botão no canto superior direito.',
      contacts_recent: 'Recentes',
      contact_edit_name: 'Editar nome',
      contact_save_name: 'Salvar nome',
      contact_activity: 'Atividade',
      contact_activity_empty: 'Ainda não há atividade com este contato',
      contact_delete: 'Excluir contato',
      contact_delete_body: 'Isso remove {{name}} dos seus contatos. Não afeta nenhum pagamento anterior.',
      contact_handle_caption: 'O apelido registrado dele · só ele pode alterá-lo',
      contact_new_title: 'Novo contato',
      contact_save: 'Salvar',
      contact_identifier: 'Identificador',
      contact_saved: 'Contato salvo',
      contact_scan_hint: 'Escaneie um código de perfil para adicionar alguém',
      contact_identifier_placeholder: 'Cole o identificador dele',
      contact_identifier_invalid: 'Isso não parece um identificador válido',
      contact_add_from_payment: 'Adicionar aos contatos',

      profile: 'Perfil',
      profile_display_name: 'Nome de exibição',
      profile_display_name_hint:
        'Mostrado publicamente ao lado do seu apelido. Qualquer pessoa que procurar por você pode ver.',
      profile_handle: 'Apelido',
      profile_handle_placeholder: 'seunome',
      profile_handle_checking: 'Verificando disponibilidade…',
      profile_handle_available: '{{handle}} está disponível',
      profile_handle_taken: '{{handle}} já está em uso',
      profile_handle_invalid:
        '3-32 caracteres: a-z, 0-9, ponto, sublinhado ou hífen; começa e termina com letra ou número',
      profile_handle_failed: 'Não foi possível verificar a disponibilidade. Tente novamente.',
      profile_handle_unavailable: 'O registro de apelido ainda não está disponível nesta versão.',
      profile_handle_registered: 'Apelido registrado',
      profile_handle_register_action: 'Registrar',
      profile_handle_too_similar: '{{handle}} é parecido demais com um já em uso',
      profile_handle_reserved: '{{handle}} está reservado',
      profile_handle_cooldown: '{{handle}} foi liberado há pouco e ainda não está livre',
      profile_handle_pending: 'Concluindo o registro…',
      profile_handle_rolled_back: '{{handle}} já estava ocupado. Você manteve {{previous}}.',
      profile_handle_changed: 'Apelido alterado',
      profile_handle_rejected: 'O registro recusou esse apelido.',
      profile_handle_clock_ahead:
        'O relógio do seu dispositivo está muito adiantado. Tente novamente em alguns minutos.',
      profile_handle_wrong_registry: 'Esse apelido não está neste registro. Abra o Perfil novamente para atualizá-lo.',
      profile_display_name_publish_failed: 'Seu nome foi salvo aqui, mas o registro não o publicou.',
      profile_qr_hint: 'Outras pessoas podem escanear isto para adicionar você como contato',

      pay_step_contacts: 'Contatos',
      pay_step_continue: 'Continuar',
      pay_review_to: 'Para',
      pay_review_amount: 'Valor',
      pay_review_note: 'Nota',
      pay_review_note_edit: 'Adicionar uma nota',
      pay_review_delivery_host: 'Entregando a',
      pay_trust_contact: 'Seu contato',
      pay_trust_handle_attested: 'Registrado',
      pay_trust_handle_domain_attested: 'Verificado por {{domain}}',
      pay_trust_unverified: 'Não verificado. Confira bem este identificador.',
      pay_address_network_mismatch: 'Este endereço pertence a uma rede diferente da que você está usando.',
      contacts_your_contacts: 'Seus contatos',
      contacts_other_people: 'Outras pessoas',
      contacts_not_saved: 'Não salvo',
      contacts_network_footer: 'Encontrado na rede. Você ainda não salvou.',
      contact_no_handle: 'Ainda sem apelido',
      contact_no_handle_registered: 'Nenhum apelido registrado',
      contact_name_section: 'Seu nome para esta pessoa',
      contact_name_hint: 'Só você vê isto. Não muda como ela se chama.',
      contact_new_name_hint: 'Só você vê isto. Salvo neste dispositivo.',
      contact_copy_identifier: 'Copiar identificador',
      profile_handle_registered_hint: 'Registrado no seu identificador. As pessoas podem encontrar você por ele.',
      profile_handle_change: 'Alterar',
      profile_handle_claim: 'Reivindicar {{handle}}',
      profile_handle_replace_warning:
        'Reivindicar substitui {{handle}}. Quem salvou você mantém o contato; o apelido antigo deixa de levar a você.',
      profile_identifier_section: 'Seu identificador',
      profile_show_qr: 'Mostrar como QR',
      profile_show_qr_hint: 'Deixe alguém escanear você para obter seu perfil',
      profile_qr_callout:
        'Escanear isto abre a BSV Wallet e adiciona você como contato, com nome, apelido e foto. Não pede pagamento — esse código fica em Receber.',
      pay_review_title: 'Revisar',
      pay_send_amount: 'Enviar {{amount}}',
      pay_sent_to: 'para {{name}}',
      pay_save_as_contact: 'Salvar como contato'
    }
  },
  bn: {
    translation: {
      token_balances_header: 'ব্যালেন্স',
      wallet_coin_switcher: 'কোন কয়েন দেখাবেন তা বেছে নিন',
      wallet_activity_empty_asset: 'এখনও কোনো {{ticker}} কার্যকলাপ নেই',
      pay_direction_pay_asset: '{{ticker}} পরিশোধ করুন',
      token_fee_footer: 'পাঠাতে নেটওয়ার্ক ফি-এর জন্য সামান্য BSV লাগে। গ্রহণ করতে লাগে না।',
      token_new_tap: 'নতুন — কে ইস্যু করে দেখতে ট্যাপ করুন',
      token_unresolved: 'অচেনা টোকেন',
      token_unresolved_sub: 'এই ওয়ালেট ইস্যুকারীকে খুঁজে পায়নি। অনলাইনে এলে আবার চেষ্টা করুন।',
      token_paused_short: 'স্থানান্তর স্থগিত',
      token_frozen_short: '{{amount}} ফ্রিজ করা',
      token_sheet_issuer: 'ইস্যুকারী',
      token_sheet_issuer_unknown: 'শনাক্ত করা যায়নি',
      token_sheet_asset_id: 'টোকেন আইডি',
      token_sheet_powers_header: '{{issuer}} কী করতে পারে',
      token_power_pause: '{{ticker}}-এর সব স্থানান্তর থামানো',
      token_power_freeze: 'নির্দিষ্ট কয়েন ফ্রিজ করা, আপনারটিও',
      token_power_admit: 'কে {{ticker}} রাখতে পারবে তা ঠিক করা',
      token_power_replace: 'ফ্রিজ করা কয়েন নতুন দিয়ে বদলানো',
      token_powers_limit:
        'আপনাকে ছাড়া তারা আপনার ওয়ালেট থেকে {{ticker}} সরাতে পারে না, আর ফ্রিজ করা কয়েন কেবল খরচ করা যায় না।',
      token_frozen_sub:
        '{{issuer}} আপনার ব্যালেন্সের {{amount}} {{ticker}} ফ্রিজ করেছে। আপত্তি জানাতে তাদের সঙ্গে যোগাযোগ করুন।',
      pay_asset_label: 'যা দিয়ে পরিশোধ',
      pay_asset_label_get: 'যাতে অর্থ গ্রহণ',
      pay_asset_cta: '{{amount}} {{ticker}} পাঠান',
      pay_asset_over_balance: 'আপনার {{ticker}} ব্যালেন্সের চেয়ে বেশি।',
      pay_asset_needs_bsv: '{{ticker}} পাঠাতে নেটওয়ার্ক ফি-এর জন্য সামান্য BSV লাগে। গ্রহণ করতে লাগে না।',
      pay_asset_get_bsv: 'BSV নিন',
      pay_asset_needs_bsv_short: 'ফি-এর জন্য BSV দরকার',
      pay_asset_frozen_note: 'আপনার ব্যালেন্সের {{amount}} {{ticker}} ফ্রিজ করা, পাঠানো যাবে না।',
      pay_asset_paused: '{{issuer}} {{ticker}} স্থানান্তর থামিয়ে রেখেছে, তাই এটি এখন পাঠানো যাবে না।',
      pay_asset_no_address: '{{ticker}} কেবল কোনো ব্যক্তি বা কাছাকাছি ডিভাইসে পাঠানো যায় — বিটকয়েন ঠিকানায় নয়।',
      pay_asset_address_status: '{{ticker}}-এর জন্য নয়',
      pay_asset_self_unregistered:
        '{{issuer}} আপনাকে {{ticker}} পাঠানোর জন্য নিবন্ধন করেনি। তবু আপনি এতে অর্থ নিতে পারবেন।',
      pay_asset_unidentified: 'এই ওয়ালেট টোকেনটি শনাক্ত করতে পারেনি, তাই এখনো পাঠাতে পারছে না।',
      pay_asset_recipient_unregistered:
        '{{issuer}} এখনো এই ব্যক্তিকে {{ticker}}-এর জন্য নিবন্ধন করেনি, তাই তাকে এতে অর্থ পাঠানো যাবে না।',
      pay_asset_recipient_blocked: '{{issuer}} এই ব্যক্তিকে অর্থ পাঠানোর অনুমতি দেয় না।',
      token_err_refused:
        '{{issuer}} এই স্থানান্তর প্রত্যাখ্যান করেছে। কিছুই পাঠানো হয়নি এবং আপনার {{ticker}} অপরিবর্তিত।',
      token_err_refused_paused:
        '{{issuer}} {{ticker}} থামিয়ে রেখেছে। কিছুই পাঠানো হয়নি এবং আপনার ব্যালেন্স অপরিবর্তিত।',
      token_err_refused_recipient:
        '{{issuer}} এই ব্যক্তিকে অর্থ পাঠানোর অনুমতি দেয় না। কিছুই পাঠানো হয়নি এবং আপনার ব্যালেন্স অপরিবর্তিত।',
      token_err_refused_frozen:
        'আপনি যে {{ticker}} পাঠাচ্ছিলেন তার কিছু অংশ এইমাত্র ফ্রিজ হয়েছে। কিছুই পাঠানো হয়নি; ব্যালেন্স হালনাগাদ করা হয়েছে।',
      token_err_send_failed: 'এই পেমেন্ট পাঠানো যায়নি: {{reason}}। আপনার ওয়ালেট থেকে কিছুই যায়নি।',
      token_err_reason_unknown: 'একটি অজানা সমস্যা',
      token_err_unreachable:
        'এই স্থানান্তর নিশ্চিত করতে আমরা {{issuer}}-এর কাছে পৌঁছাতে পারিনি। আবার চেষ্টার আগে আপনার {{ticker}} ব্যালেন্স দেখে নিন।',
      token_err_check_again: 'আবার দেখুন',
      token_err_balance_changed:
        'এটি প্রস্তুত হওয়ার সময় আপনার {{ticker}} ব্যালেন্স বদলে গেছে। পরিমাণ দেখে আবার চেষ্টা করুন।',
      token_err_busy: 'একটি {{ticker}} পেমেন্ট ইতিমধ্যেই পাঠানো হচ্ছে।',
      pay_sent_not_broadcast: 'পাঠানো হয়েছে। এটি এখনো নেটওয়ার্কে পৌঁছায়নি — আমরা চেষ্টা চালিয়ে যাব।',
      pay_sent_not_notified: 'পাঠানো হয়েছে। আমরা তাদের এখনো জানাতে পারিনি — তাদের ওয়ালেট পরেরবার দেখলে টের পাবে।',
      pay_sent_handed_to_wallet: 'পাঠানো হয়েছে। তারা সামনাসামনি নেয়নি, তাই আমরা তাদের ওয়ালেটে পাঠিয়ে দিয়েছি।',
      token_first_hold: '{{ticker}} ইস্যু করে {{issuer}}, যারা স্থানান্তর থামাতে ও কয়েন ফ্রিজ করতে পারে।',
      pay_asset_request_paused: '{{issuer}} {{ticker}} থামিয়ে রেখেছে, তাই পেমেন্ট এখন নাও আসতে পারে।',
      token_admit_title: 'আপনি এখনো {{ticker}}-এ অর্থ নিতে পারবেন না',
      token_admit_body:
        '{{issuer}} কেবল নিবন্ধিত ব্যক্তিদেরই {{ticker}} পাঠায়। ততক্ষণ আপনার উদ্দেশ্যে পাঠানো অর্থ প্রেরকের ওয়ালেট ছাড়ার আগেই প্রত্যাখ্যাত হয়।',
      token_admit_bsv_instead: 'বরং BSV-তে অর্থ নিন',
      token_row_sent: '{{ticker}} পাঠানো হয়েছে',
      token_row_received: '{{ticker}} পাওয়া গেছে',
      token_row_amount_pending: 'পরিমাণ পাওয়া যাচ্ছে না',
      token_evicted_title: 'কিছু {{ticker}} তুলে নেওয়া হয়েছে',
      token_evicted_body: '{{issuer}} আপনার ওয়ালেট থেকে {{amount}} {{ticker}} তুলে নিয়েছে।',
      local_pay_token_not_cleared: 'অফলাইনে গৃহীত · {{issuer}} এখনো নিশ্চিত করেনি',
      pay_asset_nearby_needs_network:
        'সামনাসামনি {{ticker}}-এ পরিশোধ করতে সংযোগ লাগে। সংযোগ ছাড়া আপনি BSV-তে দিতে পারেন।',
      pay_asset_link_not_held:
        'এই অনুরোধটি এমন টোকেনের জন্য যা এই ওয়ালেটে নেই। আপনি এখনও BSV-এ তাদের পেমেন্ট করতে পারেন।',
      pay_asset_nearby_wrong_overlay: 'এই অনুরোধটি এমন টোকেনের জন্য যার জন্য এই ওয়ালেট সেট করা নেই।',
      token_messagebox_mismatch:
        'আপনার ওয়ালেট অন্য একটি মেসেজ বক্সে সেট করা, তাই টোকেন পেমেন্ট আপনার কাছে না-ও পৌঁছাতে পারে।',
      token_issuer_fallback: 'ইস্যুকারী',
      token_row_hint: 'কে ইস্যু করে ও কী করতে পারে তা খোলে',
      token_attention_one: '{{issuer}}-এর নিশ্চয়তার অপেক্ষায় — বিস্তারিত দেখতে ট্যাপ করুন',
      token_attention_many: '{{count}}টি টোকেন পেমেন্ট নিষ্পত্তির অপেক্ষায় — বিস্তারিত দেখতে ট্যাপ করুন',
      token_recv_reversed: '{{issuer}} এই পেমেন্ট নিশ্চিত করতে পারেনি, তাই এটি ফিরিয়ে নেওয়া হয়েছে।',
      token_settle_paused_retry: '{{issuer}} এই মুহূর্তে {{ticker}} থামিয়ে রেখেছে — আমরা চেষ্টা চালিয়ে যাব।',
      token_settle_unreachable_retry:
        'এই স্থানান্তর নিশ্চিত করতে আমরা {{issuer}}-এর কাছে পৌঁছাতে পারিনি। আমরা চেষ্টা চালিয়ে যাব।',
      token_settle_evicted:
        'সেই পেমেন্ট সময়মতো নেটওয়ার্কে পৌঁছায়নি এবং ফিরিয়ে নেওয়া হয়েছে — আপনার {{ticker}} ব্যালেন্সে ফিরে এসেছে।',
      token_frame_unreadable: 'আপনি যে পেমেন্ট পেয়েছেন তা আমরা পড়তে পারিনি — প্রেরককে আবার পাঠাতে বলুন।',
      token_chain_reversed:
        'এই শৃঙ্খলের আগের একটি পেমেন্ট {{issuer}} প্রত্যাখ্যান করেছে, তাই এটিও ফিরিয়ে নেওয়া হয়েছে।',
      token_settle_stuck_bound:
        'এই পেমেন্ট ৩ দিনের বেশি নিষ্পত্তির অপেক্ষায় আছে। আপনি অপেক্ষা করতে পারেন বা {{issuer}}-এর সঙ্গে যোগাযোগ করুন।',
      token_cover_failed: 'এই পেমেন্ট {{issuer}}-এর রেকর্ডের সঙ্গে মেলেনি এবং গ্রহণ করা হয়নি।',
      token_sent_settling:
        'পাঠানো হয়েছে। {{issuer}}-এর সঙ্গে নিষ্পত্তি চলছে — আপনি বা {{payee}} আবার সংযুক্ত হলে নিশ্চয়তা দেখতে পাবেন।',
      token_sent_settling_unnamed:
        'পাঠানো হয়েছে। {{issuer}}-এর সঙ্গে নিষ্পত্তি চলছে — আপনাদের যেকেউ আবার সংযুক্ত হলে নিশ্চয়তা দেখা যাবে।',
      token_sent_settled: 'পাঠানো হয়েছে · {{issuer}}-এর সঙ্গে নিষ্পত্তি হয়েছে',
      token_status_settling: 'নিষ্পত্তি চলছে',
      token_status_settled: 'নিষ্পত্তি হয়েছে',
      token_status_refused: 'প্রত্যাখ্যাত',
      token_status_reversed: 'ফিরিয়ে নেওয়া হয়েছে',
      token_status_stuck: 'নিষ্পত্তির অপেক্ষায়',
      new_tab: 'নতুন ট্যাব',
      untitled: 'শিরোনামহীন',
      share: 'শেয়ার করুন',
      back_to_homepage: 'হোমপেজে ফিরুন',
      bookmarks: 'বুকমার্ক',
      add_bookmark: 'বুকমার্ক যোগ করুন',
      bookmark: 'বুকমার্ক',
      unbookmark: 'বুকমার্ক সরান',
      find_in_page: 'পৃষ্ঠায় খুঁজুন',
      find_in_page_placeholder: 'পৃষ্ঠায় খুঁজুন...',
      find_in_page_matches: '{{current}} / {{total}}',
      find_in_page_no_matches: 'কোনো মিল নেই',
      history: 'ইতিহাস',
      clear_all: 'সব মুছুন',
      clear_all_tabs: 'সব ট্যাব মুছুন',
      search_or_enter_website: 'খুঁজুন বা ওয়েবসাইট লিখুন',
      browser: 'ব্রাউজার',
      bug_report: 'বাগ রিপোর্ট',
      enable_web3: 'Web3 সক্রিয় করুন',
      tabs: 'ট্যাব',
      unlock_web3_features: 'Web3 বৈশিষ্ট্য আনলক করুন',

      cancel: 'বাতিল',
      done: 'সম্পন্ন',
      discard: 'বাতিল করুন',
      deny: 'প্রত্যাখ্যান',
      allow: 'অনুমতি দিন',
      save: 'সংরক্ষণ করুন',
      remove: 'সরান',
      delete: 'মুছুন',
      search: 'খুঁজুন',
      loading: 'লোড হচ্ছে...',
      continue: 'চালিয়ে যান',
      go_back: 'ফিরে যান',
      continue_action: 'চালিয়ে যান',
      camera_denied_description:
        'ক্যামেরা অ্যাক্সেস প্রত্যাখ্যান করা হয়েছে। এই বৈশিষ্ট্যটি ব্যবহার করতে সেটিংসে এটি সক্ষম করুন।',
      open_settings: 'সেটিংস খুলুন',
      copied: 'কপি করা হয়েছে',
      explorer_open_failed: 'ব্লক এক্সপ্লোরার খোলা যায়নি',
      tx_action_refresh: 'অবস্থা রিফ্রেশ করুন',
      tx_action_refresh_short: 'অবস্থা',
      tx_action_abort: 'লেনদেন বাতিল করুন',
      copy: 'কপি',
      copy_to_clipboard: 'ক্লিপবোর্ডে কপি করুন',
      details: 'বিবরণ',
      hide_details: 'বিবরণ লুকান',

      identity: 'পরিচয়',
      settings: 'সেটিংস',
      wallet: 'ওয়ালেট',
      activity: 'কার্যকলাপ',
      configuration: 'কনফিগারেশন',
      data_and_security: 'ডেটা ও নিরাপত্তা',
      storage_row: 'স্টোরেজ',
      storage_title: 'ওয়ালেট স্টোরেজ',
      storage_db_size: 'ডেটাবেস: {{size}}',
      storage_free_space: 'খালি জায়গা: {{size}}',
      storage_reclaimable: 'এখন পুনরুদ্ধারযোগ্য: {{size}}, {{count}}টি লেনদেনে',
      storage_held_back: '{{count}}টি লেনদেন এখনও ব্যবহৃত হওয়ায় রাখা হয়েছে',
      storage_reclaim_explain:
        'পুনরুদ্ধার সেই যাচাই ডেটা মুছে দেয় যা ওয়ালেটের আর প্রয়োজন নেই। আপনার লেনদেন, ব্যালেন্স ও ইতিহাস অপরিবর্তিত থাকে। এটি ডেটাবেস ফাইলের ভিতরে জায়গা খালি করে; সিস্টেম সংকুচিত না করা পর্যন্ত ফাইলটি ছোট হয় না।',
      storage_reclaim_confirm: 'পুনরুদ্ধার',
      storage_reclaimed: '{{count}}টি লেনদেন পুনরুদ্ধার হয়েছে',
      storage_failed: 'স্টোরেজ ব্যবহার পড়া যায়নি।',
      storage_unavailable: 'স্টোরেজ এখনও প্রস্তুত নয়।',
      backup_push_section: 'ব্যক্তিগত ব্যাকআপ',
      backup_push_toggle: 'সার্ভারে ব্যাকআপ',
      backup_erase_row: 'সার্ভার থেকে ব্যাকআপ মুছুন',
      backup_erase_title: 'আপনার ব্যাকআপ মুছবেন?',
      backup_erase_message:
        'এই ওয়ালেট ব্যাকআপ সার্ভারে যা পাঠিয়েছে তার সবকিছু, সব ডিভাইস মিলিয়ে, স্থায়ীভাবে মুছে যাবে। একই সঙ্গে ব্যাকআপ বন্ধ হয়ে যাবে। আপনার কয়েন, কী ও রিকভারি ফ্রেজ অক্ষত থাকবে — তবে নতুন ডিভাইসে এই ওয়ালেটের ইতিহাস আর পুনরুদ্ধার করা যাবে না, আর আপনার বর্তমান কিছু কয়েন সেখানে খরচ করা যাবে না।',
      backup_erase_confirm: 'মুছুন',
      backup_erase_done: 'ব্যাকআপ মুছে ফেলা হয়েছে — {{count}}টি আইটেম সরানো হয়েছে',
      backup_erase_failed: 'ব্যাকআপ মোছা যায়নি। কিছুই সরানো হয়নি — আবার চেষ্টা করুন।',
      backup_erase_no_key: 'কোনো ওয়ালেট কী নেই',
      backup_push_disclosure:
        'আপনার ওয়ালেট ডেটাবেসের একটি এনক্রিপ্টেড কপি BSV অ্যাসোসিয়েশনের সার্ভারে পাঠানো হয়, যাতে নতুন ডিভাইসে পুনরুদ্ধার করা যায়। এটি এমন কী দিয়ে এনক্রিপ্ট করা যা কেবল আপনার রিকভারি ফ্রেজ থেকেই আসে — পরিচালক পড়তে পারে না। তবুও আপনার রিকভারি ফ্রেজ বা মুদ্রিত শেয়ার দরকার: সেগুলি ছাড়া ব্যাকআপ কেউ খুলতে পারবে না, আমরাও নই।',
      backup_push_off_title: 'ব্যক্তিগত ব্যাকআপ বন্ধ করবেন?',
      backup_push_off_message:
        'শুধু রিকভারি ফ্রেজ দিয়ে এই ওয়ালেট পুনর্গঠন সম্ভব নয়। আপনার কয়েন সুরক্ষিত রাখার সঙ্গে জড়িত তথ্য কেবল ডেটাবেসেই থাকে, তাই ব্যাকআপ বন্ধ থাকলে নতুন ডিভাইস আপনার ফ্রেজ ফেরাতে পারবে কিন্তু তহবিল নয়। সার্ভারে যা আছে তা মুছে যায় না।',
      backup_push_off_confirm: 'বন্ধ করুন',
      backup_push_on_toast: 'ব্যাকআপ চালু — সিঙ্ক হচ্ছে',
      backup_push_off_toast: 'ব্যাকআপ বন্ধ — আর কিছু পাঠানো হবে না',
      permissions: 'অনুমতি',
      trust_network: 'বিশ্বাস নেটওয়ার্ক',
      // vault (machine-translated, needs native review)
      vault_title: 'ভল্ট',
      vault_row_title: 'ভল্ট',
      vault_hero_title: 'সুরক্ষিত হোল্ডিং',
      vault_hero_body:
        'দীর্ঘমেয়াদের জন্য কোল্ড স্টোরেজ, আপনার নিজের YubiKey দিয়ে লক করা। দুটি বা তার বেশি YubiKey 5 NFC লাগবে।',
      vault_unsupported_title: 'YubiKey প্রয়োজন',
      vault_unsupported_body: 'দুটি বা তার বেশি YubiKey 5 NFC, এবং সেগুলি পড়তে পারে এমন একটি ফোন।',
      vault_restore_from_chain: 'ব্লকচেইন থেকে ভল্ট পুনরুদ্ধার করুন',
      vault_restore_scanning: 'ব্লকচেইন স্ক্যান করা হচ্ছে…',
      vault_restore_found: 'ব্লকচেইনে {{count}}টি ভল্ট জমা পাওয়া গেছে।',
      vault_restore_pending: '{{count}}টি জমা পাওয়া গেছে, প্রথম নিশ্চিতকরণের অপেক্ষায় — শীঘ্রই আবার চেষ্টা করুন।',
      vault_restore_none_found: 'এই ওয়ালেটের জন্য ব্লকচেইনে কোনো ভল্ট জমা পাওয়া যায়নি।',
      vault_balance_label: 'ভল্টে আছে',
      vault_key_section: 'সিকিউরিটি কী ({{count}}/৫)',
      vault_manage_section: 'পরিচালনা',
      vault_deposit_cta: 'জমা',
      vault_withdraw_cta: 'উত্তোলন',
      vault_deposit_title: 'ভল্টে জমা করুন',
      vault_withdraw_title: 'ভল্ট থেকে উত্তোলন',
      vault_deposit_sub: 'আপনার দৈনন্দিন ব্যালেন্স থেকে। YubiKey লাগবে না।',
      vault_withdraw_sub: 'আপনার দৈনন্দিন ব্যালেন্সে ফেরত। আপনার যেকোনো ভল্ট কী ট্যাপ করুন।',
      vault_deposit_done: 'ভল্টে সরানো হয়েছে',
      vault_reclaim_done: 'বাধাপ্রাপ্ত ভল্ট জমা থেকে তহবিল পুনরুদ্ধার করা হয়েছে',
      vault_withdraw_done: 'ভল্ট থেকে উত্তোলন করা হয়েছে',
      vault_withdraw_partial: 'আরও {{count}}টি জমা বাকি — সরাতে আবার তুলুন।',
      vault_withdraw_reason: 'সই করতে আপনার YubiKey এখানে ধরুন — ভল্ট থেকে {{amount}} sats উত্তোলন করুন',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: 'এখনও উপলব্ধ নয় — এই রিলিজে ভল্টে জমা বন্ধ রাখা হয়েছে।',
      vault_not_on_mainnet_body: 'ভল্ট কেবল মেইননেটে উপলব্ধ। এটি ব্যবহার করতে ওয়ালেটটি আবার মেইননেটে ফিরিয়ে নিন।',
      vault_intro_title: 'আপনার ভল্ট সেট আপ করুন',
      vault_intro_what:
        'ভল্ট আপনার নিজের YubiKey দিয়ে টাকা লক করে। ভল্ট কী-এর কোনো অংশই কখনও এই ফোনে থাকে না; আপনার যেকোনো একটি কী এটি খোলে।',
      vault_intro_two_keys: 'আপনার কমপক্ষে দুটি YubiKey লাগবে।',
      vault_intro_apart: 'এগুলি আলাদা জায়গায় রাখুন — একসাথে রাখা দুটি কী মানে একটি কী।',
      vault_intro_backup: 'ওয়ালেটের এনক্রিপ্টেড ব্যাকআপ চালু রাখুন — এটি প্রমাণীকৃত পুনরুদ্ধার রেকর্ড সংরক্ষণ করে।',
      vault_intro_ack:
        'আমি বুঝেছি: কেবল আমার YubiKey-গুলিই এই ভল্ট খোলে। আমার রিকভারি ফ্রেজ নয়। সবগুলি হারালে টাকা চিরতরে হারিয়ে যাবে।',
      vault_intro_piv_ack:
        'আমি নিশ্চিত করছি যে এই YubiKey-এর পুরো PIV অ্যাপটি ফ্যাক্টরি-রিসেট বা ভল্টের জন্য নির্দিষ্ট। সেটআপ গ্লোবাল PIV PIN, PUK ও ম্যানেজমেন্ট কী বদলায়, সব PIV স্লটকে প্রভাবিত করতে পারে, এবং আমি নতুন PUK নিরাপদে সংরক্ষণ করব।',
      vault_intro_begin: 'শুরু করুন',
      vault_key_step_title: 'কী {{k}}, সর্বোচ্চ ৫টির মধ্যে',
      vault_key_step_replace:
        'ভল্টের জন্য নির্দিষ্ট, ফ্যাক্টরি-রিসেট YubiKey ব্যবহার করুন। এর ভল্ট স্লটে পুরনো কী থাকলে প্রতিস্থাপনের আগে সেটআপ সতর্ক করবে।',
      vault_pin_choose_title: 'একটি PIN বাছুন',
      vault_pin_choose_sub: 'এই কী ব্যবহারের প্রতিবার এটি দিতে হবে। ৬ থেকে ৮ অঙ্ক।',
      vault_pin_confirm_label: 'আবার লিখুন',
      vault_pin_mismatch: 'দুটি PIN মিলছে না।',
      vault_pin_not_default: '123456 ছাড়া অন্য কিছু বাছুন।',
      vault_puk_title: 'আপনার পুনরুদ্ধার কোড',
      vault_puk_body:
        'এটি লিখে নিরাপদ জায়গায় রাখুন। PIN ভুলে গেলে এই কী খোলার এটিই একমাত্র উপায়, এবং এটি আর দেখানো হবে না।',
      vault_puk_ack: 'আমি লিখে নিয়েছি',
      vault_reset_offer: 'এই কী রিসেট করুন',
      vault_reset_title: 'এই কী রিসেট করবেন?',
      vault_reset_body:
        'এই কী আগে ব্যবহার হয়েছে। রিসেট করলে এর PIV অ্যাপ্লিকেশনের প্রতিটি সার্টিফিকেট ও কী মুছে যাবে, শুধু ভল্ট স্লট নয়। এই কীর উপর নির্ভরশীল সবকিছু কাজ করা বন্ধ করবে।',
      vault_reset_ack: 'আমি বুঝেছি এতে এই কীর সবকিছু মুছে যাবে',
      vault_reset_unknown_ack:
        'এই ডিভাইসটি নিশ্চিত করতে পারে না যে এই কীটি অন্য কোনো প্রোফাইলের ভল্ট এখনও সুরক্ষিত করছে কিনা। আমি বুঝেছি এটি মুছে ফেললে ওই কী চিরতরে নষ্ট হতে পারে।',
      vault_reset_own_draft_ack:
        'এই কীতে আপনার আগের অসমাপ্ত চেষ্টার কী রয়েছে, এবং এই ডিভাইসে এখনও তার রেকর্ড আছে। আমি বুঝেছি রিসেট করলে তা মুছে যাবে এবং ওই চেষ্টা আর চালিয়ে নেওয়া যাবে না।',
      vault_reset_nothing_erased: 'কিছুই মোছা হয়নি। এই কীর ভল্ট স্লট খালি নয়, তাই আরও একটি বিষয় নিশ্চিত করতে হবে।',
      vault_reset_confirm: 'মুছে ফেলে রিসেট করুন',
      vault_resetting: 'কী রিসেট করা হচ্ছে…',
      vault_reset_done: 'কী রিসেট হয়েছে। চালিয়ে যেতে আবার ট্যাপ করুন।',
      vault_reset_uncertain: 'রিসেট শেষ হয়নি। এই কী হয়তো ইতিমধ্যেই মুছে গেছে — এর উপর নির্ভর করার আগে যাচাই করুন।',
      vault_err_reset_enrolled: 'ওই কী ইতিমধ্যে আপনার ভল্টের অংশ — রিসেট করলে আপনার একটি ভল্ট কী নষ্ট হবে।',
      vault_err_reset_unreadable:
        'এই ডিভাইসে আপনার ভল্টের রেকর্ড পড়া যায়নি, তাই এই কী আপনার ভল্ট কীগুলির একটি নয় তা প্রমাণ করার উপায় নেই। কিছুই মোছা হয়নি এবং রিসেট প্রত্যাখ্যান করা হয়েছে।',
      vault_enrollment_resume_pin_hint:
        'সংরক্ষিত কী আবার শুরু করতে আগে বেছে নেওয়া PIN লাগবে, নতুন কোনোটি নয়। তিনবার ভুল হলে কী লক হয়ে যাবে।',
      vault_setup_step: 'ধাপ {{n}} / {{total}}',
      vault_enrollment_resume: 'সংরক্ষিত কী চালিয়ে যান',
      vault_enrollment_reset_required:
        'এই YubiKey আর ফ্যাক্টরি অবস্থায় নেই, তাই একে বর্তমান অবস্থায় নথিভুক্ত করা যাবে না। ভল্ট কী রাখার আগে এর PIV অ্যাপ্লিকেশন মুছে ফ্যাক্টরি অবস্থায় ফেরাতে হবে।',
      vault_enrollment_state_uncertain:
        'একটি সেটআপ চেষ্টা মাঝপথে থেমে যাওয়ায় YubiKey কোন অবস্থায় রয়ে গেছে তা নিশ্চিত করা যাচ্ছে না। এর PIV অ্যাপ্লিকেশন মুছে ফ্যাক্টরি অবস্থায় না ফেরানো পর্যন্ত ওই কী আর ব্যবহার করা হবে না।',
      vault_nfc_enroll_message: 'সেট আপ করতে আপনার YubiKey এখানে ধরুন',
      vault_nfc_adopt_message: 'যাচাই করতে উদ্ধার করা এই YubiKey এখানে ধরুন',
      vault_nfc_sign_batch: 'সই করতে আপনার YubiKey এখানে ধরুন — ব্যাচ {{b}}/{{n}}',
      vault_key_use_different: 'অন্য একটি YubiKey ব্যবহার করুন',
      vault_key_setup_again: 'আবার সেট আপ করুন',
      vault_name_default: 'কী {{k}}',
      vault_more_title: 'আরেকটি কী যোগ করবেন?',
      vault_more_body: 'সর্বোচ্চ ৫টি কী। এর যেকোনো একটি একাই পুরো ভল্ট খুলতে পারে।',
      vault_more_add: 'আরেকটি কী যোগ করুন',
      vault_more_finish: 'শেষ করুন',
      vault_more_need_two: 'শেষ করার আগে দ্বিতীয় কী যোগ করুন — একটি কী মানে কোনো পুনরুদ্ধার নেই।',
      vault_leave_setup: 'সেটআপ ছেড়ে যান',
      vault_leave_title: 'সেটআপ ছেড়ে যাবেন?',
      vault_leave_body:
        'আপনি যে {{count}}টি YubiKey সেট আপ করেছেন তা এখনও সংরক্ষিত হবে না। তাদের কী থেকে যায়, তাই এক মিনিটের মধ্যে আবার যোগ করতে পারবেন।',
      vault_leave_confirm: 'ছেড়ে যান',
      vault_leave_stay: 'থাকুন',
      vault_done_body: '{{count}}টি কী এই ভল্ট খুলতে পারে। কেবল এই কী-গুলিই এটি খোলে — আপনার রিকভারি ফ্রেজ নয়।',
      vault_done_cta: 'সম্পন্ন',
      vault_add_key_row: 'কী যোগ করুন',
      vault_add_key_done: '{{nickname}} এখন থেকে করা জমা খুলতে পারবে। ভল্ট পুনরায় লক করুন যাতে এটি সবকিছু খুলতে পারে।',
      vault_key_added_toast: 'কী যোগ করা হয়েছে',
      vault_recovery_verified: 'উদ্ধার করা কী এই ডিভাইসে যাচাই হয়েছে',
      vault_relock_row: 'ভল্ট পুনরায় লক করুন',
      vault_relock_now: 'এখনই পুনরায় লক করুন',
      vault_relock_choose: 'পুনরায় লক করতে কোন কী ট্যাপ করবেন?',
      vault_relock_reason: 'আপনার বিদ্যমান কী-গুলির একটি ট্যাপ করুন ({{names}}) — যেটি এখনই যোগ করলেন সেটি নয়।',
      vault_relock_reason_generic: 'সই করতে আপনার YubiKey এখানে ধরুন — আপনার বর্তমান কী-গুলিতে ভল্ট পুনরায় লক করুন',
      vault_relock_reason_remaining: 'কেবল আপনার অবশিষ্ট কী-গুলিতে পুনরায় লক করুন: {{names}}।',
      vault_key_pending_removal: 'সরানো হয়েছে — শেষ করতে পুনরায় লক করুন',
      vault_key_removal_awaiting_network: 'সরানো হচ্ছে — নেটওয়ার্ক পুনরায় লক গ্রহণ করার অপেক্ষায়',
      vault_removal_awaiting_network: 'পুনরায় লক পাঠানো হয়েছে। নেটওয়ার্ক গ্রহণ করলেই জমা ও উত্তোলন আবার চালু হবে।',
      vault_err_too_many_active_keys: 'ভল্ট সর্বোচ্চ পাঁচটি কী দিয়ে লক হয়। জমা, পুনরায় লক বা ব্যালান্সের কিছু অংশ উত্তোলনের আগে একটি কী সরান।',
      vault_key_section_over_limit: 'সিকিউরিটি কী ({{count}}, একটি সরান)',
      vault_add_key_done_over_limit: '{{nickname}} যোগ করা হয়েছে। ভল্ট সর্বোচ্চ পাঁচটি কী দিয়ে লক হয়, তাই এটি যে কী-এর জায়গা নিচ্ছে সেটি সরান, তারপর একবার পুনরায় লক করুন।',
      vault_relock_done: 'ভল্ট আপনার বর্তমান কী-গুলিতে পুনরায় লক হয়েছে',
      vault_relock_capped: 'আরও {{count}}টি জমা বাকি — সরাতে আবার পুনরায় লক করুন।',
      vault_relock_unreachable:
        '{{count}}টি জমা কেবল {{names}} দিয়ে খোলা যায়। সেগুলির একটি দিয়ে আবার পুনরায় লক করুন।',
      vault_badge_missing: '{{count}}টি জমা এখনও {{nickname}}-এর জন্য খোলা নয়',
      vault_badge_removed: '{{count}}টি জমা এখনও একটি সরানো কী দিয়ে খোলা যায়',
      vault_key_action_rename: 'নাম বদলান',
      vault_key_action_remove: 'সরান',
      vault_rename_title: '{{nickname}}-এর নাম বদলান',
      vault_rename_save: 'সংরক্ষণ',
      vault_remove_title: '{{nickname}} সরাবেন?',
      vault_remove_body:
        'ওয়ালেট আর {{nickname}} ব্যবহার করবে না। ভল্টে থাকা টাকা পুনরায় লক না করা পর্যন্ত এটি দিয়ে খোলা যাবে (≈ {{fee}} sats)।',
      vault_remove_only: 'কেবল সরান',
      vault_key_removed_toast: 'কী সরানো হয়েছে',
      vault_export_explainer:
        'প্রতিটি ভল্ট জমা তার সল্ট গোপন রাখে যতক্ষণ না আপনি তা খরচ করেন — লকিং স্ক্রিপ্টে এর সম্পর্কে কিছুই দেখা যায় না। এনক্রিপ্টেড ব্যাকআপ চালু রাখুন, যাতে নতুন ডিভাইস আপনার YubiKey ব্যবহারের জন্য প্রয়োজনীয় কী রেকর্ড পুনরুদ্ধার ও প্রমাণীকরণ করতে পারে।',
      vault_footnote: 'কেবল এই কী-গুলিই ভল্ট খোলে। আপনার রিকভারি ফ্রেজ নয়।',
      vault_floor_line:
        'সর্বনিম্ন জমা {{floorDisplay}} ({{floorSats}} sats)। একটি ভল্ট জমা তৈরিতে প্রায় {{feeDisplay}} খরচ হয়।',
      vault_first_deposit_title: 'প্রথম ভল্ট জমা',
      vault_first_deposit_body:
        'প্রথম ভল্ট জমা — {{amount}} কেবল {{count}}টি YubiKey ({{names}}) দিয়ে খোলা যাবে। আপনার রিকভারি ফ্রেজ কাজে আসবে না।',
      vault_backup_off_title: 'প্রথমে ব্যক্তিগত ব্যাকআপ চালু করুন',
      vault_backup_off_body:
        'ওয়ালেট নতুন ভল্ট আউটপুট তৈরি করার আগে ব্যক্তিগত ব্যাকআপ চালু থাকতে হবে। এটি পুনরুদ্ধারের জন্য প্রয়োজনীয় প্রমাণীকৃত YubiKey রেকর্ড এবং ওয়ালেট ইতিহাস সংরক্ষণ করে। ভল্ট চেঞ্জ তৈরি না করে আপনি এখনও সব তুলে নিতে পারেন।',
      vault_backup_off_cta: 'ব্যাকআপ সেটিংস খুলুন',
      vault_seed_not_preserved_title: 'প্রথমে আপনার রিকভারি বাক্যাংশ সংরক্ষণ করুন',
      vault_seed_not_preserved_body:
        'আপনার ভল্ট শুধুমাত্র আপনার রিকভারি বাক্যাংশ এবং আপনার একটি YubiKey দিয়ে পুনরুদ্ধার করা যেতে পারে। YubiKey-তে এর কোনো কপি থাকে না — আপনি এটি লিখে রাখার আগে হারিয়ে গেলে এই তহবিল পুনরুদ্ধার করা যাবে না। জমা দেওয়ার আগে এখনই এটি সংরক্ষণ করুন।',
      vault_seed_not_preserved_cta: 'রিকভারি বাক্যাংশ সংরক্ষণ করুন',
      vault_choose_key: 'কোন কী ট্যাপ করবেন?',
      vault_remainder_title: 'সব তুলে নেবেন?',
      vault_remainder_body:
        '{{amount}} তুললে {{remainder}} থাকবে, যা ১০০,০০০ sat ভল্ট ন্যূনতমের নিচে। পুরো ভল্ট আপনার দৈনন্দিন ব্যালেন্সে চলে যাবে।',
      vault_remainder_all: 'সব তুলে নিন',
      vault_remainder_change: 'পরিমাণ বদলান',
      vault_unreachable_title: 'ভল্টের একটি অংশে অন্য কী দরকার',
      vault_unreachable_body:
        'ভল্টের একটি অংশে অন্য কী দরকার — {{moved}} সরানো হয়েছে। {{amount}} ধারণকারী {{count}}টি জমা কেবল {{names}} দিয়ে খোলা যায়। সেগুলির একটি দিয়ে আবার তুলুন।',
      vault_sign_progress: '{{total}}টির মধ্যে {{signed}}টি সই হয়েছে',
      vault_err_pin_locked_enroll:
        'এই YubiKey-এর PIN ব্লক করা। Yubico Authenticator-এ এর PUK দিয়ে আনব্লক করুন, অথবা অন্য YubiKey দিয়ে সেট আপ করুন।',
      vault_err_serial_mismatch_chosen:
        'এটি {{tappedName}}। আপনি {{chosenName}} বেছেছিলেন — সেটি ট্যাপ করুন, অথবা ফিরে গিয়ে {{tappedName}} বাছুন।',
      vault_err_key_already_enrolled: 'আপনি এই YubiKey ({{nickname}}) আগেই যোগ করেছেন। অন্য একটি ট্যাপ করুন।',
      vault_err_not_released: 'এই রিলিজে ভল্টে জমা বন্ধ রাখা হয়েছে।',
      vault_err_not_on_mainnet: 'ভল্ট কেবল মেইননেটে উপলব্ধ। ওয়ালেটটি মেইননেটে নিয়ে আবার চেষ্টা করুন।',
      vault_err_backup_off:
        'নতুন ভল্ট আউটপুট তৈরির আগে এনক্রিপ্টেড ব্যক্তিগত ব্যাকআপ চালু করুন। আপনি এখনও সব তুলে নিতে পারেন।',
      vault_err_not_enough_keys: 'এই ভল্টে দুটির কম কী আছে। আবার সেট আপ করুন।',
      vault_err_too_many_keys: 'একটি ভল্টে সর্বোচ্চ পাঁচটি কী থাকতে পারে।',
      vault_err_last_keys: 'ভল্টে কমপক্ষে দুটি কী দরকার। এটি সরানোর আগে আরেকটি যোগ করুন।',
      vault_err_relock_required: 'প্রথমে ভল্ট পুনরায় লক করুন যাতে আপনার অন্য কী-গুলি প্রতিটি জমা খুলতে পারে।',
      vault_err_action_pending: 'ভল্টের একটি লেনদেন এখনও শেষ হয়নি। শেষ হলে আবার চেষ্টা করুন।',
      vault_resolve_held_deposit_action: 'বাধাগ্রস্ত জমা সম্পন্ন করুন',
      vault_resolve_held_deposit_done: 'জমার সমস্যা সমাধান হয়েছে',
      vault_resolve_held_deposit_failed: 'জমার সমস্যা সমাধান করা যায়নি। আবার চেষ্টা করুন।',
      vault_resolve_held_deposit_none:
        'এখানে সমাধান করার মতো কিছু নেই — একটি মুলতুবি লেনদেন পুনরায় চেষ্টা করতে বা বাতিল করতে অ্যাক্টিভিটি ট্যাব দেখুন।',
      vault_err_key_not_committed:
        '{{nickname}} এই ভল্টের কোনো জমাই খুলতে পারে না। আপনার অন্য একটি ভল্ট কী ব্যবহার করুন।',
      vault_err_key_cannot_cover:
        '{{nickname}} ভল্টের {{total}}-এর মধ্যে {{reachable}} খুলতে পারে। সর্বোচ্চ {{reachable}} তুলুন, অথবা {{otherNames}} ব্যবহার করুন।',
      vault_err_too_small_to_relock:
        'এই ভল্টে ১০০,০০০ sats-এর কম আছে, যা পুনরায় লক করার জন্য খুব কম। বরং তুলে নিয়ে আবার জমা দিন।',
      vault_err_bad_version: 'লেনদেনটি ভুল ফরম্যাটে তৈরি হয়েছে। কিছুই সই হয়নি — আবার চেষ্টা করুন।',
      vault_err_chain_scan_failed:
        'ভল্ট জমার জন্য ব্লকচেইন স্ক্যান সম্পূর্ণ করা যায়নি। আপনার সংযোগ পরীক্ষা করে আবার চেষ্টা করুন।',
      vault_shares_unavailable: 'প্রিন্ট করার মতো কোনো কী উপাদান নেই।',
      vault_enroll_begin: 'ভল্ট সেট আপ করুন',
      vault_enroll_phase_connecting: 'আপনার কী পড়া হচ্ছে…',
      vault_enroll_phase_pin_check: 'আপনার কী যাচাই করা হচ্ছে…',
      vault_enroll_phase_checking_slot: 'ভল্ট স্লট পরীক্ষা করা হচ্ছে…',
      vault_enroll_phase_personalizing: 'আপনার PIN ও পুনরুদ্ধার কোড সেট করা হচ্ছে…',
      vault_enroll_phase_generating: 'ভল্ট কী তৈরি হচ্ছে — কী জ্বলে উঠলে স্পর্শ করুন',
      vault_enroll_phase_challenging: 'নতুন ভল্ট কী যাচাই হচ্ছে — কী জ্বলে উঠলে স্পর্শ করুন',
      vault_enroll_phase_done: 'সম্পন্ন',
      vault_touch_when_blinks: 'কী জ্বলে উঠলে সোনালি সংযোগস্থল স্পর্শ করুন।',
      vault_nfc_activation_hint:
        'এই YubiKey প্রথমবার ব্যবহার করছেন? প্রথমে এটি কয়েক সেকেন্ডের জন্য USB-C পোর্টে লাগান — নতুন কী-গুলো NFC বন্ধ অবস্থায় পাঠানো হয়, ততক্ষণ পর্যন্ত এটি চালু হয় না।',
      vault_continue: 'চালিয়ে যান',
      vault_back: 'ফিরে যান',
      vault_enrolled_toast: 'ভল্ট প্রস্তুত',
      vault_insert_key: 'আপনার YubiKey ঢোকান',
      vault_hold_key_nfc: 'আপনার YubiKey ফোনের উপরের অংশে ধরে রাখুন',
      vault_keep_holding_nfc: 'ধরে রাখুন — আনলক হচ্ছে…',
      vault_unlocking_funds: 'আপনার তহবিল আনলক করা হচ্ছে',
      vault_unlocking_sub: 'কয়েক সেকেন্ড সময় লাগবে। অ্যাপটি খোলা রাখুন।',
      vault_sending_to_network: 'নেটওয়ার্কে পাঠানো হচ্ছে',
      vault_sending_sub: 'সঙ্গে সঙ্গে না গেলে আমরা চেষ্টা চালিয়ে যাব।',
      vault_reading_key: 'কী পড়া হচ্ছে…',
      vault_enter_pin: 'YubiKey PIN দিন',
      vault_pin_sub_nfc: 'তারপর শেষ করতে আপনার কী ফোনের উপরের অংশে ধরুন।',
      vault_pin_sub_usb: 'তারপর শেষ করতে সোনালি সংযোগে স্পর্শ করুন।',
      vault_touch_contact: 'সোনালি সংযোগস্থল স্পর্শ করুন',
      vault_unlock_cta: 'আনলক',
      vault_pin_retries: '{{count}}টি চেষ্টা বাকি',
      vault_locked: 'ভল্ট লক করা',
      vault_retry: 'আবার চেষ্টা করুন',
      vault_cancel: 'বাতিল',
      vault_dismiss: 'বন্ধ করুন',
      vault_err_wrong_key: 'এই YubiKey এই ভল্টের কী-গুলির একটি নয়।',
      vault_err_serial_mismatch: 'এই YubiKey এই ভল্টের কী-গুলির একটি নয় ({{names}})।',
      vault_err_touch_timeout: 'কোনো স্পর্শ শনাক্ত হয়নি। আবার চেষ্টা করুন এবং সোনালি সংযোগস্থল স্পর্শ করুন।',
      vault_err_pin_locked:
        'এই YubiKey-এর PIN ব্লক করা। আপনার অন্য একটি ভল্ট কী ব্যবহার করুন, অথবা Yubico Authenticator-এ এর PUK দিয়ে আনব্লক করুন।',
      vault_err_pin_invalid: 'ভুল PIN।',
      vault_err_puk_invalid: 'ভুল PUK।',
      vault_err_puk_locked: 'এই YubiKey-এর PUK ব্লক করা। একটি নতুন YubiKey ব্যবহার করুন।',
      vault_err_ceremony_active: 'অন্য একটি ভল্ট কাজ এখনও YubiKey ব্যবহার করছে। সেটি শেষ হলে আবার চেষ্টা করুন।',
      vault_err_scope_changed: 'এই ভল্ট কাজের সময় ওয়ালেট বা নেটওয়ার্ক বদলেছে। বর্তমান ওয়ালেটে আবার শুরু করুন।',
      vault_err_key_removed_mid_op: 'কী সরিয়ে ফেলা হয়েছে। এটি ঢুকিয়ে আবার চেষ্টা করুন।',
      vault_err_no_key: 'কোনো YubiKey শনাক্ত হয়নি।',
      vault_err_driver_unavailable: 'এই ডিভাইসে YubiKey সমর্থন উপলব্ধ নেই।',
      vault_err_mgmt_key_custom:
        'এই কী-তে একটি কাস্টম ম্যানেজমেন্ট কী আছে এবং এটি স্বয়ংক্রিয়ভাবে ব্যবহার করা যায় না।',
      vault_err_attestation_invalid:
        'এই YubiKey-এর প্রস্তুতকারক প্রত্যয়ন যাচাই করা যায়নি। অপরিবর্তিত PIV প্রত্যয়ন স্লটসহ আসল, কারখানা-প্রত্যয়িত YubiKey ব্যবহার করুন।',
      vault_err_generic: 'কিছু ভুল হয়েছে। অনুগ্রহ করে আবার চেষ্টা করুন।',
      vault_err_template_invalid: 'ভল্টে প্রবেশের চেষ্টায় সমস্যা হয়েছে। আবার চেষ্টা করুন।',
      vault_err_not_enrolled: 'কোনো ভল্ট সেট আপ করা নেই।',
      vault_err_pin_required: 'আপনার PIN লিখুন।',
      vault_err_unsupported_platform: 'এই ডিভাইসে YubiKey সমর্থন উপলব্ধ নেই।',
      vault_err_slot_occupied: 'কী স্লট ইতিমধ্যে ব্যবহৃত হচ্ছে।',
      vault_replace_key_warning:
        'এটি প্রতিস্থাপন করলে বিদ্যমান P-256 প্রাইভেট কী স্থায়ীভাবে ধ্বংস হবে। সেই কীর উপর নির্ভরশীল ভল্টের তহবিল খরচের অযোগ্য হয়ে যেতে পারে।',
      vault_replace_key_confirm: 'বিদ্যমান কী প্রতিস্থাপন করুন',
      vault_resume_draft_confirm_title: 'এই প্রস্তুত কী ব্যবহার করবেন?',
      vault_resume_draft_confirm_body:
        'একটি পূর্বে প্রস্তুত করা কী — {{nickname}} · …{{tail}} — যোগ করার জন্য প্রস্তুত, তবে এই ডিভাইসটি এইমাত্র কার্ডে এটি যাচাই করেনি। শুধুমাত্র তখনই এগিয়ে যান যদি আপনি এটি চেনেন।',
      vault_resume_draft_confirm_use: 'এই কী ব্যবহার করুন',
      vault_resume_draft_confirm_discard: 'এটি বাতিল করুন',
      vault_err_enrollment_partial:
        'কী পরিবর্তনের পরে YubiKey সেটআপ থেমেছে। পুনরায় চেষ্টার নির্দেশ মানুন; এই কী ভল্টে যোগ হয়নি।',
      vault_err_key_not_adopted:
        'উত্তোলনে ব্যবহার করার আগে এই ডিভাইসে উদ্ধার করা YubiKey আপনার কাছে আছে তা যাচাই করুন।',
      vault_err_too_many_inputs:
        'একবারে সরানোর জন্য অনেক বেশি ভল্ট ডিপোজিট। আগে ছোট পরিমাণ তুলুন — প্রতিটি উত্তোলন ভল্টকে একত্রিতও করে, তাই পরেরবার আরও বেশি সরানো যাবে।',
      vault_err_requires_online:
        'ভল্ট লেনদেনের জন্য সংযোগ দরকার। পুনরায় সংযুক্ত হয়ে আবার চেষ্টা করুন — ভল্টের তহবিল কখনও পরে পাঠানোর জন্য সারিতে রাখা হয় না।',
      vault_err_user_cancelled: 'বাতিল করা হয়েছে।',
      vault_err_vault_empty: 'ভল্টে এখনও তোলার মতো কোনো তহবিল নেই।',
      vault_err_amount_exceeds_balance: 'এটি ভল্টে থাকা পরিমাণের চেয়ে বেশি।',
      vault_err_below_dust: 'এই পরিমাণ পাঠানোর জন্য খুব কম।',
      vault_err_no_transaction: 'স্থানান্তর সম্পন্ন করা যায়নি। আবার চেষ্টা করুন।',
      vault_err_nfc_lost: 'সংযোগ বিচ্ছিন্ন হয়েছে। কী-টি ফোনের সঙ্গে স্থিরভাবে ধরে রাখুন।',
      vault_pin_invalid_retry: 'ভুল PIN — আর {{count}} বার চেষ্টা করা যাবে। আবার চেষ্টা করতে উত্তোলন চাপুন।',
      vault_pin_invalid_retry_generic: 'ভুল PIN। আবার চেষ্টা করতে উত্তোলন চাপুন।',
      vault_disable_row: 'ভল্ট নিষ্ক্রিয় করুন',
      vault_disable_title: 'ভল্ট নিষ্ক্রিয় করবেন?',
      vault_disable_message: 'এটি এই ফোনে ভল্টের কী তালিকা মুছে দেয়। কী-গুলি আপনার YubiKey-তে থেকে যায়।',
      vault_disable_confirm: 'নিষ্ক্রিয় করুন',
      vault_disabled_toast: 'ভল্ট নিষ্ক্রিয়',
      vault_disable_blocked_title: 'আগে তহবিল তুলে নিন',
      vault_disable_blocked_message:
        'ভল্টে এখনও তহবিল আছে। বন্ধ করার আগে সবকিছু আপনার দৈনন্দিন ব্যালেন্সে তুলে নিন — না হলে ওই তহবিল লক হয়ে যাবে এবং স্বাক্ষর করার কোনো উপায় থাকবে না।',
      vault_ok: 'ঠিক আছে',
      vault_mock_toggle: 'মক YubiKey ব্যবহার করুন (dev)',
      vault_on: 'চালু',
      vault_off: 'বন্ধ',
      vault_shares_word_count: 'রিকভারি শেয়ার কেবল বারো-শব্দের ফ্রেজ সমর্থন করে। বরং আপনার ফ্রেজটি লিখে রাখুন।',
      scan_shares_legacy_title: 'পুরোনো ব্যাকআপ ফরম্যাট',
      scan_shares_legacy_message:
        'এই শেয়ারগুলো পুরোনো ফরম্যাটের — এগুলো আপনার দৈনন্দিন ব্যালেন্স ফিরিয়ে আনে, কিন্তু ভল্ট খুলতে পারে না। আপনার বারো-শব্দের রিকভারি ফ্রেজ এখনও থাকলে সেটি দিয়েই পুনরুদ্ধার করুন — সেটি সবকিছু ফিরিয়ে আনে — এবং পরে নতুন শেয়ার প্রিন্ট করুন। ফ্রেজটি হারিয়ে গেলে আপনার তহবিল নতুন ওয়ালেটে পাঠিয়ে আবার শুরু করুন।',
      scan_shares_legacy_ack: 'আমি বুঝেছি',

      delete_wallet: 'ওয়ালেট মুছুন',
      delete_wallet_warning_title: 'ওয়ালেট মুছবেন?',
      delete_wallet_warning_body:
        'এটি এই ডিভাইস থেকে আপনার ওয়ালেট কী স্থায়ীভাবে মুছে ফেলবে। যদি আপনি আপনার রিকভারি ফ্রেজ ব্যাকআপ না করেন বা রিকভারি শেয়ার প্রিন্ট না করেন, তাহলে আপনি চিরতরে আপনার ওয়ালেটে অ্যাক্সেস হারাবেন।',
      delete_wallet_confirm: 'ওয়ালেট মুছুন',
      delete_wallet_failed_title: 'ওয়ালেট মোছা ব্যর্থ হয়েছে',
      delete_wallet_failed_body: 'আপনার ওয়ালেটটি এই ডিভাইস থেকে সম্পূর্ণভাবে মোছা যায়নি। কিছু কী-উপাদান এখনও থাকতে পারে। অনুগ্রহ করে আবার চেষ্টা করুন।',
      bsv_network: 'BSV নেটওয়ার্ক',
      arc_endpoint: 'ARC এন্ডপয়েন্ট',
      arc_api_token: 'API টোকেন',
      arc_custom_url: 'কাস্টম URL',
      arc_apply: 'প্রয়োগ করুন',
      arc_reset_default: 'ডিফল্টে ফিরিয়ে নিন',
      arc_requires_token: 'টোকেন প্রয়োজন',
      arc_url_https_required: 'https হতে হবে (স্থানীয় পরীক্ষার জন্য http://localhost অনুমোদিত)',
      recovery_phrase: 'রিকভারি কী',
      you_have: 'আপনার আছে',
      switching: 'পরিবর্তন হচ্ছে...',
      mainnet: 'মেইননেট',
      testnet: 'টেস্টনেট',
      teratest: 'Teratest',
      identity_key: 'আইডেন্টিফায়ার',
      transactions: 'লেনদেন',

      no_transactions: 'এখনো কোনো লেনদেন নেই',
      tx_status_confirmed: 'নিশ্চিত',
      tx_status_received: 'পাওয়া গেছে',
      tx_status_sent: 'পাঠানো হয়েছে',
      tx_status_spent: 'খরচ হয়েছে',
      tx_status_transferred: 'স্থানান্তরিত',
      tx_status_accepted: 'গৃহীত',
      tx_status_broadcasting: 'প্রসেস হচ্ছে',
      tx_status_not_sent: 'পাঠানো হয়নি',
      tx_status_unsigned: 'খসড়া',
      tx_status_nonfinal: 'অপেক্ষমাণ',
      tx_status_pending: 'অপেক্ষমাণ',
      tx_status_failed: 'ব্যর্থ',
      tx_status_offline_queued: 'অফলাইন · সারিতে',
      tx_status_offline_sending: 'অফলাইন · পাঠানো হচ্ছে',
      tx_status_offline_rejected: 'অফলাইন · প্রত্যাখ্যাত',
      tx_copied: 'লেনদেন কপি করা হয়েছে',
      tx_not_available: 'লেনদেন পাওয়া যাচ্ছে না',
      tx_copy_failed: 'লেনদেন কপি করতে ব্যর্থ',
      tx_abort_success: 'লেনদেন বাতিল করা হয়েছে',
      tx_abort_failed: 'লেনদেন বাতিল করতে ব্যর্থ',
      tx_abort_maybe_delivered: 'বাতিল করা যাচ্ছে না — এই পেমেন্টটি ইতিমধ্যে প্রাপকের কাছে পৌঁছে থাকতে পারে',
      tx_proof_refreshed: 'লেনদেন নিশ্চিত হয়েছে',
      tx_proof_refresh_failed: 'প্রমাণ আনা যায়নি — লেনদেনটি হয়তো এখনও নিশ্চিত হয়নি',
      tx_marked_failed: 'প্রসেস হয়নি — ব্যর্থ হিসেবে চিহ্নিত ও তহবিল মুক্ত করা হয়েছে',
      tx_still_pending: 'এখনও অপেক্ষমাণ — নিশ্চিত হয়নি',
      tx_action_explorer: 'ব্লক এক্সপ্লোরারে দেখুন',
      tx_export_success: '{{count}}টি লেনদেন এক্সপোর্ট হয়েছে',
      tx_export_failed: 'লেনদেন এক্সপোর্ট করা যায়নি',
      tx_export_csv: 'CSV এক্সপোর্ট করুন',
      tx_export: 'এক্সপোর্ট করুন',
      activity_filter: 'কার্যকলাপ ফিল্টার করুন',
      activity_filter_all: 'সব',
      activity_search_placeholder: 'নোট, নাম, @হ্যান্ডেল খুঁজুন',
      activity_search_clear: 'অনুসন্ধান মুছুন',
      activity_filter_no_match: 'মিলে যাওয়া কোনো কার্যকলাপ নেই',
      wallet_balance_label: 'ব্যালেন্স',
      wallet_balance_you_have: 'আপনার আছে',
      wallet_balance_refresh: 'ব্যালেন্স রিফ্রেশ করুন',
      wallet_group_today: 'আজ',
      wallet_group_yesterday: 'গতকাল',
      back: 'পেছনে',
      wallet_payments: 'পেমেন্ট',
      wallet_vault: 'ভল্ট',
      wallet_settings: 'সেটিংস',
      wallet_activity: 'কার্যকলাপ',

      message_box_server: 'মেসেজ বক্স সার্ভার',
      message_box_required: 'পরিচয়-ভিত্তিক পেমেন্ট পাঠাতে ও গ্রহণ করতে প্রয়োজনীয়।',
      send_payment: 'পেমেন্ট পাঠান',
      recipient: 'প্রাপক',
      amount: 'পরিমাণ',
      enter_valid_amount: 'শূন্যের চেয়ে বেশি পরিমাণ লিখুন',
      display_currency: 'প্রদর্শন মুদ্রা',
      no_pending_payments: 'কোনো মুলতুবি পেমেন্ট নেই',
      outgoing_payments: 'বহির্গামী',
      payment_not_delivered: 'ডেলিভারি হয়নি',
      payment_delivered: 'ডেলিভারি হয়েছে',
      retry: 'পুনরায় চেষ্টা',
      retry_failed: 'পুনরায় চেষ্টা ব্যর্থ',
      dismiss: 'বরখাস্ত',
      valid_identity_key: 'বৈধ আইডেন্টিফায়ার প্রবেশ করা হয়েছে',
      valid_bsv_address: 'বৈধ BSV ঠিকানা প্রবেশ করা হয়েছে',
      searching: 'খুঁজছি...',
      unknown: 'অজানা',
      from_prefix: 'থেকে:',
      message_box_saved: 'মেসেজ বক্স সার্ভার সংরক্ষিত হয়েছে',
      message_box_removed: 'মেসেজ বক্স সার্ভার সরানো হয়েছে',
      message_box_off: 'বন্ধ',
      message_box_off_hint: 'মেসেজ বক্স সার্ভার বন্ধ আছে। সেটিংস › অ্যাডভান্সড থেকে চালু করুন।',
      enter_valid_url: 'একটি বৈধ URL প্রবেশ করুন',
      checking_connection: 'সংযোগ পরীক্ষা করা হচ্ছে...',
      connection_failed: 'সংযোগ ব্যর্থ',
      recipient_placeholder: 'নাম, আইডেন্টিফায়ার বা BSV ঠিকানা...',

      trust_updated: 'বিশ্বাস নেটওয়ার্ক আপডেট হয়েছে',
      failed_to_save: 'সংরক্ষণ করতে ব্যর্থ',
      confirm_delete: 'মুছে ফেলার নিশ্চয়তা দিন',
      confirm_delete_body: 'আপনি কি সত্যিই এই সার্টিফায়ারটি মুছতে চান?',
      order_certifiers_hint:
        'সার্টিফায়ারগুলো অগ্রাধিকার অনুযায়ী সাজান। অ্যাপগুলো উচ্চ-র্যাঙ্কের সার্টিফায়ারগুলো প্রথমে দেখাবে।',
      certifiers: 'সার্টিফায়ার',
      no_certifiers: 'এখনো কোনো সার্টিফায়ার নেই।',
      add_provider: 'প্রদানকারী যোগ করুন',
      unsaved_changes: 'আপনার অসংরক্ষিত পরিবর্তন আছে',
      duplicate_key_error: 'এই পাবলিক কী সহ একটি সত্তা ইতিমধ্যে তালিকায় আছে!',
      get_provider_details: 'প্রদানকারীর বিবরণ পান',
      validate_details: 'বিবরণ যাচাই করুন',
      show_advanced: 'উন্নত দেখান',
      hide_advanced: 'উন্নত লুকান',
      description_length_error: 'বিবরণ ৫ থেকে ৫০ অক্ষরের মধ্যে হতে হবে',
      enter_domain_hint: 'প্রদানকারীর ডোমেন প্রবেশ করুন',
      enter_details_hint: 'প্রদানকারীর বিবরণ প্রবেশ করুন',

      receive: 'গ্রহণ করুন',
      send: 'পাঠান',
      generating_address: 'BSV ঠিকানা তৈরি হচ্ছে...',
      imported: 'আমদানিকৃত',
      unable_to_generate_address: 'BSV ঠিকানা তৈরি করা যাচ্ছে না',
      send_bsv: 'BSV পাঠান',
      recipient_address: 'প্রাপকের BSV ঠিকানা',
      enter_bsv_address: 'BSV ঠিকানা প্রবেশ করুন',

      wallet_data: 'ওয়ালেট ডেটা',
      create_new_wallet: 'নতুন ওয়ালেট তৈরি করুন',
      generate_recovery_phrase_caption: 'একটি নতুন রিকভারি কী তৈরি করুন',
      import_existing_wallet: 'বিদ্যমান ওয়ালেট আমদানি করুন',
      paste_recovery_phrase: 'আপনার রিকভারি কী পেস্ট করুন',
      save_recovery_phrase_heading: 'আপনার রিকভারি কী সংরক্ষণ করুন',
      save_recovery_phrase_btn: 'রিকভারি কী সংরক্ষণ করুন',
      recovery_phrase_write_down: 'এই ১২টি শব্দ ক্রমানুসারে লিখুন এবং একটি নিরাপদ স্থানে সংরক্ষণ করুন।',
      acknowledgment_text:
        'আমি আমার রিকভারি কী সংরক্ষণ করেছি এবং বুঝি যে এটি হারানো সমস্ত সম্পর্কিত তহবিল, টোকেন এবং সার্টিফিকেটের স্থায়ী ক্ষতির কারণ হবে।',
      enter_recovery_words: '১২ শব্দের নিমোনিক লিখুন',
      restore_wallet_description: 'আপনার ওয়ালেট পুনরুদ্ধার করতে রিকভারি কী প্রবেশ করুন',
      backup_reminder_title: 'আপনার ওয়ালেট ব্যাকআপ করা নেই',
      backup_reminder_body: 'তহবিল হারানো এড়াতে আপনার কী এক্সপোর্ট করুন।',
      biometric_advisory_title: 'আপনার ওয়ালেট সুরক্ষিত করুন',
      wallet_balance_show_in: '{{unit}}-এ দেখান',
      tx_detail_block: 'ব্লক',
      tx_detail_block_pending: 'ব্লকের অপেক্ষায়',
      biometric_advisory_body: 'আপনার সম্পদ Face ID বা আপনার আঙুলের ছাপ দিয়ে সুরক্ষিত করুন',
      biometric_advisory_body_degraded:
        'এই ডিভাইসে Face ID বা ফিঙ্গারপ্রিন্ট সেট করা নেই, তাই আপনার ওয়ালেট কী শুধুমাত্র ডিভাইসের পাসকোড দ্বারা সুরক্ষিত।',
      import_prompt_title: 'আগে থেকেই ওয়ালেট আছে?',
      import_prompt_body: 'আপনার রিকভারি কী বা শেয়ার থেকে সেটি পুনরুদ্ধার করুন।',
      import_prompt_action: 'ব্যাকআপ থেকে ইমপোর্ট করুন',
      nearby_advisory_title: 'কাছাকাছি সংযোগ',
      nearby_advisory_body:
        'আপনাকে Wi-Fi, ব্লুটুথ বা NFC ব্যবহারের অনুমতি দিতে বলা হবে। এটি কেবল পেমেন্টের জন্য কাছের ডিভাইসের সঙ্গে সরাসরি সংযোগ করতে ব্যবহৃত হয় — কখনও আমাদের সার্ভারে পাঠানো হয় না, আর আপনার প্রোফাইল তৈরিতেও ব্যবহৃত হয় না।',
      backup_reminder_now: 'এখনই কী ব্যাকআপ করুন',
      backup_reminder_import: 'ব্যাকআপ থেকে ইমপোর্ট করুন',
      restore_backup_checking: 'আপনার এনক্রিপ্টেড ব্যাকআপ খোঁজা হচ্ছে…',
      restore_backup_progress: 'ওয়ালেট ইতিহাস পুনরুদ্ধার হচ্ছে — {{total}}-এর {{chunks}}…',
      restore_backup_failed_title: 'ইতিহাস পুনরুদ্ধার করা যায়নি',
      restore_backup_failed_message:
        'আপনার রিকভারি ফ্রেজ ঠিক আছে, কিন্তু এনক্রিপ্টেড ব্যাকআপ রিপ্লে করা যায়নি। সাধারণত আবার চেষ্টা করলেই হয়। এটি ছাড়া চালিয়ে গেলে ওয়ালেট কাজ করবে কিন্তু আগের কোনো লেনদেন থাকবে না, আর আপনার বর্তমান কিছু কয়েন খরচ করা যাবে না।',
      restore_backup_retry: 'আবার চেষ্টা করুন',
      restore_backup_skip: 'পরিত্যাগ',
      restore_backup_unverified_title: 'ব্যাকআপ অসম্পূর্ণ হতে পারে',
      restore_backup_unverified_body:
        'আপনার ওয়ালেট পুনরুদ্ধার হয়েছে, কিন্তু সার্ভারে থাকা ব্যাকআপ সম্পূর্ণ কিনা তা নিশ্চিত করা যায়নি। যদি কিছু ইতিহাস অনুপস্থিত থাকে, তাহলে অন্য ডিভাইসের আপলোড শেষ হওয়ার পর আবার পুনরুদ্ধার করুন।',
      import_wallet: 'ওয়ালেট আমদানি করুন',

      reject: 'প্রত্যাখ্যান করুন',
      authorize: 'অনুমোদন করুন',
      renewal: 'নবায়ন',
      spending_section: 'ব্যয়',
      protocols_section: 'প্রোটোকল',
      baskets_section: 'বাস্কেট',
      certificates_section: 'সার্টিফিকেট',
      requested_fields: 'অনুরোধকৃত ক্ষেত্র',

      send_max: 'সর্বোচ্চ পাঠান',
      message_box_unreachable: 'পেমেন্ট সার্ভারে পৌঁছানো যায়নি। সেটিংসে message box URL যাচাই করুন।',
      available: 'উপলব্ধ',
      note: 'নোট',
      note_placeholder: 'কীসের জন্য? (ঐচ্ছিক)',
      entire_wallet_balance: 'সম্পূর্ণ ওয়ালেট ব্যালেন্স',
      entire_vault_balance: 'ভল্টের পুরো ব্যালেন্স',
      export_wallet_data: 'ওয়ালেট ডেটা রপ্তানি করুন',
      export_unencrypted_title: 'এই ফাইলটি এনক্রিপ্ট করা নয়',
      export_unencrypted_message:
        'রপ্তানি করা ফাইলটি আপনার ওয়ালেট ডেটার একটি সাধারণ, এনক্রিপ্ট না করা কপি — যার মধ্যে রয়েছে পরিচয় ও সার্টিফিকেট তথ্য, লেনদেনের ইতিহাস এবং পরিচিতি। এই ফাইলটি যার কাছেই থাকুক না কেন, সে এটি পড়তে পারবে। এটি শুধুমাত্র বিশ্বস্ত কোথাও শেয়ার করুন।',
      import_wallet_data: 'ওয়ালেট ডেটা আমদানি করুন',
      import_conflict_title: 'ডেটাবেস ইতিমধ্যে বিদ্যমান',
      import_conflict_message:
        'এই পরিচয়ের জন্য একটি আরও সাম্প্রতিক টাইমস্ট্যাম্পসহ ওয়ালেট ডেটাবেস ইতিমধ্যে বিদ্যমান। ফাইলের নামে সবচেয়ে বড় সংখ্যার ব্যাকআপ সর্বদা ব্যবহৃত হয়।\n\nএই পুরানো ব্যাকআপ ব্যবহার করতে: প্রথমে আপনার বর্তমান ওয়ালেট ডেটা রপ্তানি করুন, অ্যাপ মুছে পুনরায় ইনস্টল করুন, তারপর শুধুমাত্র পছন্দসই ব্যাকআপ আমদানি করুন।',
      import_confirm_title: 'ডেটাবেস আমদানি করুন',
      import_confirm_message: 'এই ডেটাবেস আপনার সক্রিয় ওয়ালেট হয়ে যাবে। চালিয়ে যেতে চান?',
      import_success: 'ডেটাবেস সফলভাবে আমদানি হয়েছে। ওয়ালেট পুনরায় চালু হচ্ছে...',
      import_invalid_file: 'অবৈধ ব্যাকআপ ফাইল',
      import_invalid_file_detail:
        'এই ফাইলটি একটি বৈধ ওয়ালেট ডেটাবেস ব্যাকআপ নয়। প্রত্যাশিত ফাইলের নাম যেমন wallet-XXXXXXXX-mainnet-TIMESTAMP.db।',
      import_oversized_file: 'ব্যাকআপ ফাইলটি অনেক বড়',
      import_oversized_file_detail:
        'এই ফাইলটি একটি ওয়ালেট ব্যাকআপের জন্য প্রত্যাশিত আকারের চেয়ে বড়, তাই এটি পড়া হয়নি। একটি প্রকৃত ওয়ালেট ডেটাবেস ব্যাকআপ ফাইল নির্বাচন করুন।',
      import_untrusted_file: 'এই ব্যাকআপ আমদানি করা যায়নি',
      import_untrusted_file_detail:
        'এই ব্যাকআপটি এই ওয়ালেটের অন্তর্গত হিসেবে যাচাই করা যায়নি, অথবা এর বিষয়বস্তু বিশ্বাসযোগ্য ছিল না। এটি আমদানি করা হয়নি।',
      import_anyway: 'তবুও আমদানি করুন',
      import_invalid_input_title: 'অবৈধ ইনপুট',
      import_invalid_input_message:
        'অনুগ্রহ করে একটি বৈধ পুনরুদ্ধার বাক্যাংশ (12-24 শব্দ) অথবা 64-অক্ষরের হেক্স প্রাইভেট কী প্রবেশ করান।',
      import_setup_failed: 'ওয়ালেট সেট আপ করতে ব্যর্থ: {{error}}',
      create_wallet_refused: 'ওয়ালেট তৈরি করা যায়নি। আবার চেষ্টা করুন।',
      create_wallet_failed: 'নিমোনিক তৈরি করতে ব্যর্থ। আবার চেষ্টা করুন।',
      or: 'অথবা',
      print_recovery_shares: 'রিকভারি শেয়ার প্রিন্ট করুন',
      scan_backup_shares: 'ব্যাকআপ শেয়ার স্ক্যান করুন',
      scan_backup_shares_caption: 'প্রিন্টেড শেয়ার থেকে ওয়ালেট পুনরুদ্ধার করুন',
      scan_shares_invalid_format: 'অবৈধ শেয়ার ফরম্যাট। একটি বৈধ ব্যাকআপ শেয়ার QR কোড স্ক্যান করুন।',
      scan_shares_camera_needed: 'ক্যামেরা অ্যাক্সেস প্রয়োজন',
      scan_shares_camera_description: 'আপনার প্রিন্টেড ব্যাকআপ শেয়ার QR কোড স্ক্যান করতে ক্যামেরা অ্যাক্সেস প্রয়োজন।',
      scan_shares_grant_camera: 'ক্যামেরা অ্যাক্সেস দিন',
      scan_shares_recovering: 'ওয়ালেট পুনরুদ্ধার হচ্ছে...',
      scan_shares_scan_first: 'আপনার প্রথম ব্যাকআপ শেয়ার স্ক্যান করুন',
      scan_shares_progress: '{{scanned}}/{{needed}} শেয়ার স্ক্যান করা হয়েছে',
      scan_shares_remaining: 'আরও {{count}} শেয়ার প্রয়োজন',
      scan_shares_complete: 'সব শেয়ার সংগৃহীত!',
      scan_shares_recovery_failed: 'পুনরুদ্ধার ব্যর্থ। পুনরায় স্ক্যান করুন।',
      scan_shares_threshold_mismatch: 'থ্রেশহোল্ড পূর্ববর্তী শেয়ারের সাথে মেলে না',
      scan_shares_integrity_mismatch: 'ইন্টিগ্রিটি হ্যাশ মেলে না — শেয়ারগুলো ভিন্ন কী থেকে এসেছে',
      scan_shares_duplicate: 'এই শেয়ারটি ইতিমধ্যে স্ক্যান করা হয়েছে',
      scan_shares_biometric_title: 'বায়োমেট্রিক অ্যাক্সেস প্রয়োজন',
      scan_shares_biometric_message:
        'আপনার ওয়ালেট কী সুরক্ষিত রাখতে বায়োমেট্রিক অ্যাক্সেস প্রয়োজন। আবার চেষ্টা করুন।',
      recovery_replace_wallet_title: 'এই ফোনের ওয়ালেটটি প্রতিস্থাপন করবেন?',
      recovery_replace_wallet_body:
        'এই ফোনে আগে থেকেই একটি ওয়ালেট আছে। এগিয়ে গেলে এটি এবং এর মাধ্যমে খরচযোগ্য সবকিছু প্রতিস্থাপিত হবে। এর রিকভারি ফ্রেজ যদি অন্য কোথাও সংরক্ষিত না থাকে, তাহলে সেই অর্থ চিরতরে হারিয়ে যাবে।',
      recovery_replace_wallet_confirm: 'ওয়ালেট প্রতিস্থাপন করুন',
      invalid_bsv_address: 'অবৈধ BSV ঠিকানা',
      scan_recipient_hint: 'পেমেন্ট কোডে ক্যামেরা তাক করুন',
      scan_qr_default_hint: 'QR কোডে ক্যামেরা তাক করুন',
      scan_wallet_qr_hint: 'ওয়ালেট পেয়ারিং QR কোডে ক্যামেরা তাক করুন',

      biometric_store_wallet: 'আপনার ওয়ালেট নিরাপদে সংরক্ষণ করুন',
      biometric_load_wallet: 'আপনার ওয়ালেট নিরাপদে লোড করুন',
      biometric_unlock_wallet: 'আপনার ওয়ালেট আনলক করুন',
      wallet_lost_title: 'ওয়ালেট কী ধ্বংস হয়েছে',
      wallet_lost_body:
        'এই ডিভাইসের বায়োমেট্রিক সেটিংস পরিবর্তিত হয়েছে, তাই সিস্টেম এই ওয়ালেট রক্ষাকারী কী ধ্বংস করেছে। আপনার কয়েন ব্লকচেইনে নিরাপদ — রিকভারি ফ্রেজ দিয়ে এই ওয়ালেট পুনরুদ্ধার করুন।',
      wallet_lost_restore: 'পুনরুদ্ধার করুন',
      wallet_lost_later: 'এখন নয়',
      wallet_locked_title: 'ওয়ালেট লক করা',
      wallet_locked_body: 'আপনার ওয়ালেট কী লোড করতে আনলক করুন।',
      wallet_unlock_retry: 'ওয়ালেট আনলক করুন',
      wallet_lockout_body: 'অনেকবার ব্যর্থ চেষ্টা। প্রথমে পাসকোড দিয়ে ফোন আনলক করুন, তারপর আবার চেষ্টা করুন।',
      wallet_no_biometrics_body:
        'এই ডিভাইসে বায়োমেট্রিক সেট করা নেই। আপনার ওয়ালেট কী এখনও ডিভাইস কীস্টোর দ্বারা এনক্রিপ্ট করা, তবে বায়োমেট্রিক যাচাই দ্বারা সুরক্ষিত নয়।',

      // Connections screen
      connections: 'সংযোগ',
      advanced: 'উন্নত',
      copy_secret_words: 'গোপন শব্দ কপি করুন',
      print_recovery_keys: 'রিকভারি কী প্রিন্ট করুন',
      backup_wallet_keys: 'ওয়ালেটের কীগুলির ব্যাকআপ নিন',
      log_out: 'লগ আউট',
      connect_to_app: 'অ্যাপে সংযোগ করুন',
      no_connections_yet: 'এখনো কোনো সংযোগ নেই',
      no_connections_subtitle: 'আপনার ওয়ালেট সংযুক্ত করতে ডেস্কটপ অ্যাপ থেকে QR কোড স্ক্যান করুন',
      connections_active: 'সক্রিয়',
      connections_disconnected: 'বিচ্ছিন্ন',
      disconnect: 'সংযোগ বিচ্ছিন্ন',
      reconnect: 'পুনরায় সংযুক্ত',
      reconnect_failed: 'পুনরায় সংযোগ ব্যর্থ',
      reconnect_confirm_title: 'এই সাইটের সাথে পুনরায় সংযোগ করবেন?',
      reconnect_confirm_message: '{{origin}} আবার আপনার ওয়ালেট অ্যাক্সেস করতে সক্ষম হবে।',
      reconnect_needs_reapproval: 'এই সংযোগটি যাচাই করা যায়নি। পুনরায় সংযোগ করতে একটি নতুন কিউআর কোড স্ক্যান করুন।',
      scan_qr_code: 'QR কোড স্ক্যান করুন',
      paste_uri: 'URI পেস্ট করুন',
      invalid_qr_code: 'অবৈধ QR কোড',
      wallet_not_ready: 'ওয়ালেট প্রস্তুত নেই',
      please_log_in_first: 'প্রথমে লগইন করুন',
      unknown_error: 'অজানা ত্রুটি',

      // Pay screen
      payments: 'পেমেন্ট',
      pay: 'পেমেন্ট',
      pay_direction_pay: 'পেমেন্ট করুন',
      pay_direction_receive: 'পেমেন্ট নিন',
      pay_cell_nearby_pay: 'কাছাকাছি থাকা কেউ',
      pay_cell_nearby_pay_sub: 'তাঁর কোড স্ক্যান করুন',
      pay_cell_handle_pay: 'দূরের কেউ',
      pay_cell_handle_pay_sub: 'একটি নাম বাছুন — তাদের এই অ্যাপ লাগবে',
      pay_cell_address_pay: 'কোনো BSV ঠিকানায়',
      pay_cell_address_pay_sub: 'BSV ঠিকানা পেস্ট করুন বা স্ক্যান করুন',
      pay_cell_nearby_get: 'কাছাকাছি থাকা কেউ',
      pay_cell_nearby_get_sub: 'আপনার পেমেন্ট কোড দেখান',
      pay_cell_handle_get: 'দূরের কেউ',
      pay_cell_handle_get_sub: 'আপনার নাম শেয়ার করুন',
      pay_cell_address_get: 'কোনো BSV ঠিকানায়',
      pay_cell_address_get_sub: 'একটি BSV ঠিকানা দেখান',
      pay_method: 'পদ্ধতি',
      pay_method_nearby: 'কাছাকাছি',
      pay_method_remote_link: 'রিমোট লিংক শেয়ার করুন',
      pay_method_address: 'কোনো BSV ঠিকানায়',
      pay_pre_nearby: 'আপনারা দুজনেই এখানে আছেন।',
      pay_pre_handle: 'তিনি এই অ্যাপ ব্যবহার করেন।',
      pay_pre_address: 'আপনার কাছে তাঁর BSV ঠিকানা আছে।',
      pay_conseq_nearby: 'কয়েক সেকেন্ডেই নিষ্পত্তি হয়।',
      pay_conseq_handle: 'তাঁর ওয়ালেট পরের বার যাচাই করলেই পৌঁছে যাবে।',
      pay_conseq_address: 'অর্থ সরাসরি পাঠানো হয় — প্রাপককে কোনো বিজ্ঞপ্তি দেওয়া হয় না।',
      paid: 'পরিশোধ হয়েছে',
      pay_inbox_attention: 'যোগ করা যায়নি',
      pay_inbox_attention_hint:
        'এগুলি এসেছে কিন্তু আপনার ওয়ালেট যোগ করতে পারেনি। আবার চেষ্টা করুন, বা চিরতরে ছেড়ে দিতে বাতিল করুন।',
      pay_dismiss_confirm: 'ছেড়ে দিতে আবার ট্যাপ করুন',
      pay_dismissed: 'পেমেন্ট ছেড়ে দেওয়া হয়েছে',
      payment_arrived: 'পেমেন্ট পাওয়া গেছে',
      payment_arrived_damaged: 'এই পেমেন্ট ক্ষতিগ্রস্ত অবস্থায় এসেছে',
      discard_payment_title: 'এই পেমেন্ট বাতিল করবেন?',
      discard_payment_body: 'প্রেরককে বিবরণ আবার পাঠাতে বলা হবে। ততক্ষণ টাকা চেইনেই থাকে।',
      discard_void_title: 'এই পেমেন্টটি সম্পন্ন হয়নি',
      discard_void_body: 'এটি বাতিল হয়েছে বা অন্য কোথাও খরচ হয়ে গেছে। নিরাপদে সরিয়ে দিতে পারেন।',
      waiting_network_confirm: 'এই পেমেন্ট নিশ্চিত হওয়ার জন্য নেটওয়ার্কের অপেক্ষায়',
      finish_or_cancel_outgoing: 'নতুন পেমেন্ট পাঠানোর আগে নিচের পেমেন্টটি শেষ করুন বা বাতিল করুন',
      identity_search_unavailable: 'অনুসন্ধান এই মুহূর্তে অনুপলব্ধ। একটু পরে আবার চেষ্টা করুন।',
      abandon_payment: 'পেমেন্ট পরিত্যাগ করুন',
      finish_payment: 'পেমেন্ট শেষ করুন',
      cancel_this_payment: 'এই পেমেন্ট বাতিল করবেন?',
      cancel_payment: 'পেমেন্ট বাতিল করুন',
      resend: 'আবার পাঠান',
      resending: 'আবার পাঠানো হচ্ছে…',
      send_payment_details_again: 'বিবরণ আবার পাঠান',
      tx_action_resend_short: 'আবার পাঠান',
      tx_action_request_confirmation: 'প্রাপকের কাছে নিশ্চিতকরণ চান',
      resend_sent: 'পেমেন্টের বিবরণ আবার পাঠানো হয়েছে',
      resend_failed_no_record: 'এই ডিভাইসে এই পেমেন্টের বিবরণ পাওয়া যায়নি, তাই আবার পাঠানো যাবে না।',
      resend_failed_no_recipient: 'এই পেমেন্টে কার জন্য ছিল তা নেই, তাই আবার পাঠানো যাবে না।',
      resend_failed_no_transaction: 'এই পেমেন্টের ডেটা পাওয়া যায়নি। অনলাইনে এসে আবার চেষ্টা করুন।',
      resend_requested: 'তারা আপনাকে একটি পেমেন্ট আবার পাঠাতে বলেছে',
      request_again: 'আবার অনুরোধ করুন',
      send_again: 'আবার পাঠান',
      copy_details: 'বিবরণ কপি করুন',
      payment_bounced_resend: 'তাদের এই পেমেন্ট আবার পাঠাতে বলা হয়েছে',
      dismiss_rejected_payment: 'প্রত্যাখ্যাত পেমেন্টটি সরানো হয়েছে',
      activity_load_failed: 'কার্যকলাপ লোড করা যায়নি',
      activity_load_retry: 'আবার চেষ্টা করতে ট্যাপ করুন',
      activity_view_contact: 'পরিচিতি দেখুন',
      home_payments_need_attention: '{{count}}টি পেমেন্টে নজর দেওয়া দরকার',
      home_payments_unsent: '{{count}}টি পেমেন্ট পৌঁছায়নি',
      pay_copy: 'কপি',
      pay_share_link: 'লিঙ্ক শেয়ার করুন',
      pay_address_watching: 'এই ঠিকানায় পাঠানো অর্থ স্বয়ংক্রিয়ভাবে আপনার ওয়ালেটে যোগ হয়।',
      pay_address_earlier_day: 'আগের কোনো দিনে যান',
      pay_address_sweep_now: 'এই BSV ঠিকানাটি এখনই যাচাই করুন',
      pay_address_swept: '{{amount}} পাওয়া গেছে',

      check_wallet: 'সমস্যা সমাধান',
      settings_version: 'v{{version}}',
      error_review_actions: 'এই পেমেন্টটি আটকে আছে। সমস্যা সমাধান এটিকে মুক্ত করার চেষ্টা করতে পারে।',
      wallet_check_ok: 'সব ঠিক আছে',
      wallet_check_attention: 'কিছু চেক আপনার নজর প্রয়োজন',
      wallet_check_couldnt: 'পরীক্ষা করা যায়নি',
      wallet_check_couldnt_body: 'একটি মেরামতের ধাপ শেষ হয়নি। আপনার কয়েন নিয়ে কোনো অনুমান করা হয়নি।',
      wallet_check_summary: '{{freed}}টি আটকে থাকা কয়েন মুক্ত · {{recovered}}টি পেমেন্ট পুনরুদ্ধার',
      wallet_check_step_coins: 'আপনার কয়েন বৈধ',
      wallet_check_skip: 'এই চেক এড়িয়ে যান',
      wallet_check_skipped: 'এড়ানো হয়েছে',
      wallet_check_step_proofs: 'ট্রানজ্যাকশন প্রমাণ',
      wallet_check_step_records: 'লোকাল ডেটাবেস রেকর্ড',
      wallet_check_step_tokens: 'স্টেবলকয়েন ট্রান্সফার যাচাই হয়েছে',
      wallet_check_step_online: 'অনলাইন',
      wallet_check_step_missed: 'বাদ পড়া পেমেন্ট',
      wallet_check_step_backup: 'ব্যক্তিগত ব্যাকআপ আপলোড',
      wallet_check_step_phrase: 'রিকভারি ফ্রেজ সংরক্ষিত',
      wallet_check_progress: '{{total}}-এর মধ্যে {{current}}',
      wallet_check_stuck_title: 'এই পেমেন্টটি পাঠানো যায়নি',
      wallet_check_stuck_message: 'কিছু কয়েন আটকে আছে বলে মনে হচ্ছে। সমস্যা সমাধান সেগুলো মুক্ত করতে পারে।',
      wallet_check_failed: 'পরীক্ষা শেষ করা যায়নি। আবার চেষ্টা করুন।',

      // Local payments
      local_pay_request: 'পেমেন্টের অনুরোধ করুন',
      local_pay_send: 'পেমেন্ট পাঠান',
      local_pay_amount: 'পরিমাণ',
      local_pay_show_qr: 'এই কোডটি প্রদানকারীকে দেখান',
      local_pay_scan_qr: 'গ্রহীতার কোড স্ক্যান করুন',
      local_pay_scan_payer_qr: 'প্রদানকারীর কোড স্ক্যান করুন',
      local_pay_scan_progress: '{{total}}-এর মধ্যে {{have}}টি অংশ',
      local_pay_show_payment_qr: 'এই কোডটি গ্রহীতাকে দেখান',
      local_pay_animated_hint:
        'তাঁকে নিচের চলমান কোডটি স্ক্যান করতে বলুন — তাঁর ফোনে পুরোটা না পৌঁছানো পর্যন্ত এটি বদলাতে থাকবে।',
      local_pay_show_qr_instead: 'বদলে একটি QR কোড দেখান',
      local_pay_preparing: 'অনুরোধ প্রস্তুত করা হচ্ছে…',
      local_pay_delivering: 'পেমেন্ট পাঠানো হচ্ছে…',
      local_pay_saving: 'পেমেন্ট সংরক্ষণ করা হচ্ছে…',
      local_pay_sent: 'পেমেন্ট পাঠানো হয়েছে',
      local_pay_received: 'পেমেন্ট গ্রহণ করা হয়েছে',
      local_pay_added: 'আপনার ওয়ালেটে যোগ করা হয়েছে।',
      local_pay_added_multiple: '{{count}}টি পেমেন্ট আপনার ওয়ালেটে যোগ করা হয়েছে।',
      local_pay_received_confirming: 'আপনার ওয়ালেটের সাথে নিশ্চিত করা হচ্ছে…',
      local_pay_received_not_credited: 'এখনও আপনার ওয়ালেটে যোগ হয়নি। কিছু হস্তান্তর করার আগে কার্যকলাপ দেখে নিন।',
      local_pay_decline_broadcast_title: 'একটি প্রত্যাখ্যাত পেমেন্ট নেটওয়ার্কে চলে গেছে',
      local_pay_decline_broadcast_body:
        'যে পেমেন্টটি প্রত্যাখ্যাত হয়েছিল তা পরে নেটওয়ার্কে দেখা গেছে। এটি আবার পাঠাবেন না — কার্যকলাপ দেখুন।',
      local_pay_pending_abort_dropped_title: 'একটি অযাচাইকৃত মুক্তির অনুরোধ উপেক্ষা করা হয়েছে',
      local_pay_pending_abort_dropped_body:
        'একটি সংরক্ষিত পেমেন্ট-মুক্তির রেকর্ড যাচাই করা যায়নি, তাই কিছুই মুক্ত করা হয়নি। যদি কোনো পেমেন্ট এখনও আটকে থাকার মতো দেখায়, তবে কার্যকলাপ থেকে এটি বাতিল করুন।',
      local_pay_cancel_unverifiable_title: 'অফলাইনে থাকাকালীন এই পেমেন্ট যাচাই করা যাচ্ছে না',
      local_pay_cancel_unverifiable_body:
        'অন্য ব্যক্তি যদি এটি আগেই স্ক্যান করে থাকেন, তাহলে বাতিল করলে এই কয়েন দুইবার খরচ করা সম্ভব হবে। কেবল তখনই বাতিল করুন যদি আপনি নিশ্চিত হন যে এটি কখনো স্ক্যান করা হয়নি।',
      local_pay_cancel_unverifiable_confirm: 'তবুও বাতিল করুন',
      local_pay_queued: 'সংরক্ষিত হয়েছে — এটি স্বয়ংক্রিয়ভাবে আপনার ওয়ালেটে যোগ হবে।',
      local_pay_already_paid: 'এই অনুরোধের পেমেন্ট আগেই হয়ে গেছে।',
      local_pay_failed: 'পেমেন্ট ব্যর্থ হয়েছে',
      local_pay_network_denied:
        'লোকাল নেটওয়ার্ক অ্যাক্সেস বন্ধ আছে। কাছের ডিভাইসে পেমেন্ট করতে সেটিংসে এটি চালু করুন।',
      local_pay_radio_fallback: 'ওয়্যারলেস লিঙ্ক অনুপলব্ধ — এর পরিবর্তে অন্য ডিভাইসে এই কোডটি দেখান।',
      local_pay_floor_peer_bt_off: 'তাদের ব্লুটুথ বন্ধ, তাই এই পেমেন্ট কোডের মাধ্যমে হবে।',
      local_pay_floor_local_bt_off: 'আপনার ব্লুটুথ বন্ধ। দ্রুততর কাছাকাছি লিঙ্কের জন্য এটি চালু করুন।',
      local_pay_floor_local_ble_denied:
        'BSV Wallet-এর জন্য ব্লুটুথ অ্যাক্সেস বন্ধ। দ্রুততর কাছাকাছি লিঙ্কের জন্য সেটিংসে এটি অনুমোদন করুন।',
      local_pay_floor_cross_os: 'অন্য ডিভাইসটি এখন ব্লুটুথ ব্যবহার করতে পারছে না, তাই এই পেমেন্ট কোডের মাধ্যমে হবে।',
      local_pay_floor_peer_no_radio:
        'অন্য ডিভাইসটি ওয়্যারলেস লিঙ্কের জন্য শুনছে না, তাই এই পেমেন্ট কোডের মাধ্যমে হবে।',
      local_pay_ble_unavailable: 'ব্লুটুথ পাওয়া যাচ্ছে না। উপরের কোডটি এখনও কাজ করে।',
      local_pay_too_large: 'এই পেমেন্টের ডেটা QR কোডের মাধ্যমে হস্তান্তর করার জন্য অনেক বড়।',
      local_pay_nearby_unavailable: 'কাছাকাছি Wi-Fi পাওয়া যাচ্ছে না। উপরের কোডটি এখনও কাজ করে।',
      local_pay_retry_save: 'আবার সংরক্ষণের চেষ্টা করুন',
      local_pay_wrong_session:
        'সেই পেমেন্টটি এই অনুরোধের সঙ্গে মেলে না, তাই গ্রহণ করা হয়নি। পেমেন্টকারীকে কোডটি আবার স্ক্যান করতে বলুন।',
      local_pay_declined_mismatch:
        'প্রাপক এই পেমেন্ট গ্রহণ করেননি — এটি তাঁর অনুরোধের সঙ্গে মেলে না। তাঁর কোডটি আবার স্ক্যান করুন।',
      local_pay_declined_already_paid: 'সেই অনুরোধের অর্থ ইতিমধ্যেই পরিশোধ হয়েছে। কিছুই পাঠানো হয়নি।',
      local_pay_declined_save: 'প্রাপক পেমেন্টটি সংরক্ষণ করতে পারেননি। কিছুই পাঠানো হয়নি — আবার চেষ্টা করুন।',
      local_pay_declined_decode: 'প্রাপক পেমেন্টটি পড়তে পারেননি। কিছুই পাঠানো হয়নি — আবার চেষ্টা করুন।',
      local_pay_declined_not_covered:
        'প্রাপক ইস্যুকারীর রেকর্ডের সঙ্গে এই পেমেন্টটি যাচাই করতে পারেননি। কিছুই পাঠানো হয়নি।',
      local_pay_broadcast_pending: 'পাঠানো হয়েছে। এই ডিভাইসটি এখনও প্রসেস করাতে পারেনি — প্রাপক সেটি করবেন।',
      local_pay_any_amount: 'যেকোনো পরিমাণ',
      local_pay_choose_amount: 'পরিমাণ বেছে নিন',
      local_pay_enter_amount_send: 'আপনি কত পরিমাণ পাঠাতে চান?',
      local_pay_qr_unavailable: 'এই কোডটি তৈরি করা যায়নি। অনুরোধটি আবার শুরু করুন।',
      local_pay_presence_qr: 'কোডের মাধ্যমে — কোনও সরাসরি সংযোগ নেই',
      local_pay_presence_ready: 'কাছের এনক্রিপ্ট করা সংযোগ প্রস্তুত',
      local_pay_presence_waiting_payee: 'কাছের ডিভাইসের জন্য অপেক্ষা করা হচ্ছে',
      local_pay_presence_waiting_payer: 'প্রাপকের ডিভাইস খোঁজা হচ্ছে…',
      local_pay_presence_linked: 'এনক্রিপ্ট করা সংযোগ চালু আছে',
      local_pay_presence_paid: 'পেমেন্ট নিশ্চিত হয়েছে',
      pay_offline_title: 'আপনি অফলাইনে আছেন',
      pay_offline_body: 'এখন শুধু কাছাকাছি পেমেন্ট কাজ করছে। আপনি আবার সংযুক্ত হলেই সেগুলো প্রসেস করা হবে।',
      pay_offline_queued: '{{count}}টি পেমেন্ট আপনার আবার সংযুক্ত হওয়ার অপেক্ষায় আছে, তখনই সেগুলো প্রসেস করা হবে।',
      pay_offline_pending_title: 'প্রসেসের অপেক্ষায়',
      pay_offline_pending_body:
        '{{count}}টি পেমেন্ট এখনও প্রসেস হয়নি। প্রসেস না হওয়া পর্যন্ত কিছুই সম্পন্ন হয়নি — চেষ্টা চলতে থাকবে।',
      pay_offline_needs_internet: 'ইন্টারনেট প্রয়োজন',
      pay_offline_rejected_title: 'একটি পেমেন্ট প্রত্যাখ্যান করা হয়েছে',
      pay_offline_rejected_body:
        'নেটওয়ার্ক {{sender}}-এর কাছ থেকে {{via}}-এর মাধ্যমে {{when}}-এ পাওয়া একটি পেমেন্ট প্রত্যাখ্যান করেছে।',
      pay_offline_unknown_sender: 'একজন অজানা প্রেরক',
      pay_offline_unknown_via: 'একটি অজানা উপায়',
      pay_received_not_broadcast: 'অফলাইনে গৃহীত · এখনও প্রসেস করা হয়নি',
      pay_offline_sent_rejected_title: 'একটি পেমেন্ট পাঠানো যায়নি',
      pay_offline_sent_rejected_body: 'নেটওয়ার্ক {{when}}-এ আপনার পাঠানো একটি পেমেন্ট প্রত্যাখ্যান করেছে।',
      pay_offline_send_now: 'এখনই পাঠান',
      pay_offline_stalled_body: 'সারিতে থাকা কিছু পেমেন্ট স্বয়ংক্রিয়ভাবে পাঠানো যাচ্ছে না: {{detail}}',
      offline_stall_no_request: 'সারিতে থাকা একটি পেমেন্টের পাঠানোর রেকর্ড নেই।',
      offline_stall_bad_beef: 'সারিতে থাকা একটি পেমেন্টের লেনদেন ডেটা পড়া যায়নি।',
      offline_stall_foreign_ancestor:
        'সারিতে থাকা একটি পেমেন্ট এমন একটি মূল লেনদেনের অপেক্ষায় আছে, যা অন্য একটি ওয়ালেট কখনও প্রসেস করেনি।',
      pay_offline_kv_pending: '{{count}}টি কাছাকাছি পেমেন্ট এই ওয়ালেটে যুক্ত হওয়ার অপেক্ষায়।',
      pay_offline_kv_stuck: '{{count}}টি কাছাকাছি পেমেন্ট যোগ করা যায়নি। প্রেরককে আবার পাঠাতে বলুন।',
      pay_offline_kv_corrupt: 'এই ডিভাইসে ক্ষতিগ্রস্ত পেমেন্ট ডেটা পাওয়া গেছে।',
      pay_offline_kv_corrupt_body:
        'ক্ষতিগ্রস্ত পেমেন্টগুলো এই ডিভাইসে আলাদা করে রাখা হয়েছে, মুছে ফেলা হয়নি। এর কিছু হয়তো ইতিমধ্যেই আপনার ব্যালেন্সে যোগ হয়ে গেছে, তাই কোনো পেমেন্ট এখনও না পেলে তবেই প্রেরককে আবার পাঠাতে বলুন।',
      pay_offline_show_code: 'কোড আবার দেখান',
      pay_parked_cancel: 'পেমেন্ট বাতিল করুন',
      pay_parked_already_sent: 'এই পেমেন্ট ইতিমধ্যে পাঠানো হয়েছে',
      token_cancel_already_sent: 'এই পেমেন্টটি ইতিমধ্যে সম্পন্ন হয়েছে এবং এটি বাতিল করা যাবে না।',
      tx_status_parked: 'হস্তান্তর হয়নি',
      tx_status_import_hold: 'আমদানির পর স্থগিত',
      resend_sending: 'মেসেজ বক্সের মাধ্যমে পাঠানো হচ্ছে',
      cancelling_payment: 'পেমেন্ট বাতিল করা হচ্ছে',

      contacts: 'পরিচিতি',
      contacts_search_placeholder: 'নাম বা হ্যান্ডেল দিয়ে অনুসন্ধান করুন',
      contacts_empty_title: 'এখনও কোনো পরিচিতি নেই',
      contacts_empty_body:
        'আপনি যাদের অর্থ প্রদান করেন তারা সংরক্ষণ করার পর এখানে দেখা যাবে। উপরের ডানদিকের বোতাম দিয়ে কারো প্রোফাইল কোড স্ক্যান করুন।',
      contacts_recent: 'সাম্প্রতিক',
      contact_edit_name: 'নাম সম্পাদনা করুন',
      contact_save_name: 'নাম সংরক্ষণ করুন',
      contact_activity: 'কার্যকলাপ',
      contact_activity_empty: 'এই পরিচিতির সাথে এখনও কোনো কার্যকলাপ নেই',
      contact_delete: 'পরিচিতি মুছুন',
      contact_delete_body: 'এটি {{name}} কে আপনার পরিচিতি থেকে সরিয়ে দেবে। এটি অতীতের কোনো লেনদেনকে প্রভাবিত করবে না।',
      contact_handle_caption: 'তাদের নিবন্ধিত হ্যান্ডেল · শুধু তারাই এটি পরিবর্তন করতে পারবে',
      contact_new_title: 'নতুন পরিচিতি',
      contact_save: 'সংরক্ষণ করুন',
      contact_identifier: 'শনাক্তকারী',
      contact_saved: 'পরিচিতি সংরক্ষিত হয়েছে',
      contact_scan_hint: 'কাউকে যোগ করতে একটি প্রোফাইল কোড স্ক্যান করুন',
      contact_identifier_placeholder: 'তাদের শনাক্তকারী পেস্ট করুন',
      contact_identifier_invalid: 'এটি একটি বৈধ শনাক্তকারী বলে মনে হচ্ছে না',
      contact_add_from_payment: 'পরিচিতিতে যোগ করুন',

      profile: 'প্রোফাইল',
      profile_display_name: 'প্রদর্শন নাম',
      profile_display_name_hint: 'আপনার হ্যান্ডেলের পাশে প্রকাশ্যে দেখানো হয়। যে কেউ আপনাকে খুঁজলে এটি দেখতে পাবে।',
      profile_handle: 'হ্যান্ডেল',
      profile_handle_placeholder: 'আপনার হ্যান্ডেল',
      profile_handle_checking: 'উপলব্ধতা যাচাই করা হচ্ছে…',
      profile_handle_available: '{{handle}} উপলব্ধ আছে',
      profile_handle_taken: '{{handle}} ইতিমধ্যে নেওয়া হয়েছে',
      profile_handle_invalid: '৩-৩২ অক্ষর: a-z, 0-9, ডট, আন্ডারস্কোর বা হাইফেন; শুরু ও শেষ অক্ষর বা সংখ্যা দিয়ে',
      profile_handle_failed: 'উপলব্ধতা যাচাই করা যায়নি। আবার চেষ্টা করুন।',
      profile_handle_unavailable: 'এই বিল্ডে হ্যান্ডেল নিবন্ধন এখনও উপলব্ধ নয়।',
      profile_handle_registered: 'হ্যান্ডেল নিবন্ধিত হয়েছে',
      profile_handle_register_action: 'নিবন্ধন করুন',
      profile_handle_too_similar: '{{handle}} ইতিমধ্যে ব্যবহৃত একটির সঙ্গে খুব মিল',
      profile_handle_reserved: '{{handle}} সংরক্ষিত',
      profile_handle_cooldown: '{{handle}} সম্প্রতি ছেড়ে দেওয়া হয়েছে, এখনো খালি নয়',
      profile_handle_pending: 'নিবন্ধন শেষ করা হচ্ছে…',
      profile_handle_rolled_back: '{{handle}} নেওয়া হয়ে গেছে। আপনি {{previous}} রেখেছেন।',
      profile_handle_changed: 'হ্যান্ডেল পরিবর্তিত হয়েছে',
      profile_handle_rejected: 'রেজিস্ট্রি সেই হ্যান্ডেলটি প্রত্যাখ্যান করেছে।',
      profile_handle_clock_ahead: 'আপনার ডিভাইসের ঘড়ি অনেক এগিয়ে আছে। কয়েক মিনিট পরে আবার চেষ্টা করুন।',
      profile_handle_wrong_registry: 'সেই হ্যান্ডেলটি এই রেজিস্ট্রিতে নেই। রিফ্রেশ করতে প্রোফাইল আবার খুলুন।',
      profile_display_name_publish_failed: 'আপনার নাম এখানে সংরক্ষিত হয়েছে, কিন্তু রেজিস্ট্রি এটি প্রকাশ করেনি।',
      profile_qr_hint: 'অন্যরা আপনাকে পরিচিতি হিসেবে যোগ করতে এটি স্ক্যান করতে পারে',

      pay_step_contacts: 'পরিচিতি',
      pay_step_continue: 'চালিয়ে যান',
      pay_review_to: 'প্রতি',
      pay_review_amount: 'পরিমাণ',
      pay_review_note: 'নোট',
      pay_review_note_edit: 'একটি নোট যোগ করুন',
      pay_review_delivery_host: 'যেখানে পাঠানো হচ্ছে',
      pay_trust_contact: 'আপনার পরিচিতি',
      pay_trust_handle_attested: 'নিবন্ধিত',
      pay_trust_handle_domain_attested: '{{domain}} দ্বারা যাচাইকৃত',
      pay_trust_unverified: 'যাচাই করা হয়নি। এই শনাক্তকারীটি ভালোভাবে যাচাই করুন।',
      pay_address_network_mismatch:
        'এই ঠিকানাটি আপনি যে নেটওয়ার্ক ব্যবহার করছেন তার থেকে ভিন্ন একটি নেটওয়ার্কের অন্তর্গত।',
      contacts_your_contacts: 'আপনার পরিচিতি',
      contacts_other_people: 'অন্য ব্যক্তিরা',
      contacts_not_saved: 'সংরক্ষিত নয়',
      contacts_network_footer: 'নেটওয়ার্কে পাওয়া গেছে। আপনি সংরক্ষণ করেননি।',
      contact_no_handle: 'এখনও কোনো হ্যান্ডেল নেই',
      contact_no_handle_registered: 'কোনো হ্যান্ডেল নিবন্ধিত নয়',
      contact_name_section: 'তাদের জন্য আপনার নাম',
      contact_name_hint: 'শুধু আপনি এটি দেখেন। তারা নিজেকে কী বলে তা বদলায় না।',
      contact_new_name_hint: 'শুধু আপনি এটি দেখেন। এই ডিভাইসে সংরক্ষিত।',
      contact_copy_identifier: 'শনাক্তকারী কপি করুন',
      profile_handle_registered_hint: 'আপনার শনাক্তকারীতে নিবন্ধিত। লোকেরা এর মাধ্যমে আপনাকে খুঁজে পেতে পারে।',
      profile_handle_change: 'পরিবর্তন',
      profile_handle_claim: '{{handle}} নিন',
      profile_handle_replace_warning:
        'নিলে {{handle}} প্রতিস্থাপিত হবে। যারা আপনাকে সংরক্ষণ করেছেন তাদের পরিচিতি থাকবে; পুরোনো হ্যান্ডেল আর আপনার কাছে পৌঁছাবে না।',
      profile_identifier_section: 'আপনার শনাক্তকারী',
      profile_show_qr: 'QR হিসেবে দেখান',
      profile_show_qr_hint: 'কাউকে স্ক্যান করতে দিন যাতে তারা আপনার প্রোফাইল পায়',
      profile_qr_callout:
        'এটি স্ক্যান করলে BSV Wallet খোলে এবং আপনার নাম, হ্যান্ডেল ও ছবি সহ আপনাকে পরিচিতি হিসেবে যোগ করে। এটি কোনো পেমেন্ট চায় না — সেই কোডটি “পেমেন্ট নিন”-এ আছে।',
      pay_review_title: 'পর্যালোচনা',
      pay_send_amount: '{{amount}} পাঠান',
      pay_sent_to: '{{name}}-কে',
      pay_save_as_contact: 'পরিচিতি হিসেবে সংরক্ষণ করুন'
    }
  },
  ru: {
    translation: {
      token_balances_header: 'Балансы',
      wallet_coin_switcher: 'Выберите, какую монету показывать',
      wallet_activity_empty_asset: 'Пока нет операций в {{ticker}}',
      pay_direction_pay_asset: 'Оплатить в {{ticker}}',
      token_fee_footer: 'Для отправки нужно немного BSV на комиссию сети. Для получения — нет.',
      token_new_tap: 'Новое — нажмите, чтобы узнать эмитента',
      token_unresolved: 'Неизвестный токен',
      token_unresolved_sub: 'Кошелёк не смог определить эмитента. Повторите попытку в сети.',
      token_paused_short: 'Переводы приостановлены',
      token_frozen_short: '{{amount}} заморожено',
      token_sheet_issuer: 'Эмитент',
      token_sheet_issuer_unknown: 'Не определён',
      token_sheet_asset_id: 'ID токена',
      token_sheet_powers_header: 'Что может {{issuer}}',
      token_power_pause: 'Приостановить все переводы {{ticker}}',
      token_power_freeze: 'Заморозить отдельные монеты, в том числе ваши',
      token_power_admit: 'Решать, кто может владеть {{ticker}}',
      token_power_replace: 'Заменить замороженную монету новой',
      token_powers_limit:
        'Без вас они не могут вывести {{ticker}} из вашего кошелька, а замороженную монету просто нельзя потратить.',
      token_frozen_sub:
        '{{issuer}} заморозил(а) {{amount}} {{ticker}} вашего баланса. Обратитесь к ним для оспаривания.',
      pay_asset_label: 'Платить в',
      pay_asset_label_get: 'Получать в',
      pay_asset_cta: 'Отправить {{amount}} {{ticker}}',
      pay_asset_over_balance: 'Больше вашего баланса {{ticker}}.',
      pay_asset_needs_bsv: 'Для отправки {{ticker}} нужно немного BSV на комиссию сети. Для получения — нет.',
      pay_asset_get_bsv: 'Получить BSV',
      pay_asset_needs_bsv_short: 'Нужны BSV на комиссию',
      pay_asset_frozen_note: '{{amount}} {{ticker}} вашего баланса заморожено и не может быть отправлено.',
      pay_asset_paused: '{{issuer}} приостановил(а) переводы {{ticker}}, поэтому сейчас отправить нельзя.',
      pay_asset_no_address: '{{ticker}} можно отправить только человеку или устройству рядом — но не на биткойн-адрес.',
      pay_asset_address_status: 'Недоступно для {{ticker}}',
      pay_asset_self_unregistered:
        '{{issuer}} не зарегистрировал(а) вас для отправки {{ticker}}. Получать вы всё ещё можете.',
      pay_asset_unidentified: 'Кошелёк не смог распознать этот токен, поэтому пока не может его отправить.',
      pay_asset_recipient_unregistered:
        '{{issuer}} ещё не зарегистрировал(а) этого человека для {{ticker}}, поэтому платёж невозможен.',
      pay_asset_recipient_blocked: '{{issuer}} не разрешает платежи этому человеку.',
      token_err_refused:
        '{{issuer}} отклонил(а) этот перевод. Ничего не отправлено, ваш баланс {{ticker}} не изменился.',
      token_err_refused_paused: '{{issuer}} приостановил(а) {{ticker}}. Ничего не отправлено, баланс не изменился.',
      token_err_refused_recipient:
        '{{issuer}} не разрешает платежи этому человеку. Ничего не отправлено, баланс не изменился.',
      token_err_refused_frozen:
        'Часть {{ticker}}, которую вы отправляли, только что заморожена. Ничего не отправлено; баланс обновлён.',
      token_err_send_failed: 'Не удалось отправить этот платёж: {{reason}}. Из вашего кошелька ничего не ушло.',
      token_err_reason_unknown: 'неизвестная проблема',
      token_err_unreachable:
        'Не удалось связаться с {{issuer}} для подтверждения перевода. Проверьте баланс {{ticker}}, прежде чем повторять.',
      token_err_check_again: 'Проверить снова',
      token_err_balance_changed:
        'Ваш баланс {{ticker}} изменился, пока платёж готовился. Проверьте сумму и попробуйте снова.',
      token_err_busy: 'Платёж в {{ticker}} уже отправляется.',
      pay_sent_not_broadcast: 'Отправлено. Пока не дошло до сети — продолжаем попытки.',
      pay_sent_not_notified:
        'Отправлено. Сообщить пока не удалось — получатель увидит платёж при следующей проверке кошелька.',
      pay_sent_handed_to_wallet: 'Отправлено. Лично платёж не приняли, поэтому мы отправили его в кошелёк получателя.',
      token_first_hold:
        '{{ticker}} выпускает {{issuer}}, который может приостанавливать переводы и замораживать монеты.',
      pay_asset_request_paused: '{{issuer}} приостановил(а) {{ticker}}, поэтому платёж пока может не дойти.',
      token_admit_title: 'Вы пока не можете получать платежи в {{ticker}}',
      token_admit_body:
        '{{issuer}} отправляет {{ticker}} только зарегистрированным людям. До этого платежи вам отклоняются ещё до выхода из кошелька отправителя.',
      token_admit_bsv_instead: 'Получить оплату в BSV',
      token_row_sent: 'Отправлено {{ticker}}',
      token_row_received: 'Получено {{ticker}}',
      token_row_amount_pending: 'Сумма недоступна',
      token_evicted_title: 'Часть {{ticker}} была изъята',
      token_evicted_body: '{{issuer}} изъял(а) {{amount}} {{ticker}} из вашего кошелька.',
      local_pay_token_not_cleared: 'Получено офлайн · ещё не подтверждено {{issuer}}',
      pay_asset_nearby_needs_network:
        'Для оплаты в {{ticker}} при личной встрече нужна связь. Без неё можно платить в BSV.',
      pay_asset_link_not_held:
        'Этот запрос — для токена, которого нет в этом кошельке. Вы всё ещё можете заплатить им в BSV.',
      pay_asset_nearby_wrong_overlay: 'Этот запрос — для токена, который не настроен в этом кошельке.',
      token_messagebox_mismatch:
        'Ваш кошелёк настроен на другой почтовый ящик, поэтому платежи в токенах могут не дойти.',
      token_issuer_fallback: 'эмитент',
      token_row_hint: 'Открывает эмитента и его возможности',
      token_attention_one: 'Ожидаем подтверждения от {{issuer}} — нажмите для подробностей',
      token_attention_many: '{{count}} платежей в токенах ожидают расчёта — нажмите для подробностей',
      token_recv_reversed: '{{issuer}} не смог подтвердить этот платёж, и он был отменён.',
      token_settle_paused_retry: '{{issuer}} сейчас приостановил(а) {{ticker}} — продолжаем попытки.',
      token_settle_unreachable_retry:
        'Не удалось связаться с {{issuer}} для подтверждения перевода. Продолжаем попытки.',
      token_settle_evicted: 'Тот платёж не дошёл до сети вовремя и был отменён — ваши {{ticker}} вернулись на баланс.',
      token_frame_unreadable: 'Не удалось прочитать полученный платёж — попросите отправителя отправить его снова.',
      token_chain_reversed: '{{issuer}} отклонил(а) более ранний платёж в этой цепочке, поэтому этот тоже отменён.',
      token_settle_stuck_bound:
        'Этот платёж ожидает расчёта более 3 дней. Можно подождать ещё или связаться с {{issuer}}.',
      token_cover_failed: 'Этот платёж не сходится с записями {{issuer}} и не был принят.',
      token_sent_settling:
        'Отправлено. Идёт расчёт с {{issuer}} — подтверждение появится, когда вы или {{payee}} снова выйдете в сеть.',
      token_sent_settling_unnamed:
        'Отправлено. Идёт расчёт с {{issuer}} — подтверждение появится, когда кто-то из вас выйдет в сеть.',
      token_sent_settled: 'Отправлено · расчёт с {{issuer}} завершён',
      token_status_settling: 'Идёт расчёт',
      token_status_settled: 'Расчёт завершён',
      token_status_refused: 'Отклонён',
      token_status_reversed: 'Отменён',
      token_status_stuck: 'Ожидает расчёта',
      new_tab: 'Новая вкладка',
      untitled: 'Без названия',
      share: 'Поделиться',
      back_to_homepage: 'На главную страницу',
      bookmarks: 'Закладки',
      add_bookmark: 'Добавить закладку',
      bookmark: 'Закладка',
      unbookmark: 'Удалить закладку',
      find_in_page: 'Найти на странице',
      find_in_page_placeholder: 'Найти на странице...',
      find_in_page_matches: '{{current}} из {{total}}',
      find_in_page_no_matches: 'Нет совпадений',
      history: 'История',
      clear_all: 'Очистить все',
      clear_all_tabs: 'Закрыть все вкладки',
      search_or_enter_website: 'Поиск или введите сайт',
      browser: 'Браузер',
      bug_report: 'Отчёт об ошибке',
      enable_web3: 'Включить Web3',
      tabs: 'Вкладки',
      unlock_web3_features: 'Разблокировать функции Web3',

      cancel: 'Отмена',
      done: 'Готово',
      discard: 'Отклонить',
      deny: 'Отклонить',
      allow: 'Разрешить',
      save: 'Сохранить',
      remove: 'Удалить',
      delete: 'Удалить',
      search: 'Поиск',
      loading: 'Загрузка...',
      continue: 'Продолжить',
      go_back: 'Назад',
      continue_action: 'Продолжить',
      camera_denied_description:
        'Доступ к камере был отклонён. Вы можете включить его в Настройках, чтобы использовать эту функцию.',
      open_settings: 'Открыть Настройки',
      copied: 'Скопировано',
      explorer_open_failed: 'Не удалось открыть обозреватель блоков',
      tx_action_refresh: 'Обновить статус',
      tx_action_refresh_short: 'Статус',
      tx_action_abort: 'Отменить транзакцию',
      copy: 'Копировать',
      copy_to_clipboard: 'Скопировать в буфер обмена',
      details: 'Подробности',
      hide_details: 'Скрыть подробности',

      identity: 'Идентичность',
      settings: 'Настройки',
      wallet: 'Кошелёк',
      activity: 'Активность',
      configuration: 'Конфигурация',
      data_and_security: 'Данные и безопасность',
      storage_row: 'Хранилище',
      storage_title: 'Хранилище кошелька',
      storage_db_size: 'База данных: {{size}}',
      storage_free_space: 'Свободно: {{size}}',
      storage_reclaimable: 'Можно освободить: {{size}} в {{count}} транзакциях',
      storage_held_back: '{{count}} транзакций сохранено — ещё используются',
      storage_reclaim_explain:
        'Освобождение удаляет данные проверки, которые кошельку больше не нужны. Транзакции, балансы и история не затрагиваются. Место освобождается внутри файла базы данных; сам файл не уменьшится до сжатия системой.',
      storage_reclaim_confirm: 'Освободить',
      storage_reclaimed: 'Освобождено транзакций: {{count}}',
      storage_failed: 'Не удалось прочитать использование хранилища.',
      storage_unavailable: 'Хранилище ещё не готово.',
      backup_push_section: 'Приватная копия',
      backup_push_toggle: 'Резервировать на сервере',
      backup_erase_row: 'Удалить копию с сервера',
      backup_erase_title: 'Удалить вашу резервную копию?',
      backup_erase_message:
        'Всё, что этот кошелёк отправил на сервер резервных копий, будет удалено безвозвратно, по всем устройствам. Резервирование одновременно отключается. Монеты, ключи и сид-фраза не затрагиваются — но новое устройство больше не сможет восстановить историю этого кошелька, а часть ваших нынешних монет будет там непригодна к трате.',
      backup_erase_confirm: 'Удалить',
      backup_erase_done: 'Копия удалена — удалено элементов: {{count}}',
      backup_erase_failed: 'Не удалось удалить копию. Ничего не удалено — попробуйте снова.',
      backup_erase_no_key: 'Ключ кошелька недоступен',
      backup_push_disclosure:
        'Зашифрованная копия базы данных кошелька отправляется на сервер BSV Association, чтобы вы могли восстановить её на новом устройстве. Она зашифрована ключом, который выводится только из вашей сид-фразы — оператор не может её прочитать. Сид-фраза или напечатанные доли по-прежнему необходимы: без них копию не откроет никто, включая нас.',
      backup_push_off_title: 'Отключить приватную копию?',
      backup_push_off_message:
        'Одна сид-фраза не восстановит этот кошелёк. Сведения, связанные с защитой ваших монет, есть только в базе данных, поэтому с выключенным резервированием новое устройство восстановит фразу, но не средства. Уже отправленное на сервер не удаляется.',
      backup_push_off_confirm: 'Отключить',
      backup_push_on_toast: 'Резервирование включено — синхронизируем',
      backup_push_off_toast: 'Резервирование выключено — больше ничего не отправится',
      permissions: 'Разрешения',
      trust_network: 'Сеть доверия',
      // vault (machine-translated, needs native review)
      vault_title: 'Хранилище',
      vault_row_title: 'Хранилище',
      vault_hero_title: 'Защищённые активы',
      vault_hero_body:
        'Холодное хранение на долгий срок, запертое вашими собственными YubiKey. Нужны два или более YubiKey 5 NFC.',
      vault_unsupported_title: 'Требуется YubiKey',
      vault_unsupported_body: 'Два или более YubiKey 5 NFC и телефон, который может их считать.',
      vault_restore_from_chain: 'Восстановить хранилище из блокчейна',
      vault_restore_scanning: 'Сканирование блокчейна…',
      vault_restore_found: 'На блокчейне найдено {{count}} депозит(ов) хранилища.',
      vault_restore_pending:
        'Найдено {{count}} депозит(ов), ожидающих первого подтверждения — повторите попытку позже.',
      vault_restore_none_found: 'Депозиты хранилища для этого кошелька на блокчейне не найдены.',
      vault_balance_label: 'в хранилище',
      vault_key_section: 'Ключи безопасности ({{count}} из 5)',
      vault_manage_section: 'Управление',
      vault_deposit_cta: 'Пополнить',
      vault_withdraw_cta: 'Вывести',
      vault_deposit_title: 'Пополнить хранилище',
      vault_withdraw_title: 'Вывести из хранилища',
      vault_deposit_sub: 'С вашего обычного баланса. YubiKey не нужен.',
      vault_withdraw_sub: 'Обратно на обычный баланс. Приложите любой из ключей хранилища.',
      vault_deposit_done: 'Перемещено в хранилище',
      vault_reclaim_done: 'Средства возвращены из прерванного пополнения хранилища',
      vault_withdraw_done: 'Выведено из хранилища',
      vault_withdraw_partial: 'Осталось ещё {{count}} пополнений — выведите снова, чтобы перенести их.',
      vault_withdraw_reason: 'Приложите YubiKey сюда для подписи — вывести {{amount}} sats из хранилища',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: 'Пока недоступно — пополнение хранилища отключено в этой версии.',
      vault_not_on_mainnet_body:
        'Хранилище доступно только в основной сети. Переключите кошелёк обратно на основную сеть, чтобы им пользоваться.',
      vault_intro_title: 'Настройте хранилище',
      vault_intro_what:
        'Хранилище запирает деньги вашими собственными YubiKey. Ничего из ключа хранилища никогда не хранится на этом телефоне; любой из ваших ключей открывает его.',
      vault_intro_two_keys: 'Нужно как минимум два YubiKey.',
      vault_intro_apart: 'Храните их в разных местах — два ключа, лежащие вместе, это один ключ.',
      vault_intro_backup:
        'Не выключайте зашифрованную резервную копию кошелька — она сохраняет аутентифицированные данные восстановления.',
      vault_intro_ack:
        'Я понимаю: только мои YubiKey открывают это хранилище. Моя фраза восстановления — нет. Если я потеряю их все, деньги пропадут.',
      vault_intro_piv_ack:
        'Я подтверждаю, что всё приложение PIV на этом YubiKey сброшено до заводских настроек или выделено для хранилища. Настройка меняет глобальные PIN, PUK и ключ управления PIV, может затронуть все слоты PIV, а новый PUK я сохраню в безопасном месте.',
      vault_intro_begin: 'Начать',
      vault_key_step_title: 'Ключ {{k}} из не более 5',
      vault_key_step_replace:
        'Используйте сброшенный до заводского состояния YubiKey, выделенный для хранилища. Если в его слоте есть старый ключ, настройка предупредит перед заменой.',
      vault_pin_choose_title: 'Придумайте PIN-код',
      vault_pin_choose_sub: 'Его нужно будет вводить при каждом использовании этого ключа. От 6 до 8 цифр.',
      vault_pin_confirm_label: 'Введите ещё раз',
      vault_pin_mismatch: 'PIN-коды не совпадают.',
      vault_pin_not_default: 'Выберите что-нибудь кроме 123456.',
      vault_puk_title: 'Ваш код восстановления',
      vault_puk_body:
        'Запишите его и храните в надёжном месте. Это единственный способ разблокировать ключ, если PIN-код забыт, и повторно он показан не будет.',
      vault_puk_ack: 'Я записал его',
      vault_reset_offer: 'Сбросить этот ключ',
      vault_reset_title: 'Сбросить этот ключ?',
      vault_reset_body:
        'Этот ключ уже использовался. Сброс стирает все сертификаты и ключи в его PIV-приложении, а не только слот хранилища. Всё, что на него опирается, перестанет работать.',
      vault_reset_ack: 'Я понимаю, что это сотрёт с ключа всё',
      vault_reset_unknown_ack:
        'Это устройство не может подтвердить, защищает ли этот ключ хранилище другого профиля. Я понимаю, что стирание может уничтожить его безвозвратно.',
      vault_reset_own_draft_ack:
        'В этом ключе лежит незавершённый ключ вашей прежней попытки, и запись о нём ещё есть на этом устройстве. Я понимаю, что сброс сотрёт его и продолжить ту попытку будет нельзя.',
      vault_reset_nothing_erased:
        'Ничего не стёрто. Слот хранилища на этом ключе не пуст, поэтому нужно подтвердить ещё одно.',
      vault_reset_confirm: 'Стереть и сбросить',
      vault_resetting: 'Сброс ключа…',
      vault_reset_done: 'Ключ сброшен. Приложите его снова, чтобы продолжить.',
      vault_reset_uncertain:
        'Сброс не завершился. Ключ мог быть уже стёрт — проверьте его, прежде чем на него полагаться.',
      vault_err_reset_enrolled: 'Этот ключ уже входит в ваше хранилище — сброс уничтожил бы один из его ключей.',
      vault_err_reset_unreadable:
        'Записи вашего хранилища на этом устройстве не удалось прочитать, поэтому нельзя доказать, что этот ключ не входит в ваше хранилище. Ничего не стёрто, сброс отклонён.',
      vault_enrollment_resume_pin_hint:
        'Чтобы продолжить сохранённый ключ, нужен PIN, выбранный для него раньше, а не новый. Три неверные попытки заблокируют ключ.',
      vault_setup_step: 'Шаг {{n}} из {{total}}',
      vault_enrollment_resume: 'Продолжить сохранённый ключ',
      vault_enrollment_reset_required:
        'Этот YubiKey больше не находится в заводском состоянии, поэтому его нельзя зарегистрировать как есть. Прежде чем он сможет хранить ключ хранилища, его приложение PIV нужно стереть и вернуть к заводскому состоянию.',
      vault_enrollment_state_uncertain:
        'Попытка настройки прервалась, поэтому невозможно определить, в каком состоянии остался YubiKey. Этот ключ не будет использован снова, пока его приложение PIV не будет стёрто и возвращено к заводскому состоянию.',
      vault_nfc_enroll_message: 'Приложите YubiKey сюда, чтобы настроить его',
      vault_nfc_adopt_message: 'Приложите восстановленный YubiKey сюда для проверки',
      vault_nfc_sign_batch: 'Приложите YubiKey сюда для подписи — пакет {{b}} из {{n}}',
      vault_key_use_different: 'Использовать другой YubiKey',
      vault_key_setup_again: 'Настроить заново',
      vault_name_default: 'Ключ {{k}}',
      vault_more_title: 'Добавить ещё ключ?',
      vault_more_body: 'До 5 ключей. Любой из них сам по себе открывает всё хранилище.',
      vault_more_add: 'Добавить ещё ключ',
      vault_more_finish: 'Готово',
      vault_more_need_two: 'Добавьте второй ключ перед завершением — один ключ означает отсутствие восстановления.',
      vault_leave_setup: 'Выйти из настройки',
      vault_leave_title: 'Выйти из настройки?',
      vault_leave_body:
        'Настроенные вами YubiKey ({{count}}) пока не будут сохранены. Ключи на них остаются, так что вы сможете добавить их снова через минуту.',
      vault_leave_confirm: 'Выйти',
      vault_leave_stay: 'Остаться',
      vault_done_body:
        '{{count}} ключей могут открыть это хранилище. Только эти ключи открывают его — фраза восстановления нет.',
      vault_done_cta: 'Готово',
      vault_add_key_row: 'Добавить ключ',
      vault_add_key_done:
        '{{nickname}} может открывать пополнения, сделанные с этого момента. Перезапирайте хранилище, чтобы он мог открыть всё.',
      vault_key_added_toast: 'Ключ добавлен',
      vault_recovery_verified: 'Восстановленный ключ проверен на этом устройстве',
      vault_relock_row: 'Перезапереть хранилище',
      vault_relock_now: 'Перезапереть сейчас',
      vault_relock_choose: 'Каким ключом вы будете перезапирать?',
      vault_relock_reason: 'Приложите один из существующих ключей ({{names}}) — не тот, что вы только что добавили.',
      vault_relock_reason_generic: 'Приложите YubiKey сюда для подписи — перезапереть хранилище на текущие ключи',
      vault_relock_reason_remaining: 'Перезаприте только на оставшиеся ключи: {{names}}.',
      vault_key_pending_removal: 'Удалён — перезаприте, чтобы завершить',
      vault_key_removal_awaiting_network: 'Удаляется — ждём, пока сеть примет перезапирание',
      vault_removal_awaiting_network: 'Перезапирание отправлено. Пополнение и вывод снова станут доступны, как только сеть его примет.',
      vault_err_too_many_active_keys: 'Хранилище запирается не более чем пятью ключами. Удалите один, прежде чем пополнять, перезапирать или выводить часть баланса.',
      vault_key_section_over_limit: 'Ключи безопасности ({{count}}, удалите один)',
      vault_add_key_done_over_limit: '{{nickname}} добавлен. Хранилище запирается не более чем пятью ключами: удалите ключ, который он заменяет, затем перезаприте один раз.',
      vault_relock_done: 'Хранилище перезаперто на текущие ключи',
      vault_relock_capped: 'Осталось ещё {{count}} пополнений — перезапирайте снова, чтобы перенести их.',
      vault_relock_unreachable:
        '{{count}} пополнений могут открыть только {{names}}. Перезапирайте снова одним из этих ключей.',
      vault_badge_missing: '{{count}} пополнений ещё не открыты для {{nickname}}',
      vault_badge_removed: '{{count}} пополнений всё ещё открыты для удалённого ключа',
      vault_key_action_rename: 'Переименовать',
      vault_key_action_remove: 'Удалить',
      vault_rename_title: 'Переименовать {{nickname}}',
      vault_rename_save: 'Сохранить',
      vault_remove_title: 'Удалить {{nickname}}?',
      vault_remove_body:
        'Кошелёк перестанет использовать {{nickname}}. Деньги, уже лежащие в хранилище, остаются доступны этому ключу до перезапирания (≈ {{fee}} сат).',
      vault_remove_only: 'Только удалить',
      vault_key_removed_toast: 'Ключ удалён',
      vault_export_explainer:
        'Каждое пополнение хранилища скрывает свою соль до момента, когда вы её потратите — в скрипте блокировки об этом ничего не видно. Не выключайте зашифрованную резервную копию, чтобы новое устройство могло восстановить и проверить записи ключей, необходимые для использования ваших YubiKey.',
      vault_footnote: 'Только эти ключи открывают хранилище. Фраза восстановления — нет.',
      vault_floor_line:
        'Минимальное пополнение {{floorDisplay}} ({{floorSats}} сат). Создание пополнения хранилища стоит около {{feeDisplay}}.',
      vault_first_deposit_title: 'Первое пополнение хранилища',
      vault_first_deposit_body:
        'Первое пополнение хранилища — {{amount}} можно будет открыть только {{count}} ключами YubiKey ({{names}}). Фраза восстановления не поможет.',
      vault_backup_off_title: 'Включите приватную копию',
      vault_backup_off_body:
        'Приватная копия должна быть включена до создания нового выхода хранилища. Она сохраняет проверенные записи YubiKey и историю кошелька, необходимые для восстановления. Вы по-прежнему можете вывести всё без создания сдачи в хранилище.',
      vault_backup_off_cta: 'Открыть настройки копии',
      vault_seed_not_preserved_title: 'Сначала сохраните фразу восстановления',
      vault_seed_not_preserved_body:
        'Ваше Хранилище можно восстановить только с помощью фразы восстановления и одного из ваших YubiKey. YubiKey не хранит её копию — если она будет потеряна до того, как вы её запишете, эти средства нельзя будет восстановить. Сохраните её сейчас, прежде чем вносить депозит.',
      vault_seed_not_preserved_cta: 'Сохранить фразу восстановления',
      vault_choose_key: 'Какой ключ вы приложите?',
      vault_remainder_title: 'Вывести всё?',
      vault_remainder_body:
        'После вывода {{amount}} останется {{remainder}} — меньше минимума хранилища в 100 000 сат. Всё хранилище перейдёт на ваш обычный баланс.',
      vault_remainder_all: 'Вывести всё',
      vault_remainder_change: 'Изменить сумму',
      vault_unreachable_title: 'Части хранилища нужен другой ключ',
      vault_unreachable_body:
        'Части хранилища нужен другой ключ — перенесено {{moved}}. {{count}} пополнений на {{amount}} могут открыть только {{names}}. Выведите снова одним из этих ключей.',
      vault_sign_progress: 'Подписано {{signed}} из {{total}}',
      vault_err_pin_locked_enroll:
        'PIN этого YubiKey заблокирован. Разблокируйте его PUK-кодом в Yubico Authenticator или настройте другой YubiKey.',
      vault_err_serial_mismatch_chosen:
        'Это {{tappedName}}. Вы выбрали {{chosenName}} — приложите его или вернитесь и выберите {{tappedName}}.',
      vault_err_key_already_enrolled: 'Вы уже добавили этот YubiKey ({{nickname}}). Приложите другой.',
      vault_err_not_released: 'Пополнение хранилища отключено в этой версии.',
      vault_err_not_on_mainnet:
        'Хранилище доступно только в основной сети. Переключите кошелёк на основную сеть и повторите попытку.',
      vault_err_backup_off:
        'Включите зашифрованную приватную копию перед созданием нового выхода хранилища. Вы по-прежнему можете вывести всё.',
      vault_err_not_enough_keys: 'В этом хранилище меньше двух ключей. Настройте его заново.',
      vault_err_too_many_keys: 'В хранилище может быть не более пяти ключей.',
      vault_err_last_keys: 'Хранилищу нужно как минимум два ключа. Добавьте другой ключ, прежде чем удалять этот.',
      vault_err_relock_required: 'Сначала перезапирайте хранилище, чтобы другие ключи могли открыть каждое пополнение.',
      vault_err_action_pending: 'Транзакция хранилища ещё не завершена. Повторите попытку, когда она завершится.',
      vault_resolve_held_deposit_action: 'Завершить прерванный депозит',
      vault_resolve_held_deposit_done: 'Проблема с депозитом решена',
      vault_resolve_held_deposit_failed: 'Не удалось решить проблему с депозитом. Повторите попытку.',
      vault_resolve_held_deposit_none:
        'Здесь нечего решать — проверьте вкладку «Активность» на наличие ожидающей транзакции, чтобы повторить или отменить её.',
      vault_err_key_not_committed:
        '{{nickname}} не может открыть ни одно пополнение в этом хранилище. Используйте другой ключ хранилища.',
      vault_err_key_cannot_cover:
        '{{nickname}} может открыть {{reachable}} из {{total}} в хранилище. Выведите не более {{reachable}} или используйте {{otherNames}}.',
      vault_err_too_small_to_relock:
        'В этом хранилище меньше 100 000 сат — слишком мало для перезапирания. Выведите их и пополните снова.',
      vault_err_bad_version: 'Транзакция собрана в неверном формате. Ничего не подписано — попробуйте снова.',
      vault_err_chain_scan_failed:
        'Не удалось завершить сканирование блокчейна на предмет депозитов хранилища. Проверьте соединение и повторите попытку.',
      vault_shares_unavailable: 'Нет ключевого материала для печати.',
      vault_enroll_begin: 'Настроить хранилище',
      vault_enroll_phase_connecting: 'Чтение ключа…',
      vault_enroll_phase_pin_check: 'Проверка ключа…',
      vault_enroll_phase_checking_slot: 'Проверка слота хранилища…',
      vault_enroll_phase_personalizing: 'Настройка PIN-кода и кода восстановления…',
      vault_enroll_phase_generating: 'Создание ключа хранилища — коснитесь ключа, когда он замигает',
      vault_enroll_phase_challenging: 'Проверка нового ключа хранилища — коснитесь ключа, когда он замигает',
      vault_enroll_phase_done: 'Готово',
      vault_touch_when_blinks: 'Коснитесь золотого контакта, когда ключ замигает.',
      vault_nfc_activation_hint:
        'Впервые используете этот YubiKey? Сначала подключите его к порту USB-C на несколько секунд — новые ключи поставляются с отключённым NFC, пока это не будет сделано.',
      vault_continue: 'Продолжить',
      vault_back: 'Назад',
      vault_enrolled_toast: 'Хранилище готово',
      vault_insert_key: 'Вставьте ваш YubiKey',
      vault_hold_key_nfc: 'Приложите YubiKey к верхней части телефона',
      vault_keep_holding_nfc: 'Держите — разблокировка…',
      vault_unlocking_funds: 'Разблокируем ваши средства',
      vault_unlocking_sub: 'Это займёт несколько секунд. Не закрывайте приложение.',
      vault_sending_to_network: 'Отправляем в сеть',
      vault_sending_sub: 'Если сразу не пройдёт, мы продолжим попытки.',
      vault_reading_key: 'Чтение ключа…',
      vault_enter_pin: 'Введите PIN YubiKey',
      vault_pin_sub_nfc: 'Затем приложите ключ к верхней части телефона, чтобы завершить.',
      vault_pin_sub_usb: 'Затем коснитесь золотого контакта, чтобы завершить.',
      vault_touch_contact: 'Коснитесь золотого контакта',
      vault_unlock_cta: 'Разблокировать',
      vault_pin_retries: 'Осталось попыток: {{count}}',
      vault_locked: 'Хранилище заблокировано',
      vault_retry: 'Повторить',
      vault_cancel: 'Отмена',
      vault_dismiss: 'Закрыть',
      vault_err_wrong_key: 'Этот YubiKey не входит в ключи этого хранилища.',
      vault_err_serial_mismatch: 'Этот YubiKey не входит в ключи этого хранилища ({{names}}).',
      vault_err_touch_timeout: 'Касание не обнаружено. Повторите и коснитесь золотого контакта.',
      vault_err_pin_locked:
        'PIN этого YubiKey заблокирован. Используйте другой ключ хранилища или разблокируйте этот PUK-кодом в Yubico Authenticator.',
      vault_err_pin_invalid: 'Неверный PIN-код.',
      vault_err_puk_invalid: 'Неверный PUK-код.',
      vault_err_puk_locked: 'PUK этого YubiKey заблокирован. Используйте новый YubiKey.',
      vault_err_ceremony_active:
        'Другая операция хранилища всё ещё использует YubiKey. Дождитесь её завершения и повторите попытку.',
      vault_err_scope_changed: 'Во время операции изменился кошелёк или сеть. Начните заново в текущем кошельке.',
      vault_err_key_removed_mid_op: 'Ключ извлечён. Вставьте его и повторите.',
      vault_err_no_key: 'YubiKey не обнаружен.',
      vault_err_driver_unavailable: 'Поддержка YubiKey недоступна на этом устройстве.',
      vault_err_mgmt_key_custom:
        'У этого ключа задан пользовательский ключ управления, и его нельзя использовать автоматически.',
      vault_err_attestation_invalid:
        'Не удалось проверить заводскую аттестацию этого YubiKey. Используйте подлинный YubiKey с заводской аттестацией и неизменённым слотом аттестации PIV.',
      vault_err_generic: 'Что-то пошло не так. Попробуйте ещё раз.',
      vault_err_template_invalid: 'При попытке доступа к хранилищу произошла ошибка. Попробуйте ещё раз.',
      vault_err_not_enrolled: 'Хранилище не настроено.',
      vault_err_pin_required: 'Введите PIN-код.',
      vault_err_unsupported_platform: 'Поддержка YubiKey недоступна на этом устройстве.',
      vault_err_slot_occupied: 'Слот ключа уже занят.',
      vault_replace_key_warning:
        'Замена безвозвратно уничтожит существующий закрытый ключ P-256. Средства хранилища, которым нужен этот ключ, могут стать непотратимыми.',
      vault_replace_key_confirm: 'Заменить существующий ключ',
      vault_resume_draft_confirm_title: 'Использовать этот подготовленный ключ?',
      vault_resume_draft_confirm_body:
        'Ранее подготовленный ключ — {{nickname}} · …{{tail}} — готов к добавлению, но это устройство только что не проверяло его на карте. Продолжайте, только если узнаёте его.',
      vault_resume_draft_confirm_use: 'Использовать этот ключ',
      vault_resume_draft_confirm_discard: 'Отклонить',
      vault_err_enrollment_partial:
        'Настройка YubiKey остановилась после изменения ключа. Следуйте указаниям для повтора; этот ключ не добавлен в хранилище.',
      vault_err_key_not_adopted:
        'Подтвердите владение восстановленным YubiKey на этом устройстве перед выводом средств с его помощью.',
      vault_err_too_many_inputs:
        'Слишком много вкладов сейфа для одного перевода. Сначала снимите меньшую сумму — каждое снятие также объединяет сейф, поэтому следующее переведёт больше.',
      vault_err_requires_online:
        'Переводы сейфа требуют соединения. Подключитесь и повторите — средства сейфа никогда не ставятся в очередь.',
      vault_err_user_cancelled: 'Отменено.',
      vault_err_vault_empty: 'В хранилище пока нет средств для вывода.',
      vault_err_amount_exceeds_balance: 'Это больше, чем есть в хранилище.',
      vault_err_below_dust: 'Эта сумма слишком мала для отправки.',
      vault_err_no_transaction: 'Не удалось выполнить перевод. Попробуйте ещё раз.',
      vault_err_nfc_lost: 'Связь прервалась. Держите ключ неподвижно у телефона.',
      vault_pin_invalid_retry:
        'Неверный PIN — осталось попыток: {{count}}. Нажмите «Вывести» ещё раз, чтобы повторить.',
      vault_pin_invalid_retry_generic: 'Неверный PIN. Нажмите «Вывести» ещё раз, чтобы повторить.',
      vault_disable_row: 'Отключить хранилище',
      vault_disable_title: 'Отключить хранилище?',
      vault_disable_message:
        'Это стирает список ключей хранилища на этом телефоне. Сами ключи остаются на ваших YubiKey.',
      vault_disable_confirm: 'Отключить',
      vault_disabled_toast: 'Хранилище отключено',
      vault_disable_blocked_title: 'Сначала выведите средства',
      vault_disable_blocked_message:
        'В хранилище ещё есть средства. Выведите всё на повседневный баланс перед отключением — иначе эти средства окажутся заблокированы без возможности их подписать.',
      vault_ok: 'ОК',
      vault_mock_toggle: 'Использовать тестовый YubiKey (dev)',
      vault_on: 'Вкл.',
      vault_off: 'Выкл.',
      vault_shares_word_count:
        'Доли восстановления поддерживают только фразы из двенадцати слов. Запишите фразу вручную.',
      scan_shares_legacy_title: 'Устаревший формат резервной копии',
      scan_shares_legacy_message:
        'Эти доли в устаревшем формате: они восстанавливают повседневный баланс, но не открывают хранилище. Если у вас сохранилась фраза восстановления из двенадцати слов, восстановите кошелёк из неё — она возвращает всё — и затем распечатайте новые доли. Если фраза утрачена, отправьте средства в новый кошелёк и начните заново.',
      scan_shares_legacy_ack: 'Понятно',

      delete_wallet: 'Удалить кошелёк',
      delete_wallet_warning_title: 'Удалить кошелёк?',
      delete_wallet_warning_body:
        'Это безвозвратно удалит ключи вашего кошелька с этого устройства. Если вы не создали резервную копию фразы восстановления или не распечатали доли восстановления, вы навсегда потеряете доступ к кошельку.',
      delete_wallet_confirm: 'Удалить кошелёк',
      delete_wallet_failed_title: 'Не удалось удалить кошелёк',
      delete_wallet_failed_body: 'Не удалось полностью удалить ваш кошелёк с этого устройства. Часть ключевых данных может всё ещё оставаться. Пожалуйста, попробуйте снова.',
      bsv_network: 'Сеть BSV',
      arc_endpoint: 'Конечная точка ARC',
      arc_api_token: 'API-токен',
      arc_custom_url: 'Свой URL',
      arc_apply: 'Применить',
      arc_reset_default: 'Сбросить по умолчанию',
      arc_requires_token: 'нужен токен',
      arc_url_https_required: 'Должен быть https (или http://localhost для локального тестирования)',
      recovery_phrase: 'Ключ восстановления',
      you_have: 'У вас',
      switching: 'Переключение...',
      mainnet: 'Основная сеть',
      testnet: 'Тестовая сеть',
      teratest: 'Teratest',
      identity_key: 'Идентификатор',
      transactions: 'Транзакции',

      no_transactions: 'Транзакций пока нет',
      tx_status_confirmed: 'Подтверждено',
      tx_status_received: 'Получено',
      tx_status_sent: 'Отправлено',
      tx_status_spent: 'Потрачено',
      tx_status_transferred: 'Переведено',
      tx_status_accepted: 'Принята',
      tx_status_broadcasting: 'Обработка',
      tx_status_not_sent: 'Не отправлено',
      tx_status_unsigned: 'Черновик',
      tx_status_nonfinal: 'В ожидании',
      tx_status_pending: 'В ожидании',
      tx_status_failed: 'Ошибка',
      tx_status_offline_queued: 'Офлайн · в очереди',
      tx_status_offline_sending: 'Офлайн · отправка',
      tx_status_offline_rejected: 'Офлайн · отклонено',
      tx_copied: 'Транзакция скопирована',
      tx_not_available: 'Транзакция недоступна',
      tx_copy_failed: 'Не удалось скопировать транзакцию',
      tx_abort_success: 'Транзакция отменена',
      tx_abort_failed: 'Не удалось отменить транзакцию',
      tx_abort_maybe_delivered: 'Невозможно отменить — этот платёж мог уже дойти до получателя',
      tx_proof_refreshed: 'Транзакция подтверждена',
      tx_proof_refresh_failed: 'Не удалось получить доказательство — возможно, транзакция ещё не подтверждена',
      tx_marked_failed: 'Не обработана — помечена как неудачная, средства освобождены',
      tx_still_pending: 'Всё ещё в ожидании — пока не подтверждена',
      tx_action_explorer: 'Посмотреть в обозревателе блоков',
      tx_export_success: 'Экспортировано транзакций: {{count}}',
      tx_export_failed: 'Не удалось экспортировать транзакции',
      tx_export_csv: 'Экспорт в CSV',
      tx_export: 'Экспорт',
      activity_filter: 'Фильтр активности',
      activity_filter_all: 'Все',
      activity_search_placeholder: 'Поиск по заметкам, именам, @юзернеймам',
      activity_search_clear: 'Очистить поиск',
      activity_filter_no_match: 'Ничего не найдено',
      wallet_balance_label: 'Баланс',
      wallet_balance_you_have: 'У вас',
      wallet_balance_refresh: 'Обновить баланс',
      wallet_group_today: 'Сегодня',
      wallet_group_yesterday: 'Вчера',
      back: 'Назад',
      wallet_payments: 'Платежи',
      wallet_vault: 'Хранилище',
      wallet_settings: 'Настройки',
      wallet_activity: 'Активность',

      message_box_server: 'Сервер почтового ящика',
      message_box_required: 'Требуется для отправки и получения платежей на основе идентификатора.',
      send_payment: 'Отправить платёж',
      recipient: 'Получатель',
      amount: 'Сумма',
      enter_valid_amount: 'Введите сумму больше нуля',
      display_currency: 'Валюта отображения',
      no_pending_payments: 'Нет ожидающих платежей',
      outgoing_payments: 'Исходящие',
      payment_not_delivered: 'Не доставлено',
      payment_delivered: 'Доставлено',
      retry: 'Повторить',
      retry_failed: 'Повтор не удался',
      dismiss: 'Скрыть',
      valid_identity_key: 'Введён действительный идентификатор',
      valid_bsv_address: 'Введён действительный BSV-адрес',
      searching: 'Поиск...',
      unknown: 'Неизвестно',
      from_prefix: 'От:',
      message_box_saved: 'Сервер почтового ящика сохранён',
      message_box_removed: 'Сервер почтового ящика удалён',
      message_box_off: 'Выкл.',
      message_box_off_hint: 'Сервер почтового ящика выключен. Включите его в Настройки › Дополнительно.',
      enter_valid_url: 'Введите действительный URL',
      checking_connection: 'Проверка соединения...',
      connection_failed: 'Ошибка соединения',
      recipient_placeholder: 'Имя, идентификатор или BSV-адрес...',

      trust_updated: 'Сеть доверия обновлена',
      failed_to_save: 'Не удалось сохранить',
      confirm_delete: 'Подтвердить удаление',
      confirm_delete_body: 'Вы уверены, что хотите удалить этого сертификатора?',
      order_certifiers_hint:
        'Упорядочите сертификаторов по приоритету. Приложения будут отображать сертификаторов с более высоким рейтингом первыми.',
      certifiers: 'Сертификаторы',
      no_certifiers: 'Сертификаторов пока нет.',
      add_provider: 'Добавить провайдера',
      unsaved_changes: 'У вас есть несохранённые изменения',
      duplicate_key_error: 'Объект с этим открытым ключом уже есть в списке!',
      get_provider_details: 'Получить сведения о провайдере',
      validate_details: 'Проверить данные',
      show_advanced: 'Показать расширенные',
      hide_advanced: 'Скрыть расширенные',
      description_length_error: 'Описание должно содержать от 5 до 50 символов',
      enter_domain_hint: 'Введите домен провайдера',
      enter_details_hint: 'Введите данные провайдера',

      receive: 'Получить',
      send: 'Отправить',
      generating_address: 'Генерация BSV-адреса...',
      imported: 'Импортировано',
      unable_to_generate_address: 'Невозможно создать BSV-адрес',
      send_bsv: 'Отправить BSV',
      recipient_address: 'BSV-адрес получателя',
      enter_bsv_address: 'Введите адрес BSV',

      wallet_data: 'Данные кошелька',
      create_new_wallet: 'Создать новый кошелёк',
      generate_recovery_phrase_caption: 'Создать новый ключ восстановления',
      import_existing_wallet: 'Импортировать существующий кошелёк',
      paste_recovery_phrase: 'Вставьте ключ восстановления',
      save_recovery_phrase_heading: 'Сохраните ключ восстановления',
      save_recovery_phrase_btn: 'Сохранить ключ восстановления',
      recovery_phrase_write_down: 'Запишите эти 12 слов по порядку и храните их в надёжном месте.',
      acknowledgment_text:
        'Я сохранил(а) свой ключ восстановления и понимаю, что его утеря приведёт к полной и необратимой потере всех связанных средств, токенов и сертификатов.',
      enter_recovery_words: 'Введите фразу из 12 слов',
      restore_wallet_description: 'Введите ключ восстановления для восстановления кошелька',
      backup_reminder_title: 'У вашего кошелька нет резервной копии',
      backup_reminder_body: 'Экспортируйте ключи, чтобы не потерять средства.',
      biometric_advisory_title: 'Защитите свой кошелёк',
      wallet_balance_show_in: 'Показать в {{unit}}',
      tx_detail_block: 'Блок',
      tx_detail_block_pending: 'Ожидает блока',
      biometric_advisory_body: 'Защитите свои активы с помощью Face ID или отпечатка пальца',
      biometric_advisory_body_degraded:
        'На этом устройстве не настроены Face ID или отпечаток пальца, поэтому ключ вашего кошелька защищён только паролем устройства.',
      import_prompt_title: 'Уже есть кошелёк?',
      import_prompt_body: 'Восстановите его из ключа восстановления или долей.',
      import_prompt_action: 'Импортировать из копии',
      nearby_advisory_title: 'Подключение поблизости',
      nearby_advisory_body:
        'Вас попросят разрешить доступ к Wi-Fi, Bluetooth или NFC. Это нужно только для прямого соединения с устройством рядом для оплаты — данные не отправляются на наши серверы и не используются для составления вашего профиля.',
      backup_reminder_now: 'Сохранить ключи сейчас',
      backup_reminder_import: 'Импортировать из копии',
      restore_backup_checking: 'Поиск вашей зашифрованной резервной копии…',
      restore_backup_progress: 'Восстановление истории — {{chunks}} из {{total}}…',
      restore_backup_failed_title: 'Не удалось восстановить историю',
      restore_backup_failed_message:
        'Ваша сид-фраза в порядке, но зашифрованную копию не удалось воспроизвести. Обычно достаточно повторить попытку. Продолжив без неё, вы получите рабочий кошелёк без прошлых транзакций, а часть ваших нынешних монет останется непригодной к трате.',
      restore_backup_retry: 'Повторить',
      restore_backup_skip: 'Отказаться',
      restore_backup_unverified_title: 'Резервная копия может быть неполной',
      restore_backup_unverified_body:
        'Ваш кошелёк восстановлен, но не удалось подтвердить, что резервная копия на сервере полная. Если часть истории отсутствует, повторите восстановление позже, когда другое устройство закончит загрузку.',
      import_wallet: 'Импортировать кошелёк',

      reject: 'Отклонить',
      authorize: 'Авторизовать',
      renewal: 'Продление',
      spending_section: 'Расходы',
      protocols_section: 'Протоколы',
      baskets_section: 'Корзины',
      certificates_section: 'Сертификаты',
      requested_fields: 'Запрошенные поля',

      send_max: 'Отправить максимум',
      message_box_unreachable: 'Не удалось связаться с платёжным сервером. Проверьте URL message box в настройках.',
      available: 'доступно',
      note: 'Заметка',
      note_placeholder: 'За что? (необязательно)',
      entire_wallet_balance: 'Весь баланс кошелька',
      entire_vault_balance: 'Весь баланс хранилища',
      export_wallet_data: 'Экспортировать данные кошелька',
      export_unencrypted_title: 'Этот файл не зашифрован',
      export_unencrypted_message:
        'Экспортированный файл — это обычная, незашифрованная копия данных вашего кошелька, включая сведения об идентификации и сертификатах, историю транзакций и контакты. Любой, кто получит этот файл, сможет его прочитать. Делитесь им только там, где вы уверены в надёжности.',
      import_wallet_data: 'Импортировать данные кошелька',
      import_conflict_title: 'База данных уже существует',
      import_conflict_message:
        'База данных кошелька для этой личности с более поздней меткой времени уже существует. Резервная копия с наибольшим числом в имени файла всегда используется.\n\nЧтобы использовать эту старую копию: экспортируйте текущие данные, удалите и переустановите приложение, затем импортируйте только нужную резервную копию.',
      import_confirm_title: 'Импортировать базу данных',
      import_confirm_message: 'Эта база данных станет вашим активным кошельком. Продолжить?',
      import_success: 'База данных успешно импортирована. Перезапуск кошелька...',
      import_invalid_file: 'Недопустимый файл резервной копии',
      import_invalid_file_detail:
        'Этот файл не является допустимой резервной копией базы данных кошелька. Ожидаемое имя файла: wallet-XXXXXXXX-mainnet-TIMESTAMP.db.',
      import_oversized_file: 'Файл резервной копии слишком большой',
      import_oversized_file_detail:
        'Этот файл больше, чем должна быть резервная копия кошелька, поэтому он не был прочитан. Выберите настоящий файл резервной копии базы данных кошелька.',
      import_untrusted_file: 'Не удалось импортировать эту резервную копию',
      import_untrusted_file_detail:
        'Не удалось подтвердить, что эта резервная копия принадлежит этому кошельку, или её содержимому нельзя доверять. Она не была импортирована.',
      import_anyway: 'Импортировать',
      import_invalid_input_title: 'Неверные данные',
      import_invalid_input_message:
        'Введите действительную фразу восстановления (12–24 слова) или 64-значный шестнадцатеричный приватный ключ.',
      import_setup_failed: 'Не удалось настроить кошелёк: {{error}}',
      create_wallet_refused: 'Не удалось создать кошелёк. Попробуйте снова.',
      create_wallet_failed: 'Не удалось сгенерировать мнемоническую фразу. Попробуйте снова.',
      or: 'или',
      print_recovery_shares: 'Распечатать доли восстановления',
      scan_backup_shares: 'Сканировать резервные доли',
      scan_backup_shares_caption: 'Восстановить кошелёк из напечатанных долей',
      scan_shares_invalid_format: 'Неверный формат доли. Отсканируйте действительный QR-код резервной доли.',
      scan_shares_camera_needed: 'Требуется доступ к камере',
      scan_shares_camera_description:
        'Доступ к камере необходим для сканирования QR-кодов напечатанных резервных долей.',
      scan_shares_grant_camera: 'Предоставить доступ к камере',
      scan_shares_recovering: 'Восстановление кошелька...',
      scan_shares_scan_first: 'Отсканируйте первую резервную долю',
      scan_shares_progress: '{{scanned}} из {{needed}} долей отсканировано',
      scan_shares_remaining: 'Требуется ещё {{count}} доля',
      scan_shares_complete: 'Все доли собраны!',
      scan_shares_recovery_failed: 'Восстановление не удалось. Попробуйте отсканировать снова.',
      scan_shares_threshold_mismatch: 'Порог не совпадает с предыдущими долями',
      scan_shares_integrity_mismatch: 'Хеш целостности не совпадает — доли от разных ключей',
      scan_shares_duplicate: 'Эта доля уже была отсканирована',
      scan_shares_biometric_title: 'Требуется биометрия',
      scan_shares_biometric_message: 'Биометрия нужна для защиты ключей вашего кошелька. Попробуйте ещё раз.',
      recovery_replace_wallet_title: 'Заменить кошелёк на этом телефоне?',
      recovery_replace_wallet_body:
        'На этом телефоне уже есть кошелёк. Продолжив, вы замените его и всё, что он может потратить. Если его фраза восстановления не сохранена где-то ещё, эти деньги будут потеряны навсегда.',
      recovery_replace_wallet_confirm: 'Заменить кошелёк',
      invalid_bsv_address: 'Недопустимый адрес BSV',
      scan_recipient_hint: 'Направьте камеру на платёжный код',
      scan_qr_default_hint: 'Направьте камеру на QR-код',
      scan_wallet_qr_hint: 'Направьте камеру на QR-код сопряжения кошелька',

      biometric_store_wallet: 'Безопасно сохранить ваш кошелёк',
      biometric_load_wallet: 'Безопасно загрузить ваш кошелёк',
      biometric_unlock_wallet: 'Разблокируйте кошелёк',
      wallet_lost_title: 'Ключ кошелька уничтожен',
      wallet_lost_body:
        'На этом устройстве изменились биометрические настройки, поэтому система уничтожила ключ, защищавший этот кошелёк. Ваши монеты в безопасности в блокчейне — восстановите кошелёк с помощью секретной фразы.',
      wallet_lost_restore: 'Восстановить',
      wallet_lost_later: 'Не сейчас',
      wallet_locked_title: 'Кошелёк заблокирован',
      wallet_locked_body: 'Разблокируйте, чтобы загрузить ключи кошелька.',
      wallet_unlock_retry: 'Разблокировать кошелёк',
      wallet_lockout_body: 'Слишком много неудачных попыток. Разблокируйте телефон кодом-паролем и попробуйте снова.',
      wallet_no_biometrics_body:
        'На этом устройстве не настроена биометрия. Ключи кошелька по-прежнему зашифрованы хранилищем ключей устройства, но не защищены биометрической проверкой.',

      // Connections screen
      connections: 'Подключения',
      advanced: 'Расширенные',
      copy_secret_words: 'Скопировать секретные слова',
      print_recovery_keys: 'Распечатать ключи восстановления',
      backup_wallet_keys: 'Резервная копия ключей кошелька',
      log_out: 'Выйти',
      connect_to_app: 'Подключить приложение',
      no_connections_yet: 'Нет подключений',
      no_connections_subtitle: 'Отсканируйте QR-код из настольного приложения для подключения кошелька',
      connections_active: 'Активные',
      connections_disconnected: 'Отключено',
      disconnect: 'Отключить',
      reconnect: 'Переподключить',
      reconnect_failed: 'Ошибка переподключения',
      reconnect_confirm_title: 'Переподключиться к этому сайту?',
      reconnect_confirm_message: '{{origin}} снова сможет получить доступ к вашему кошельку.',
      reconnect_needs_reapproval: 'Не удалось проверить это подключение. Отсканируйте новый QR-код, чтобы переподключиться.',
      scan_qr_code: 'Сканировать QR-код',
      paste_uri: 'Вставить URI',
      invalid_qr_code: 'Неверный QR-код',
      wallet_not_ready: 'Кошелёк не готов',
      please_log_in_first: 'Сначала войдите в систему',
      unknown_error: 'Неизвестная ошибка',

      // Pay screen
      payments: 'Платежи',
      pay: 'Оплата',
      pay_direction_pay: 'Заплатить',
      pay_direction_receive: 'Получить',
      pay_cell_nearby_pay: 'Кто-то поблизости',
      pay_cell_nearby_pay_sub: 'Отсканируйте код получателя',
      pay_cell_handle_pay: 'Кто-то удалённо',
      pay_cell_handle_pay_sub: 'Выберите имя — ему нужно это приложение',
      pay_cell_address_pay: 'На BSV-адрес',
      pay_cell_address_pay_sub: 'Вставьте или отсканируйте BSV-адрес',
      pay_cell_nearby_get: 'Кто-то поблизости',
      pay_cell_nearby_get_sub: 'Покажите свой код для оплаты',
      pay_cell_handle_get: 'Кто-то удалённо',
      pay_cell_handle_get_sub: 'Поделитесь своим именем',
      pay_cell_address_get: 'На BSV-адрес',
      pay_cell_address_get_sub: 'Покажите BSV-адрес',
      pay_method: 'Способ',
      pay_method_nearby: 'Рядом',
      pay_method_remote_link: 'Поделиться ссылкой',
      pay_method_address: 'На BSV-адрес',
      pay_pre_nearby: 'Вы оба рядом.',
      pay_pre_handle: 'У получателя есть это приложение.',
      pay_pre_address: 'У вас есть BSV-адрес получателя.',
      pay_conseq_nearby: 'Проходит за считаные секунды.',
      pay_conseq_handle: 'Придёт, когда кошелёк получателя проверит входящие.',
      pay_conseq_address: 'Деньги уходят — получатель не получит уведомление.',
      paid: 'Оплачено',
      pay_inbox_attention: 'Не удалось добавить',
      pay_inbox_attention_hint:
        'Они пришли, но кошелёк не смог их добавить. Повторите попытку или отклоните, чтобы отказаться от них навсегда.',
      pay_dismiss_confirm: 'Нажмите снова, чтобы отказаться',
      pay_dismissed: 'Платёж отклонён',
      payment_arrived: 'Платёж получен',
      payment_arrived_damaged: 'Этот платёж пришёл повреждённым',
      discard_payment_title: 'Отклонить этот платёж?',
      discard_payment_body: 'Отправителя попросят прислать данные заново. До этого деньги остаются в блокчейне.',
      discard_void_title: 'Этот платёж не прошёл',
      discard_void_body: 'Он был отменён или потрачен в другом месте. Его можно спокойно убрать.',
      waiting_network_confirm: 'Ожидаем подтверждения этого платежа сетью',
      finish_or_cancel_outgoing: 'Завершите или отмените платёж ниже, прежде чем отправлять новый',
      identity_search_unavailable: 'Поиск сейчас недоступен. Попробуйте через минуту.',
      abandon_payment: 'Отказаться от платежа',
      finish_payment: 'Завершить платёж',
      cancel_this_payment: 'Отменить этот платёж?',
      cancel_payment: 'Отменить платёж',
      resend: 'Отправить снова',
      resending: 'Отправляем снова…',
      send_payment_details_again: 'Отправить данные снова',
      tx_action_resend_short: 'Отправить снова',
      tx_action_request_confirmation: 'Запросить подтверждение у получателя',
      resend_sent: 'Детали платежа отправлены снова',
      resend_failed_no_record: 'На этом устройстве нет данных об этом платеже, поэтому отправить его снова нельзя.',
      resend_failed_no_recipient: 'В этом платеже не указан получатель, поэтому отправить его снова нельзя.',
      resend_failed_no_transaction: 'Не удалось получить данные платежа. Повторите попытку в сети.',
      resend_requested: 'Вас попросили повторно отправить платёж',
      request_again: 'Запросить снова',
      send_again: 'Отправить снова',
      copy_details: 'Скопировать данные',
      payment_bounced_resend: 'Запрошена повторная отправка этого платежа',
      dismiss_rejected_payment: 'Отклонённый платёж убран',
      activity_load_failed: 'Не удалось загрузить активность',
      activity_load_retry: 'Нажмите, чтобы повторить',
      activity_view_contact: 'Просмотреть контакт',
      home_payments_need_attention: 'Платежей, требующих внимания: {{count}}',
      home_payments_unsent: 'Недоставленных платежей: {{count}}',
      pay_copy: 'Копировать',
      pay_share_link: 'Поделиться ссылкой',
      pay_address_watching: 'Деньги, отправленные на этот адрес, добавляются в ваш кошелёк автоматически.',
      pay_address_earlier_day: 'Перейти к более раннему дню',
      pay_address_sweep_now: 'Проверить этот BSV-адрес сейчас',
      pay_address_swept: 'Получено {{amount}}',

      check_wallet: 'Диагностика',
      settings_version: 'v{{version}}',
      error_review_actions: 'Этот платёж завис. «Диагностика» может попробовать его освободить.',
      wallet_check_ok: 'Всё в порядке',
      wallet_check_attention: 'Некоторые проверки требуют внимания',
      wallet_check_couldnt: 'Не удалось проверить',
      wallet_check_couldnt_body: 'Шаг восстановления не завершился. Никаких предположений о ваших монетах не делалось.',
      wallet_check_summary: 'Освобождено зависших монет: {{freed}} · Восстановлено платежей: {{recovered}}',
      wallet_check_step_coins: 'Ваши монеты действительны',
      wallet_check_skip: 'Пропустить эту проверку',
      wallet_check_skipped: 'Пропущено',
      wallet_check_step_proofs: 'Подтверждения транзакций',
      wallet_check_step_records: 'Записи локальной базы данных',
      wallet_check_step_tokens: 'Переводы стейблкоинов проверены',
      wallet_check_step_online: 'Подключение к сети',
      wallet_check_step_missed: 'Пропущенные платежи',
      wallet_check_step_backup: 'Приватная резервная копия загружена',
      wallet_check_step_phrase: 'Фраза восстановления сохранена',
      wallet_check_progress: '{{current}} из {{total}}',
      wallet_check_stuck_title: 'Не удалось отправить этот платёж',
      wallet_check_stuck_message: 'Некоторые монеты выглядят зависшими. «Диагностика» может их освободить.',
      wallet_check_failed: 'Не удалось завершить проверку. Попробуйте ещё раз.',

      // Local payments
      local_pay_request: 'Запросить платёж',
      local_pay_send: 'Отправить платёж',
      local_pay_amount: 'Сумма',
      local_pay_show_qr: 'Покажите этот код плательщику',
      local_pay_scan_qr: 'Отсканируйте код получателя',
      local_pay_scan_payer_qr: 'Отсканируйте код плательщика',
      local_pay_scan_progress: 'Получено {{have}} из {{total}} частей',
      local_pay_show_payment_qr: 'Покажите этот код получателю',
      local_pay_animated_hint:
        'Попросите отсканировать анимированный код ниже — он будет меняться, пока телефон не получит его целиком.',
      local_pay_show_qr_instead: 'Показать QR-код',
      local_pay_preparing: 'Подготовка запроса…',
      local_pay_delivering: 'Отправка платежа…',
      local_pay_saving: 'Сохранение платежа…',
      local_pay_sent: 'Платёж отправлен',
      local_pay_received: 'Платёж получен',
      local_pay_added: 'Добавлено в ваш кошелёк.',
      local_pay_added_multiple: '{{count}} платежей добавлено в ваш кошелёк.',
      local_pay_received_confirming: 'Подтверждается вашим кошельком…',
      local_pay_received_not_credited:
        'Ещё не добавлено в ваш кошелёк. Проверьте Активность, прежде чем что-либо передавать.',
      local_pay_decline_broadcast_title: 'Отклонённый платёж всё же прошёл',
      local_pay_decline_broadcast_body:
        'Платёж, который был отклонён, с тех пор появился в сети. Не отправляйте его снова — проверьте Активность.',
      local_pay_pending_abort_dropped_title: 'Непроверенный запрос на освобождение проигнорирован',
      local_pay_pending_abort_dropped_body:
        'Не удалось проверить сохранённую запись об освобождении платежа, поэтому ничего не было освобождено. Если платёж всё ещё выглядит зависшим, отмените его в разделе Активность.',
      local_pay_cancel_unverifiable_title: 'Проверить этот платёж без подключения нельзя',
      local_pay_cancel_unverifiable_body:
        'Если получатель уже отсканировал платёж, отмена позволит потратить эти монеты повторно. Отменяйте только если уверены, что его никогда не сканировали.',
      local_pay_cancel_unverifiable_confirm: 'Всё равно отменить',
      local_pay_queued: 'Сохранено — платёж будет добавлен в кошелёк автоматически.',
      local_pay_already_paid: 'Этот запрос уже оплачен.',
      local_pay_failed: 'Платёж не прошёл',
      local_pay_network_denied:
        'Доступ к локальной сети выключен. Включите его в «Настройках», чтобы платить устройствам поблизости.',
      local_pay_radio_fallback: 'Беспроводное соединение недоступно — покажите этот код другому устройству.',
      local_pay_floor_peer_bt_off: 'У них выключен Bluetooth, поэтому этот платёж пройдёт по коду.',
      local_pay_floor_local_bt_off: 'У вас выключен Bluetooth. Включите его для более быстрой связи поблизости.',
      local_pay_floor_local_ble_denied:
        'Доступ к Bluetooth для BSV Wallet выключен. Разрешите его в «Настройках» для более быстрой связи поблизости.',
      local_pay_floor_cross_os:
        'Другое устройство сейчас не может использовать Bluetooth, поэтому этот платёж пройдёт по коду.',
      local_pay_floor_peer_no_radio:
        'Другое устройство не ожидает беспроводного соединения, поэтому этот платёж пройдёт по коду.',
      local_pay_ble_unavailable: 'Bluetooth недоступен. Код выше всё ещё работает.',
      local_pay_too_large: 'Данные этого платежа слишком велики, чтобы передать их через QR-коды.',
      local_pay_nearby_unavailable: 'Wi-Fi поблизости недоступен. Код выше всё ещё работает.',
      local_pay_retry_save: 'Попробовать сохранить снова',
      local_pay_wrong_session:
        'Этот платёж не соответствует запросу и не был принят. Попросите плательщика отсканировать код ещё раз.',
      local_pay_declined_mismatch:
        'Получатель не принял этот платёж — он не соответствует запросу. Отсканируйте его код ещё раз.',
      local_pay_declined_already_paid: 'Этот запрос уже оплачен. Ничего не отправлено.',
      local_pay_declined_save: 'Получатель не смог сохранить платёж. Ничего не отправлено — попробуйте ещё раз.',
      local_pay_declined_decode: 'Получатель не смог прочитать платёж. Ничего не отправлено — попробуйте ещё раз.',
      local_pay_declined_not_covered:
        'Получатель не смог сверить этот платёж с записями эмитента. Ничего не отправлено.',
      local_pay_broadcast_pending:
        'Отправлено. Это устройство пока не смогло провести обработку — это сделает получатель.',
      local_pay_any_amount: 'Любая сумма',
      local_pay_choose_amount: 'Выберите сумму',
      local_pay_enter_amount_send: 'Какую сумму вы хотите отправить?',
      local_pay_qr_unavailable: 'Не удалось создать этот код. Начните запрос заново.',
      local_pay_presence_qr: 'По коду — прямого соединения нет',
      local_pay_presence_ready: 'Шифрованное соединение поблизости готово',
      local_pay_presence_waiting_payee: 'Ожидание устройства поблизости',
      local_pay_presence_waiting_payer: 'Поиск устройства получателя…',
      local_pay_presence_linked: 'Шифрованное соединение открыто',
      local_pay_presence_paid: 'Платёж подтверждён',
      pay_offline_title: 'Вы не в сети',
      pay_offline_body:
        'Сейчас работают только платежи поблизости. Они будут обработаны, как только вы снова окажетесь в сети.',
      pay_offline_queued: '{{count}} платеж(ей) ожидают обработки, как только вы снова окажетесь в сети.',
      pay_offline_pending_title: 'Ожидают обработки',
      pay_offline_pending_body:
        '{{count}} платеж(ей) ещё не обработаны. Пока это не произойдёт, ничего не завершено; попытки продолжаются.',
      pay_offline_needs_internet: 'Нужен интернет',
      pay_offline_rejected_title: 'Платёж был отклонён',
      pay_offline_rejected_body: 'Сеть отклонила платёж, переданный {{sender}} через {{via}} {{when}}.',
      pay_offline_unknown_sender: 'неизвестный отправитель',
      pay_offline_unknown_via: 'неизвестный способ',
      pay_received_not_broadcast: 'Получено без сети · ещё не обработано',
      pay_offline_sent_rejected_title: 'Не удалось доставить платёж',
      pay_offline_sent_rejected_body: 'Сеть отклонила платёж, который вы отправили {{when}}.',
      pay_offline_send_now: 'Отправить сейчас',
      pay_offline_stalled_body: 'Некоторые платежи в очереди не удаётся отправить автоматически: {{detail}}',
      offline_stall_no_request: 'У платежа в очереди отсутствует запись об отправке.',
      offline_stall_bad_beef: 'Не удалось прочитать данные транзакции платежа в очереди.',
      offline_stall_foreign_ancestor:
        'Платёж в очереди ждёт родительскую транзакцию, которую другой кошелёк так и не обработал.',
      pay_offline_kv_pending: 'Платежей поблизости, ожидающих добавления в этот кошелёк: {{count}}.',
      pay_offline_kv_stuck: 'Не удалось добавить {{count}} ближайших платежей. Попросите отправителя отправить снова.',
      pay_offline_kv_corrupt: 'На этом устройстве найдены повреждённые данные платежа.',
      pay_offline_kv_corrupt_body:
        'Затронутые платежи отложены на этом устройстве, а не удалены. Некоторые из них могли уже попасть в ваш баланс, поэтому просите отправителя отправить снова только если его платёж всё ещё не пришёл.',
      pay_offline_show_code: 'Показать код снова',
      pay_parked_cancel: 'Отменить платёж',
      pay_parked_already_sent: 'Этот платёж уже отправлен',
      token_cancel_already_sent: 'Этот платёж уже прошёл, и его нельзя отменить.',
      tx_status_parked: 'Не передан',
      tx_status_import_hold: 'Удержано (импортировано)',
      resend_sending: 'Отправка через почтовый ящик',
      cancelling_payment: 'Отмена платежа',

      contacts: 'Контакты',
      contacts_search_placeholder: 'Поиск по имени или юзернейму',
      contacts_empty_title: 'Пока нет контактов',
      contacts_empty_body:
        'Люди, которым вы платите, появятся здесь после сохранения. Отсканируйте код профиля кнопкой в правом верхнем углу.',
      contacts_recent: 'Недавние',
      contact_edit_name: 'Изменить имя',
      contact_save_name: 'Сохранить имя',
      contact_activity: 'Активность',
      contact_activity_empty: 'Пока нет активности с этим контактом',
      contact_delete: 'Удалить контакт',
      contact_delete_body: 'Это удалит {{name}} из ваших контактов. Это не повлияет на прошлые платежи.',
      contact_handle_caption: 'Их зарегистрированный юзернейм · изменить может только владелец',
      contact_new_title: 'Новый контакт',
      contact_save: 'Сохранить',
      contact_identifier: 'Идентификатор',
      contact_saved: 'Контакт сохранён',
      contact_scan_hint: 'Отсканируйте код профиля, чтобы добавить контакт',
      contact_identifier_placeholder: 'Вставьте его идентификатор',
      contact_identifier_invalid: 'Это не похоже на действительный идентификатор',
      contact_add_from_payment: 'Добавить в контакты',

      profile: 'Профиль',
      profile_display_name: 'Отображаемое имя',
      profile_display_name_hint: 'Показывается публично рядом с вашим юзернеймом. Его увидит любой, кто вас найдёт.',
      profile_handle: 'Юзернейм',
      profile_handle_placeholder: 'вашюзернейм',
      profile_handle_checking: 'Проверка доступности…',
      profile_handle_available: '{{handle}} доступен',
      profile_handle_taken: '{{handle}} уже занят',
      profile_handle_invalid:
        '3-32 символа: a-z, 0-9, точка, подчёркивание или дефис; начинается и заканчивается буквой или цифрой',
      profile_handle_failed: 'Не удалось проверить доступность. Попробуйте снова.',
      profile_handle_unavailable: 'Регистрация юзернейма пока недоступна в этой версии.',
      profile_handle_registered: 'Юзернейм зарегистрирован',
      profile_handle_register_action: 'Зарегистрировать',
      profile_handle_too_similar: '{{handle}} слишком похож на уже занятый',
      profile_handle_reserved: '{{handle}} зарезервирован',
      profile_handle_cooldown: '{{handle}} недавно освобождён и пока недоступен',
      profile_handle_pending: 'Завершаем регистрацию…',
      profile_handle_rolled_back: '{{handle}} уже заняли. За вами осталось {{previous}}.',
      profile_handle_changed: 'Юзернейм изменён',
      profile_handle_rejected: 'Реестр отклонил этот юзернейм.',
      profile_handle_clock_ahead: 'Часы устройства слишком спешат. Попробуйте через несколько минут.',
      profile_handle_wrong_registry: 'Этого юзернейма нет в этом реестре. Откройте профиль заново, чтобы обновить его.',
      profile_display_name_publish_failed: 'Имя сохранено на устройстве, но реестр его не опубликовал.',
      profile_qr_hint: 'Другие могут отсканировать это, чтобы добавить вас в контакты',

      pay_step_contacts: 'Контакты',
      pay_step_continue: 'Продолжить',
      pay_review_to: 'Кому',
      pay_review_amount: 'Сумма',
      pay_review_note: 'Заметка',
      pay_review_note_edit: 'Добавить заметку',
      pay_review_delivery_host: 'Доставка на',
      pay_trust_contact: 'Ваш контакт',
      pay_trust_handle_attested: 'Зарегистрирован',
      pay_trust_handle_domain_attested: 'Подтверждено {{domain}}',
      pay_trust_unverified: 'Не проверено. Внимательно проверьте этот идентификатор.',
      pay_address_network_mismatch: 'Этот адрес принадлежит другой сети, отличной от той, которую вы используете.',
      contacts_your_contacts: 'Ваши контакты',
      contacts_other_people: 'Другие люди',
      contacts_not_saved: 'Не сохранён',
      contacts_network_footer: 'Найдено в сети. Вы этого не сохраняли.',
      contact_no_handle: 'Юзернейма пока нет',
      contact_no_handle_registered: 'Юзернейм не зарегистрирован',
      contact_name_section: 'Ваше имя для него',
      contact_name_hint: 'Видите только вы. Это не меняет того, как он называет себя.',
      contact_new_name_hint: 'Видите только вы. Сохранено на этом устройстве.',
      contact_copy_identifier: 'Скопировать идентификатор',
      profile_handle_registered_hint: 'Зарегистрирован на ваш идентификатор. По нему вас могут найти.',
      profile_handle_change: 'Изменить',
      profile_handle_claim: 'Занять {{handle}}',
      profile_handle_replace_warning:
        'Это заменит {{handle}}. У тех, кто вас сохранил, контакт останется; старый юзернейм больше не будет вести к вам.',
      profile_identifier_section: 'Ваш идентификатор',
      profile_show_qr: 'Показать как QR',
      profile_show_qr_hint: 'Дайте кому-нибудь отсканировать вас, чтобы получить ваш профиль',
      profile_qr_callout:
        'Сканирование откроет BSV Wallet и добавит вас в контакты — с именем, юзернеймом и фото. Оплаты оно не запрашивает: тот код находится в разделе «Получить».',
      pay_review_title: 'Проверка',
      pay_send_amount: 'Отправить {{amount}}',
      pay_sent_to: 'для {{name}}',
      pay_save_as_contact: 'Сохранить как контакт'
    }
  },
  id: {
    translation: {
      token_balances_header: 'Saldo',
      wallet_coin_switcher: 'Pilih koin yang ditampilkan',
      wallet_activity_empty_asset: 'Belum ada aktivitas {{ticker}}',
      pay_direction_pay_asset: 'Bayar {{ticker}}',
      token_fee_footer: 'Mengirim perlu sedikit BSV untuk biaya jaringan. Menerima tidak.',
      token_new_tap: 'Baru — ketuk untuk melihat penerbitnya',
      token_unresolved: 'Token tidak dikenali',
      token_unresolved_sub: 'Dompet ini tidak dapat menemukan penerbitnya. Coba lagi saat daring.',
      token_paused_short: 'Transfer dijeda',
      token_frozen_short: '{{amount}} dibekukan',
      token_sheet_issuer: 'Diterbitkan oleh',
      token_sheet_issuer_unknown: 'Tidak teridentifikasi',
      token_sheet_asset_id: 'ID token',
      token_sheet_powers_header: 'Yang dapat dilakukan {{issuer}}',
      token_power_pause: 'Menjeda semua transfer {{ticker}}',
      token_power_freeze: 'Membekukan koin tertentu, termasuk milik Anda',
      token_power_admit: 'Menentukan siapa yang boleh memegang {{ticker}}',
      token_power_replace: 'Mengganti koin beku dengan yang baru',
      token_powers_limit:
        'Mereka tidak bisa memindahkan {{ticker}} dari dompet Anda tanpa Anda, dan koin yang dibekukan memang tidak bisa dibelanjakan.',
      token_frozen_sub: '{{issuer}} membekukan {{amount}} {{ticker}} dari saldo Anda. Hubungi mereka untuk menyanggah.',
      pay_asset_label: 'Membayar dengan',
      pay_asset_label_get: 'Dibayar dalam',
      pay_asset_cta: 'Kirim {{amount}} {{ticker}}',
      pay_asset_over_balance: 'Melebihi saldo {{ticker}} Anda.',
      pay_asset_needs_bsv: 'Mengirim {{ticker}} perlu sedikit BSV untuk biaya jaringan. Menerima tidak.',
      pay_asset_get_bsv: 'Dapatkan BSV',
      pay_asset_needs_bsv_short: 'Perlu BSV untuk biaya',
      pay_asset_frozen_note: '{{amount}} {{ticker}} dari saldo Anda dibekukan dan tidak bisa dikirim.',
      pay_asset_paused: '{{issuer}} menjeda transfer {{ticker}}, jadi ini belum bisa dikirim.',
      pay_asset_no_address:
        '{{ticker}} hanya bisa dikirim ke orang atau perangkat di dekat Anda — bukan ke alamat Bitcoin.',
      pay_asset_address_status: 'Tidak tersedia untuk {{ticker}}',
      pay_asset_self_unregistered:
        '{{issuer}} belum mendaftarkan Anda untuk mengirim {{ticker}}. Anda tetap bisa menerimanya.',
      pay_asset_unidentified: 'Dompet ini tidak dapat mengenali token ini, jadi belum bisa mengirimnya.',
      pay_asset_recipient_unregistered:
        '{{issuer}} belum mendaftarkan orang ini untuk {{ticker}}, jadi dia belum bisa dibayar dengannya.',
      pay_asset_recipient_blocked: '{{issuer}} tidak mengizinkan pembayaran ke orang ini.',
      token_err_refused: '{{issuer}} menolak transfer ini. Tidak ada yang terkirim dan {{ticker}} Anda tidak berubah.',
      token_err_refused_paused: '{{issuer}} menjeda {{ticker}}. Tidak ada yang terkirim dan saldo Anda tidak berubah.',
      token_err_refused_recipient:
        '{{issuer}} tidak mengizinkan pembayaran ke orang ini. Tidak ada yang terkirim dan saldo Anda tidak berubah.',
      token_err_refused_frozen:
        'Sebagian {{ticker}} yang Anda kirim baru saja dibekukan. Tidak ada yang terkirim; saldo Anda telah diperbarui.',
      token_err_send_failed: 'Tidak dapat mengirim pembayaran ini: {{reason}}. Tidak ada yang keluar dari dompet Anda.',
      token_err_reason_unknown: 'masalah yang tidak diketahui',
      token_err_unreachable:
        'Kami tidak dapat menghubungi {{issuer}} untuk memastikan transfer ini. Periksa saldo {{ticker}} Anda sebelum mencoba lagi.',
      token_err_check_again: 'Periksa lagi',
      token_err_balance_changed: 'Saldo {{ticker}} Anda berubah saat ini disiapkan. Periksa jumlahnya lalu coba lagi.',
      token_err_busy: 'Pembayaran {{ticker}} sedang dikirim.',
      pay_sent_not_broadcast: 'Terkirim. Belum sampai ke jaringan — kami akan terus mencoba.',
      pay_sent_not_notified:
        'Terkirim. Kami belum bisa memberi tahu mereka — akan terlihat saat dompet mereka memeriksa lagi.',
      pay_sent_handed_to_wallet: 'Terkirim. Mereka tidak mengambilnya langsung, jadi kami kirim ke dompet mereka.',
      token_first_hold: '{{ticker}} diterbitkan oleh {{issuer}}, yang dapat menjeda transfer dan membekukan koin.',
      pay_asset_request_paused: '{{issuer}} menjeda {{ticker}}, jadi pembayaran mungkin belum sampai.',
      token_admit_title: 'Anda belum bisa dibayar dalam {{ticker}}',
      token_admit_body:
        '{{issuer}} hanya mengirim {{ticker}} kepada orang yang telah didaftarkan. Sampai itu terjadi, pembayaran untuk Anda ditolak sebelum keluar dari dompet pengirim.',
      token_admit_bsv_instead: 'Dibayar dengan BSV saja',
      token_row_sent: '{{ticker}} terkirim',
      token_row_received: '{{ticker}} diterima',
      token_row_amount_pending: 'Jumlah tidak tersedia',
      token_evicted_title: 'Sebagian {{ticker}} ditarik',
      token_evicted_body: '{{issuer}} menarik {{amount}} {{ticker}} dari dompet Anda.',
      local_pay_token_not_cleared: 'Diterima secara offline · belum dikonfirmasi {{issuer}}',
      pay_asset_nearby_needs_network:
        'Membayar dengan {{ticker}} secara langsung perlu koneksi. Tanpa koneksi, Anda bisa membayar dengan BSV.',
      pay_asset_link_not_held:
        'Permintaan ini untuk token yang tidak dimiliki dompet ini. Anda masih bisa membayar mereka dengan BSV.',
      pay_asset_nearby_wrong_overlay: 'Permintaan ini untuk token yang belum diatur di dompet ini.',
      token_messagebox_mismatch:
        'Dompet Anda diatur ke kotak pesan lain, sehingga pembayaran token mungkin tidak sampai.',
      token_issuer_fallback: 'penerbit',
      token_row_hint: 'Membuka penerbit dan wewenangnya',
      token_attention_one: 'Menunggu konfirmasi {{issuer}} — ketuk untuk detail',
      token_attention_many: '{{count}} pembayaran token menunggu penyelesaian — ketuk untuk detail',
      token_recv_reversed: 'Pembayaran ini tidak dapat dikonfirmasi oleh {{issuer}} dan telah dibatalkan.',
      token_settle_paused_retry: '{{issuer}} sedang menjeda {{ticker}} — kami akan terus mencoba.',
      token_settle_unreachable_retry:
        'Kami tidak dapat menghubungi {{issuer}} untuk memastikan transfer ini. Kami akan terus mencoba.',
      token_settle_evicted:
        'Pembayaran itu tidak sampai ke jaringan tepat waktu dan telah dibatalkan — {{ticker}} Anda kembali ke saldo.',
      token_frame_unreadable: 'Kami tidak bisa membaca pembayaran yang Anda terima — minta pengirim mengirimnya lagi.',
      token_chain_reversed: '{{issuer}} menolak pembayaran sebelumnya dalam rantai ini, jadi yang ini juga dibatalkan.',
      token_settle_stuck_bound:
        'Pembayaran ini sudah menunggu penyelesaian lebih dari 3 hari. Anda bisa menunggu atau menghubungi {{issuer}}.',
      token_cover_failed: 'Pembayaran ini tidak cocok dengan catatan {{issuer}} dan tidak diterima.',
      token_sent_settling:
        'Terkirim. Sedang diselesaikan dengan {{issuer}} — konfirmasinya muncul saat Anda atau {{payee}} tersambung lagi.',
      token_sent_settling_unnamed:
        'Terkirim. Sedang diselesaikan dengan {{issuer}} — konfirmasinya muncul saat salah satu dari Anda tersambung lagi.',
      token_sent_settled: 'Terkirim · diselesaikan dengan {{issuer}}',
      token_status_settling: 'Sedang diselesaikan',
      token_status_settled: 'Selesai',
      token_status_refused: 'Ditolak',
      token_status_reversed: 'Dibatalkan',
      token_status_stuck: 'Menunggu penyelesaian',
      new_tab: 'Tab Baru',
      untitled: 'Tanpa judul',
      share: 'Bagikan',
      back_to_homepage: 'Kembali ke Beranda',
      bookmarks: 'Bookmark',
      add_bookmark: 'Tambah Bookmark',
      bookmark: 'Bookmark',
      unbookmark: 'Hapus Bookmark',
      find_in_page: 'Cari di halaman',
      find_in_page_placeholder: 'Cari di halaman...',
      find_in_page_matches: '{{current}} dari {{total}}',
      find_in_page_no_matches: 'Tidak ditemukan',
      history: 'Riwayat',
      clear_all: 'Hapus Semua',
      clear_all_tabs: 'Tutup Semua Tab',
      search_or_enter_website: 'Cari atau masukkan website',
      browser: 'Browser',
      bug_report: 'Laporan Bug',
      enable_web3: 'Aktifkan Web3',
      tabs: 'Tab',
      unlock_web3_features: 'Buka fitur Web3',

      cancel: 'Batal',
      done: 'Selesai',
      discard: 'Buang',
      deny: 'Tolak',
      allow: 'Izinkan',
      save: 'Simpan',
      remove: 'Hapus',
      delete: 'Hapus',
      search: 'Cari',
      loading: 'Memuat...',
      continue: 'Lanjutkan',
      go_back: 'Kembali',
      continue_action: 'Lanjutkan',
      camera_denied_description:
        'Akses kamera ditolak. Anda dapat mengaktifkannya di Pengaturan untuk menggunakan fitur ini.',
      open_settings: 'Buka Pengaturan',
      copied: 'Tersalin',
      explorer_open_failed: 'Tidak dapat membuka penjelajah blok',
      tx_action_refresh: 'Perbarui status',
      tx_action_refresh_short: 'Status',
      tx_action_abort: 'Batalkan transaksi',
      copy: 'Salin',
      copy_to_clipboard: 'Salin ke Clipboard',
      details: 'Detail',
      hide_details: 'Sembunyikan detail',

      identity: 'Identitas',
      settings: 'Pengaturan',
      wallet: 'Dompet',
      activity: 'Aktivitas',
      configuration: 'Konfigurasi',
      data_and_security: 'Data & Keamanan',
      storage_row: 'Penyimpanan',
      storage_title: 'Penyimpanan dompet',
      storage_db_size: 'Basis data: {{size}}',
      storage_free_space: 'Ruang bebas: {{size}}',
      storage_reclaimable: 'Dapat direklamasi: {{size}} pada {{count}} transaksi',
      storage_held_back: '{{count}} transaksi ditahan karena masih dipakai',
      storage_reclaim_explain:
        'Reklamasi menghapus data validasi yang tidak lagi diperlukan dompet. Transaksi, saldo, dan riwayat Anda tidak tersentuh. Ini membebaskan ruang di dalam berkas basis data; berkasnya sendiri baru menyusut setelah sistem memadatkannya.',
      storage_reclaim_confirm: 'Reklamasi',
      storage_reclaimed: '{{count}} transaksi direklamasi',
      storage_failed: 'Tidak dapat membaca penggunaan penyimpanan.',
      storage_unavailable: 'Penyimpanan belum siap.',
      backup_push_section: 'Cadangan pribadi',
      backup_push_toggle: 'Cadangkan ke server',
      backup_erase_row: 'Hapus cadangan dari server',
      backup_erase_title: 'Hapus cadangan Anda?',
      backup_erase_message:
        'Semua yang dikirim dompet ini ke server cadangan dihapus permanen, di semua perangkat. Pencadangan sekaligus dimatikan. Koin, kunci, dan frasa pemulihan Anda tidak tersentuh — tetapi perangkat baru tidak lagi dapat memulihkan riwayat dompet ini, dan sebagian koin Anda yang ada tidak dapat dibelanjakan di sana.',
      backup_erase_confirm: 'Hapus',
      backup_erase_done: 'Cadangan dihapus — {{count}} item dihapus',
      backup_erase_failed: 'Cadangan tidak dapat dihapus. Tidak ada yang dihapus — coba lagi.',
      backup_erase_no_key: 'Tidak ada kunci dompet',
      backup_push_disclosure:
        'Salinan terenkripsi basis data dompet Anda dikirim ke server BSV Association agar dapat dipulihkan di perangkat baru. Terenkripsi dengan kunci yang hanya bisa diturunkan dari frasa pemulihan Anda — operator tidak dapat membacanya. Anda tetap memerlukan frasa pemulihan atau bagian cetak: tanpa itu cadangan tidak bisa dibuka siapa pun, termasuk kami.',
      backup_push_off_title: 'Matikan cadangan pribadi?',
      backup_push_off_message:
        'Frasa pemulihan saja tidak dapat membangun ulang dompet ini. Informasi yang terkait dengan pengamanan koin Anda hanya ada di basis data, jadi dengan cadangan mati perangkat baru dapat memulihkan frasa tetapi bukan dana Anda. Yang sudah ada di server tidak dihapus.',
      backup_push_off_confirm: 'Matikan',
      backup_push_on_toast: 'Cadangan aktif — sedang menyusul',
      backup_push_off_toast: 'Cadangan mati — tidak ada lagi yang dikirim',
      permissions: 'Izin',
      trust_network: 'Jaringan Kepercayaan',
      // vault (machine-translated, needs native review)
      vault_title: 'Brankas',
      vault_row_title: 'Brankas',
      vault_hero_title: 'Aset Terlindungi',
      vault_hero_body:
        'Penyimpanan dingin jangka panjang, dikunci dengan YubiKey milik Anda sendiri. Memerlukan dua YubiKey 5 NFC atau lebih.',
      vault_unsupported_title: 'Memerlukan YubiKey',
      vault_unsupported_body: 'Dua YubiKey 5 NFC atau lebih, plus ponsel yang bisa membacanya.',
      vault_restore_from_chain: 'Pulihkan brankas dari blockchain',
      vault_restore_scanning: 'Memindai blockchain…',
      vault_restore_found: 'Ditemukan {{count}} setoran brankas di blockchain.',
      vault_restore_pending: '{{count}} setoran ditemukan, menunggu konfirmasi pertamanya — coba lagi sebentar lagi.',
      vault_restore_none_found: 'Tidak ada setoran brankas yang ditemukan di blockchain untuk dompet ini.',
      vault_balance_label: 'Brankas berisi',
      vault_key_section: 'Kunci keamanan ({{count}} dari 5)',
      vault_manage_section: 'Kelola',
      vault_deposit_cta: 'Setor',
      vault_withdraw_cta: 'Tarik',
      vault_deposit_title: 'Setor ke brankas',
      vault_withdraw_title: 'Tarik dari brankas',
      vault_deposit_sub: 'Dari saldo harian Anda. Tidak perlu YubiKey.',
      vault_withdraw_sub: 'Kembali ke saldo harian Anda. Tempelkan salah satu kunci brankas Anda.',
      vault_deposit_done: 'Dipindahkan ke brankas',
      vault_reclaim_done: 'Dana dipulihkan dari setoran brankas yang terputus',
      vault_withdraw_done: 'Ditarik dari brankas',
      vault_withdraw_partial: '{{count}} setoran lagi tersisa — tarik lagi untuk memindahkannya.',
      vault_withdraw_reason: 'Tempelkan YubiKey Anda di sini untuk menandatangani — tarik {{amount}} sats dari brankas',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: 'Belum tersedia — setoran brankas dimatikan pada rilis ini.',
      vault_not_on_mainnet_body:
        'Brankas hanya tersedia di jaringan utama. Kembalikan dompet ke jaringan utama untuk menggunakannya.',
      vault_intro_title: 'Siapkan brankas Anda',
      vault_intro_what:
        'Brankas mengunci uang dengan YubiKey milik Anda sendiri. Tidak ada bagian dari kunci brankas yang pernah ada di ponsel ini; salah satu kunci Anda saja cukup untuk membukanya.',
      vault_intro_two_keys: 'Anda memerlukan setidaknya dua YubiKey.',
      vault_intro_apart: 'Simpan di tempat yang berbeda — dua kunci yang disimpan bersama sama dengan satu kunci.',
      vault_intro_backup:
        'Biarkan cadangan terenkripsi dompet tetap aktif — cadangan ini menyimpan catatan pemulihan yang terautentikasi.',
      vault_intro_ack:
        'Saya mengerti: hanya YubiKey saya yang membuka brankas ini. Frasa pemulihan saya tidak. Jika semuanya hilang, uangnya hilang.',
      vault_intro_piv_ack:
        'Saya mengonfirmasi bahwa seluruh aplikasi PIV YubiKey ini telah direset pabrik atau dikhususkan untuk brankas. Penyiapan mengubah PIN, PUK, dan kunci manajemen PIV global, dapat memengaruhi semua slot PIV, dan saya akan menyimpan PUK baru dengan aman.',
      vault_intro_begin: 'Mulai',
      vault_key_step_title: 'Kunci {{k}} dari maksimal 5',
      vault_key_step_replace:
        'Gunakan YubiKey yang disetel ulang ke pabrik dan khusus untuk brankas. Bila slot brankasnya berisi kunci lama, penyiapan akan memperingatkan sebelum menggantinya.',
      vault_pin_choose_title: 'Pilih PIN',
      vault_pin_choose_sub: 'Anda akan memasukkannya setiap kali memakai kunci ini. 6 sampai 8 digit.',
      vault_pin_confirm_label: 'Masukkan sekali lagi',
      vault_pin_mismatch: 'Kedua PIN tidak sama.',
      vault_pin_not_default: 'Pilih selain 123456.',
      vault_puk_title: 'Kode pemulihan Anda',
      vault_puk_body:
        'Catat dan simpan di tempat aman. Ini satu-satunya cara membuka kunci ini bila PIN terlupa, dan tidak akan ditampilkan lagi.',
      vault_puk_ack: 'Saya sudah mencatatnya',
      vault_reset_offer: 'Setel ulang kunci ini',
      vault_reset_title: 'Setel ulang kunci ini?',
      vault_reset_body:
        'Kunci ini pernah dipakai. Menyetel ulang menghapus setiap sertifikat dan kunci di aplikasi PIV-nya, bukan hanya slot brankas. Semua yang bergantung padanya berhenti berfungsi.',
      vault_reset_ack: 'Saya paham ini menghapus semua isi kunci ini',
      vault_reset_unknown_ack:
        'Perangkat ini tidak dapat memastikan apakah kunci ini masih mengamankan brankas profil lain. Saya paham menghapusnya berpotensi memusnahkan kunci itu selamanya.',
      vault_reset_own_draft_ack:
        'Kunci ini berisi kunci yang belum selesai dari percobaan Anda sebelumnya, dan perangkat ini masih menyimpan catatannya. Saya paham menyetel ulang akan menghapusnya dan percobaan itu tidak dapat dilanjutkan.',
      vault_reset_nothing_erased:
        'Tidak ada yang dihapus. Slot brankas pada kunci ini tidak kosong, jadi ada satu hal lagi untuk dikonfirmasi.',
      vault_reset_confirm: 'Hapus dan setel ulang',
      vault_resetting: 'Menyetel ulang kunci…',
      vault_reset_done: 'Kunci disetel ulang. Tempelkan lagi untuk melanjutkan.',
      vault_reset_uncertain:
        'Penyetelan ulang tidak selesai. Kunci ini mungkin sudah terhapus — periksa dulu sebelum mengandalkannya.',
      vault_err_reset_enrolled:
        'Kunci itu sudah menjadi bagian brankas Anda — menyetel ulangnya akan menghancurkan salah satu kunci brankas.',
      vault_err_reset_unreadable:
        'Catatan brankas Anda di perangkat ini tidak dapat dibaca, jadi tidak ada cara membuktikan kunci ini bukan salah satu kunci brankas Anda. Tidak ada yang dihapus dan penyetelan ulang ditolak.',
      vault_enrollment_resume_pin_hint:
        'Melanjutkan kunci tersimpan memerlukan PIN yang Anda pilih untuknya sebelumnya, bukan yang baru. Tiga kali salah akan mengunci kunci.',
      vault_setup_step: 'Langkah {{n}} dari {{total}}',
      vault_enrollment_resume: 'Lanjutkan kunci tersimpan',
      vault_enrollment_reset_required:
        'YubiKey ini tidak lagi dalam keadaan pabrik, sehingga tidak dapat didaftarkan apa adanya. Aplikasi PIV-nya harus dihapus dan dikembalikan ke keadaan pabrik sebelum dapat menyimpan kunci brankas.',
      vault_enrollment_state_uncertain:
        'Upaya penyiapan terhenti, sehingga tidak ada cara untuk memastikan dalam keadaan apa YubiKey itu ditinggalkan. Kunci tersebut tidak akan dipakai lagi sampai aplikasi PIV-nya dihapus dan dikembalikan ke keadaan pabrik.',
      vault_nfc_enroll_message: 'Tempelkan YubiKey Anda di sini untuk menyiapkannya',
      vault_nfc_adopt_message: 'Tempelkan YubiKey yang dipulihkan ini untuk memverifikasinya',
      vault_nfc_sign_batch: 'Tempelkan YubiKey Anda di sini untuk menandatangani — kelompok {{b}} dari {{n}}',
      vault_key_use_different: 'Gunakan YubiKey lain',
      vault_key_setup_again: 'Siapkan lagi',
      vault_name_default: 'Kunci {{k}}',
      vault_more_title: 'Tambah kunci lain?',
      vault_more_body: 'Hingga 5 kunci. Salah satunya saja bisa membuka seluruh brankas.',
      vault_more_add: 'Tambah kunci lain',
      vault_more_finish: 'Selesai',
      vault_more_need_two: 'Tambahkan kunci kedua sebelum selesai — satu kunci berarti tidak ada pemulihan.',
      vault_leave_setup: 'Keluar dari penyiapan',
      vault_leave_title: 'Keluar dari penyiapan?',
      vault_leave_body:
        '{{count}} YubiKey yang Anda siapkan belum akan disimpan. Kuncinya tetap ada, jadi Anda bisa menambahkannya lagi sebentar lagi.',
      vault_leave_confirm: 'Keluar',
      vault_leave_stay: 'Tetap di sini',
      vault_done_body:
        '{{count}} kunci dapat membuka brankas ini. Hanya kunci-kunci ini yang membukanya — frasa pemulihan Anda tidak.',
      vault_done_cta: 'Selesai',
      vault_add_key_row: 'Tambah kunci',
      vault_add_key_done:
        '{{nickname}} dapat membuka setoran yang dibuat mulai sekarang. Kunci ulang brankas agar ia bisa membuka semuanya.',
      vault_key_added_toast: 'Kunci ditambahkan',
      vault_recovery_verified: 'Kunci yang dipulihkan telah diverifikasi di perangkat ini',
      vault_relock_row: 'Kunci ulang brankas',
      vault_relock_now: 'Kunci ulang sekarang',
      vault_relock_choose: 'Kunci mana yang akan Anda tempelkan untuk mengunci ulang?',
      vault_relock_reason:
        'Tempelkan salah satu kunci Anda yang sudah ada ({{names}}) — bukan yang baru saja Anda tambahkan.',
      vault_relock_reason_generic:
        'Tempelkan YubiKey Anda di sini untuk menandatangani — kunci ulang brankas ke kunci Anda saat ini',
      vault_relock_reason_remaining: 'Kunci ulang hanya ke kunci Anda yang tersisa: {{names}}.',
      vault_key_pending_removal: 'Dihapus — kunci ulang untuk menyelesaikan',
      vault_key_removal_awaiting_network: 'Sedang dihapus — menunggu jaringan menerima kunci ulang',
      vault_removal_awaiting_network: 'Kunci ulang terkirim. Setoran dan penarikan terbuka lagi setelah jaringan menerimanya.',
      vault_err_too_many_active_keys: 'Brankas dikunci dengan paling banyak lima kunci. Hapus satu sebelum menyetor, mengunci ulang, atau menarik sebagian saldo.',
      vault_key_section_over_limit: 'Kunci keamanan ({{count}}, hapus satu)',
      vault_add_key_done_over_limit: '{{nickname}} ditambahkan. Brankas dikunci dengan paling banyak lima kunci, jadi hapus kunci yang digantikannya, lalu kunci ulang sekali.',
      vault_relock_done: 'Brankas dikunci ulang ke kunci Anda saat ini',
      vault_relock_capped: '{{count}} setoran lagi tersisa — kunci ulang lagi untuk memindahkannya.',
      vault_relock_unreachable:
        '{{count}} setoran hanya bisa dibuka oleh {{names}}. Kunci ulang lagi dengan salah satu kunci itu.',
      vault_badge_missing: '{{count}} setoran belum terbuka untuk {{nickname}}',
      vault_badge_removed: '{{count}} setoran masih terbuka untuk kunci yang dihapus',
      vault_key_action_rename: 'Ubah nama',
      vault_key_action_remove: 'Hapus',
      vault_rename_title: 'Ubah nama {{nickname}}',
      vault_rename_save: 'Simpan',
      vault_remove_title: 'Hapus {{nickname}}?',
      vault_remove_body:
        'Dompet akan berhenti menggunakan {{nickname}}. Uang yang sudah ada di brankas tetap bisa dibuka olehnya sampai Anda mengunci ulang (≈ {{fee}} sat).',
      vault_remove_only: 'Hapus saja',
      vault_key_removed_toast: 'Kunci dihapus',
      vault_export_explainer:
        'Setiap setoran brankas menyembunyikan salt-nya sampai Anda membelanjakannya — tidak ada yang terlihat mengenainya di skrip pengunci. Biarkan cadangan terenkripsi tetap aktif agar perangkat baru dapat memulihkan dan mengautentikasi catatan kunci yang diperlukan untuk memakai YubiKey Anda.',
      vault_footnote: 'Hanya kunci-kunci ini yang membuka brankas. Frasa pemulihan Anda tidak.',
      vault_floor_line:
        'Setoran minimum {{floorDisplay}} ({{floorSats}} sat). Membuat setoran brankas berbiaya sekitar {{feeDisplay}}.',
      vault_first_deposit_title: 'Setoran brankas pertama',
      vault_first_deposit_body:
        'Setoran brankas pertama — {{amount}} hanya bisa dibuka dengan {{count}} YubiKey ({{names}}). Frasa pemulihan Anda tidak akan membantu.',
      vault_backup_off_title: 'Aktifkan cadangan pribadi',
      vault_backup_off_body:
        'Cadangan pribadi harus aktif sebelum dompet membuat output brankas baru. Cadangan ini menyimpan catatan YubiKey terautentikasi dan riwayat dompet yang diperlukan untuk pemulihan. Anda tetap dapat menarik semuanya tanpa membuat kembalian brankas.',
      vault_backup_off_cta: 'Buka pengaturan cadangan',
      vault_seed_not_preserved_title: 'Simpan dulu frasa pemulihan Anda',
      vault_seed_not_preserved_body:
        'Vault Anda hanya bisa dipulihkan dengan frasa pemulihan Anda ditambah salah satu YubiKey Anda. YubiKey tidak menyimpan salinannya — jika frasa itu hilang sebelum Anda mencatatnya, dana ini tidak dapat dipulihkan. Simpan sekarang sebelum melakukan setoran.',
      vault_seed_not_preserved_cta: 'Simpan frasa pemulihan',
      vault_choose_key: 'Kunci mana yang akan Anda tempelkan?',
      vault_remainder_title: 'Tarik semuanya?',
      vault_remainder_body:
        'Menarik {{amount}} menyisakan {{remainder}}, di bawah minimum brankas 100.000 sat. Seluruh brankas akan pindah ke saldo harian Anda.',
      vault_remainder_all: 'Tarik semuanya',
      vault_remainder_change: 'Ubah jumlah',
      vault_unreachable_title: 'Sebagian brankas memerlukan kunci lain',
      vault_unreachable_body:
        'Sebagian brankas memerlukan kunci lain — {{moved}} dipindahkan. {{count}} setoran senilai {{amount}} hanya bisa dibuka oleh {{names}}. Tarik lagi dengan salah satu kunci itu.',
      vault_sign_progress: '{{signed}} dari {{total}} ditandatangani',
      vault_err_pin_locked_enroll:
        'PIN YubiKey ini terblokir. Buka blokirnya dengan PUK di Yubico Authenticator, atau siapkan dengan YubiKey lain.',
      vault_err_serial_mismatch_chosen:
        'Itu {{tappedName}}. Anda memilih {{chosenName}} — tempelkan itu, atau kembali dan pilih {{tappedName}}.',
      vault_err_key_already_enrolled: 'Anda sudah menambahkan YubiKey ini ({{nickname}}). Tempelkan yang lain.',
      vault_err_not_released: 'Setoran brankas dimatikan pada rilis ini.',
      vault_err_not_on_mainnet:
        'Brankas hanya tersedia di jaringan utama. Alihkan dompet ke jaringan utama lalu coba lagi.',
      vault_err_backup_off:
        'Aktifkan cadangan pribadi terenkripsi sebelum membuat output brankas baru. Anda tetap dapat menarik semuanya.',
      vault_err_not_enough_keys: 'Brankas ini memiliki kurang dari dua kunci. Siapkan lagi.',
      vault_err_too_many_keys: 'Brankas menampung paling banyak lima kunci.',
      vault_err_last_keys: 'Brankas memerlukan setidaknya dua kunci. Tambahkan kunci lain sebelum menghapus yang ini.',
      vault_err_relock_required: 'Kunci ulang brankas dahulu agar kunci Anda yang lain bisa membuka setiap setoran.',
      vault_err_action_pending: 'Sebuah transaksi brankas belum selesai. Coba lagi setelah selesai.',
      vault_resolve_held_deposit_action: 'Selesaikan deposit yang tertunda',
      vault_resolve_held_deposit_done: 'Masalah deposit teratasi',
      vault_resolve_held_deposit_failed: 'Tidak dapat mengatasi masalah deposit. Coba lagi.',
      vault_resolve_held_deposit_none:
        'Tidak ada yang bisa diselesaikan di sini — periksa tab Aktivitas untuk transaksi tertunda yang bisa dicoba lagi atau dibatalkan.',
      vault_err_key_not_committed:
        '{{nickname}} tidak bisa membuka satu pun setoran di brankas ini. Gunakan kunci brankas Anda yang lain.',
      vault_err_key_cannot_cover:
        '{{nickname}} bisa membuka {{reachable}} dari {{total}} di brankas. Tarik hingga {{reachable}}, atau gunakan {{otherNames}}.',
      vault_err_too_small_to_relock:
        'Brankas ini berisi kurang dari 100.000 sat, terlalu kecil untuk dikunci ulang. Tarik saja lalu setor lagi.',
      vault_err_bad_version: 'Transaksi dibangun dalam format yang salah. Tidak ada yang ditandatangani — coba lagi.',
      vault_err_chain_scan_failed:
        'Tidak dapat menyelesaikan pemindaian blockchain untuk setoran vault. Periksa koneksi Anda dan coba lagi.',
      vault_shares_unavailable: 'Tidak ada materi kunci yang bisa dicetak.',
      vault_enroll_begin: 'Siapkan brankas',
      vault_enroll_phase_connecting: 'Membaca kunci Anda…',
      vault_enroll_phase_pin_check: 'Memeriksa kunci Anda…',
      vault_enroll_phase_checking_slot: 'Memeriksa slot brankas…',
      vault_enroll_phase_personalizing: 'Menyiapkan PIN dan kode pemulihan Anda…',
      vault_enroll_phase_generating: 'Membuat kunci brankas — sentuh kunci saat berkedip',
      vault_enroll_phase_challenging: 'Memverifikasi kunci brankas baru — sentuh kunci saat berkedip',
      vault_enroll_phase_done: 'Selesai',
      vault_touch_when_blinks: 'Sentuh kontak emas saat kunci berkedip.',
      vault_nfc_activation_hint:
        'Pertama kali memakai YubiKey ini? Colokkan dulu ke port USB-C selama beberapa detik — kunci baru dikirim dengan NFC nonaktif sampai saat itu.',
      vault_continue: 'Lanjutkan',
      vault_back: 'Kembali',
      vault_enrolled_toast: 'Brankas siap',
      vault_insert_key: 'Masukkan YubiKey Anda',
      vault_hold_key_nfc: 'Tempelkan YubiKey ke bagian atas ponsel Anda',
      vault_keep_holding_nfc: 'Tahan terus — sedang membuka…',
      vault_unlocking_funds: 'Membuka dana Anda',
      vault_unlocking_sub: 'Ini memakan waktu beberapa detik. Biarkan aplikasi tetap terbuka.',
      vault_sending_to_network: 'Mengirim ke jaringan',
      vault_sending_sub: 'Jika tidak langsung berhasil, kami akan terus mencoba.',
      vault_reading_key: 'Membaca kunci…',
      vault_enter_pin: 'Masukkan PIN YubiKey',
      vault_pin_sub_nfc: 'Lalu tempelkan kunci Anda ke bagian atas ponsel untuk menyelesaikan.',
      vault_pin_sub_usb: 'Lalu sentuh kontak emasnya untuk menyelesaikan.',
      vault_touch_contact: 'Sentuh kontak emas',
      vault_unlock_cta: 'Buka',
      vault_pin_retries: '{{count}} percobaan tersisa',
      vault_locked: 'Brankas terkunci',
      vault_retry: 'Coba lagi',
      vault_cancel: 'Batal',
      vault_dismiss: 'Tutup',
      vault_err_wrong_key: 'YubiKey ini bukan salah satu kunci brankas ini.',
      vault_err_serial_mismatch: 'YubiKey ini bukan salah satu kunci brankas ini ({{names}}).',
      vault_err_touch_timeout: 'Tidak ada sentuhan terdeteksi. Coba lagi dan sentuh kontak emas.',
      vault_err_pin_locked:
        'PIN YubiKey ini terblokir. Gunakan kunci brankas Anda yang lain, atau buka blokirnya dengan PUK di Yubico Authenticator.',
      vault_err_pin_invalid: 'PIN salah.',
      vault_err_puk_invalid: 'PUK salah.',
      vault_err_puk_locked: 'PUK YubiKey ini terblokir. Gunakan YubiKey baru.',
      vault_err_ceremony_active:
        'Operasi brankas lain masih menggunakan YubiKey. Tunggu hingga selesai lalu coba lagi.',
      vault_err_scope_changed: 'Dompet atau jaringan berubah selama operasi ini. Mulai lagi di dompet saat ini.',
      vault_err_key_removed_mid_op: 'Kunci dicabut. Masukkan dan coba lagi.',
      vault_err_no_key: 'YubiKey tidak terdeteksi.',
      vault_err_driver_unavailable: 'Dukungan YubiKey tidak tersedia di perangkat ini.',
      vault_err_mgmt_key_custom: 'Kunci ini memiliki kunci manajemen khusus dan tidak dapat digunakan secara otomatis.',
      vault_err_attestation_invalid:
        'Pengesahan produsen YubiKey ini tidak dapat diverifikasi. Gunakan YubiKey asli yang disahkan pabrik dengan slot pengesahan PIV yang belum diubah.',
      vault_err_generic: 'Terjadi kesalahan. Silakan coba lagi.',
      vault_err_template_invalid: 'Terjadi kesalahan saat mencoba mengakses brankas. Silakan coba lagi.',
      vault_err_not_enrolled: 'Belum ada brankas yang disiapkan.',
      vault_err_pin_required: 'Masukkan PIN Anda.',
      vault_err_unsupported_platform: 'Dukungan YubiKey tidak tersedia di perangkat ini.',
      vault_err_slot_occupied: 'Slot kunci sudah digunakan.',
      vault_replace_key_warning:
        'Menggantinya akan menghancurkan kunci privat P-256 yang ada secara permanen. Dana brankas yang memerlukan kunci itu bisa menjadi tidak dapat dibelanjakan.',
      vault_replace_key_confirm: 'Ganti kunci yang ada',
      vault_resume_draft_confirm_title: 'Gunakan kunci yang sudah disiapkan ini?',
      vault_resume_draft_confirm_body:
        'Kunci yang disiapkan sebelumnya — {{nickname}} · …{{tail}} — siap ditambahkan, tetapi perangkat ini belum baru saja memverifikasinya pada kartu. Lanjutkan hanya jika Anda mengenalinya.',
      vault_resume_draft_confirm_use: 'Gunakan kunci ini',
      vault_resume_draft_confirm_discard: 'Buang kunci ini',
      vault_err_enrollment_partial:
        'Penyiapan YubiKey berhenti setelah kunci diubah. Ikuti petunjuk mencoba lagi; kunci ini belum ditambahkan ke brankas.',
      vault_err_key_not_adopted:
        'Verifikasi kepemilikan YubiKey yang dipulihkan ini di perangkat sebelum menggunakannya untuk penarikan.',
      vault_err_too_many_inputs:
        'Terlalu banyak setoran vault untuk dipindahkan sekaligus. Tarik jumlah lebih kecil dulu — setiap penarikan juga mengonsolidasi vault, jadi berikutnya bisa lebih banyak.',
      vault_err_requires_online:
        'Transfer vault memerlukan koneksi. Sambungkan lagi dan coba ulang — dana vault tidak pernah dimasukkan ke antrean.',
      vault_err_user_cancelled: 'Dibatalkan.',
      vault_err_vault_empty: 'Brankas belum memiliki dana untuk ditarik.',
      vault_err_amount_exceeds_balance: 'Jumlah itu melebihi isi brankas.',
      vault_err_below_dust: 'Jumlah itu terlalu kecil untuk dikirim.',
      vault_err_no_transaction: 'Transfer tidak dapat diselesaikan. Silakan coba lagi.',
      vault_err_nfc_lost: 'Koneksi terputus. Tahan kunci dengan stabil di ponsel Anda.',
      vault_pin_invalid_retry: 'PIN salah — sisa {{count}} percobaan. Ketuk Tarik lagi untuk mencoba ulang.',
      vault_pin_invalid_retry_generic: 'PIN salah. Ketuk Tarik lagi untuk mencoba ulang.',
      vault_disable_row: 'Nonaktifkan brankas',
      vault_disable_title: 'Nonaktifkan brankas?',
      vault_disable_message: 'Ini melupakan daftar kunci brankas di ponsel ini. Kuncinya tetap ada di YubiKey Anda.',
      vault_disable_confirm: 'Nonaktifkan',
      vault_disabled_toast: 'Brankas dinonaktifkan',
      vault_disable_blocked_title: 'Tarik dananya dulu',
      vault_disable_blocked_message:
        'Brankas masih menyimpan dana. Tarik semuanya ke saldo harian Anda sebelum menonaktifkan — jika tidak, dana itu akan terkunci tanpa cara untuk menandatanganinya.',
      vault_ok: 'OK',
      vault_mock_toggle: 'Gunakan YubiKey tiruan (dev)',
      vault_on: 'Aktif',
      vault_off: 'Nonaktif',
      vault_shares_word_count: 'Bagian pemulihan hanya mendukung frasa dua belas kata. Tulis frasa Anda saja.',
      scan_shares_legacy_title: 'Format cadangan lama',
      scan_shares_legacy_message:
        'Bagian-bagian ini berformat lama yang memulihkan saldo harian Anda tetapi tidak dapat membuka brankas. Jika Anda masih memiliki frasa pemulihan dua belas kata, pulihkan dari frasa itu — frasa itu memulihkan semuanya — lalu cetak bagian yang baru. Jika frasanya hilang, kirim dana Anda ke dompet baru dan mulai lagi dari awal.',
      scan_shares_legacy_ack: 'Saya mengerti',

      delete_wallet: 'Hapus Dompet',
      delete_wallet_warning_title: 'Hapus Dompet?',
      delete_wallet_warning_body:
        'Ini akan menghapus kunci dompet Anda secara permanen dari perangkat ini. Jika Anda belum mencadangkan frasa pemulihan atau mencetak bagian pemulihan, Anda akan kehilangan akses ke dompet Anda selamanya.',
      delete_wallet_confirm: 'Hapus Dompet',
      delete_wallet_failed_title: 'Gagal Menghapus Dompet',
      delete_wallet_failed_body: 'Dompet Anda tidak dapat sepenuhnya dihapus dari perangkat ini. Sebagian materi kunci mungkin masih ada. Silakan coba lagi.',
      bsv_network: 'Jaringan BSV',
      arc_endpoint: 'Endpoint ARC',
      arc_api_token: 'Token API',
      arc_custom_url: 'URL Khusus',
      arc_apply: 'Terapkan',
      arc_reset_default: 'Kembalikan ke Bawaan',
      arc_requires_token: 'perlu token',
      arc_url_https_required: 'Harus https (atau http://localhost untuk pengujian lokal)',
      recovery_phrase: 'Kunci Pemulihan',
      you_have: 'Anda memiliki',
      switching: 'Mengalihkan...',
      mainnet: 'Mainnet',
      testnet: 'Testnet',
      teratest: 'Teratest',
      identity_key: 'Pengenal',
      transactions: 'Transaksi',

      no_transactions: 'Belum ada transaksi',
      tx_status_confirmed: 'Dikonfirmasi',
      tx_status_received: 'Diterima',
      tx_status_sent: 'Terkirim',
      tx_status_spent: 'Dibelanjakan',
      tx_status_transferred: 'Dipindahkan',
      tx_status_accepted: 'Diterima',
      tx_status_broadcasting: 'Memproses',
      tx_status_not_sent: 'Tidak Dikirim',
      tx_status_unsigned: 'Draf',
      tx_status_nonfinal: 'Menunggu',
      tx_status_pending: 'Menunggu',
      tx_status_failed: 'Gagal',
      tx_status_offline_queued: 'Offline · antre',
      tx_status_offline_sending: 'Offline · mengirim',
      tx_status_offline_rejected: 'Offline · ditolak',
      tx_copied: 'Transaksi disalin',
      tx_not_available: 'Transaksi tidak tersedia',
      tx_copy_failed: 'Gagal menyalin transaksi',
      tx_abort_success: 'Transaksi dibatalkan',
      tx_abort_failed: 'Gagal membatalkan transaksi',
      tx_abort_maybe_delivered: 'Tidak dapat dibatalkan — pembayaran ini mungkin sudah diterima penerima',
      tx_proof_refreshed: 'Transaksi terkonfirmasi',
      tx_proof_refresh_failed: 'Tidak dapat mengambil bukti — transaksi mungkin belum dikonfirmasi',
      tx_marked_failed: 'Tidak diproses — ditandai gagal dan dana dilepaskan',
      tx_still_pending: 'Masih tertunda — belum terkonfirmasi',
      tx_action_explorer: 'Lihat di block explorer',
      tx_export_success: '{{count}} transaksi diekspor',
      tx_export_failed: 'Gagal mengekspor transaksi',
      tx_export_csv: 'Ekspor CSV',
      tx_export: 'Ekspor',
      activity_filter: 'Filter aktivitas',
      activity_filter_all: 'Semua',
      activity_search_placeholder: 'Cari catatan, nama, @handle',
      activity_search_clear: 'Hapus pencarian',
      activity_filter_no_match: 'Tidak ada aktivitas yang cocok',
      wallet_balance_label: 'Saldo',
      wallet_balance_you_have: 'Anda memiliki',
      wallet_balance_refresh: 'Perbarui saldo',
      wallet_group_today: 'Hari ini',
      wallet_group_yesterday: 'Kemarin',
      back: 'Kembali',
      wallet_payments: 'Pembayaran',
      wallet_vault: 'Brankas',
      wallet_settings: 'Pengaturan',
      wallet_activity: 'Aktivitas',

      message_box_server: 'Server Kotak Pesan',
      message_box_required: 'Diperlukan untuk mengirim dan menerima pembayaran berbasis identitas.',
      send_payment: 'Kirim Pembayaran',
      recipient: 'Penerima',
      amount: 'Jumlah',
      enter_valid_amount: 'Masukkan jumlah lebih dari nol',
      display_currency: 'Mata uang tampilan',
      no_pending_payments: 'Tidak ada pembayaran tertunda',
      outgoing_payments: 'Keluar',
      payment_not_delivered: 'Belum terkirim',
      payment_delivered: 'Terkirim',
      retry: 'Coba lagi',
      retry_failed: 'Gagal mencoba lagi',
      dismiss: 'Tutup',
      valid_identity_key: 'Pengenal valid dimasukkan',
      valid_bsv_address: 'Alamat BSV valid dimasukkan',
      searching: 'Mencari...',
      unknown: 'Tidak Diketahui',
      from_prefix: 'Dari:',
      message_box_saved: 'Server kotak pesan disimpan',
      message_box_removed: 'Server kotak pesan dihapus',
      message_box_off: 'Mati',
      message_box_off_hint: 'Server kotak pesan mati. Nyalakan di Pengaturan › Lanjutan.',
      enter_valid_url: 'Masukkan URL yang valid',
      checking_connection: 'Memeriksa koneksi...',
      connection_failed: 'Koneksi gagal',
      recipient_placeholder: 'Nama, pengenal, atau alamat BSV...',

      trust_updated: 'Jaringan kepercayaan diperbarui',
      failed_to_save: 'Gagal menyimpan',
      confirm_delete: 'Konfirmasi Hapus',
      confirm_delete_body: 'Apakah Anda yakin ingin menghapus sertifikator ini?',
      order_certifiers_hint:
        'Urutkan sertifikator berdasarkan prioritas. Aplikasi akan menampilkan sertifikator berperingkat lebih tinggi terlebih dahulu.',
      certifiers: 'Sertifikator',
      no_certifiers: 'Belum ada sertifikator.',
      add_provider: 'Tambah Penyedia',
      unsaved_changes: 'Anda memiliki perubahan yang belum disimpan',
      duplicate_key_error: 'Entitas dengan kunci publik ini sudah ada di daftar!',
      get_provider_details: 'Dapatkan Detail Penyedia',
      validate_details: 'Validasi Detail',
      show_advanced: 'Tampilkan Lanjutan',
      hide_advanced: 'Sembunyikan Lanjutan',
      description_length_error: 'Deskripsi harus antara 5 dan 50 karakter',
      enter_domain_hint: 'Masukkan domain penyedia',
      enter_details_hint: 'Masukkan detail penyedia',

      receive: 'Terima',
      send: 'Kirim',
      generating_address: 'Membuat alamat BSV...',
      imported: 'Diimpor',
      unable_to_generate_address: 'Tidak dapat membuat alamat BSV',
      send_bsv: 'Kirim BSV',
      recipient_address: 'Alamat BSV Penerima',
      enter_bsv_address: 'Masukkan alamat BSV',

      wallet_data: 'Data Dompet',
      create_new_wallet: 'Buat Dompet Baru',
      generate_recovery_phrase_caption: 'Buat kunci pemulihan baru',
      import_existing_wallet: 'Impor Dompet yang Ada',
      paste_recovery_phrase: 'Tempel kunci pemulihan Anda',
      save_recovery_phrase_heading: 'Simpan Kunci Pemulihan Anda',
      save_recovery_phrase_btn: 'Simpan Kunci Pemulihan',
      recovery_phrase_write_down: 'Tulis 12 kata ini secara berurutan dan simpan di tempat yang aman.',
      acknowledgment_text:
        'Saya telah menyimpan kunci pemulihan saya dan memahami bahwa kehilangannya akan mengakibatkan hilangnya semua dana, token, dan sertifikat terkait secara total dan permanen.',
      enter_recovery_words: 'Masukkan frasa 12 kata',
      restore_wallet_description: 'Masukkan kunci pemulihan untuk memulihkan dompet Anda',
      backup_reminder_title: 'Dompet Anda Belum Dicadangkan',
      backup_reminder_body: 'Ekspor kunci Anda agar dana tidak hilang.',
      biometric_advisory_title: 'Amankan Dompet Anda',
      wallet_balance_show_in: 'Tampilkan dalam {{unit}}',
      tx_detail_block: 'Blok',
      tx_detail_block_pending: 'Menunggu blok',
      biometric_advisory_body: 'Lindungi aset Anda dengan Face ID atau sidik jari Anda',
      biometric_advisory_body_degraded:
        'Perangkat ini belum mengatur Face ID atau sidik jari, jadi kunci dompet Anda hanya dilindungi oleh kode sandi perangkat.',
      import_prompt_title: 'Sudah punya dompet?',
      import_prompt_body: 'Pulihkan dari kunci pemulihan atau bagian cadangan Anda.',
      import_prompt_action: 'Impor dari cadangan',
      nearby_advisory_title: 'Menghubungkan Terdekat',
      nearby_advisory_body:
        'Anda akan diminta mengizinkan akses Wi-Fi, Bluetooth, atau NFC. Ini hanya dipakai untuk menghubungkan Anda langsung ke perangkat terdekat demi pembayaran — tidak pernah dikirim ke server kami, dan tidak pernah dipakai untuk membuat profil tentang Anda.',
      backup_reminder_now: 'Cadangkan kunci sekarang',
      backup_reminder_import: 'Impor dari cadangan',
      restore_backup_checking: 'Mencari cadangan terenkripsi Anda…',
      restore_backup_progress: 'Memulihkan riwayat dompet — {{chunks}} dari {{total}}…',
      restore_backup_failed_title: 'Riwayat tidak dapat dipulihkan',
      restore_backup_failed_message:
        'Frasa pemulihan Anda benar, tetapi cadangan terenkripsi tidak dapat diputar ulang. Biasanya cukup dicoba lagi. Melanjutkan tanpanya memberi dompet yang berfungsi tanpa transaksi lama, dan sebagian koin Anda yang ada tidak akan dapat dibelanjakan.',
      restore_backup_retry: 'Coba lagi',
      restore_backup_skip: 'Tinggalkan',
      restore_backup_unverified_title: 'Cadangan mungkin belum lengkap',
      restore_backup_unverified_body:
        'Dompet Anda telah dipulihkan, tetapi cadangan di server tidak dapat dipastikan lengkap. Jika ada riwayat yang hilang, pulihkan lagi nanti setelah perangkat lain selesai mengunggah.',
      import_wallet: 'Impor Dompet',

      reject: 'Tolak',
      authorize: 'Otorisasi',
      renewal: 'Pembaruan',
      spending_section: 'Pengeluaran',
      protocols_section: 'Protokol',
      baskets_section: 'Keranjang',
      certificates_section: 'Sertifikat',
      requested_fields: 'Kolom yang diminta',

      send_max: 'Kirim Maksimal',
      message_box_unreachable: 'Tidak dapat menghubungi server pembayaran. Periksa URL message box di pengaturan.',
      available: 'tersedia',
      note: 'Catatan',
      note_placeholder: 'Untuk apa? (opsional)',
      entire_wallet_balance: 'Seluruh saldo dompet',
      entire_vault_balance: 'Seluruh saldo brankas',
      export_wallet_data: 'Ekspor Data Dompet',
      export_unencrypted_title: 'File Ini Tidak Terenkripsi',
      export_unencrypted_message:
        'File yang diekspor adalah salinan data dompet Anda yang polos dan tidak terenkripsi — termasuk informasi identitas dan sertifikat, riwayat transaksi, dan kontak. Siapa pun yang mendapatkan file ini dapat membacanya. Bagikan hanya ke tempat yang Anda percayai.',
      import_wallet_data: 'Impor Data Dompet',
      import_conflict_title: 'Basis Data Sudah Ada',
      import_conflict_message:
        'Basis data dompet untuk identitas ini sudah ada dengan timestamp yang lebih baru. Cadangan dengan angka tertinggi di nama file selalu digunakan.\n\nUntuk menggunakan cadangan lama ini: ekspor data dompet Anda saat ini, hapus dan instal ulang aplikasi, lalu impor hanya cadangan yang diinginkan.',
      import_confirm_title: 'Impor Basis Data',
      import_confirm_message: 'Basis data ini akan menjadi dompet aktif Anda. Lanjutkan?',
      import_success: 'Basis data berhasil diimpor. Memulai ulang dompet...',
      import_invalid_file: 'File Cadangan Tidak Valid',
      import_invalid_file_detail:
        'File ini bukan cadangan basis data dompet yang valid. Nama file yang diharapkan seperti wallet-XXXXXXXX-mainnet-TIMESTAMP.db.',
      import_oversized_file: 'File Cadangan Terlalu Besar',
      import_oversized_file_detail:
        'File ini lebih besar dari yang seharusnya untuk cadangan dompet, sehingga tidak dibaca. Pilih file cadangan basis data dompet yang asli.',
      import_untrusted_file: 'Tidak dapat mengimpor cadangan ini',
      import_untrusted_file_detail:
        'Cadangan ini tidak dapat diverifikasi sebagai milik dompet ini, atau isinya tidak dapat dipercaya. Cadangan ini tidak diimpor.',
      import_anyway: 'Tetap Impor',
      import_invalid_input_title: 'Masukan Tidak Valid',
      import_invalid_input_message:
        'Masukkan frasa pemulihan yang valid (12–24 kata) atau kunci pribadi heksadesimal 64 karakter.',
      import_setup_failed: 'Gagal menyiapkan dompet: {{error}}',
      create_wallet_refused: 'Tidak dapat membuat dompet. Silakan coba lagi.',
      create_wallet_failed: 'Gagal membuat mnemonic. Silakan coba lagi.',
      or: 'atau',
      print_recovery_shares: 'Cetak Bagian Pemulihan',
      scan_backup_shares: 'Pindai Bagian Cadangan',
      scan_backup_shares_caption: 'Pulihkan dompet dari bagian yang dicetak',
      scan_shares_invalid_format: 'Format bagian tidak valid. Pindai kode QR bagian cadangan yang valid.',
      scan_shares_camera_needed: 'Akses Kamera Diperlukan',
      scan_shares_camera_description: 'Akses kamera diperlukan untuk memindai kode QR bagian cadangan yang dicetak.',
      scan_shares_grant_camera: 'Berikan Akses Kamera',
      scan_shares_recovering: 'Memulihkan dompet...',
      scan_shares_scan_first: 'Pindai bagian cadangan pertama Anda',
      scan_shares_progress: '{{scanned}} dari {{needed}} bagian dipindai',
      scan_shares_remaining: '{{count}} bagian lagi diperlukan',
      scan_shares_complete: 'Semua bagian terkumpul!',
      scan_shares_recovery_failed: 'Pemulihan gagal. Silakan coba pindai lagi.',
      scan_shares_threshold_mismatch: 'Ambang batas tidak cocok dengan bagian sebelumnya',
      scan_shares_integrity_mismatch: 'Hash integritas tidak cocok — bagian berasal dari kunci yang berbeda',
      scan_shares_duplicate: 'Bagian ini sudah dipindai',
      scan_shares_biometric_title: 'Akses Biometrik Diperlukan',
      scan_shares_biometric_message:
        'Akses biometrik diperlukan untuk melindungi kunci dompet Anda. Silakan coba lagi.',
      recovery_replace_wallet_title: 'Ganti dompet di ponsel ini?',
      recovery_replace_wallet_body:
        'Ponsel ini sudah menyimpan sebuah dompet. Melanjutkan akan menggantinya beserta semua yang bisa dibelanjakannya. Jika frasa pemulihannya tidak disimpan di tempat lain, uang itu akan hilang selamanya.',
      recovery_replace_wallet_confirm: 'Ganti dompet',
      invalid_bsv_address: 'Alamat BSV tidak valid',
      scan_recipient_hint: 'Arahkan kamera ke kode pembayaran',
      scan_qr_default_hint: 'Arahkan kamera ke kode QR',
      scan_wallet_qr_hint: 'Arahkan kamera ke kode QR pemasangan dompet',

      biometric_store_wallet: 'Simpan dompet Anda dengan aman',
      biometric_load_wallet: 'Muat dompet Anda dengan aman',
      biometric_unlock_wallet: 'Buka kunci dompet Anda',
      wallet_lost_title: 'Kunci dompet dimusnahkan',
      wallet_lost_body:
        'Pengaturan biometrik perangkat ini berubah, sehingga sistem memusnahkan kunci yang melindungi dompet ini. Koin Anda aman di blockchain — pulihkan dompet ini dengan frasa pemulihan Anda.',
      wallet_lost_restore: 'Pulihkan',
      wallet_lost_later: 'Nanti saja',
      wallet_locked_title: 'Dompet terkunci',
      wallet_locked_body: 'Buka kunci untuk memuat kunci dompet Anda.',
      wallet_unlock_retry: 'Buka kunci dompet',
      wallet_lockout_body: 'Terlalu banyak percobaan gagal. Buka kunci ponsel Anda dengan kode sandi, lalu coba lagi.',
      wallet_no_biometrics_body:
        'Perangkat ini tidak memiliki biometrik. Kunci dompet Anda tetap dienkripsi oleh keystore perangkat, tetapi tidak dilindungi pemeriksaan biometrik.',

      // Connections screen
      connections: 'Koneksi',
      advanced: 'Lanjutan',
      copy_secret_words: 'Salin Kata Rahasia',
      print_recovery_keys: 'Cetak Kunci Pemulihan',
      backup_wallet_keys: 'Cadangkan Kunci Dompet',
      log_out: 'Keluar',
      connect_to_app: 'Hubungkan ke Aplikasi',
      no_connections_yet: 'Belum ada koneksi',
      no_connections_subtitle: 'Pindai kode QR dari aplikasi desktop untuk menghubungkan dompet Anda',
      connections_active: 'Aktif',
      connections_disconnected: 'Terputus',
      disconnect: 'Putuskan',
      reconnect: 'Hubungkan Kembali',
      reconnect_failed: 'Gagal Menghubungkan Kembali',
      reconnect_confirm_title: 'Sambungkan kembali ke situs ini?',
      reconnect_confirm_message: '{{origin}} akan dapat mengakses dompet Anda lagi.',
      reconnect_needs_reapproval: 'Koneksi ini tidak dapat diverifikasi. Pindai kode QR baru untuk menghubungkan kembali.',
      scan_qr_code: 'Pindai Kode QR',
      paste_uri: 'Tempel URI',
      invalid_qr_code: 'Kode QR Tidak Valid',
      wallet_not_ready: 'Dompet belum siap',
      please_log_in_first: 'Silakan masuk terlebih dahulu',
      unknown_error: 'Kesalahan tidak diketahui',

      // Pay screen
      payments: 'Pembayaran',
      pay: 'Bayar',
      pay_direction_pay: 'Bayar',
      pay_direction_receive: 'Terima bayaran',
      pay_cell_nearby_pay: 'Seseorang di dekat Anda',
      pay_cell_nearby_pay_sub: 'Pindai kodenya',
      pay_cell_handle_pay: 'Seseorang dari jauh',
      pay_cell_handle_pay_sub: 'Pilih nama — mereka perlu aplikasi ini',
      pay_cell_address_pay: 'Ke alamat BSV',
      pay_cell_address_pay_sub: 'Tempel atau pindai alamat BSV',
      pay_cell_nearby_get: 'Seseorang di dekat Anda',
      pay_cell_nearby_get_sub: 'Tunjukkan kode pembayaran Anda',
      pay_cell_handle_get: 'Seseorang dari jauh',
      pay_cell_handle_get_sub: 'Bagikan nama Anda',
      pay_cell_address_get: 'Ke alamat BSV',
      pay_cell_address_get_sub: 'Tunjukkan alamat BSV',
      pay_method: 'Metode',
      pay_method_nearby: 'Terdekat',
      pay_method_remote_link: 'Bagikan tautan jarak jauh',
      pay_method_address: 'Ke alamat BSV',
      pay_pre_nearby: 'Anda berdua ada di sini.',
      pay_pre_handle: 'Dia memakai aplikasi ini.',
      pay_pre_address: 'Anda punya alamat BSV-nya.',
      pay_conseq_nearby: 'Selesai dalam beberapa detik.',
      pay_conseq_handle: 'Masuk saat dompetnya memeriksa lagi.',
      pay_conseq_address: 'Uang langsung terkirim — penerima tidak diberi tahu.',
      paid: 'Dibayar',
      pay_inbox_attention: 'Tidak bisa ditambahkan',
      pay_inbox_attention_hint:
        'Ini sudah masuk tetapi dompet Anda tidak bisa menambahkannya. Coba lagi, atau abaikan untuk melepaskannya selamanya.',
      pay_dismiss_confirm: 'Ketuk lagi untuk melepaskan',
      pay_dismissed: 'Pembayaran dilepaskan',
      payment_arrived: 'Pembayaran diterima',
      payment_arrived_damaged: 'Pembayaran ini tiba dalam keadaan rusak',
      discard_payment_title: 'Buang pembayaran ini?',
      discard_payment_body:
        'Pengirim akan diminta mengirim detailnya lagi. Dananya tetap ada di rantai sampai itu terjadi.',
      discard_void_title: 'Pembayaran ini tidak berhasil',
      discard_void_body: 'Pembayaran dibatalkan atau dananya dibelanjakan di tempat lain. Aman untuk diabaikan.',
      waiting_network_confirm: 'Menunggu jaringan mengonfirmasi pembayaran ini',
      finish_or_cancel_outgoing: 'Selesaikan atau batalkan pembayaran di bawah sebelum mengirim yang baru',
      identity_search_unavailable: 'Pencarian sedang tidak tersedia. Coba lagi sebentar lagi.',
      abandon_payment: 'Tinggalkan pembayaran',
      finish_payment: 'Selesaikan pembayaran',
      cancel_this_payment: 'Batalkan pembayaran ini?',
      cancel_payment: 'Batalkan pembayaran',
      resend: 'Kirim ulang',
      resending: 'Mengirim ulang…',
      send_payment_details_again: 'Kirim detailnya lagi',
      tx_action_resend_short: 'Kirim ulang',
      tx_action_request_confirmation: 'Minta konfirmasi dari penerima',
      resend_sent: 'Detail pembayaran dikirim ulang',
      resend_failed_no_record: 'Detail pembayaran ini tidak ada di perangkat ini, jadi tidak bisa dikirim ulang.',
      resend_failed_no_recipient: 'Pembayaran ini tidak mencatat penerimanya, jadi tidak bisa dikirim ulang.',
      resend_failed_no_transaction: 'Tidak bisa mengambil data pembayaran ini. Coba lagi saat online.',
      resend_requested: 'Mereka meminta Anda mengirim ulang sebuah pembayaran',
      request_again: 'Minta lagi',
      send_again: 'Kirim lagi',
      copy_details: 'Salin detail',
      payment_bounced_resend: 'Mereka diminta mengirim pembayaran ini lagi',
      dismiss_rejected_payment: 'Pembayaran yang ditolak diabaikan',
      activity_load_failed: 'Tidak dapat memuat aktivitas',
      activity_load_retry: 'Ketuk untuk mencoba lagi',
      activity_view_contact: 'Lihat kontak',
      home_payments_need_attention: '{{count}} pembayaran perlu perhatian',
      home_payments_unsent: '{{count}} pembayaran belum terkirim',
      pay_copy: 'Salin',
      pay_share_link: 'Bagikan tautan',
      pay_address_watching: 'Uang yang dikirim ke sini otomatis ditambahkan ke dompet Anda.',
      pay_address_earlier_day: 'Buka hari sebelumnya',
      pay_address_sweep_now: 'Periksa alamat BSV ini sekarang',
      pay_address_swept: 'Diterima {{amount}}',

      check_wallet: 'Pemecahan masalah',
      settings_version: 'v{{version}}',
      error_review_actions: 'Pembayaran ini macet. Pemecahan masalah dapat mencoba melepaskannya.',
      wallet_check_ok: 'Semuanya terlihat baik',
      wallet_check_attention: 'Beberapa pemeriksaan perlu perhatian Anda',
      wallet_check_couldnt: 'Tidak dapat memeriksa',
      wallet_check_couldnt_body:
        'Satu langkah perbaikan tidak selesai. Tidak ada asumsi apa pun yang dibuat tentang koin Anda.',
      wallet_check_summary: '{{freed}} koin macet dilepaskan · {{recovered}} pembayaran dipulihkan',
      wallet_check_step_coins: 'Koin Anda valid',
      wallet_check_skip: 'Lewati pemeriksaan ini',
      wallet_check_skipped: 'Dilewati',
      wallet_check_step_proofs: 'Bukti transaksi',
      wallet_check_step_records: 'Catatan basis data lokal',
      wallet_check_step_tokens: 'Transfer stablecoin terverifikasi',
      wallet_check_step_online: 'Koneksi internet',
      wallet_check_step_missed: 'Pembayaran yang terlewat',
      wallet_check_step_backup: 'Cadangan pribadi terunggah',
      wallet_check_step_phrase: 'Frasa pemulihan tersimpan',
      wallet_check_progress: '{{current}} dari {{total}}',
      wallet_check_stuck_title: 'Tidak dapat mengirim pembayaran ini',
      wallet_check_stuck_message: 'Beberapa koin tampak macet. Pemecahan masalah dapat melepaskannya.',
      wallet_check_failed: 'Pemeriksaan tidak selesai. Coba lagi.',

      // Local payments
      local_pay_request: 'Minta Pembayaran',
      local_pay_send: 'Kirim Pembayaran',
      local_pay_amount: 'Jumlah',
      local_pay_show_qr: 'Tunjukkan kode ini kepada pembayar',
      local_pay_scan_qr: 'Pindai kode penerima',
      local_pay_scan_payer_qr: 'Pindai kode pembayar',
      local_pay_scan_progress: '{{have}} dari {{total}} bagian',
      local_pay_show_payment_qr: 'Tunjukkan kode ini kepada penerima',
      local_pay_animated_hint:
        'Minta mereka memindai kode animasi di bawah — kode terus berubah sampai ponsel mereka menerimanya utuh.',
      local_pay_show_qr_instead: 'Tampilkan kode QR saja',
      local_pay_preparing: 'Menyiapkan permintaan…',
      local_pay_delivering: 'Mengirim pembayaran…',
      local_pay_saving: 'Menyimpan pembayaran…',
      local_pay_sent: 'Pembayaran terkirim',
      local_pay_received: 'Pembayaran diterima',
      local_pay_added: 'Ditambahkan ke dompet Anda.',
      local_pay_added_multiple: '{{count}} pembayaran ditambahkan ke dompet Anda.',
      local_pay_received_confirming: 'Mengonfirmasi dengan dompet Anda…',
      local_pay_received_not_credited:
        'Belum ditambahkan ke dompet Anda. Periksa Aktivitas sebelum menyerahkan apa pun.',
      local_pay_decline_broadcast_title: 'Pembayaran yang ditolak ternyata tersiar',
      local_pay_decline_broadcast_body:
        'Pembayaran yang sebelumnya ditolak kini muncul di jaringan. Jangan kirim lagi — periksa Aktivitas.',
      local_pay_pending_abort_dropped_title: 'Permintaan pelepasan yang tidak terverifikasi diabaikan',
      local_pay_pending_abort_dropped_body:
        'Catatan pelepasan pembayaran yang tersimpan tidak dapat diverifikasi, sehingga tidak ada yang dilepaskan. Jika pembayaran masih terlihat macet, batalkan dari Aktivitas.',
      local_pay_cancel_unverifiable_title: 'Tidak dapat memeriksa pembayaran ini saat offline',
      local_pay_cancel_unverifiable_body:
        'Jika orang lain sudah memindainya, membatalkan akan memungkinkan koin ini dibelanjakan dua kali. Batalkan hanya jika Anda yakin itu belum pernah dipindai.',
      local_pay_cancel_unverifiable_confirm: 'Batalkan saja',
      local_pay_queued: 'Tersimpan — akan ditambahkan ke dompet Anda secara otomatis.',
      local_pay_already_paid: 'Permintaan ini sudah dibayar.',
      local_pay_failed: 'Pembayaran gagal',
      local_pay_network_denied:
        'Akses Jaringan Lokal nonaktif. Aktifkan di Pengaturan untuk membayar perangkat di dekat Anda.',
      local_pay_radio_fallback:
        'Tautan nirkabel tidak tersedia — tunjukkan kode ini ke perangkat lain sebagai gantinya.',
      local_pay_floor_peer_bt_off: 'Bluetooth mereka mati, jadi pembayaran ini akan dilakukan lewat kode.',
      local_pay_floor_local_bt_off: 'Bluetooth Anda mati. Nyalakan untuk tautan terdekat yang lebih cepat.',
      local_pay_floor_local_ble_denied:
        'Akses Bluetooth untuk BSV Wallet nonaktif. Izinkan di Pengaturan untuk tautan terdekat yang lebih cepat.',
      local_pay_floor_cross_os:
        'Perangkat lain tidak dapat menggunakan Bluetooth saat ini, jadi pembayaran ini akan dilakukan lewat kode.',
      local_pay_floor_peer_no_radio:
        'Perangkat lain tidak menunggu tautan nirkabel, jadi pembayaran ini akan dilakukan lewat kode.',
      local_pay_ble_unavailable: 'Bluetooth tidak tersedia. Kode di atas tetap berfungsi.',
      local_pay_too_large: 'Data pembayaran ini terlalu besar untuk dikirim melalui kode QR.',
      local_pay_nearby_unavailable: 'Wi-Fi terdekat tidak tersedia. Kode di atas tetap berfungsi.',
      local_pay_retry_save: 'Coba simpan lagi',
      local_pay_wrong_session:
        'Pembayaran itu tidak cocok dengan permintaan ini dan tidak diterima. Minta pembayar memindai kodenya lagi.',
      local_pay_declined_mismatch:
        'Penerima tidak menerima pembayaran ini — tidak cocok dengan permintaannya. Pindai kodenya lagi.',
      local_pay_declined_already_paid: 'Permintaan itu sudah dibayar. Tidak ada yang dikirim.',
      local_pay_declined_save: 'Penerima tidak dapat menyimpan pembayaran. Tidak ada yang dikirim — coba lagi.',
      local_pay_declined_decode: 'Penerima tidak dapat membaca pembayaran. Tidak ada yang dikirim — coba lagi.',
      local_pay_declined_not_covered:
        'Penerima tidak dapat memverifikasi pembayaran ini dengan catatan penerbit. Tidak ada yang dikirim.',
      local_pay_broadcast_pending: 'Terkirim. Perangkat ini belum bisa memprosesnya — penerima yang akan melakukannya.',
      local_pay_any_amount: 'Jumlah bebas',
      local_pay_choose_amount: 'Pilih jumlah',
      local_pay_enter_amount_send: 'Berapa jumlah yang ingin Anda kirim?',
      local_pay_qr_unavailable: 'Kode ini tidak dapat dibuat. Mulai permintaan lagi.',
      local_pay_presence_qr: 'Lewat kode — tanpa koneksi langsung',
      local_pay_presence_ready: 'Koneksi terenkripsi di dekat Anda siap',
      local_pay_presence_waiting_payee: 'Menunggu perangkat di dekat Anda',
      local_pay_presence_waiting_payer: 'Mencari perangkat penerima…',
      local_pay_presence_linked: 'Koneksi terenkripsi terbuka',
      local_pay_presence_paid: 'Pembayaran dikonfirmasi',
      pay_offline_title: 'Anda sedang offline',
      pay_offline_body:
        'Saat ini hanya pembayaran di dekat Anda yang berfungsi. Pembayaran akan diproses begitu Anda tersambung kembali.',
      pay_offline_queued: '{{count}} pembayaran menunggu diproses begitu Anda tersambung kembali.',
      pay_offline_pending_title: 'Menunggu diproses',
      pay_offline_pending_body:
        '{{count}} pembayaran belum diproses. Belum ada yang selesai sampai itu terjadi; percobaan akan terus dilakukan.',
      pay_offline_needs_internet: 'Perlu internet',
      pay_offline_rejected_title: 'Sebuah pembayaran ditolak',
      pay_offline_rejected_body:
        'Jaringan menolak pembayaran yang diserahkan oleh {{sender}} melalui {{via}} pada {{when}}.',
      pay_offline_unknown_sender: 'pengirim tidak dikenal',
      pay_offline_unknown_via: 'metode yang tidak diketahui',
      pay_received_not_broadcast: 'Diterima secara offline · belum diproses',
      pay_offline_sent_rejected_title: 'Sebuah pembayaran tidak dapat dikirim',
      pay_offline_sent_rejected_body: 'Jaringan menolak pembayaran yang Anda kirim pada {{when}}.',
      pay_offline_send_now: 'Kirim sekarang',
      pay_offline_stalled_body: 'Beberapa pembayaran dalam antrean tidak dapat dikirim secara otomatis: {{detail}}',
      offline_stall_no_request: 'Sebuah pembayaran dalam antrean kehilangan catatan pengirimannya.',
      offline_stall_bad_beef: 'Data transaksi sebuah pembayaran dalam antrean tidak dapat dibaca.',
      offline_stall_foreign_ancestor:
        'Sebuah pembayaran dalam antrean menunggu transaksi induk yang tidak pernah diproses oleh dompet lain.',
      pay_offline_kv_pending: '{{count}} pembayaran terdekat menunggu untuk ditambahkan ke dompet ini.',
      pay_offline_kv_stuck: '{{count}} pembayaran terdekat tidak bisa ditambahkan. Minta pengirim mengirim ulang.',
      pay_offline_kv_corrupt: 'Ditemukan data pembayaran yang rusak di perangkat ini.',
      pay_offline_kv_corrupt_body:
        'Pembayaran yang terdampak disimpan terpisah di perangkat ini, bukan dihapus. Sebagian mungkin sudah masuk ke saldo Anda, jadi minta pengirim mengirim ulang hanya jika pembayarannya masih belum ada.',
      pay_offline_show_code: 'Tampilkan kode lagi',
      pay_parked_cancel: 'Batalkan pembayaran',
      pay_parked_already_sent: 'Pembayaran ini sudah terkirim',
      token_cancel_already_sent: 'Pembayaran ini sudah berhasil dan tidak dapat dibatalkan.',
      tx_status_parked: 'Belum diserahkan',
      tx_status_import_hold: 'Ditahan (diimpor)',
      resend_sending: 'Mengirim lewat kotak pesan',
      cancelling_payment: 'Membatalkan pembayaran',

      contacts: 'Kontak',
      contacts_search_placeholder: 'Cari berdasarkan nama atau handle',
      contacts_empty_title: 'Belum ada kontak',
      contacts_empty_body:
        'Orang yang Anda bayar akan muncul di sini setelah disimpan. Pindai kode profil seseorang dengan tombol di kanan atas.',
      contacts_recent: 'Terbaru',
      contact_edit_name: 'Edit nama',
      contact_save_name: 'Simpan nama',
      contact_activity: 'Aktivitas',
      contact_activity_empty: 'Belum ada aktivitas dengan kontak ini',
      contact_delete: 'Hapus kontak',
      contact_delete_body: 'Ini akan menghapus {{name}} dari kontak Anda. Tidak memengaruhi pembayaran sebelumnya.',
      contact_handle_caption: 'Handle terdaftar mereka · hanya mereka yang bisa mengubahnya',
      contact_new_title: 'Kontak Baru',
      contact_save: 'Simpan',
      contact_identifier: 'Identifier',
      contact_saved: 'Kontak disimpan',
      contact_scan_hint: 'Pindai kode profil untuk menambahkan seseorang',
      contact_identifier_placeholder: 'Tempel Identifier mereka',
      contact_identifier_invalid: 'Itu bukan Identifier yang valid',
      contact_add_from_payment: 'Tambahkan ke kontak',

      profile: 'Profil',
      profile_display_name: 'Nama tampilan',
      profile_display_name_hint:
        'Ditampilkan secara publik di samping handle Anda. Siapa pun yang mencari Anda bisa melihatnya.',
      profile_handle: 'Handle',
      profile_handle_placeholder: 'namaanda',
      profile_handle_checking: 'Memeriksa ketersediaan…',
      profile_handle_available: '{{handle}} tersedia',
      profile_handle_taken: '{{handle}} sudah digunakan',
      profile_handle_invalid:
        '3-32 karakter: a-z, 0-9, titik, garis bawah, atau tanda hubung; diawali dan diakhiri huruf atau angka',
      profile_handle_failed: 'Tidak dapat memeriksa ketersediaan. Coba lagi.',
      profile_handle_unavailable: 'Pendaftaran handle belum tersedia di build ini.',
      profile_handle_registered: 'Handle terdaftar',
      profile_handle_register_action: 'Daftar',
      profile_handle_too_similar: '{{handle}} terlalu mirip dengan yang sudah dipakai',
      profile_handle_reserved: '{{handle}} sudah dicadangkan',
      profile_handle_cooldown: '{{handle}} baru saja dilepas dan belum bisa dipakai',
      profile_handle_pending: 'Menyelesaikan pendaftaran…',
      profile_handle_rolled_back: '{{handle}} sudah diambil. Anda tetap memakai {{previous}}.',
      profile_handle_changed: 'Handle diubah',
      profile_handle_rejected: 'Registri menolak handle itu.',
      profile_handle_clock_ahead: 'Jam perangkat Anda terlalu maju. Coba lagi beberapa menit lagi.',
      profile_handle_wrong_registry: 'Handle itu tidak ada di registri ini. Buka Profil lagi untuk menyegarkannya.',
      profile_display_name_publish_failed: 'Nama Anda tersimpan di sini, tetapi registri tidak memublikasikannya.',
      profile_qr_hint: 'Orang lain dapat memindai ini untuk menambahkan Anda sebagai kontak',

      pay_step_contacts: 'Kontak',
      pay_step_continue: 'Lanjutkan',
      pay_review_to: 'Kepada',
      pay_review_amount: 'Jumlah',
      pay_review_note: 'Catatan',
      pay_review_note_edit: 'Tambahkan catatan',
      pay_review_delivery_host: 'Mengirim ke',
      pay_trust_contact: 'Kontak Anda',
      pay_trust_handle_attested: 'Terdaftar',
      pay_trust_handle_domain_attested: 'Diverifikasi oleh {{domain}}',
      pay_trust_unverified: 'Belum diverifikasi. Periksa kembali Identifier ini.',
      pay_address_network_mismatch: 'Alamat ini berasal dari jaringan yang berbeda dari yang sedang Anda gunakan.',
      contacts_your_contacts: 'Kontak Anda',
      contacts_other_people: 'Orang lain',
      contacts_not_saved: 'Belum disimpan',
      contacts_network_footer: 'Ditemukan di jaringan. Belum Anda simpan.',
      contact_no_handle: 'Belum ada handle',
      contact_no_handle_registered: 'Tidak ada handle terdaftar',
      contact_name_section: 'Nama Anda untuk mereka',
      contact_name_hint: 'Hanya Anda yang melihatnya. Tidak mengubah sebutan mereka untuk diri sendiri.',
      contact_new_name_hint: 'Hanya Anda yang melihatnya. Disimpan di perangkat ini.',
      contact_copy_identifier: 'Salin Identifier',
      profile_handle_registered_hint: 'Terdaftar pada Identifier Anda. Orang bisa menemukan Anda lewat itu.',
      profile_handle_change: 'Ubah',
      profile_handle_claim: 'Klaim {{handle}}',
      profile_handle_replace_warning:
        'Mengklaim akan mengganti {{handle}}. Orang yang menyimpan Anda tetap punya kontaknya; handle lama tidak lagi mengarah ke Anda.',
      profile_identifier_section: 'Identifier Anda',
      profile_show_qr: 'Tampilkan sebagai QR',
      profile_show_qr_hint: 'Biarkan seseorang memindai Anda untuk mendapatkan profil Anda',
      profile_qr_callout:
        'Memindai ini membuka BSV Wallet dan menambahkan Anda sebagai kontak, dengan nama, handle, dan foto Anda. Ini tidak meminta pembayaran — kode itu ada di Terima bayaran.',
      pay_review_title: 'Tinjau',
      pay_send_amount: 'Kirim {{amount}}',
      pay_sent_to: 'ke {{name}}',
      pay_save_as_contact: 'Simpan sebagai kontak'
    }
  },
  ja: {
    translation: {
      token_balances_header: '残高',
      wallet_coin_switcher: '表示するコインを選択',
      wallet_activity_empty_asset: '{{ticker}} の履歴はまだありません',
      pay_direction_pay_asset: '{{ticker}} で支払う',
      token_fee_footer: '送金にはネットワーク手数料として少額の BSV が必要です。受け取りには不要です。',
      token_new_tap: '新規 — 発行者を見るにはタップ',
      token_unresolved: '不明なトークン',
      token_unresolved_sub: 'このウォレットは発行者を照会できませんでした。オンラインのときに再試行してください。',
      token_paused_short: '送金は一時停止中',
      token_frozen_short: '{{amount}} 凍結中',
      token_sheet_issuer: '発行者',
      token_sheet_issuer_unknown: '未確認',
      token_sheet_asset_id: 'トークン ID',
      token_sheet_powers_header: '{{issuer}} にできること',
      token_power_pause: '{{ticker}} のすべての送金を停止する',
      token_power_freeze: '特定のコインを凍結する（あなたのものも含む）',
      token_power_admit: '{{ticker}} を保有できる人を決める',
      token_power_replace: '凍結されたコインを新しいものに置き換える',
      token_powers_limit:
        'あなたなしで {{ticker}} をウォレットから動かすことはできず、凍結されたコインは単に使えなくなります。',
      token_frozen_sub:
        '{{issuer}} があなたの残高のうち {{amount}} {{ticker}} を凍結しました。異議がある場合は連絡してください。',
      pay_asset_label: '支払いに使う',
      pay_asset_label_get: '受け取る通貨',
      pay_asset_cta: '{{amount}} {{ticker}} を送る',
      pay_asset_over_balance: '{{ticker}} の残高を超えています。',
      pay_asset_needs_bsv: '{{ticker}} の送金にはネットワーク手数料として少額の BSV が必要です。受け取りには不要です。',
      pay_asset_get_bsv: 'BSV を入手',
      pay_asset_needs_bsv_short: '手数料用の BSV が必要',
      pay_asset_frozen_note: '残高のうち {{amount}} {{ticker}} は凍結されており送金できません。',
      pay_asset_paused: '{{issuer}} が {{ticker}} の送金を停止しているため、現在は送れません。',
      pay_asset_no_address: '{{ticker}} は人または近くのデバイスにのみ送れます。ビットコインアドレスには送れません。',
      pay_asset_address_status: '{{ticker}} では利用できません',
      pay_asset_self_unregistered:
        '{{issuer}} はあなたを {{ticker}} の送金者として登録していません。受け取りは引き続き可能です。',
      pay_asset_unidentified: 'このウォレットはこのトークンを識別できないため、まだ送金できません。',
      pay_asset_recipient_unregistered:
        '{{issuer}} はこの相手を {{ticker}} 用に登録していないため、この通貨では支払えません。',
      pay_asset_recipient_blocked: '{{issuer}} はこの相手への支払いを許可していません。',
      token_err_refused: '{{issuer}} がこの送金を拒否しました。何も送られておらず、{{ticker}} の残高は変わりません。',
      token_err_refused_paused: '{{issuer}} が {{ticker}} を停止しました。何も送られておらず、残高は変わりません。',
      token_err_refused_recipient:
        '{{issuer}} はこの相手への支払いを許可していません。何も送られておらず、残高は変わりません。',
      token_err_refused_frozen:
        '送金しようとしていた {{ticker}} の一部が凍結されました。何も送られておらず、残高を更新しました。',
      token_err_send_failed: 'この支払いを送信できませんでした：{{reason}}。ウォレットからは何も出ていません。',
      token_err_reason_unknown: '不明な問題',
      token_err_unreachable:
        'この送金を確認するため {{issuer}} に接続できませんでした。再試行の前に {{ticker}} の残高を確認してください。',
      token_err_check_again: 'もう一度確認',
      token_err_balance_changed: '準備中に {{ticker}} の残高が変わりました。金額を確認して再試行してください。',
      token_err_busy: '{{ticker}} の支払いがすでに送信中です。',
      pay_sent_not_broadcast: '送信しました。まだネットワークに届いていません — 再試行を続けます。',
      pay_sent_not_notified:
        '送信しました。相手にはまだ通知できていません — 相手のウォレットが次に確認したときに表示されます。',
      pay_sent_handed_to_wallet: '送信しました。対面で受け取られなかったため、相手のウォレットへ送りました。',
      token_first_hold: '{{ticker}} は {{issuer}} が発行しており、送金の停止やコインの凍結ができます。',
      pay_asset_request_paused: '{{issuer}} が {{ticker}} を停止しているため、支払いはまだ届かないことがあります。',
      token_admit_title: 'まだ {{ticker}} で受け取ることはできません',
      token_admit_body:
        '{{issuer}} は登録済みの相手にしか {{ticker}} を送りません。それまでは、あなた宛ての支払いは送金者のウォレットを出る前に拒否されます。',
      token_admit_bsv_instead: '代わりに BSV で受け取る',
      token_row_sent: '{{ticker}} を送金',
      token_row_received: '{{ticker}} を受取',
      token_row_amount_pending: '金額を取得できません',
      token_evicted_title: '一部の {{ticker}} が引き揚げられました',
      token_evicted_body: '{{issuer}} があなたのウォレットから {{amount}} {{ticker}} を引き揚げました。',
      local_pay_token_not_cleared: 'オフラインで受取 · {{issuer}} は未確認',
      pay_asset_nearby_needs_network:
        '対面で {{ticker}} を支払うには接続が必要です。接続がなくても BSV なら支払えます。',
      pay_asset_link_not_held:
        'このリクエストは、このウォレットが保有していないトークン向けです。BSV での支払いは引き続き可能です。',
      pay_asset_nearby_wrong_overlay: 'このリクエストは、このウォレットが対応していないトークン向けです。',
      token_messagebox_mismatch:
        'ウォレットが別のメッセージボックスに設定されているため、トークンの支払いが届かない場合があります。',
      token_issuer_fallback: '発行者',
      token_row_hint: '発行者とその権限を開きます',
      token_attention_one: '{{issuer}} の確認待ち — タップして詳細',
      token_attention_many: '{{count}} 件のトークン支払いが決済待ちです — タップして詳細',
      token_recv_reversed: 'この支払いは {{issuer}} に確認されず、取り消されました。',
      token_settle_paused_retry: '現在 {{issuer}} が {{ticker}} を停止しています — 再試行を続けます。',
      token_settle_unreachable_retry: 'この送金を確認するため {{issuer}} に接続できませんでした。再試行を続けます。',
      token_settle_evicted:
        'その支払いは期限内にネットワークへ届かず取り消されました — {{ticker}} は残高に戻っています。',
      token_frame_unreadable: '受け取った支払いを読み取れませんでした — 送金者に再送を依頼してください。',
      token_chain_reversed: 'この連鎖の前の支払いが {{issuer}} に拒否されたため、この支払いも取り消されました。',
      token_settle_stuck_bound:
        'この支払いは 3 日以上決済を待っています。このまま待つか、{{issuer}} に連絡してください。',
      token_cover_failed: 'この支払いは {{issuer}} の記録と一致しないため、受け付けられませんでした。',
      token_sent_settling: '送信しました。{{issuer}} と決済中 — あなたか {{payee}} が再接続すると確認が表示されます。',
      token_sent_settling_unnamed: '送信しました。{{issuer}} と決済中 — どちらかが再接続すると確認が表示されます。',
      token_sent_settled: '送信済み · {{issuer}} と決済完了',
      token_status_settling: '決済中',
      token_status_settled: '決済済み',
      token_status_refused: '拒否されました',
      token_status_reversed: '取り消されました',
      token_status_stuck: '決済待ち',
      // Browser UI
      new_tab: '新しいタブ',
      untitled: '無題',
      share: '共有',
      back_to_homepage: 'ホームページに戻る',
      bookmarks: 'ブックマーク',
      add_bookmark: 'ブックマークを追加',
      bookmark: 'ブックマーク',
      unbookmark: 'ブックマーク解除',
      find_in_page: 'ページ内検索',
      find_in_page_placeholder: 'ページ内を検索...',
      find_in_page_matches: '{{current}} / {{total}}',
      find_in_page_no_matches: '一致なし',
      history: '履歴',
      clear_all: 'すべて消去',
      clear_all_tabs: 'すべてのタブを閉じる',
      search_or_enter_website: '検索またはURLを入力',
      browser: 'ブラウザ',
      bug_report: 'バグ報告',
      enable_web3: 'Web3を有効化',
      tabs: 'タブ',
      unlock_web3_features: 'Web3機能を解除',

      // General actions
      cancel: 'キャンセル',
      done: '完了',
      discard: '破棄',
      deny: '拒否',
      allow: '許可',
      save: '保存',
      remove: '削除',
      delete: '削除',
      search: '検索',
      loading: '読み込み中...',
      continue: '続行',
      go_back: '戻る',
      continue_action: '続行',
      camera_denied_description:
        'カメラへのアクセスが拒否されました。この機能を使用するには、設定からカメラを有効にしてください。',
      open_settings: '設定を開く',
      copied: 'コピーしました',
      explorer_open_failed: 'ブロックエクスプローラーを開けませんでした',
      tx_action_refresh: '状態を更新',
      tx_action_refresh_short: '状態',
      tx_action_abort: '取引をキャンセル',
      copy: 'コピー',
      copy_to_clipboard: 'クリップボードにコピー',
      details: '詳細',
      hide_details: '詳細を隠す',

      // Navigation
      identity: 'アイデンティティ',
      settings: '設定',
      wallet: 'ウォレット',
      activity: 'アクティビティ',
      configuration: '設定',
      data_and_security: 'データとセキュリティ',
      storage_row: 'ストレージ',
      storage_title: 'ウォレットのストレージ',
      storage_db_size: 'データベース: {{size}}',
      storage_free_space: '空き容量: {{size}}',
      storage_reclaimable: '現在回収可能: {{size}}（{{count}} 件の取引）',
      storage_held_back: '{{count}} 件の取引は使用中のため保持',
      storage_reclaim_explain:
        '回収するとウォレットに不要になった検証データが削除されます。取引・残高・履歴はそのままです。データベースファイル内の空きは増えますが、システムが圧縮するまでファイル自体は小さくなりません。',
      storage_reclaim_confirm: '回収',
      storage_reclaimed: '{{count}} 件の取引を回収しました',
      storage_failed: 'ストレージ使用量を読み取れませんでした。',
      storage_unavailable: 'ストレージの準備ができていません。',
      backup_push_section: 'プライベートバックアップ',
      backup_push_toggle: 'サーバーにバックアップ',
      backup_erase_row: 'サーバーのバックアップを削除',
      backup_erase_title: 'バックアップを削除しますか？',
      backup_erase_message:
        'このウォレットがバックアップサーバーに送信したすべてのデータが、全端末分まとめて完全に削除されます。同時にバックアップも無効になります。コイン・鍵・リカバリーフレーズはそのままですが、新しい端末でこのウォレットの履歴を復元できなくなり、現在お持ちのコインの一部もそこでは使用できません。',
      backup_erase_confirm: '削除',
      backup_erase_done: 'バックアップを削除しました — {{count}} 件を削除',
      backup_erase_failed: 'バックアップを削除できませんでした。何も削除されていません — もう一度お試しください。',
      backup_erase_no_key: '利用できるウォレット鍵がありません',
      backup_push_disclosure:
        '新しい端末で復元できるよう、ウォレットデータベースの暗号化コピーが BSV アソシエーションのサーバーへ送信されます。復号鍵はあなたのリカバリーフレーズからのみ導出され、運営者は中身を読めません。それでもリカバリーフレーズまたは印刷したシェアは必要です。なければ当社を含め誰もバックアップを開けません。',
      backup_push_off_title: 'プライベートバックアップを無効にしますか？',
      backup_push_off_message:
        'リカバリーフレーズだけではこのウォレットを再構築できません。コインを保護するための情報はデータベースにしか存在しないため、バックアップを切ると新しい端末でフレーズは復元できても資金は復元できません。すでにサーバーにあるデータは削除されません。',
      backup_push_off_confirm: '無効にする',
      backup_push_on_toast: 'バックアップ有効 — 同期中',
      backup_push_off_toast: 'バックアップ無効 — これ以上送信されません',
      permissions: '権限',
      trust_network: '信頼ネットワーク',
      // vault (machine-translated, needs native review)
      vault_title: '保管庫',
      vault_row_title: '保管庫',
      vault_hero_title: '安全な保有資産',
      vault_hero_body:
        '長期保有向けのコールドストレージ。あなた自身の YubiKey でロックします。YubiKey 5 NFC が 2 本以上必要です。',
      vault_unsupported_title: 'YubiKey が必要です',
      vault_unsupported_body: 'YubiKey 5 NFC が 2 本以上と、それを読み取れるスマートフォン。',
      vault_restore_from_chain: 'ブロックチェーンから保管庫を復元',
      vault_restore_scanning: 'ブロックチェーンをスキャン中…',
      vault_restore_found: 'ブロックチェーン上で {{count}} 件の保管庫の入金が見つかりました。',
      vault_restore_pending:
        '{{count}} 件の入金が見つかりましたが、初回承認待ちです。しばらくしてから再試行してください。',
      vault_restore_none_found: 'このウォレットの保管庫の入金はブロックチェーン上に見つかりませんでした。',
      vault_balance_label: '保管庫の保有額',
      vault_key_section: 'セキュリティキー（{{count}}/5）',
      vault_manage_section: '管理',
      vault_deposit_cta: '入金',
      vault_withdraw_cta: '出金',
      vault_deposit_title: '保管庫に入金',
      vault_withdraw_title: '保管庫から出金',
      vault_deposit_sub: '普段の残高から。YubiKey は不要です。',
      vault_withdraw_sub: '普段の残高に戻します。保管庫の鍵のいずれかをかざしてください。',
      vault_deposit_done: '保管庫に移動しました',
      vault_reclaim_done: '中断された保管庫への入金から資金を回収しました',
      vault_withdraw_done: '保管庫から出金しました',
      vault_withdraw_partial: 'あと {{count}} 件の入金が残っています — 移動するには再度引き出してください。',
      vault_withdraw_reason: '署名するには YubiKey をここにかざしてください — 保管庫から {{amount}} sats を出金',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: 'まだ利用できません — このリリースでは保管庫への入金は無効になっています。',
      vault_not_on_mainnet_body:
        '保管庫はメインネットでのみ利用できます。ご利用にはウォレットをメインネットに戻してください。',
      vault_intro_title: '保管庫を設定する',
      vault_intro_what:
        '保管庫はあなた自身の YubiKey で資金をロックします。保管庫の鍵はこのスマートフォン上に一切存在せず、どの鍵一本でも開けられます。',
      vault_intro_two_keys: 'YubiKey が少なくとも 2 本必要です。',
      vault_intro_apart: '別々の場所に保管してください — 一緒に保管した 2 本の鍵は 1 本と同じです。',
      vault_intro_backup:
        'ウォレットの暗号化バックアップをオンにしておいてください — 認証済みの復旧記録が保存されます。',
      vault_intro_ack:
        '理解しました：この保管庫を開けられるのは自分の YubiKey だけで、リカバリーフレーズでは開けられません。すべて失えば資金は失われます。',
      vault_intro_piv_ack:
        'この YubiKey の PIV アプリ全体が工場出荷状態にリセット済み、または保管庫専用であることを確認します。設定により全体の PIV PIN、PUK、管理キーが変更され、すべての PIV スロットに影響する可能性があります。新しい PUK は安全に記録します。',
      vault_intro_begin: '開始',
      vault_key_step_title: '鍵 {{k}}（最大 5 本）',
      vault_key_step_replace:
        '保管庫専用に初期化した YubiKey を使ってください。保管庫スロットに古い鍵がある場合、置き換える前に確認が表示されます。',
      vault_pin_choose_title: 'PIN を決めてください',
      vault_pin_choose_sub: 'このキーを使うたびに入力します。6〜8 桁の数字。',
      vault_pin_confirm_label: 'もう一度入力',
      vault_pin_mismatch: '2 つの PIN が一致しません。',
      vault_pin_not_default: '123456 以外を選んでください。',
      vault_puk_title: '復旧コード',
      vault_puk_body:
        '書き留めて安全な場所に保管してください。PIN を忘れたときにこのキーを解除できる唯一の方法で、二度と表示されません。',
      vault_puk_ack: '書き留めました',
      vault_reset_offer: 'このキーをリセット',
      vault_reset_title: 'このキーをリセットしますか？',
      vault_reset_body:
        'このキーは以前に使われています。リセットすると、保管庫スロットだけでなく PIV アプリケーション内のすべての証明書と鍵が消去されます。このキーに依存しているものは動作しなくなります。',
      vault_reset_ack: 'このキーの内容がすべて消えることを理解しました',
      vault_reset_unknown_ack:
        'このデバイスでは、このキーが別のプロファイルの保管庫を保護しているかどうかを確認できません。消去するとその鍵が永久に失われる可能性があることを理解しました。',
      vault_reset_own_draft_ack:
        'このキーには、以前の未完了の試行で作られた鍵が入っており、この端末にはその記録が残っています。リセットするとそれが消え、その試行は再開できなくなることを理解しました。',
      vault_reset_nothing_erased:
        '何も消去されていません。このキーの保管庫スロットは空ではないため、もう一点だけ確認が必要です。',
      vault_reset_confirm: '消去してリセット',
      vault_resetting: 'キーをリセットしています…',
      vault_reset_done: 'キーをリセットしました。続けるにはもう一度かざしてください。',
      vault_reset_uncertain:
        'リセットは完了しませんでした。このキーは既に消去されている可能性があります。頼りにする前に確認してください。',
      vault_err_reset_enrolled: 'そのキーは既に保管庫の一部です。リセットすると保管庫の鍵が 1 つ失われます。',
      vault_err_reset_unreadable:
        'この端末の保管庫の記録を読み取れなかったため、このキーがあなたの保管庫の鍵でないと確かめられません。何も消去されず、リセットは拒否されました。',
      vault_enrollment_resume_pin_hint:
        '保存済みのキーを再開するには、以前そのキーに設定した PIN が必要です。新しい PIN ではありません。3 回間違えるとキーはロックされます。',
      vault_setup_step: 'ステップ {{n}}／{{total}}',
      vault_enrollment_resume: '保存済みキーを再開',
      vault_enrollment_reset_required:
        'この YubiKey は初期状態ではないため、このままでは登録できません。保管庫の鍵を保存するには、PIV アプリを消去して初期状態に戻す必要があります。',
      vault_enrollment_state_uncertain:
        'セットアップの試行が中断されたため、その YubiKey がどの状態で残されたか確認できません。PIV アプリを消去して初期状態に戻すまで、その鍵は再び使用されません。',
      vault_nfc_enroll_message: '設定するには YubiKey をここにかざしてください',
      vault_nfc_adopt_message: '復元したこの YubiKey をここにかざして確認してください',
      vault_nfc_sign_batch: '署名するには YubiKey をここにかざしてください — バッチ {{b}}/{{n}}',
      vault_key_use_different: '別の YubiKey を使う',
      vault_key_setup_again: 'もう一度設定する',
      vault_name_default: '鍵 {{k}}',
      vault_more_title: '別の鍵を追加しますか？',
      vault_more_body: '最大 5 本。どの 1 本でも単独で保管庫全体を開けられます。',
      vault_more_add: '別の鍵を追加',
      vault_more_finish: '完了',
      vault_more_need_two: '完了する前に 2 本目の鍵を追加してください — 鍵が 1 本では復旧できません。',
      vault_leave_setup: '設定をやめる',
      vault_leave_title: '設定をやめますか？',
      vault_leave_body:
        '設定した {{count}} 本の YubiKey はまだ保存されません。鍵は本体に残るので、すぐにまた追加できます。',
      vault_leave_confirm: 'やめる',
      vault_leave_stay: '続ける',
      vault_done_body:
        '{{count}} 本の鍵でこの保管庫を開けられます。開けられるのはこれらの鍵だけで、リカバリーフレーズでは開けられません。',
      vault_done_cta: '完了',
      vault_add_key_row: '鍵を追加',
      vault_add_key_done:
        '{{nickname}} は今後の入金を開けられます。すべてを開けられるように保管庫を再ロックしてください。',
      vault_key_added_toast: '鍵を追加しました',
      vault_recovery_verified: '復元した鍵をこのデバイスで確認しました',
      vault_relock_row: '保管庫を再ロック',
      vault_relock_now: '今すぐ再ロック',
      vault_relock_choose: '再ロックにはどの鍵をかざしますか？',
      vault_relock_reason: '既存の鍵のいずれか（{{names}}）をかざしてください — 今追加した鍵ではありません。',
      vault_relock_reason_generic: '署名するには YubiKey をここにかざしてください — 保管庫を現在の鍵に再ロックする',
      vault_relock_reason_remaining: '残りの鍵だけに再ロックします：{{names}}。',
      vault_key_pending_removal: '削除済み — 完了するには再ロック',
      vault_key_removal_awaiting_network: '削除中 — ネットワークによる再ロックの受理待ち',
      vault_removal_awaiting_network: '再ロックを送信しました。ネットワークに受理されると入金と出金を再開できます。',
      vault_err_too_many_active_keys: '保管庫をロックできる鍵は最大 5 本です。入金、再ロック、残高の一部の出金の前に、鍵を 1 本削除してください。',
      vault_key_section_over_limit: 'セキュリティキー（{{count}}、1 本削除してください）',
      vault_add_key_done_over_limit: '{{nickname}} を追加しました。保管庫をロックできる鍵は最大 5 本なので、この鍵で置き換える鍵を削除してから、再ロックを 1 回行ってください。',
      vault_relock_done: '保管庫を現在の鍵に再ロックしました',
      vault_relock_capped: 'あと {{count}} 件の入金が残っています — 移動するには再度再ロックしてください。',
      vault_relock_unreachable:
        '{{count}} 件の入金は {{names}} でしか開けられません。それらの鍵のいずれかで再度再ロックしてください。',
      vault_badge_missing: '{{count}} 件の入金がまだ {{nickname}} で開けられません',
      vault_badge_removed: '{{count}} 件の入金がまだ削除済みの鍵で開けられます',
      vault_key_action_rename: '名前を変更',
      vault_key_action_remove: '削除',
      vault_rename_title: '{{nickname}} の名前を変更',
      vault_rename_save: '保存',
      vault_remove_title: '{{nickname}} を削除しますか？',
      vault_remove_body:
        'ウォレットは {{nickname}} の使用を停止します。保管庫に既にある資金は、再ロックするまでこの鍵で開けられたままです（約 {{fee}} sats）。',
      vault_remove_only: '削除のみ',
      vault_key_removed_toast: '鍵を削除しました',
      vault_export_explainer:
        '保管庫への各入金は、使うまでソルトを隠したままにします — ロックスクリプトにはそれについて何も表示されません。暗号化バックアップをオンにしておくと、新しい端末で YubiKey の使用に必要な鍵記録を復元して認証できます。',
      vault_footnote: '保管庫を開けられるのはこれらの鍵だけです。リカバリーフレーズでは開けられません。',
      vault_floor_line:
        '最低入金額 {{floorDisplay}}（{{floorSats}} sats）。保管庫への入金の作成には約 {{feeDisplay}} かかります。',
      vault_first_deposit_title: '保管庫への初回入金',
      vault_first_deposit_body:
        '保管庫への初回入金 — {{amount}} は {{count}} 本の YubiKey（{{names}}）でしか開けられなくなります。リカバリーフレーズは役に立ちません。',
      vault_backup_off_title: 'プライベートバックアップをオンにしてください',
      vault_backup_off_body:
        'ウォレットが新しい保管庫出力を作成する前に、プライベートバックアップをオンにする必要があります。復旧に必要な認証済み YubiKey 記録とウォレット履歴が保存されます。保管庫のおつりを作成せずに、すべてを引き出すことはできます。',
      vault_backup_off_cta: 'バックアップ設定を開く',
      vault_seed_not_preserved_title: '先に復元フレーズを保存してください',
      vault_seed_not_preserved_body:
        'このVaultは復元フレーズとYubiKeyのいずれか1つがなければ復元できません。YubiKeyにはそのコピーは保存されません——書き留める前に紛失すると、この資金は復元できなくなります。入金する前に今すぐ保存してください。',
      vault_seed_not_preserved_cta: '復元フレーズを保存',
      vault_choose_key: 'どの鍵をかざしますか？',
      vault_remainder_title: 'すべて引き出しますか？',
      vault_remainder_body:
        '{{amount}} を引き出すと {{remainder}} が残り、保管庫の最低額 100,000 sat を下回ります。保管庫全体が普段の残高に移動します。',
      vault_remainder_all: 'すべて引き出す',
      vault_remainder_change: '金額を変更',
      vault_unreachable_title: '保管庫の一部には別の鍵が必要です',
      vault_unreachable_body:
        '保管庫の一部には別の鍵が必要です — {{moved}} を移動しました。{{amount}} 分の {{count}} 件の入金は {{names}} でしか開けられません。それらの鍵のいずれかで再度引き出してください。',
      vault_sign_progress: '{{total}} 件中 {{signed}} 件に署名済み',
      vault_err_pin_locked_enroll:
        'この YubiKey の PIN はブロックされています。Yubico Authenticator で PUK を使って解除するか、別の YubiKey で設定してください。',
      vault_err_serial_mismatch_chosen:
        'これは {{tappedName}} です。選択したのは {{chosenName}} です — それをかざすか、戻って {{tappedName}} を選択してください。',
      vault_err_key_already_enrolled: 'この YubiKey（{{nickname}}）は既に追加済みです。別の鍵をかざしてください。',
      vault_err_not_released: 'このリリースでは保管庫への入金は無効になっています。',
      vault_err_not_on_mainnet:
        '保管庫はメインネットでのみ利用できます。ウォレットをメインネットに切り替えてからお試しください。',
      vault_err_backup_off:
        '新しい保管庫出力を作成する前に、暗号化プライベートバックアップをオンにしてください。すべてを引き出すことはできます。',
      vault_err_not_enough_keys: 'この保管庫の鍵は 2 本未満です。もう一度設定してください。',
      vault_err_too_many_keys: '保管庫の鍵は最大 5 本です。',
      vault_err_last_keys: '保管庫には少なくとも 2 本の鍵が必要です。この鍵を削除する前に別の鍵を追加してください。',
      vault_err_relock_required: '他の鍵ですべての入金を開けられるように、まず保管庫を再ロックしてください。',
      vault_err_action_pending: '保管庫の取引がまだ完了していません。完了してからもう一度お試しください。',
      vault_resolve_held_deposit_action: '中断された預け入れを完了する',
      vault_resolve_held_deposit_done: '預け入れの問題を解決しました',
      vault_resolve_held_deposit_failed: '預け入れの問題を解決できませんでした。もう一度お試しください。',
      vault_resolve_held_deposit_none:
        'ここで解決できるものはありません。保留中の取引を再試行またはキャンセルするには、アクティビティタブを確認してください。',
      vault_err_key_not_committed:
        '{{nickname}} ではこの保管庫の入金をどれも開けられません。別の保管庫の鍵を使ってください。',
      vault_err_key_cannot_cover:
        '{{nickname}} は保管庫の {{total}} のうち {{reachable}} を開けられます。{{reachable}} まで引き出すか、代わりに {{otherNames}} を使ってください。',
      vault_err_too_small_to_relock:
        'この保管庫の残高は 100,000 sats 未満で、再ロックするには少なすぎます。代わりに引き出してから再度入金してください。',
      vault_err_bad_version:
        'トランザクションが誤った形式で作成されました。何も署名されていません — もう一度お試しください。',
      vault_err_chain_scan_failed:
        'Vaultへの入金についてブロックチェーンのスキャンを完了できませんでした。接続を確認してもう一度お試しください。',
      vault_shares_unavailable: '印刷できる鍵データがありません。',
      vault_enroll_begin: '保管庫を設定',
      vault_enroll_phase_connecting: 'キーを読み取っています…',
      vault_enroll_phase_pin_check: 'キーを確認しています…',
      vault_enroll_phase_checking_slot: '保管庫スロットを確認しています…',
      vault_enroll_phase_personalizing: 'PIN と復旧コードを設定しています…',
      vault_enroll_phase_generating: '保管庫の鍵を作成中 — 点滅したらキーにタッチしてください',
      vault_enroll_phase_challenging: '新しい保管庫の鍵を確認中 — 点滅したらキーにタッチしてください',
      vault_enroll_phase_done: '完了',
      vault_touch_when_blinks: 'キーが点滅したら金色の接点にタッチしてください。',
      vault_nfc_activation_hint:
        'このYubiKeyを初めて使いますか？まず数秒間USB-Cポートに挿してください——新しいキーはNFCが無効の状態で出荷され、挿すまで有効になりません。',
      vault_continue: '続ける',
      vault_back: '戻る',
      vault_enrolled_toast: '保管庫の準備ができました',
      vault_insert_key: 'YubiKey を挿入してください',
      vault_hold_key_nfc: 'YubiKey をスマートフォンの上部にかざしてください',
      vault_keep_holding_nfc: 'そのまま保持してください — 解錠中…',
      vault_unlocking_funds: '資金を解錠しています',
      vault_unlocking_sub: '数秒かかります。アプリを開いたままにしてください。',
      vault_sending_to_network: 'ネットワークに送信中',
      vault_sending_sub: 'すぐに通らない場合も、再試行を続けます。',
      vault_reading_key: 'キーを読み取っています…',
      vault_enter_pin: 'YubiKey の PIN を入力',
      vault_pin_sub_nfc: 'その後、鍵をスマートフォンの上部にかざして完了してください。',
      vault_pin_sub_usb: 'その後、金色の接点に触れて完了してください。',
      vault_touch_contact: '金色の接点にタッチ',
      vault_unlock_cta: 'ロック解除',
      vault_pin_retries: '残り {{count}} 回',
      vault_locked: '保管庫がロックされています',
      vault_retry: '再試行',
      vault_cancel: 'キャンセル',
      vault_dismiss: '閉じる',
      vault_err_wrong_key: 'この YubiKey はこの保管庫の鍵ではありません。',
      vault_err_serial_mismatch: 'この YubiKey はこの保管庫の鍵ではありません（{{names}}）。',
      vault_err_touch_timeout: 'タッチが検出されませんでした。もう一度金色の接点にタッチしてください。',
      vault_err_pin_locked:
        'この YubiKey の PIN はブロックされています。別の保管庫の鍵を使うか、Yubico Authenticator で PUK を使って解除してください。',
      vault_err_pin_invalid: 'PIN が正しくありません。',
      vault_err_puk_invalid: 'PUK が正しくありません。',
      vault_err_puk_locked: 'この YubiKey の PUK はブロックされています。新しい YubiKey を使用してください。',
      vault_err_ceremony_active: '別の保管庫操作がまだ YubiKey を使用しています。完了を待ってから再試行してください。',
      vault_err_scope_changed:
        'この操作中にウォレットまたはネットワークが変わりました。現在のウォレットでもう一度開始してください。',
      vault_err_key_removed_mid_op: 'キーが取り外されました。挿入してもう一度お試しください。',
      vault_err_no_key: 'YubiKey が検出されませんでした。',
      vault_err_driver_unavailable: 'このデバイスでは YubiKey がサポートされていません。',
      vault_err_mgmt_key_custom: 'このキーにはカスタム管理キーが設定されており、自動的に使用できません。',
      vault_err_attestation_invalid:
        'この YubiKey のメーカー証明を検証できませんでした。PIV 証明スロットが変更されていない、工場証明済みの正規 YubiKey を使用してください。',
      vault_err_generic: '問題が発生しました。もう一度お試しください。',
      vault_err_template_invalid: '保管庫へのアクセス中に問題が発生しました。もう一度お試しください。',
      vault_err_not_enrolled: '保管庫が設定されていません。',
      vault_err_pin_required: 'PIN を入力してください。',
      vault_err_unsupported_platform: 'このデバイスでは YubiKey がサポートされていません。',
      vault_err_slot_occupied: 'キースロットはすでに使用されています。',
      vault_replace_key_warning:
        '置き換えると既存の P-256 秘密鍵は完全に破棄されます。その鍵を必要とする保管庫の残高は使用できなくなる可能性があります。',
      vault_replace_key_confirm: '既存のキーを置き換える',
      vault_resume_draft_confirm_title: 'この準備済みキーを使用しますか？',
      vault_resume_draft_confirm_body:
        '以前に準備されたキー — {{nickname}} · …{{tail}} — が追加可能な状態ですが、このデバイスは今カード上でそれを確認したわけではありません。心当たりがある場合のみ続行してください。',
      vault_resume_draft_confirm_use: 'このキーを使用',
      vault_resume_draft_confirm_discard: '破棄する',
      vault_err_enrollment_partial:
        '鍵の変更後に YubiKey の設定が停止しました。再試行の案内に従ってください。この鍵は保管庫に追加されていません。',
      vault_err_key_not_adopted: '復元したこの YubiKey を出金に使う前に、このデバイスで所持を確認してください。',
      vault_err_too_many_inputs:
        '一度に移動する保管庫の預入が多すぎます。まず少額を引き出してください — 引き出しごとに保管庫が統合されるため、次はより多く移動できます。',
      vault_err_requires_online:
        '保管庫の送金には接続が必要です。再接続してからお試しください — 保管庫の資金が後送信のキューに入ることはありません。',
      vault_err_user_cancelled: 'キャンセルしました。',
      vault_err_vault_empty: '保管庫にはまだ出金できる資金がありません。',
      vault_err_amount_exceeds_balance: '保管庫の残高を超えています。',
      vault_err_below_dust: 'この金額は少なすぎて送金できません。',
      vault_err_no_transaction: '送金を完了できませんでした。もう一度お試しください。',
      vault_err_nfc_lost: '接続が切れました。鍵をスマートフォンにしっかり当ててください。',
      vault_pin_invalid_retry: 'PIN が違います — 残り {{count}} 回。もう一度「出金」をタップしてやり直してください。',
      vault_pin_invalid_retry_generic: 'PIN が違います。もう一度「出金」をタップしてやり直してください。',
      vault_disable_row: '保管庫を無効にする',
      vault_disable_title: '保管庫を無効にしますか？',
      vault_disable_message: 'このスマートフォン上の保管庫の鍵リストを消去します。鍵自体は YubiKey に残ります。',
      vault_disable_confirm: '無効にする',
      vault_disabled_toast: '保管庫を無効にしました',
      vault_disable_blocked_title: '先に資金を出金してください',
      vault_disable_blocked_message:
        '保管庫にはまだ資金があります。無効にする前にすべて日常の残高へ出金してください — そうしないと、署名する手段がないまま資金がロックされます。',
      vault_ok: 'OK',
      vault_mock_toggle: 'モック YubiKey を使用（開発）',
      vault_on: 'オン',
      vault_off: 'オフ',
      vault_shares_word_count:
        'リカバリーシェアは 12 語のフレーズにのみ対応しています。代わりにフレーズを書き留めてください。',
      scan_shares_legacy_title: '古いバックアップ形式',
      scan_shares_legacy_message:
        'これらのシェアは古い形式で、日常の残高は復元できますが保管庫は開けません。12 語のリカバリーフレーズがまだ手元にあるなら、そちらから復元してください — すべてを復元できます — そのうえで新しいシェアを印刷してください。フレーズを失っている場合は、資金を新しいウォレットへ送って最初からやり直してください。',
      scan_shares_legacy_ack: '理解しました',

      // Account
      delete_wallet: 'ウォレットを削除',
      delete_wallet_warning_title: 'ウォレットを削除しますか？',
      delete_wallet_warning_body:
        'このデバイスからウォレットの鍵が完全に削除されます。リカバリーフレーズのバックアップまたはリカバリーシェアの印刷を行っていない場合、ウォレットへのアクセスを永久に失います。',
      delete_wallet_confirm: 'ウォレットを削除',
      delete_wallet_failed_title: 'ウォレットの削除に失敗しました',
      delete_wallet_failed_body: 'このデバイスからウォレットを完全に削除できませんでした。一部の鍵データが残っている可能性があります。もう一度お試しください。',
      bsv_network: 'BSVネットワーク',
      arc_endpoint: 'ARC エンドポイント',
      arc_api_token: 'API トークン',
      arc_custom_url: 'カスタム URL',
      arc_apply: '適用',
      arc_reset_default: 'デフォルトに戻す',
      arc_requires_token: 'トークンが必要',
      arc_url_https_required: 'https である必要があります（ローカルテストは http://localhost 可）',
      recovery_phrase: 'リカバリーキー',
      you_have: '残高',
      switching: '切り替え中...',
      mainnet: 'メインネット',
      testnet: 'テストネット',
      teratest: 'テラテスト',
      identity_key: '識別子',
      transactions: '取引',

      // Transactions screen
      no_transactions: '取引履歴はまだありません',
      tx_status_confirmed: '承認済み',
      tx_status_received: '受取済み',
      tx_status_sent: '送金済み',
      tx_status_spent: '支出済み',
      tx_status_transferred: '移動済み',
      tx_status_accepted: '受理済み',
      tx_status_broadcasting: '処理中',
      tx_status_not_sent: '未送信',
      tx_status_unsigned: '下書き',
      tx_status_nonfinal: '保留中',
      tx_status_pending: '保留中',
      tx_status_failed: '失敗',
      tx_status_offline_queued: 'オフライン · キュー待ち',
      tx_status_offline_sending: 'オフライン · 送信中',
      tx_status_offline_rejected: 'オフライン · 拒否済み',
      tx_copied: 'トランザクションをコピーしました',
      tx_not_available: 'トランザクションは利用できません',
      tx_copy_failed: 'トランザクションのコピーに失敗しました',
      tx_abort_success: 'トランザクションを中止しました',
      tx_abort_failed: 'トランザクションの中止に失敗しました',
      tx_abort_maybe_delivered: 'キャンセルできません — この支払いは既に受取人に届いている可能性があります',
      tx_proof_refreshed: '取引が承認されました',
      tx_proof_refresh_failed: '証明を取得できませんでした — 取引はまだ確認されていない可能性があります',
      tx_marked_failed: '未処理 — 失敗として記録し、資金を解放しました',
      tx_still_pending: 'まだ保留中 — 未承認です',
      tx_action_explorer: 'ブロックエクスプローラーで表示',
      tx_export_success: '{{count}} 件の取引をエクスポートしました',
      tx_export_failed: '取引のエクスポートに失敗しました',
      tx_export_csv: 'CSV をエクスポート',
      tx_export: 'エクスポート',
      activity_filter: 'アクティビティを絞り込む',
      activity_filter_all: 'すべて',
      activity_search_placeholder: 'メモ・名前・@ハンドルで検索',
      activity_search_clear: '検索をクリア',
      activity_filter_no_match: '一致するアクティビティはありません',
      wallet_balance_label: '残高',
      wallet_balance_you_have: '残高',
      wallet_balance_refresh: '残高を更新',
      wallet_group_today: '今日',
      wallet_group_yesterday: '昨日',
      back: '戻る',
      wallet_payments: '支払い',
      wallet_vault: '保管庫',
      wallet_settings: '設定',
      wallet_activity: 'アクティビティ',

      // Identity / Payments screen
      message_box_server: 'メッセージボックスサーバー',
      message_box_required: 'アイデンティティベースの支払いの送受信に必要です。',
      send_payment: '送金',
      recipient: '受取人',
      amount: '金額',
      enter_valid_amount: 'ゼロより大きい金額を入力してください',
      display_currency: '表示通貨',
      no_pending_payments: '保留中の支払いはありません',
      outgoing_payments: '送信済み',
      payment_not_delivered: '未配信',
      payment_delivered: '配信済み',
      retry: '再試行',
      retry_failed: '再試行に失敗しました',
      dismiss: '閉じる',
      valid_identity_key: '有効な識別子が入力されました',
      valid_bsv_address: '有効なBSVアドレスが入力されました',
      searching: '検索中...',
      unknown: '不明',
      from_prefix: '送信元:',
      message_box_saved: 'メッセージボックスサーバーを保存しました',
      message_box_removed: 'メッセージボックスサーバーを削除しました',
      message_box_off: 'オフ',
      message_box_off_hint: 'メッセージボックスサーバーがオフです。設定 › 詳細でオンにしてください。',
      enter_valid_url: '有効なURLを入力してください',
      checking_connection: '接続を確認中...',
      connection_failed: '接続に失敗しました',
      recipient_placeholder: '名前、識別子、またはBSVアドレス...',

      // Local Payments screen

      // Trust screen
      trust_updated: '信頼ネットワークを更新しました',
      failed_to_save: '保存に失敗しました',
      confirm_delete: '削除の確認',
      confirm_delete_body: 'この証明者を削除しますか？',
      order_certifiers_hint: '証明者を優先度順に並べます。アプリは上位の証明者を優先表示します。',
      certifiers: '証明者',
      no_certifiers: '証明者はまだいません。',
      add_provider: 'プロバイダーを追加',
      unsaved_changes: '未保存の変更があります',
      duplicate_key_error: 'この公開鍵を持つエンティティは既にリストに存在します！',
      get_provider_details: 'プロバイダー情報を取得',
      validate_details: '情報を検証',
      show_advanced: '詳細を表示',
      hide_advanced: '詳細を非表示',
      description_length_error: '説明は5～50文字で入力してください',
      enter_domain_hint: 'プロバイダーのドメインを入力',
      enter_details_hint: 'プロバイダーの情報を入力',

      // Legacy payments screen
      receive: '受取',
      send: '送金',
      generating_address: 'BSVアドレスを生成中...',
      imported: 'インポート済み',
      unable_to_generate_address: 'BSVアドレスを生成できません',
      send_bsv: 'BSVを送金',
      recipient_address: '受取人のBSVアドレス',
      enter_bsv_address: 'BSVアドレスを入力',

      // Recovery key / wallet setup screen
      wallet_data: 'ウォレットデータ',
      create_new_wallet: '新しいウォレットを作成',
      generate_recovery_phrase_caption: '新しいリカバリーキーを生成',
      import_existing_wallet: '既存のウォレットをインポート',
      paste_recovery_phrase: 'リカバリーキーを貼り付け',
      save_recovery_phrase_heading: 'リカバリーキーを保存',
      save_recovery_phrase_btn: 'リカバリーキーを保存',
      recovery_phrase_write_down: 'これらの12個の単語を順番に書き留め、安全な場所に保管してください。',
      acknowledgment_text:
        'リカバリーキーを保存しました。紛失すると、関連するすべての資金、トークン、証明書に永久にアクセスできなくなることを理解しています。',
      enter_recovery_words: '12 語のニーモニックを入力',
      restore_wallet_description: 'リカバリーキーを入力してウォレットを復元',
      backup_reminder_title: 'ウォレットがバックアップされていません',
      backup_reminder_body: '資金を失わないよう、鍵をエクスポートしてください。',
      biometric_advisory_title: 'ウォレットを保護',
      wallet_balance_show_in: '{{unit}} で表示',
      tx_detail_block: 'ブロック',
      tx_detail_block_pending: 'ブロック待ち',
      biometric_advisory_body: 'Face ID または指紋で資産を保護します',
      biometric_advisory_body_degraded:
        'このデバイスにはFace IDや指紋認証が設定されていないため、ウォレットの鍵は端末のパスコードのみで保護されます。',
      import_prompt_title: 'すでにウォレットをお持ちですか？',
      import_prompt_body: 'リカバリーキーまたはシェアから復元できます。',
      import_prompt_action: 'バックアップから読み込む',
      nearby_advisory_title: '近くの端末に接続',
      nearby_advisory_body:
        'Wi-Fi、Bluetooth、NFC へのアクセス許可を求められます。これは支払いのために近くの端末と直接つながるためだけに使われます — 当社のサーバーに送信されることはなく、あなたのプロファイル作成にも使われません。',
      backup_reminder_now: '今すぐ鍵をバックアップ',
      backup_reminder_import: 'バックアップから読み込む',
      restore_backup_checking: '暗号化バックアップを探しています…',
      restore_backup_progress: '履歴を復元中 — {{total}} 件中 {{chunks}} 件…',
      restore_backup_failed_title: '履歴を復元できませんでした',
      restore_backup_failed_message:
        'リカバリーフレーズには問題ありませんが、暗号化バックアップを再生できませんでした。通常は再試行で解決します。復元せずに続けるとウォレットは使えますが過去の取引は残らず、現在お持ちのコインの一部は使用できません。',
      restore_backup_retry: '再試行',
      restore_backup_skip: '破棄',
      restore_backup_unverified_title: 'バックアップが不完全な可能性があります',
      restore_backup_unverified_body:
        'ウォレットは復元されましたが、サーバー上のバックアップが完全であることを確認できませんでした。履歴の一部が欠けている場合は、他の端末のアップロードが完了してから再度復元してください。',
      import_wallet: 'ウォレットをインポート',

      // Permission sheet
      reject: '拒否',
      authorize: '承認',
      renewal: '更新',
      spending_section: '支出',
      protocols_section: 'プロトコル',
      baskets_section: 'バスケット',
      certificates_section: '証明書',
      requested_fields: 'リクエストされたフィールド',

      // Amount input
      send_max: '最大額を送金',
      message_box_unreachable: '決済サーバーに接続できませんでした。設定でメッセージボックスのURLを確認してください。',
      available: '利用可能',
      note: 'メモ',
      note_placeholder: '何のため？（任意）',
      entire_wallet_balance: 'ウォレットの全残高',
      entire_vault_balance: '保管庫の全額',
      export_wallet_data: 'ウォレットデータをエクスポート',
      export_unencrypted_title: 'このファイルは暗号化されていません',
      export_unencrypted_message:
        'エクスポートされたファイルは、ウォレットデータの暗号化されていない平文のコピーです。身元情報や証明書情報、取引履歴、連絡先が含まれます。このファイルを入手した人は誰でも内容を読むことができます。信頼できる相手・場所とのみ共有してください。',
      import_wallet_data: 'ウォレットデータをインポート',
      import_conflict_title: 'データベースが既に存在します',
      import_conflict_message:
        'このアイデンティティのウォレットデータベースは、より新しいタイムスタンプで既に存在します。ファイル名の最大番号のバックアップが常に使用されます。\n\nこの古いバックアップを使用するには：現在のウォレットデータをエクスポートし、アプリを削除して再インストールし、目的のバックアップのみをインポートしてください。',
      import_confirm_title: 'データベースをインポート',
      import_confirm_message: 'このデータベースがアクティブなウォレットになります。続行しますか？',
      import_success: 'データベースのインポートに成功しました。ウォレットを再起動しています...',
      import_invalid_file: '無効なバックアップファイル',
      import_invalid_file_detail:
        'このファイルは有効なウォレットデータベースのバックアップではありません。wallet-XXXXXXXX-mainnet-TIMESTAMP.db のようなファイル名が必要です。',
      import_oversized_file: 'バックアップファイルが大きすぎます',
      import_oversized_file_detail:
        'このファイルはウォレットのバックアップとして想定されるサイズを超えているため、読み込まれませんでした。正しいウォレットデータベースのバックアップファイルを選択してください。',
      import_untrusted_file: 'このバックアップをインポートできません',
      import_untrusted_file_detail:
        'このバックアップがこのウォレットに属することを確認できなかったか、内容を信頼できませんでした。インポートされませんでした。',
      import_anyway: 'インポート',
      import_invalid_input_title: '無効な入力',
      import_invalid_input_message:
        '有効なリカバリーフレーズ（12〜24語）または64文字の16進数の秘密鍵を入力してください。',
      import_setup_failed: 'ウォレットの設定に失敗しました: {{error}}',
      create_wallet_refused: 'ウォレットを作成できませんでした。もう一度お試しください。',
      create_wallet_failed: 'ニーモニックの生成に失敗しました。もう一度お試しください。',

      // Backup shares
      or: 'または',
      print_recovery_shares: 'リカバリーシェアを印刷',
      scan_backup_shares: 'バックアップシェアをスキャン',
      scan_backup_shares_caption: '印刷されたシェアからウォレットを復元',
      scan_shares_invalid_format: '無効なシェア形式です。有効なバックアップシェアのQRコードをスキャンしてください。',
      scan_shares_camera_needed: 'カメラへのアクセスが必要です',
      scan_shares_camera_description:
        '印刷されたバックアップシェアのQRコードをスキャンするには、カメラへのアクセスが必要です。',
      scan_shares_grant_camera: 'カメラへのアクセスを許可',
      scan_shares_recovering: 'ウォレットを復元中...',
      scan_shares_scan_first: '最初のバックアップシェアをスキャンしてください',
      scan_shares_progress: '{{scanned}} / {{needed}} シェアをスキャン済み',
      scan_shares_remaining: 'あと{{count}}シェア必要です',
      scan_shares_complete: 'すべてのシェアを収集しました！',
      scan_shares_recovery_failed: '復元に失敗しました。もう一度スキャンしてください。',
      scan_shares_threshold_mismatch: 'しきい値が以前のシェアと一致しません',
      scan_shares_integrity_mismatch: '整合性ハッシュが一致しません — シェアが異なる鍵から生成されています',
      scan_shares_duplicate: 'このシェアは既にスキャンされています',
      scan_shares_biometric_title: '生体認証が必要です',
      scan_shares_biometric_message: 'ウォレットの鍵を保護するため生体認証が必要です。もう一度お試しください。',
      recovery_replace_wallet_title: 'この端末のウォレットを置き換えますか？',
      recovery_replace_wallet_body:
        'この端末には既にウォレットがあります。続けると、それとそれで使えるすべての資金が置き換えられます。リカバリーフレーズがどこにも保存されていない場合、そのお金は永久に失われます。',
      recovery_replace_wallet_confirm: 'ウォレットを置き換える',
      invalid_bsv_address: '無効なBSVアドレス',
      scan_recipient_hint: '支払いコードにカメラを向けてください',
      scan_qr_default_hint: 'QRコードにカメラを向けてください',
      scan_wallet_qr_hint: 'ウォレットペアリングのQRコードにカメラを向けてください',

      // Biometric prompts
      biometric_store_wallet: 'ウォレットを安全に保存',
      biometric_load_wallet: 'ウォレットを安全に読み込み',
      biometric_unlock_wallet: 'ウォレットのロックを解除',
      wallet_lost_title: 'ウォレットの鍵が破棄されました',
      wallet_lost_body:
        'この端末の生体認証設定が変更されたため、システムがこのウォレットを保護していた鍵を破棄しました。コインはブロックチェーン上で安全です。リカバリーフレーズでこのウォレットを復元してください。',
      wallet_lost_restore: '復元',
      wallet_lost_later: 'あとで',
      wallet_locked_title: 'ウォレットはロック中',
      wallet_locked_body: 'ウォレットの鍵を読み込むにはロックを解除してください。',
      wallet_unlock_retry: 'ロックを解除',
      wallet_lockout_body:
        '失敗が続いたため一時的にロックされました。パスコードで端末のロックを解除してから、もう一度お試しください。',
      wallet_no_biometrics_body:
        'この端末では生体認証が設定されていません。ウォレットの鍵は端末のキーストアで暗号化されていますが、生体認証では保護されていません。',

      // Connections screen
      connections: '接続',
      advanced: '詳細設定',
      copy_secret_words: 'シークレットワードをコピー',
      print_recovery_keys: 'リカバリーキーを印刷',
      backup_wallet_keys: 'ウォレットの鍵をバックアップ',
      log_out: 'ログアウト',
      connect_to_app: 'アプリに接続',
      no_connections_yet: 'まだ接続がありません',
      no_connections_subtitle: 'デスクトップアプリのQRコードをスキャンしてウォレットを接続してください',
      connections_active: 'アクティブ',
      connections_disconnected: '切断済み',
      disconnect: '切断',
      reconnect: '再接続',
      reconnect_failed: '再接続に失敗しました',
      reconnect_confirm_title: 'このサイトに再接続しますか？',
      reconnect_confirm_message: '{{origin}} が再びウォレットにアクセスできるようになります。',
      reconnect_needs_reapproval: 'この接続を確認できませんでした。再接続するには新しいQRコードをスキャンしてください。',
      scan_qr_code: 'QRコードをスキャン',
      paste_uri: 'URIをペースト',
      invalid_qr_code: '無効なQRコード',
      wallet_not_ready: 'ウォレットの準備ができていません',
      please_log_in_first: 'まずログインしてください',
      unknown_error: '不明なエラー',

      // Pay screen
      payments: '支払い',
      pay: '支払い',
      pay_direction_pay: '支払う',
      pay_direction_receive: '受け取る',
      pay_cell_nearby_pay: '近くにいる人',
      pay_cell_nearby_pay_sub: '相手のコードをスキャン',
      pay_cell_handle_pay: '遠くにいる人',
      pay_cell_handle_pay_sub: '名前を選択 — 相手にもこのアプリが必要です',
      pay_cell_address_pay: 'BSVアドレスへ',
      pay_cell_address_pay_sub: 'BSVアドレスを貼り付けるかスキャン',
      pay_cell_nearby_get: '近くにいる人',
      pay_cell_nearby_get_sub: '自分の支払いコードを見せる',
      pay_cell_handle_get: '遠くにいる人',
      pay_cell_handle_get_sub: '自分の名前を共有',
      pay_cell_address_get: 'BSVアドレスへ',
      pay_cell_address_get_sub: 'BSVアドレスを表示',
      pay_method: '方法',
      pay_method_nearby: '近く',
      pay_method_remote_link: 'リモートリンクを共有',
      pay_method_address: 'BSVアドレスへ',
      pay_pre_nearby: 'お互いがその場にいます。',
      pay_pre_handle: '相手はこのアプリを使っています。',
      pay_pre_address: '相手のBSVアドレスがわかっています。',
      pay_conseq_nearby: '数秒で完了します。',
      pay_conseq_handle: '相手のウォレットが次に確認したときに届きます。',
      pay_conseq_address: '送金されます — 相手に通知は届きません。',
      paid: '支払い完了',
      pay_inbox_attention: '追加できませんでした',
      pay_inbox_attention_hint:
        '届いていますが、ウォレットに追加できませんでした。再試行するか、完全に諦める場合は破棄してください。',
      pay_dismiss_confirm: 'もう一度タップで破棄',
      pay_dismissed: '支払いを破棄しました',
      payment_arrived: '支払いを受け取りました',
      payment_arrived_damaged: 'この支払いデータは破損して届きました',
      discard_payment_title: 'この支払いを破棄しますか？',
      discard_payment_body: '送信者に詳細の再送を依頼します。それまで資金はチェーン上に残ります。',
      discard_void_title: 'この支払いは完了しませんでした',
      discard_void_body: 'キャンセルされたか、別の場所で使われました。閉じても問題ありません。',
      waiting_network_confirm: 'この支払いのネットワーク承認を待っています',
      finish_or_cancel_outgoing: '新しい支払いを送る前に、下の支払いを完了するかキャンセルしてください',
      identity_search_unavailable: '現在、検索を利用できません。しばらくしてからお試しください。',
      abandon_payment: '支払いを中止',
      finish_payment: '支払いを完了',
      cancel_this_payment: 'この支払いをキャンセルしますか？',
      cancel_payment: '支払いをキャンセル',
      resend: '再送信',
      resending: '再送信中…',
      send_payment_details_again: '詳細を再送信',
      tx_action_resend_short: '再送信',
      tx_action_request_confirmation: '受取人に確認を依頼',
      resend_sent: '支払いの詳細を再送信しました',
      resend_failed_no_record: 'この端末にこの支払いの詳細がないため、再送信できません。',
      resend_failed_no_recipient: 'この支払いには送り先の記録がないため、再送信できません。',
      resend_failed_no_transaction: '支払いのデータを取得できませんでした。オンラインのときにもう一度お試しください。',
      resend_requested: '支払いの再送を求められています',
      request_again: 'もう一度リクエスト',
      send_again: 'もう一度送信',
      copy_details: '詳細をコピー',
      payment_bounced_resend: 'この支払いの再送を依頼しました',
      dismiss_rejected_payment: '拒否された支払いを閉じました',
      activity_load_failed: 'アクティビティを読み込めませんでした',
      activity_load_retry: 'タップして再試行',
      activity_view_contact: '連絡先を見る',
      home_payments_need_attention: '{{count}} 件の支払いに対応が必要です',
      home_payments_unsent: '{{count}} 件の支払いが未送達です',
      pay_copy: 'コピー',
      pay_share_link: 'リンクを共有',
      pay_address_watching: 'このアドレスに送られた資金は自動でウォレットに追加されます。',
      pay_address_earlier_day: 'それ以前の日にさかのぼる',
      pay_address_sweep_now: 'このBSVアドレスを今すぐ確認',
      pay_address_swept: '{{amount}} を受け取りました',

      check_wallet: 'トラブルシューティング',
      settings_version: 'v{{version}}',
      error_review_actions: 'この支払いは滞留しています。「トラブルシューティング」で解消を試せます。',
      wallet_check_ok: '問題は見つかりません',
      wallet_check_attention: 'いくつかの項目に確認が必要です',
      wallet_check_couldnt: '確認できませんでした',
      wallet_check_couldnt_body: '修復の手順が完了しませんでした。コインについて何も推測はしていません。',
      wallet_check_summary: '滞留コイン {{freed}} 件を解放 · 支払い {{recovered}} 件を復旧',
      wallet_check_step_coins: '残高（コイン）が有効',
      wallet_check_skip: 'この項目をスキップ',
      wallet_check_skipped: 'スキップ済み',
      wallet_check_step_proofs: '取引の証明',
      wallet_check_step_records: 'ローカルデータベースの記録',
      wallet_check_step_tokens: 'ステーブルコイン送金を検証済み',
      wallet_check_step_online: 'オンライン状態',
      wallet_check_step_missed: '受け取り漏れの支払い',
      wallet_check_step_backup: 'プライベートバックアップの送信',
      wallet_check_step_phrase: 'リカバリーフレーズの保管',
      wallet_check_progress: '{{current}} / {{total}}',
      wallet_check_stuck_title: 'この支払いを送信できませんでした',
      wallet_check_stuck_message: '一部のコインが滞留しているようです。「トラブルシューティング」で解放できます。',
      wallet_check_failed: '確認を完了できませんでした。もう一度お試しください。',

      // Local payments
      local_pay_request: '支払いをリクエスト',
      local_pay_send: '支払いを送信',
      local_pay_amount: '金額',
      local_pay_show_qr: 'このコードを支払う側に見せてください',
      local_pay_scan_qr: '受け取る側のコードをスキャン',
      local_pay_scan_payer_qr: '支払う側のコードをスキャン',
      local_pay_scan_progress: '{{have}}/{{total}} 個受信済み',
      local_pay_show_payment_qr: 'このコードを受け取る側に見せてください',
      local_pay_animated_hint:
        '下のアニメーションコードをスキャンしてもらってください。相手の端末が全て受け取るまでコードは変化し続けます。',
      local_pay_show_qr_instead: '代わりにQRコードを表示',
      local_pay_preparing: 'リクエストを準備しています…',
      local_pay_delivering: '支払いを送信しています…',
      local_pay_saving: '支払いを保存しています…',
      local_pay_sent: '支払いを送信しました',
      local_pay_received: '支払いを受け取りました',
      local_pay_added: 'ウォレットに追加しました。',
      local_pay_added_multiple: '{{count}}件の支払いをウォレットに追加しました。',
      local_pay_received_confirming: 'ウォレットで確認しています…',
      local_pay_received_not_credited:
        'まだウォレットに追加されていません。何かを渡す前にアクティビティを確認してください。',
      local_pay_decline_broadcast_title: '拒否した支払いがネットワークに流れました',
      local_pay_decline_broadcast_body:
        '拒否された支払いが、その後ネットワーク上に現れました。再送しないでください — アクティビティを確認してください。',
      local_pay_pending_abort_dropped_title: '未検証の解放リクエストを無視しました',
      local_pay_pending_abort_dropped_body:
        '保存されていた支払い解放記録を検証できなかったため、何も解放されませんでした。支払いがまだ止まっているように見える場合は、アクティビティからキャンセルしてください。',
      local_pay_cancel_unverifiable_title: 'オフラインではこの支払いを確認できません',
      local_pay_cancel_unverifiable_body:
        '相手が既にスキャン済みの場合、キャンセルするとこのコインが二重に使われる可能性があります。一度もスキャンされていないと確信できる場合のみキャンセルしてください。',
      local_pay_cancel_unverifiable_confirm: 'それでもキャンセル',
      local_pay_queued: '保存しました — 自動的にウォレットへ追加されます。',
      local_pay_already_paid: 'このリクエストはすでに支払われています。',
      local_pay_failed: '支払いに失敗しました',
      local_pay_network_denied:
        'ローカルネットワークへのアクセスがオフです。近くの端末に支払うには「設定」でオンにしてください。',
      local_pay_radio_fallback: '無線リンクが利用できません — 代わりにこのコードを相手の端末に表示してください。',
      local_pay_floor_peer_bt_off: '相手のBluetoothがオフのため、この支払いはコードで行います。',
      local_pay_floor_local_bt_off: 'Bluetoothがオフです。オンにすると、より速い近距離リンクが使えます。',
      local_pay_floor_local_ble_denied:
        'BSV WalletのBluetoothアクセスがオフです。「設定」で許可すると、より速い近距離リンクが使えます。',
      local_pay_floor_cross_os: '相手の端末は現在Bluetoothを使えないため、この支払いはコードで行います。',
      local_pay_floor_peer_no_radio: '相手の端末は無線リンクを待ち受けていないため、この支払いはコードで行います。',
      local_pay_ble_unavailable: 'Bluetoothを利用できません。上のコードは引き続き使えます。',
      local_pay_too_large: 'この支払いのデータはQRコードで受け渡すには大きすぎます。',
      local_pay_nearby_unavailable: '近くのWi-Fiを利用できません。上のコードは引き続き使えます。',
      local_pay_retry_save: 'もう一度保存する',
      local_pay_wrong_session:
        'その支払いはこのリクエストと一致しないため、受け取りませんでした。支払う方にコードをもう一度スキャンしてもらってください。',
      local_pay_declined_mismatch:
        '受取人がこの支払いを受け付けませんでした。リクエストと一致しません。もう一度コードをスキャンしてください。',
      local_pay_declined_already_paid: 'そのリクエストはすでに支払い済みです。何も送金されていません。',
      local_pay_declined_save:
        '受取人が支払いを保存できませんでした。何も送金されていません — もう一度お試しください。',
      local_pay_declined_decode:
        '受取人が支払いを読み取れませんでした。何も送金されていません — もう一度お試しください。',
      local_pay_declined_not_covered:
        '受取人がこの支払いを発行者の記録と照合できませんでした。何も送金されていません。',
      local_pay_broadcast_pending: '送金しました。この端末ではまだ処理できていません — 受取人が処理します。',
      local_pay_any_amount: '金額は任意',
      local_pay_choose_amount: '金額を選択',
      local_pay_enter_amount_send: 'いくら送金しますか？',
      local_pay_qr_unavailable: 'このコードを作成できませんでした。もう一度リクエストを始めてください。',
      local_pay_presence_qr: 'コードで受け渡し — 接続はありません',
      local_pay_presence_ready: '近くの暗号化リンクの準備ができました',
      local_pay_presence_waiting_payee: '近くの端末を待っています',
      local_pay_presence_waiting_payer: '受取人の端末を探しています…',
      local_pay_presence_linked: '暗号化リンクが確立しました',
      local_pay_presence_paid: '支払いを確認しました',
      pay_offline_title: 'オフラインです',
      pay_offline_body: '現在は近くへの支払いのみ利用できます。再接続すると処理されます。',
      pay_offline_queued: '{{count}}件の支払いが、再接続後に処理されるのを待っています。',
      pay_offline_pending_title: '処理待ち',
      pay_offline_pending_body:
        '{{count}}件の支払いがまだ処理されていません。処理されるまでは完了ではなく、再試行を続けます。',
      pay_offline_needs_internet: 'インターネットが必要です',
      pay_offline_rejected_title: '支払いが拒否されました',
      pay_offline_rejected_body: 'ネットワークは、{{when}}に{{via}}経由で{{sender}}から渡された支払いを拒否しました。',
      pay_offline_unknown_sender: '不明な送信者',
      pay_offline_unknown_via: '不明な方法',
      pay_received_not_broadcast: 'オフラインで受け取り済み · まだ処理されていません',
      pay_offline_sent_rejected_title: '支払いを届けられませんでした',
      pay_offline_sent_rejected_body: 'ネットワークは、あなたが{{when}}に送った支払いを拒否しました。',
      pay_offline_send_now: '今すぐ送信',
      pay_offline_stalled_body: '一部のキュー内の支払いは自動的に送信できません: {{detail}}',
      offline_stall_no_request: '待機中の支払いに送信記録がありません。',
      offline_stall_bad_beef: '待機中の支払いの取引データを読み取れませんでした。',
      offline_stall_foreign_ancestor: '待機中の支払いが、別のウォレットが処理しなかった親取引を待っています。',
      pay_offline_kv_pending: 'このウォレットへの追加を待っている近くの支払いが {{count}} 件あります。',
      pay_offline_kv_stuck: '{{count}} 件の近くの支払いを追加できませんでした。送り主に再送を依頼してください。',
      pay_offline_kv_corrupt: 'この端末で破損した支払いデータが見つかりました。',
      pay_offline_kv_corrupt_body:
        '該当する支払いはこの端末に取り分けてあり、削除されていません。すでに残高に加算されているものもあるため、支払いがまだ届いていない場合にのみ、送り主に再送を依頼してください。',
      pay_offline_show_code: 'コードをもう一度表示',
      pay_parked_cancel: '支払いをキャンセル',
      pay_parked_already_sent: 'この支払いはすでに送信されました',
      token_cancel_already_sent: 'この支払いはすでに完了しており、キャンセルできません。',
      tx_status_parked: '未受け渡し',
      tx_status_import_hold: 'インポートで保留中',
      resend_sending: 'メッセージボックス経由で送信中',
      cancelling_payment: '支払いをキャンセル中',

      contacts: '連絡先',
      contacts_search_placeholder: '名前またはハンドルで検索',
      contacts_empty_title: 'まだ連絡先がありません',
      contacts_empty_body:
        '支払った相手は、保存するとここに表示されます。右上のボタンで相手のプロフィールコードをスキャンしてください。',
      contacts_recent: '最近',
      contact_edit_name: '名前を編集',
      contact_save_name: '名前を保存',
      contact_activity: 'アクティビティ',
      contact_activity_empty: 'この連絡先とのアクティビティはまだありません',
      contact_delete: '連絡先を削除',
      contact_delete_body: '{{name}} を連絡先から削除します。過去の支払いには影響しません。',
      contact_handle_caption: '相手の登録済みハンドル · 変更できるのは本人のみです',
      contact_new_title: '新しい連絡先',
      contact_save: '保存',
      contact_identifier: '識別子',
      contact_saved: '連絡先を保存しました',
      contact_scan_hint: 'プロフィールコードをスキャンして追加',
      contact_identifier_placeholder: '相手の識別子を貼り付け',
      contact_identifier_invalid: '有効な識別子ではないようです',
      contact_add_from_payment: '連絡先に追加',

      profile: 'プロフィール',
      profile_display_name: '表示名',
      profile_display_name_hint: 'ハンドルの横に公開表示されます。あなたを検索した人は誰でも見られます。',
      profile_handle: 'ハンドル',
      profile_handle_placeholder: 'あなたのハンドル',
      profile_handle_checking: '空き状況を確認中…',
      profile_handle_available: '{{handle}} は使用可能です',
      profile_handle_taken: '{{handle}} はすでに使用されています',
      profile_handle_invalid: '3〜32文字：a-z、0-9、ドット、アンダースコア、ハイフン。先頭と末尾は英字か数字',
      profile_handle_failed: '空き状況を確認できませんでした。もう一度お試しください。',
      profile_handle_unavailable: 'このビルドではハンドル登録はまだ利用できません。',
      profile_handle_registered: 'ハンドルを登録しました',
      profile_handle_register_action: '登録',
      profile_handle_too_similar: '{{handle}} は既存のものと似すぎています',
      profile_handle_reserved: '{{handle}} は予約済みです',
      profile_handle_cooldown: '{{handle}} は最近解放されたため、まだ利用できません',
      profile_handle_pending: '登録を完了しています…',
      profile_handle_rolled_back: '{{handle}} は取得済みでした。{{previous}} をそのまま使います。',
      profile_handle_changed: 'ハンドルを変更しました',
      profile_handle_rejected: 'レジストリがそのハンドルを拒否しました。',
      profile_handle_clock_ahead: '端末の時計が進みすぎています。数分後にもう一度お試しください。',
      profile_handle_wrong_registry:
        'そのハンドルはこのレジストリにありません。プロフィールを開き直して更新してください。',
      profile_display_name_publish_failed: '名前はこの端末に保存しましたが、レジストリには公開できませんでした。',
      profile_qr_hint: '他の人がこれをスキャンしてあなたを連絡先に追加できます',

      pay_step_contacts: '連絡先',
      pay_step_continue: '次へ',
      pay_review_to: '宛先',
      pay_review_amount: '金額',
      pay_review_note: 'メモ',
      pay_review_note_edit: 'メモを追加',
      pay_review_delivery_host: '配信先',
      pay_trust_contact: 'あなたの連絡先',
      pay_trust_handle_attested: '登録済み',
      pay_trust_handle_domain_attested: '{{domain}} により確認済み',
      pay_trust_unverified: '未確認です。この識別子をよく確認してください。',
      pay_address_network_mismatch: 'このアドレスは、現在使用中のネットワークとは異なるネットワークのものです。',
      contacts_your_contacts: 'あなたの連絡先',
      contacts_other_people: 'その他の人',
      contacts_not_saved: '未保存',
      contacts_network_footer: 'ネットワーク上で見つかりました。あなたはまだ保存していません。',
      contact_no_handle: 'ハンドルはまだありません',
      contact_no_handle_registered: 'ハンドル未登録',
      contact_name_section: 'あなたがつける名前',
      contact_name_hint: 'あなたにだけ表示されます。相手の自称は変わりません。',
      contact_new_name_hint: 'あなたにだけ表示されます。この端末に保存されます。',
      contact_copy_identifier: '識別子をコピー',
      profile_handle_registered_hint: 'あなたの識別子に登録済みです。これで他の人があなたを見つけられます。',
      profile_handle_change: '変更',
      profile_handle_claim: '{{handle}} を取得',
      profile_handle_replace_warning:
        '取得すると {{handle}} は置き換わります。あなたを保存した人の連絡先は残りますが、古いハンドルはあなたに繋がらなくなります。',
      profile_identifier_section: 'あなたの識別子',
      profile_show_qr: 'QRで表示',
      profile_show_qr_hint: '誰かにスキャンしてもらい、あなたのプロフィールを渡します',
      profile_qr_callout:
        'これをスキャンすると BSV Wallet が開き、名前・ハンドル・写真つきであなたが連絡先に追加されます。支払いは求めません。支払い用のコードは「受け取る」にあります。',
      pay_review_title: '確認',
      pay_send_amount: '{{amount}} を送金',
      pay_sent_to: '{{name}} へ',
      pay_save_as_contact: '連絡先として保存'
    }
  },
  pl: {
    translation: {
      token_balances_header: 'Salda',
      wallet_coin_switcher: 'Wybierz, którą monetę pokazać',
      wallet_activity_empty_asset: 'Brak aktywności {{ticker}}',
      pay_direction_pay_asset: 'Zapłać w {{ticker}}',
      token_fee_footer: 'Wysyłanie wymaga odrobiny BSV na opłatę sieciową. Odbieranie nie.',
      token_new_tap: 'Nowe — dotknij, aby zobaczyć emitenta',
      token_unresolved: 'Nierozpoznany token',
      token_unresolved_sub: 'Ten portfel nie mógł sprawdzić emitenta. Spróbuj ponownie, będąc online.',
      token_paused_short: 'Transfery wstrzymane',
      token_frozen_short: '{{amount}} zamrożone',
      token_sheet_issuer: 'Emitent',
      token_sheet_issuer_unknown: 'Nie zidentyfikowano',
      token_sheet_asset_id: 'Identyfikator tokena',
      token_sheet_powers_header: 'Co może zrobić {{issuer}}',
      token_power_pause: 'Wstrzymać wszystkie transfery {{ticker}}',
      token_power_freeze: 'Zamrozić wybrane monety, także Twoje',
      token_power_admit: 'Decydować, kto może posiadać {{ticker}}',
      token_power_replace: 'Zastąpić zamrożoną monetę nową',
      token_powers_limit:
        'Nie mogą wyprowadzić {{ticker}} z Twojego portfela bez Ciebie, a zamrożonej monety po prostu nie da się wydać.',
      token_frozen_sub:
        '{{issuer}} zamroził {{amount}} {{ticker}} Twojego salda. Skontaktuj się z nim, aby to zakwestionować.',
      pay_asset_label: 'Płacisz w',
      pay_asset_label_get: 'Otrzymujesz w',
      pay_asset_cta: 'Wyślij {{amount}} {{ticker}}',
      pay_asset_over_balance: 'Więcej niż Twoje saldo {{ticker}}.',
      pay_asset_needs_bsv: 'Wysłanie {{ticker}} wymaga odrobiny BSV na opłatę sieciową. Odbieranie nie.',
      pay_asset_get_bsv: 'Zdobądź BSV',
      pay_asset_needs_bsv_short: 'Potrzebne BSV na opłatę',
      pay_asset_frozen_note: '{{amount}} {{ticker}} Twojego salda jest zamrożone i nie można go wysłać.',
      pay_asset_paused: '{{issuer}} wstrzymał transfery {{ticker}}, więc teraz nie można tego wysłać.',
      pay_asset_no_address: '{{ticker}} można wysłać tylko do osoby lub urządzenia w pobliżu — nie na adres Bitcoin.',
      pay_asset_address_status: 'Niedostępne dla {{ticker}}',
      pay_asset_self_unregistered:
        '{{issuer}} nie zarejestrował Cię do wysyłania {{ticker}}. Nadal możesz w nim otrzymywać płatności.',
      pay_asset_unidentified: 'Ten portfel nie rozpoznał tego tokena, więc nie może go jeszcze wysłać.',
      pay_asset_recipient_unregistered:
        '{{issuer}} nie zarejestrował jeszcze tej osoby dla {{ticker}}, więc nie można jej w nim zapłacić.',
      pay_asset_recipient_blocked: '{{issuer}} nie zezwala na płatności do tej osoby.',
      token_err_refused: '{{issuer}} odrzucił ten transfer. Nic nie wysłano, a Twoje {{ticker}} pozostaje bez zmian.',
      token_err_refused_paused: '{{issuer}} wstrzymał {{ticker}}. Nic nie wysłano, a Twoje saldo pozostaje bez zmian.',
      token_err_refused_recipient:
        '{{issuer}} nie zezwala na płatności do tej osoby. Nic nie wysłano, a saldo pozostaje bez zmian.',
      token_err_refused_frozen:
        'Część {{ticker}}, którą wysyłałeś, została właśnie zamrożona. Nic nie wysłano; saldo zostało zaktualizowane.',
      token_err_send_failed: 'Nie udało się wysłać tej płatności: {{reason}}. Nic nie opuściło Twojego portfela.',
      token_err_reason_unknown: 'nieznany problem',
      token_err_unreachable:
        'Nie udało się połączyć z {{issuer}}, aby potwierdzić ten transfer. Sprawdź saldo {{ticker}}, zanim spróbujesz ponownie.',
      token_err_check_again: 'Sprawdź ponownie',
      token_err_balance_changed:
        'Twoje saldo {{ticker}} zmieniło się w trakcie przygotowania. Sprawdź kwotę i spróbuj ponownie.',
      token_err_busy: 'Płatność w {{ticker}} jest już wysyłana.',
      pay_sent_not_broadcast: 'Wysłano. Jeszcze nie dotarło do sieci — będziemy próbować dalej.',
      pay_sent_not_notified:
        'Wysłano. Nie udało się jeszcze powiadomić odbiorcy — zobaczy to przy następnym sprawdzeniu portfela.',
      pay_sent_handed_to_wallet: 'Wysłano. Odbiorca nie odebrał osobiście, więc wysłaliśmy płatność do jego portfela.',
      token_first_hold: '{{ticker}} jest emitowany przez {{issuer}}, który może wstrzymać transfery i zamrozić monety.',
      pay_asset_request_paused: '{{issuer}} wstrzymał {{ticker}}, więc płatność może jeszcze nie dotrzeć.',
      token_admit_title: 'Nie możesz jeszcze otrzymywać płatności w {{ticker}}',
      token_admit_body:
        '{{issuer}} wysyła {{ticker}} tylko do zarejestrowanych osób. Do tego czasu płatności do Ciebie są odrzucane, zanim opuszczą portfel nadawcy.',
      token_admit_bsv_instead: 'Zamiast tego otrzymaj płatność w BSV',
      token_row_sent: 'Wysłano {{ticker}}',
      token_row_received: 'Otrzymano {{ticker}}',
      token_row_amount_pending: 'Kwota niedostępna',
      token_evicted_title: 'Część {{ticker}} została wycofana',
      token_evicted_body: '{{issuer}} wycofał {{amount}} {{ticker}} z Twojego portfela.',
      local_pay_token_not_cleared: 'Odebrano offline · jeszcze nie potwierdzone przez {{issuer}}',
      pay_asset_nearby_needs_network:
        'Płatność w {{ticker}} na miejscu wymaga połączenia. Bez niego możesz zapłacić w BSV.',
      pay_asset_link_not_held:
        'To żądanie dotyczy tokena, którego ten portfel nie posiada. Nadal możesz zapłacić w BSV.',
      pay_asset_nearby_wrong_overlay: 'To żądanie dotyczy tokena, dla którego ten portfel nie jest skonfigurowany.',
      token_messagebox_mismatch:
        'Twój portfel jest ustawiony na inną skrzynkę wiadomości, więc płatności tokenami mogą do Ciebie nie dotrzeć.',
      token_issuer_fallback: 'emitent',
      token_row_hint: 'Otwiera emitenta i jego uprawnienia',
      token_attention_one: 'Czekamy na potwierdzenie od {{issuer}} — dotknij po szczegóły',
      token_attention_many: '{{count}} płatności tokenami czeka na rozliczenie — dotknij po szczegóły',
      token_recv_reversed: 'Ta płatność nie mogła zostać potwierdzona przez {{issuer}} i została cofnięta.',
      token_settle_paused_retry: '{{issuer}} właśnie wstrzymał {{ticker}} — będziemy próbować dalej.',
      token_settle_unreachable_retry:
        'Nie udało się połączyć z {{issuer}}, aby potwierdzić ten transfer. Będziemy próbować dalej.',
      token_settle_evicted:
        'Ta płatność nie dotarła do sieci na czas i została cofnięta — Twoje {{ticker}} wróciło na saldo.',
      token_frame_unreadable: 'Nie udało się odczytać otrzymanej płatności — poproś nadawcę o ponowne wysłanie.',
      token_chain_reversed:
        '{{issuer}} odrzucił wcześniejszą płatność w tym łańcuchu, więc ta również została cofnięta.',
      token_settle_stuck_bound:
        'Ta płatność czeka na rozliczenie ponad 3 dni. Możesz czekać dalej lub skontaktować się z {{issuer}}.',
      token_cover_failed: 'Ta płatność nie zgadza się z zapisami {{issuer}} i nie została przyjęta.',
      token_sent_settling:
        'Wysłano. Rozliczanie z {{issuer}} — potwierdzenie pojawi się, gdy Ty lub {{payee}} ponownie się połączycie.',
      token_sent_settling_unnamed:
        'Wysłano. Rozliczanie z {{issuer}} — potwierdzenie pojawi się, gdy któreś z was wróci do sieci.',
      token_sent_settled: 'Wysłano · rozliczone z {{issuer}}',
      token_status_settling: 'Rozliczanie',
      token_status_settled: 'Rozliczone',
      token_status_refused: 'Odrzucone',
      token_status_reversed: 'Cofnięte',
      token_status_stuck: 'Czeka na rozliczenie',
      // Browser UI
      new_tab: 'Nowa karta',
      untitled: 'Bez tytułu',
      share: 'Udostępnij',
      back_to_homepage: 'Wróć na stronę główną',
      bookmarks: 'Zakładki',
      add_bookmark: 'Dodaj zakładkę',
      bookmark: 'Zakładka',
      unbookmark: 'Usuń zakładkę',
      find_in_page: 'Znajdź na stronie',
      find_in_page_placeholder: 'Znajdź na stronie...',
      find_in_page_matches: '{{current}} z {{total}}',
      find_in_page_no_matches: 'Brak wyników',
      history: 'Historia',
      clear_all: 'Wyczyść wszystko',
      clear_all_tabs: 'Zamknij wszystkie karty',
      search_or_enter_website: 'Szukaj lub wpisz adres strony',
      browser: 'Przeglądarka',
      bug_report: 'Zgłoś błąd',
      enable_web3: 'Włącz Web3',
      tabs: 'Karty',
      unlock_web3_features: 'Odblokuj funkcje Web3',

      // General actions
      cancel: 'Anuluj',
      done: 'Gotowe',
      discard: 'Odrzuć',
      deny: 'Odmów',
      allow: 'Zezwól',
      save: 'Zapisz',
      remove: 'Usuń',
      delete: 'Usuń',
      search: 'Szukaj',
      loading: 'Ładowanie...',
      continue: 'Kontynuuj',
      go_back: 'Wróć',
      continue_action: 'Kontynuuj',
      camera_denied_description:
        'Dostęp do kamery został zablokowany. Możesz go włączyć w Ustawieniach, aby korzystać z tej funkcji.',
      open_settings: 'Otwórz Ustawienia',
      copied: 'Skopiowano',
      explorer_open_failed: 'Nie można otworzyć eksploratora bloków',
      tx_action_refresh: 'Odśwież status',
      tx_action_refresh_short: 'Status',
      tx_action_abort: 'Anuluj transakcję',
      copy: 'Kopiuj',
      copy_to_clipboard: 'Kopiuj do schowka',
      details: 'Szczegóły',
      hide_details: 'Ukryj szczegóły',

      // Navigation
      identity: 'Tożsamość',
      settings: 'Ustawienia',
      wallet: 'Portfel',
      activity: 'Aktywność',
      configuration: 'Konfiguracja',
      data_and_security: 'Dane i bezpieczeństwo',
      storage_row: 'Pamięć',
      storage_title: 'Pamięć portfela',
      storage_db_size: 'Baza danych: {{size}}',
      storage_free_space: 'Wolne miejsce: {{size}}',
      storage_reclaimable: 'Do odzyskania: {{size}} w {{count}} transakcjach',
      storage_held_back: '{{count}} transakcji wstrzymanych — nadal w użyciu',
      storage_reclaim_explain:
        'Odzyskiwanie usuwa dane walidacji, których portfel już nie potrzebuje. Transakcje, salda i historia pozostają nietknięte. Zwalnia miejsce wewnątrz pliku bazy danych; sam plik zmniejszy się po kompaktowaniu przez system.',
      storage_reclaim_confirm: 'Odzyskaj',
      storage_reclaimed: 'Odzyskano {{count}} transakcji',
      storage_failed: 'Nie udało się odczytać użycia pamięci.',
      storage_unavailable: 'Pamięć nie jest jeszcze gotowa.',
      backup_push_section: 'Prywatna kopia',
      backup_push_toggle: 'Kopia na serwerze',
      backup_erase_row: 'Usuń kopię z serwera',
      backup_erase_title: 'Usunąć kopię zapasową?',
      backup_erase_message:
        'Wszystko, co ten portfel wysłał na serwer kopii, zostanie trwale usunięte, na wszystkich urządzeniach. Jednocześnie kopie zostaną wyłączone. Monety, klucze i fraza odzyskiwania pozostają nietknięte — ale nowe urządzenie nie odtworzy już historii tego portfela, a część twoich obecnych monet nie będzie tam do wydania.',
      backup_erase_confirm: 'Usuń',
      backup_erase_done: 'Kopia usunięta — usunięto {{count}} elementów',
      backup_erase_failed: 'Nie udało się usunąć kopii. Nic nie zostało usunięte — spróbuj ponownie.',
      backup_erase_no_key: 'Brak dostępnego klucza portfela',
      backup_push_disclosure:
        'Zaszyfrowana kopia bazy danych portfela jest wysyłana na serwer BSV Association, abyś mógł odzyskać portfel na nowym urządzeniu. Jest zaszyfrowana kluczem, który wyprowadza tylko Twoja fraza odzyskiwania — operator nie może jej odczytać. Fraza lub wydrukowane udziały pozostają niezbędne: bez nich nikt nie otworzy kopii, my również.',
      backup_push_off_title: 'Wyłączyć prywatną kopię?',
      backup_push_off_message:
        'Sama fraza odzyskiwania nie odtworzy tego portfela. Informacje związane z zabezpieczeniem twoich monet istnieją tylko w bazie danych, więc z wyłączoną kopią nowe urządzenie odtworzy frazę, ale nie środki. Nic, co już jest na serwerze, nie zostaje usunięte.',
      backup_push_off_confirm: 'Wyłącz',
      backup_push_on_toast: 'Kopie włączone — nadrabiam zaległości',
      backup_push_off_toast: 'Kopie wyłączone — nic więcej nie zostanie wysłane',
      permissions: 'Uprawnienia',
      trust_network: 'Sieć zaufania',
      // vault (machine-translated, needs native review)
      vault_title: 'Sejf',
      vault_row_title: 'Sejf',
      vault_hero_title: 'Bezpieczne środki',
      vault_hero_body:
        'Zimne przechowywanie na długi termin, zablokowane Twoimi własnymi kluczami YubiKey. Wymaga dwóch lub więcej YubiKey 5 NFC.',
      vault_unsupported_title: 'Wymaga YubiKey',
      vault_unsupported_body: 'Dwa lub więcej YubiKey 5 NFC oraz telefon, który potrafi je odczytać.',
      vault_restore_from_chain: 'Przywróć sejf z blockchaina',
      vault_restore_scanning: 'Skanowanie blockchaina…',
      vault_restore_found: 'Znaleziono {{count}} depozyt(ów) sejfu w blockchainie.',
      vault_restore_pending:
        'Znaleziono {{count}} depozyt(ów), oczekujących na pierwsze potwierdzenie — spróbuj ponownie za chwilę.',
      vault_restore_none_found: 'Nie znaleziono żadnych depozytów sejfu w blockchainie dla tego portfela.',
      vault_balance_label: 'sejf zawiera',
      vault_key_section: 'Klucze bezpieczeństwa ({{count}} z 5)',
      vault_manage_section: 'Zarządzaj',
      vault_deposit_cta: 'Wpłać',
      vault_withdraw_cta: 'Wypłać',
      vault_deposit_title: 'Wpłać do sejfu',
      vault_withdraw_title: 'Wypłać z sejfu',
      vault_deposit_sub: 'Z Twojego codziennego salda. YubiKey nie jest potrzebny.',
      vault_withdraw_sub: 'Z powrotem na codzienne saldo. Przyłóż dowolny z kluczy sejfu.',
      vault_deposit_done: 'Przeniesiono do sejfu',
      vault_reclaim_done: 'Środki odzyskane z przerwanej wpłaty do sejfu',
      vault_withdraw_done: 'Wypłacono z sejfu',
      vault_withdraw_partial: 'Pozostało jeszcze {{count}} wpłat — wypłać ponownie, aby je przenieść.',
      vault_withdraw_reason: 'Przyłóż tutaj klucz YubiKey, aby podpisać — wypłać {{amount}} sats z sejfu',
      // ── R1C vault: not-released state, wizard, key management, transfers ──
      vault_not_released_body: 'Jeszcze niedostępne — wpłaty do sejfu są wyłączone w tym wydaniu.',
      vault_not_on_mainnet_body:
        'Sejf jest dostępny wyłącznie w sieci głównej. Aby z niego korzystać, przełącz portfel z powrotem na sieć główną.',
      vault_intro_title: 'Skonfiguruj swój sejf',
      vault_intro_what:
        'Sejf zamyka pieniądze Twoimi własnymi kluczami YubiKey. Żaden fragment klucza sejfu nigdy nie istnieje na tym telefonie; otwiera go dowolny z Twoich kluczy.',
      vault_intro_two_keys: 'Potrzebujesz co najmniej dwóch kluczy YubiKey.',
      vault_intro_apart: 'Trzymaj je w różnych miejscach — dwa klucze przechowywane razem to jeden klucz.',
      vault_intro_backup:
        'Zostaw włączoną zaszyfrowaną kopię zapasową portfela — zachowuje ona uwierzytelniony zapis odzyskiwania.',
      vault_intro_ack:
        'Rozumiem: ten sejf otwierają tylko moje klucze YubiKey. Moja fraza odzyskiwania — nie. Jeśli stracę je wszystkie, pieniądze przepadną.',
      vault_intro_piv_ack:
        'Potwierdzam, że cała aplikacja PIV tego YubiKey została przywrócona do ustawień fabrycznych albo jest przeznaczona wyłącznie do sejfu. Konfiguracja zmienia globalny PIN, PUK i klucz zarządzania PIV, może wpłynąć na wszystkie sloty PIV, a nowy PUK zapiszę w bezpiecznym miejscu.',
      vault_intro_begin: 'Rozpocznij',
      vault_key_step_title: 'Klucz {{k}} z maks. 5',
      vault_key_step_replace:
        'Użyj klucza YubiKey zresetowanego fabrycznie i przeznaczonego dla sejfu. Jeśli w gnieździe sejfu jest stary klucz, konfiguracja ostrzeże przed jego zastąpieniem.',
      vault_pin_choose_title: 'Wybierz PIN',
      vault_pin_choose_sub: 'Będziesz go podawać przy każdym użyciu tego klucza. Od 6 do 8 cyfr.',
      vault_pin_confirm_label: 'Wpisz go ponownie',
      vault_pin_mismatch: 'Te dwa PIN-y nie są takie same.',
      vault_pin_not_default: 'Wybierz coś innego niż 123456.',
      vault_puk_title: 'Twój kod odzyskiwania',
      vault_puk_body:
        'Zapisz go i trzymaj w bezpiecznym miejscu. To jedyny sposób na odblokowanie tego klucza po zapomnieniu PIN-u, a nie zostanie pokazany ponownie.',
      vault_puk_ack: 'Zapisałem go',
      vault_reset_offer: 'Zresetuj ten klucz',
      vault_reset_title: 'Zresetować ten klucz?',
      vault_reset_body:
        'Ten klucz był już używany. Reset kasuje każdy certyfikat i klucz w jego aplikacji PIV, nie tylko gniazdo sejfu. Wszystko, co na nim polega, przestanie działać.',
      vault_reset_ack: 'Rozumiem, że to skasuje całą zawartość tego klucza',
      vault_reset_unknown_ack:
        'To urządzenie nie może potwierdzić, czy ten klucz nadal zabezpiecza sejf innego profilu. Rozumiem, że skasowanie może zniszczyć ten klucz na zawsze.',
      vault_reset_own_draft_ack:
        'Ten klucz zawiera niedokończony klucz z Twojej wcześniejszej próby, a to urządzenie wciąż ma jej zapis. Rozumiem, że reset go skasuje i tamtej próby nie da się wznowić.',
      vault_reset_nothing_erased:
        'Nic nie skasowano. Gniazdo sejfu tego klucza nie jest puste, więc pozostaje jeszcze jedno do potwierdzenia.',
      vault_reset_confirm: 'Skasuj i zresetuj',
      vault_resetting: 'Resetowanie klucza…',
      vault_reset_done: 'Klucz zresetowany. Przyłóż go ponownie, aby kontynuować.',
      vault_reset_uncertain:
        'Reset się nie zakończył. Ten klucz mógł już zostać skasowany — sprawdź go, zanim na nim polegniesz.',
      vault_err_reset_enrolled: 'Ten klucz jest już częścią Twojego sejfu — reset zniszczyłby jeden z kluczy sejfu.',
      vault_err_reset_unreadable:
        'Nie udało się odczytać zapisów Twojego sejfu na tym urządzeniu, więc nie można dowieść, że ten klucz nie jest jednym z kluczy sejfu. Nic nie skasowano, a reset został odrzucony.',
      vault_enrollment_resume_pin_hint:
        'Wznowienie zapisanego klucza wymaga PIN-u wybranego dla niego wcześniej, nie nowego. Trzy błędne próby zablokują klucz.',
      vault_setup_step: 'Krok {{n}} z {{total}}',
      vault_enrollment_resume: 'Wznów zapisany klucz',
      vault_enrollment_reset_required:
        'Ten klucz YubiKey nie jest już w stanie fabrycznym, więc nie można go zarejestrować w obecnej postaci. Aby mógł przechowywać klucz sejfu, jego aplikacja PIV musi zostać wymazana i przywrócona do stanu fabrycznego.',
      vault_enrollment_state_uncertain:
        'Próba konfiguracji została przerwana, więc nie ma pewności, w jakim stanie pozostał klucz YubiKey. Ten klucz nie zostanie ponownie użyty, dopóki jego aplikacja PIV nie zostanie wymazana i przywrócona do stanu fabrycznego.',
      vault_nfc_enroll_message: 'Przyłóż tutaj klucz YubiKey, aby go skonfigurować',
      vault_nfc_adopt_message: 'Przyłóż tutaj odzyskany klucz YubiKey, aby go zweryfikować',
      vault_nfc_sign_batch: 'Przyłóż tutaj klucz YubiKey, aby podpisać — partia {{b}} z {{n}}',
      vault_key_use_different: 'Użyj innego klucza YubiKey',
      vault_key_setup_again: 'Skonfiguruj ponownie',
      vault_name_default: 'Klucz {{k}}',
      vault_more_title: 'Dodać kolejny klucz?',
      vault_more_body: 'Do 5 kluczy. Każdy z nich sam otwiera cały sejf.',
      vault_more_add: 'Dodaj kolejny klucz',
      vault_more_finish: 'Zakończ',
      vault_more_need_two: 'Dodaj drugi klucz przed zakończeniem — jeden klucz oznacza brak odzyskiwania.',
      vault_leave_setup: 'Opuść konfigurację',
      vault_leave_title: 'Opuścić konfigurację?',
      vault_leave_body:
        'Skonfigurowane klucze YubiKey ({{count}}) nie zostaną jeszcze zapisane. Zachowują swoje klucze, więc za chwilę możesz dodać je ponownie.',
      vault_leave_confirm: 'Opuść',
      vault_leave_stay: 'Zostań',
      vault_done_body:
        '{{count}} kluczy może otworzyć ten sejf. Otwierają go tylko te klucze — Twoja fraza odzyskiwania nie.',
      vault_done_cta: 'Gotowe',
      vault_add_key_row: 'Dodaj klucz',
      vault_add_key_done:
        '{{nickname}} może otwierać wpłaty dokonane od teraz. Zablokuj sejf ponownie, aby mógł otworzyć wszystko.',
      vault_key_added_toast: 'Klucz dodany',
      vault_recovery_verified: 'Odzyskany klucz zweryfikowano na tym urządzeniu',
      vault_relock_row: 'Zablokuj sejf ponownie',
      vault_relock_now: 'Zablokuj ponownie teraz',
      vault_relock_choose: 'Który klucz przyłożysz, aby zablokować ponownie?',
      vault_relock_reason: 'Przyłóż jeden z istniejących kluczy ({{names}}) — nie ten, który właśnie dodałeś.',
      vault_relock_reason_generic:
        'Przyłóż tutaj klucz YubiKey, aby podpisać — zablokuj sejf ponownie na obecne klucze',
      vault_relock_reason_remaining: 'Zablokuj ponownie tylko na pozostałych kluczach: {{names}}.',
      vault_key_pending_removal: 'Usunięty — zablokuj ponownie, aby zakończyć',
      vault_key_removal_awaiting_network: 'Usuwanie — czekamy, aż sieć przyjmie ponowną blokadę',
      vault_removal_awaiting_network: 'Ponowna blokada wysłana. Wpłaty i wypłaty wrócą, gdy sieć ją przyjmie.',
      vault_err_too_many_active_keys: 'Sejf można zablokować najwyżej pięcioma kluczami. Usuń jeden, zanim wpłacisz, zablokujesz ponownie lub wypłacisz część salda.',
      vault_key_section_over_limit: 'Klucze bezpieczeństwa ({{count}}, usuń jeden)',
      vault_add_key_done_over_limit: 'Dodano {{nickname}}. Sejf można zablokować najwyżej pięcioma kluczami, więc usuń klucz, który zastępuje, a potem zablokuj ponownie raz.',
      vault_relock_done: 'Sejf zablokowany ponownie na obecne klucze',
      vault_relock_capped: 'Pozostało jeszcze {{count}} wpłat — zablokuj ponownie, aby je przenieść.',
      vault_relock_unreachable:
        '{{count}} wpłat może otworzyć tylko {{names}}. Zablokuj ponownie jednym z tych kluczy.',
      vault_badge_missing: '{{count}} wpłat jeszcze nieotwartych dla {{nickname}}',
      vault_badge_removed: '{{count}} wpłat nadal otwartych dla usuniętego klucza',
      vault_key_action_rename: 'Zmień nazwę',
      vault_key_action_remove: 'Usuń',
      vault_rename_title: 'Zmień nazwę {{nickname}}',
      vault_rename_save: 'Zapisz',
      vault_remove_title: 'Usunąć {{nickname}}?',
      vault_remove_body:
        'Portfel przestanie używać {{nickname}}. Pieniądze już w sejfie pozostają dostępne dla tego klucza do ponownego zablokowania (≈ {{fee}} sat).',
      vault_remove_only: 'Tylko usuń',
      vault_key_removed_toast: 'Klucz usunięty',
      vault_export_explainer:
        'Każda wpłata do sejfu skrywa swoją sól, aż do jej wydania — nic o niej nie widać w skrypcie blokującym. Zostaw włączoną zaszyfrowaną kopię zapasową, aby nowe urządzenie mogło przywrócić i uwierzytelnić zapisy kluczy potrzebne do użycia Twoich YubiKey.',
      vault_footnote: 'Sejf otwierają tylko te klucze. Twoja fraza odzyskiwania — nie.',
      vault_floor_line:
        'Minimalna wpłata {{floorDisplay}} ({{floorSats}} sat). Utworzenie wpłaty do sejfu kosztuje około {{feeDisplay}}.',
      vault_first_deposit_title: 'Pierwsza wpłata do sejfu',
      vault_first_deposit_body:
        'Pierwsza wpłata do sejfu — {{amount}} będzie można otworzyć tylko {{count}} kluczami YubiKey ({{names}}). Twoja fraza odzyskiwania nie pomoże.',
      vault_backup_off_title: 'Włącz prywatną kopię',
      vault_backup_off_body:
        'Prywatna kopia musi być włączona, zanim portfel utworzy nowe wyjście sejfu. Przechowuje uwierzytelnione zapisy YubiKey i historię portfela potrzebne do odzyskania. Nadal możesz wypłacić wszystko bez tworzenia reszty w sejfie.',
      vault_backup_off_cta: 'Otwórz ustawienia kopii',
      vault_seed_not_preserved_title: 'Najpierw zapisz swoją frazę odzyskiwania',
      vault_seed_not_preserved_body:
        'Twój Skarbiec można odzyskać wyłącznie za pomocą frazy odzyskiwania i jednego z Twoich kluczy YubiKey. Klucze YubiKey nie przechowują jej kopii — jeśli zgubisz ją, zanim ją zapiszesz, tych środków nie da się odzyskać. Zapisz ją teraz, zanim dokonasz wpłaty.',
      vault_seed_not_preserved_cta: 'Zapisz frazę odzyskiwania',
      vault_choose_key: 'Który klucz przyłożysz?',
      vault_remainder_title: 'Wypłacić wszystko?',
      vault_remainder_body:
        'Wypłata {{amount}} pozostawia {{remainder}}, czyli mniej niż minimum sejfu 100 000 sat. Cały sejf przejdzie na Twoje codzienne saldo.',
      vault_remainder_all: 'Wypłać wszystko',
      vault_remainder_change: 'Zmień kwotę',
      vault_unreachable_title: 'Część sejfu wymaga innego klucza',
      vault_unreachable_body:
        'Część sejfu wymaga innego klucza — przeniesiono {{moved}}. {{count}} wpłat o wartości {{amount}} może otworzyć tylko {{names}}. Wypłać ponownie jednym z tych kluczy.',
      vault_sign_progress: 'Podpisano {{signed}} z {{total}}',
      vault_err_pin_locked_enroll:
        'PIN tego klucza YubiKey jest zablokowany. Odblokuj go kodem PUK w Yubico Authenticator lub skonfiguruj inny klucz YubiKey.',
      vault_err_serial_mismatch_chosen:
        'To {{tappedName}}. Wybrałeś {{chosenName}} — przyłóż go albo wróć i wybierz {{tappedName}}.',
      vault_err_key_already_enrolled: 'Ten klucz YubiKey ({{nickname}}) został już dodany. Przyłóż inny.',
      vault_err_not_released: 'Wpłaty do sejfu są wyłączone w tym wydaniu.',
      vault_err_not_on_mainnet:
        'Sejf jest dostępny wyłącznie w sieci głównej. Przełącz portfel na sieć główną i spróbuj ponownie.',
      vault_err_backup_off:
        'Włącz zaszyfrowaną prywatną kopię przed utworzeniem nowego wyjścia sejfu. Nadal możesz wypłacić wszystko.',
      vault_err_not_enough_keys: 'Ten sejf ma mniej niż dwa klucze. Skonfiguruj go ponownie.',
      vault_err_too_many_keys: 'Sejf może mieć najwyżej pięć kluczy.',
      vault_err_last_keys: 'Sejf wymaga co najmniej dwóch kluczy. Dodaj inny klucz, zanim usuniesz ten.',
      vault_err_relock_required: 'Najpierw zablokuj sejf ponownie, aby pozostałe klucze mogły otworzyć każdą wpłatę.',
      vault_err_action_pending: 'Transakcja sejfu jeszcze się nie zakończyła. Spróbuj ponownie, gdy się zakończy.',
      vault_resolve_held_deposit_action: 'Zakończ przerwany depozyt',
      vault_resolve_held_deposit_done: 'Problem z depozytem rozwiązany',
      vault_resolve_held_deposit_failed: 'Nie udało się rozwiązać problemu z depozytem. Spróbuj ponownie.',
      vault_resolve_held_deposit_none:
        'Nie ma tu nic do rozwiązania — sprawdź zakładkę Aktywność, aby ponowić lub anulować oczekującą transakcję.',
      vault_err_key_not_committed:
        '{{nickname}} nie może otworzyć żadnej wpłaty w tym sejfie. Użyj innego klucza sejfu.',
      vault_err_key_cannot_cover:
        '{{nickname}} może otworzyć {{reachable}} z {{total}} w sejfie. Wypłać do {{reachable}} albo użyj {{otherNames}}.',
      vault_err_too_small_to_relock:
        'Ten sejf zawiera mniej niż 100 000 sat — za mało, by zablokować go ponownie. Zamiast tego wypłać i wpłać ponownie.',
      vault_err_bad_version: 'Transakcja została zbudowana w złym formacie. Nic nie podpisano — spróbuj ponownie.',
      vault_err_chain_scan_failed:
        'Nie udało się zakończyć skanowania blockchainu w poszukiwaniu depozytów do Skarbca. Sprawdź połączenie i spróbuj ponownie.',
      vault_shares_unavailable: 'Brak materiału klucza do wydrukowania.',
      vault_enroll_begin: 'Skonfiguruj sejf',
      vault_enroll_phase_connecting: 'Odczytywanie klucza…',
      vault_enroll_phase_pin_check: 'Sprawdzanie klucza…',
      vault_enroll_phase_checking_slot: 'Sprawdzanie gniazda sejfu…',
      vault_enroll_phase_personalizing: 'Ustawianie PIN-u i kodu odzyskiwania…',
      vault_enroll_phase_generating: 'Tworzenie klucza sejfu — dotknij klucza, gdy zamiga',
      vault_enroll_phase_challenging: 'Weryfikowanie nowego klucza sejfu — dotknij klucza, gdy zamiga',
      vault_enroll_phase_done: 'Gotowe',
      vault_touch_when_blinks: 'Dotknij złotego styku, gdy klucz zamiga.',
      vault_nfc_activation_hint:
        'Pierwszy raz używasz tego YubiKey? Podłącz go najpierw do portu USB-C na kilka sekund — nowe klucze są wysyłane z wyłączonym NFC, dopóki to nie nastąpi.',
      vault_continue: 'Kontynuuj',
      vault_back: 'Wstecz',
      vault_enrolled_toast: 'Sejf gotowy',
      vault_insert_key: 'Włóż YubiKey',
      vault_hold_key_nfc: 'Przyłóż YubiKey do górnej części telefonu',
      vault_keep_holding_nfc: 'Trzymaj dalej — odblokowywanie…',
      vault_unlocking_funds: 'Odblokowywanie środków',
      vault_unlocking_sub: 'To zajmie kilka sekund. Nie zamykaj aplikacji.',
      vault_sending_to_network: 'Wysyłanie do sieci',
      vault_sending_sub: 'Jeśli nie przejdzie od razu, będziemy próbować dalej.',
      vault_reading_key: 'Odczytywanie klucza…',
      vault_enter_pin: 'Wpisz PIN klucza YubiKey',
      vault_pin_sub_nfc: 'Następnie przyłóż klucz do górnej części telefonu, aby zakończyć.',
      vault_pin_sub_usb: 'Następnie dotknij złotego styku, aby zakończyć.',
      vault_touch_contact: 'Dotknij złotego styku',
      vault_unlock_cta: 'Odblokuj',
      vault_pin_retries: 'Pozostało prób: {{count}}',
      vault_locked: 'Sejf zablokowany',
      vault_retry: 'Spróbuj ponownie',
      vault_cancel: 'Anuluj',
      vault_dismiss: 'Zamknij',
      vault_err_wrong_key: 'Ten klucz YubiKey nie jest jednym z kluczy tego sejfu.',
      vault_err_serial_mismatch: 'Ten klucz YubiKey nie jest jednym z kluczy tego sejfu ({{names}}).',
      vault_err_touch_timeout: 'Nie wykryto dotknięcia. Spróbuj ponownie i dotknij złotego styku.',
      vault_err_pin_locked:
        'PIN tego klucza YubiKey jest zablokowany. Użyj innego klucza sejfu albo odblokuj ten kodem PUK w Yubico Authenticator.',
      vault_err_pin_invalid: 'Nieprawidłowy PIN.',
      vault_err_puk_invalid: 'Nieprawidłowy PUK.',
      vault_err_puk_locked: 'PUK tego klucza YubiKey jest zablokowany. Użyj nowego klucza YubiKey.',
      vault_err_ceremony_active:
        'Inna operacja sejfu nadal używa klucza YubiKey. Poczekaj na jej zakończenie i spróbuj ponownie.',
      vault_err_scope_changed:
        'Podczas tej operacji zmienił się portfel lub sieć. Zacznij ponownie w bieżącym portfelu.',
      vault_err_key_removed_mid_op: 'Klucz został wyjęty. Włóż go i spróbuj ponownie.',
      vault_err_no_key: 'Nie wykryto YubiKey.',
      vault_err_driver_unavailable: 'Obsługa YubiKey jest niedostępna na tym urządzeniu.',
      vault_err_mgmt_key_custom: 'Ten klucz ma niestandardowy klucz zarządzania i nie może być użyty automatycznie.',
      vault_err_attestation_invalid:
        'Nie udało się zweryfikować atestacji producenta tego klucza YubiKey. Użyj oryginalnego, fabrycznie atestowanego klucza YubiKey z niezmienionym gniazdem atestacji PIV.',
      vault_err_generic: 'Coś poszło nie tak. Spróbuj ponownie.',
      vault_err_template_invalid: 'Coś poszło nie tak przy próbie dostępu do sejfu. Spróbuj ponownie.',
      vault_err_not_enrolled: 'Nie skonfigurowano żadnego sejfu.',
      vault_err_pin_required: 'Wprowadź PIN.',
      vault_err_unsupported_platform: 'Obsługa YubiKey jest niedostępna na tym urządzeniu.',
      vault_err_slot_occupied: 'Gniazdo klucza jest już zajęte.',
      vault_replace_key_warning:
        'Zastąpienie trwale niszczy istniejący klucz prywatny P-256. Środki w sejfie wymagające tego klucza mogą stać się niemożliwe do wydania.',
      vault_replace_key_confirm: 'Zastąp istniejący klucz',
      vault_resume_draft_confirm_title: 'Użyć tego przygotowanego klucza?',
      vault_resume_draft_confirm_body:
        'Wcześniej przygotowany klucz — {{nickname}} · …{{tail}} — jest gotowy do dodania, ale to urządzenie właśnie go nie zweryfikowało na karcie. Kontynuuj tylko wtedy, gdy go rozpoznajesz.',
      vault_resume_draft_confirm_use: 'Użyj tego klucza',
      vault_resume_draft_confirm_discard: 'Odrzuć go',
      vault_err_enrollment_partial:
        'Konfiguracja YubiKey zatrzymała się po zmianie klucza. Postępuj zgodnie z instrukcją ponowienia; klucz nie został dodany do sejfu.',
      vault_err_key_not_adopted:
        'Zweryfikuj posiadanie odzyskanego klucza YubiKey na tym urządzeniu przed użyciem go do wypłaty.',
      vault_err_too_many_inputs:
        'Zbyt wiele wpłat sejfu do przeniesienia naraz. Najpierw wypłać mniejszą kwotę — każda wypłata konsoliduje też sejf, więc następna przeniesie więcej.',
      vault_err_requires_online:
        'Przelewy sejfu wymagają połączenia. Połącz się ponownie i spróbuj — środki sejfu nigdy nie trafiają do kolejki.',
      vault_err_user_cancelled: 'Anulowano.',
      vault_err_vault_empty: 'W sejfie nie ma jeszcze środków do wypłaty.',
      vault_err_amount_exceeds_balance: 'To więcej, niż zawiera sejf.',
      vault_err_below_dust: 'Ta kwota jest zbyt mała, aby ją wysłać.',
      vault_err_no_transaction: 'Nie udało się dokończyć przelewu. Spróbuj ponownie.',
      vault_err_nfc_lost: 'Połączenie zostało przerwane. Trzymaj klucz nieruchomo przy telefonie.',
      vault_pin_invalid_retry:
        'Błędny PIN — pozostało prób: {{count}}. Dotknij ponownie Wypłać, aby spróbować jeszcze raz.',
      vault_pin_invalid_retry_generic: 'Błędny PIN. Dotknij ponownie Wypłać, aby spróbować jeszcze raz.',
      vault_disable_row: 'Wyłącz sejf',
      vault_disable_title: 'Wyłączyć sejf?',
      vault_disable_message: 'To usuwa listę kluczy sejfu z tego telefonu. Klucze pozostają na Twoich YubiKey.',
      vault_disable_confirm: 'Wyłącz',
      vault_disabled_toast: 'Sejf wyłączony',
      vault_disable_blocked_title: 'Najpierw wypłać środki',
      vault_disable_blocked_message:
        'W sejfie wciąż są środki. Przed wyłączeniem wypłać wszystko na saldo codzienne — inaczej te środki zostaną zablokowane bez możliwości ich podpisania.',
      vault_ok: 'OK',
      vault_mock_toggle: 'Użyj próbnego YubiKey (dev)',
      vault_on: 'Wł.',
      vault_off: 'Wył.',
      vault_shares_word_count:
        'Udziały odzyskiwania obsługują tylko frazy dwunastowyrazowe. Zapisz zamiast tego swoją frazę.',
      scan_shares_legacy_title: 'Starszy format kopii zapasowej',
      scan_shares_legacy_message:
        'Te udziały mają starszy format, który odtwarza saldo codzienne, ale nie otworzy sejfu. Jeśli nadal masz dwunastowyrazową frazę odzyskiwania, odtwórz portfel z niej — odzyskuje wszystko — a potem wydrukuj nowe udziały. Jeśli fraza przepadła, wyślij środki do nowego portfela i zacznij od nowa.',
      scan_shares_legacy_ack: 'Rozumiem',

      // Account
      delete_wallet: 'Usuń portfel',
      delete_wallet_warning_title: 'Usunąć portfel?',
      delete_wallet_warning_body:
        'Spowoduje to trwałe usunięcie kluczy portfela z tego urządzenia. Jeśli nie wykonałeś kopii zapasowej frazy odzyskiwania ani nie wydrukowałeś udziałów odzyskiwania, utracisz dostęp do portfela na zawsze.',
      delete_wallet_confirm: 'Usuń portfel',
      delete_wallet_failed_title: 'Nie udało się usunąć portfela',
      delete_wallet_failed_body: 'Nie udało się całkowicie usunąć portfela z tego urządzenia. Część danych klucza może nadal być obecna. Spróbuj ponownie.',
      bsv_network: 'Sieć BSV',
      arc_endpoint: 'Punkt końcowy ARC',
      arc_api_token: 'Token API',
      arc_custom_url: 'Własny adres URL',
      arc_apply: 'Zastosuj',
      arc_reset_default: 'Przywróć domyślne',
      arc_requires_token: 'wymaga tokenu',
      arc_url_https_required: 'Musi być https (lub http://localhost do testów lokalnych)',
      recovery_phrase: 'Klucz odzyskiwania',
      you_have: 'Posiadasz',
      switching: 'Przełączanie...',
      mainnet: 'Mainnet',
      testnet: 'Testnet',
      teratest: 'Teratest',
      identity_key: 'Identyfikator',
      transactions: 'Transakcje',

      // Transactions screen
      no_transactions: 'Brak transakcji',
      tx_status_confirmed: 'Potwierdzona',
      tx_status_received: 'Otrzymano',
      tx_status_sent: 'Wysłano',
      tx_status_spent: 'Wydano',
      tx_status_transferred: 'Przeniesiono',
      tx_status_accepted: 'Zaakceptowana',
      tx_status_broadcasting: 'Przetwarzanie',
      tx_status_not_sent: 'Niewysłana',
      tx_status_unsigned: 'Szkic',
      tx_status_nonfinal: 'Oczekująca',
      tx_status_pending: 'Oczekująca',
      tx_status_failed: 'Nieudana',
      tx_status_offline_queued: 'Offline · w kolejce',
      tx_status_offline_sending: 'Offline · wysyłanie',
      tx_status_offline_rejected: 'Offline · odrzucona',
      tx_copied: 'Transakcja skopiowana',
      tx_not_available: 'Transakcja niedostępna',
      tx_copy_failed: 'Nie udało się skopiować transakcji',
      tx_abort_success: 'Transakcja przerwana',
      tx_abort_failed: 'Nie udało się przerwać transakcji',
      tx_abort_maybe_delivered: 'Nie można anulować — ta płatność mogła już dotrzeć do odbiorcy',
      tx_proof_refreshed: 'Transakcja potwierdzona',
      tx_proof_refresh_failed: 'Nie udało się pobrać dowodu — transakcja może nie być jeszcze potwierdzona',
      tx_marked_failed: 'Nieprzetworzona — oznaczono jako nieudaną, środki zwolnione',
      tx_still_pending: 'Wciąż oczekuje — jeszcze niepotwierdzona',
      tx_action_explorer: 'Zobacz w eksploratorze bloków',
      tx_export_success: 'Wyeksportowano transakcje: {{count}}',
      tx_export_failed: 'Nie udało się wyeksportować transakcji',
      tx_export_csv: 'Eksportuj CSV',
      tx_export: 'Eksportuj',
      activity_filter: 'Filtruj aktywność',
      activity_filter_all: 'Wszystko',
      activity_search_placeholder: 'Szukaj w notatkach, nazwach, @pseudonimach',
      activity_search_clear: 'Wyczyść wyszukiwanie',
      activity_filter_no_match: 'Brak pasującej aktywności',
      wallet_balance_label: 'Saldo',
      wallet_balance_you_have: 'Posiadasz',
      wallet_balance_refresh: 'Odśwież saldo',
      wallet_group_today: 'Dziś',
      wallet_group_yesterday: 'Wczoraj',
      back: 'Wstecz',
      wallet_payments: 'Płatności',
      wallet_vault: 'Sejf',
      wallet_settings: 'Ustawienia',
      wallet_activity: 'Aktywność',

      // Identity / Payments screen
      message_box_server: 'Serwer skrzynki wiadomości',
      message_box_required: 'Wymagany do wysyłania i odbierania płatności opartych na tożsamości.',
      send_payment: 'Wyślij płatność',
      recipient: 'Odbiorca',
      amount: 'Kwota',
      enter_valid_amount: 'Wpisz kwotę większą od zera',
      display_currency: 'Waluta wyświetlania',
      no_pending_payments: 'Brak oczekujących płatności',
      outgoing_payments: 'Wychodzące',
      payment_not_delivered: 'Nie dostarczono',
      payment_delivered: 'Dostarczono',
      retry: 'Ponów',
      retry_failed: 'Ponowienie nie powiodło się',
      dismiss: 'Odrzuć',
      valid_identity_key: 'Wprowadzono prawidłowy identyfikator',
      valid_bsv_address: 'Wprowadzono prawidłowy adres BSV',
      searching: 'Wyszukiwanie...',
      unknown: 'Nieznany',
      from_prefix: 'Od:',
      message_box_saved: 'Serwer skrzynki wiadomości zapisany',
      message_box_removed: 'Serwer skrzynki wiadomości usunięty',
      message_box_off: 'Wył.',
      message_box_off_hint: 'Serwer skrzynki wiadomości jest wyłączony. Włącz go w Ustawienia › Zaawansowane.',
      enter_valid_url: 'Wprowadź prawidłowy adres URL',
      checking_connection: 'Sprawdzanie połączenia...',
      connection_failed: 'Połączenie nie powiodło się',
      recipient_placeholder: 'Nazwa, identyfikator lub adres BSV...',

      // Local Payments screen

      // Trust screen
      trust_updated: 'Sieć zaufania zaktualizowana',
      failed_to_save: 'Nie udało się zapisać',
      confirm_delete: 'Potwierdź usunięcie',
      confirm_delete_body: 'Czy na pewno chcesz usunąć tego certyfikatora?',
      order_certifiers_hint:
        'Uporządkuj certyfikatorów według priorytetu. Aplikacje będą wyświetlać wyżej ocenionych certyfikatorów jako pierwsze.',
      certifiers: 'Certyfikatorzy',
      no_certifiers: 'Brak certyfikatorów.',
      add_provider: 'Dodaj dostawcę',
      unsaved_changes: 'Masz niezapisane zmiany',
      duplicate_key_error: 'Podmiot z tym kluczem publicznym jest już na liście!',
      get_provider_details: 'Pobierz dane dostawcy',
      validate_details: 'Zweryfikuj dane',
      show_advanced: 'Pokaż zaawansowane',
      hide_advanced: 'Ukryj zaawansowane',
      description_length_error: 'Opis musi mieć od 5 do 50 znaków',
      enter_domain_hint: 'Wprowadź domenę dostawcy',
      enter_details_hint: 'Wprowadź dane dostawcy',

      // Legacy payments screen
      receive: 'Odbierz',
      send: 'Wyślij',
      generating_address: 'Generowanie adresu BSV...',
      imported: 'Zaimportowano',
      unable_to_generate_address: 'Nie można wygenerować adresu BSV',
      send_bsv: 'Wyślij BSV',
      recipient_address: 'Adres BSV odbiorcy',
      enter_bsv_address: 'Wprowadź adres BSV',

      // Recovery key / wallet setup screen
      wallet_data: 'Dane portfela',
      create_new_wallet: 'Utwórz nowy portfel',
      generate_recovery_phrase_caption: 'Wygeneruj nowy klucz odzyskiwania',
      import_existing_wallet: 'Importuj istniejący portfel',
      paste_recovery_phrase: 'Wklej klucz odzyskiwania',
      save_recovery_phrase_heading: 'Zapisz klucz odzyskiwania',
      save_recovery_phrase_btn: 'Zapisz klucz odzyskiwania',
      recovery_phrase_write_down: 'Zapisz te 12 słów w kolejności i przechowuj je w bezpiecznym miejscu.',
      acknowledgment_text:
        'Zapisałem swój klucz odzyskiwania i rozumiem, że jego utrata spowoduje całkowitą i trwałą utratę wszystkich powiązanych środków, tokenów i certyfikatów.',
      enter_recovery_words: 'Wpisz frazę 12 słów',
      restore_wallet_description: 'Wprowadź klucz odzyskiwania, aby przywrócić portfel',
      backup_reminder_title: 'Twój portfel nie ma kopii zapasowej',
      backup_reminder_body: 'Wyeksportuj klucze, aby nie stracić środków.',
      biometric_advisory_title: 'Zabezpiecz swój portfel',
      wallet_balance_show_in: 'Pokaż w {{unit}}',
      tx_detail_block: 'Blok',
      tx_detail_block_pending: 'Oczekuje na blok',
      biometric_advisory_body: 'Chroń swoje aktywa za pomocą Face ID lub odcisku palca',
      biometric_advisory_body_degraded:
        'To urządzenie nie ma skonfigurowanego Face ID ani odcisku palca, więc klucz Twojego portfela jest chroniony tylko kodem dostępu urządzenia.',
      import_prompt_title: 'Masz już portfel?',
      import_prompt_body: 'Odtwórz go z klucza odzyskiwania lub udziałów.',
      import_prompt_action: 'Importuj z kopii zapasowej',
      nearby_advisory_title: 'Łączenie w pobliżu',
      nearby_advisory_body:
        'Poprosimy Cię o zgodę na dostęp do Wi-Fi, Bluetooth lub NFC. Służy to wyłącznie do bezpośredniego połączenia z urządzeniem w pobliżu na potrzeby płatności — nigdy nie trafia na nasze serwery i nigdy nie służy do budowania Twojego profilu.',
      backup_reminder_now: 'Zrób kopię kluczy teraz',
      backup_reminder_import: 'Importuj z kopii zapasowej',
      restore_backup_checking: 'Szukanie zaszyfrowanej kopii zapasowej…',
      restore_backup_progress: 'Przywracanie historii — {{chunks}} z {{total}}…',
      restore_backup_failed_title: 'Nie udało się przywrócić historii',
      restore_backup_failed_message:
        'Twoja fraza odzyskiwania jest poprawna, ale zaszyfrowanej kopii nie udało się odtworzyć. Zwykle wystarcza ponowna próba. Kontynuacja bez niej daje działający portfel bez wcześniejszych transakcji, a część twoich obecnych monet nie będzie do wydania.',
      restore_backup_retry: 'Spróbuj ponownie',
      restore_backup_skip: 'Porzuć',
      restore_backup_unverified_title: 'Kopia zapasowa może być niekompletna',
      restore_backup_unverified_body:
        'Twój portfel został przywrócony, ale nie udało się potwierdzić, że kopia zapasowa na serwerze jest kompletna. Jeśli brakuje części historii, przywróć ponownie później, gdy drugie urządzenie zakończy przesyłanie.',
      import_wallet: 'Importuj portfel',

      // Permission sheet
      reject: 'Odrzuć',
      authorize: 'Autoryzuj',
      renewal: 'Odnowienie',
      spending_section: 'Wydatki',
      protocols_section: 'Protokoły',
      baskets_section: 'Koszyki',
      certificates_section: 'Certyfikaty',
      requested_fields: 'Żądane pola',

      // Amount input
      send_max: 'Wyślij maksimum',
      message_box_unreachable:
        'Nie można połączyć się z serwerem płatności. Sprawdź adres URL message box w ustawieniach.',
      available: 'dostępne',
      note: 'Notatka',
      note_placeholder: 'Za co? (opcjonalnie)',
      entire_wallet_balance: 'Całe saldo portfela',
      entire_vault_balance: 'Całe saldo sejfu',
      export_wallet_data: 'Eksportuj dane portfela',
      export_unencrypted_title: 'Ten plik nie jest zaszyfrowany',
      export_unencrypted_message:
        'Wyeksportowany plik to zwykła, niezaszyfrowana kopia danych Twojego portfela — w tym informacji o tożsamości i certyfikatach, historii transakcji i kontaktów. Każdy, kto zdobędzie ten plik, będzie mógł go odczytać. Udostępniaj go wyłącznie w zaufanym miejscu.',
      import_wallet_data: 'Importuj dane portfela',
      import_conflict_title: 'Baza danych już istnieje',
      import_conflict_message:
        'Baza danych portfela dla tej tożsamości już istnieje z nowszym znacznikiem czasu. Zawsze używana jest kopia zapasowa z najwyższym numerem w nazwie pliku.\n\nAby użyć tej starszej kopii zapasowej: wyeksportuj bieżące dane portfela, usuń i ponownie zainstaluj aplikację, a następnie zaimportuj tylko żądaną kopię zapasową.',
      import_confirm_title: 'Importuj bazę danych',
      import_confirm_message: 'Ta baza danych stanie się Twoim aktywnym portfelem. Kontynuować?',
      import_success: 'Baza danych zaimportowana pomyślnie. Ponowne uruchamianie portfela...',
      import_invalid_file: 'Nieprawidłowy plik kopii zapasowej',
      import_invalid_file_detail:
        'Ten plik nie jest prawidłową kopią zapasową bazy danych portfela. Oczekiwana nazwa pliku w formacie wallet-XXXXXXXX-mainnet-TIMESTAMP.db.',
      import_oversized_file: 'Plik kopii zapasowej jest zbyt duży',
      import_oversized_file_detail:
        'Ten plik jest większy, niż powinna być kopia zapasowa portfela, więc nie został odczytany. Wybierz prawdziwy plik kopii zapasowej bazy danych portfela.',
      import_untrusted_file: 'Nie można zaimportować tej kopii zapasowej',
      import_untrusted_file_detail:
        'Nie udało się zweryfikować, że ta kopia zapasowa należy do tego portfela, lub jej zawartości nie można było zaufać. Nie została zaimportowana.',
      import_anyway: 'Importuj mimo to',
      import_invalid_input_title: 'Nieprawidłowe dane wejściowe',
      import_invalid_input_message:
        'Wprowadź prawidłową frazę odzyskiwania (12–24 słowa) lub 64-znakowy szesnastkowy klucz prywatny.',
      import_setup_failed: 'Nie udało się skonfigurować portfela: {{error}}',
      create_wallet_refused: 'Nie można utworzyć portfela. Spróbuj ponownie.',
      create_wallet_failed: 'Nie udało się wygenerować mnemonika. Spróbuj ponownie.',

      // Backup shares
      or: 'lub',
      print_recovery_shares: 'Wydrukuj udziały odzyskiwania',
      scan_backup_shares: 'Skanuj udziały kopii zapasowej',
      scan_backup_shares_caption: 'Odzyskaj portfel z wydrukowanych udziałów',
      scan_shares_invalid_format: 'Nieprawidłowy format udziału. Zeskanuj prawidłowy kod QR udziału kopii zapasowej.',
      scan_shares_camera_needed: 'Wymagany dostęp do kamery',
      scan_shares_camera_description:
        'Dostęp do kamery jest wymagany do skanowania kodów QR wydrukowanych udziałów kopii zapasowej.',
      scan_shares_grant_camera: 'Przyznaj dostęp do kamery',
      scan_shares_recovering: 'Odzyskiwanie portfela...',
      scan_shares_scan_first: 'Zeskanuj pierwszy udział kopii zapasowej',
      scan_shares_progress: '{{scanned}} z {{needed}} udziałów zeskanowanych',
      scan_shares_remaining: 'Potrzeba jeszcze {{count}} udziałów',
      scan_shares_complete: 'Wszystkie udziały zebrane!',
      scan_shares_recovery_failed: 'Odzyskiwanie nie powiodło się. Spróbuj zeskanować ponownie.',
      scan_shares_threshold_mismatch: 'Próg nie zgadza się z poprzednimi udziałami',
      scan_shares_integrity_mismatch: 'Suma kontrolna integralności nie zgadza się — udziały pochodzą z różnych kluczy',
      scan_shares_duplicate: 'Ten udział został już zeskanowany',
      scan_shares_biometric_title: 'Wymagany dostęp biometryczny',
      scan_shares_biometric_message:
        'Dostęp biometryczny jest potrzebny, aby chronić klucze portfela. Spróbuj ponownie.',
      recovery_replace_wallet_title: 'Zastąpić portfel na tym telefonie?',
      recovery_replace_wallet_body:
        'Ten telefon już przechowuje portfel. Kontynuowanie zastąpi go razem z wszystkim, co może wydać. Jeśli jego fraza odzyskiwania nie jest zapisana gdzie indziej, te pieniądze przepadną na zawsze.',
      recovery_replace_wallet_confirm: 'Zastąp portfel',
      invalid_bsv_address: 'Nieprawidłowy adres BSV',
      scan_recipient_hint: 'Skieruj kamerę na kod płatności',
      scan_qr_default_hint: 'Skieruj kamerę na kod QR',
      scan_wallet_qr_hint: 'Skieruj kamerę na kod QR parowania portfela',

      // Biometric prompts
      biometric_store_wallet: 'Bezpiecznie zapisz portfel',
      biometric_load_wallet: 'Bezpiecznie wczytaj portfel',
      biometric_unlock_wallet: 'Odblokuj portfel',
      wallet_lost_title: 'Klucz portfela został zniszczony',
      wallet_lost_body:
        'To urządzenie zmieniło ustawienia biometryczne, więc system zniszczył klucz chroniący ten portfel. Twoje monety są bezpieczne w blockchainie — przywróć ten portfel za pomocą frazy odzyskiwania.',
      wallet_lost_restore: 'Przywróć',
      wallet_lost_later: 'Nie teraz',
      wallet_locked_title: 'Portfel zablokowany',
      wallet_locked_body: 'Odblokuj, aby wczytać klucze portfela.',
      wallet_unlock_retry: 'Odblokuj portfel',
      wallet_lockout_body: 'Zbyt wiele nieudanych prób. Odblokuj telefon kodem, a następnie spróbuj ponownie.',
      wallet_no_biometrics_body:
        'To urządzenie nie ma skonfigurowanej biometrii. Klucze portfela są nadal szyfrowane przez magazyn kluczy urządzenia, ale nie chroni ich weryfikacja biometryczna.',

      // Connections screen
      connections: 'Połączenia',
      advanced: 'Zaawansowane',
      copy_secret_words: 'Kopiuj tajne słowa',
      print_recovery_keys: 'Drukuj klucze odzyskiwania',
      backup_wallet_keys: 'Utwórz kopię zapasową kluczy portfela',
      log_out: 'Wyloguj',
      connect_to_app: 'Połącz z aplikacją',
      no_connections_yet: 'Brak połączeń',
      no_connections_subtitle: 'Zeskanuj kod QR z aplikacji desktopowej, aby połączyć portfel',
      connections_active: 'Aktywne',
      connections_disconnected: 'Rozłączone',
      disconnect: 'Rozłącz',
      reconnect: 'Połącz ponownie',
      reconnect_failed: 'Ponowne połączenie nie powiodło się',
      reconnect_confirm_title: 'Połączyć ponownie z tą witryną?',
      reconnect_confirm_message: '{{origin}} będzie ponownie mieć dostęp do Twojego portfela.',
      reconnect_needs_reapproval: 'Nie można zweryfikować tego połączenia. Zeskanuj nowy kod QR, aby połączyć się ponownie.',
      scan_qr_code: 'Skanuj kod QR',
      paste_uri: 'Wklej URI',
      invalid_qr_code: 'Nieprawidłowy kod QR',
      wallet_not_ready: 'Portfel nie jest gotowy',
      please_log_in_first: 'Najpierw się zaloguj',
      unknown_error: 'Nieznany błąd',

      // Pay screen
      payments: 'Płatności',
      pay: 'Płatność',
      pay_direction_pay: 'Zapłać',
      pay_direction_receive: 'Przyjmij płatność',
      pay_cell_nearby_pay: 'Ktoś w pobliżu',
      pay_cell_nearby_pay_sub: 'Zeskanuj jego kod',
      pay_cell_handle_pay: 'Ktoś zdalnie',
      pay_cell_handle_pay_sub: 'Wybierz nazwę — potrzebują tej aplikacji',
      pay_cell_address_pay: 'Na adres BSV',
      pay_cell_address_pay_sub: 'Wklej lub zeskanuj adres BSV',
      pay_cell_nearby_get: 'Ktoś w pobliżu',
      pay_cell_nearby_get_sub: 'Pokaż swój kod płatności',
      pay_cell_handle_get: 'Ktoś zdalnie',
      pay_cell_handle_get_sub: 'Udostępnij swoją nazwę',
      pay_cell_address_get: 'Na adres BSV',
      pay_cell_address_get_sub: 'Pokaż adres BSV',
      pay_method: 'Metoda',
      pay_method_nearby: 'W pobliżu',
      pay_method_remote_link: 'Udostępnij link zdalny',
      pay_method_address: 'Na adres BSV',
      pay_pre_nearby: 'Oboje jesteście na miejscu.',
      pay_pre_handle: 'Ta osoba używa tej aplikacji.',
      pay_pre_address: 'Masz jej adres BSV.',
      pay_conseq_nearby: 'Rozlicza się w kilka sekund.',
      pay_conseq_handle: 'Dotrze, gdy jej portfel następny raz sprawdzi wiadomości.',
      pay_conseq_address: 'Pieniądze zostają wysłane — odbiorca nie dostaje powiadomienia.',
      paid: 'Zapłacono',
      pay_inbox_attention: 'Nie udało się dodać',
      pay_inbox_attention_hint:
        'Dotarły, ale portfel nie mógł ich dodać. Spróbuj ponownie lub odrzuć, aby zrezygnować z nich na zawsze.',
      pay_dismiss_confirm: 'Dotknij ponownie, aby zrezygnować',
      pay_dismissed: 'Zrezygnowano z płatności',
      payment_arrived: 'Płatność otrzymana',
      payment_arrived_damaged: 'Ta płatność dotarła uszkodzona',
      discard_payment_title: 'Odrzucić tę płatność?',
      discard_payment_body:
        'Nadawca zostanie poproszony o ponowne przesłanie szczegółów. Do tego czasu środki pozostają w łańcuchu.',
      discard_void_title: 'Ta płatność nie doszła do skutku',
      discard_void_body: 'Została anulowana lub środki wydano gdzie indziej. Można ją bezpiecznie odrzucić.',
      waiting_network_confirm: 'Czekamy, aż sieć potwierdzi tę płatność',
      finish_or_cancel_outgoing: 'Zakończ lub anuluj poniższą płatność, zanim wyślesz nową',
      identity_search_unavailable: 'Wyszukiwanie jest teraz niedostępne. Spróbuj za chwilę.',
      abandon_payment: 'Porzuć płatność',
      finish_payment: 'Zakończ płatność',
      cancel_this_payment: 'Anulować tę płatność?',
      cancel_payment: 'Anuluj płatność',
      resend: 'Wyślij ponownie',
      resending: 'Wysyłanie ponownie…',
      send_payment_details_again: 'Wyślij szczegóły ponownie',
      tx_action_resend_short: 'Wyślij ponownie',
      tx_action_request_confirmation: 'Poproś odbiorcę o potwierdzenie',
      resend_sent: 'Szczegóły płatności wysłane ponownie',
      resend_failed_no_record: 'Na tym urządzeniu nie ma szczegółów tej płatności, więc nie można jej wysłać ponownie.',
      resend_failed_no_recipient: 'Ta płatność nie zawiera odbiorcy, więc nie można jej wysłać ponownie.',
      resend_failed_no_transaction:
        'Nie udało się pobrać danych tej płatności. Spróbuj ponownie po połączeniu z siecią.',
      resend_requested: 'Poproszono Cię o ponowne wysłanie płatności',
      request_again: 'Poproś ponownie',
      send_again: 'Wyślij ponownie',
      copy_details: 'Kopiuj szczegóły',
      payment_bounced_resend: 'Poproszono o ponowne wysłanie tej płatności',
      dismiss_rejected_payment: 'Odrzucona płatność została zamknięta',
      activity_load_failed: 'Nie udało się wczytać aktywności',
      activity_load_retry: 'Dotknij, aby spróbować ponownie',
      activity_view_contact: 'Zobacz kontakt',
      home_payments_need_attention: 'Płatności wymagające uwagi: {{count}}',
      home_payments_unsent: 'Niedostarczone płatności: {{count}}',
      pay_copy: 'Kopiuj',
      pay_share_link: 'Udostępnij link',
      pay_address_watching: 'Pieniądze wysłane na ten adres są automatycznie dodawane do Twojego portfela.',
      pay_address_earlier_day: 'Wróć do wcześniejszego dnia',
      pay_address_sweep_now: 'Sprawdź teraz ten adres BSV',
      pay_address_swept: 'Otrzymano {{amount}}',

      check_wallet: 'Rozwiązywanie problemów',
      settings_version: 'v{{version}}',
      error_review_actions: 'Ta płatność utknęła. Rozwiązywanie problemów może spróbować ją uwolnić.',
      wallet_check_ok: 'Wszystko wygląda dobrze',
      wallet_check_attention: 'Niektóre sprawdzenia wymagają uwagi',
      wallet_check_couldnt: 'Nie udało się sprawdzić',
      wallet_check_couldnt_body:
        'Krok naprawczy nie został ukończony. Nie przyjęto żadnych założeń co do Twoich monet.',
      wallet_check_summary: 'Uwolnione zablokowane monety: {{freed}} · Odzyskane płatności: {{recovered}}',
      wallet_check_step_coins: 'Twoje monety są prawidłowe',
      wallet_check_skip: 'Pomiń to sprawdzenie',
      wallet_check_skipped: 'Pominięto',
      wallet_check_step_proofs: 'Dowody transakcji',
      wallet_check_step_records: 'Zapisy lokalnej bazy danych',
      wallet_check_step_tokens: 'Przelewy stablecoin zweryfikowane',
      wallet_check_step_online: 'Połączenie z siecią',
      wallet_check_step_missed: 'Pominięte płatności',
      wallet_check_step_backup: 'Kopia prywatna wysłana',
      wallet_check_step_phrase: 'Fraza odzyskiwania zapisana',
      wallet_check_progress: '{{current}} z {{total}}',
      wallet_check_stuck_title: 'Nie udało się wysłać tej płatności',
      wallet_check_stuck_message: 'Niektóre monety wyglądają na zablokowane. Rozwiązywanie problemów może je uwolnić.',
      wallet_check_failed: 'Nie udało się dokończyć sprawdzania. Spróbuj ponownie.',

      // Local payments
      local_pay_request: 'Poproś o płatność',
      local_pay_send: 'Wyślij płatność',
      local_pay_amount: 'Kwota',
      local_pay_show_qr: 'Pokaż ten kod osobie płacącej',
      local_pay_scan_qr: 'Zeskanuj kod odbiorcy',
      local_pay_scan_payer_qr: 'Zeskanuj kod osoby płacącej',
      local_pay_scan_progress: '{{have}} z {{total}} części',
      local_pay_show_payment_qr: 'Pokaż ten kod odbiorcy',
      local_pay_animated_hint:
        'Poproś o zeskanowanie animowanego kodu poniżej — będzie się zmieniał, aż telefon odbierze całość.',
      local_pay_show_qr_instead: 'Pokaż zamiast tego kod QR',
      local_pay_preparing: 'Przygotowywanie prośby…',
      local_pay_delivering: 'Wysyłanie płatności…',
      local_pay_saving: 'Zapisywanie płatności…',
      local_pay_sent: 'Płatność wysłana',
      local_pay_received: 'Płatność odebrana',
      local_pay_added: 'Dodano do Twojego portfela.',
      local_pay_added_multiple: 'Dodano {{count}} płatności do Twojego portfela.',
      local_pay_received_confirming: 'Potwierdzanie w portfelu…',
      local_pay_received_not_credited:
        'Jeszcze nie dodano do Twojego portfela. Sprawdź Aktywność, zanim coś przekażesz.',
      local_pay_decline_broadcast_title: 'Odrzucona płatność jednak przeszła',
      local_pay_decline_broadcast_body:
        'Płatność, która została odrzucona, pojawiła się od tego czasu w sieci. Nie wysyłaj jej ponownie — sprawdź Aktywność.',
      local_pay_pending_abort_dropped_title: 'Zignorowano niezweryfikowaną prośbę o zwolnienie',
      local_pay_pending_abort_dropped_body:
        'Nie udało się zweryfikować zapisanego rekordu zwolnienia płatności, więc nic nie zostało zwolnione. Jeśli płatność nadal wygląda na zablokowaną, anuluj ją w Aktywności.',
      local_pay_cancel_unverifiable_title: 'Nie można sprawdzić tej płatności offline',
      local_pay_cancel_unverifiable_body:
        'Jeśli druga osoba już to zeskanowała, anulowanie pozwoli wydać te monety dwukrotnie. Anuluj tylko wtedy, gdy jesteś pewien, że nigdy nie zostało zeskanowane.',
      local_pay_cancel_unverifiable_confirm: 'Anuluj mimo to',
      local_pay_queued: 'Zapisano — zostanie automatycznie dodane do portfela.',
      local_pay_already_paid: 'Ta prośba została już opłacona.',
      local_pay_failed: 'Płatność nie powiodła się',
      local_pay_network_denied:
        'Dostęp do sieci lokalnej jest wyłączony. Włącz go w Ustawieniach, aby płacić urządzeniom w pobliżu.',
      local_pay_radio_fallback: 'Połączenie bezprzewodowe niedostępne — pokaż ten kod drugiemu urządzeniu.',
      local_pay_floor_peer_bt_off: 'Ich Bluetooth jest wyłączony, więc ta płatność przejdzie przez kod.',
      local_pay_floor_local_bt_off:
        'Twój Bluetooth jest wyłączony. Włącz go, aby uzyskać szybsze połączenie w pobliżu.',
      local_pay_floor_local_ble_denied:
        'Dostęp do Bluetooth dla BSV Wallet jest wyłączony. Zezwól na niego w Ustawieniach, aby uzyskać szybsze połączenie w pobliżu.',
      local_pay_floor_cross_os:
        'Drugie urządzenie nie może teraz używać Bluetooth, więc ta płatność przejdzie przez kod.',
      local_pay_floor_peer_no_radio:
        'Drugie urządzenie nie oczekuje połączenia bezprzewodowego, więc ta płatność przejdzie przez kod.',
      local_pay_ble_unavailable: 'Bluetooth jest niedostępny. Kod powyżej nadal działa.',
      local_pay_too_large: 'Dane tej płatności są zbyt duże, aby przekazać je za pomocą kodów QR.',
      local_pay_nearby_unavailable: 'Wi-Fi w pobliżu jest niedostępne. Kod powyżej nadal działa.',
      local_pay_retry_save: 'Spróbuj zapisać ponownie',
      local_pay_wrong_session:
        'Ta płatność nie pasuje do tego żądania i nie została przyjęta. Poproś płatnika o ponowne zeskanowanie kodu.',
      local_pay_declined_mismatch:
        'Odbiorca nie przyjął tej płatności — nie pasuje do jego żądania. Zeskanuj jego kod ponownie.',
      local_pay_declined_already_paid: 'To żądanie zostało już opłacone. Nic nie wysłano.',
      local_pay_declined_save: 'Odbiorca nie mógł zapisać płatności. Nic nie wysłano — spróbuj ponownie.',
      local_pay_declined_decode: 'Odbiorca nie mógł odczytać płatności. Nic nie wysłano — spróbuj ponownie.',
      local_pay_declined_not_covered:
        'Odbiorca nie mógł zweryfikować tej płatności w rejestrach emitenta. Nic nie wysłano.',
      local_pay_broadcast_pending: 'Wysłano. To urządzenie nie zdołało jeszcze tego przetworzyć — zrobi to odbiorca.',
      local_pay_any_amount: 'Dowolna kwota',
      local_pay_choose_amount: 'Wybierz kwotę',
      local_pay_enter_amount_send: 'Jaką kwotę chcesz wysłać?',
      local_pay_qr_unavailable: 'Nie udało się utworzyć tego kodu. Rozpocznij prośbę od nowa.',
      local_pay_presence_qr: 'Przez kod — brak połączenia na żywo',
      local_pay_presence_ready: 'Szyfrowane połączenie w pobliżu gotowe',
      local_pay_presence_waiting_payee: 'Czekam na urządzenie w pobliżu',
      local_pay_presence_waiting_payer: 'Szukam urządzenia odbiorcy…',
      local_pay_presence_linked: 'Szyfrowane połączenie otwarte',
      local_pay_presence_paid: 'Płatność potwierdzona',
      pay_offline_title: 'Jesteś offline',
      pay_offline_body:
        'Teraz działają tylko płatności w pobliżu. Zostaną przetworzone, gdy tylko połączysz się ponownie.',
      pay_offline_queued: '{{count}} płatności czeka na przetworzenie, gdy tylko połączysz się ponownie.',
      pay_offline_pending_title: 'Oczekuje na przetworzenie',
      pay_offline_pending_body:
        '{{count}} płatności jeszcze nie zostało przetworzonych. Nic nie jest rozliczone, dopóki się to nie stanie; próby będą ponawiane.',
      pay_offline_needs_internet: 'Wymaga internetu',
      pay_offline_rejected_title: 'Płatność została odrzucona',
      pay_offline_rejected_body:
        'Sieć odrzuciła płatność przekazaną przez {{sender}} za pośrednictwem {{via}} w dniu {{when}}.',
      pay_offline_unknown_sender: 'nieznany nadawca',
      pay_offline_unknown_via: 'nieznanej metody',
      pay_received_not_broadcast: 'Odebrano offline · jeszcze nie przetworzono',
      pay_offline_sent_rejected_title: 'Nie udało się dostarczyć płatności',
      pay_offline_sent_rejected_body: 'Sieć odrzuciła płatność wysłaną przez Ciebie w dniu {{when}}.',
      pay_offline_send_now: 'Wyślij teraz',
      pay_offline_stalled_body: 'Niektórych oczekujących płatności nie można wysłać automatycznie: {{detail}}',
      offline_stall_no_request: 'Płatności w kolejce brakuje zapisu wysyłki.',
      offline_stall_bad_beef: 'Nie udało się odczytać danych transakcji płatności w kolejce.',
      offline_stall_foreign_ancestor:
        'Płatność w kolejce czeka na transakcję nadrzędną, której inny portfel nigdy nie przetworzył.',
      pay_offline_kv_pending: 'Płatności w pobliżu czekające na dodanie do tego portfela: {{count}}.',
      pay_offline_kv_stuck: 'Nie udało się dodać {{count}} płatności w pobliżu. Poproś nadawcę o ponowne wysłanie.',
      pay_offline_kv_corrupt: 'Na tym urządzeniu znaleziono uszkodzone dane płatności.',
      pay_offline_kv_corrupt_body:
        'Objęte tym płatności zostały odłożone na tym urządzeniu, a nie usunięte. Część z nich mogła już trafić do Twojego salda, więc poproś nadawcę o ponowne wysłanie tylko wtedy, gdy jego płatność nadal nie dotarła.',
      pay_offline_show_code: 'Pokaż kod ponownie',
      pay_parked_cancel: 'Anuluj płatność',
      pay_parked_already_sent: 'Ta płatność została już wysłana',
      token_cancel_already_sent: 'Ta płatność już przeszła i nie można jej anulować.',
      tx_status_parked: 'Nie przekazano',
      tx_status_import_hold: 'Wstrzymane (zaimportowane)',
      resend_sending: 'Wysyłanie przez skrzynkę wiadomości',
      cancelling_payment: 'Anulowanie płatności',

      contacts: 'Kontakty',
      contacts_search_placeholder: 'Szukaj po nazwie lub pseudonimie',
      contacts_empty_title: 'Brak kontaktów',
      contacts_empty_body:
        'Osoby, którym płacisz, pojawią się tutaj po zapisaniu. Zeskanuj kod profilu danej osoby przyciskiem w prawym górnym rogu.',
      contacts_recent: 'Ostatnie',
      contact_edit_name: 'Edytuj nazwę',
      contact_save_name: 'Zapisz nazwę',
      contact_activity: 'Aktywność',
      contact_activity_empty: 'Brak aktywności z tym kontaktem',
      contact_delete: 'Usuń kontakt',
      contact_delete_body:
        'Spowoduje to usunięcie {{name}} z Twoich kontaktów. Nie wpływa to na żadne wcześniejsze płatności.',
      contact_handle_caption: 'Ich zarejestrowany pseudonim · tylko oni mogą go zmienić',
      contact_new_title: 'Nowy kontakt',
      contact_save: 'Zapisz',
      contact_identifier: 'Identyfikator',
      contact_saved: 'Kontakt zapisany',
      contact_scan_hint: 'Zeskanuj kod profilu, aby kogoś dodać',
      contact_identifier_placeholder: 'Wklej jego identyfikator',
      contact_identifier_invalid: 'To nie wygląda na prawidłowy identyfikator',
      contact_add_from_payment: 'Dodaj do kontaktów',

      profile: 'Profil',
      profile_display_name: 'Nazwa wyświetlana',
      profile_display_name_hint: 'Widoczna publicznie obok Twojego pseudonimu. Zobaczy ją każdy, kto Cię wyszuka.',
      profile_handle: 'Pseudonim',
      profile_handle_placeholder: 'twojanazwa',
      profile_handle_checking: 'Sprawdzanie dostępności…',
      profile_handle_available: '{{handle}} jest dostępny',
      profile_handle_taken: '{{handle}} jest już zajęty',
      profile_handle_invalid:
        '3-32 znaki: a-z, 0-9, kropka, podkreślenie lub myślnik; zaczyna się i kończy literą lub cyfrą',
      profile_handle_failed: 'Nie udało się sprawdzić dostępności. Spróbuj ponownie.',
      profile_handle_unavailable: 'Rejestracja pseudonimu nie jest jeszcze dostępna w tej wersji.',
      profile_handle_registered: 'Pseudonim zarejestrowany',
      profile_handle_register_action: 'Zarejestruj',
      profile_handle_too_similar: '{{handle}} jest zbyt podobny do już używanego',
      profile_handle_reserved: '{{handle}} jest zarezerwowany',
      profile_handle_cooldown: '{{handle}} został niedawno zwolniony i nie jest jeszcze wolny',
      profile_handle_pending: 'Kończenie rejestracji…',
      profile_handle_rolled_back: '{{handle}} został zajęty. Zachowujesz {{previous}}.',
      profile_handle_changed: 'Pseudonim zmieniony',
      profile_handle_rejected: 'Rejestr odrzucił ten pseudonim.',
      profile_handle_clock_ahead: 'Zegar urządzenia jest zbyt przyspieszony. Spróbuj ponownie za kilka minut.',
      profile_handle_wrong_registry:
        'Tego pseudonimu nie ma w tym rejestrze. Otwórz Profil ponownie, aby go odświeżyć.',
      profile_display_name_publish_failed: 'Twoja nazwa została zapisana tutaj, ale rejestr jej nie opublikował.',
      profile_qr_hint: 'Inni mogą to zeskanować, aby dodać Cię jako kontakt',

      pay_step_contacts: 'Kontakty',
      pay_step_continue: 'Dalej',
      pay_review_to: 'Do',
      pay_review_amount: 'Kwota',
      pay_review_note: 'Notatka',
      pay_review_note_edit: 'Dodaj notatkę',
      pay_review_delivery_host: 'Dostarczanie do',
      pay_trust_contact: 'Twój kontakt',
      pay_trust_handle_attested: 'Zarejestrowany',
      pay_trust_handle_domain_attested: 'Zweryfikowane przez {{domain}}',
      pay_trust_unverified: 'Niezweryfikowane. Dokładnie sprawdź ten identyfikator.',
      pay_address_network_mismatch: 'Ten adres należy do innej sieci niż ta, z której obecnie korzystasz.',
      contacts_your_contacts: 'Twoje kontakty',
      contacts_other_people: 'Inne osoby',
      contacts_not_saved: 'Niezapisany',
      contacts_network_footer: 'Znaleziono w sieci. Nie zapisano przez Ciebie.',
      contact_no_handle: 'Brak pseudonimu',
      contact_no_handle_registered: 'Brak zarejestrowanego pseudonimu',
      contact_name_section: 'Twoja nazwa dla tej osoby',
      contact_name_hint: 'Widzisz to tylko Ty. Nie zmienia tego, jak ta osoba się nazywa.',
      contact_new_name_hint: 'Widzisz to tylko Ty. Zapisane na tym urządzeniu.',
      contact_copy_identifier: 'Kopiuj identyfikator',
      profile_handle_registered_hint: 'Zarejestrowany na Twój identyfikator. Inni mogą Cię po nim znaleźć.',
      profile_handle_change: 'Zmień',
      profile_handle_claim: 'Zajmij {{handle}}',
      profile_handle_replace_warning:
        'Zajęcie zastąpi {{handle}}. Osoby, które Cię zapisały, zachowają kontakt; stary pseudonim przestanie prowadzić do Ciebie.',
      profile_identifier_section: 'Twój identyfikator',
      profile_show_qr: 'Pokaż jako QR',
      profile_show_qr_hint: 'Pozwól komuś Cię zeskanować, aby dostał Twój profil',
      profile_qr_callout:
        'Zeskanowanie otwiera BSV Wallet i dodaje Cię jako kontakt — z imieniem, pseudonimem i zdjęciem. Nie prosi o płatność; ten kod jest w sekcji Otrzymaj.',
      pay_review_title: 'Podsumowanie',
      pay_send_amount: 'Wyślij {{amount}}',
      pay_sent_to: 'do {{name}}',
      pay_save_as_contact: 'Zapisz jako kontakt'
    }
  }
}

// Define supported languages
const supportedLanguages = ['en', 'es', 'zh', 'hi', 'fr', 'ar', 'pt', 'bn', 'ru', 'id', 'ja', 'pl']

// Validate and ensure we use a supported language
if (!supportedLanguages.includes(detectedLanguage)) {
  console.warn(`⚠️ Detected language "${detectedLanguage}" is not supported. Falling back to English.`)
  detectedLanguage = 'en'
}

console.log('🌍 Final language to use:', detectedLanguage)
console.log('📋 Supported languages:', supportedLanguages)

i18n
  .use(initReactI18next)
  .init({
    resources,
    lng: detectedLanguage, // Use the validated detected language
    fallbackLng: 'en',
    interpolation: {
      escapeValue: false
    }
  })
  .then(() => {
    console.log('✅ i18n initialized successfully')
    console.log('🌐 Current language set to:', i18n.language)
    console.log('📋 Available languages:', Object.keys(resources))
    console.log('🎯 Fallback language:', i18n.options.fallbackLng)

    // Test basic translation functionality
    const testKey = 'new_tab'
    const translation = i18n.t(testKey)
    console.log(`🧪 Test translation for "${testKey}":`, translation)

    if (translation === testKey) {
      console.warn('⚠️ Translation not working - returned key instead of translated text')
    } else {
      console.log('✅ Basic translation test passed')
    }
  })
  .catch(error => {
    console.error('❌ i18n initialization failed:', error)
  })

interface LanguageContextType {
  currentLanguage: string
  setCurrentLanguage: (language: string) => void
}

const LanguageContext = createContext<LanguageContextType>({
  currentLanguage: 'en',
  setCurrentLanguage: () => {}
})

interface LanguageProviderProps {
  children: ReactNode
}

export const LanguageProvider: React.FC<LanguageProviderProps> = ({ children }) => {
  const [currentLanguage, setCurrentLanguage] = useState(i18n.language)

  console.log('🔄 LanguageProvider initialized with language:', currentLanguage)

  const handleLanguageChange = (language: string) => {
    console.log('🔄 Language changing from', currentLanguage, 'to', language)
    setCurrentLanguage(language)
    i18n
      .changeLanguage(language)
      .then(() => {
        console.log('✅ Language successfully changed to:', i18n.language)
      })
      .catch(error => {
        console.error('❌ Failed to change language:', error)
      })
  }

  return (
    <LanguageContext.Provider value={{ currentLanguage, setCurrentLanguage: handleLanguageChange }}>
      {children}
    </LanguageContext.Provider>
  )
}

export const useLanguage = (): LanguageContextType => useContext(LanguageContext)

export type TranslationKey = keyof typeof resources.en.translation

// Exported so tests can assert every language carries the same key set as English.
export { resources }

export default i18n

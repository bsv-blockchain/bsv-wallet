...
const ALL_CODES: VaultErrorCode[] = [
  ...
  'key-not-adopted', 'too-many-keys', 'too-many-active-keys', 'last-keys', 'relock-required', 'action-pending', 'key-not-committed', 'key-cannot-cover',
...
describe('vaultErrorCopy', () => {
  ...
  test('plain codes use the code-derived key (aliases from the old ERROR_COPY are gone)', () => {
    ...
    // Distinct from not-released on purpose: vault_err_not_released says the
    // BUILD is off, which is false for a released build on testnet and hides
    // the one thing the user can act on.
    expect(vaultErrorCopy('not-on-mainnet')).toBe('vault_err_not_on_mainnet')
    expect(vaultErrorCopy('not-on-mainnet')).not.toBe(vaultErrorCopy('not-released'))
